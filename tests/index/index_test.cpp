...
void SplitInsertTest(index::Index *index, VarlenPool *pool,
                     size_t scale_factor) {
  // Loop based on scale factor
  for (size_t scale_itr = 1; scale_itr <= scale_factor; scale_itr++) {
    // Insert a bunch of keys based on scale itr
    ...
    key5->SetValue(0, ValueFactory::GetIntegerValue(500 * scale_itr), pool);
    key5->SetValue(1, ValueFactory::GetStringValue("e"), pool);
...
TEST(IndexTests, SplitConsolidateTest) {
  auto pool = TestingHarness::GetInstance().GetTestingPool();
  std::vector<ItemPointer> locations;
  ...
  // Single threaded test
  size_t scale_factor = 1;
  LaunchParallelTest(1, SplitInsertTest, index.get(), pool, scale_factor);
  LaunchParallelTest(1, DeleteTest, index.get(), pool, scale_factor);
...
void SplitInsertIndexDeltaTest(index::Index *index, VarlenPool *pool,
                               size_t scale_factor) {
  // Loop based on scale factor
  for (size_t scale_itr = 1; scale_itr <= scale_factor; scale_itr++) {
    // Insert a bunch of keys based on scale itr
    ...
    key6->SetValue(0, ValueFactory::GetIntegerValue(600 * scale_itr), pool);
    key6->SetValue(1, ValueFactory::GetStringValue("f"), pool);

    key7->SetValue(0, ValueFactory::GetIntegerValue(700 * scale_itr), pool);
    key7->SetValue(1, ValueFactory::GetStringValue("h"), pool);

    keynonce->SetValue(0, ValueFactory::GetIntegerValue(6000 * scale_itr),
                       pool);
    ...
    index->InsertEntry(key6.get(), item0);
    index->InsertEntry(key6.get(), item2);

    index->InsertEntry(key7.get(), item0);
    index->InsertEntry(key7.get(), item0);
    index->InsertEntry(key7.get(), item1);
  }
}

TEST(IndexTests, IndexDeltaTest) {
  auto pool = TestingHarness::GetInstance().GetTestingPool();
  std::vector<ItemPointer> locations;
  ...
  // Single threaded test
  size_t scale_factor = 1;
  LaunchParallelTest(1, SplitInsertIndexDeltaTest, index.get(), pool,
                     scale_factor);
  LaunchParallelTest(1, DeleteTest, index.get(), pool, scale_factor);
...
TEST(IndexTests, MultiThreadedIndexDeltaTest) {
  auto pool = TestingHarness::GetInstance().GetTestingPool();
  std::vector<ItemPointer> locations;
  ...
  size_t scale_factor = 1;
  LaunchParallelTest(num_threads, SplitInsertIndexDeltaTest, index.get(), pool,
                     scale_factor);

  locations = index->ScanAllKeys();
  EXPECT_EQ(locations.size(), 16 * num_threads);
...
// INSERT HELPER FUNCTION
//  void InsertDeleteTest(index::Index *index, VarlenPool *pool, size_t
//  scale_factor){
//
//    // Loop based on scale factor
//    for(size_t scale_itr = 1; scale_itr <= scale_factor; scale_itr++) {
//      // Insert a bunch of keys based on scale itr
//      std::unique_ptr<storage::Tuple> key0(new storage::Tuple(key_schema,
//      true));
//      std::unique_ptr<storage::Tuple> key1(new storage::Tuple(key_schema,
//      true));
//      std::unique_ptr<storage::Tuple> key2(new storage::Tuple(key_schema,
//      true));
//      std::unique_ptr<storage::Tuple> key3(new storage::Tuple(key_schema,
//      true));
//      std::unique_ptr<storage::Tuple> key4(new storage::Tuple(key_schema,
//      true));
