...
bool PacketManager::process_startup_packet(Packet *pkt) {
  ...
  if (!packet_endmessage(pkt, &client)) return false;
    ...
    case 'Q': {
      ...
      for (auto &query : queries) {
        if (query.empty()) {
          resp.reset();
          resp.msg_type = 'I';
          if (!packet_endmessage(&resp, &client)) return false;
          if (!send_ready_for_query(TXN_IDLE)) return false;
        }

        if (!put_dummy_row_desc()) return false;
        ...
        if (!complete_command(5)) return false;
      }
