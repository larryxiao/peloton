...
CreateRestrictedProcess(char *cmd, PROCESS_INFORMATION *processInfo,
  ...
  if (Advapi32Handle != NULL) {
    _CreateRestrictedToken = (__CreateRestrictedToken)GetProcAddress(
        Advapi32Handle, "CreateRestrictedToken");
  }

  if (_CreateRestrictedToken == NULL) {
    fprintf(
        stderr,
        _("%s: WARNING: cannot create restricted tokens on this platform\n"),
        progname);
    if (Advapi32Handle != NULL) FreeLibrary(Advapi32Handle);
    return 0;
  }
  ...
  if (!b) {
    fprintf(stderr,
            _("%s: could not create restricted token: error code %lu\n"),
            progname, GetLastError());
    return 0;
  }
  ...
  if (!CreateProcessAsUser(restrictedToken, NULL, cmd, NULL, NULL, TRUE,
...
void get_restricted_token(const char *progname) {
  ...
  if ((restrict_env = getenv("PG_RESTRICT_EXEC")) == NULL ||
    ...
    if ((restrictedToken = CreateRestrictedProcess(cmdline, &pi, progname)) ==
        0) {
      fprintf(
          stderr,
