...
#define log_error4(str, param, arg1) \
  (fprintf(stderr, str, param, arg1), fputc('\n', stderr))
#endif
...
int find_my_exec(const char *argv0, char *retpath) {
  ...
  if (!getcwd(cwd, MAXPGPATH)) {
    log_error(_("could not identify current directory: %s"), strerror(errno));
    return -1;
  }
  ...
  if (first_dir_separator(argv0) != NULL) {
    ...
    if (validate_exec(retpath) == 0) return resolve_symlinks(retpath);
  ...
  if ((path = getenv("PATH")) && *path) {
    ...
    do {
      ...
      if (!endp) endp = startp + strlen(startp); /* point to end */
...
static int resolve_symlinks(char *path) {
  ...
  for (;;) {
    ...
    if (lstat(fname, &buf) < 0 || !S_ISLNK(buf.st_mode)) break;
  ...
  if (chdir(orig_wd) == -1) {
    log_error4(_("could not change directory to \"%s\": %s"), orig_wd,
               strerror(errno));
    return -1;
  }
#endif /* HAVE_READLINK */
...
int find_other_exec(const char *argv0, const char *target,
  ...
  if (find_my_exec(argv0, retpath) < 0) return -1;
  ...
  *last_dir_separator(retpath) = '\0';
  ...
  snprintf(retpath + strlen(retpath), MAXPGPATH - strlen(retpath), "/%s%s",
           target, EXE);

  if (validate_exec(retpath) != 0) return -1;

  snprintf(cmd, sizeof(cmd), "\"%s\" -V", retpath);

  if (!pipe_read_line(cmd, line, sizeof(line))) return -1;
...
static char *pipe_read_line(char *cmd, char *line, int maxsize) {
  ...
  if (pclose_check(pgver)) return NULL;
  ...
  if (!CreatePipe(&childstdoutrd, &childstdoutwr, &sattr, 0)) return NULL;

  if (!DuplicateHandle(GetCurrentProcess(), childstdoutrd, GetCurrentProcess(),
                       &childstdoutrddup, 0, FALSE, DUPLICATE_SAME_ACCESS)) {
...
int pclose_check(FILE *stream) {
  ...
  if (exitstatus == 0) return 0; /* all is well */
...
void set_pglocale_pgservice(const char *argv0, const char *app) {
  ...
  if (find_my_exec(argv0, my_exec_path) < 0) return;
...
BOOL AddUserToTokenDacl(HANDLE hToken) {
  ...
  if (!GetTokenInformation(hToken, tic, (LPVOID)NULL, dwTokenInfoLength,
                           &dwSize)) {
    if (GetLastError() == ERROR_INSUFFICIENT_BUFFER) {
      ...
      if (!GetTokenInformation(hToken, tic, (LPVOID)ptdd, dwSize, &dwSize)) {
        log_error("could not get token information: error code %lu",
                  GetLastError());
        goto cleanup;
      }
    } else {
      log_error("could not get token information buffer size: error code %lu",
                GetLastError());
      goto cleanup;
    }
  }
  ...
  if (!AddAccessAllowedAceEx(pacl, ACL_REVISION, OBJECT_INHERIT_ACE,
    ...
    log_error("could not add access allowed ACE: error code %lu",
              GetLastError());
    goto cleanup;
  }
  ...
  if (!SetTokenInformation(hToken, tic, (LPVOID)&tddNew, dwNewAclSize)) {
    log_error("could not set token information: error code %lu",
              GetLastError());
    goto cleanup;
  }
...
cleanup:
  if (pTokenUser) LocalFree((HLOCAL)pTokenUser);

  if (pacl) LocalFree((HLOCAL)pacl);

  if (ptdd) LocalFree((HLOCAL)ptdd);
...
static BOOL GetTokenUser(HANDLE hToken, PTOKEN_USER *ppTokenUser) {
  ...
  *ppTokenUser = NULL;
  ...
  if (!GetTokenInformation(hToken, TokenUser, *ppTokenUser, dwLength,
                           &dwLength)) {
