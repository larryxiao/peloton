...
char* wait_result_to_str(int exitstatus) {
  ...
  if (WIFEXITED(exitstatus)) {
    ...
    switch (WEXITSTATUS(exitstatus)) {
      ...
      default:
        snprintf(str, sizeof(str), _("child process exited with exit code %d"),
...
#elif defined(HAVE_DECL_SYS_SIGLIST) && HAVE_DECL_SYS_SIGLIST
  {
    ...
    snprintf(str2, sizeof(str2), "%d: %s", WTERMSIG(exitstatus),
             WTERMSIG(exitstatus) < NSIG ? sys_siglist[WTERMSIG(exitstatus)]
                                         : "(unknown)");
    snprintf(str, sizeof(str), _("child process was terminated by signal %s"),
             str2);
  }
#else
    snprintf(str, sizeof(str), _("child process was terminated by signal %d"),
             WTERMSIG(exitstatus));
#endif
  else
    snprintf(str, sizeof(str),
