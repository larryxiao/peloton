...
Size BackgroundWorkerShmemSize(void) {
  ...
  size = add_size(size,
                  mul_size(max_worker_processes, sizeof(BackgroundWorkerSlot)));
...
void BackgroundWorkerShmemInit(void) {
  ...
  BackgroundWorkerData = static_cast<BackgroundWorkerArray *>(ShmemInitStruct(
      "Background Worker Data", BackgroundWorkerShmemSize(), &found));
...
static RegisteredBgWorker *FindRegisteredWorkerBySlotNumber(int slotno) {
  ...
  slist_foreach(siter, &BackgroundWorkerList) {
    ...
    rw = slist_container(RegisteredBgWorker, rw_lnode, siter.cur);
    if (rw->rw_shmem_slot == slotno) return rw;
  }
...
void BackgroundWorkerStateChange(void) {
  ...
  for (slotno = 0; slotno < max_worker_processes; ++slotno) {
    ...
    if (slot->terminate) {
      ...
      slot->pid = 0;
      slot->in_use = false;
      if (notify_pid != 0) kill(notify_pid, SIGUSR1);
     ...
     */
    ascii_safe_strlcpy(rw->rw_worker.bgw_name, slot->worker.bgw_name,
                       BGW_MAXLEN);
    ...
    /* Log it! */
    ereport(LOG, (errmsg("registering background worker \"%s\"",
...
void ForgetBackgroundWorker(slist_mutable_iter *cur) {
  ...
  ereport(LOG, (errmsg("unregistering background worker \"%s\"",
                       rw->rw_worker.bgw_name)));
...
void BackgroundWorkerStopNotifications(pid_t pid) {
  ...
  slist_foreach(siter, &BackgroundWorkerList) {
    ...
    rw = slist_container(RegisteredBgWorker, rw_lnode, siter.cur);
    if (rw->rw_worker.bgw_notify_pid == pid) rw->rw_worker.bgw_notify_pid = 0;
  }
}
 ...
 * shared memory.
 */
BackgroundWorker *BackgroundWorkerEntry(int slotno) {
  ...
  if (worker->bgw_flags & BGWORKER_BACKEND_DATABASE_CONNECTION) {
    if (!(worker->bgw_flags & BGWORKER_SHMEM_ACCESS)) {
      ereport(elevel, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                       errmsg(
                           "background worker \"%s\": must attach to shared "
                           "memory in order to request a database connection",
                           worker->bgw_name)));
      return false;
    }
    ...
    if (worker->bgw_start_time == BgWorkerStart_PostmasterStart) {
      ereport(elevel, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                       errmsg(
                           "background worker \"%s\": cannot request database "
                           "access if starting at postmaster start",
                           worker->bgw_name)));
      return false;
    }
...
void StartBackgroundWorker(void) {
  ...
  if (worker == NULL) elog(FATAL, "unable to find bgworker entry");
  ...
  /* Apply PostAuthDelay */
  if (PostAuthDelay > 0) pg_usleep(PostAuthDelay * 1000000L);
 ...
 */
  ...
  if (worker->bgw_main != NULL)
    ...
    entrypt = (bgworker_main_type)load_external_function(
        worker->bgw_library_name, worker->bgw_function_name, 1, NULL);
...
/*
 * Register a new___ background worker while processing
...
void RegisterBackgroundWorker(BackgroundWorker *worker) {
  ...
  if (!process_shared_preload_libraries_in_progress) {
    if (!IsUnderPostmaster)
      ereport(LOG, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                    errmsg(
                        "background worker \"%s\": must be registered in "
                        "shared_preload_libraries",
                        worker->bgw_name)));
    return;
  }

  if (!SanityCheckBackgroundWorker(worker, LOG)) return;

  if (worker->bgw_notify_pid != 0) {
    ereport(LOG, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                  errmsg(
                      "background worker \"%s\": only dynamic background "
                      "workers can request notification",
                      worker->bgw_name)));
    return;
  }
  ...
  if (++numworkers > max_worker_processes) {
    ereport(LOG, (errcode(ERRCODE_CONFIGURATION_LIMIT_EXCEEDED),
                  ...
                  errdetail_plural(
                      "Up to %d background worker can be registered with the "
                      "current settings.",
                      "Up to %d background workers can be registered with the "
                      "current settings.",
...
bool RegisterDynamicBackgroundWorker(BackgroundWorker *worker,
   ...
   * structure.
   */
  if (!IsUnderPostmaster) return false;

  if (!SanityCheckBackgroundWorker(worker, ERROR)) return false;
  ...
  if (success) SendPostmasterSignal(PMSIGNAL_BACKGROUND_WORKER_CHANGE);

  /*
   * If we found a slot and the user has provided a handle, initialize it.
   */
  if (success && handle) {
    *handle = static_cast<BackgroundWorkerHandle *>(
        palloc(sizeof(BackgroundWorkerHandle)));
    (*handle)->slot = slotno;
...
BgwHandleStatus WaitForBackgroundWorkerStartup(BackgroundWorkerHandle *handle,
  ...
  PG_TRY();
  {
    for (;;) {
      ...
      if (status == BGWH_STARTED) *pidp = pid;
      if (status != BGWH_NOT_YET_STARTED) break;

      rc = WaitLatch(MyLatch, WL_LATCH_SET | WL_POSTMASTER_DEATH, 0);
...
BgwHandleStatus WaitForBackgroundWorkerShutdown(
  ...
  PG_TRY();
  {
    for (;;) {
      ...
      status = GetBackgroundWorkerPid(handle, &pid);
      if (status == BGWH_STOPPED) return status;
