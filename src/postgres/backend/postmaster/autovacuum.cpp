...
static autovac_table *table_recheck_autovac(
    Oid relid, HTAB *table_toast_map, TupleDesc pg_class_desc,
...
static PgStat_StatTabEntry *get_pgstat_tabentry_relid(
    Oid relid, bool isshared, PgStat_StatDBEntry *shared,
 ...
 */
void AutovacuumLauncherIAm(void) { am_autovacuum_launcher = true; }
#endif
...
NON_EXEC_STATIC void AutoVacLauncherMain(int argc, char *argv[]) {
  ...
  if (PostAuthDelay) pg_usleep(PostAuthDelay * 1000000L);
 ...
 */
  ...
  AutovacMemCxt = AllocSetContextCreate(
      TopMemoryContext, "Autovacuum Launcher", ALLOCSET_DEFAULT_MINSIZE,
   ...
   */
  SetConfigOption("default_transaction_isolation", "read committed", PGC_SUSET,
                  PGC_S_OVERRIDE);
  ...
  if (!AutoVacuumingActive()) {
    if (!got_SIGTERM) do_start_worker();
    proc_exit(0); /* done */
  }
 ...
 * databases in the autovacuum_naptime period.  The new___ database is put at
 *the
...
static void rebuild_database_list(Oid newdb) {
  ...
  newcxt = AllocSetContextCreate(
      AutovacMemCxt, "AV dblist", ALLOCSET_DEFAULT_MINSIZE,
      ALLOCSET_DEFAULT_INITSIZE, ALLOCSET_DEFAULT_MAXSIZE);
  tmpcxt = AllocSetContextCreate(
      newcxt, "tmp AV dblist", ALLOCSET_DEFAULT_MINSIZE,
      ALLOCSET_DEFAULT_INITSIZE, ALLOCSET_DEFAULT_MAXSIZE);
  ...
  if (OidIsValid(newdb)) {
    ...
    if (entry != NULL) {
      ...
      db = static_cast<avl_dbase *>(
          hash_search(dbhash, &newdb, HASH_ENTER, NULL));
  ...
  dlist_foreach(iter, &DatabaseList) {
    ...
    db = static_cast<avl_dbase *>(
        hash_search(dbhash, &(avdb->adl_datid), HASH_ENTER, &found));
  ...
  foreach (cell, dblist) {
    ...
    db = static_cast<avl_dbase *>(
        hash_search(dbhash, &(avdb->adw_datid), HASH_ENTER, &found));
  ...
  if (nelems > 0) {
    ...
    for (i = 0; i < nelems; i++) {
      ...
      current_time =
          TimestampTzPlusMilliseconds(current_time, millis_increment);
  ...
  if (DatabaseListCxt != NULL) MemoryContextDelete(DatabaseListCxt);
...
static int db_comparator(const void *a, const void *b) {
  if (((const avl_dbase *)a)->adl_score == ((const avl_dbase *)b)->adl_score)
    ...
    return (((const avl_dbase *)a)->adl_score <
...
static Oid do_start_worker(void) {
   ...
   */
  tmpcxt = AllocSetContextCreate(
      CurrentMemoryContext, "Start worker tmp cxt", ALLOCSET_DEFAULT_MINSIZE,
      ALLOCSET_DEFAULT_INITSIZE, ALLOCSET_DEFAULT_MAXSIZE);
  oldcxt = MemoryContextSwitchTo(tmpcxt);
  ...
  if (multiForceLimit < FirstMultiXactId) multiForceLimit -= FirstMultiXactId;
  ...
  foreach (cell, dblist) {
    ...
    dlist_reverse_foreach(iter, &DatabaseList) {
      ...
      if (dbp->adl_datid == tmp->adw_datid) {
         ...
         */
        if (!TimestampDifferenceExceeds(dbp->adl_next_worker, current_time,
                                        0) &&
...
static void launch_worker(TimestampTz now) {
  ...
  if (OidIsValid(dbid)) {
     ...
     */
    if (!found) rebuild_database_list(dbid);
  }
}
 ...
 * We need this set from the outside, before InitProcess is called
 */
void AutovacuumWorkerIAm(void) { am_autovacuum_worker = true; }
#endif
...
static void autovac_balance_cost(void) {
   ...
   */
  int vac_cost_limit =
      (autovacuum_vac_cost_limit > 0 ? autovacuum_vac_cost_limit
                                     : VacuumCostLimit);
  int vac_cost_delay =
  ...
  if (vac_cost_limit <= 0 || vac_cost_delay <= 0) return;
  ...
  dlist_foreach(iter, &AutoVacuumShmem->av_runningWorkers) {
    ...
    if (worker->wi_proc != NULL)
      elog(DEBUG2,
           "autovac_balance_cost(pid=%u db=%u, rel=%u, dobalance=%s "
           "cost_limit=%d, cost_limit_base=%d, cost_delay=%d)",
           worker->wi_proc->pid, worker->wi_dboid, worker->wi_tableoid,
           worker->wi_dobalance ? "yes" : "no", worker->wi_cost_limit,
           worker->wi_cost_limit_base, worker->wi_cost_delay);
  }
}
...
static void do_autovacuum(void) {
   ...
   */
  AutovacMemCxt = AllocSetContextCreate(
      TopMemoryContext, "AV worker", ALLOCSET_DEFAULT_MINSIZE,
      ALLOCSET_DEFAULT_INITSIZE, ALLOCSET_DEFAULT_MAXSIZE);
  MemoryContextSwitchTo(AutovacMemCxt);
  ...
  table_toast_map =
      hash_create("TOAST to main relid map", 100, &ctl, HASH_ELEM | HASH_BLOBS);
  ...
  while ((tuple = heap_getnext(relScan, ForwardScanDirection)) != NULL) {
    ...
    tabentry = get_pgstat_tabentry_relid(relid, classForm->relisshared, shared,
                                         dbentry);
    ...
    if (classForm->relpersistence == RELPERSISTENCE_TEMP) {
      ...
      if (backendID == MyBackendId || BackendIdGetProc(backendID) == NULL) {
        ...
        if (wraparound) {
          ...
          ereport(LOG, (errmsg(
                           "autovacuum: found orphan temp table \"%s\".\"%s\" "
                           "in database \"%s\"",
                           get_namespace_name(classForm->relnamespace),
                           NameStr(classForm->relname),
                           get_database_name(MyDatabaseId))));
      ...
      if (dovacuum || doanalyze) table_oids = lappend_oid(table_oids, relid);
      ...
      if (OidIsValid(classForm->reltoastrelid)) {
        ...
        hentry = static_cast<av_relation *>(hash_search(
            table_toast_map, &classForm->reltoastrelid, HASH_ENTER, &found));
  ...
  ScanKeyInit(&key, Anum_pg_class_relkind, BTEqualStrategyNumber, F_CHAREQ,
  ...
  while ((tuple = heap_getnext(relScan, ForwardScanDirection)) != NULL) {
     ...
     */
    if (classForm->relpersistence == RELPERSISTENCE_TEMP) continue;
    ...
    if (relopts == NULL) {
      ...
      hentry = static_cast<av_relation *>(
          hash_search(table_toast_map, &relid, HASH_FIND, &found));
      if (found && hentry->ar_hasrelopts) relopts = &hentry->ar_reloptions;
    }
    ...
    if (dovacuum) table_oids = lappend_oid(table_oids, relid);
  }
  ...
  PortalContext = AllocSetContextCreate(
      AutovacMemCxt, "Autovacuum Portal", ALLOCSET_DEFAULT_INITSIZE,
  ...
  foreach (cell, table_oids) {
    ...
    dlist_foreach(iter, &AutoVacuumShmem->av_runningWorkers) {
      ...
      if (worker->wi_dboid != MyDatabaseId) continue;
    ...
    if (!tab->at_relname || !tab->at_nspname || !tab->at_datname) goto deleted;
    ...
    PG_CATCH();
    {
      ...
      if (tab->at_vacoptions & VACOPT_VACUUM)
        errcontext("automatic vacuum of table \"%s.%s.%s\"", tab->at_datname,
                   tab->at_nspname, tab->at_relname);
      else
        errcontext("automatic analyze of table \"%s.%s.%s\"", tab->at_datname,
                   tab->at_nspname, tab->at_relname);
      EmitErrorReport();
  ...
  /* be tidy */
  deleted:
    if (tab->at_datname != NULL) pfree(tab->at_datname);
    if (tab->at_nspname != NULL) pfree(tab->at_nspname);
    if (tab->at_relname != NULL) pfree(tab->at_relname);
    pfree(tab);
...
static PgStat_StatTabEntry *get_pgstat_tabentry_relid(
  ...
  if (isshared) {
    if (PointerIsValid(shared))
      tabentry = static_cast<PgStat_StatTabEntry *>(
          hash_search(shared->tables, &relid, HASH_FIND, NULL));
  } else if (PointerIsValid(dbentry))
    tabentry = static_cast<PgStat_StatTabEntry *>(
        hash_search(dbentry->tables, &relid, HASH_FIND, NULL));
...
static autovac_table *table_recheck_autovac(
  ...
  if (!HeapTupleIsValid(classTup)) return NULL;
  ...
  if (classForm->relkind == RELKIND_TOASTVALUE && avopts == NULL &&
    ...
    hentry = static_cast<av_relation *>(
        hash_search(table_toast_map, &relid, HASH_FIND, &found));
    if (found && hentry->ar_hasrelopts) avopts = &hentry->ar_reloptions;
  }

  /* fetch the pgstat table entry */
  tabentry =
      get_pgstat_tabentry_relid(relid, classForm->relisshared, shared, dbentry);
  ...
  /* ignore ANALYZE for toast tables */
  if (classForm->relkind == RELKIND_TOASTVALUE) doanalyze = false;
  ...
  if (doanalyze || dovacuum) {
    ...
    multixact_freeze_table_age =
        (avopts && avopts->multixact_freeze_table_age >= 0)
            ? avopts->multixact_freeze_table_age
            : default_multixact_freeze_table_age;

    tab = static_cast<autovac_table *>(palloc(sizeof(autovac_table)));
    tab->at_relid = relid;
    tab->at_vacoptions = VACOPT_SKIPTOAST | (dovacuum ? VACOPT_VACUUM : 0) |
...
static void relation_needs_vacanalyze(Oid relid, AutoVacOpts *relopts,
  ...
  if (xidForceLimit < FirstNormalTransactionId)
    xidForceLimit -= FirstNormalTransactionId;
  force_vacuum =
      (TransactionIdIsNormal(classForm->relfrozenxid) &&
       TransactionIdPrecedes(classForm->relfrozenxid, xidForceLimit));
  if (!force_vacuum) {
    multiForceLimit = recentMulti - multixact_freeze_max_age;
    if (multiForceLimit < FirstMultiXactId) multiForceLimit -= FirstMultiXactId;
    force_vacuum = MultiXactIdPrecedes(classForm->relminmxid, multiForceLimit);
  }
  *wraparound = force_vacuum;
  ...
  if (PointerIsValid(tabentry) && AutoVacuumingActive()) {
    ...
    elog(DEBUG3, "%s: vac: %.0f (threshold %.0f), anl: %.0f (threshold %.0f)",
         NameStr(classForm->relname), vactuples, vacthresh, anltuples,
         anlthresh);
  ...
  if (relid == StatisticRelationId) *doanalyze = false;
}
...
#define MAX_AUTOVAC_ACTIV_LEN (NAMEDATALEN * 2 + 56)
  ...
  if (tab->at_vacoptions & VACOPT_VACUUM)
    snprintf(activity, MAX_AUTOVAC_ACTIV_LEN, "autovacuum: VACUUM%s",
  ...
  snprintf(activity + len, MAX_AUTOVAC_ACTIV_LEN - len, " %s.%s%s",
           tab->at_nspname, tab->at_relname,
...
bool AutoVacuumingActive(void) {
  if (!autovacuum_start_daemon || !pgstat_track_counts) return false;
  return true;
}
 ...
 *		process.
 */
bool IsAutoVacuumLauncherProcess(void) { return am_autovacuum_launcher; }

bool IsAutoVacuumWorkerProcess(void) { return am_autovacuum_worker; }
...
Size AutoVacuumShmemSize(void) {
  ...
  size = MAXALIGN(size);
  size =
      add_size(size, mul_size(autovacuum_max_workers, sizeof(WorkerInfoData)));
  return size;
}
...
void AutoVacuumShmemInit(void) {
  ...
  AutoVacuumShmem = (AutoVacuumShmemStruct *)ShmemInitStruct(
      "AutoVacuum Data", AutoVacuumShmemSize(), &found);
