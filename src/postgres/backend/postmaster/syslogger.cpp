...
#endif
NON_EXEC_STATIC void SysLoggerMain(int argc, char *argv[])
 ...
 */
#ifndef WIN32
  if (syslogPipe[1] >= 0) close(syslogPipe[1]);
  syslogPipe[1] = -1;
#else
  if (syslogPipe[1]) CloseHandle(syslogPipe[1]);
  syslogPipe[1] = 0;
#endif
  ...
  for (;;) {
    ...
    if (Log_RotationAge > 0 && !rotation_disabled) {
      ...
      if (delay > 0) {
        if (delay > INT_MAX / 1000) delay = INT_MAX / 1000;
 ...
 */
    ...
    rc = WaitLatchOrSocket(MyLatch,
                           ...
                           syslogPipe[0], cur_timeout);

    if (rc & WL_SOCKET_READABLE) {
      int bytesRead;

      bytesRead = read(syslogPipe[0], logbuffer + bytes_in_logbuffer,
    ...
    (void)WaitLatch(MyLatch, WL_LATCH_SET | cur_flags, cur_timeout);
...
static void process_pipe_input(char *logbuffer, int *bytes_in_logbuffer) {
  ...
  while (count >= (int)(offsetof(PipeProtoHeader, data) + 1)) {
    ...
    if (p.nuls[0] == '\0' && p.nuls[1] == '\0' && p.len > 0 &&
        p.len <= PIPE_MAX_PAYLOAD && p.pid != 0 &&
        (p.is_last == 't' || p.is_last == 'f' || p.is_last == 'T' ||
         p.is_last == 'F')) {
      List *buffer_list;
      ListCell *cell;
      ...
      dest = (p.is_last == 'T' || p.is_last == 'F') ? LOG_DESTINATION_CSVLOG
      ...
      foreach (cell, buffer_list) {
        ...
        if (buf->pid == 0 && free_slot == NULL) free_slot = buf;
      }
  ...
  if (count > 0 && cursor != logbuffer) memmove(logbuffer, cursor, count);
...
static unsigned int __stdcall pipeThread(void *arg) {
  ...
  for (;;) {
    ...
    result = ReadFile(syslogPipe[0], logbuffer + bytes_in_logbuffer,
                      sizeof(logbuffer) - bytes_in_logbuffer, &bytesRead, 0);
    ...
    if (!result) {
      ...
      if (error == ERROR_HANDLE_EOF || error == ERROR_BROKEN_PIPE) break;
      _dosmaperr(error);
...
static void logfile_rotate(bool time_based_rotation, int size_rotation_for) {
  ...
  if (csvlogFile != NULL) csvfilename = logfile_getname(fntime, ".csv");
  ...
  if (time_based_rotation || (size_rotation_for & LOG_DESTINATION_STDERR)) {
    ...
    if (!fh) {
      ...
      if (csvfilename) pfree(csvfilename);
      return;
    }
    ...
    if (last_file_name != NULL) pfree(last_file_name);
...
static char *logfile_getname(pg_time_t timestamp, const char *suffix) {
  ...
  if (suffix != NULL) {
    len = strlen(filename);
    if (len > 4 && (strcmp(filename + (len - 4), ".log") == 0)) len -= 4;
