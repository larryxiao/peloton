...
void CheckpointerMain(void) {
  ...
  checkpointer_context = AllocSetContextCreate(
      TopMemoryContext, "Checkpointer", ALLOCSET_DEFAULT_MINSIZE,
  ...
  for (;;) {
    ...
    if (elapsed_secs >= CheckPointTimeout) {
      if (!do_checkpoint) BgWriterStats.m_timed_checkpoints++;
      do_checkpoint = true;
    ...
    if (do_checkpoint) {
       ...
       */
      if (flags & CHECKPOINT_END_OF_RECOVERY) do_restartpoint = false;
       ...
       */
      ckpt_active = true;
      if (!do_restartpoint) ckpt_start_recptr = GetInsertRecPtr();
      ckpt_start_time = now;
    ...
    now = (pg_time_t)time(NULL);
    elapsed_secs = now - last_checkpoint_time;
    if (elapsed_secs >= CheckPointTimeout) continue; /* no sleep for us ... */
...
static void CheckArchiveTimeout(void) {
  ...
  if (XLogArchiveTimeout <= 0 || RecoveryInProgress()) return;

  now = (pg_time_t)time(NULL);

  /* First we do a quick check using possibly-stale local state. */
  if ((int)(now - last_xlog_switch_time) < XLogArchiveTimeout) return;
...
static bool ImmediateCheckpointRequested(void) {
  if (checkpoint_requested) {
     ...
     */
    if (cps->ckpt_flags & CHECKPOINT_IMMEDIATE) return true;
  }
  return false;
}
...
void CheckpointWriteDelay(int flags, double progress) {
  ...
  if (!AmCheckpointerProcess()) return;

  /*
   * Perform the usual duties and take a nap, unless we're behind schedule,
   * in which case we just try to catch up as quickly as possible.
   */
  if (!(flags & CHECKPOINT_IMMEDIATE) && !shutdown_requested &&
...
static bool IsCheckpointOnSchedule(double progress) {
  ...
  if (!RecoveryInProgress()) {
    ...
    elapsed_xlogs = (((double)(recptr - ckpt_start_recptr)) / XLogSegSize) /
                    CheckPointSegments;
...
void RequestCheckpoint(int flags) {
  ...
  if (flags & CHECKPOINT_WAIT) {
    ...
    for (;;) {
      ...
      if (new_started != old_started) break;
    ...
    for (;;) {
      ...
      if (new_done - new_started >= 0) break;
 ...
 */
bool ForwardFsyncRequest(RelFileNode rnode, ForkNumber forknum,
                         BlockNumber segno) {
  CheckpointerRequest *request;
  bool too_full;

  if (!IsUnderPostmaster) return false; /* probably shouldn't even get here */
  ...
  if (!AmBackgroundWriterProcess()) CheckpointerShmem->num_backend_writes++;
  ...
  if (CheckpointerShmem->checkpointer_pid == 0 ||
     ...
     * fsync
     */
    if (!AmBackgroundWriterProcess()) CheckpointerShmem->num_backend_fsync++;
...
static bool CompactCheckpointerRequestQueue(void) {
  ...
  skip_slot = static_cast<bool *>(
      palloc0(sizeof(bool) * CheckpointerShmem->num_requests));
  ...
  for (n = 0; n < CheckpointerShmem->num_requests; n++) {
    ...
    slotmap = static_cast<CheckpointerSlotMapping *>(
        hash_search(htab, request, HASH_ENTER, &found));
    if (found) {
...
bool FirstCallSinceLastCheckpoint(void) {
  ...
  if (new_done != ckpt_done) FirstCall = true;
