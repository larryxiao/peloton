 ...
 */
static int findprefix(struct cnfa *cnfa, struct colormap *cm, chr *string,
                      size_t *slength);
...
int pg_regprefix(regex_t *re, chr **string, size_t *slength) {
  ...
  /* sanity checks */
  if (string == NULL || slength == NULL) return REG_INVARG;
  *string = NULL; /* initialize for failure cases */
  *slength = 0;
  if (re == NULL || re->re_magic != REMAGIC) return REG_INVARG;
  ...
  if (g->info & REG_UIMPOSSIBLE) return REG_NOMATCH;
  ...
  *string = (chr *)MALLOC(cnfa->nstates * sizeof(chr));
  if (*string == NULL) return REG_ESPACE;
...
static int /* regprefix return code */
    findprefix(struct cnfa *cnfa, struct colormap *cm, chr *string,
               size_t *slength) {
  int st;
  int nextst;
  color thiscolor;
  ...
  do {
    ...
    for (ca = cnfa->states[st]; ca->co != COLORLESS; ca++) {
      ...
      if (ca->co >= cnfa->ncolors) continue;
      /* We can also ignore BOS/BOL arcs */
      if (ca->co == cnfa->bos[0] || ca->co == cnfa->bos[1]) continue;
    ...
    if (thiscolor == COLORLESS) break;
    /* The color must be a singleton */
    if (cm->cd[thiscolor].nchrs != 1) break;
    ...
    if (GETCOLOR(cm, c) != thiscolor) break;
  ...
  if (nextst == cnfa->post) return REG_EXACT;
   ...
   */
  if (*slength > 0) return REG_PREFIX;
