    ...
    if (collation == DEFAULT_COLLATION_OID)
      ...
      ereport(
          ...
          (errcode(ERRCODE_INDETERMINATE_COLLATION),
           errmsg(
               "could not determine which collation to use for regular "
               "expression"),
...
static pg_wchar pg_wc_toupper(pg_wchar c) {
  switch (pg_regex_strategy) {
    case PG_REGEX_LOCALE_C:
      if (c <= (pg_wchar)127) return pg_ascii_toupper((unsigned char)c);
      return c;
    case PG_REGEX_LOCALE_WIDE:
    ...
    case PG_REGEX_LOCALE_1BYTE:
      /* force C behavior for ASCII characters, per comments above */
      if (c <= (pg_wchar)127) return pg_ascii_toupper((unsigned char)c);
      if (c <= (pg_wchar)UCHAR_MAX) return toupper((unsigned char)c);
      return c;
    case PG_REGEX_LOCALE_WIDE_L:
#if defined(HAVE_LOCALE_T) && defined(USE_WIDE_UPPER_LOWER)
      if (sizeof(wchar_t) >= 4 || c <= (pg_wchar)0xFFFF)
...
static pg_wchar pg_wc_tolower(pg_wchar c) {
  switch (pg_regex_strategy) {
    case PG_REGEX_LOCALE_C:
      if (c <= (pg_wchar)127) return pg_ascii_tolower((unsigned char)c);
      return c;
    case PG_REGEX_LOCALE_WIDE:
      /* force C behavior for ASCII characters, per comments above */
      if (c <= (pg_wchar)127) return pg_ascii_tolower((unsigned char)c);
    ...
    case PG_REGEX_LOCALE_1BYTE:
      /* force C behavior for ASCII characters, per comments above */
      if (c <= (pg_wchar)127) return pg_ascii_tolower((unsigned char)c);
      if (c <= (pg_wchar)UCHAR_MAX) return tolower((unsigned char)c);
      return c;
    case PG_REGEX_LOCALE_WIDE_L:
#if defined(HAVE_LOCALE_T) && defined(USE_WIDE_UPPER_LOWER)
      if (sizeof(wchar_t) >= 4 || c <= (pg_wchar)0xFFFF)
...
static bool store_match(pg_ctype_cache *pcc, pg_wchar chr1, int nchrs) {
  ...
  if (nchrs > 1) {
    if (pcc->cv.nranges >= pcc->cv.rangespace) {
      pcc->cv.rangespace *= 2;
      newchrs =
          (chr *)realloc(pcc->cv.ranges, pcc->cv.rangespace * sizeof(chr) * 2);
      if (newchrs == NULL) return false;
...
static struct cvec *pg_ctype_get_cache(pg_wc_probefunc probefunc) {
  ...
  if (pcc->cv.chrs == NULL || pcc->cv.ranges == NULL) goto out_of_memory;
  ...
  for (cur_chr = 0; cur_chr <= max_chr; cur_chr++) {
    if ((*probefunc)(cur_chr))
      ...
      if (!store_match(pcc, cur_chr - nmatches, nmatches)) goto out_of_memory;
      nmatches = 0;
    }
  }
  ...
  if (pcc->cv.nchrs == 0) {
    ...
    if (newchrs == NULL) goto out_of_memory;
    pcc->cv.chrs = newchrs;
 ...
 */
out_of_memory:
  if (pcc->cv.chrs) free(pcc->cv.chrs);
  if (pcc->cv.ranges) free(pcc->cv.ranges);
  free(pcc);
