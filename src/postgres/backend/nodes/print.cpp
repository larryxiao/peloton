 ...
 */
void elog_node_display(int lev, const char *title, const void *obj,
                       bool pretty) {
  char *s;
  char *f;
...
#define LINELEN 78
  ...
  i = 0;
  for (;;) {
    for (j = 0; j < LINELEN && dump[i] != '\0'; i++, j++) line[j] = dump[i];
    if (dump[i] == '\0') break;
    if (dump[i] == ' ') {
...
void print_rt(const List *rtable) {
  ...
  foreach (l, rtable) {
    ...
    switch (rte->rtekind) {
      case RTE_RELATION:
        printf("%d\t%s\t%u\t%c", i, rte->eref->aliasname, rte->relid,
               rte->relkind);
        break;
      case RTE_SUBQUERY:
