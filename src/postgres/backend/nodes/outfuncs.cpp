...
static void _outList(StringInfo str, const List *node) {
  ...
  foreach (lc, node) {
    ...
    if (IsA(node, List)) {
      ...
      if (lnext(lc)) appendStringInfoChar(str, ' ');
...
static void _outBitmapset(StringInfo str, const Bitmapset *bms) {
  ...
  x = -1;
  while ((x = bms_next_member(bms, x)) >= 0) appendStringInfo(str, " %d", x);
  appendStringInfoChar(str, ')');
}
...
static void _outDatum(StringInfo str, Datum value, int typlen, bool typbyval) {
  ...
  if (typbyval) {
    ...
    if (!PointerIsValid(s))
      ...
      appendStringInfo(str, "%u [ ", (unsigned int)length);
      for (i = 0; i < length; i++) appendStringInfo(str, "%d ", (int)(s[i]));
      appendStringInfoChar(str, ']');
    }
  }
}
...
static void _outEquivalenceClass(StringInfo str, const EquivalenceClass *node) {
   ...
   */
  while (node->ec_merged) node = node->ec_merged;
...
static void _outTableSampleClause(StringInfo str,
...
static void _outValue(StringInfo str, const Value *value) {
  switch (value->type) {
    ...
    case T_String:
       ...
       */
      appendStringInfoChar(str, '"');
      if (value->val.str[0] != '\0') _outToken(str, value->val.str);
      appendStringInfoChar(str, '"');
      break;
    case T_BitString:
...
static void _outNode(StringInfo str, const void *obj) {
  if (obj == NULL)
    ...
    _outList(str, static_cast<const List *>(obj));
    ...
    switch (nodeTag(obj)) {
      ...
      case T_AlternativeSubPlan:
        _outAlternativeSubPlan(str,
                               static_cast<const AlternativeSubPlan *>(obj));
        break;
      case T_FieldSelect:
      ...
      case T_ConvertRowtypeExpr:
        _outConvertRowtypeExpr(str,
                               static_cast<const ConvertRowtypeExpr *>(obj));
        break;
      case T_CollateExpr:
      ...
      case T_CoerceToDomainValue:
        _outCoerceToDomainValue(str,
                                static_cast<const CoerceToDomainValue *>(obj));
        break;
      case T_SetToDefault:
      ...
      case T_CreateForeignTableStmt:
        _outCreateForeignTableStmt(
            str, static_cast<const CreateForeignTableStmt *>(obj));
        break;
      case T_ImportForeignSchemaStmt:
        _outImportForeignSchemaStmt(
            str, static_cast<const ImportForeignSchemaStmt *>(obj));
        break;
      case T_IndexStmt:
