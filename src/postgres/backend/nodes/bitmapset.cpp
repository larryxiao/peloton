...
bool bms_equal(const Bitmapset *a, const Bitmapset *b) {
  ...
  for (i = 0; i < shortlen; i++) {
    if (shorter->words[i] != longer->words[i]) return false;
  }
...
Bitmapset *bms_make_singleton(int x) {
  ...
  if (x < 0) elog(ERROR, "negative bitmapset member not allowed");
  wordnum = WORDNUM(x);
  bitnum = BITNUM(x);
...
Bitmapset *bms_union(const Bitmapset *a, const Bitmapset *b) {
  ...
  /* Handle cases where either input is NULL */
  if (a == NULL) return bms_copy(b);
  if (b == NULL) return bms_copy(a);
  ...
  for (i = 0; i < otherlen; i++) result->words[i] |= other->words[i];
  return result;
}
...
Bitmapset *bms_intersect(const Bitmapset *a, const Bitmapset *b) {
  ...
  for (i = 0; i < resultlen; i++) result->words[i] &= other->words[i];
  return result;
}
...
Bitmapset *bms_difference(const Bitmapset *a, const Bitmapset *b) {
  ...
  for (i = 0; i < shortlen; i++) result->words[i] &= ~b->words[i];
  return result;
}
...
bool bms_is_subset(const Bitmapset *a, const Bitmapset *b) {
  ...
  /* Handle cases where either input is NULL */
  if (a == NULL) return true; /* empty set is a subset of anything */
  if (b == NULL) return bms_is_empty(a);
  /* Check common words */
  shortlen = Min(a->nwords, b->nwords);
  for (i = 0; i < shortlen; i++) {
...
BMS_Comparison bms_subset_compare(const Bitmapset *a, const Bitmapset *b) {
  ...
  /* Handle cases where either input is NULL */
  if (a == NULL) {
    if (b == NULL) return BMS_EQUAL;
    return bms_is_empty(b) ? BMS_EQUAL : BMS_SUBSET1;
  }
  if (b == NULL) return bms_is_empty(a) ? BMS_EQUAL : BMS_SUBSET2;
  /* Check common words */
  ...
  for (i = 0; i < shortlen; i++) {
    ...
    if ((aword & ~bword) != 0) {
      ...
      if (result == BMS_SUBSET1) return BMS_DIFFERENT;
      result = BMS_SUBSET2;
    }
    if ((bword & ~aword) != 0) {
      /* b is not a subset of a */
      if (result == BMS_SUBSET2) return BMS_DIFFERENT;
      result = BMS_SUBSET1;
    }
  }
  /* Check extra words */
...
bool bms_is_member(int x, const Bitmapset *a) {
  ...
  wordnum = WORDNUM(x);
  bitnum = BITNUM(x);
  if (wordnum >= a->nwords) return false;
  if ((a->words[wordnum] & ((bitmapword)1 << bitnum)) != 0) return true;
  return false;
}
...
bool bms_overlap(const Bitmapset *a, const Bitmapset *b) {
  ...
  shortlen = Min(a->nwords, b->nwords);
  for (i = 0; i < shortlen; i++) {
    if ((a->words[i] & b->words[i]) != 0) return true;
  }
  return false;
}
...
int bms_singleton_member(const Bitmapset *a) {
  ...
  if (a == NULL) elog(ERROR, "bitmapset is empty");
  nwords = a->nwords;
  ...
  if (result < 0) elog(ERROR, "bitmapset is empty");
  return result;
}
...
bool bms_get_singleton_member(const Bitmapset *a, int *member) {
  ...
  for (wordnum = 0; wordnum < nwords; wordnum++) {
    ...
    if (w != 0) {
      if (result >= 0 || HAS_MULTIPLE_ONES(w)) return false;
      result = wordnum * BITS_PER_BITMAPWORD;
      while ((w & 255) == 0) {
        w >>= 8;
        result += 8;
...
Bitmapset *bms_add_member(Bitmapset *a, int x) {
  ...
  if (wordnum >= a->nwords) {
    ...
    for (i = oldnwords; i < a->nwords; i++) a->words[i] = 0;
  }
...
Bitmapset *bms_del_member(Bitmapset *a, int x) {
  ...
  wordnum = WORDNUM(x);
  bitnum = BITNUM(x);
  if (wordnum < a->nwords) a->words[wordnum] &= ~((bitmapword)1 << bitnum);
  return a;
}
...
Bitmapset *bms_int_members(Bitmapset *a, const Bitmapset *b) {
  ...
  shortlen = Min(a->nwords, b->nwords);
  for (i = 0; i < shortlen; i++) a->words[i] &= b->words[i];
  for (; i < a->nwords; i++) a->words[i] = 0;
  return a;
}
...
uint32 bms_hash_value(const Bitmapset *a) {
  ...
  if (a == NULL) return 0; /* All empty sets hash to 0 */
  for (lastword = a->nwords; --lastword >= 0;) {
    if (a->words[lastword] != 0) break;
  }
  if (lastword < 0) return 0; /* All empty sets hash to 0 */
