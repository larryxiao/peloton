...
static void tbm_create_pagetable(TIDBitmap *tbm) {
  ...
  tbm->pagetable =
      hash_create("TIDBitmap", 128, /* start small and extend */
  ...
  if (tbm->status == TBM_ONE_PAGE) {
    ...
    page = (PagetableEntry *)hash_search(
        tbm->pagetable, (void *)&tbm->entry1.blockno, HASH_ENTER, &found);
    Assert(!found);
...
void tbm_free(TIDBitmap *tbm) {
  if (tbm->pagetable) hash_destroy(tbm->pagetable);
  if (tbm->spages) pfree(tbm->spages);
  if (tbm->schunks) pfree(tbm->schunks);
  pfree(tbm);
}
 ...
 */
void tbm_add_tuples(TIDBitmap *tbm, const ItemPointer tids, int ntids,
                    bool recheck) {
...
void tbm_add_page(TIDBitmap *tbm, BlockNumber pageno) {
  ...
  if (tbm->nentries > tbm->maxentries) tbm_lossify(tbm);
}
...
static void tbm_union_page(TIDBitmap *a, const PagetableEntry *bpage) {
  ...
  if (bpage->ischunk) {
    ...
    for (wordnum = 0; wordnum < WORDS_PER_CHUNK; wordnum++) {
      ...
      if (w != 0) {
        ...
        while (w != 0) {
          if (w & 1) tbm_mark_page_lossy(a, pg);
          pg++;
          w >>= 1;
        }
      }
  ...
  if (a->nentries > a->maxentries) tbm_lossify(a);
}
    ...
    while ((apage = (PagetableEntry *)hash_seq_search(&status)) != NULL) {
      if (tbm_intersect_page(a, apage, b)) {
        ...
        if (apage->ischunk)
          ...
          a->npages--;
        a->nentries--;
        if (hash_search(a->pagetable, (void *)&apage->blockno, HASH_REMOVE,
                        NULL) == NULL)
...
static bool tbm_intersect_page(TIDBitmap *a, PagetableEntry *apage,
  ...
  if (apage->ischunk) {
    ...
    if (bpage != NULL) {
      ...
      for (wordnum = 0; wordnum < WORDS_PER_PAGE; wordnum++) {
        apage->words[wordnum] &= bpage->words[wordnum];
        if (apage->words[wordnum] != 0) candelete = false;
      }
      apage->recheck |= bpage->recheck;
    }
 ...
 */
bool tbm_is_empty(const TIDBitmap *tbm) { return (tbm->nentries == 0); }
...
TBMIterator *tbm_begin_iterate(TIDBitmap *tbm) {
  ...
  if (tbm->status == TBM_HASH && !tbm->iterating) {
    ...
    if (!tbm->spages && tbm->npages > 0)
      tbm->spages = (PagetableEntry **)MemoryContextAlloc(
          tbm->mcxt, tbm->npages * sizeof(PagetableEntry *));
    if (!tbm->schunks && tbm->nchunks > 0)
      tbm->schunks = (PagetableEntry **)MemoryContextAlloc(
          tbm->mcxt, tbm->nchunks * sizeof(PagetableEntry *));
...
TBMIterateResult *tbm_iterate(TBMIterator *iterator) {
  ...
  while (iterator->schunkptr < tbm->nchunks) {
    ...
    while (schunkbit < PAGES_PER_CHUNK) {
      ...
      if ((chunk->words[wordnum] & ((bitmapword)1 << bitnum)) != 0) break;
      schunkbit++;
    }
  ...
  if (iterator->spageptr < tbm->npages) {
    ...
    for (wordnum = 0; wordnum < WORDS_PER_PAGE; wordnum++) {
      ...
      if (w != 0) {
        ...
        while (w != 0) {
          if (w & 1) output->offsets[ntuples++] = (OffsetNumber)off;
          off++;
          w >>= 1;
        }
      }
 ...
 */
void tbm_end_iterate(TBMIterator *iterator) { pfree(iterator); }
...
static const PagetableEntry *tbm_find_pageentry(const TIDBitmap *tbm,
  ...
  if (tbm->status == TBM_ONE_PAGE) {
    page = &tbm->entry1;
    if (page->blockno != pageno) return NULL;
    Assert(!page->ischunk);
    return page;
  }

  page = (PagetableEntry *)hash_search(tbm->pagetable, (void *)&pageno,
                                       HASH_FIND, NULL);
  if (page == NULL) return NULL;
  if (page->ischunk) return NULL; /* don't want a lossy chunk header */
  return page;
}
...
static PagetableEntry *tbm_get_pageentry(TIDBitmap *tbm, BlockNumber pageno) {
  ...
  if (tbm->status == TBM_EMPTY) {
    ...
    if (tbm->status == TBM_ONE_PAGE) {
      page = &tbm->entry1;
      if (page->blockno == pageno) return page;
...
static bool tbm_page_is_lossy(const TIDBitmap *tbm, BlockNumber pageno) {
  ...
  if (tbm->nchunks == 0) return false;
  Assert(tbm->status == TBM_HASH);

  bitno = pageno % PAGES_PER_CHUNK;
  chunk_pageno = pageno - bitno;
  page = (PagetableEntry *)hash_search(tbm->pagetable, (void *)&chunk_pageno,
                                       HASH_FIND, NULL);
  if (page != NULL && page->ischunk) {
    int wordnum = WORDNUM(bitno);
    int bitnum = BITNUM(bitno);

    if ((page->words[wordnum] & ((bitmapword)1 << bitnum)) != 0) return true;
  }
  return false;
}
...
static void tbm_mark_page_lossy(TIDBitmap *tbm, BlockNumber pageno) {
  ...
  if (tbm->status != TBM_HASH) tbm_create_pagetable(tbm);
   ...
   */
  if (bitno != 0) {
    if (hash_search(tbm->pagetable, (void *)&pageno, HASH_REMOVE, NULL) !=
        NULL) {
...
static void tbm_lossify(TIDBitmap *tbm) {
  ...
  while ((page = (PagetableEntry *)hash_seq_search(&status)) != NULL) {
     ...
     */
    if ((page->blockno % PAGES_PER_CHUNK) == 0) continue;
