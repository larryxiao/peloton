...
static ListCell *add_new_cell(List *list, ListCell *prev_cell) {
  ...
  if (list->tail == prev_cell) list->tail = new_cell;
...
bool list_member(const List *list, const void *datum) {
  ...
  foreach (cell, list) {
    if (equal(lfirst(cell), datum)) return true;
  }
...
bool list_member_ptr(const List *list, const void *datum) {
  ...
  foreach (cell, list) {
    if (lfirst(cell) == datum) return true;
  }
...
bool list_member_int(const List *list, int datum) {
  ...
  foreach (cell, list) {
    if (lfirst_int(cell) == datum) return true;
  }
...
bool list_member_oid(const List *list, Oid datum) {
  ...
  foreach (cell, list) {
    if (lfirst_oid(cell) == datum) return true;
  }
...
List *list_delete(List *list, void *datum) {
  ...
  prev = NULL;
  foreach (cell, list) {
    if (equal(lfirst(cell), datum)) return list_delete_cell(list, cell, prev);
...
List *list_delete_ptr(List *list, void *datum) {
  ...
  prev = NULL;
  foreach (cell, list) {
    if (lfirst(cell) == datum) return list_delete_cell(list, cell, prev);
...
List *list_delete_int(List *list, int datum) {
  ...
  prev = NULL;
  foreach (cell, list) {
    if (lfirst_int(cell) == datum) return list_delete_cell(list, cell, prev);
...
List *list_delete_oid(List *list, Oid datum) {
  ...
  prev = NULL;
  foreach (cell, list) {
    if (lfirst_oid(cell) == datum) return list_delete_cell(list, cell, prev);
...
List *list_delete_first(List *list) {
  ...
  if (list == NIL) return NIL; /* would an error be better? */
...
List *list_difference(const List *list1, const List *list2) {
  ...
  if (list2 == NIL) return list_copy(list1);
...
List *list_concat_unique(List *list1, List *list2) {
  ...
  foreach (cell, list2) {
    if (!list_member(list1, lfirst(cell))) list1 = lappend(list1, lfirst(cell));
  }
 ...
 */
void list_free(List *list) { list_free_private(list, false); }
...
int length(const List *list) { return list_length(list); }
