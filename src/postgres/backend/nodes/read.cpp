...
char *pg_strtok(int *length) {
  ...
  *length = local_str - ret_str;
  ...
  if (*length == 2 && ret_str[0] == '<' && ret_str[1] == '>') *length = 0;
...
char *debackslash(char *token, int length) {
  ...
  while (length > 0) {
    if (*token == '\\' && length > 1) token++, length--;
    *ptr++ = *token++;
    length--;
  }
  *ptr = '\0';
...
static NodeTag nodeTokenType(char *token, int length) {
  ...
  numptr = token;
  numlen = length;
  if (*numptr == '+' || *numptr == '-') numptr++, numlen--;
...
void *nodeRead(char *token, int tok_len) {
  ...
  switch ((int)type) {
    ...
    case LEFT_PAREN: {
       ...
       */
      token = pg_strtok(&tok_len);
      if (token == NULL) elog(ERROR, "unterminated List structure");
