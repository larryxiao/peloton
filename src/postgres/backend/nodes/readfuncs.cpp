...
#define READ_NODE_FIELD(fldname)                               \
  ...
  (void) token;               /* in case not used elsewhere */ \
  local_node->fldname =                                        \
      static_cast<decltype(local_node->fldname)>(nodeRead(NULL, 0))
...
static Bitmapset *_readBitmapset(void) {
  ...
  if (token == NULL) elog(ERROR, "incomplete Bitmapset structure");
  if (length != 1 || token[0] != '(')
  ...
  for (;;) {
    ...
    token = pg_strtok(&length);
    if (token == NULL) elog(ERROR, "unterminated Bitmapset structure");
    if (length == 1 && token[0] == ')') break;
 ...
 */
static RangeTableSample *_readRangeTableSample(void) {
