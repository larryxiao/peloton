...
static void out_infobits(StringInfo buf, uint8 infobits) {
  if (infobits & XLHL_XMAX_IS_MULTI) appendStringInfoString(buf, "IS_MULTI ");
  if (infobits & XLHL_XMAX_LOCK_ONLY) appendStringInfoString(buf, "LOCK_ONLY ");
  if (infobits & XLHL_XMAX_EXCL_LOCK) appendStringInfoString(buf, "EXCL_LOCK ");
...
void heap_desc(StringInfo buf, XLogReaderState *record) {
  ...
  if (info == XLOG_HEAP_INSERT) {
    ...
    appendStringInfo(buf, "off %u xmax %u ", xlrec->old_offnum,
                     xlrec->old_xmax);
    out_infobits(buf, xlrec->old_infobits_set);
    appendStringInfo(buf, "; new___ off %u xmax %u", xlrec->new_offnum,
                     xlrec->new_xmax);
...
void heap2_desc(StringInfo buf, XLogReaderState *record) {
  ...
  if (info == XLOG_HEAP2_CLEAN) {
    ...
    appendStringInfo(buf, "cutoff xid %u ntuples %u", xlrec->cutoff_xid,
                     xlrec->ntuples);
    ...
    appendStringInfo(buf, "rel %u/%u/%u; tid %u/%u", xlrec->target_node.spcNode,
                     xlrec->target_node.dbNode, xlrec->target_node.relNode,
                     ItemPointerGetBlockNumber(&(xlrec->target_tid)),
                     ItemPointerGetOffsetNumber(&(xlrec->target_tid)));
    appendStringInfo(buf, "; cmin: %u, cmax: %u, combo: %u", xlrec->cmin,
