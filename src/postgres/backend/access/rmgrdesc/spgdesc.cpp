  ...
  switch (info) {
    ...
    case XLOG_SPGIST_ADD_LEAF: {
      ...
      appendStringInfo(buf, "; off %u; headoff %u; parentoff %u",
                       ...
                       xlrec->offnumParent);
      if (xlrec->newPage) appendStringInfo(buf, " (newpage)");
      if (xlrec->storesNulls) appendStringInfo(buf, " (nulls)");
    } break;
    ...
    case XLOG_SPGIST_SPLIT_TUPLE:
      appendStringInfo(buf,
                       "prefix off: %u, postfix off: %u (same %d, new___ %d)",
    ...
    case XLOG_SPGIST_PICKSPLIT: {
      ...
      if (xlrec->innerIsParent) appendStringInfo(buf, " (innerIsParent)");
      if (xlrec->isRootSplit) appendStringInfo(buf, " (isRootSplit)");
    } break;
