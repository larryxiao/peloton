...
void brin_desc(StringInfo buf, XLogReaderState *record) {
  ...
  if (info == XLOG_BRIN_CREATE_INDEX) {
    ...
    appendStringInfo(buf, "v%d pagesPerRange %u", xlrec->version,
                     xlrec->pagesPerRange);
    ...
    appendStringInfo(
        buf, "heapBlk %u pagesPerRange %u old offnum %u, new___ offnum %u",
        xlrec->insert.heapBlk, xlrec->insert.pagesPerRange, xlrec->oldOffnum,
        xlrec->insert.offnum);
