...
static void standby_desc_running_xacts(StringInfo buf,
  ...
  appendStringInfo(buf, "nextXid %u latestCompletedXid %u oldestRunningXid %u",
                   xlrec->nextXid, xlrec->latestCompletedXid,
  ...
  if (xlrec->subxid_overflow) appendStringInfoString(buf, "; subxid ovf");
}
...
void standby_desc(StringInfo buf, XLogReaderState *record) {
  ...
  if (info == XLOG_STANDBY_LOCK) {
    ...
    for (i = 0; i < xlrec->nlocks; i++)
      appendStringInfo(buf, "xid %u db %u rel %u ", xlrec->locks[i].xid,
                       xlrec->locks[i].dbOid, xlrec->locks[i].relOid);
