 ...
 */
Datum tsm_system_init(PG_FUNCTION_ARGS) {
  ...
  sampler =
      static_cast<SystemSamplerData *>(palloc0(sizeof(SystemSamplerData)));
 ...
 */
Datum tsm_system_nextblock(PG_FUNCTION_ARGS) {
  TableSampleDesc *tsdesc = (TableSampleDesc *)PG_GETARG_POINTER(0);
  SystemSamplerData *sampler = (SystemSamplerData *)tsdesc->tsmdata;
  BlockNumber blockno;

  if (!BlockSampler_HasMore(&sampler->bs)) PG_RETURN_UINT32(InvalidBlockNumber);
 ...
 * with this block.
 */
Datum tsm_system_nexttuple(PG_FUNCTION_ARGS) {
  TableSampleDesc *tsdesc = (TableSampleDesc *)PG_GETARG_POINTER(0);
  ...
  if (tupoffset > maxoffset) tupoffset = InvalidOffsetNumber;
 ...
 * Cleanup method.
 */
Datum tsm_system_end(PG_FUNCTION_ARGS) {
  TableSampleDesc *tsdesc = (TableSampleDesc *)PG_GETARG_POINTER(0);
 ...
 */
Datum tsm_system_reset(PG_FUNCTION_ARGS) {
  TableSampleDesc *tsdesc = (TableSampleDesc *)PG_GETARG_POINTER(0);
  SystemSamplerData *sampler = (SystemSamplerData *)tsdesc->tsmdata;
 ...
 * Costing function.
 */
Datum tsm_system_cost(PG_FUNCTION_ARGS) {
