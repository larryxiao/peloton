 ...
 */
TableSampleDesc *tablesample_init(SampleScanState *scanstate,
  ...
  InitFunctionCallInfoData(fcinfo, &tsdesc->tsminit, list_length(args) + 2,
  ...
  if (tablesample->repeatable) {
    ExprState *argstate =
        ExecInitExpr((Expr *)tablesample->repeatable, (PlanState *)scanstate);
    fcinfo.arg[1] = ExecEvalExpr(argstate, econtext, &fcinfo.argnull[1], NULL);
    if (fcinfo.argnull[1])
      ereport(ERROR,
 ...
 */
HeapTuple tablesample_getnext(TableSampleDesc *desc) {
  ...
  if (!scan->rs_inited) {
    ...
    blockno = DatumGetInt32(
        FunctionCall1(&desc->tsmnextblock, PointerGetDatum(desc)));
   ...
   */
  if (!pagemode) LockBuffer(scan->rs_cbuf, BUFFER_LOCK_SHARE);
  ...
  for (;;) {
    ...
    tupoffset = DatumGetUInt16(
        FunctionCall3(&desc->tsmnexttuple, PointerGetDatum(desc),
                      UInt32GetDatum(blockno), UInt16GetDatum(maxoffset)));
    ...
    if (OffsetNumberIsValid(tupoffset)) {
      ...
      if (OidIsValid(desc->tsmexaminetuple.fn_oid)) {
        found = DatumGetBool(
            FunctionCall4(&desc->tsmexaminetuple, PointerGetDatum(desc),
                          UInt32GetDatum(blockno), PointerGetDatum(tuple),
      ...
      if (found) {
        if (!pagemode) LockBuffer(scan->rs_cbuf, BUFFER_LOCK_UNLOCK);
        break;
      } else {
 ...
 */
void tablesample_reset(TableSampleDesc *desc) {
 ...
 */
void tablesample_end(TableSampleDesc *desc) {
...
static bool SampleTupleVisible(HeapTuple tuple, OffsetNumber tupoffset,
  ...
  if (scan->rs_pageatatime) {
    int start = 0, end = scan->rs_ntuples - 1;
