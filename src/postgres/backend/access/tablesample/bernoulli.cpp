 ...
 */
Datum tsm_bernoulli_init(PG_FUNCTION_ARGS) {
  ...
  if (percent < 0 || percent > 100)
    ereport(ERROR, (errcode(ERRCODE_NUMERIC_VALUE_OUT_OF_RANGE),
                    ...
                    errhint(
                        "Sample size must be numeric value between 0 and 100 "
                        "(inclusive).")));

  sampler = static_cast<BernoulliSamplerData *>(
      palloc0(sizeof(BernoulliSamplerData)));
 ...
 * end of the relation.
 */
Datum tsm_bernoulli_nextblock(PG_FUNCTION_ARGS) {
  TableSampleDesc *tsdesc = (TableSampleDesc *)PG_GETARG_POINTER(0);
  ...
  if (sampler->blockno == InvalidBlockNumber)
    ...
    if (sampler->blockno >= sampler->nblocks) sampler->blockno = 0;
 ...
 */
Datum tsm_bernoulli_nexttuple(PG_FUNCTION_ARGS) {
  TableSampleDesc *tsdesc = (TableSampleDesc *)PG_GETARG_POINTER(0);
  ...
  if (tupoffset > maxoffset) /* Tell SampleScan that we want next block. */
 ...
 * Cleanup method.
 */
Datum tsm_bernoulli_end(PG_FUNCTION_ARGS) {
  TableSampleDesc *tsdesc = (TableSampleDesc *)PG_GETARG_POINTER(0);
 ...
 */
Datum tsm_bernoulli_reset(PG_FUNCTION_ARGS) {
  TableSampleDesc *tsdesc = (TableSampleDesc *)PG_GETARG_POINTER(0);
  BernoulliSamplerData *sampler = (BernoulliSamplerData *)tsdesc->tsmdata;
 ...
 * Costing function.
 */
Datum tsm_bernoulli_cost(PG_FUNCTION_ARGS) {
