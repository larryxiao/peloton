...
TupleConversionMap *convert_tuples_by_position(TupleDesc indesc,
  ...
  for (i = 0; i < n; i++) {
    ...
    if (att->attisdropped) continue; /* attrMap[i] is already 0 */
    noutcols++;
    ...
    if (attrMap[i] == 0) same = false; /* we'll complain below */
  }

  /* Check for unused input columns */
  for (; j < indesc->natts; j++) {
    if (indesc->attrs[j]->attisdropped) continue;
    nincols++;
      ...
      if (strcmp(attname, NameStr(att->attname)) == 0) {
        ...
        if (atttypid != att->atttypid || atttypmod != att->atttypmod)
          ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH),
                          ...
                          errdetail(
                              "Attribute \"%s\" of type %s does not match "
                              "corresponding attribute of type %s.",
                              attname, format_type_be(outdesc->tdtypeid),
