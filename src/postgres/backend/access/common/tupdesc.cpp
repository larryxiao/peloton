...
TupleDesc CreateTemplateTupleDesc(int natts, bool hasoid) {
  ...
  stg = static_cast<char *>(
      palloc(attroffset + natts * MAXALIGN(ATTRIBUTE_FIXED_PART_SIZE)));
  desc = (TupleDesc)stg;
...
TupleDesc CreateTupleDescCopyConstr(TupleDesc tupdesc) {
  ...
  if (constr) {
    ...
    if ((cpy->num_defval = constr->num_defval) > 0) {
      cpy->defval =
          (AttrDefault *)palloc(cpy->num_defval * sizeof(AttrDefault));
      memcpy(cpy->defval, constr->defval,
 ...
 * !!! Constraints and defaults are not copied !!!
 */
void TupleDescCopyEntry(TupleDesc dst, AttrNumber dstAttno, TupleDesc src,
                        AttrNumber srcAttno) {
  /*
   * sanity checks
   */
  ...
  if (tupdesc->constr) {
    if (tupdesc->constr->num_defval > 0) {
      ...
      for (i = tupdesc->constr->num_defval - 1; i >= 0; i--) {
        if (attrdef[i].adbin) pfree(attrdef[i].adbin);
      }
      pfree(attrdef);
    }
...
void DecrTupleDescRefCount(TupleDesc tupdesc) {
  ...
  if (--tupdesc->tdrefcount == 0) FreeTupleDesc(tupdesc);
}
...
bool equalTupleDescs(TupleDesc tupdesc1, TupleDesc tupdesc2) {
  ...
  if (tupdesc1->natts != tupdesc2->natts) return false;
  if (tupdesc1->tdtypeid != tupdesc2->tdtypeid) return false;
  if (tupdesc1->tdhasoid != tupdesc2->tdhasoid) return false;
  ...
  for (i = 0; i < tupdesc1->natts; i++) {
    ...
    if (attr1->attstattarget != attr2->attstattarget) return false;
    if (attr1->attlen != attr2->attlen) return false;
    if (attr1->attndims != attr2->attndims) return false;
    if (attr1->atttypmod != attr2->atttypmod) return false;
    if (attr1->attbyval != attr2->attbyval) return false;
    if (attr1->attstorage != attr2->attstorage) return false;
    if (attr1->attalign != attr2->attalign) return false;
    if (attr1->attnotnull != attr2->attnotnull) return false;
    if (attr1->atthasdef != attr2->atthasdef) return false;
    if (attr1->attisdropped != attr2->attisdropped) return false;
    if (attr1->attislocal != attr2->attislocal) return false;
    if (attr1->attinhcount != attr2->attinhcount) return false;
    if (attr1->attcollation != attr2->attcollation) return false;
  ...
  if (tupdesc1->constr != NULL) {
    ...
    if (constr2 == NULL) return false;
    if (constr1->has_not_null != constr2->has_not_null) return false;
    n = constr1->num_defval;
    if (n != (int)constr2->num_defval) return false;
    for (i = 0; i < n; i++) {
      ...
      for (j = 0; j < n; defval2++, j++) {
        if (defval1->adnum == defval2->adnum) break;
      }
      if (j >= n) return false;
      if (strcmp(defval1->adbin, defval2->adbin) != 0) return false;
    }
    n = constr1->num_check;
    if (n != (int)constr2->num_check) return false;
    for (i = 0; i < n; i++) {
...
void TupleDescInitEntry(TupleDesc desc, AttrNumber attributeNumber,
                        const char *attributeName, Oid oidtypeid, int32 typmod,
                        int attdim) {
  HeapTuple tuple;
 ...
 * entry.
 */
void TupleDescInitEntryCollation(TupleDesc desc, AttrNumber attributeNumber,
                                 Oid collationid) {
  /*
   * sanity checks
   */
...
TupleDesc BuildDescForRelation(List *schema) {
  ...
  foreach (l, schema) {
    ...
    if (aclresult != ACLCHECK_OK) aclcheck_error_type(aclresult, atttypid);
    ...
    if (entry->storage) desc->attrs[attnum - 1]->attstorage = entry->storage;
 ...
 */
TupleDesc BuildDescFromLists(List *names, List *types, List *typmods,
                             List *collations) {
  int natts;
  AttrNumber attnum;
  ListCell *l1;
