...
Size heap_compute_data_size(TupleDesc tupleDesc, Datum *values, bool *isnull) {
  ...
  for (i = 0; i < numberOfAttributes; i++) {
    ...
    if (ATT_IS_PACKABLE(atti) && VARATT_CAN_MAKE_SHORT(DatumGetPointer(val))) {
      ...
      data_length =
          att_align_datum(data_length, atti->attalign, atti->attlen, val);
 ...
 */
void heap_fill_tuple(TupleDesc tupleDesc, Datum *values, bool *isnull,
...
bool heap_attisnull(HeapTuple tup, int attnum) {
  if (attnum > (int)HeapTupleHeaderGetNatts(tup->t_data)) return true;

  if (attnum > 0) {
    if (HeapTupleNoNulls(tup)) return false;
...
Datum nocachegetattr(HeapTuple tuple, int attnum, TupleDesc tupleDesc) {
  ...
  if (!slow) {
    ...
    while (j < natts && att[j]->attcacheoff > 0) j++;

    off = att[j - 1]->attcacheoff + att[j - 1]->attlen;

    for (; j < natts; j++) {
      if (att[j]->attlen <= 0) break;
    ...
    for (i = 0;; i++) /* loop exit is at "break" */
    {
      ...
      if (usecache && att[i]->attcacheoff >= 0)
         ...
         */
        if (usecache && off == att_align_nominal(off, att[i]->attalign))
      ...
      if (usecache && att[i]->attlen <= 0) usecache = false;
    }
  }
 ...
 */
HeapTuple heap_formtuple(TupleDesc tupleDescriptor, Datum *values,
                         char *nulls) {
  HeapTuple tuple; /* return tuple */
  int numberOfAttributes = tupleDescriptor->natts;
  ...
  for (i = 0; i < numberOfAttributes; i++) boolNulls[i] = (nulls[i] == 'n');
 ...
 * indicated by tupleDesc->natts.  The new___ tuple is constructed using the
 *data
...
HeapTuple heap_modify_tuple(HeapTuple tuple, TupleDesc tupleDesc,
  ...
  newTuple->t_self = tuple->t_self;
  newTuple->t_tableOid = tuple->t_tableOid;
  if (tupleDesc->tdhasoid) HeapTupleSetOid(newTuple, HeapTupleGetOid(tuple));
 ...
 * around for use by old add-on modules.
 */
HeapTuple heap_modifytuple(HeapTuple tuple, TupleDesc tupleDesc,
                           Datum *replValues, char *replNulls,
                           char *replActions) {
  HeapTuple result;
  int numberOfAttributes = tupleDesc->natts;
  bool *boolNulls = (bool *)palloc(numberOfAttributes * sizeof(bool));
  ...
  result =
      heap_modify_tuple(tuple, tupleDesc, replValues, boolNulls, boolActions);
 ...
 * around for use by old add-on modules.
 */
void heap_deformtuple(HeapTuple tuple, TupleDesc tupleDesc, Datum *values,
                      char *nulls) {
  int natts = tupleDesc->natts;
...
void slot_getallattrs(TupleTableSlot *slot) {
  ...
  if (slot->tts_nvalid == tdesc_natts) return;
...
void slot_getsomeattrs(TupleTableSlot *slot, int attnum) {
  ...
  /* Quick out if we have 'em all already */
  if (slot->tts_nvalid >= attnum) return;
...
bool slot_attisnull(TupleTableSlot *slot, int attnum) {
   ...
   * fast path if desired attribute already cached
   */
  if (attnum <= slot->tts_nvalid) return slot->tts_isnull[attnum - 1];

  /*
   * return NULL if attnum is out of range according to the tupdesc
   */
  if (attnum > tupleDesc->natts) return true;
 ...
 * heap_freetuple
 */
void heap_freetuple(HeapTuple htup) { pfree(htup); }
 ...
 * The result is allocated in the current memory context.
 */
MinimalTuple heap_form_minimal_tuple(TupleDesc tupleDescriptor, Datum *values,
                                     bool *isnull) {
  ...
  heap_fill_tuple(tupleDescriptor, values, isnull, (char *)tuple + hoff,
                  data_len, &tuple->t_infomask,
 ...
 */
void heap_free_minimal_tuple(MinimalTuple mtup) { pfree(mtup); }
