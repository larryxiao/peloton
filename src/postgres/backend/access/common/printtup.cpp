...
static void printtup_startup(DestReceiver *self, int operation,
  ...
  myState->tmpcontext = AllocSetContextCreate(
      CurrentMemoryContext, "printtup", ALLOCSET_DEFAULT_MINSIZE,
  ...
  if (PG_PROTOCOL_MAJOR(FrontendProtocol) < 3) {
    ...
    if (portalName == NULL || portalName[0] == '\0') portalName = "blank";
  ...
  if (myState->sendDescrip && mc_state == nullptr)
    SendRowDescriptionMessage(typeinfo, FetchPortalTargetList(portal),
                              portal->formats);
 ...
 */
void SendRowDescriptionMessage(TupleDesc typeinfo, List *targetlist,
                               int16 *formats) {
  ...
  for (i = 0; i < natts; ++i) {
    ...
    if (proto >= 2) pq_sendint(&buf, atttypmod, sizeof(atttypmod));
 ...
 */
static void printtup_prepare_info(DR_printtup *myState, TupleDesc typeinfo,
                                  int numAttrs) {
  int16 *formats = myState->portal->formats;
  int i;

  /* get rid of any old data */
  if (myState->myinfo) pfree(myState->myinfo);
  myState->myinfo = NULL;
  ...
  myState->myinfo =
      (PrinttupAttrInfo *)palloc0(numAttrs * sizeof(PrinttupAttrInfo));
  ...
  for (i = 0; i < numAttrs; i++) {
    ...
    if (format == 0) {
      getTypeOutputInfo(typeinfo->attrs[i]->atttypid, &thisState->typoutput,
                        &thisState->typisvarlena);
      fmgr_info(thisState->typoutput, &thisState->finfo);
    } else if (format == 1) {
      getTypeBinaryOutputInfo(typeinfo->attrs[i]->atttypid, &thisState->typsend,
                              &thisState->typisvarlena);
...
static void printtup_20(TupleTableSlot *slot, DestReceiver *self,
  ...
  k = 1 << 7;
  for (i = 0; i < natts; ++i) {
    if (!slot->tts_isnull[i]) j |= k; /* set bit if not null */
    k >>= 1;
    if (k == 0) /* end of byte? */
    {
      pq_sendint(&buf, j, 1);
...
static void printtup_shutdown(DestReceiver *self) {
  ...
  if (myState->tmpcontext) MemoryContextDelete(myState->tmpcontext);
 ...
 * ----------------
 */
static void printtup_destroy(DestReceiver *self) { pfree(self); }
...
static void printatt(unsigned attributeId, Form_pg_attribute attributeP,
                     char *value) {
  printf(
      "\t%2d: %s%s%s%s\t(typeid___ = %u, len = %d, typmod = %d, byval = %c)\n",
      attributeId, NameStr(attributeP->attname), value != NULL ? " = \"" : "",
      value != NULL ? value : "", value != NULL ? "\"" : "",
      (unsigned int)(attributeP->atttypid), attributeP->attlen,
      attributeP->atttypmod, attributeP->attbyval ? 't' : 'f');
}
...
void debugStartup(DestReceiver *self, int operation, TupleDesc typeinfo,
   ...
   */
  for (i = 0; i < natts; ++i) printatt((unsigned)i + 1, attinfo[i], NULL);
  printf("\t----\n");
}
