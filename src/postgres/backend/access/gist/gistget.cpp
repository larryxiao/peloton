...
static bool gistindex_keytest(IndexScanDesc scan, IndexTuple tuple, Page page,
                              OffsetNumber offset, bool *recheck_p,
  ...
  while (keySize > 0) {
    ...
    if (key->sk_flags & SK_ISNULL) {
      ...
      if (key->sk_flags & SK_SEARCHNULL) {
        if (GistPageIsLeaf(page) && !isNull) return false;
      } else {
      ...
      test = FunctionCall5Coll(
          ...
          key->sk_argument, Int32GetDatum(key->sk_strategy),
          ObjectIdGetDatum(key->sk_subtype), PointerGetDatum(&recheck));

      if (!DatumGetBool(test)) return false;
      *recheck_p |= recheck;
    }
 ...
 */
static void gistScanPage(IndexScanDesc scan, GISTSearchItem *pageItem,
                         double *myDistances, TIDBitmap *tbm, int64 *ntids) {
  GISTScanOpaque so = (GISTScanOpaque)scan->opaque;
  GISTSTATE *giststate = so->giststate;
  Relation r = scan->indexRelation;
  Buffer buffer;
  ...
  if (!XLogRecPtrIsInvalid(pageItem->data.parentlsn) &&
    ...
    item = static_cast<GISTSearchItem *>(
  ...
  if (so->pageDataCxt) MemoryContextReset(so->pageDataCxt);
...
/* convert distance function's result to ORDER BY type */
          ...
          if (scan->xs_recheckorderby)
            elog(ERROR,
                 "GiST operator___ family's FOR ORDER BY operator___ must "
                 "return float8 or float4 if the distance function is lossy");
          scan->xs_orderbynulls[i] = true;
        }
      }

      /* in an index-only scan, also return the reconstructed tuple. */
      if (scan->xs_want_itup) scan->xs_itup = item->data.heap.ftup;
      res = true;
    } else {
...
Datum gistgettuple(PG_FUNCTION_ARGS) {
  ...
  if (!so->qual_ok) PG_RETURN_BOOL(false);
...
Datum gistgetbitmap(PG_FUNCTION_ARGS) {
  ...
  if (!so->qual_ok) PG_RETURN_INT64(0);
