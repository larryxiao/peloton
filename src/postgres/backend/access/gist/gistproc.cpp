...
Datum gist_box_consistent(PG_FUNCTION_ARGS) {
  ...
  *recheck = false;
  ...
  if (GIST_LEAF(entry))
    PG_RETURN_BOOL(
        gist_box_leaf_consistent(DatumGetBoxP(entry->key), query, strategy));
  else
    PG_RETURN_BOOL(
        rtree_internal_consistent(DatumGetBoxP(entry->key), query, strategy));
}

static void adjustBox(BOX *b, BOX *addon) {
  if (b->high.x < addon->high.x) b->high.x = addon->high.x;
  if (b->low.x > addon->low.x) b->low.x = addon->low.x;
  if (b->high.y < addon->high.y) b->high.y = addon->high.y;
  if (b->low.y > addon->low.y) b->low.y = addon->low.y;
}
 ...
 */
typedef struct { double lower, upper; } SplitInterval;
 ...
 */
static inline void g_box_consider_split(ConsiderSplitContext *context,
                                        int dimNum, double rightLower,
                                        int minLeftCount, double leftUpper,
                                        int maxLeftCount) {
   ...
   */
  ratio =
      ((float4)Min(leftCount, rightCount)) / ((float4)context->entriesCount);
 ...
 */
static double box_penalty(BOX *original, BOX *new___) {
  ...
  return union_width * union_height -
         (original->high.x - original->low.x) *
 ...
 *
 * For details see:
 * "A new___ double sorting-based node splitting algorithm for R-tree", A.
 *Korotkov
...
Datum gist_box_picksplit(PG_FUNCTION_ARGS) {
  ...
  for (dim = 0; dim < 2; dim++) {
    ...
    while (true) {
       ...
       * left group.
       */
      while (i2 < nentries && intervalsUpper[i2].upper <= leftUpper) i2++;
    ...
    while (true) {
       ...
       * right group.
       */
      while (i1 >= 0 && intervalsLower[i1].lower >= rightLower) i1--;

      /*
       * Consider found split.
       */
      g_box_consider_split(&context, dim, rightLower, i1 + 1, leftUpper,
                           i2 + 1);
    }
  }
  ...
  if (commonEntriesCount > 0) {
    ...
    for (i = 0; i < commonEntriesCount; i++) {
      ...
      commonEntries[i].delta =
          Abs(box_penalty(leftBox, box) - box_penalty(rightBox, box));
    }
...
static bool gist_box_leaf_consistent(BOX *key, BOX *query,
  ...
  switch (strategy) {
    case RTLeftStrategyNumber:
      retval = DatumGetBool(DirectFunctionCall2(box_left, PointerGetDatum(key),
                                                PointerGetDatum(query)));
      break;
    case RTOverLeftStrategyNumber:
      retval = DatumGetBool(DirectFunctionCall2(
          box_overleft, PointerGetDatum(key), PointerGetDatum(query)));
      break;
    case RTOverlapStrategyNumber:
      retval = DatumGetBool(DirectFunctionCall2(
          box_overlap, PointerGetDatum(key), PointerGetDatum(query)));
      break;
    case RTOverRightStrategyNumber:
      retval = DatumGetBool(DirectFunctionCall2(
          box_overright, PointerGetDatum(key), PointerGetDatum(query)));
      break;
    case RTRightStrategyNumber:
      retval = DatumGetBool(DirectFunctionCall2(box_right, PointerGetDatum(key),
                                                PointerGetDatum(query)));
      break;
    case RTSameStrategyNumber:
      retval = DatumGetBool(DirectFunctionCall2(box_same, PointerGetDatum(key),
                                                PointerGetDatum(query)));
      break;
    case RTContainsStrategyNumber:
    case RTOldContainsStrategyNumber:
      retval = DatumGetBool(DirectFunctionCall2(
          box_contain, PointerGetDatum(key), PointerGetDatum(query)));
      break;
    case RTContainedByStrategyNumber:
    case RTOldContainedByStrategyNumber:
      retval = DatumGetBool(DirectFunctionCall2(
          box_contained, PointerGetDatum(key), PointerGetDatum(query)));
      break;
    case RTOverBelowStrategyNumber:
      retval = DatumGetBool(DirectFunctionCall2(
          box_overbelow, PointerGetDatum(key), PointerGetDatum(query)));
      break;
    case RTBelowStrategyNumber:
      retval = DatumGetBool(DirectFunctionCall2(box_below, PointerGetDatum(key),
                                                PointerGetDatum(query)));
      break;
    case RTAboveStrategyNumber:
      retval = DatumGetBool(DirectFunctionCall2(box_above, PointerGetDatum(key),
                                                PointerGetDatum(query)));
      break;
    case RTOverAboveStrategyNumber:
      retval = DatumGetBool(DirectFunctionCall2(
          box_overabove, PointerGetDatum(key), PointerGetDatum(query)));
      break;
    default:
...
static double size_box(BOX *box) {
  if (box->high.x <= box->low.x || box->high.y <= box->low.y) return 0.0;
...
static bool rtree_internal_consistent(BOX *key, BOX *query,
  ...
  switch (strategy) {
    ...
    case RTOverLeftStrategyNumber:
      retval = !DatumGetBool(DirectFunctionCall2(
          box_right, PointerGetDatum(key), PointerGetDatum(query)));
      break;
    case RTOverlapStrategyNumber:
      retval = DatumGetBool(DirectFunctionCall2(
          box_overlap, PointerGetDatum(key), PointerGetDatum(query)));
    ...
    case RTOverBelowStrategyNumber:
      retval = !DatumGetBool(DirectFunctionCall2(
          box_above, PointerGetDatum(key), PointerGetDatum(query)));
      break;
    case RTBelowStrategyNumber:
      retval = !DatumGetBool(DirectFunctionCall2(
          box_overabove, PointerGetDatum(key), PointerGetDatum(query)));
    ...
    case RTOverAboveStrategyNumber:
      retval = !DatumGetBool(DirectFunctionCall2(
          box_below, PointerGetDatum(key), PointerGetDatum(query)));
      break;
    default:
      elog(ERROR, "unrecognized strategy number: %d", strategy);
      retval = false; /* keep compiler quiet */
...
Datum gist_point_compress(PG_FUNCTION_ARGS) {
  ...
  if (entry->leafkey) /* Point, actually */
  {
    ...
    gistentryinit(*retval, BoxPGetDatum(box), entry->rel, entry->page,
                  entry->offset, FALSE);
...
#define point_point_distance(p1, p2)                                     \
  DatumGetFloat8(DirectFunctionCall2(point_distance, PointPGetDatum(p1), \
                                     PointPGetDatum(p2)))
...
static bool gist_point_consistent_internal(StrategyNumber strategy, bool isLeaf,
                                           BOX *key, Point *query) {
  bool result = false;
  ...
  switch (strategy) {
    ...
    case RTSameStrategyNumber:
      if (isLeaf) {
        ...
        result = (FPle(query->x, key->high.x) && FPge(query->x, key->low.x) &&
                  FPle(query->y, key->high.y) && FPge(query->y, key->low.y));
      }
      break;
    default:
      elog(ERROR, "unrecognized strategy number: %d", strategy);
  ...
  switch (strategyGroup) {
    case PointStrategyNumberGroup:
      result = gist_point_consistent_internal(
          strategy % GeoStrategyNumberOffset, GIST_LEAF(entry),
    ...
    case BoxStrategyNumberGroup: {
      ...
      result = (key->high.x >= query->low.x && key->low.x <= query->high.x &&
                key->high.y >= query->low.y && key->low.y <= query->high.y);
      *recheck = false;
    } break;
    case PolygonStrategyNumberGroup: {
      ...
      result = DatumGetBool(DirectFunctionCall5(
          gist_poly_consistent, PointerGetDatum(entry), PolygonPGetDatum(query),
          Int16GetDatum(RTOverlapStrategyNumber), 0, PointerGetDatum(recheck)));
    ...
    case CircleStrategyNumberGroup: {
      ...
      result = DatumGetBool(DirectFunctionCall5(
          gist_circle_consistent, PointerGetDatum(entry),
...
Datum gist_point_distance(PG_FUNCTION_ARGS) {
  ...
  switch (strategyGroup) {
    case PointStrategyNumberGroup:
      distance = computeDistance(GIST_LEAF(entry), DatumGetBoxP(entry->key),
                                 PG_GETARG_POINT_P(1));
      break;
    default:
      elog(ERROR, "unrecognized strategy number: %d", strategy);
 ...
 * type.
 */
Datum gist_bbox_distance(PG_FUNCTION_ARGS) {
  GISTENTRY *entry = (GISTENTRY *)PG_GETARG_POINTER(0);
  StrategyNumber strategy = (StrategyNumber)PG_GETARG_UINT16(2);
  bool *recheck = (bool *)PG_GETARG_POINTER(4);
  double distance;
  ...
  *recheck = true;
  ...
  switch (strategyGroup) {
    case PointStrategyNumberGroup:
      distance = computeDistance(false, DatumGetBoxP(entry->key),
                                 PG_GETARG_POINT_P(1));
      break;
    default:
