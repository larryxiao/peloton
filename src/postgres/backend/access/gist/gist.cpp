...
static bool gistinserttuple(GISTInsertState *state, GISTInsertStack *stack,
                            GISTSTATE *giststate, IndexTuple tuple,
                            OffsetNumber oldoffnum);
static bool gistinserttuples(GISTInsertState *state, GISTInsertStack *stack,
                             GISTSTATE *giststate, IndexTuple *tuples, int ntup,
                             OffsetNumber oldoffnum, Buffer leftchild,
                             Buffer rightchild, bool unlockbuf,
...
#define ROTATEDIST(d)                                           \
  do {                                                          \
    SplitedPageLayout *tmp =                                    \
        (SplitedPageLayout *)palloc(sizeof(SplitedPageLayout)); \
...
MemoryContext createTempGistContext(void) {
  return AllocSetContextCreate(
      CurrentMemoryContext, "GiST temporary context", ALLOCSET_DEFAULT_MINSIZE,
...
Datum gistinsert(PG_FUNCTION_ARGS) {
  ...
  itup = gistFormTuple(giststate, r, values, isnull,
  ...
  *splitinfo = NIL;
  ...
  if (is_split) {
    ...
    if (is_rootsplit) {
      ...
      for (ptr = dist; ptr; ptr = ptr->next) ndownlinks++;
      downlinks = static_cast<IndexTupleData **>(
          palloc(sizeof(IndexTuple) * ndownlinks));
      for (i = 0, ptr = dist; ptr; ptr = ptr->next) downlinks[i++] = ptr->itup;
      ...
      for (ptr = dist; ptr; ptr = ptr->next) {
        GISTPageSplitInfo *si =
            static_cast<GISTPageSplitInfo *>(palloc(sizeof(GISTPageSplitInfo)));
    ...
    for (ptr = dist; ptr; ptr = ptr->next) {
      ...
      for (i = 0; i < ptr->block.num; i++) {
        ...
        if (PageAddItem(ptr->page, (Item)data, IndexTupleSize(thistup),
                        i + FirstOffsetNumber, false,
     ...
     */
    if (RelationNeedsWAL(rel)) XLogEnsureRecordSpace(npage, 1 + npage * 2);
     ...
     */
    for (ptr = dist; ptr; ptr = ptr->next) MarkBufferDirty(ptr->buffer);
    if (BufferIsValid(leftchildbuf)) MarkBufferDirty(leftchildbuf);
    ...
    if (RelationNeedsWAL(rel))
      recptr = gistXLogSplit(rel->rd_node, blkno, is_leaf, dist, oldrlink,
                             oldnsn, leftchildbuf, markfollowright);
    else
    ...
    if (OffsetNumberIsValid(oldoffnum)) PageIndexTupleDelete(page, oldoffnum);
    ...
    if (RelationNeedsWAL(rel)) {
      ...
      recptr = gistXLogUpdate(rel->rd_node, buffer, deloffs, ndeloffs, itup,
                              ntup, leftchildbuf);
  ...
  for (;;) {
    ...
    if (GistFollowRight(stack->page)) {
      if (!xlocked) {
        ...
        if (!GistFollowRight(stack->page)) continue;
      }
    ...
    if (!GistPageIsLeaf(stack->page)) {
      ...
      for (; stack; stack = stack->parent) ReleaseBuffer(stack->buffer);
      break;
    }
  }
}
 ...
 */
static IndexTuple gistformdownlink(Relation rel, Buffer buf,
                                   GISTSTATE *giststate,
                                   GISTInsertStack *stack) {
  ...
  maxoff = PageGetMaxOffsetNumber(page);
  for (offset = FirstOffsetNumber; offset <= maxoff;
       offset = OffsetNumberNext(offset)) {
    IndexTuple ituple =
        (IndexTuple)PageGetItem(page, PageGetItemId(page, offset));
    ...
    if (downlink == NULL)
      ...
      if (newdownlink) downlink = newdownlink;
    }
  }
...
static bool gistinserttuples(GISTInsertState *state, GISTInsertStack *stack,
   ...
   * parent.
   */
  if (BufferIsValid(rightchild)) UnlockReleaseBuffer(rightchild);
...
static void gistfinishsplit(GISTInsertState *state, GISTInsertStack *stack,
  ...
  reversed = NIL;
  foreach (lc, splitinfo) { reversed = lcons(lfirst(lc), reversed); }
  ...
  while (list_length(reversed) > 2) {
    ...
    if (gistinserttuples(state, stack->parent, giststate, &right->downlink, 1,
                         InvalidOffsetNumber, left->buf, right->buf, false,
                         false)) {
      /*
 ...
 */
SplitedPageLayout *gistSplit(Relation r, Page page,
  ...
  if (!gistfitpage(rvectup, v.splitVector.spl_nright)) {
    ...
    res->list =
        gistfillitupvec(rvectup, v.splitVector.spl_nright, &(res->lenlist));
  ...
  if (!gistfitpage(lvectup, v.splitVector.spl_nleft)) {
    ...
    resptr = subres =
        gistSplit(r, page, lvectup, v.splitVector.spl_nleft, giststate);
    ...
    res->list =
        gistfillitupvec(lvectup, v.splitVector.spl_nleft, &(res->lenlist));
...
GISTSTATE *initGISTstate(Relation index) {
  ...
  if (index->rd_att->natts > INDEX_MAX_KEYS)
    elog(ERROR, "numberOfAttributes %d > %d", index->rd_att->natts,
         INDEX_MAX_KEYS);

  /* Create the memory context that will hold the GISTSTATE */
  scanCxt = AllocSetContextCreate(
      CurrentMemoryContext, "GiST scan context", ALLOCSET_DEFAULT_MINSIZE,
      ALLOCSET_DEFAULT_INITSIZE, ALLOCSET_DEFAULT_MAXSIZE);
  ...
  for (i = 0; i < index->rd_att->natts; i++) {
    ...
    if (OidIsValid(index_getprocid(index, i + 1, GIST_FETCH_PROC)))
      fmgr_info_copy(&(giststate->fetchFn[i]),
                     index_getprocinfo(index, i + 1, GIST_FETCH_PROC), scanCxt);
    else
