 ...
 * gistunionsubkey.
 */
static void gistunionsubkeyvec(GISTSTATE *giststate, IndexTuple *itvec,
                               GistSplitUnion *gsvp) {
  ...
  for (i = 0; i < gsvp->len; i++) {
    if (gsvp->dontcare && gsvp->dontcare[gsvp->entries[i]]) continue;

    cleanedItVec[cleanedLen++] = itvec[gsvp->entries[i] - 1];
  }

  gistMakeUnionItVec(giststate, cleanedItVec, cleanedLen, gsvp->attr,
                     gsvp->isnull);
 ...
 */
static void gistunionsubkey(GISTSTATE *giststate, IndexTuple *itvec,
                            GistSplitVector *spl) {
  GistSplitUnion gsvp;
 ...
 */
static int findDontCares(Relation r, GISTSTATE *giststate, GISTENTRY *valvec,
   ...
   * check for nulls
   */
  gistentryinit(entry, spl->splitVector.spl_rdatum, r, NULL, (OffsetNumber)0,
                FALSE);
  for (i = 0; i < spl->splitVector.spl_nleft; i++) {
    int j = spl->splitVector.spl_left[i];
    float penalty =
        gistpenalty(giststate, attno, &entry, false, &valvec[j], false);
  ...
  gistentryinit(entry, spl->splitVector.spl_ldatum, r, NULL, (OffsetNumber)0,
                FALSE);
 ...
 * at attno.
 */
static void placeOne(Relation r, GISTSTATE *giststate, GistSplitVector *v,
  ...
  for (; attno < giststate->tupdesc->natts; attno++) {
    ...
    if (lpenalty != rpenalty) {
      if (lpenalty > rpenalty) toLeft = false;
      break;
    }
  }
 ...
 */
static void supportSecondarySplit(Relation r, GISTSTATE *giststate, int attno,
  ...
  if (sv->spl_ldatum_exists && sv->spl_rdatum_exists) {
    ...
    if (penalty1 > penalty2) leaveOnLeft = false;
  } else {
 ...
 */
static void genericPickSplit(GISTSTATE *giststate, GistEntryVector *entryvec,
   ...
   */
  evec = static_cast<GistEntryVector *>(
      palloc(sizeof(GISTENTRY) * entryvec->n + GEVHDRSZ));

  evec->n = v->spl_nleft;
  memcpy(evec->vector, entryvec->vector + FirstOffsetNumber,
         sizeof(GISTENTRY) * evec->n);
  v->spl_ldatum = FunctionCall2Coll(
      &giststate->unionFn[attno], giststate->supportCollation[attno],
 ...
 */
static bool gistUserPicksplit(Relation r, GistEntryVector *entryvec, int attno,
  ...
  FunctionCall2Coll(&giststate->picksplitFn[attno],
                    ...
                    PointerGetDatum(entryvec), PointerGetDatum(sv));
  ...
  if (sv->spl_nleft == 0 || sv->spl_nright == 0) {
    ...
    ereport(DEBUG1,
            (errcode(ERRCODE_INTERNAL_ERROR),
             ...
             errhint(
                 "The index is not optimal. To optimize it, contact a "
                 "developer, or try to use the column as the second one in the "
  ...
  if (sv->spl_ldatum_exists || sv->spl_rdatum_exists)
    supportSecondarySplit(r, giststate, attno, sv, v->spl_lattr[attno],
                          v->spl_rattr[attno]);
    ...
    if (NumDontCare > 0) {
      ...
      if (NumDontCare == 1) {
        ...
        for (toMove = FirstOffsetNumber; toMove < entryvec->n; toMove++) {
          if (v->spl_dontcare[toMove]) break;
        }
 ...
 */
void gistSplitByKey(Relation r, Page page, IndexTuple *itup, int len,
  ...
  entryvec = static_cast<GistEntryVector *>(
      palloc(GEVHDRSZ + (len + 1) * sizeof(GISTENTRY)));
  entryvec->n = len + 1;
  ...
  for (i = 1; i <= len; i++) {
    ...
    gistdentryinit(giststate, attno, &(entryvec->vector[i]), datum, r, page, i,
                   FALSE, IsNull);
    if (IsNull) offNullTuples[nOffNullTuples++] = i;
  }
  ...
  if (nOffNullTuples == len) {
    ...
    v->splitVector.spl_left =
        (OffsetNumber *)palloc(len * sizeof(OffsetNumber));
    ...
    if (gistUserPicksplit(r, entryvec, attno, v, itup, len, giststate)) {
      ...
      if (v->spl_dontcare == NULL) {
        ...
        backupSplit.spl_left =
            (OffsetNumber *)palloc(sizeof(OffsetNumber) * len);
