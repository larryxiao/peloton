...
static void gistBuildCallback(Relation index, HeapTuple htup, Datum *values,
...
static void gistInitBuffering(GISTBuildState *buildstate) {
   ...
   */
  itupAvgSize =
      (double)buildstate->indtuplesSize / (double)buildstate->indtuples;
  ...
  for (;;) {
    ...
    if (subtreesize > effective_cache_size / 4) break;
  ...
  buildstate->gfbb =
      gistInitBuildBuffers(pagesPerBuffer, levelStep, gistGetMaxLevel(index));
...
static bool gistProcessItup(GISTBuildState *buildstate, IndexTuple itup,
  ...
  for (;;) {
    ...
    if (LEVEL_HAS_BUFFERS(level, gfbb) && level != startlevel) break;
    ...
    if (level > 1) gistMemorizeParent(buildstate, childblkno, blkno);
  ...
  if (LEVEL_HAS_BUFFERS(level, gfbb)) {
    ...
    if (BUFFER_OVERFLOWED(childNodeBuffer, gfbb)) result = true;
  } else {
    /*
 ...
 *
 * Returns the block number of the page where the (first) new___ or updated
 *tuple
...
static BlockNumber gistbufferinginserttuples(GISTBuildState *buildstate,
  ...
  is_split =
      gistplacetopage(buildstate->indexrel, buildstate->freespace,
                      buildstate->giststate, buffer, itup, ntup, oldoffnum,
                      &placed_to_blk, InvalidBuffer, &splitinfo, false);
  ...
  if (splitinfo) {
    ...
    parentBuffer =
        gistBufferingFindCorrectParent(buildstate, BufferGetBlockNumber(buffer),
                                       level, &parentblk, &downlinkoffnum);
    ...
    gistRelocateBuildBuffersOnSplit(gfbb, buildstate->giststate,
                                    buildstate->indexrel, level, buffer,
                                    splitinfo);
    ...
    foreach (lc, splitinfo) {
      GISTPageSplitInfo *splitinfo =
          static_cast<GISTPageSplitInfo *>(lfirst(lc));
       ...
       * harm).
       */
      if (level > 1) gistMemorizeAllDownlinks(buildstate, splitinfo->buf);
    ...
    gistbufferinginserttuples(buildstate, parentBuffer, level + 1, downlinks,
                              ndownlinks, downlinkoffnum, InvalidBlockNumber,
                              InvalidOffsetNumber);
...
static void gistProcessEmptyingQueue(GISTBuildState *buildstate) {
  ...
  while (gfbb->bufferEmptyingQueue != NIL) {
    ...
    while (true) {
      ...
      if (!gistPopItupFromNodeBuffer(gfbb, emptyingNodeBuffer, &itup)) break;
...
static int gistGetMaxLevel(Relation index) {
  ...
  while (true) {
     ...
     */
    itup =
        (IndexTuple)PageGetItem(page, PageGetItemId(page, FirstOffsetNumber));
...
static void gistMemorizeParent(GISTBuildState *buildstate, BlockNumber child,
  ...
  entry = (ParentMapEntry *)hash_search(
      buildstate->parentMap, (const void *)&child, HASH_ENTER, &found);
...
static BlockNumber gistGetParent(GISTBuildState *buildstate,
  ...
  entry = (ParentMapEntry *)hash_search(
      buildstate->parentMap, (const void *)&child, HASH_FIND, &found);
  if (!found)
