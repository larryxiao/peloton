...
static int pairingheap_GISTSearchItem_cmp(const pairingheap_node *a,
  ...
  if (GISTSearchItemIsHeap(*sa) && !GISTSearchItemIsHeap(*sb)) return 1;
  if (!GISTSearchItemIsHeap(*sa) && GISTSearchItemIsHeap(*sb)) return -1;
...
Datum gistbeginscan(PG_FUNCTION_ARGS) {
  ...
  so->distances =
      static_cast<double *>(palloc(sizeof(double) * scan->numberOfOrderBys));
  so->qual_ok = true; /* in case there are zero keys */
  if (scan->numberOfOrderBys > 0) {
    scan->xs_orderbyvals =
        static_cast<Datum *>(palloc0(sizeof(Datum) * scan->numberOfOrderBys));
    scan->xs_orderbynulls =
        static_cast<bool *>(palloc(sizeof(bool) * scan->numberOfOrderBys));
...
Datum gistrescan(PG_FUNCTION_ARGS) {
  ...
  if (so->queue == NULL) {
    ...
    so->queueCxt = AllocSetContextCreate(
        so->giststate->scanCxt, "GiST queue context", ALLOCSET_DEFAULT_MINSIZE,
  ...
  if (scan->xs_want_itup && !scan->xs_itupdesc) {
    ...
    for (attno = 1; attno <= natts; attno++) {
      TupleDescInitEntry(so->giststate->fetchTupdesc, attno, NULL,
                         scan->indexRelation->rd_opcintype[attno - 1], -1, 0);
    }
    scan->xs_itupdesc = so->giststate->fetchTupdesc;

    so->pageDataCxt = AllocSetContextCreate(
        so->giststate->scanCxt, "GiST page data context",
  ...
  if (key && scan->numberOfKeys > 0) {
    ...
    for (i = 0; i < scan->numberOfKeys; i++) {
      ...
      if (!first_time) skey->sk_func.fn_extra = fn_extras[i];
