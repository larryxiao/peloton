 ...
 */
GISTBuildBuffers *gistInitBuildBuffers(int pagesPerBuffer, int levelStep,
                                       int maxLevel) {
  ...
  gfbb->buffersOnLevels =
      (List **)palloc(sizeof(List *) * gfbb->buffersOnLevelsLen);
...
GISTNodeBuffer *gistGetNodeBuffer(GISTBuildBuffers *gfbb, GISTSTATE *giststate,
  ...
  nodeBuffer = (GISTNodeBuffer *)hash_search(
      gfbb->nodeBuffersTab, (const void *)&nodeBlocknum, HASH_ENTER, &found);
  if (!found) {
    /*
    ...
    gfbb->buffersOnLevels[level] =
        lcons(nodeBuffer, gfbb->buffersOnLevels[level]);
...
static void gistAddLoadedBuffer(GISTBuildBuffers *gfbb,
  ...
  if (nodeBuffer->isTemp) return;

  /* Enlarge the array if needed */
  if (gfbb->loadedBuffersCount >= gfbb->loadedBuffersLen) {
    gfbb->loadedBuffersLen *= 2;
    gfbb->loadedBuffers = (GISTNodeBuffer **)repalloc(
        gfbb->loadedBuffers, gfbb->loadedBuffersLen * sizeof(GISTNodeBuffer *));
  }
...
static void gistGetItupFromPage(GISTNodeBufferPage *pageBuffer,
  ...
  ptr = (IndexTuple)((char *)pageBuffer + BUFFER_PAGE_DATA_OFFSET +
                     PAGE_FREE_SPACE(pageBuffer));
...
void gistPushItupToNodeBuffer(GISTBuildBuffers *gfbb,
  ...
  if (!nodeBuffer->pageBuffer) gistLoadNodeBuffer(gfbb, nodeBuffer);
...
bool gistPopItupFromNodeBuffer(GISTBuildBuffers *gfbb,
   ...
   */
  if (nodeBuffer->blocksCount <= 0) return false;
...
static void gistBuffersReleaseBlock(GISTBuildBuffers *gfbb, long blocknum) {
  ...
  if (gfbb->nFreeBlocks >= gfbb->freeBlocksLen) {
    ...
    gfbb->freeBlocks =
        (long *)repalloc(gfbb->freeBlocks, gfbb->freeBlocksLen * sizeof(long));
  }
...
void gistRelocateBuildBuffersOnSplit(GISTBuildBuffers *gfbb,
  ...
  if (!LEVEL_HAS_BUFFERS(level, gfbb)) return;

  /*
   * Get the node buffer of the splitted page.
   */
  blocknum = BufferGetBlockNumber(buffer);
  nodeBuffer = static_cast<GISTNodeBuffer *>(
      hash_search(gfbb->nodeBuffersTab, &blocknum, HASH_FIND, &found));
  if (!found) {
  ...
  foreach (lc, splitinfo) {
    ...
    gistDeCompressAtt(giststate, r, si->downlink, NULL, (OffsetNumber)0,
  ...
  while (gistPopItupFromNodeBuffer(gfbb, &oldBuf, &itup)) {
    ...
    for (i = 0; i < splitPagesCount; i++) {
      ...
      for (j = 0; j < r->rd_att->natts; j++) {
        ...
        usize = gistpenalty(giststate, j, &splitPageInfo->entry[j],
                            splitPageInfo->isnull[j], &entry[j], isnull[j]);
    ...
    newtup = gistgetadjusted(r, targetBufferInfo->splitinfo->downlink, itup,
                             giststate);
    if (newtup) {
      gistDeCompressAtt(giststate, r, newtup, NULL, (OffsetNumber)0,
