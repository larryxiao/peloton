...
void gistfillbuffer(Page page, IndexTuple *itup, int len, OffsetNumber off) {
  ...
  for (i = 0; i < len; i++) {
    ...
    if (l == InvalidOffsetNumber)
      elog(ERROR,
           "failed to add item to GiST index page, item %d out of %d, size %d "
           "bytes",
           i, len, (int)sz);
    off++;
  }
 ...
 */
bool gistnospace(Page page, IndexTuple *itvec, int len, OffsetNumber todelete,
                 Size freespace) {
 ...
 */
IndexTuple *gistjoinvector(IndexTuple *itvec, int *len, IndexTuple *additvec,
                           int addlen) {
  itvec = (IndexTuple *)repalloc((void *)itvec,
                                 sizeof(IndexTuple) * ((*len) + addlen));
...
IndexTupleData *gistfillitupvec(IndexTuple *vec, int veclen, int *memlen) {
  ...
  *memlen = 0;
  ...
  for (i = 0; i < veclen; i++) *memlen += IndexTupleSize(vec[i]);
...
void gistMakeUnionItVec(GISTSTATE *giststate, IndexTuple *itvec, int len,
  ...
  for (i = 0; i < giststate->tupdesc->natts; i++) {
    ...
    for (j = 0; j < len; j++) {
      ...
      gistdentryinit(giststate, i, evec->vector + evec->n, datum, NULL, NULL,
                     (OffsetNumber)0, FALSE, IsNull);
      evec->n++;
    }
    ...
    if (evec->n == 0) {
      ...
      attr[i] = FunctionCall2Coll(
          &giststate->unionFn[i], giststate->supportCollation[i],
          PointerGetDatum(evec), PointerGetDatum(&attrsize));
 ...
 */
IndexTuple gistunion(Relation r, IndexTuple *itvec, int len,
                     GISTSTATE *giststate) {
 ...
 */
void gistMakeUnionKey(GISTSTATE *giststate, int attno, GISTENTRY *entry1,
                      bool isnull1, GISTENTRY *entry2, bool isnull2, Datum *dst,
                      bool *dstisnull) {
  ...
  if (isnull1 && isnull2) {
    ...
    *dst = FunctionCall2Coll(&giststate->unionFn[attno],
                             ...
                             PointerGetDatum(evec), PointerGetDatum(&dstsize));
  }
}
 ...
 */
void gistDeCompressAtt(GISTSTATE *giststate, Relation r, IndexTuple tuple,
                       Page p, OffsetNumber o, GISTENTRY *attdata,
                       bool *isnull) {
  int i;
 ...
 */
IndexTuple gistgetadjusted(Relation r, IndexTuple oldtup, IndexTuple addtup,
                           GISTSTATE *giststate) {
  bool neednew = FALSE;
  ...
  gistDeCompressAtt(giststate, r, oldtup, NULL, (OffsetNumber)0, oldentries,
                    oldisnull);

  gistDeCompressAtt(giststate, r, addtup, NULL, (OffsetNumber)0, addentries,
                    addisnull);

  for (i = 0; i < r->rd_att->natts; i++) {
    gistMakeUnionKey(giststate, i, oldentries + i, oldisnull[i], addentries + i,
                     addisnull[i], attr + i, isnull + i);

    if (neednew) /* we already need new___ key, so we can skip check */
      continue;
 ...
 */
OffsetNumber gistchoose(Relation r, Page p,
  ...
  for (i = FirstOffsetNumber; i <= maxoff; i = OffsetNumberNext(i)) {
    ...
    for (j = 0; j < r->rd_att->natts; j++) {
      ...
      if (best_penalty[j] < 0 || usize < best_penalty[j]) {
        ...
        if (j < r->rd_att->natts - 1) best_penalty[j + 1] = -1;
 ...
 */
void gistdentryinit(GISTSTATE *giststate, int nkey, GISTENTRY *e, Datum k,
                    Relation r, Page pg, OffsetNumber o, bool l, bool isNull) {
  if (!isNull) {
    GISTENTRY *dep;

    gistentryinit(*e, k, r, pg, o, l);
    dep = (GISTENTRY *)DatumGetPointer(FunctionCall1Coll(
        &giststate->decompressFn[nkey], giststate->supportCollation[nkey],
        PointerGetDatum(e)));
...
IndexTuple gistFormTuple(GISTSTATE *giststate, Relation r, Datum attdata[],
                         bool isnull[], bool isleaf) {
  ...
  for (i = 0; i < r->rd_att->natts; i++) {
    if (isnull[i])
      ...
      cep = (GISTENTRY *)DatumGetPointer(FunctionCall1Coll(
          &giststate->compressFn[i], giststate->supportCollation[i],
...
static Datum gistFetchAtt(GISTSTATE *giststate, int nkey, Datum k, Relation r) {
  ...
  fep = (GISTENTRY *)DatumGetPointer(FunctionCall1Coll(
      &giststate->fetchFn[nkey], giststate->supportCollation[nkey],
...
float gistpenalty(GISTSTATE *giststate, int attno, GISTENTRY *orig,
                  bool isNullOrig, GISTENTRY *add, bool isNullAdd) {
  float penalty = 0.0;

  if (giststate->penaltyFn[attno].fn_strict == FALSE ||
      (isNullOrig == FALSE && isNullAdd == FALSE)) {
    FunctionCall3Coll(&giststate->penaltyFn[attno],
                      giststate->supportCollation[attno], PointerGetDatum(orig),
                      PointerGetDatum(add), PointerGetDatum(&penalty));
    /* disallow negative or NaN penalty */
    if (isnan(penalty) || penalty < 0.0) penalty = 0.0;
...
Buffer gistNewBuffer(Relation r) {
  ...
  for (;;) {
    ...
    if (blkno == InvalidBlockNumber) break; /* nothing left in FSM */
    ...
    if (ConditionalLockBuffer(buffer)) {
      ...
      if (PageIsNew(page)) return buffer; /* OK to use, if never initialized */

      gistcheckpage(r, buffer);

      if (GistPageIsDeleted(page)) return buffer; /* OK to use */
  ...
  if (needLock) LockRelationForExtension(r, ExclusiveLock);

  buffer = ReadBuffer(r, P_NEW);
  LockBuffer(buffer, GIST_EXCLUSIVE);

  if (needLock) UnlockRelationForExtension(r, ExclusiveLock);
...
Datum gistoptions(PG_FUNCTION_ARGS) {
  ...
  static const relopt_parse_elt tab[] = {
      ...
      {"buffering", RELOPT_TYPE_STRING,
       offsetof(GiSTOptions, bufferingModeOffset)}};

  options =
      parseRelOptions(reloptions, validate, RELOPT_KIND_GIST, &numoptions);

  /* if none set, we're done */
  if (numoptions == 0) PG_RETURN_NULL();

  rdopts = static_cast<GiSTOptions *>(
