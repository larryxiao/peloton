...
static void pushStackIfSplited(Page page, GistBDItem *stack) {
  ...
  if (stack->blkno != GIST_ROOT_BLKNO &&
      !XLogRecPtrIsInvalid(stack->parentlsn) &&
  ...
  while (stack) {
    ...
    if (GistPageIsLeaf(page)) {
      ...
      if (ntodelete) {
        ...
        for (i = 0; i < ntodelete; i++) PageIndexTupleDelete(page, todelete[i]);
        GistMarkTuplesDeleted(page);

        if (RelationNeedsWAL(rel)) {
          XLogRecPtr recptr;

          recptr = gistXLogUpdate(rel->rd_node, buffer, todelete, ntodelete,
