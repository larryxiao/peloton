...
static void resetSpGistScanOpaque(SpGistScanOpaque so) {
  ...
  if (so->want_itup) {
    ...
    for (i = 0; i < so->nPtrs; i++) pfree(so->indexTups[i]);
  }
  so->iPtr = so->nPtrs = 0;
}
...
static void spgPrepareScanKeys(IndexScanDesc scan) {
  ...
  if (haveIsNull && haveNotNull) qual_ok = false;
...
Datum spgbeginscan(PG_FUNCTION_ARGS) {
  ...
  so->tempCxt = AllocSetContextCreate(
      CurrentMemoryContext, "SP-GiST search temporary context",
 ...
 */
static bool spgLeafTest(Relation index, SpGistScanOpaque so,
                        SpGistLeafTuple leafTuple, bool isnull, int level,
                        Datum reconstructedValue, Datum *leafValue,
                        bool *recheck) {
  bool result;
  Datum leafDatum;
  ...
  result = DatumGetBool(FunctionCall2Coll(procinfo, index->rd_indcollation[0],
                                          PointerGetDatum(&in),
                                          PointerGetDatum(&out)));
 ...
 */
static void spgWalk(Relation index, SpGistScanOpaque so, bool scanWholeIndex,
  ...
  while (scanWholeIndex || !reportedSome) {
    ...
    if (so->scanStack == NIL) break; /* there are no more pages to scan */
  ...
  redirect:
    ...
    if (SpGistPageIsLeaf(page)) {
      ...
      if (SpGistBlockIsRoot(blkno)) {
        ...
        for (offset = FirstOffsetNumber; offset <= max; offset++) {
          leafTuple =
              (SpGistLeafTuple)PageGetItem(page, PageGetItemId(page, offset));
          ...
          if (spgLeafTest(index, so, leafTuple, isnull, stackEntry->level,
                          stackEntry->reconstructedValue, &leafValue,
                          &recheck)) {
        ...
        while (offset != InvalidOffsetNumber) {
          ...
          if (leafTuple->tupstate != SPGIST_LIVE) {
            if (leafTuple->tupstate == SPGIST_REDIRECT) {
              ...
              Assert(ItemPointerGetBlockNumber(&stackEntry->ptr) !=
    ...
    } else /* page is inner */
    {
      ...
      if (!isnull) {
        ...
        for (i = 0; i < in.nNodes; i++) out.nodeNumbers[i] = i;
      }
      ...
      if (innerTuple->allTheSame)
        if (out.nNodes != 0 && out.nNodes != in.nNodes)
          elog(ERROR,
               "inconsistent inner_consistent results for allTheSame inner "
               "tuple");
      ...
      for (i = 0; i < out.nNodes; i++) {
        ...
        if (ItemPointerIsValid(&nodes[nodeN]->t_tid)) {
          ...
          newEntry =
              static_cast<ScanStackEntry *>(palloc(sizeof(ScanStackEntry)));
          ...
          if (out.reconstructedValues)
            newEntry->reconstructedValue =
                datumCopy(out.reconstructedValues[i],
                          so->state.attType.attbyval, so->state.attType.attlen);
          else
...
static void storeGettuple(SpGistScanOpaque so, ItemPointer heapPtr,
                          Datum leafValue, bool isnull, bool recheck) {
  ...
  if (so->want_itup) {
     ...
     */
    so->indexTups[so->nPtrs] =
        index_form_tuple(so->indexTupDesc, &leafValue, &isnull);
  }
  so->nPtrs++;
}
...
Datum spggettuple(PG_FUNCTION_ARGS) {
  ...
  for (;;) {
    ...
    if (so->nPtrs == 0) break; /* must have completed scan */
  }
