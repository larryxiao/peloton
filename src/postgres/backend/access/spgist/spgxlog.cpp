 ...
 */
static void addOrReplaceTuple(Page page, Item tuple, int size,
                              OffsetNumber offset) {
  if (offset <= PageGetMaxOffsetNumber(page)) {
    SpGistDeadTuple dt =
        (SpGistDeadTuple)PageGetItem(page, PageGetItemId(page, offset));
...
static void spgRedoAddLeaf(XLogReaderState *record) {
    ...
    if (xldata->offnumLeaf != xldata->offnumHeadLeaf) {
      ...
      if (xldata->offnumHeadLeaf != InvalidOffsetNumber) {
        ...
        head = (SpGistLeafTuple)PageGetItem(
            page, PageGetItemId(page, xldata->offnumHeadLeaf));
  ...
  if (xldata->offnumParent != InvalidOffsetNumber) {
    if (XLogReadBufferForRedo(record, 1, &buffer) == BLK_NEEDS_REDO) {
      ...
      tuple = (SpGistInnerTuple)PageGetItem(
          page, PageGetItemId(page, xldata->offnumParent));
...
static void spgRedoMoveLeafs(XLogReaderState *record) {
  ...
  if (XLogReadBufferForRedo(record, 0, &buffer) == BLK_NEEDS_REDO) {
    ...
    spgPageIndexMultiDelete(
        &state, page, toDelete, xldata->nMoves,
  ...
  if (!XLogRecHasBlockRef(record, 1)) {
    ...
    if (action == BLK_NEEDS_REDO) {
      ...
      if (xldata->parentBlk == 1) {
        ...
        parentTuple = (SpGistInnerTuple)PageGetItem(
            page, PageGetItemId(page, xldata->offnumParent));

        spgUpdateNodeLink(parentTuple, xldata->nodeI, blknoNew,
                          xldata->offnumNew);
      }
      PageSetLSN(page, lsn);
      MarkBufferDirty(buffer);
    ...
    if (XLogReadBufferForRedo(record, 0, &buffer) == BLK_NEEDS_REDO) {
      ...
      if (state.isBuild)
        ...
        dt = spgFormDeadTuple(&state, SPGIST_REDIRECT, blknoNew,
                              xldata->offnumNew);

      PageIndexTupleDelete(page, xldata->offnum);
      if (PageAddItem(page, (Item)dt, dt->size, xldata->offnum, false, false) !=
          xldata->offnum)
        elog(ERROR, "failed to add item of size %u to SPGiST index page",
             dt->size);
...
static void spgRedoPickSplit(XLogReaderState *record) {
    ...
    if (XLogReadBufferForRedo(record, 0, &srcBuffer) == BLK_NEEDS_REDO) {
      ...
      if (!state.isBuild)
        spgPageIndexMultiDelete(&state, srcPage, toDelete, xldata->nDelete,
                                SPGIST_REDIRECT, SPGIST_PLACEHOLDER, blknoInner,
                                xldata->offnumInner);
      else
        spgPageIndexMultiDelete(&state, srcPage, toDelete, xldata->nDelete,
  ...
  for (i = 0; i < xldata->nInsert; i++) {
    ...
    if (page == NULL) continue; /* no need to touch this page */
  ...
  if (action == BLK_NEEDS_REDO) {
    ...
    if (xldata->innerIsParent) {
      ...
      parent = (SpGistInnerTuple)PageGetItem(
          page, PageGetItemId(page, xldata->offnumParent));
  ...
  if (BufferIsValid(innerBuffer)) UnlockReleaseBuffer(innerBuffer);
  ...
  if (XLogRecHasBlockRef(record, 3)) {
    ...
    if (BufferIsValid(parentBuffer)) UnlockReleaseBuffer(parentBuffer);
  } else
...
static void spgRedoVacuumLeaf(XLogReaderState *record) {
  ...
  if (XLogReadBufferForRedo(record, 0, &buffer) == BLK_NEEDS_REDO) {
    ...
    spgPageIndexMultiDelete(&state, page, toDead, xldata->nDead, SPGIST_DEAD,
                            SPGIST_DEAD, InvalidBlockNumber,
                            InvalidOffsetNumber);
...
static void spgRedoVacuumRedirect(XLogReaderState *record) {
  ...
  if (XLogReadBufferForRedo(record, 0, &buffer) == BLK_NEEDS_REDO) {
    ...
    for (i = 0; i < xldata->nToPlaceholder; i++) {
      ...
      dt = (SpGistDeadTuple)PageGetItem(
          page, PageGetItemId(page, itemToPlaceholder[i]));
    ...
    if (xldata->firstPlaceholder != InvalidOffsetNumber) {
      ...
      toDelete =
          static_cast<OffsetNumber *>(palloc(sizeof(OffsetNumber) * max));
...
void spg_xlog_startup(void) {
  opCtx =
      AllocSetContextCreate(CurrentMemoryContext, "SP-GiST temporary context",
