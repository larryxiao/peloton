...
static Datum formTextDatum(const char *data, int datalen) {
  ...
  if (datalen + VARHDRSZ_SHORT <= VARATT_SHORT_MAX) {
    ...
    if (datalen) memcpy(p + VARHDRSZ_SHORT, data, datalen);
  } else {
...
Datum spg_text_choose(PG_FUNCTION_ARGS) {
  ...
  if (in->hasPrefix) {
    ...
    commonLen = commonPrefix(inStr + in->level, prefixStr, inSize - in->level,
                             prefixSize);
    ...
    if (commonLen == prefixSize) {
      ...
      if (prefixSize - commonLen == 1) {
        ...
        out->result.splitTuple.postfixPrefixDatum = formTextDatum(
            prefixStr + commonLen + 1, prefixSize - commonLen - 1);
      }
  ...
  if (searchChar(in->nodeLabels, in->nNodes, nodeChar, &i)) {
    ...
    if (inSize - in->level - levelAdd > 0)
      out->result.matchNode.restDatum = formTextDatum(
          inStr + in->level + levelAdd, inSize - in->level - levelAdd);
    else
...
Datum spg_text_picksplit(PG_FUNCTION_ARGS) {
  ...
  for (i = 1; i < in->nTuples && commonLen > 0; i++) {
    ...
    if (tmp < commonLen) commonLen = tmp;
  }
...
Datum spg_text_inner_consistent(PG_FUNCTION_ARGS) {
   ...
   */
  Assert(in->level == 0
             ? DatumGetPointer(in->reconstructedValue) == NULL
             : VARSIZE_ANY_EXHDR(DatumGetPointer(in->reconstructedValue)) ==
                   in->level);
  ...
  if (in->level)
    memcpy(VARDATA(reconstrText),
           VARDATA(DatumGetPointer(in->reconstructedValue)), in->level);
  if (prefixSize)
    memcpy(((char *)VARDATA(reconstrText)) + in->level, VARDATA_ANY(prefixText),
           prefixSize);
...
Datum spg_text_leaf_consistent(PG_FUNCTION_ARGS) {
  ...
  if (VARSIZE_ANY_EXHDR(leafValue) == 0 && level > 0) {
    ...
    if (level) memcpy(fullValue, VARDATA(reconstrValue), level);
  ...
  for (j = 0; j < in->nkeys; j++) {
    ...
    if (strategy > 10) {
      ...
      r = varstr_cmp(fullValue, Min(queryLen, fullLen), VARDATA_ANY(query),
                     Min(queryLen, fullLen), PG_GET_COLLATION());
    } else {
