...
static void spgistBuildCallback(Relation index, HeapTuple htup, Datum *values,
...
Datum spgbuild(PG_FUNCTION_ARGS) {
  ...
  buildstate.tmpCtx = AllocSetContextCreate(
      CurrentMemoryContext, "SP-GiST build temporary context",
...
Datum spgbuildempty(PG_FUNCTION_ARGS) {
  ...
  smgrwrite(index->rd_smgr, INIT_FORKNUM, SPGIST_METAPAGE_BLKNO, (char *)page,
            true);
  if (XLogIsNeeded())
  ...
  smgrwrite(index->rd_smgr, INIT_FORKNUM, SPGIST_ROOT_BLKNO, (char *)page,
            true);
  if (XLogIsNeeded())
    log_newpage(&index->rd_smgr->smgr_rnode.node, INIT_FORKNUM,
  ...
  smgrwrite(index->rd_smgr, INIT_FORKNUM, SPGIST_NULL_BLKNO, (char *)page,
            true);
  if (XLogIsNeeded())
    log_newpage(&index->rd_smgr->smgr_rnode.node, INIT_FORKNUM,
...
Datum spginsert(PG_FUNCTION_ARGS) {
  ...
  insertCtx = AllocSetContextCreate(
      CurrentMemoryContext, "SP-GiST insert temporary context",
      ALLOCSET_DEFAULT_MINSIZE, ALLOCSET_DEFAULT_INITSIZE,
      ALLOCSET_DEFAULT_MAXSIZE);
