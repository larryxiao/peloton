...
Datum spg_quad_picksplit(PG_FUNCTION_ARGS) {
  ...
  for (i = 0; i < in->nTuples; i++) sorted[i] = DatumGetPointP(in->datums[i]);
  ...
  out->leafTupleDatums =
      static_cast<Datum *>(palloc(sizeof(Datum) * in->nTuples));
...
Datum spg_quad_inner_consistent(PG_FUNCTION_ARGS) {
  ...
  for (i = 0; i < in->nkeys; i++) {
    ...
    switch (in->scankeys[i].sk_strategy) {
      case RTLeftStrategyNumber:
        if (SPTEST(point_right, centroid, query)) which &= (1 << 3) | (1 << 4);
        break;
      case RTRightStrategyNumber:
        if (SPTEST(point_left, centroid, query)) which &= (1 << 1) | (1 << 2);
        break;
      case RTSameStrategyNumber:
        which &= (1 << getQuadrant(centroid, query));
        break;
      case RTBelowStrategyNumber:
        if (SPTEST(point_above, centroid, query)) which &= (1 << 2) | (1 << 3);
        break;
      case RTAboveStrategyNumber:
        if (SPTEST(point_below, centroid, query)) which &= (1 << 1) | (1 << 4);
        break;
    ...
    if (which == 0) break; /* no need to consider remaining conditions */
  }
  ...
  out->nNodes = 0;
  for (i = 1; i <= 4; i++) {
    if (which & (1 << i)) out->nodeNumbers[out->nNodes++] = i - 1;
  }
