 ...
 */
static void vacuumLeafPage(spgBulkDeleteState *bds, Relation index,
                           Buffer buffer, bool forPending) {
  ...
  for (i = FirstOffsetNumber; i <= max; i++) {
    ...
    if (lt->tupstate == SPGIST_LIVE) {
      ...
      if (bds->callback(&lt->heapPtr, bds->callback_state)) {
        ...
        if (!forPending) bds->stats->num_index_tuples += 1;
      }
  ...
  if (nDeletable == 0) return; /* nothing more to do */
  ...
  for (i = FirstOffsetNumber; i <= max; i++) {
    ...
    if (head->tupstate != SPGIST_LIVE) continue; /* can't be a chain member */
    if (predecessor[i] != 0) continue;           /* not a chain head */
  ...
  spgPageIndexMultiDelete(&bds->spgstate, page, toDead, xlrec.nDead,
                          SPGIST_DEAD, SPGIST_DEAD, InvalidBlockNumber,
                          InvalidOffsetNumber);

  spgPageIndexMultiDelete(&bds->spgstate, page, toPlaceholder,
                          xlrec.nPlaceholder, SPGIST_PLACEHOLDER,
                          SPGIST_PLACEHOLDER, InvalidBlockNumber,
                          InvalidOffsetNumber);
  ...
  if (RelationNeedsWAL(index)) {
    ...
    XLogRegisterData((char *)toPlaceholder,
                     sizeof(OffsetNumber) * xlrec.nPlaceholder);
 ...
 */
static void vacuumLeafRoot(spgBulkDeleteState *bds, Relation index,
                           Buffer buffer) {
  Page page = BufferGetPage(buffer);
  ...
  if (xlrec.nDelete == 0) return; /* nothing more to do */
...
static void vacuumRedirectAndPlaceholder(Relation index, Buffer buffer) {
  ...
  for (i = max; i >= FirstOffsetNumber &&
    ...
    if (dt->tupstate == SPGIST_PLACEHOLDER) {
      if (!hasNonPlaceholder) firstPlaceholder = i;
    } else {
  ...
  if (firstPlaceholder != InvalidOffsetNumber) {
     ...
     */
    for (i = firstPlaceholder; i <= max; i++) itemnos[i - firstPlaceholder] = i;
  ...
  if (hasUpdate) MarkBufferDirty(buffer);
...
static void spgprocesspending(spgBulkDeleteState *bds) {
  ...
  for (pitem = bds->pendingList; pitem != NULL; pitem = pitem->next) {
    if (pitem->done) continue; /* ignore already-done items */
    ...
    if (PageIsNew(page) || SpGistPageIsDeleted(page)) {
      ...
      for (nitem = pitem->next; nitem != NULL; nitem = nitem->next) {
        if (ItemPointerGetBlockNumber(&nitem->tid) == blkno) nitem->done = true;
      }
    } else {
      /*
      ...
      for (nitem = pitem; nitem != NULL; nitem = nitem->next) {
        ...
        if (ItemPointerGetBlockNumber(&nitem->tid) == blkno) {
          ...
          innerTuple =
              (SpGistInnerTuple)PageGetItem(page, PageGetItemId(page, offset));
...
static bool dummy_callback(ItemPointer itemptr, void *state) { return false; }
