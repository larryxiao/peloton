 ...
 * split operation.
 */
void spgUpdateNodeLink(SpGistInnerTuple tup, int nodeN, BlockNumber blkno,
                       OffsetNumber offset) {
  int i;
...
static SpGistInnerTuple addNode(SpGistState *state, SpGistInnerTuple tuple,
  ...
  nodes = static_cast<SpGistNodeTupleData **>(
      palloc(sizeof(SpGistNodeTuple) * (tuple->nNodes + 1)));
  ...
  return spgFormInnerTuple(state, (tuple->prefixSize > 0),
                           SGITDATUM(tuple, state), tuple->nNodes + 1, nodes);
}

/* qsort comparator for sorting OffsetNumbers */
static int cmpOffsetNumbers(const void *a, const void *b) {
  if (*(const OffsetNumber *)a == *(const OffsetNumber *)b) return 0;
...
void spgPageIndexMultiDelete(SpGistState *state, Page page,
                             OffsetNumber *itemnos, int nitems, int firststate,
                             int reststate, BlockNumber blkno,
                             OffsetNumber offnum) {
  ...
  if (nitems == 0) return; /* nothing to do */
  ...
  for (i = 0; i < nitems; i++) {
    ...
    if (PageAddItem(page, (Item)tuple, tuple->size, itemno, false, false) !=
        itemno)
 ...
 * WAL action).
 */
static void saveNodeLink(Relation index, SPPageDesc *parent, BlockNumber blkno,
                         OffsetNumber offnum) {
  SpGistInnerTuple innerTuple;

  innerTuple = (SpGistInnerTuple)PageGetItem(
      parent->page, PageGetItemId(parent->page, parent->offnum));
 ...
 */
static void addLeafTuple(Relation index, SpGistState *state,
                         SpGistLeafTuple leafTuple, SPPageDesc *current,
  ...
  if (current->offnum == InvalidOffsetNumber ||
    ...
    current->offnum = SpGistPageAddNewItem(
        state, current->page, (Item)leafTuple, leafTuple->size, NULL, false);
    ...
    head = (SpGistLeafTuple)PageGetItem(
        current->page, PageGetItemId(current->page, current->offnum));
    if (head->tupstate == SPGIST_LIVE) {
      leafTuple->nextOffset = head->nextOffset;
      offnum = SpGistPageAddNewItem(state, current->page, (Item)leafTuple,
                                    leafTuple->size, NULL, false);
      ...
      if (PageAddItem(current->page, (Item)leafTuple, leafTuple->size,
 ...
 */
static void moveLeafs(Relation index, SpGistState *state, SPPageDesc *current,
                      SPPageDesc *parent, SpGistLeafTuple newLeafTuple,
                      bool isNulls) {
  ...
  nbuf = SpGistGetBuffer(index, GBUF_LEAF | (isNulls ? GBUF_NULLS : 0), size,
                         &xlrec.newPage);
  ...
  if (!replaceDead) {
    for (i = 0; i < nDelete; i++) {
      it = (SpGistLeafTuple)PageGetItem(
          current->page, PageGetItemId(current->page, toDelete[i]));
      ...
      r = SpGistPageAddNewItem(state, npage, (Item)it, it->size, &startOffset,
                               false);
  ...
  r = SpGistPageAddNewItem(state, npage, (Item)newLeafTuple, newLeafTuple->size,
                           &startOffset, false);
  toInsert[nInsert] = r;
  nInsert++;
  ...
  if (RelationNeedsWAL(index)) {
    ...
    XLogRegisterBuffer(0, current->buffer, REGBUF_STANDARD);
    XLogRegisterBuffer(
        1, nbuf, REGBUF_STANDARD | (xlrec.newPage ? REGBUF_WILL_INIT : 0));
 ...
 * exclude the last tuple (which is the incoming new___ tuple that forced a
 *split)
...
static bool checkAllTheSame(spgPickSplitIn *in, spgPickSplitOut *out,
  ...
  *includeNew = true;
  ...
  for (i = 1; i < limit; i++) {
    if (out->mapTuplesToNodes[i] != theNode) return false;
  }
  ...
  for (i = 0; i < in->nTuples; i++) out->mapTuplesToNodes[i] = i % out->nNodes;
  ...
  if (out->nodeLabels) {
    ...
    for (i = 0; i < out->nNodes; i++) out->nodeLabels[i] = theLabel;
  }
 ...
 */
static bool doPickSplit(Relation index, SpGistState *state, SPPageDesc *current,
                        SPPageDesc *parent, SpGistLeafTuple newLeafTuple,
  ...
  if (!isNulls) {
    ...
    FunctionCall2Coll(procinfo, index->rd_indcollation[0], PointerGetDatum(&in),
                      PointerGetDatum(&out));
    ...
    for (i = 0; i < in.nTuples; i++) {
      newLeafs[i] =
          spgFormLeafTuple(state, heapPtrs + i, out.leafTupleDatums[i], false);
    ...
    out.mapTuplesToNodes =
        static_cast<int *>(palloc0(sizeof(int) * in.nTuples));
   ...
   * fit on one page.
   */
  allTheSame = checkAllTheSame(&in, &out, totalLeafSizes > SPGIST_PAGE_CAPACITY,
                               &includeNew);
  ...
  for (i = 0; i < out.nNodes; i++) {
    ...
    if (!labelisnull) label = out.nodeLabels[i];
  ...
  if (totalLeafSizes <= currentFreeSpace) {
    ...
    newLeafBuffer = SpGistGetBuffer(
        index, GBUF_LEAF | (isNulls ? GBUF_NULLS : 0),
  ...
  if (!SpGistBlockIsRoot(current->blkno)) {
    ...
    if (state->isBuild &&
      ...
      if (!state->isBuild) {
         ...
         */
        if (nToDelete > 0) redirectTuplePos = toDelete[0];
  ...
  for (i = 0; i < nToInsert; i++) {
    ...
    /* Insert it on page */
    newoffset =
        SpGistPageAddNewItem(state, BufferGetPage(leafBuffer), (Item)it,
                             it->size, &startOffsets[leafPageSelect[i]], false);
  ...
  if (newInnerBuffer == parent->buffer && newInnerBuffer != InvalidBuffer) {
    ...
    xlrec.offnumInner = current->offnum = SpGistPageAddNewItem(
        state, current->page, (Item)innerTuple, innerTuple->size, NULL, false);
    ...
    if (redirectTuplePos != InvalidOffsetNumber)
      setRedirectionTuple(&saveCurrent, redirectTuplePos, current->blkno,
                          current->offnum);
...
static void spgMatchNodeAction(Relation index, SpGistState *state,
                               SpGistInnerTuple innerTuple, SPPageDesc *current,
 ...
 */
static void spgAddNodeAction(Relation index, SpGistState *state,
                             SpGistInnerTuple innerTuple, SPPageDesc *current,
  ...
  if (PageGetExactFreeSpace(current->page) >=
    ...
    PageIndexTupleDelete(current->page, current->offnum);
    if (PageAddItem(current->page, (Item)newInnerTuple, newInnerTuple->size,
                    current->offnum, false, false) != current->offnum)
      elog(ERROR, "failed to add item of size %u to SPGiST index page",
           newInnerTuple->size);
    ...
    xlrec.offnumNew = current->offnum =
        SpGistPageAddNewItem(state, current->page, (Item)newInnerTuple,
     ...
     */
    if (state->isBuild)
      dt = spgFormDeadTuple(state, SPGIST_PLACEHOLDER, InvalidBlockNumber,
                            InvalidOffsetNumber);
    else
      dt = spgFormDeadTuple(state, SPGIST_REDIRECT, current->blkno,
                            current->offnum);

    PageIndexTupleDelete(saveCurrent.page, saveCurrent.offnum);
    if (PageAddItem(saveCurrent.page, (Item)dt, dt->size, saveCurrent.offnum,
 ...
 */
static void spgSplitNodeAction(Relation index, SpGistState *state,
                               SpGistInnerTuple innerTuple, SPPageDesc *current,
                               spgChooseOut *out) {
  ...
  prefixTuple =
      spgFormInnerTuple(state, out->result.splitTuple.prefixHasPrefix,
   ...
   */
  nodes = static_cast<SpGistNodeTupleData **>(
      palloc(sizeof(SpGistNodeTuple) * innerTuple->nNodes));
  SGITITERATE(innerTuple, i, node) { nodes[i] = node; }

  postfixTuple = spgFormInnerTuple(
      state, out->result.splitTuple.postfixHasPrefix,
  ...
  PageIndexTupleDelete(current->page, current->offnum);
  xlrec.offnumPrefix =
      PageAddItem(current->page, (Item)prefixTuple, prefixTuple->size,
  ...
  if (newBuffer == InvalidBuffer) {
    ...
    xlrec.offnumPostfix = postfixOffset =
        SpGistPageAddNewItem(state, current->page, (Item)postfixTuple,
                             postfixTuple->size, NULL, false);
    xlrec.postfixBlkSame = true;
  } else {
    postfixBlkno = BufferGetBlockNumber(newBuffer);
    xlrec.offnumPostfix = postfixOffset = SpGistPageAddNewItem(
  ...
  prefixTuple = (SpGistInnerTuple)PageGetItem(
      current->page, PageGetItemId(current->page, current->offnum));
  spgUpdateNodeLink(prefixTuple, 0, postfixBlkno, postfixOffset);
 ...
 */
bool spgdoinsert(Relation index, SpGistState *state, ItemPointer heapPtr,
   ...
   */
  if (!isnull) procinfo = index_getprocinfo(index, 1, SPGIST_CHOOSE_PROC);
  ...
  for (;;) {
    ...
    if (current.blkno == InvalidBlockNumber) {
       ...
       */
      current.buffer =
          SpGistGetBuffer(index, GBUF_LEAF | (isnull ? GBUF_NULLS : 0),
                          Min(leafSize, SPGIST_PAGE_CAPACITY), &isNew);
      current.blkno = BufferGetBlockNumber(current.buffer);
    } else if (parent.buffer == InvalidBuffer) {
    ...
    if (SpGistPageIsLeaf(current.page)) {
      ...
      if (leafTuple->size + sizeof(ItemIdData) <=
        ...
        break;
                      ...
                      checkSplitConditions(index, state, &current, &nToSplit)) <
                     SPGIST_PAGE_CAPACITY / 2 &&
                 nToSplit < 64 &&
                 leafTuple->size + sizeof(ItemIdData) + sizeToSplit <=
        ...
        /* picksplit */
        if (doPickSplit(index, state, &current, &parent, leafTuple, level,
                        isnull, isNew))
