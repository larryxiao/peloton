...
static Datum _bt_find_extreme_element(IndexScanDesc scan, ScanKey skey,
                                      StrategyNumber strat, Datum *elems,
                                      int nelems);
...
static bool _bt_check_rowcompare(ScanKey skey, IndexTuple tuple,
...
ScanKey _bt_mkscankey(Relation rel, IndexTuple itup) {
  ...
  for (i = 0; i < natts; i++) {
    ...
    ScanKeyEntryInitializeWithInfo(&skey[i], flags, (AttrNumber)(i + 1),
...
void _bt_preprocess_array_keys(IndexScanDesc scan) {
  ...
  if (so->arrayContext == NULL)
    so->arrayContext = AllocSetContextCreate(
        CurrentMemoryContext, "BTree Array Context", ALLOCSET_SMALL_MINSIZE,
  ...
  memcpy(so->arrayKeyData, scan->keyData,
         scan->numberOfKeys * sizeof(ScanKeyData));

  /* Allocate space for per-array data in the workspace context */
  so->arrayKeys =
      (BTArrayKeyInfo *)palloc0(numArrayKeys * sizeof(BTArrayKeyInfo));
  ...
  for (i = 0; i < numberOfKeys; i++) {
    ...
    if (!(cur->sk_flags & SK_SEARCHARRAY)) continue;
    ...
    deconstruct_array(arrayval, ARR_ELEMTYPE(arrayval), elmlen, elmbyval,
                      elmalign, &elem_values, &elem_nulls, &num_elems);
    ...
    for (j = 0; j < num_elems; j++) {
      if (!elem_nulls[j]) elem_values[num_nonnulls++] = elem_values[j];
    }
    ...
    switch (cur->sk_strategy) {
      ...
      case BTLessEqualStrategyNumber:
        cur->sk_argument = _bt_find_extreme_element(
            scan, cur, BTGreaterStrategyNumber, elem_values, num_nonnulls);
        continue;
      ...
      case BTGreaterStrategyNumber:
        cur->sk_argument = _bt_find_extreme_element(
            scan, cur, BTLessStrategyNumber, elem_values, num_nonnulls);
        continue;
      default:
    ...
    num_elems = _bt_sort_array_elements(
        scan, cur, (indoption[cur->sk_attno - 1] & INDOPTION_DESC) != 0,
...
static Datum _bt_find_extreme_element(IndexScanDesc scan, ScanKey skey,
  ...
  if (elemtype == InvalidOid) elemtype = rel->rd_opcintype[skey->sk_attno - 1];
   ...
   * at all.
   */
  cmp_op = get_opfamily_member(rel->rd_opfamily[skey->sk_attno - 1], elemtype,
                               elemtype, strat);
  if (!OidIsValid(cmp_op))
    elog(ERROR, "missing operator___ %d(%u,%u) in opfamily %u", strat, elemtype,
  ...
  for (i = 1; i < nelems; i++) {
    if (DatumGetBool(
            FunctionCall2Coll(&flinfo, skey->sk_collation, elems[i], result)))
      result = elems[i];
  }
...
static int _bt_sort_array_elements(IndexScanDesc scan, ScanKey skey,
  ...
  if (nelems <= 1) return nelems; /* no work to do */
   ...
   * all.
   */
  cmp_proc = get_opfamily_proc(rel->rd_opfamily[skey->sk_attno - 1], elemtype,
                               elemtype, BTORDER_PROC);
  if (!RegProcedureIsValid(cmp_proc))
  ...
  cxt.reverse = reverse;
  qsort_arg((void *)elems, nelems, sizeof(Datum), _bt_compare_array_elements,
            (void *)&cxt);
  ...
  for (i = 1; i < nelems; i++) {
    ...
    compare = DatumGetInt32(FunctionCall2Coll(&cxt.flinfo, cxt.collation,
                                              elems[last_non_dup], elems[i]));
    if (compare != 0) elems[++last_non_dup] = elems[i];
  }
...
static int _bt_compare_array_elements(const void *a, const void *b, void *arg) {
  ...
  compare =
      DatumGetInt32(FunctionCall2Coll(&cxt->flinfo, cxt->collation, da, db));
  if (cxt->reverse) compare = -compare;
  return compare;
}
...
void _bt_preprocess_keys(IndexScanDesc scan) {
  ...
  if (numberOfKeys == 1) {
    ...
    if (!_bt_fix_scankey_strategy(cur, indoption)) so->qual_ok = false;
    memcpy(outkeys, cur, sizeof(ScanKeyData));
    so->numberOfKeys = 1;
    /* We can mark the qual as required if it's for first index col */
    if (cur->sk_attno == 1) _bt_mark_scankey_required(outkeys);
    return;
  }
  ...
  for (i = 0;; cur++, i++) {
    ...
    if (i == numberOfKeys || cur->sk_attno != attno) {
      ...
      if (xform[BTEqualStrategyNumber - 1]) {
        ...
        for (j = BTMaxStrategyNumber; --j >= 0;) {
          ...
          if (!chk || j == (BTEqualStrategyNumber - 1)) continue;
    ...
    if (cur->sk_flags & SK_ROW_HEADER) {
      ...
      if (numberOfEqualCols == attno - 1) _bt_mark_scankey_required(outkey);
...
static bool _bt_compare_scankey_args(IndexScanDesc scan, ScanKey op,
  ...
  if (lefttype == InvalidOid) lefttype = opcintype;
  righttype = rightarg->sk_subtype;
  if (righttype == InvalidOid) righttype = opcintype;
  optype = op->sk_subtype;
  if (optype == InvalidOid) optype = opcintype;
  ...
  if (lefttype == opcintype && righttype == optype) {
    *result = DatumGetBool(FunctionCall2Coll(&op->sk_func, op->sk_collation,
                                             leftarg->sk_argument,
   ...
   */
  strat = op->sk_strategy;
  if (op->sk_flags & SK_BT_DESC) strat = BTCommuteStrategyNumber(strat);
  ...
  if (OidIsValid(cmp_op)) {
    ...
    if (RegProcedureIsValid(cmp_proc)) {
      *result = DatumGetBool(OidFunctionCall2Coll(cmp_proc, op->sk_collation,
                                                  leftarg->sk_argument,
                                                  rightarg->sk_argument));
      return true;
    }
...
static bool _bt_fix_scankey_strategy(ScanKey skey, int16 *indoption) {
  ...
  if (skey->sk_flags & SK_ROW_HEADER) {
    ...
    for (;;) {
      ...
      if (subkey->sk_flags & SK_ROW_END) break;
      subkey++;
    }
  }
  ...
  *continuescan = true; /* default assumption */
  ...
  if (scan->ignore_killed_tuples && ItemIdIsDead(iid)) {
    ...
    if (ScanDirectionIsForward(dir)) {
      if (offnum < PageGetMaxOffsetNumber(page)) return NULL;
    } else {
      BTPageOpaque opaque = (BTPageOpaque)PageGetSpecialPointer(page);

      if (offnum > P_FIRSTDATAKEY(opaque)) return NULL;
    }
  ...
  for (key = so->keyData, ikey = 0; ikey < keysz; key++, ikey++) {
     ...
     */
    if (key->sk_flags & SK_BT_MATCHED && !IndexTupleHasNulls(tuple)) continue;
    ...
    if (key->sk_flags & SK_ISNULL) {
      ...
      if (key->sk_flags & SK_SEARCHNULL) {
        if (isNull) continue; /* tuple satisfies this qual */
      } else {
    ...
    test = FunctionCall2Coll(&key->sk_func, key->sk_collation, datum,
                             key->sk_argument);
...
static bool _bt_check_rowcompare(ScanKey skey, IndexTuple tuple,
  ...
  for (;;) {
    ...
    if (subkey->sk_flags & SK_ISNULL) {
       ...
       */
      if (subkey != (ScanKey)DatumGetPointer(skey->sk_argument)) subkey--;
    ...
    cmpresult = DatumGetInt32(FunctionCall2Coll(
        &subkey->sk_func, subkey->sk_collation, datum, subkey->sk_argument));

    if (subkey->sk_flags & SK_BT_DESC) cmpresult = -cmpresult;
...
void _bt_killitems(IndexScanDesc scan) {
  ...
  for (i = 0; i < numKilled; i++) {
    ...
    if (offnum < minoff) continue; /* pure paranoia */
    while (offnum <= maxoff) {
      ItemId iid = PageGetItemId(page, offnum);
