...
static void btbuildCallback(Relation index, HeapTuple htup, Datum *values,
...
Datum btbuild(PG_FUNCTION_ARGS) {
  ...
  if (log_btree_build_stats) ResetUsage();
  ...
  /* do the heap scan */
  reltuples = IndexBuildHeapScan(heap, index, indexInfo, true, btbuildCallback,
                                 (void *)&buildstate);
  ...
  if (buildstate.spool2) _bt_spooldestroy(buildstate.spool2);
...
Datum btgettuple(PG_FUNCTION_ARGS) {
  ...
  if (so->numArrayKeys && !BTScanPosIsValid(so->currPos)) {
    ...
    if (so->numArrayKeys < 0) PG_RETURN_BOOL(false);
...
Datum btgetbitmap(PG_FUNCTION_ARGS) {
  ...
  if (so->numArrayKeys) {
    /* punt if we have any unsatisfiable array keys */
    if (so->numArrayKeys < 0) PG_RETURN_INT64(ntids);
    ...
    if (_bt_first(scan, ForwardScanDirection)) {
      ...
      for (;;) {
        ...
        if (++so->currPos.itemIndex > so->currPos.lastItem) {
          ...
          if (!_bt_next(scan, ForwardScanDirection)) break;
        }
...
Datum btrescan(PG_FUNCTION_ARGS) {
  ...
  if (BTScanPosIsValid(so->currPos)) {
    ...
    if (so->numKilled > 0) _bt_killitems(scan);
...
Datum btendscan(PG_FUNCTION_ARGS) {
  ...
  /* Release storage */
  if (so->keyData != NULL) pfree(so->keyData);
  /* so->arrayKeyData and so->arrayKeys are in arrayContext */
  if (so->arrayContext != NULL) MemoryContextDelete(so->arrayContext);
  if (so->killedItems != NULL) pfree(so->killedItems);
  if (so->currTuples != NULL) pfree(so->currTuples);
...
Datum btmarkpos(PG_FUNCTION_ARGS) {
  ...
  if (so->numArrayKeys) _bt_mark_array_keys(scan);
...
Datum btrestrpos(PG_FUNCTION_ARGS) {
  ...
  if (so->numArrayKeys) _bt_restore_array_keys(scan);
  ...
  if (so->markItemIndex >= 0) {
    ...
    if (BTScanPosIsValid(so->markPos)) {
      ...
      if (BTScanPosIsPinned(so->markPos)) IncrBufferRefCount(so->markPos.buf);
...
Datum btbulkdelete(PG_FUNCTION_ARGS) {
  ...
  IndexBulkDeleteResult *volatile stats =
...
restart:
  ...
  if (!PageIsNew(page)) _bt_checkpage(rel, buf);
  ...
  if (_bt_page_recyclable(page)) {
     ...
     */
    if (blkno > vstate->lastBlockLocked) vstate->lastBlockLocked = blkno;
    ...
    if (ndeletable > 0) {
       ...
       */
      if (blkno > vstate->lastBlockVacuumed) vstate->lastBlockVacuumed = blkno;
 ...
 */
Datum btcanreturn(PG_FUNCTION_ARGS) { PG_RETURN_BOOL(true); }
