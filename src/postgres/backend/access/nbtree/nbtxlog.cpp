...
static void _bt_restore_page(Page page, char *from, int len) {
  ...
  for (i = nitems - 1; i >= 0; i--) {
    if (PageAddItem(page, items[i], itemsizes[i], nitems - i, false, false) ==
        InvalidOffsetNumber)
...
static void btree_xlog_insert(bool isleaf, bool ismeta,
   ...
   */
  if (!isleaf) _bt_clear_incomplete_split(record, 1);
  if (XLogReadBufferForRedo(record, 0, &buffer) == BLK_NEEDS_REDO) {
    ...
    if (PageAddItem(page, (Item)datapos, datalen, xlrec->offnum, false,
   ...
   */
  if (ismeta) _bt_restore_meta(record, 2);
}

static void btree_xlog_split(bool onleft, bool isroot,
                             XLogReaderState *record) {
  XLogRecPtr lsn = record->EndRecPtr;
  ...
  if (!XLogRecGetBlockTag(record, 2, NULL, NULL, &rnext)) rnext = P_NONE;
   ...
   */
  if (!isleaf) _bt_clear_incomplete_split(record, 3);
  ...
  if (XLogReadBufferForRedo(record, 0, &lbuf) == BLK_NEEDS_REDO) {
    ...
    /* Set high key */
    leftoff = P_HIKEY;
    if (PageAddItem(newlpage, left_hikey, left_hikeysz, P_HIKEY, false,
                    false) == InvalidOffsetNumber)
    ...
    if (isleaf) lopaque->btpo_flags |= BTP_LEAF;
  ...
  if (BufferIsValid(lbuf)) UnlockReleaseBuffer(lbuf);
 ...
 */
static TransactionId btree_xlog_delete_get_latestRemovedXid(
    XLogReaderState *record) {
   ...
   */
  if (CountDBBackends(InvalidOid) == 0) return latestRemovedXid;
  ...
  if (!reachedConsistency)
    elog(PANIC,
         "btree_xlog_delete_get_latestRemovedXid: cannot operate with "
         "inconsistent data");
  ...
  if (!BufferIsValid(ibuffer)) return InvalidTransactionId;
  ...
  for (i = 0; i < xlrec->nitems; i++) {
    ...
    hbuffer =
        XLogReadBufferExtended(xlrec->hnode, MAIN_FORKNUM, hblkno, RBM_NORMAL);
...
static void btree_xlog_unlink_page(uint8 info, XLogReaderState *record) {
  ...
  if (info == XLOG_BTREE_UNLINK_PAGE_META) _bt_restore_meta(record, 4);
}
...
static void btree_xlog_newroot(XLogReaderState *record) {
  ...
  if (xlrec->level == 0) pageop->btpo_flags |= BTP_LEAF;
  pageop->btpo_cycleid = 0;
