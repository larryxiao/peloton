...
static bool _bt_lock_branch_parent(Relation rel, BlockNumber child,
                                   BTStack stack, Buffer *topparent,
                                   OffsetNumber *topoff, BlockNumber *target,
...
Buffer _bt_getroot(Relation rel, int access) {
  ...
  if (metad->btm_version != BTREE_VERSION)
    ereport(ERROR, (errcode(ERRCODE_INDEX_CORRUPTED),
                    errmsg(
                        ...
                        "code version %d",
                        RelationGetRelationName(rel), metad->btm_version,
                        BTREE_VERSION)));
  ...
  if (metad->btm_root == P_NONE) {
     ...
     */
    rel->rd_amcache =
        MemoryContextAlloc(rel->rd_indexcxt, sizeof(BTMetaPageData));
    ...
    if (rootopaque->btpo.level != rootlevel)
      elog(ERROR, "root page %u of index \"%s\" has level %u, expected %u",
           rootblkno, RelationGetRelationName(rel), rootopaque->btpo.level,
           rootlevel);
  }
...
Buffer _bt_getbuf(Relation rel, BlockNumber blkno, int access) {
  ...
  if (blkno != P_NEW) {
    ...
    for (;;) {
      ...
      if (blkno == InvalidBlockNumber) break;
      buf = ReadBuffer(rel, blkno);
    ...
    if (needLock) LockRelationForExtension(rel, ExclusiveLock);
     ...
     */
    if (needLock) UnlockRelationForExtension(rel, ExclusiveLock);
...
Buffer _bt_relandgetbuf(Relation rel, Buffer obuf, BlockNumber blkno,
  ...
  Assert(blkno != P_NEW);
  if (BufferIsValid(obuf)) LockBuffer(obuf, BUFFER_LOCK_UNLOCK);
 ...
 */
void _bt_relbuf(Relation rel, Buffer buf) { UnlockReleaseBuffer(buf); }
 ...
 */
void _bt_delitems_vacuum(Relation rel, Buffer buf, OffsetNumber *itemnos,
                         int nitems, BlockNumber lastBlockVacuumed) {
  Page page = BufferGetPage(buf);
  BTPageOpaque opaque;
  ...
  /* Fix the page */
  if (nitems > 0) PageIndexMultiDelete(page, itemnos, nitems);
 ...
 */
void _bt_delitems_delete(Relation rel, Buffer buf, OffsetNumber *itemnos,
                         int nitems, Relation heapRel) {
  Page page = BufferGetPage(buf);
  BTPageOpaque opaque;
...
static bool _bt_lock_branch_parent(Relation rel, BlockNumber child,
  ...
  if (pbuf == InvalidBuffer)
    elog(ERROR,
         "failed to re-find parent key in index \"%s\" for deletion target "
         "page %u",
    ...
    if (poffset == P_FIRSTDATAKEY(opaque)) {
      ...
      *rightsib = opaque->btpo_next;
      ...
      if (_bt_is_page_halfdead(rel, *rightsib)) {
        elog(DEBUG1,
             "could not delete page %u because its right sibling %u is "
             "half-dead",
             parent, *rightsib);
        return false;
      }

      return _bt_lock_branch_parent(rel, parent, stack->bts_parent, topparent,
                                    topoff, target, rightsib);
    } else {
      /* Unsafe to delete */
      _bt_relbuf(rel, pbuf);
      return false;
        ...
        ereport(LOG,
                (errcode(ERRCODE_INDEX_CORRUPTED),
                 errmsg("index \"%s\" contains a half-dead internal page",
                        RelationGetRelationName(rel)),
                 errhint(
                     "This can be caused by an interrupted VACUUM in version "
                     "9.3 or older, before upgrade. Please REINDEX it.")));
      _bt_relbuf(rel, buf);
      return ndeleted;
    }
...
static bool _bt_mark_page_halfdead(Relation rel, Buffer leafbuf,
  ...
  target = leafblkno;
  if (!_bt_lock_branch_parent(rel, leafblkno, stack, &topparent, &topoff,
                              &target, &rightsib))
    return false;
  ...
  if (ItemPointerGetBlockNumber(&(itup->t_tid)) != rightsib)
    elog(ERROR,
         "right sibling %u of block %u is not next child %u of block %u in "
         "index \"%s\"",
...
static bool _bt_unlink_halfdead_page(Relation rel, Buffer leafbuf,
   ...
   * table.)
   */
  if (target != leafblkno) LockBuffer(leafbuf, BT_WRITE);
  if (leftsib != P_NONE) {
  ...
  if (P_RIGHTMOST(opaque) || P_ISROOT(opaque) || P_ISDELETED(opaque)) {
    elog(ERROR,
         "half-dead page changed status unexpectedly in block %u of index "
         "\"%s\"",
  ...
  if (target == leafblkno) {
    ...
    nextchild = ItemPointerGetBlockNumber(
        &((IndexTuple)PageGetItem(page, itemid))->t_tid);
    if (nextchild == leafblkno) nextchild = InvalidBlockNumber;
  }
  ...
  *rightsib_empty = (P_FIRSTDATAKEY(opaque) > PageGetMaxOffsetNumber(page));
  ...
  MarkBufferDirty(buf);
  if (BufferIsValid(lbuf)) MarkBufferDirty(lbuf);
  if (target != leafblkno) MarkBufferDirty(leafbuf);
  ...
  if (RelationNeedsWAL(rel)) {
    ...
    if (BufferIsValid(lbuf)) XLogRegisterBuffer(1, lbuf, REGBUF_STANDARD);
    XLogRegisterBuffer(2, rbuf, REGBUF_STANDARD);
    if (target != leafblkno) XLogRegisterBuffer(3, leafbuf, REGBUF_WILL_INIT);
  ...
  /* release siblings */
  if (BufferIsValid(lbuf)) _bt_relbuf(rel, lbuf);
  _bt_relbuf(rel, rbuf);
   ...
   * kept locked.
   */
  if (target != leafblkno) _bt_relbuf(rel, buf);
