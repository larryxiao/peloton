 ...
 */
BTStack _bt_search(Relation rel, int keysz, ScanKey scankey, bool nextkey,
...
Buffer _bt_moveright(Relation rel, Buffer buf, int keysz, ScanKey scankey,
                     bool nextkey, bool forupdate, BTStack stack, int access) {
  Page page;
  BTPageOpaque opaque;
  int32 cmpval;
 ...
 *----------
 */
int32 _bt_compare(Relation rel, int keysz, ScanKey scankey, Page page,
                  OffsetNumber offnum) {
  ...
  for (i = 1; i <= keysz; i++) {
    ...
    } else if (isNull) /* key is NOT_NULL and item is NULL */
    {
      ...
      if (!(scankey->sk_flags & SK_BT_DESC)) result = -result;
    }
...
bool _bt_first(IndexScanDesc scan, ScanDirection dir) {
      ...
      if (i >= so->numberOfKeys || cur->sk_attno != curattr) {
        ...
        if (chosen == NULL && impliesNN != NULL &&
          ...
          ScanKeyEntryInitialize(
              ...
              curattr, ((impliesNN->sk_flags & SK_BT_NULLS_FIRST)
         ...
         * next attribute).
         */
        if (i >= so->numberOfKeys || cur->sk_attno != curattr + 1) break;
   ...
   * there.
   */
  if (keysCount == 0) return _bt_endpoint(scan, dir);
  ...
  for (i = 0; i < keysCount; i++) {
    ...
    if (cur->sk_flags & SK_ROW_HEADER) {
      ...
      if (subkey->sk_flags & SK_ISNULL) return false;
      ...
      if (i == keysCount - 1) {
        ...
        for (;;) {
          ...
          if (subkey->sk_flags & SK_ISNULL) break; /* can't use null keys */
          Assert(keysCount < INDEX_MAX_KEYS);
      ...
      if (cur->sk_subtype == rel->rd_opcintype[i] ||
        ...
        ScanKeyEntryInitialize(scankeys + i, cur->sk_flags, cur->sk_attno,
                               InvalidStrategy, cur->sk_subtype,
   ...
   * next page as needed.)
   */
  if (goback) offnum = OffsetNumberPrev(offnum);
  ...
  switch (startKeys[0]->sk_strategy) {
    ...
    case BTGreaterEqualStrategyNumber:
    case BTGreaterStrategyNumber:
      if (ScanDirectionIsForward(dir)) startKeys[0]->sk_flags |= SK_BT_MATCHED;
      break;

    case BTLessEqualStrategyNumber:
    case BTLessStrategyNumber:
      if (ScanDirectionIsBackward(dir)) startKeys[0]->sk_flags |= SK_BT_MATCHED;
      break;
...
static bool _bt_steppage(IndexScanDesc scan, ScanDirection dir) {
  ...
  if (so->markItemIndex >= 0) {
    ...
    if (BTScanPosIsPinned(so->currPos)) IncrBufferRefCount(so->currPos.buf);
  ...
  if (ScanDirectionIsForward(dir)) {
    ...
    for (;;) {
      ...
      if (!P_IGNORE(opaque)) {
        ...
        if (_bt_readpage(scan, dir, PageGetMaxOffsetNumber(page))) break;
      }
    }
  }
...
static Buffer _bt_walk_left(Relation rel, Buffer buf) {
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      if (P_RIGHTMOST(opaque) || ++tries > 4) break;
      blkno = opaque->btpo_next;
...
Buffer _bt_get_endpoint(Relation rel, uint32 level, bool rightmost) {
  ...
  if (!BufferIsValid(buf)) return InvalidBuffer;
  ...
  for (;;) {
    ...
    /* Done? */
    if (opaque->btpo.level == level) break;
    if (opaque->btpo.level < level)
      elog(ERROR, "btree level %u not found in index \"%s\"", level,
           RelationGetRelationName(rel));
