...
static TransactionId _bt_check_unique(Relation rel, IndexTuple itup,
                                      ...
                                      OffsetNumber offset, ScanKey itup_scankey,
                                      IndexUniqueCheck checkUnique,
                                      bool *is_unique,
                                      uint32 *speculativeToken);
static void _bt_findinsertloc(Relation rel, Buffer *bufptr,
                              OffsetNumber *offsetptr, int keysz,
                              ScanKey scankey, IndexTuple newtup, BTStack stack,
                              Relation heapRel);
static void _bt_insertonpg(Relation rel, Buffer buf, Buffer cbuf, BTStack stack,
                           IndexTuple itup, OffsetNumber newitemoff,
                           bool split_only_page);
static Buffer _bt_split(Relation rel, Buffer buf, Buffer cbuf,
                        OffsetNumber firstright, OffsetNumber newitemoff,
 ...
 */
bool _bt_doinsert(Relation rel, IndexTuple itup, IndexUniqueCheck checkUnique,
                  Relation heapRel) {
...
top:
  ...
  if (checkUnique != UNIQUE_CHECK_EXISTING) {
    ...
    /* do the insertion */
    _bt_findinsertloc(rel, &buf, &offset, natts, itup_scankey, itup, stack,
                      heapRel);
  ...
  *is_unique = true;
  ...
  for (;;) {
    ...
    if (offset < maxoff)
      ...
      if (!_bt_isequal(itupdesc, page, P_HIKEY, natts, itup_scankey)) break;
    ...
    ereport(ERROR,
            (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
             errmsg("index row size %zu exceeds maximum %zu for index \"%s\"",
                    itemsz, BTMaxItemSize(page), RelationGetRelationName(rel)),
             errhint(
...
static void _bt_insertonpg(Relation rel, Buffer buf, Buffer cbuf, BTStack stack,
  ...
  if (PageGetFreeSpace(page) < itemsz) {
    ...
    firstright =
        _bt_findsplitloc(rel, page, newitemoff, itemsz, &newitemonleft);

    /* split the buffer into left and right halves */
    rbuf = _bt_split(rel, buf, cbuf, firstright, newitemoff, itemsz, itup,
                     newitemonleft);
    ...
    if (BufferIsValid(cbuf)) _bt_relbuf(rel, cbuf);
    _bt_relbuf(rel, buf);
  }
}
...
static Buffer _bt_split(Relation rel, Buffer buf, Buffer cbuf,
  ...
  if (!P_RIGHTMOST(oopaque)) {
    ...
    if (PageAddItem(rightpage, (Item)item, itemsz, rightoff, false, false) ==
        InvalidOffsetNumber) {
  ...
  if (PageAddItem(leftpage, (Item)item, itemsz, leftoff, false, false) ==
      InvalidOffsetNumber) {
    memset(rightpage, 0, BufferGetPageSize(rbuf));
    elog(ERROR,
    ...
    if (i == newitemoff) {
      if (newitemonleft) {
        if (!_bt_pgaddtup(leftpage, newitemsz, newitem, leftoff)) {
          memset(rightpage, 0, BufferGetPageSize(rbuf));
          elog(ERROR,
               "failed to add new___ item to the left sibling"
               " while splitting block %u of index \"%s\"",
               origpagenumber, RelationGetRelationName(rel));
        }
        leftoff = OffsetNumberNext(leftoff);
        ...
        if (!_bt_pgaddtup(rightpage, newitemsz, newitem, rightoff)) {
          memset(rightpage, 0, BufferGetPageSize(rbuf));
          elog(ERROR,
               "failed to add new___ item to the right sibling"
               " while splitting block %u of index \"%s\"",
               origpagenumber, RelationGetRelationName(rel));
        }
        rightoff = OffsetNumberNext(rightoff);
  ...
  if (RelationNeedsWAL(rel)) {
    ...
    if (!P_RIGHTMOST(ropaque)) XLogRegisterBuffer(2, sbuf, REGBUF_STANDARD);
    if (BufferIsValid(cbuf)) XLogRegisterBuffer(3, cbuf, REGBUF_STANDARD);
    ...
    if (isroot)
      xlinfo =
          newitemonleft ? XLOG_BTREE_SPLIT_L_ROOT : XLOG_BTREE_SPLIT_R_ROOT;
    else
  ...
  if (!P_RIGHTMOST(ropaque)) _bt_relbuf(rel, sbuf);
...
static void _bt_checksplitloc(FindSplitData *state,
  ...
  newitemisfirstonright =
      (firstoldonright == state->newitemoff && !newitemonleft);
  ...
  rightfree =
      state->rightspace - (state->olddataitemstotal - olddataitemstoleft);
...
static void _bt_insert_parent(Relation rel, Buffer buf, Buffer rbuf,
  ...
  if (is_root) {
    ...
    _bt_insertonpg(rel, pbuf, buf, stack->bts_parent, new_item,
...
Buffer _bt_getstackbuf(Relation rel, BTStack stack, int access) {
  ...
  for (;;) {
    ...
    if (!P_IGNORE(opaque)) {
       ...
       */
      if (start < minoff) start = minoff;

      /*
       * Need this check too, to guard against possibility that page
       * split since we visited it originally.
       */
      if (start > maxoff) start = OffsetNumberNext(maxoff);
       ...
       */
      for (offnum = start; offnum <= maxoff;
           offnum = OffsetNumberNext(offnum)) {
        itemid = PageGetItemId(page, offnum);
...
static Buffer _bt_newroot(Relation rel, Buffer lbuf, Buffer rbuf) {
   ...
   */
  if (PageAddItem(rootpage, (Item)left_item, left_item_sz, P_HIKEY, false,
                  false) == InvalidOffsetNumber)
    elog(PANIC,
         "failed to add leftkey to new___ root page"
         " while splitting block %u of index \"%s\"",
  ...
  if (PageAddItem(rootpage, (Item)right_item, right_item_sz, P_FIRSTKEY, false,
                  false) == InvalidOffsetNumber)
    elog(PANIC,
         "failed to add rightkey to new___ root page"
         " while splitting block %u of index \"%s\"",
         BufferGetBlockNumber(lbuf), RelationGetRelationName(rel));
...
static bool _bt_isequal(TupleDesc itupdesc, Page page, OffsetNumber offnum,
  ...
  for (i = 1; i <= keysz; i++) {
    ...
    if (isNull || (scankey->sk_flags & SK_ISNULL)) return false;

    result = DatumGetInt32(FunctionCall2Coll(
        &scankey->sk_func, scankey->sk_collation, datum, scankey->sk_argument));
...
static void _bt_vacuum_one_page(Relation rel, Buffer buffer, Relation heapRel) {
  ...
  for (offnum = minoff; offnum <= maxoff; offnum = OffsetNumberNext(offnum)) {
    ...
    if (ItemIdIsDead(itemId)) deletable[ndeletable++] = offnum;
  }
