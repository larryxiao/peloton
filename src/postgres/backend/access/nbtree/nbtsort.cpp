 ...
 */
BTSpool *_bt_spoolinit(Relation heap, Relation index, bool isunique,
                       bool isdead) {
  ...
  btspool->sortstate =
      tuplesort_begin_index_btree(heap, index, isunique, btKbytes, false);
 ...
 */
void _bt_spool(BTSpool *btspool, ItemPointer self, Datum *values,
               bool *isnull) {
  tuplesort_putindextuplevalues(btspool->sortstate, btspool->index, self,
                                values, isnull);
}
...
void _bt_leafbuild(BTSpool *btspool, BTSpool *btspool2) {
  ...
  if (btspool2) tuplesort_performsort(btspool2->sortstate);
...
/*
 * allocate workspace for a new___, clean btree page, not linked to any
 * siblings.
 */
static Page _bt_blnewpage(uint32 level) {
  Page page;
 ...
 */
static void _bt_blwritepage(BTWriteState *wstate, Page page,
                            BlockNumber blkno) {
  ...
  while (blkno > wstate->btws_pages_written) {
    if (!wstate->btws_zeropage) wstate->btws_zeropage = (Page)palloc0(BLCKSZ);
    /* don't set checksum for all-zero page */
    smgrextend(wstate->index->rd_smgr, MAIN_FORKNUM,
               wstate->btws_pages_written++, (char *)wstate->btws_zeropage,
               true);
  }
...
static BTPageState *_bt_pagestate(BTWriteState *wstate, uint32 level) {
  ...
  if (level > 0)
    ...
    state->btps_full =
        RelationGetTargetPageFreeSpace(wstate->index, BTREE_DEFAULT_FILLFACTOR);
  /* no parent level, yet */
 ...
 * btree leaves.
 */
static void _bt_load(BTWriteState *wstate, BTSpool *btspool,
                     BTSpool *btspool2) {
  ...
  int i, keysz = RelationGetNumberOfAttributes(wstate->index);
    ...
    for (;;) {
      ...
      if (itup2 == NULL) {
        ...
        for (i = 1; i <= keysz; i++) {
          ...
          compare = ApplySortComparator(attrDatum1, isNull1, attrDatum2,
                                        isNull2, entry);
          if (compare > 0) {
            load1 = false;
            break;
      ...
      if (state == NULL) state = _bt_pagestate(wstate, 0);
      ...
      if (load1) {
        ...
        if (should_free2) pfree(itup2);
        itup2 =
            tuplesort_getindextuple(btspool2->sortstate, true, &should_free2);
      }
    }
    pfree(sortKeys);
  } else {
    /* merge is unnecessary */
    while ((itup = tuplesort_getindextuple(btspool->sortstate, true,
                                           &should_free)) != NULL) {
      /* When we see first tuple, create first index page */
      if (state == NULL) state = _bt_pagestate(wstate, 0);
