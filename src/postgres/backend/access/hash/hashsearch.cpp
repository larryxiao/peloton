...
bool _hash_next(IndexScanDesc scan, ScanDirection dir) {
   ...
   */
  if (!_hash_step(scan, &buf, dir)) return false;
 ...
 */
static void _hash_readnext(Relation rel, Buffer *bufp, Page *pagep,
 ...
 */
static void _hash_readprev(Relation rel, Buffer *bufp, Page *pagep,
                           HashPageOpaque *opaquep) {
  BlockNumber blkno;
  ...
  *bufp = InvalidBuffer;
  ...
  if (BlockNumberIsValid(blkno)) {
    *bufp =
        _hash_getbuf(rel, blkno, HASH_READ, LH_BUCKET_PAGE | LH_OVERFLOW_PAGE);
    *pagep = BufferGetPage(*bufp);
    *opaquep = (HashPageOpaque)PageGetSpecialPointer(*pagep);
  }
}
...
bool _hash_first(IndexScanDesc scan, ScanDirection dir) {
   ...
   * items in the index.
   */
  if (cur->sk_flags & SK_ISNULL) return false;
