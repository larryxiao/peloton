...
static void hashbuildCallback(Relation index, HeapTuple htup, Datum *values,
...
Datum hashinsert(PG_FUNCTION_ARGS) {
   ...
   */
  if (isnull[0]) PG_RETURN_BOOL(false);
...
Datum hashgettuple(PG_FUNCTION_ARGS) {
  ...
  if (ItemPointerIsValid(current)) {
    ...
    for (offnum = ItemPointerGetOffsetNumber(current); offnum <= maxoffnum;
         offnum = OffsetNumberNext(offnum)) {
      IndexTuple itup;

      itup = (IndexTuple)PageGetItem(page, PageGetItemId(page, offnum));
      if (ItemPointerEquals(&(so->hashso_heappos), &(itup->t_tid))) break;
    }
    if (offnum > maxoffnum)
  ...
  if (scan->ignore_killed_tuples) {
    while (res) {
      ...
      if (!ItemIdIsDead(PageGetItemId(page, offnum))) break;
      res = _hash_next(scan, dir);
    }
  }
...
Datum hashvacuumcleanup(PG_FUNCTION_ARGS) {
  ...
  if (stats == NULL) PG_RETURN_POINTER(NULL);
