...
static void _hash_splitbucket(Relation rel, Buffer metabuf, Bucket obucket,
                              Bucket nbucket, BlockNumber start_oblkno,
...
Buffer _hash_getbuf(Relation rel, BlockNumber blkno, int access, int flags) {
  ...
  if (blkno == P_NEW) elog(ERROR, "hash AM does not use P_NEW");

  buf = ReadBuffer(rel, blkno);

  if (access != HASH_NOLOCK) LockBuffer(buf, access);
 ...
 */
Buffer _hash_getbuf_with_strategy(Relation rel, BlockNumber blkno, int access,
                                  int flags, BufferAccessStrategy bstrategy) {
  Buffer buf;
 ...
 */
void _hash_relbuf(Relation rel, Buffer buf) { UnlockReleaseBuffer(buf); }
 ...
 */
void _hash_dropbuf(Relation rel, Buffer buf) { ReleaseBuffer(buf); }
 ...
 */
void _hash_chgbufaccess(Relation rel, Buffer buf, int from_access,
                        int to_access) {
  if (from_access == HASH_WRITE) MarkBufferDirty(buf);
  if (from_access != HASH_NOLOCK) LockBuffer(buf, BUFFER_LOCK_UNLOCK);
  if (to_access != HASH_NOLOCK) LockBuffer(buf, to_access);
}
...
uint32 _hash_metapinit(Relation rel, double num_tuples, ForkNumber forkNum) {
  ...
  ffactor =
      RelationGetTargetPageUsage(rel, HASH_DEFAULT_FILLFACTOR) / item_width;
  ...
  for (i = _hash_log2(metap->hashm_bsize); i > 0; --i) {
    if ((1 << i) <= metap->hashm_bsize) break;
  }
  Assert(i > 0);
...
void _hash_expandtable(Relation rel, Buffer metabuf) {
   ...
   * _hash_metapinit().
   */
  if (metap->hashm_maxbucket >= (uint32)0x7FFFFFFE) goto fail;
  ...
  if (_hash_has_active_scan(rel, old_bucket)) goto fail;

  if (!_hash_try_getlock(rel, start_oblkno, HASH_EXCLUSIVE)) goto fail;
  ...
  _hash_splitbucket(rel, metabuf, old_bucket, new_bucket, start_oblkno,
                    buf_nblkno, maxbucket, highmask, lowmask);
 ...
 * We need to interlock against _hash_getovflpage() adding a new___ overflow
 *page
...
static bool _hash_alloc_buckets(Relation rel, BlockNumber firstblock,
   ...
   * the index anymore.
   */
  if (lastblock < firstblock || lastblock == InvalidBlockNumber) return false;
...
static void _hash_splitbucket(Relation rel, Buffer metabuf, Bucket obucket,
  ...
  for (;;) {
    ...
    for (ooffnum = FirstOffsetNumber; ooffnum <= omaxoffnum;
    ...
    if (!BlockNumberIsValid(oblkno)) break;
