...
static uint32 blkno_to_bitno(HashMetaPage metap, BlockNumber ovflblkno) {
  ...
  for (i = 1; i <= splitnum; i++) {
    if (ovflblkno <= (BlockNumber)(1 << i)) break; /* oops */
...
Buffer _hash_addovflpage(Relation rel, Buffer metabuf, Buffer buf) {
  ...
  for (;;) {
    ...
    if (!BlockNumberIsValid(nextblkno)) break;
 ...
 *	on the bucket, too.
 */
BlockNumber _hash_freeovflpage(Relation rel, Buffer ovflbuf,
  ...
  if (BlockNumberIsValid(prevblkno)) {
    Buffer prevbuf = _hash_getbuf_with_strategy(
        rel, prevblkno, HASH_WRITE, LH_BUCKET_PAGE | LH_OVERFLOW_PAGE,
        bstrategy);
  ...
  if (BlockNumberIsValid(nextblkno)) {
    Buffer nextbuf = _hash_getbuf_with_strategy(rel, nextblkno, HASH_WRITE,
 ...
 */
void _hash_initbitmap(Relation rel, HashMetaPage metap, BlockNumber blkno,
                      ForkNumber forkNum) {
  Buffer buf;
  Page pg;
  HashPageOpaque op;
...
void _hash_squeezebucket(Relation rel, Bucket bucket, BlockNumber bucket_blkno,
  ...
  do {
    ...
    if (rbuf != InvalidBuffer) _hash_relbuf(rel, rbuf);
    rbuf = _hash_getbuf_with_strategy(rel, rblkno, HASH_WRITE, LH_OVERFLOW_PAGE,
                                      bstrategy);
  ...
  for (;;) {
    ...
    for (roffnum = FirstOffsetNumber; roffnum <= maxroffnum;
      ...
      while (PageGetFreeSpace(wpage) < itemsz) {
        ...
        wbuf = _hash_getbuf_with_strategy(rel, wblkno, HASH_WRITE,
                                          LH_OVERFLOW_PAGE, bstrategy);
        wpage = BufferGetPage(wbuf);
        wopaque = (HashPageOpaque)PageGetSpecialPointer(wpage);
