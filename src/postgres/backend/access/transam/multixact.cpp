...
MultiXactId MultiXactIdExpand(MultiXactId multi, TransactionId xid,
  ...
  if (nmembers < 0) {
    ...
    debug_elog4(DEBUG2, "Expand: %u has no members, create singleton %u", multi,
                newMulti);
    return newMulti;
  }
   ...
   */
  newMembers =
      (MultiXactMember *)palloc(sizeof(MultiXactMember) * (nmembers + 1));
...
bool MultiXactIdIsRunning(MultiXactId multi, bool isLockOnly) {
  ...
  for (i = 0; i < nmembers; i++) {
    if (TransactionIdIsInProgress(members[i].xid)) {
      debug_elog4(DEBUG2, "IsRunning: member %d (%u) is running", i,
                  members[i].xid);
      pfree(members);
      return true;
    }
...
void MultiXactIdSetOldestMember(void) {
  if (!MultiXactIdIsValid(OldestMemberMXactId[MyBackendId])) {
    ...
    if (nextMXact < FirstMultiXactId) nextMXact = FirstMultiXactId;

    OldestMemberMXactId[MyBackendId] = nextMXact;

    LWLockRelease(MultiXactGenLock);

    debug_elog4(DEBUG2, "MultiXact: setting OldestMember[%d] = %u", MyBackendId,
                nextMXact);
  }
}
...
static void MultiXactIdSetOldestVisible(void) {
  if (!MultiXactIdIsValid(OldestVisibleMXactId[MyBackendId])) {
    ...
    if (oldestMXact < FirstMultiXactId) oldestMXact = FirstMultiXactId;
...
MultiXactId ReadNextMultiXactId(void) {
  ...
  if (mxid < FirstMultiXactId) mxid = FirstMultiXactId;
...
static void RecordNewMultiXact(MultiXactId multi, MultiXactOffset offset,
  ...
  *offptr = offset;
  ...
  for (i = 0; i < nmembers; i++, offset++) {
    ...
    memberptr =
        (TransactionId *)(MultiXactMemberCtl->shared->page_buffer[slotno] +
                          memberoff);

    *memberptr = members[i].xid;

    flagsptr =
        (uint32 *)(MultiXactMemberCtl->shared->page_buffer[slotno] + flagsoff);
  ...
  if (!MultiXactIdPrecedes(result, MultiXactState->multiVacLimit) ||
    ...
    if (IsUnderPostmaster && !MultiXactIdPrecedes(result, multiStopLimit)) {
      ...
      if (oldest_datname)
        ereport(ERROR, (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
                        errmsg(
                            ...
                            "new___ MultiXactIds to avoid wraparound data loss "
                            "in database \"%s\"",
                            oldest_datname),
                        errhint(
      ...
      if (oldest_datname)
        ereport(WARNING,
                (errmsg_plural(
                     "database \"%s\" must be vacuumed before %u more "
                     "MultiXactId is used",
                     "database \"%s\" must be vacuumed before %u more "
                     "MultiXactIds are used",
                     multiWrapLimit - result, oldest_datname,
        ...
        ereport(WARNING,
                (errmsg_plural(
                     "database with OID %u must be vacuumed before %u more "
                     "MultiXactId is used",
                     "database with OID %u must be vacuumed before %u more "
                     "MultiXactIds are used",
                     multiWrapLimit - result, oldest_datoid,
                     multiWrapLimit - result),
                 errhint(
                     "Execute a database-wide VACUUM in that database.\n"
                     "You might also need to commit or roll back old prepared "
    ...
    LWLockAcquire(MultiXactGenLock, LW_EXCLUSIVE);
    result = MultiXactState->nextMXact;
    if (result < FirstMultiXactId) result = FirstMultiXactId;
  }
  ...
  if (MultiXactOffsetWouldWrap(MultiXactState->offsetStopLimit, nextOffset,
    ...
    ereport(ERROR,
            (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
             ...
             errdetail_plural(
                 "This command would create a multixact with %u members, but "
                 "the remaining space is only enough for %u member.",
                 "This command would create a multixact with %u members, but "
                 "the remaining space is only enough for %u members.",
                 MultiXactState->offsetStopLimit - nextOffset - 1, nmembers,
                 MultiXactState->offsetStopLimit - nextOffset - 1),
             errhint(
                 "Execute a database-wide VACUUM in database with OID %u with "
                 "reduced vacuum_multixact_freeze_min_age and "
                 "vacuum_multixact_freeze_table_age settings.",
                 MultiXactState->oldestMultiXactDB)));
  else if (MultiXactOffsetWouldWrap(MultiXactState->offsetStopLimit, nextOffset,
                                    nmembers +
    ...
    ereport(WARNING,
            (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
             errmsg(
                 "database with OID %u must be vacuumed before %d more "
                 "multixact members are used",
                 MultiXactState->oldestMultiXactDB,
...
int GetMultiXactIdMembers(MultiXactId multi, MultiXactMember **members,
  ...
  if (!MultiXactIdIsValid(multi)) return -1;
...
retry:
  ...
  if (nextMXact == tmpMXact) {
    ...
    if (tmpMXact < FirstMultiXactId) tmpMXact = FirstMultiXactId;
  ...
  *members = ptr;
  ...
  for (i = 0; i < length; i++, offset++) {
    ...
    ptr[truelength].status = static_cast<MultiXactStatus>(
        (*flagsptr >> bshift) & MXACT_MEMBER_XACT_BITMASK);
    truelength++;
  }
...
static int mxactMemberComparator(const void *arg1, const void *arg2) {
  ...
  if (member1.xid > member2.xid) return 1;
  if (member1.xid < member2.xid) return -1;
  if (member1.status > member2.status) return 1;
  if (member1.status < member2.status) return -1;
  return 0;
}
...
static MultiXactId mXactCacheGetBySet(int nmembers, MultiXactMember *members) {
  ...
  dlist_foreach(iter, &MXactCache) {
    ...
    if (entry->nmembers != nmembers) continue;

    /*
     * We assume the cache entries are sorted, and that the unused bits in
     * "status" are zeroed.
     */
    if (memcmp(members, entry->members, nmembers * sizeof(MultiXactMember)) ==
        0) {
...
static void mXactCachePut(MultiXactId multi, int nmembers,
  ...
  if (MXactContext == NULL) {
    ...
    MXactContext =
        AllocSetContextCreate(TopTransactionContext, "MultiXact Cache Context",
  ...
  entry = (mXactCacheEnt *)MemoryContextAlloc(
      MXactContext,
      offsetof(mXactCacheEnt, members) + nmembers * sizeof(MultiXactMember));
...
char *mxid_to_string(MultiXactId multi, int nmembers,
                     MultiXactMember *members) {
  static char *str = NULL;
  StringInfoData buf;
  int i;

  if (str != NULL) pfree(str);
...
void AtPrepare_MultiXact(void) {
  ...
  if (MultiXactIdIsValid(myOldestMember))
    RegisterTwoPhaseRecord(TWOPHASE_RM_MULTIXACT_ID, 0, &myOldestMember,
                           sizeof(MultiXactId));
}
 ...
 */
void multixact_twophase_recover(TransactionId xid, uint16 info, void *recdata,
                                uint32 len) {
  BackendId dummyBackendId = TwoPhaseGetDummyBackendId(xid);
 ...
 */
void multixact_twophase_postcommit(TransactionId xid, uint16 info,
 ...
 */
void multixact_twophase_postabort(TransactionId xid, uint16 info, void *recdata,
                                  uint32 len) {
...
#define SHARED_MULTIXACT_STATE_SIZE                                          \
  add_size(                                                                  \
      offsetof(MultiXactStateData, perBackendXactIds) + sizeof(MultiXactId), \
...
void MultiXactShmemInit(void) {
  ...
  SimpleLruInit(MultiXactOffsetCtl, "MultiXactOffset Ctl",
                NUM_MXACTOFFSET_BUFFERS, 0, MultiXactOffsetControlLock,
  ...
  MultiXactState = static_cast<MultiXactStateData *>(ShmemInitStruct(
      "Shared MultiXact State", SHARED_MULTIXACT_STATE_SIZE, &found));
...
static int ZeroMultiXactOffsetPage(int pageno, bool writeXlog) {
  ...
  if (writeXlog) WriteMZeroPageXlogRec(pageno, XLOG_MULTIXACT_ZERO_OFF_PAGE);
...
static int ZeroMultiXactMemberPage(int pageno, bool writeXlog) {
  ...
  if (writeXlog) WriteMZeroPageXlogRec(pageno, XLOG_MULTIXACT_ZERO_MEM_PAGE);
 ...
 * new___ MultiXactId would result in an error because the page on which the
 *new___
 ...
 */
void MultiXactGetCheckptMulti(bool is_shutdown, MultiXactId *nextMulti,
...
void SetMultiXactIdLimit(MultiXactId oldest_datminmxid, Oid oldest_datoid) {
  ...
  if (multiWrapLimit < FirstMultiXactId) multiWrapLimit += FirstMultiXactId;
  ...
  if (multiStopLimit < FirstMultiXactId) multiStopLimit -= FirstMultiXactId;
  ...
  if (multiWarnLimit < FirstMultiXactId) multiWarnLimit -= FirstMultiXactId;
  ...
  if (multiVacLimit < FirstMultiXactId) multiVacLimit += FirstMultiXactId;
  ...
  if (MultiXactIdPrecedes(multiWarnLimit, curMulti) && !InRecovery) {
    ...
    if (oldest_datname)
      ereport(WARNING, (errmsg_plural(
                            ...
                            "database \"%s\" must be vacuumed before %u more "
                            "MultiXactIds are used",
                            multiWrapLimit - curMulti, oldest_datname,
                            multiWrapLimit - curMulti),
                        errhint(
                            "To avoid a database shutdown, execute a "
                            "database-wide VACUUM in that database.\n"
                            "You might also need to commit or roll back old "
                            "prepared transactions.")));
    else
      ereport(WARNING, (errmsg_plural(
                            "database with OID %u must be vacuumed before %u "
                            "more MultiXactId is used",
                            "database with OID %u must be vacuumed before %u "
                            "more MultiXactIds are used",
                            multiWrapLimit - curMulti, oldest_datoid,
                            multiWrapLimit - curMulti),
                        errhint(
                            "To avoid a database shutdown, execute a "
                            "database-wide VACUUM in that database.\n"
 ...
 */
void MultiXactSetSafeTruncate(MultiXactId safeTruncateMulti) {
  LWLockAcquire(MultiXactGenLock, LW_EXCLUSIVE);
...
static void ExtendMultiXactOffset(MultiXactId multi) {
   ...
   */
  if (MultiXactIdToOffsetEntry(multi) != 0 && multi != FirstMultiXactId) return;
...
static void DetermineSafeOldestOffset(MultiXactId oldestMXact) {
  ...
  oldestOffset -=
      oldestOffset % (MULTIXACT_MEMBERS_PER_PAGE * SLRU_PAGES_PER_SEGMENT);
 ...
 * exist.
 */
static void ReadMultiXactCounts(uint32 *multixacts, MultiXactOffset *members) {
  MultiXactOffset nextOffset;
  MultiXactOffset oldestOffset;
  MultiXactId oldestMultiXactId;
...
int MultiXactMemberFreezeThreshold(void) {
   ...
   */
  fraction =
      (double)(members - MULTIXACT_MEMBER_SAFE_THRESHOLD) /
      (MULTIXACT_MEMBER_DANGER_THRESHOLD - MULTIXACT_MEMBER_SAFE_THRESHOLD);
  victim_multixacts = multixacts * fraction;

  /* fraction could be > 1.0, but lowest possible freeze age is zero */
  if (victim_multixacts > multixacts) return 0;
...
static bool SlruScanDirCbRemoveMembers(SlruCtl ctl, char *filename, int segpage,
                                       void *data) {
  MembersLiveRange *range = (MembersLiveRange *)data;
  MultiXactOffset nextOffset;

  if ((segpage == range->rangeStart) || (segpage == range->rangeEnd))
   ...
   * one run.
   */
  if (!((range->rangeStart > range->rangeEnd && segpage > range->rangeEnd &&
         segpage < range->rangeStart) ||
...
typedef struct mxtruncinfo { int earliestExistingPage; } mxtruncinfo;

/*
 * SlruScanDirectory callback
 *		This callback determines the earliest existing page number.
 */
static bool SlruScanDirCbFindEarliest(SlruCtl ctl, char *filename, int segpage,
                                      void *data) {
...
void TruncateMultiXact(void) {
  ...
  if (earliest < FirstMultiXactId) earliest = FirstMultiXactId;

  /* nothing to do */
  if (MultiXactIdPrecedes(oldestMXact, earliest)) return;
  ...
  SimpleLruTruncate(MultiXactOffsetCtl, MultiXactIdToOffsetPage(oldestMXact));
...
Datum pg_get_multixact_members(PG_FUNCTION_ARGS) {
  ...
  if (SRF_IS_FIRSTCALL()) {
    ...
    multi->nmembers =
        GetMultiXactIdMembers(mxid, &multi->members, false, false);
    multi->iter = 0;
  ...
  if (multi->nmembers > 0) pfree(multi->members);
  pfree(multi);
