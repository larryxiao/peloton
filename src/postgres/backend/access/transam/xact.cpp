...
thread_local static TransactionState CurrentTransactionState =
 ...
 */
TransactionId GetTopTransactionIdIfAny(void) { return XactTopTransactionId; }
...
TransactionId GetStableLatestTransactionId(void) {
  ...
  if (lxid != MyProc->lxid) {
    ...
    if (!TransactionIdIsValid(stablexid)) stablexid = ReadNewTransactionId();
  }
...
static void AssignTransactionId(TransactionState s) {
  ...
  if (isSubXact && !TransactionIdIsValid(s->parent->transactionId)) {
    ...
    parents = static_cast<TransactionStateData **>(
        palloc(sizeof(TransactionState) * s->nestingLevel));
     ...
     */
    while (parentOffset != 0) AssignTransactionId(parents[--parentOffset]);
  ...
  if (!isSubXact) XactTopTransactionId = s->transactionId;
   ...
   */
  if (!isSubXact) RegisterPredicateLockingXid(s->transactionId);
...
bool SubTransactionIsActive(SubTransactionId subxid) {
  ...
  for (s = CurrentTransactionState; s != NULL; s = s->parent) {
    if (s->state == TRANS_ABORT) continue;
    if (s->subTransactionId == subxid) return true;
  }
  return false;
}
...
TimestampTz GetCurrentTransactionStopTimestamp(void) {
  if (xactStopTimestamp != 0) return xactStopTimestamp;
...
bool TransactionIdIsCurrentTransactionId(TransactionId xid) {
   ...
   * any non-normal XID.
   */
  if (!TransactionIdIsNormal(xid)) return false;
  ...
  for (s = CurrentTransactionState; s != NULL; s = s->parent) {
    ...
    if (TransactionIdEquals(xid, s->transactionId)) return true;
 ...
 */
void ForceSyncCommit(void) { forceSyncCommit = true; }
 ...
 *	AtStart_Cache
 */
static void AtStart_Cache(void) { AcceptInvalidationMessages(); }
...
static void AtStart_Memory(void) {
  ...
  if (TransactionAbortContext == NULL)
    TransactionAbortContext =
        AllocSetContextCreate(TopMemoryContext, "TransactionAbortContext",
...
static void AtSubStart_Memory(void) {
   ...
   */
  CurTransactionContext = AllocSetContextCreate(
      CurTransactionContext, "CurTransactionContext", ALLOCSET_DEFAULT_MINSIZE,
      ALLOCSET_DEFAULT_INITSIZE, ALLOCSET_DEFAULT_MAXSIZE);
...
static TransactionId RecordTransactionCommit(void) {
  ...
  if ((wrote_xlog && markXidCommitted &&
     ...
     */
    if (markXidCommitted) TransactionIdCommitTree(xid, nchildren, children);
  } else {
    /*
   ...
   */
  if (wrote_xlog && markXidCommitted) SyncRepWaitForLSN(XactLastRecEnd);
...
static void AtSubCommit_childXids(void) {
  ...
  if (s->parent->maxChildXids < new_nChildXids) {
     ...
     */
    new_maxChildXids =
        Min(new_nChildXids * 2, (int)(MaxAllocSize / sizeof(TransactionId)));
    ...
    if (s->parent->childXids == NULL)
      new_childXids = static_cast<TransactionId *>(MemoryContextAlloc(
          TopTransactionContext, new_maxChildXids * sizeof(TransactionId)));
    else
      new_childXids = static_cast<TransactionId *>(repalloc(
          s->parent->childXids, new_maxChildXids * sizeof(TransactionId)));
  ...
  if (s->nChildXids > 0)
    memcpy(&s->parent->childXids[s->parent->nChildXids + 1], s->childXids,
           s->nChildXids * sizeof(TransactionId));

  s->parent->nChildXids = new_nChildXids;

  /* Release child's array to avoid leakage */
  if (s->childXids != NULL) pfree(s->childXids);
...
static TransactionId RecordTransactionAbort(bool isSubXact) {
   ...
   */
  if (!isSubXact) XLogSetAsyncXactLSN(XactLastRecEnd);
   ...
   */
  if (isSubXact) XidCacheRemoveRunningXids(xid, nchildren, children, latestXid);
...
static void AtCleanup_Memory(void) {
   ...
   * Release all transaction-local memory.
   */
  if (TopTransactionContext != NULL) MemoryContextDelete(TopTransactionContext);
  TopTransactionContext = NULL;
  CurTransactionContext = NULL;
  CurrentTransactionState->curTransactionContext = NULL;
}
...
static void AtSubCleanup_Memory(void) {
   ...
   * of the subxact).
   */
  if (s->curTransactionContext) MemoryContextDelete(s->curTransactionContext);
  s->curTransactionContext = NULL;
}
...
static void PrepareTransaction(void) {
  ...
  if (MyXactAccessedTempRel)
    ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                    errmsg(
                        "cannot PREPARE a transaction that has operated on "
                        "temporary tables")));
   ...
   */
  gxact =
      MarkAsPreparing(xid, prepareGID, prepared_at, GetUserId(), MyDatabaseId);
  prepareGID = NULL;
  ...
  switch (s->blockState) {
    case TBLOCK_DEFAULT:
      if (s->state == TRANS_DEFAULT) {
         ...
         * AbortTransaction.
         */
        if (s->state == TRANS_START) s->state = TRANS_INPROGRESS;
        AbortTransaction();
        CleanupTransaction();
      }
      break;
...
void RegisterXactCallback(XactCallback callback, void *arg) {
  ...
  item = (XactCallbackItem *)MemoryContextAlloc(TopMemoryContext,
...
void RegisterSubXactCallback(SubXactCallback callback, void *arg) {
  ...
  item = (SubXactCallbackItem *)MemoryContextAlloc(TopMemoryContext,
...
bool EndTransactionBlock(void) {
  ...
  switch (s->blockState) {
     ...
     */
    case TBLOCK_STARTED:
      ereport(WARNING, (errcode(ERRCODE_NO_ACTIVE_SQL_TRANSACTION),
...
void DefineSavepoint(char *name) {
  ...
  switch (s->blockState) {
    ...
    case TBLOCK_SUBINPROGRESS:
       ...
       */
      if (name) s->name = MemoryContextStrdup(TopTransactionContext, name);
      break;
...
void ReleaseSavepoint(List *options) {
  ...
  foreach (cell, options) {
    ...
    if (strcmp(elem->defname, "savepoint_name") == 0) name = strVal(elem->arg);
  }

  Assert(PointerIsValid(name));

  for (target = s; PointerIsValid(target); target = target->parent) {
    if (PointerIsValid(target->name) && strcmp(target->name, name) == 0) break;
  }
...
void RollbackAndReleaseCurrentSubTransaction(void) {
   ...
   */
  if (s->blockState == TBLOCK_SUBINPROGRESS) AbortSubTransaction();
...
bool IsTransactionOrTransactionBlock(void) {
  ...
  if (s->blockState == TBLOCK_DEFAULT) return false;
...
bool IsSubTransaction(void) {
  ...
  if (s->nestingLevel >= 2) return true;
...
static void CommitSubTransaction(void) {
  ...
  /* Post-commit cleanup */
  if (TransactionIdIsValid(s->transactionId)) AtSubCommit_childXids();
...
static void AbortSubTransaction(void) {
  ...
  if (s->curTransactionOwner) {
    ...
    /* Post-abort cleanup */
    if (TransactionIdIsValid(s->transactionId)) AtSubAbort_childXids();
    ...
    ResourceOwnerRelease(s->curTransactionOwner, RESOURCE_RELEASE_LOCKS, false,
                         false);
    ResourceOwnerRelease(s->curTransactionOwner, RESOURCE_RELEASE_AFTER_LOCKS,
                         false, false);
    AtSubAbort_smgr();
...
static void PopTransaction(void) {
  ...
  if (s->parent == NULL) elog(FATAL, "PopTransaction with no parent");
...
Size EstimateTransactionStateSpace(void) {
  ...
  for (s = CurrentTransactionState; s != NULL; s = s->parent) {
    if (TransactionIdIsValid(s->transactionId)) nxids = add_size(nxids, 1);
...
void SerializeTransactionState(Size maxsize, char *start_address) {
  ...
  workspace =
      static_cast<TransactionId *>(palloc(nxids * sizeof(TransactionId)));
  for (s = CurrentTransactionState; s != NULL; s = s->parent) {
    if (TransactionIdIsValid(s->transactionId))
...
static void ShowTransactionStateRec(TransactionState s) {
  ...
  if (s->parent) ShowTransactionStateRec(s->parent);

  /* use ereport to suppress computation if msg will not be printed */
  ereport(
      DEBUG3,
      (errmsg_internal(
          "name: %s; blockState: %13s; state: %7s, xid/subid/cid: %u/%u/%u%s, "
          "nestlvl: %d, children: %s",
          PointerIsValid(s->name) ? s->name : "unnamed",
          BlockStateAsString(s->blockState), TransStateAsString(s->state),
          (unsigned int)s->transactionId, (unsigned int)s->subTransactionId,
          (unsigned int)currentCommandId, currentCommandIdUsed ? " (used)" : "",
 ...
 * otherwise.
 */
XLogRecPtr XactLogCommitRecord(TimestampTz commit_time, int nsubxacts,
                               TransactionId *subxacts, int nrels,
                               RelFileNode *rels, int nmsgs,
 ...
 * otherwise.
 */
XLogRecPtr XactLogAbortRecord(TimestampTz abort_time, int nsubxacts,
                              TransactionId *subxacts, int nrels,
                              RelFileNode *rels, TransactionId twophase_xid) {
  xl_xact_abort xlrec;
  xl_xact_xinfo xl_xinfo;
  xl_xact_subxacts xl_subxacts;
  xl_xact_relfilenodes xl_relfilenodes;
 ...
 */
static void xact_redo_commit(xl_xact_parsed_commit *parsed, TransactionId xid,
  ...
  Assert(!!(parsed->xinfo & XACT_XINFO_HAS_ORIGIN) ==
  ...
  if (standbyState == STANDBY_DISABLED) {
    ...
    ProcessCommittedInvalidationMessages(
        ...
        XactCompletionRelcacheInitFileInval(parsed->xinfo), parsed->dbId,
        parsed->tsId);
  ...
  if (parsed->xinfo & XACT_XINFO_HAS_ORIGIN) {
    ...
    replorigin_advance(origin_id, parsed->origin_lsn, lsn, false /* backward */,
                       false /* WAL */);
  }
  ...
  if (parsed->nrels > 0) {
    ...
    for (i = 0; i < parsed->nrels; i++) {
      ...
      unsigned int fork;  // Peloton Porting. Changed type from ForkNumber to
                          // unsigned int
   ...
   */
  if (XactCompletionForceSyncCommit(parsed->xinfo)) XLogFlush(lsn);
}
...
static void xact_redo_abort(xl_xact_parsed_abort *parsed, TransactionId xid) {
  ...
  for (i = 0; i < parsed->nrels; i++) {
    SMgrRelation srel = smgropen(parsed->xnodes[i], InvalidBackendId);
    unsigned int
        fork;  // Peloton Porting. Changed type from ForkNumber to unsigned int
...
void xact_redo(XLogReaderState *record) {
  ...
  if (info == XLOG_XACT_COMMIT || info == XLOG_XACT_COMMIT_PREPARED) {
    ...
    if (info == XLOG_XACT_COMMIT) {
      ...
      xact_redo_commit(&parsed, XLogRecGetXid(record), record->EndRecPtr,
    ...
    RecreateTwoPhaseFile(XLogRecGetXid(record), XLogRecGetData(record),
