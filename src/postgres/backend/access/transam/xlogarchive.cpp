...
bool RestoreArchivedFile(char *path, const char *xlogfname,
  ...
  if (recoveryRestoreCommand == NULL) goto not_available;
  ...
  if (stat(xlogpath, &stat_buf) != 0) {
    ...
    if (unlink(xlogpath) != 0)
      ereport(FATAL, (errcode_for_file_access(),
                      errmsg("could not remove file \"%s\": %m", xlogpath)));
  }
  ...
  *dp = '\0';
  ...
  ereport(DEBUG3, (errmsg_internal("executing restore command \"%s\"",
                                   xlogRestoreCmd)));
  ...
  if (rc == 0) {
    ...
    if (stat(xlogpath, &stat_buf) == 0) {
      if (expectedSize > 0 && stat_buf.st_size != expectedSize) {
        ...
        ereport(LOG,
                (errmsg("restored log file \"%s\" from archive", xlogfname)));
   ...
   * too.
   */
  if (WIFSIGNALED(rc) && WTERMSIG(rc) == SIGTERM) proc_exit(1);

  signaled = WIFSIGNALED(rc) || WEXITSTATUS(rc) > 125;

  ereport(signaled ? FATAL : DEBUG2,
          (errmsg("could not restore file \"%s\" from archive: %s", xlogfname,
 ...
 */
void ExecuteRecoveryCommand(char *command, char *commandName,
                            bool failOnSignal) {
  ...
  *dp = '\0';
  ...
  if (rc != 0) {
    ...
    ereport(
        (signaled && failOnSignal) ? FATAL : WARNING,
        /*------
...
void KeepFileRestoredFromArchive(char *path, char *xlogfname) {
  ...
  if (rename(path, xlogfpath) < 0)
    ereport(ERROR, (errcode_for_file_access(),
                    errmsg("could not rename file \"%s\" to \"%s\": %m", path,
                           xlogfpath)));
   ...
   */
  if (reload) WalSndRqstFileReload();
...
void XLogArchiveNotify(const char *xlog) {
  ...
  if (IsUnderPostmaster) SendPostmasterSignal(PMSIGNAL_WAKEN_ARCHIVER);
}
...
void XLogArchiveForceDone(const char *xlog) {
  ...
  if (stat(archiveDone, &stat_buf) == 0) return;
...
bool XLogArchiveCheckDone(const char *xlog) {
  ...
  if (!XLogArchivingActive()) return true;

  /* First check for .done --- this means archiver is done with it */
  StatusFilePath(archiveStatusPath, xlog, ".done");
  if (stat(archiveStatusPath, &stat_buf) == 0) return true;

  /* check for .ready --- this means archiver is still busy with it */
  StatusFilePath(archiveStatusPath, xlog, ".ready");
  if (stat(archiveStatusPath, &stat_buf) == 0) return false;
...
bool XLogArchiveIsBusy(const char *xlog) {
  ...
  if (stat(archiveStatusPath, &stat_buf) != 0 && errno == ENOENT) return false;
