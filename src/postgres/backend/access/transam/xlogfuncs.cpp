...
Datum pg_start_backup(PG_FUNCTION_ARGS) {
  ...
  startpoint = do_pg_start_backup(backupidstr, fast, NULL, NULL, dir, NULL,
                                  NULL, false, true);
...
Datum pg_xlogfile_name_offset(PG_FUNCTION_ARGS) {
  ...
  TupleDescInitEntry(resultTupleDesc, (AttrNumber)1, "file_name", TEXTOID, -1,
                     0);
  TupleDescInitEntry(resultTupleDesc, (AttrNumber)2, "file_offset", INT4OID, -1,
                     0);
...
Datum pg_xlog_replay_pause(PG_FUNCTION_ARGS) {
  ...
  if (!RecoveryInProgress())
    ereport(ERROR, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                    ...
                    errhint(
                        "Recovery control functions can only be executed "
                        "during recovery.")));
...
Datum pg_xlog_location_diff(PG_FUNCTION_ARGS) {
  ...
  result =
      DirectFunctionCall2(pg_lsn_mi, PG_GETARG_DATUM(0), PG_GETARG_DATUM(1));
 ...
 */
Datum pg_is_in_backup(PG_FUNCTION_ARGS) { PG_RETURN_BOOL(BackupInProgress()); }
