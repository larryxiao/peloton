...
static void report_invalid_page(int elevel, RelFileNode node, ForkNumber forkno,
...
static void log_invalid_page(RelFileNode node, ForkNumber forkno,
                             BlockNumber blkno, bool present) {
  ...
  if (invalid_page_tab == NULL) {
    ...
    invalid_page_tab = hash_create("XLOG invalid-page table", 100, &ctl,
  ...
  key.forkno = forkno;
  key.blkno = blkno;
  hentry = (xl_invalid_page *)hash_search(invalid_page_tab, (void *)&key,
                                          HASH_ENTER, &found);
...
static void forget_invalid_pages(RelFileNode node, ForkNumber forkno,
                                 BlockNumber minblkno) {
  HASH_SEQ_STATUS status;
  xl_invalid_page *hentry;

  if (invalid_page_tab == NULL) return; /* nothing to do */

  hash_seq_init(&status, invalid_page_tab);

  while ((hentry = (xl_invalid_page *)hash_seq_search(&status)) != NULL) {
    if (RelFileNodeEquals(hentry->key.node, node) &&
        hentry->key.forkno == forkno && hentry->key.blkno >= minblkno) {
 ...
 */
XLogRedoAction XLogReadBufferForRedo(XLogReaderState *record, uint8 block_id,
                                     Buffer *buf) {
  return XLogReadBufferForRedoExtended(record, block_id, RBM_NORMAL, false,
                                       buf);
}
  ...
  if (XLogRecHasBlockImage(record, block_id)) {
    *buf = XLogReadBufferExtended(
    ...
    if ((record->blocks[block_id].flags & BKPBLOCK_WILL_INIT) != 0 &&
      ...
      elog(PANIC,
           "block with WILL_INIT flag in WAL record must be zeroed by redo "
           "routine");
    }
 ...
 */
Buffer XLogReadBufferExtended(RelFileNode rnode, ForkNumber forknum,
  ...
  if (blkno < lastblock) {
    ...
    if (mode == RBM_NORMAL_NO_LOG) return InvalidBuffer;
...
Relation CreateFakeRelcacheEntry(RelFileNode rnode) {
  ...
  fakeentry =
      static_cast<FakeRelCacheEntry>(palloc0(sizeof(FakeRelCacheEntryData)));
 ...
 */
void XLogTruncateRelation(RelFileNode rnode, ForkNumber forkNum,
                          BlockNumber nblocks) {
