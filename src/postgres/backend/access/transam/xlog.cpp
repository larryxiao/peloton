...
thread_local static RecoveryTargetAction recoveryTargetAction =
 ...
 * or start a new___ one; so it can be used to tell if the current transaction
 *has
...
#define UsableBytesInSegment                           \
  ((XLOG_SEG_SIZE / XLOG_BLCKSZ) * UsableBytesInPage - \
...
thread_local static XLogSource readSource =
    static_cast<XLogSource>(0); /* XLOG_FROM_* code */
 ...
 * next.
 */
thread_local static XLogSource currentSource =
    static_cast<XLogSource>(0); /* XLOG_FROM_* code */
...
thread_local static XLogSource XLogReceiptSource =
    static_cast<XLogSource>(0); /* XLOG_FROM_* code */
...
static bool InstallXLogFileSegment(XLogSegNo *segno, char *tmppath,
                                   ...
                                   bool use_lock);
static int XLogFileRead(XLogSegNo segno, int emode, TimeLineID tli, int source,
                        bool notexistOk);
...
static void RemoveOldXlogFiles(XLogSegNo segno, XLogRecPtr PriorRedoPtr,
                               XLogRecPtr endptr);
static void RemoveXlogFile(const char *segname, XLogRecPtr PriorRedoPtr,
                           XLogRecPtr endptr);
...
static XLogRecord *ReadCheckpointRecord(XLogReaderState *xlogreader,
                                        XLogRecPtr RecPtr, int whichChkpti,
                                        bool report);
...
XLogRecPtr XLogInsertRecord(XLogRecData *rdata, XLogRecPtr fpw_lsn) {
  ...
  bool isLogSwitch =
      (rechdr->xl_rmid == RM_XLOG_ID && rechdr->xl_info == XLOG_SWITCH);
  XLogRecPtr StartPos;
  XLogRecPtr EndPos;
  ...
  if (inserted) {
     ...
     */
    CopyXLogRecordToWAL(rechdr->xl_tot_len, isLogSwitch, rdata, StartPos,
                        EndPos);
  } else {
    /*
  ...
  if (StartPos / XLOG_BLCKSZ != EndPos / XLOG_BLCKSZ) {
    ...
    if (XLogCtl->LogwrtRqst.Write < EndPos) XLogCtl->LogwrtRqst.Write = EndPos;
  ...
  if (XLOG_DEBUG) {
    ...
    initStringInfo(&buf);
    appendStringInfo(&buf, "INSERT @ %X/%X: ", (uint32)(EndPos >> 32),
                     (uint32)EndPos);
    ...
    if (!debug_reader) debug_reader = XLogReaderAllocate(NULL, NULL);
...
static void WALInsertLockAcquire(void) {
  ...
  if (lockToTry == -1) lockToTry = MyProc->pgprocno % NUM_XLOGINSERT_LOCKS;
  MyLockNo = lockToTry;
...
static void WALInsertLockAcquireExclusive(void) {
  ...
  for (i = 0; i < NUM_XLOGINSERT_LOCKS - 1; i++) {
    LWLockAcquireWithVar(&WALInsertLocks[i].l.lock,
                         &WALInsertLocks[i].l.insertingAt, PG_UINT64_MAX);
  }
  LWLockAcquireWithVar(&WALInsertLocks[i].l.lock,
...
static void WALInsertLockUpdateInsertingAt(XLogRecPtr insertingAt) {
  if (holdingAllLocks) {
    ...
    LWLockUpdateVar(&WALInsertLocks[MyLockNo].l.lock,
                    &WALInsertLocks[MyLockNo].l.insertingAt, insertingAt);
}
...
static XLogRecPtr WaitXLogInsertionsToFinish(XLogRecPtr upto) {
  ...
  if (MyProc == NULL) elog(PANIC, "cannot wait without a PGPROC structure");
  ...
  if (upto > reservedUpto) {
    elog(LOG,
         "request to flush past end of generated WAL; request %X/%X, currpos "
         "%X/%X",
         (uint32)(upto >> 32), (uint32)upto, (uint32)(reservedUpto >> 32),
         (uint32)reservedUpto);
    upto = reservedUpto;
  }
  ...
  for (i = 0; i < NUM_XLOGINSERT_LOCKS; i++) {
    ...
    do {
      ...
      if (LWLockWaitForVar(&WALInsertLocks[i].l.lock,
                           &WALInsertLocks[i].l.insertingAt, insertingat,
                           &insertingat)) {
...
static char *GetXLogBuffer(XLogRecPtr ptr) {
  ...
  if (ptr / XLOG_BLCKSZ == cachedPage) {
    ...
    Assert(((XLogPageHeader)cachedPos)->xlp_pageaddr ==
           ptr - (ptr % XLOG_BLCKSZ));
  ...
  if (expectedEndPtr != endptr) {
    ...
    if (expectedEndPtr != endptr)
      elog(PANIC, "could not find WAL buffer for %X/%X", (uint32)(ptr >> 32),
           (uint32)ptr);
  } else {
    /*
...
static void AdvanceXLInsertBuffer(XLogRecPtr upto, bool opportunistic) {
  ...
  while (upto >= XLogCtl->InitializedUpTo || opportunistic) {
     ...
     */
    if (!Insert->forcePageWrites) NewPage->xlp_info |= XLP_BKP_REMOVABLE;
...
#ifdef WAL_DEBUG
  if (npages > 0) {
    elog(DEBUG1, "initialized %d pages, upto %X/%X", npages,
...
static void CalculateCheckpointSegments(void) {
  ...
  if (CheckPointSegments < 1) CheckPointSegments = 1;
}
...
static XLogSegNo XLOGfileslop(XLogRecPtr PriorRedoPtr) {
  ...
  recycleSegNo =
      (XLogSegNo)ceil(((double)PriorRedoPtr + distance) / XLOG_SEG_SIZE);

  if (recycleSegNo < minSegNo) recycleSegNo = minSegNo;
  if (recycleSegNo > maxSegNo) recycleSegNo = maxSegNo;
...
static bool XLogCheckpointNeeded(XLogSegNo new_segno) {
  ...
  if (new_segno >= old_segno + (uint64)(CheckPointSegments - 1)) return true;
  return false;
}
...
static void XLogWrite(XLogwrtRqst WriteRqst, bool flexible) {
  ...
  while (LogwrtResult.Write < WriteRqst.Write) {
    ...
    finishing_seg =
        !ispartialpage && (startoffset + npages * XLOG_BLCKSZ) >= XLogSegSize;
    ...
    if (last_iteration || curridx == XLogCtl->XLogCacheBlck || finishing_seg) {
      ...
      if (finishing_seg) {
        ...
        if (XLogArchivingActive()) XLogArchiveNotifySeg(openLogSegNo);
...
void XLogSetAsyncXactLSN(XLogRecPtr asyncXactLSN) {
  ...
  if (!sleeping) {
    ...
    if (WriteRqstPtr <= LogwrtResult.Flush) return;
  }
   ...
   */
  if (ProcGlobal->walwriterLatch) SetLatch(ProcGlobal->walwriterLatch);
}
...
static void UpdateMinRecoveryPoint(XLogRecPtr lsn, bool force) {
  ...
  if (!updateMinRecoveryPoint || (!force && lsn <= minRecoveryPoint)) return;
  ...
  if (minRecoveryPoint == 0)
    ...
    if (!force && newMinRecoveryPoint < lsn)
      elog(WARNING,
           ...
           (uint32)(newMinRecoveryPoint >> 32), (uint32)newMinRecoveryPoint);
    ...
    if (ControlFile->minRecoveryPoint < newMinRecoveryPoint) {
      ...
      ereport(DEBUG2,
              (errmsg("updated min recovery point to %X/%X on timeline %u",
                      ...
                      (uint32)minRecoveryPoint, newMinRecoveryPointTLI)));
    }
  }
...
void XLogFlush(XLogRecPtr record) {
  ...
  for (;;) {
    ...
    /* done already? */
    if (record <= LogwrtResult.Flush) break;
...
bool XLogNeedsFlush(XLogRecPtr record) {
  ...
  if (RecoveryInProgress()) {
    ...
    if (record <= minRecoveryPoint || !updateMinRecoveryPoint) return false;

    /*
     * Update local copy of minRecoveryPoint. But if the lock is busy,
     * just return a conservative guess.
     */
    if (!LWLockConditionalAcquire(ControlFileLock, LW_SHARED)) return true;
    minRecoveryPoint = ControlFile->minRecoveryPoint;
    minRecoveryPointTLI = ControlFile->minRecoveryPointTLI;
    LWLockRelease(ControlFileLock);
     ...
     * here too.
     */
    if (minRecoveryPoint == 0) updateMinRecoveryPoint = false;
  ...
  /* Quick exit if already known flushed */
  if (record <= LogwrtResult.Flush) return false;
...
int XLogFileInit(XLogSegNo logsegno, bool *use_existent, bool use_lock) {
  ...
  if (!InstallXLogFileSegment(&installed_segno, tmppath, *use_existent,
                              max_segno, use_lock)) {
    /*
 ...
 */
static char *XLogFileCopy(char *dstfname, char *srcfname, int upto) {
  ...
  for (nbytes = 0; nbytes < XLogSegSize; nbytes += sizeof(buffer)) {
     ...
     */
    if (nread < sizeof(buffer)) memset(buffer, 0, sizeof(buffer));

    if (nread > 0) {
      if (nread > sizeof(buffer)) nread = sizeof(buffer);
      errno = 0;
      if (read(srcfd, buffer, nread) != nread) {
        if (errno != 0)
          ereport(ERROR, (errcode_for_file_access(),
                          errmsg("could not read file \"%s\": %m", srcpath)));
        else
          ereport(ERROR, (errmsg("not enough data in file \"%s\"", srcpath)));
      }
    }
    errno = 0;
...
static bool InstallXLogFileSegment(XLogSegNo *segno, char *tmppath,
   ...
   */
  if (use_lock) LWLockAcquire(ControlFileLock, LW_EXCLUSIVE);
 ...
 */
  ...
  if (link(tmppath, path) < 0) {
    ...
    ereport(LOG, (errcode_for_file_access(),
                  errmsg(
                      "could not link file \"%s\" to \"%s\" (initialization of "
                      "log file): %m",
                      tmppath, path)));
    return false;
  }
  ...
  if (rename(tmppath, path) < 0) {
    ...
    ereport(LOG, (errcode_for_file_access(),
                  errmsg(
                      "could not rename file \"%s\" to \"%s\" (initialization "
                      "of log file): %m",
                      tmppath, path)));
    return false;
  }
...
static int XLogFileRead(XLogSegNo segno, int emode, TimeLineID tli, int source,
  ...
  switch (source) {
    case XLOG_FROM_ARCHIVE:
      ...
      restoredFromArchive = RestoreArchivedFile(path, xlogfname, "RECOVERYXLOG",
                                                XLogSegSize, InRedo);
      if (!restoredFromArchive) return -1;
      break;
  ...
  if (fd >= 0) {
    ...
    if (source != XLOG_FROM_STREAM) XLogReceiptTime = GetCurrentTimestamp();
...
static int XLogFileReadAnyTLI(XLogSegNo segno, int emode, int source) {
  ...
  foreach (cell, tles) {
    ...
    if (tli < curFileTLI) break; /* don't bother looking at too-old TLIs */

    if (source == XLOG_FROM_ANY || source == XLOG_FROM_ARCHIVE) {
      fd = XLogFileRead(segno, emode, tli, XLOG_FROM_ARCHIVE, true);
      if (fd != -1) {
        elog(DEBUG1, "got WAL segment from archive");
        if (!expectedTLEs) expectedTLEs = tles;
        return fd;
      }
    }
...
static void UpdateLastRemovedPtr(char *filename) {
  ...
  SpinLockAcquire(&XLogCtl->info_lck);
  if (segno > XLogCtl->lastRemovedSegNo) XLogCtl->lastRemovedSegNo = segno;
  SpinLockRelease(&XLogCtl->info_lck);
}
 ...
 */
static void RemoveNonParentXlogFiles(XLogRecPtr switchpoint,
                                     TimeLineID newTLI) {
  DIR *xldir;
  struct dirent *xlde;
  ...
  while ((xlde = ReadDir(xldir, XLOGDIR)) != NULL) {
    /* Ignore files that are not XLOG segments */
    if (!IsXLogFileName(xlde->d_name)) continue;
...
static void RemoveXlogFile(const char *segname, XLogRecPtr PriorRedoPtr,
   ...
   */
  if (endlogSegNo <= recycleSegNo && lstat(path, &statbuf) == 0 &&
...
static void ValidateXLOGDirectoryStructure(void) {
  ...
  if (stat(XLOGDIR, &stat_buf) != 0 || !S_ISDIR(stat_buf.st_mode))
    ereport(FATAL,
            (errmsg("required WAL directory \"%s\" does not exist", XLOGDIR)));
  ...
  if (stat(path, &stat_buf) == 0) {
    ...
    if (!S_ISDIR(stat_buf.st_mode))
      ereport(FATAL,
              (errmsg("required WAL directory \"%s\" does not exist", path)));
  } else {
...
static XLogRecord *ReadRecord(XLogReaderState *xlogreader, XLogRecPtr RecPtr,
  ...
  for (;;) {
    ...
    if (record == NULL) {
       ...
       */
      if (errormsg)
        ereport(emode_for_corrupt_record(emode, RecPtr ? RecPtr : EndRecPtr),
     ...
     */
      ...
      ereport(emode_for_corrupt_record(emode, RecPtr ? RecPtr : EndRecPtr),
              (errmsg("unexpected timeline ID %u in log segment %s, offset %u",
                      xlogreader->latestPageTLI, fname, offset)));
      record = NULL;
    }
    ...
    if (record) {
      ...
      if (!InArchiveRecovery && ArchiveRecoveryRequested && !fetching_ckpt) {
        ereport(
            DEBUG1,
            (errmsg_internal(
                "reached end of WAL in pg_xlog, entering archive recovery")));
        InArchiveRecovery = true;
        if (StandbyModeRequested) StandbyMode = true;
...
static bool rescanLatestTimeLine(void) {
  ...
  if (!found) {
    ereport(
        LOG,
        (errmsg(
            "new___ timeline %u is not a child of database system timeline %u",
            newtarget, ThisTimeLineID)));
    return false;
  }
  ...
  if (currentTle->end < EndRecPtr) {
    ereport(LOG, (errmsg(
                     "new___ timeline %u forked off current database system "
                     "timeline %u before current recovery point %X/%X",
                     newtarget, ThisTimeLineID, (uint32)(EndRecPtr >> 32),
                     (uint32)EndRecPtr)));
    return false;
  }
  ...
  ereport(LOG, (errmsg("new___ target timeline is %u", recoveryTargetTLI)));
...
static void WriteControlFile(void) {
  ...
  if (sizeof(ControlFileData) > PG_CONTROL_SIZE)
    elog(PANIC,
         "sizeof(ControlFileData) is larger than PG_CONTROL_SIZE; fix either "
         "one");
...
static void ReadControlFile(void) {
  ...
  if (ControlFile->pg_control_version != PG_CONTROL_VERSION &&
    ...
    ereport(FATAL, (errmsg("database files are incompatible with server"),
                    errdetail(
                        ...
                        " but the server was compiled with PG_CONTROL_VERSION "
                        "%d (0x%08x).",
                        ControlFile->pg_control_version,
                        ControlFile->pg_control_version, PG_CONTROL_VERSION,
                        PG_CONTROL_VERSION),
                    errhint(
                        "This could be a problem of mismatched byte ordering.  "
  ...
  if (ControlFile->floatFormat != FLOATFORMAT_VALUE)
    ereport(FATAL,
            (errmsg("database files are incompatible with server"),
             errdetail(
                 "The database cluster appears to use a different "
                 "floating-point number format than the server executable."),
             errhint("It looks like you need to initdb.")));
  ...
  if (ControlFile->xlog_seg_size != XLOG_SEG_SIZE)
    ereport(FATAL,
            (errmsg("database files are incompatible with server"),
             errdetail(
                 "The database cluster was initialized with XLOG_SEG_SIZE %d,"
  ...
  if (ControlFile->float4ByVal != true)
    ereport(FATAL,
            (errmsg("database files are incompatible with server"),
             errdetail(
                 "The database cluster was initialized without USE_FLOAT4_BYVAL"
  ...
  if (ControlFile->float4ByVal != false)
    ereport(FATAL,
            (errmsg("database files are incompatible with server"),
             errdetail(
                 "The database cluster was initialized with USE_FLOAT4_BYVAL"
...
void UpdateControlFile(void) {
  ...
  errno = 0;
  if (write(fd, ControlFile, sizeof(ControlFileData)) !=
...
Size XLOGShmemSize(void) {
  ...
  size = add_size(
      size, mul_size(sizeof(WALInsertLockPadded), NUM_XLOGINSERT_LOCKS + 1));
  /* xlblocks array */
...
void XLOGShmemInit(void) {
  ...
  if (walDebugCxt == NULL) {
    walDebugCxt = AllocSetContextCreate(
        TopMemoryContext, "WAL Debug", ALLOCSET_DEFAULT_MINSIZE,
        ALLOCSET_DEFAULT_INITSIZE, ALLOCSET_DEFAULT_MAXSIZE);
    MemoryContextAllowInCriticalSection(walDebugCxt, true);
  }
#endif

  ControlFile = (ControlFileData *)ShmemInitStruct(
      "Control File", sizeof(ControlFileData), &foundCFile);
  XLogCtl =
      (XLogCtlData *)ShmemInitStruct("XLOG Ctl", XLOGShmemSize(), &foundXLog);
  ...
  XLogCtl->Insert.WALInsertLockTranche.array_stride =
   ...
   * reasons why).
   */
  if (!IsBootstrapProcessingMode()) ReadControlFile();
}
...
static void readRecoveryCommandFile(void) {
  ...
  if (fd == NULL) {
    if (errno == ENOENT) return; /* not there, so no archive recovery */
  ...
  for (item = head; item; item = item->next) {
    if (strcmp(item->name, "restore_command") == 0) {
      ...
      ereport(DEBUG2, (errmsg_internal("restore_command = '%s'",
                                       recoveryRestoreCommand)));
    } else if (strcmp(item->name, "recovery_end_command") == 0) {
      recoveryEndCommand = pstrdup(item->value);
      ereport(DEBUG2, (errmsg_internal("recovery_end_command = '%s'",
                                       recoveryEndCommand)));
    } else if (strcmp(item->name, "archive_cleanup_command") == 0) {
      archiveCleanupCommand = pstrdup(item->value);
      ereport(DEBUG2, (errmsg_internal("archive_cleanup_command = '%s'",
      ...
      if (strcmp(item->value, "pause") == 0)
        ...
        ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                        ...
                        errhint(
                            "The allowed values are \"pause\", \"promote\" and "
                            "\"shutdown\".")));

      ereport(DEBUG2,
              (errmsg_internal("recovery_target_action = '%s'", item->value)));
      ...
      if (errno == EINVAL || errno == ERANGE)
        ereport(FATAL,
                (errmsg("recovery_target_xid is not a valid number: \"%s\"",
                        item->value)));
      ereport(DEBUG2,
              (errmsg_internal("recovery_target_xid = %u", recoveryTargetXid)));
      ...
      recoveryTargetTime = DatumGetTimestampTz(
          DirectFunctionCall3(timestamptz_in, CStringGetDatum(item->value),
      ...
      ereport(DEBUG2, (errmsg_internal("recovery_target_name = '%s'",
                                       recoveryTargetName)));
      ...
      ereport(DEBUG2, (errmsg_internal("recovery_target = '%s'", item->value)));
      ...
      ereport(DEBUG2,
              (errmsg_internal("recovery_target_inclusive = %s", item->value)));
      ...
      ereport(DEBUG2, (errmsg_internal("recovery_min_apply_delay = '%s'",
                                       item->value)));
    } else
      ereport(FATAL,
              (errmsg("unrecognized recovery parameter \"%s\"", item->name)));
  }
  ...
  if (StandbyModeRequested) {
    if (PrimaryConnInfo == NULL && recoveryRestoreCommand == NULL)
      ereport(WARNING,
              (errmsg(
                   "recovery command file \"%s\" specified neither "
                   "primary_conninfo nor restore_command",
                   RECOVERY_COMMAND_FILE),
               errhint(
                   "The database server will regularly poll the pg_xlog "
                   "subdirectory to check for files placed there.")));
  } else {
    if (recoveryRestoreCommand == NULL)
      ereport(FATAL, (errmsg(
                         "recovery command file \"%s\" must specify "
  ...
  if (recoveryTargetAction == RECOVERY_TARGET_ACTION_PAUSE &&
      recoveryTargetActionSet && !EnableHotStandby)
    recoveryTargetAction = RECOVERY_TARGET_ACTION_SHUTDOWN;
...
static bool recoveryStopsAfter(XLogReaderState *record) {
  ...
  if (xact_info == XLOG_XACT_COMMIT || xact_info == XLOG_XACT_COMMIT_PREPARED ||
    ...
    if (getRecordTimestamp(record, &recordXtime)) SetLatestXTime(recordXtime);
...
static void recoveryPausesHere(void) {
  ...
  if (!LocalHotStandbyActive) return;
...
static bool recoveryApplyDelay(XLogReaderState *record) {
   ...
   * record
   */
  TimestampDifference(GetCurrentTimestamp(), recoveryDelayUntilTime, &secs,
                      &microsecs);
  if (secs <= 0 && microsecs <= 0) return false;
  ...
  while (true) {
    ...
    if (CheckForStandbyTrigger()) break;
    ...
    if (secs <= 0 && microsecs / 1000 <= 0) break;

    elog(DEBUG2, "recovery apply delay %ld seconds, %d milliseconds", secs,
         microsecs / 1000);
  ...
  do {                                                                     \
    if ((currValue) < (minValue))                                          \
      ereport(ERROR,                                                       \
              (errcode(ERRCODE_INVALID_PARAMETER_VALUE),                   \
               errmsg(                                                     \
                   "hot standby is not possible because "                  \
...
#define RecoveryRequiresBoolParameter(param_name, currValue, masterValue)    \
  do {                                                                       \
    ...
    if (_currValue != _masterValue)                                          \
      ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),              \
                      errmsg(                                                \
                          "hot standby is not possible because it requires " \
                          "\"%s\" to be same on master and standby (master " \
                          "has \"%s\", standby has \"%s\")",                 \
                          param_name, _masterValue ? "true" : "false",       \
...
static void CheckRequiredParameterValues(void) {
  ...
  if (ArchiveRecoveryRequested && ControlFile->wal_level == WAL_LEVEL_MINIMAL) {
    ereport(
        ...
        (errmsg(
         ...
         errhint(
             "This happens if you temporarily set wal_level=minimal without "
             "taking a new___ base backup.")));
  }
  ...
  if (ArchiveRecoveryRequested && EnableHotStandby) {
    if (ControlFile->wal_level < WAL_LEVEL_HOT_STANDBY)
      ereport(ERROR,
              (errmsg(
                   "hot standby is not possible because wal_level was not set "
                   "to \"hot_standby\" or higher on the master server"),
               errhint(
                   "Either set wal_level to \"hot_standby\" on the master, or "
...
/* This is just to allow attaching to startup process with a debugger */
  ...
  if (ArchiveRecoveryRequested) {
    if (StandbyModeRequested)
      ...
      ereport(LOG, (errmsg("starting point-in-time recovery to XID %u",
                           recoveryTargetXid)));
   ...
   * recovery.
   */
  if (StandbyModeRequested) OwnLatch(&XLogCtl->recoveryWakeupLatch);
    ...
    if (record != NULL) {
      ...
      if (checkPoint.redo < checkPointLoc) {
        if (!ReadRecord(xlogreader, checkPoint.redo, LOG, false))
          ereport(FATAL, (errmsg(
                              "could not find redo location referenced by "
                              "checkpoint record"),
                          errhint(
                              "If you are not restoring from a backup, try "
                              "removing the file \"%s/backup_label\".",
                              DataDir)));
      }
    } else {
  ...
  if (tliOfPointInHistory(checkPointLoc, expectedTLEs) !=
     ...
     */
    switchpoint = tliSwitchPoint(ControlFile->checkPointCopy.ThisTimeLineID,
                                 expectedTLEs, NULL);
    ereport(
        FATAL,
        (errmsg("requested timeline %u is not a child of this server's history",
                recoveryTargetTLI),
         errdetail(
             "Latest checkpoint is at %X/%X on timeline %u, but in the history "
             "of the requested timeline, the server forked off from that "
             "timeline at %X/%X.",
  ...
  if (!XLogRecPtrIsInvalid(ControlFile->minRecoveryPoint) &&
    ...
    ereport(FATAL, (errmsg(
                       "requested timeline %u does not contain minimum "
  ...
  ereport(DEBUG1, (errmsg("next transaction ID: %u/%u; next OID: %u",
                          checkPoint.nextXidEpoch, checkPoint.nextXid,
                          checkPoint.nextOid)));
  ereport(DEBUG1, (errmsg("next MultiXactId: %u; next MultiXactOffset: %u",
                          checkPoint.nextMulti, checkPoint.nextMultiOffset)));
  ereport(DEBUG1, (errmsg("oldest unfrozen transaction ID: %u, in database %u",
                          checkPoint.oldestXid, checkPoint.oldestXidDB)));
  ereport(DEBUG1, (errmsg("oldest MultiXactId: %u, in database %u",
                          checkPoint.oldestMulti, checkPoint.oldestMultiDB)));
  ereport(DEBUG1,
          (errmsg("commit timestamp Xid oldest/newest: %u/%u",
                  checkPoint.oldestCommitTs, checkPoint.newestCommitTs)));
  ...
  SetTransactionIdLimit(checkPoint.oldestXid, checkPoint.oldestXidDB);
  SetMultiXactIdLimit(checkPoint.oldestMulti, checkPoint.oldestMultiDB);
  SetCommitTsLimit(checkPoint.oldestCommitTs, checkPoint.newestCommitTs);
  ...
  if (InRecovery) {
    ...
    for (rmid = 0; rmid <= RM_MAX_ID; rmid++) {
      if (RmgrTable[rmid].rm_startup != NULL) RmgrTable[rmid].rm_startup();
    }
    ...
    if (record != NULL) {
      ...
      ereport(LOG, (errmsg("redo starts at %X/%X", (uint32)(ReadRecPtr >> 32),
                           (uint32)ReadRecPtr)));
      ...
      if (reachedStopPoint) {
        if (!reachedConsistency)
          ereport(FATAL, (errmsg(
                             "requested recovery stop point is before "
      ...
      for (rmid = 0; rmid <= RM_MAX_ID; rmid++) {
        if (RmgrTable[rmid].rm_cleanup != NULL) RmgrTable[rmid].rm_cleanup();
      }

      ereport(LOG, (errmsg("redo done at %X/%X", (uint32)(ReadRecPtr >> 32),
                           (uint32)ReadRecPtr)));
   ...
   */
  if (InRecovery) ResetUnloggedRelations(UNLOGGED_RELATION_INIT);

  /*
   * We don't need the latch anymore. It's not strictly necessary to disown
   * it, but let's do it for the sake of tidiness.
   */
  if (StandbyModeRequested) DisownLatch(&XLogCtl->recoveryWakeupLatch);
   ...
   */
  if (InRecovery && (EndOfLog < minRecoveryPoint ||
    ...
    if (ArchiveRecoveryRequested || ControlFile->backupEndRequired) {
      if (ControlFile->backupEndRequired)
        ereport(FATAL, (errmsg("WAL ends before end of online backup"),
                        errhint(
                            "All WAL generated while online backup was taken "
                            "must be available at recovery.")));
      else if (!XLogRecPtrIsInvalid(ControlFile->backupStartPoint))
        ereport(FATAL, (errmsg("WAL ends before end of online backup"),
                        errhint(
                            "Online backup started with pg_start_backup() must "
                            "be ended with pg_stop_backup(), and all WAL up to "
  ...
  if (ArchiveRecoveryRequested) {
    ...
    ereport(LOG, (errmsg("selected new___ timeline ID: %u", ThisTimeLineID)));
   ...
   */
  if (ArchiveRecoveryRequested) exitArchiveRecovery(EndOfLogTLI, EndOfLog);
  ...
  if (InRecovery) {
    ...
    if (bgwriterLaunched) {
      ...
      if (!fast_promoted)
        RequestCheckpoint(CHECKPOINT_END_OF_RECOVERY | CHECKPOINT_IMMEDIATE |
                          CHECKPOINT_WAIT);
    } else
   ...
   */
  if (fast_promoted) RequestCheckpoint(CHECKPOINT_FORCE);
}
...
static void CheckRecoveryConsistency(void) {
   ...
   */
  if (XLogRecPtrIsInvalid(minRecoveryPoint)) return;
...
bool XLogInsertAllowed(void) {
   ...
   * done.
   */
  if (LocalXLogInsertAllowed >= 0) return (bool)LocalXLogInsertAllowed;
...
XLogRecPtr GetRedoRecPtr(void) {
  ...
  if (RedoRecPtr < ptr) RedoRecPtr = ptr;
...
void GetNextXidAndEpoch(TransactionId *xid, uint32 *epoch) {
   ...
   */
  if (nextXid < ckptXid) ckptXidEpoch++;
...
void ShutdownXLOG(int code, Datum arg) {
  ...
  if (RecoveryInProgress())
     ...
     */
    if (XLogArchivingActive() && XLogArchiveCommandSet()) RequestXLogSwitch();
...
static void LogCheckpointEnd(bool restartpoint) {
  ...
  TimestampDifference(CheckpointStats.ckpt_write_t, CheckpointStats.ckpt_sync_t,
                      &write_secs, &write_usecs);

  TimestampDifference(CheckpointStats.ckpt_sync_t,
                      CheckpointStats.ckpt_sync_end_t, &sync_secs, &sync_usecs);
  ...
  TimestampDifference(CheckpointStats.ckpt_start_t, CheckpointStats.ckpt_end_t,
  ...
  longest_usecs =
      CheckpointStats.ckpt_longest_sync - (uint64)longest_secs * 1000000;
  ...
  elog(LOG,
       ...
       CheckpointStats.ckpt_segs_added, CheckpointStats.ckpt_segs_removed,
       CheckpointStats.ckpt_segs_recycled, write_secs, write_usecs / 1000,
       sync_secs, sync_usecs / 1000, total_secs, total_usecs / 1000,
       CheckpointStats.ckpt_sync_rels, longest_secs, longest_usecs / 1000,
...
void CreateCheckPoint(int flags) {
   ...
   */
  if (flags & CHECKPOINT_END_OF_RECOVERY) LocalSetXLogInsertAllowed();
   ...
   */
  if (log_checkpoints) LogCheckpointStart(flags, false);
  ...
  if (!shutdown) checkPoint.nextOid += ShmemVariableCache->oidCount;
  LWLockRelease(OidGenLock);

  MultiXactGetCheckptMulti(shutdown, &checkPoint.nextMulti,
                           &checkPoint.nextMultiOffset, &checkPoint.oldestMulti,
   ...
   */
  if (!shutdown && XLogStandbyInfoActive()) LogStandbySnapshot();
  ...
  recptr = XLogInsert(
      RM_XLOG_ID, shutdown ? XLOG_CHECKPOINT_SHUTDOWN : XLOG_CHECKPOINT_ONLINE);
  ...
  if (shutdown && checkPoint.redo != ProcLastRecPtr)
    ereport(PANIC, (errmsg(
                       "concurrent transaction log activity while database "
   ...
   */
  LWLockAcquire(ControlFileLock, LW_EXCLUSIVE);
  if (shutdown) ControlFile->state = DB_SHUTDOWNED;
  ControlFile->prevCheckPoint = ControlFile->checkPoint;
   ...
   */
  if (!shutdown) PreallocXlogFiles(recptr);
   ...
   */
  if (!RecoveryInProgress()) TruncateSUBTRANS(GetOldestXmin(NULL, false));
  ...
  TRACE_POSTGRESQL_CHECKPOINT_DONE(CheckpointStats.ckpt_bufs_written, NBuffers,
...
static void CreateEndOfRecoveryRecord(void) {
  ...
  /* sanity check */
  if (!RecoveryInProgress()) elog(ERROR, "can only be used to end recovery");
...
bool CreateRestartPoint(int flags) {
  ...
  if (XLogRecPtrIsInvalid(lastCheckPointRecPtr) ||
    ...
    ereport(DEBUG2, (errmsg("skipping restartpoint, already performed at %X/%X",
   ...
   */
  if (EnableHotStandby) TruncateSUBTRANS(GetOldestXmin(NULL, false));
...
XLogRecPtr XLogRestorePoint(const char *rpName) {
  ...
  ereport(LOG, (errmsg("restore point \"%s\" created at %X/%X", rpName,
...
void UpdateFullPageWrites(void) {
   ...
   * can update it.
   */
  if (fullPageWrites == Insert->fullPageWrites) return;
...
static void checkTimeLineSwitch(XLogRecPtr lsn, TimeLineID newTLI,
  ...
  if (prevTLI != ThisTimeLineID)
    ereport(PANIC, (errmsg(
                       "unexpected previous timeline ID %u (current timeline "
                       "ID %u) in checkpoint record",
  ...
  if (!XLogRecPtrIsInvalid(minRecoveryPoint) && lsn < minRecoveryPoint &&
    ...
    ereport(PANIC, (errmsg(
                       "unexpected timeline ID %u in checkpoint record, before "
                       "reaching minimum recovery point %X/%X on timeline %u",
                       newTLI, (uint32)(minRecoveryPoint >> 32),
                       (uint32)minRecoveryPoint, minRecoveryPointTLI)));
...
void xlog_redo(XLogReaderState *record) {
  ...
  if (info == XLOG_NEXTOID) {
    ...
    if (XLogReadBufferForRedo(record, 0, &buffer) != BLK_RESTORED)
      elog(ERROR,
           "unexpected XLogReadBufferForRedo result when restoring backup "
           "block");
...
static void xlog_outrec(StringInfo buf, XLogReaderState *record) {
  ...
  appendStringInfo(buf, "prev %X/%X; xid %u",
                   ...
                   (uint32)XLogRecGetPrev(record), XLogRecGetXid(record));
  ...
  for (block_id = 0; block_id <= record->max_block_id; block_id++) {
    ...
    if (forknum != MAIN_FORKNUM)
      appendStringInfo(buf, "; blkref #%u: rel %u/%u/%u, fork %u, blk %u",
                       block_id, rnode.spcNode, rnode.dbNode, rnode.relNode,
                       forknum, blk);
    else
      appendStringInfo(buf, "; blkref #%u: rel %u/%u/%u, blk %u", block_id,
                       rnode.spcNode, rnode.dbNode, rnode.relNode, blk);
    if (XLogRecHasBlockImage(record, block_id)) appendStringInfo(buf, " FPW");
  }
}
#endif /* WAL_DEBUG */
...
static int get_sync_bit(int method) {
   ...
   */
  if (!XLogIsNeeded() && !AmWalReceiverProcess()) o_direct_flag = PG_O_DIRECT;
 ...
 * Instead, there would-be contents are returned in *labelfile and
 **tblspcmapfile,
 ...
 */
XLogRecPtr do_pg_start_backup(const char *backupidstr, bool fast,
                              TimeLineID *starttli_p, char **labelfile,
                              DIR *tblspcdir, List **tablespaces,
  ...
  if (!backup_started_in_recovery && !XLogIsNeeded())
    ereport(ERROR,
            (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
             ...
             errhint(
                 "wal_level must be set to \"archive\", \"hot_standby\", or "
  ...
  PG_ENSURE_ERROR_CLEANUP(pg_start_backup_callback,
                          (Datum)BoolGetDatum(exclusive));
  {
     ...
     */
    if (!backup_started_in_recovery) RequestXLogSwitch();
    ...
    while ((de = ReadDir(tblspcdir, "pg_tblspc")) != NULL) {
      ...
      if (tablespaces) *tablespaces = lappend(*tablespaces, ti);
    ...
    if (exclusive) {
      ...
      if (stat(BACKUP_LABEL_FILE, &stat_buf) != 0) {
        ...
        ereport(ERROR, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                        errmsg("a backup is already in progress"),
                        errhint(
                            "If you're sure there is no backup in progress, "
                            "remove file \"%s\" and try again.",
                            BACKUP_LABEL_FILE)));
      ...
      if (fwrite(labelfbuf.data, labelfbuf.len, 1, fp) != 1 ||
          fflush(fp) != 0 || pg_fsync(fileno(fp)) != 0 || ferror(fp) ||
          FreeFile(fp))
        ereport(ERROR,
                (errcode_for_file_access(),
                 errmsg("could not write file \"%s\": %m", BACKUP_LABEL_FILE)));
      ...
      if (tblspc_mapfbuf.len > 0) {
        if (stat(TABLESPACE_MAP, &stat_buf) != 0) {
          if (errno != ENOENT)
            ereport(ERROR,
                    (errcode_for_file_access(),
                     errmsg("could not stat file \"%s\": %m", TABLESPACE_MAP)));
        } else
          ereport(ERROR, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                          errmsg("a backup is already in progress"),
                          errhint(
        ...
        if (!fp)
          ereport(ERROR,
                  (errcode_for_file_access(),
                   errmsg("could not create file \"%s\": %m", TABLESPACE_MAP)));
        if (fwrite(tblspc_mapfbuf.data, tblspc_mapfbuf.len, 1, fp) != 1 ||
            fflush(fp) != 0 || pg_fsync(fileno(fp)) != 0 || ferror(fp) ||
            FreeFile(fp))
          ereport(ERROR,
                  (errcode_for_file_access(),
                   errmsg("could not write file \"%s\": %m", TABLESPACE_MAP)));
      }

      pfree(tblspc_mapfbuf.data);
    } else {
      *labelfile = labelfbuf.data;
      if (tblspc_mapfbuf.len > 0) *tblspcmapfile = tblspc_mapfbuf.data;
    }
  }
  PG_END_ENSURE_ERROR_CLEANUP(pg_start_backup_callback,
                              (Datum)BoolGetDatum(exclusive));

  /*
   * We're done.  As a convenience, return the starting WAL location.
   */
  if (starttli_p) *starttli_p = starttli;
  return startpoint;
}
...
XLogRecPtr do_pg_stop_backup(char *labelfile, bool waitforarchive,
  ...
  if (exclusive) {
    ...
    if (stat(BACKUP_LABEL_FILE, &statbuf)) {
      if (errno != ENOENT)
        ereport(ERROR,
                (errcode_for_file_access(),
                 errmsg("could not stat file \"%s\": %m", BACKUP_LABEL_FILE)));
      ereport(ERROR, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
    ...
    if (unlink(BACKUP_LABEL_FILE) != 0)
      ereport(ERROR,
              (errcode_for_file_access(),
               errmsg("could not remove file \"%s\": %m", BACKUP_LABEL_FILE)));
   ...
   */
  if (sscanf(labelfile, "START WAL LOCATION: %X/%X (file %24s)%c", &hi, &lo,
             startxlogfilename, &ch) != 4 ||
      ch != '\n')
    ereport(ERROR, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
  ...
  if (backup_started_in_recovery) {
    ...
    if (stoptli_p) *stoptli_p = stoptli;
    return stoppoint;
  }
  ...
  fprintf(fp, "STOP WAL LOCATION: %X/%X (file %s)\n", (uint32)(stoppoint >> 32),
          (uint32)stoppoint, stopxlogfilename);
  ...
  if (waitforarchive && XLogArchivingActive()) {
    ...
    while (XLogArchiveIsBusy(lastxlogfilename) ||
      ...
      if (++waits >= seconds_before_warning) {
        ...
        ereport(WARNING,
                (errmsg(
                     "pg_stop_backup still waiting for all required WAL "
                     "segments to be archived (%d seconds elapsed)",
                     waits),
                 errhint(
                     "Check that your archive_command is executing properly.  "
                     "pg_stop_backup can be canceled safely, "
                     "but the database backup will not be usable without all "
                     "the WAL segments.")));
      }
    }

    ereport(NOTICE, (errmsg(
                        "pg_stop_backup complete, all required WAL segments "
                        "have been archived")));
  } else if (waitforarchive)
    ereport(NOTICE, (errmsg(
                        "WAL archiving is not enabled; you must ensure that "
                        "all required WAL segments are copied through other "
                        "means to complete the backup")));
...
static bool read_backup_label(XLogRecPtr *checkPointLoc,
  ...
  *backupFromStandby = false;
   ...
   * format).
   */
  if (fscanf(lfp, "START WAL LOCATION: %X/%X (file %08X%16s)%c", &hi, &lo, &tli,
             startxlogfilename, &ch) != 5 ||
      ch != '\n')
    ereport(FATAL, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                    errmsg("invalid data in file \"%s\"", BACKUP_LABEL_FILE)));
  RedoStartLSN = ((uint64)hi) << 32 | lo;
  if (fscanf(lfp, "CHECKPOINT LOCATION: %X/%X%c", &hi, &lo, &ch) != 3 ||
      ch != '\n')
    ereport(FATAL, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                    errmsg("invalid data in file \"%s\"", BACKUP_LABEL_FILE)));
  *checkPointLoc = ((uint64)hi) << 32 | lo;
  ...
  if (fscanf(lfp, "BACKUP METHOD: %19s\n", backuptype) == 1) {
    if (strcmp(backuptype, "streamed") == 0) *backupEndRequired = true;
  }

  if (fscanf(lfp, "BACKUP FROM: %19s\n", backupfrom) == 1) {
    if (strcmp(backupfrom, "standby") == 0) *backupFromStandby = true;
  }
...
void CancelBackup(void) {
  ...
  if (stat(BACKUP_LABEL_FILE, &stat_buf) < 0) return;
  ...
  if (rename(BACKUP_LABEL_FILE, BACKUP_LABEL_OLD) == 0) {
    ereport(LOG, (errmsg("online backup mode canceled"),
                  errdetail("\"%s\" was renamed to \"%s\".", BACKUP_LABEL_FILE,
                            BACKUP_LABEL_OLD)));
  } else {
  ...
  if (stat(TABLESPACE_MAP, &stat_buf) < 0) return;
  ...
  if (rename(TABLESPACE_MAP, TABLESPACE_MAP_OLD) == 0) {
    ereport(LOG, (errmsg("online backup mode canceled"),
                  errdetail("\"%s\" was renamed to \"%s\".", TABLESPACE_MAP,
                            TABLESPACE_MAP_OLD)));
  } else {
    ereport(WARNING, (errcode_for_file_access(),
                      errmsg("online backup mode was not canceled"),
...
retry:
  ...
  if (readFile < 0 || (readSource == XLOG_FROM_STREAM &&
                       receivedUpto < targetPagePtr + reqLen)) {
    if (!WaitForWALToBecomeAvailable(targetPagePtr + reqLen,
                                     private___->randAccess,
                                     private___->fetching_ckpt, targetRecPtr)) {
  ...
  if (lseek(readFile, (off_t)readOff, SEEK_SET) < 0) {
    ...
    ereport(emode_for_corrupt_record(emode, targetPagePtr + reqLen),
            (errcode_for_file_access(),
             errmsg("could not seek in log segment %s to offset %u: %m", fname,
                    readOff)));
  ...
  if (read(readFile, readBuf, XLOG_BLCKSZ) != XLOG_BLCKSZ) {
    ...
    ereport(emode_for_corrupt_record(emode, targetPagePtr + reqLen),
            (errcode_for_file_access(),
             errmsg("could not read from log segment %s, offset %u: %m", fname,
                    readOff)));
    goto next_record_is_invalid;
  }
  ...
  for (;;) {
    ...
    if (lastSourceFailed) {
      switch (currentSource) {
          ...
          if (PrimaryConnInfo) {
            ...
            if (fetching_ckpt) {
              ...
              if (curFileTLI > 0 && tli < curFileTLI)
                elog(ERROR,
                     "according to history file, WAL location %X/%X belongs to "
                     "timeline %u, but previous recovered WAL file came from "
                     "timeline %u",
                     (uint32)(ptr >> 32), (uint32)ptr, tli, curFileTLI);
            }
            curFileTLI = tli;
        ...
        case XLOG_FROM_STREAM:
           ...
           */
          if (WalRcvStreaming()) ShutdownWalRcv();
          ...
          if (!TimestampDifferenceExceeds(last_fail_time, now,
            ...
            wait_time =
                wal_retrieve_retry_interval - (secs * 1000 + usecs / 1000);
       ...
       */
      if (InArchiveRecovery) currentSource = XLOG_FROM_ARCHIVE;
    }
    ...
    switch (currentSource) {
      ...
      case XLOG_FROM_PG_XLOG:
        ...
        if (readFile >= 0) return true; /* success! */
      ...
      case XLOG_FROM_STREAM: {
        ...
        if (havedata) {
           ...
           */
          if (readFile < 0) {
            if (!expectedTLEs) expectedTLEs = readTimeLineHistory(receiveTLI);
            readFile = XLogFileRead(readSegNo, PANIC, receiveTLI,
...
static bool CheckForStandbyTrigger(void) {
  ...
  if (TriggerFile == NULL) return false;
  ...
  if (stat(TriggerFile, &stat_buf) == 0) {
    ...
    ereport(ERROR,
            (errcode_for_file_access(),
             errmsg("could not stat trigger file \"%s\": %m", TriggerFile)));
 ...
 */
void WakeupRecovery(void) { SetLatch(&XLogCtl->recoveryWakeupLatch); }
