...
XLogReaderState *XLogReaderAllocate(XLogPageReadCB pagereadfunc,
  ...
  state->errormsg_buf = static_cast<char *>(
...
void XLogReaderFree(XLogReaderState *state) {
  ...
  for (block_id = 0; block_id <= state->max_block_id; block_id++) {
    if (state->blocks[block_id].in_use) {
      if (state->blocks[block_id].data) pfree(state->blocks[block_id].data);
    }
  }
  if (state->main_data) pfree(state->main_data);

  pfree(state->errormsg_buf);
  if (state->readRecordBuf) pfree(state->readRecordBuf);
  pfree(state->readBuf);
  pfree(state);
}
 ...
 */
XLogRecord *XLogReadRecord(XLogReaderState *state, XLogRecPtr RecPtr,
                           char **errormsg) {
  XLogRecord *record;
  ...
  *errormsg = NULL;
  ...
  if (RecPtr == InvalidXLogRecPtr) {
    ...
    if (state->ReadRecPtr == InvalidXLogRecPtr) randAccess = true;
  ...
  if (total_len > len) {
    ...
    do {
      ...
      if (pageHeader->xlp_rem_len == 0 ||
        ...
        report_invalid_record(state, "invalid contrecord length %u at %X/%X",
                              pageHeader->xlp_rem_len, (uint32)(RecPtr >> 32),
                              (uint32)RecPtr);
        goto err;
      }
      ...
      if (pageHeader->xlp_rem_len < len) len = pageHeader->xlp_rem_len;
    ...
    record = (XLogRecord *)state->readRecordBuf;
    if (!ValidXLogRecord(state, record, RecPtr)) goto err;
...
err:
  ...
  if (state->errormsg_buf[0] != '\0') *errormsg = state->errormsg_buf;
...
static int ReadPageInternal(XLogReaderState *state, XLogRecPtr pageptr,
  ...
  if (targetSegNo != state->readSegNo && targetPageOff != 0) {
    ...
    if (!ValidXLogPageHeader(state, targetSegmentPtr, hdr)) goto err;
  }
   ...
   */
  readLen =
      state->read_page(state, pageptr, Max(reqLen, SizeOfXLogShortPHD),
                       state->currRecPtr, state->readBuf, &state->readPageTLI);
  if (readLen < 0) goto err;

  Assert(readLen <= XLOG_BLCKSZ);

  /* Do we have enough data to check the header length? */
  if (readLen <= SizeOfXLogShortPHD) goto err;
   ...
   */
  if (!ValidXLogPageHeader(state, pageptr, hdr)) goto err;
...
static bool ValidXLogPageHeader(XLogReaderState *state, XLogRecPtr recptr,
  ...
  if (hdr->xlp_magic != XLOG_PAGE_MAGIC) {
    ...
    report_invalid_record(
        state, "invalid magic number %04X in log segment %s, offset %u",
  ...
  if (hdr->xlp_info & XLP_LONG_HEADER) {
    ...
    if (state->system_identifier &&
      ...
      report_invalid_record(state,
                            ...
                            "file database system identifier is %s, pg_control "
  ...
  if (hdr->xlp_pageaddr != recaddr) {
    ...
    report_invalid_record(
        state, "unexpected pageaddr %X/%X in log segment %s, offset %u",
        (uint32)(hdr->xlp_pageaddr >> 32), (uint32)hdr->xlp_pageaddr, fname,
        offset);
    return false;
  }
  ...
  if (recptr > state->latestPagePtr) {
    if (hdr->xlp_tli < state->latestPageTLI) {
      ...
      report_invalid_record(state,
                            "out-of-sequence timeline ID %u (after %u) in log "
                            "segment %s, offset %u",
 ...
 */
bool DecodeXLogRecord(XLogReaderState *state, XLogRecord *record,
                      char **errormsg) {
/*
...
#define COPY_HEADER_FIELD(_dst, _size)         \
  do {                                         \
    if (remaining < _size) goto shortdata_err; \
    ...
    if (block_id == XLR_BLOCK_ID_DATA_SHORT) {
      ...
      if (block_id <= state->max_block_id) {
        report_invalid_record(state, "out-of-order block_id %u at %X/%X",
                              block_id, (uint32)(state->ReadRecPtr >> 32),
      ...
      if (blk->has_image) {
        ...
        if ((blk->bimg_info & BKPIMAGE_HAS_HOLE) &&
          ...
          report_invalid_record(
              ...
              (unsigned int)blk->bimg_len, (uint32)(state->ReadRecPtr >> 32),
              (uint32)state->ReadRecPtr);
          goto err;
        }
        /*
        ...
        if (!(blk->bimg_info & BKPIMAGE_HAS_HOLE) &&
          ...
          report_invalid_record(
              state,
              "BKPIMAGE_HAS_HOLE not set, but hole offset %u length %u at "
              "%X/%X",
              (unsigned int)blk->hole_offset, (unsigned int)blk->hole_length,
              (uint32)(state->ReadRecPtr >> 32), (uint32)state->ReadRecPtr);
          goto err;
        ...
        if (!(blk->bimg_info & BKPIMAGE_HAS_HOLE) &&
          ...
          report_invalid_record(
              state,
              "neither BKPIMAGE_HAS_HOLE nor BKPIMAGE_IS_COMPRESSED set, but "
  ...
  if (remaining != datatotal) goto shortdata_err;
 ...
 */
bool XLogRecGetBlockTag(XLogReaderState *record, uint8 block_id,
                        RelFileNode *rnode, ForkNumber *forknum,
                        BlockNumber *blknum) {
  DecodedBkpBlock *bkpb;

  if (!record->blocks[block_id].in_use) return false;

  bkpb = &record->blocks[block_id];
  if (rnode) *rnode = bkpb->rnode;
  if (forknum) *forknum = bkpb->forknum;
  if (blknum) *blknum = bkpb->blkno;
  return true;
}
...
char *XLogRecGetBlockData(XLogReaderState *record, uint8 block_id, Size *len) {
  ...
  if (!record->blocks[block_id].in_use) return NULL;
...
bool RestoreBlockImage(XLogReaderState *record, uint8 block_id, char *page) {
  ...
  if (!record->blocks[block_id].in_use) return false;
  if (!record->blocks[block_id].has_image) return false;
  ...
  if (bkpb->bimg_info & BKPIMAGE_IS_COMPRESSED) {
    ...
    if (pglz_decompress(ptr, bkpb->bimg_len, tmp, BLCKSZ - bkpb->hole_length) <
        0) {
      report_invalid_record(record,
                            "invalid compressed image at %X/%X, block %d",
