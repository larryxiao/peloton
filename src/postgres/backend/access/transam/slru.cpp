...
static bool SlruScanDirCbDeleteCutoff(SlruCtl ctl, char *filename, int segpage,
                                      void *data);
...
void SimpleLruInit(SlruCtl ctl, const char *name, int nslots, int nlsns,
  ...
  shared = (SlruShared)ShmemInitStruct(name, SimpleLruShmemSize(nslots, nlsns),
                                       &found);
 ...
 *
 * This should be called each time we create a new___ page, and each time we
 *read
 ...
 * Control lock must be held at entry, and will be held at exit.
 */
int SimpleLruReadPage(SlruCtl ctl, int pageno, bool write_ok,
                      TransactionId xid) {
  SlruShared shared = ctl->shared;
...
static void SlruInternalWritePage(SlruCtl ctl, int slotno, SlruFlush fdata) {
  ...
  if (!ok && fdata) {
    ...
    for (i = 0; i < fdata->num_files; i++) CloseTransientFile(fdata->fd[i]);
  }
  ...
  if (!ok) shared->page_dirty[slotno] = true;
  ...
  /* Now it's okay to ereport if we failed */
  if (!ok) SlruReportIOError(ctl, pageno, InvalidTransactionId);
}
...
static bool SlruPhysicalReadPage(SlruCtl ctl, int pageno, int slotno) {
  ...
  if (fd < 0) {
    ...
    ereport(LOG,
            (errmsg("file \"%s\" doesn't exist, reading as zeroes", path)));
    MemSet(shared->page_buffer[slotno], 0, BLCKSZ);
    return true;
  }
...
static bool SlruPhysicalWritePage(SlruCtl ctl, int pageno, int slotno,
  ...
  if (lseek(fd, (off_t)offset, SEEK_SET) < 0) {
    slru_errcause = SLRU_SEEK_FAILED;
    slru_errno = errno;
    if (!fdata) CloseTransientFile(fd);
    return false;
  }
...
static int SlruSelectLRUPage(SlruCtl ctl, int pageno) {
  ...
  for (;;) {
    ...
    for (slotno = 0; slotno < shared->num_slots; slotno++) {
      ...
      if (shared->page_status[slotno] == SLRU_PAGE_EMPTY) return slotno;
      ...
      if (this_page_number == shared->latest_page_number) continue;
     ...
     */
    if (!shared->page_dirty[bestvalidslot]) return bestvalidslot;
...
restart:
  ...
  for (slotno = 0; slotno < shared->num_slots; slotno++) {
    if (shared->page_status[slotno] == SLRU_PAGE_EMPTY) continue;
    if (!ctl->PagePrecedes(shared->page_number[slotno], cutoffPage)) continue;
...
void SlruDeleteSegment(SlruCtl ctl, char *filename) {
  char path[MAXPGPATH];
 ...
 */
bool SlruScanDirCbReportPresence(SlruCtl ctl, char *filename, int segpage,
                                 void *data) {
...
static bool SlruScanDirCbDeleteCutoff(SlruCtl ctl, char *filename, int segpage,
  ...
  if (ctl->PagePrecedes(segpage, cutoffPage)) SlruDeleteSegment(ctl, filename);
 ...
 */
bool SlruScanDirCbDeleteAll(SlruCtl ctl, char *filename, int segpage,
                            void *data) {
...
bool SlruScanDirectory(SlruCtl ctl, SlruScanCallback callback, void *data) {
  ...
  while ((clde = ReadDir(cldir, ctl->Dir)) != NULL) {
    ...
    if ((len == 4 || len == 5 || len == 6) &&
      ...
      elog(DEBUG2, "SlruScanDirectory invoking callback on %s/%s", ctl->Dir,
           clde->d_name);
