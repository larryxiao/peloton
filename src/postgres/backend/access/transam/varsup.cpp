    ...
    if (IsUnderPostmaster && TransactionIdFollowsOrEquals(xid, xidStopLimit)) {
      ...
      if (oldest_datname)
        ereport(ERROR, (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
                        errmsg(
                            "database is not accepting commands to avoid "
                            "wraparound data loss in database \"%s\"",
                            oldest_datname),
                        errhint(
                            "Stop the postmaster and vacuum that database in "
                            "single-user mode.\n"
    ...
    if (oldest_datname)
      ereport(WARNING,
              (errmsg("database \"%s\" must be vacuumed within %u transactions",
               ...
               errhint(
                   "To avoid a database shutdown, execute a database-wide "
...
Oid GetNewObjectId(void) {
  ...
  /* safety check, we should never get this far in a HS slave */
  if (RecoveryInProgress()) elog(ERROR, "cannot assign OIDs during recovery");
