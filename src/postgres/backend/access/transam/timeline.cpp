...
List *readTimeLineHistory(TimeLineID targetTLI) {
   ...
   * future reference.
   */
  if (fromArchive) KeepFileRestoredFromArchive(path, histfname);
...
void writeTimeLineHistory(TimeLineID newTLI, TimeLineID parentTLI,
  ...
  fd = OpenTransientFile(tmppath, O_RDWR | O_CREAT | O_EXCL, S_IRUSR | S_IWUSR);
  if (fd < 0)
  ...
  snprintf(buffer, sizeof(buffer), "%s%u\t%X/%X\t%s\n", (srcfd < 0) ? "" : "\n",
           parentTLI, (uint32)(switchpoint >> 32), (uint32)(switchpoint),
           reason);
 ...
 */
  ...
  if (link(tmppath, path) < 0)
    ereport(ERROR, (errcode_for_file_access(),
                    errmsg("could not link file \"%s\" to \"%s\": %m", tmppath,
                           path)));
  unlink(tmppath);
#else
...
bool tliInHistory(TimeLineID tli, List *expectedTLEs) {
  ...
  foreach (cell, expectedTLEs) {
    if (((TimeLineHistoryEntry *)lfirst(cell))->tli == tli) return true;
  }
...
XLogRecPtr tliSwitchPoint(TimeLineID tli, List *history, TimeLineID *nextTLI) {
  ...
  if (nextTLI) *nextTLI = 0;
  foreach (cell, history) {
    TimeLineHistoryEntry *tle = (TimeLineHistoryEntry *)lfirst(cell);

    if (tle->tli == tli) return tle->end;
    if (nextTLI) *nextTLI = tle->tli;
  }

  ereport(
      ERROR,
      (errmsg("requested timeline %u is not in this server's history", tli)));
