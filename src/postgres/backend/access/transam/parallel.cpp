...
ParallelContext *CreateParallelContext(parallel_worker_main_type entrypoint,
   ...
   * background workers.
   */
  if (dynamic_shared_memory_type == DSM_IMPL_NONE) nworkers = 0;
...
void InitializeParallelDSM(ParallelContext *pcxt) {
  ...
  if (pcxt->seg != NULL)
    pcxt->toc =
        shm_toc_create(PARALLEL_MAGIC, dsm_segment_address(pcxt->seg), segsize);
  else {
    pcxt->nworkers = 0;
  ...
  fps = (FixedParallelState *)shm_toc_allocate(pcxt->toc,
  ...
  if (pcxt->nworkers > 0) {
    ...
    libraryspace =
        static_cast<char *>(shm_toc_allocate(pcxt->toc, library_len));
    ...
    combocidspace =
        static_cast<char *>(shm_toc_allocate(pcxt->toc, combocidlen));
    ...
    pcxt->worker = static_cast<ParallelWorkerInfo *>(
        palloc0(sizeof(ParallelWorkerInfo) * pcxt->nworkers));
     ...
     */
    error_queue_space = static_cast<char *>(shm_toc_allocate(
        pcxt->toc, PARALLEL_ERROR_QUEUE_SIZE * pcxt->nworkers));
    ...
    if (pcxt->library_name != NULL) {
      ...
      extensionstate = static_cast<char *>(shm_toc_allocate(
          pcxt->toc, lnamelen + strlen(pcxt->function_name) + 2));
...
void WaitForParallelWorkersToFinish(ParallelContext *pcxt) {
  ...
  if (pcxt->toc != NULL) {
    ...
    fps = static_cast<FixedParallelState *>(
        shm_toc_lookup(pcxt->toc, PARALLEL_KEY_FIXED));
...
void DestroyParallelContext(ParallelContext *pcxt) {
  ...
  for (i = 0; i < pcxt->nworkers; ++i) {
    ...
    if (pcxt->worker[i].bgwhandle == NULL) continue;
 ...
 */
bool ParallelContextActive(void) { return !dlist_is_empty(&pcxt_list); }
 ...
 */
static void HandleParallelMessage(ParallelContext *pcxt, int i,
                                  StringInfo msg) {
  char msgtype;
  ...
  switch (msgtype) {
    ...
    default: {
      elog(ERROR, "unknown message type: %c (%d bytes)", msgtype, msg->len);
    }
  }
}
...
void AtEOSubXact_Parallel(bool isCommit, SubTransactionId mySubId) {
  while (!dlist_is_empty(&pcxt_list)) {
    ...
    if (pcxt->subid != mySubId) break;
    if (isCommit) elog(WARNING, "leaked parallel context");
...
static void ParallelWorkerMain(Datum main_arg) {
  ...
  CurrentMemoryContext = AllocSetContextCreate(
      TopMemoryContext, "parallel worker", ALLOCSET_DEFAULT_MINSIZE,
  ...
  fps = static_cast<FixedParallelState *>(
      shm_toc_lookup(toc, PARALLEL_KEY_FIXED));
  Assert(fps != NULL);
   ...
   */
  error_queue_space =
      static_cast<char *>(shm_toc_lookup(toc, PARALLEL_KEY_ERROR_QUEUE));
  ...
  tstatespace =
      static_cast<char *>(shm_toc_lookup(toc, PARALLEL_KEY_TRANSACTION_STATE));
  StartParallelWorkerTransaction(tstatespace);

  /* Restore combo CID state. */
  combocidspace =
      static_cast<char *>(shm_toc_lookup(toc, PARALLEL_KEY_COMBO_CID));
  Assert(combocidspace != NULL);
  RestoreComboCIDState(combocidspace);

  /* Restore transaction snapshot. */
  tsnapspace = static_cast<char *>(
      shm_toc_lookup(toc, PARALLEL_KEY_TRANSACTION_SNAPSHOT));
  ...
  asnapspace =
      static_cast<char *>(shm_toc_lookup(toc, PARALLEL_KEY_ACTIVE_SNAPSHOT));
...
static void ParallelExtensionTrampoline(dsm_segment *seg, shm_toc *toc) {
  ...
  extensionstate = static_cast<char *>(
      shm_toc_lookup(toc, PARALLEL_KEY_EXTENSION_TRAMPOLINE));
  ...
  entrypt = (parallel_worker_main_type)load_external_function(
      library_name, function_name, 1, NULL);
  entrypt(seg, toc);
}
...
void ParallelWorkerReportLastRecEnd(XLogRecPtr last_xlog_end) {
  ...
  Assert(fps != NULL);
  SpinLockAcquire(&fps->mutex);
  if (fps->last_xlog_end < last_xlog_end) fps->last_xlog_end = last_xlog_end;
  SpinLockRelease(&fps->mutex);
}
