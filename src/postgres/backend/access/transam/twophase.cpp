...
static void RecordTransactionCommitPrepared(
    TransactionId xid, int nchildren, TransactionId *children, int nrels,
    RelFileNode *rels, int ninvalmsgs, SharedInvalidationMessage *invalmsgs,
    bool initfileinval);
static void RecordTransactionAbortPrepared(TransactionId xid, int nchildren,
                                           TransactionId *children, int nrels,
                                           RelFileNode *rels);
...
Size TwoPhaseShmemSize(void) {
  ...
  size =
      add_size(size, mul_size(max_prepared_xacts, sizeof(GlobalTransaction)));
  size = MAXALIGN(size);
  size = add_size(size,
                  mul_size(max_prepared_xacts, sizeof(GlobalTransactionData)));
...
void TwoPhaseShmemInit(void) {
  ...
  TwoPhaseState = static_cast<TwoPhaseStateData *>(ShmemInitStruct(
      "Prepared Transaction Table", TwoPhaseShmemSize(), &found));
...
static GlobalTransaction LockGXact(const char *gid, Oid user) {
  ...
  for (i = 0; i < TwoPhaseState->numPrepXacts; i++) {
    ...
    if (!gxact->valid) continue;
    if (strcmp(gxact->gid, gid) != 0) continue;
    ...
    if (user != gxact->owner && !superuser_arg(user))
      ereport(ERROR,
              (errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
               ...
               errhint(
                   "Must be superuser or the user that prepared the "
                   "transaction.")));
    ...
    if (MyDatabaseId != proc->databaseId)
      ereport(ERROR,
              (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
               ...
               errhint(
                   "Connect to the database where the transaction was prepared "
                   "to finish it.")));
...
Datum pg_prepared_xact(PG_FUNCTION_ARGS) {
  ...
  if (SRF_IS_FIRSTCALL()) {
    ...
    TupleDescInitEntry(tupdesc, (AttrNumber)3, "prepared", TIMESTAMPTZOID, -1,
                       0);
...
static GlobalTransaction TwoPhaseGetGXact(TransactionId xid) {
   ...
   */
  if (xid == cached_xid) return cached_gxact;
...
static void save_state_data(const void *data, uint32 len) {
  ...
  if (padlen > records.bytes_free) {
    records.tail->next =
        static_cast<StateFileChunk *>(palloc0(sizeof(StateFileChunk)));
...
void StartPrepare(GlobalTransaction gxact) {
  ...
  hdr.ninvalmsgs =
      xactGetCommittedInvalidationMessages(&invalmsgs, &hdr.initfileinval);
  ...
  if (fd < 0)
    ereport(ERROR,
            (errcode_for_file_access(),
             errmsg("could not create two-phase state file \"%s\": %m", path)));
 ...
 */
void RegisterTwoPhaseRecord(TwoPhaseRmgrId rmid, uint16 info, const void *data,
                            uint32 len) {
  ...
  if (len > 0) save_state_data(data, len);
}
...
static char *ReadTwoPhaseFile(TransactionId xid, bool give_warnings) {
  ...
  if (fd < 0) {
    if (give_warnings)
      ereport(WARNING,
              (errcode_for_file_access(),
               errmsg("could not open two-phase state file \"%s\": %m", path)));
    return NULL;
  }
  ...
  if (fstat(fd, &stat)) {
    ...
    if (give_warnings)
      ereport(WARNING,
              (errcode_for_file_access(),
               errmsg("could not stat two-phase state file \"%s\": %m", path)));
    return NULL;
  }
  ...
  if (read(fd, buf, stat.st_size) != stat.st_size) {
    ...
    if (give_warnings)
      ereport(WARNING,
              (errcode_for_file_access(),
               errmsg("could not read two-phase state file \"%s\": %m", path)));
    pfree(buf);
    return NULL;
  }
...
bool StandbyTransactionIdIsPrepared(TransactionId xid) {
  ...
  if (max_prepared_xacts <= 0) return false; /* nothing to do */
...
void FinishPreparedTransaction(const char *gid, bool isCommit) {
  ...
  if (isCommit)
    RecordTransactionCommitPrepared(
        xid, hdr->nsubxacts, children, hdr->ncommitrels, commitrels,
  ...
  if (hdr->initfileinval) RelationCacheInitFilePostInvalidate();
...
static void ProcessRecords(char *bufptr, TransactionId xid,
  ...
  for (;;) {
    ...
    if (record->rmid == TWOPHASE_RM_END_ID) break;
  ...
  if (unlink(path))
    if (errno != ENOENT || giveWarning)
      ereport(
          WARNING,
          (errcode_for_file_access(),
           errmsg("could not remove two-phase state file \"%s\": %m", path)));
}
  ...
  for (i = 0; i < nxids; i++) {
    ...
    if (fd < 0) {
      if (errno == ENOENT) {
        ...
        if (!TransactionIdIsPrepared(xid)) continue;
...
TransactionId PrescanPreparedTransactions(TransactionId **xids_p,
  ...
  while ((clde = ReadDir(cldir, static_cast<const char *>(TWOPHASE_DIR))) !=
         NULL) {
    if (strlen(clde->d_name) == 8 &&
      ...
      if (TransactionIdFollowsOrEquals(xid, origNextXid)) {
        ereport(WARNING, (errmsg("removing future two-phase state file \"%s\"",
                                 clde->d_name)));
        RemoveTwoPhaseFile(xid, true);
        continue;
      }
      ...
      buf = ReadTwoPhaseFile(xid, true);
      if (buf == NULL) {
        ereport(WARNING, (errmsg("removing corrupt two-phase state file \"%s\"",
                                 clde->d_name)));
        RemoveTwoPhaseFile(xid, true);
        continue;
      }
       ...
       */
      if (TransactionIdPrecedes(xid, result)) result = xid;
      ...
      if (xids_p) {
        if (nxids == allocsize) {
          if (nxids == 0) {
            allocsize = 10;
            xids = static_cast<TransactionId *>(
                palloc(allocsize * sizeof(TransactionId)));
          } else {
            allocsize = allocsize * 2;
            xids = static_cast<TransactionId *>(
                repalloc(xids, allocsize * sizeof(TransactionId)));
          }
        }
        xids[nxids++] = xid;
      }
  ...
  while ((clde = ReadDir(cldir, static_cast<const char *>(TWOPHASE_DIR))) !=
    ...
    if (strlen(clde->d_name) == 8 &&
      ...
      if (TransactionIdDidCommit(xid) || TransactionIdDidAbort(xid)) {
        ereport(WARNING, (errmsg("removing stale two-phase state file \"%s\"",
                                 clde->d_name)));
        RemoveTwoPhaseFile(xid, true);
        continue;
      }
...
void RecoverPreparedTransactions(void) {
  ...
  while ((clde = ReadDir(cldir, static_cast<const char *>(dir))) != NULL) {
    if (strlen(clde->d_name) == 8 &&
       ...
       * been fsynced).
       */
      gxact = MarkAsPreparing(xid, hdr->gid, hdr->prepared_at, hdr->owner,
                              hdr->database);
       ...
       */
      if (InHotStandby) StandbyReleaseLockTree(xid, hdr->nsubxacts, subxids);
...
static void RecordTransactionCommitPrepared(
  ...
  recptr = XactLogCommitRecord(GetCurrentTimestamp(), nchildren, children,
                               nrels, rels, ninvalmsgs, invalmsgs,
                               initfileinval, false, xid);
...
static void RecordTransactionAbortPrepared(TransactionId xid, int nchildren,
  ...
  recptr = XactLogAbortRecord(GetCurrentTimestamp(), nchildren, children, nrels,
                              rels, xid);
