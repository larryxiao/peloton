 ...
 */
static int32 writeListPage(Relation index, Buffer buffer, IndexTuple *tuples,
                           int32 ntuples, BlockNumber rightlink) {
...
static void makeSublist(Relation index, IndexTuple *tuples, int32 ntuples,
  ...
  for (i = 0; i < ntuples; i++) {
    if (curBuffer == InvalidBuffer) {
      ...
      if (prevBuffer != InvalidBuffer) {
        res->nPendingPages++;
        writeListPage(index, prevBuffer, tuples + startTuple, i - startTuple,
  ...
  res->tailFreeSize = writeListPage(index, curBuffer, tuples + startTuple,
                                    ntuples - startTuple, InvalidBlockNumber);
  res->nPendingPages++;
...
void ginHeapTupleFastInsert(GinState *ginstate, GinTupleCollector *collector) {
  ...
  if (collector->sumsize + collector->ntuples * sizeof(ItemIdData) >
    ...
    if (metadata->head == InvalidBlockNumber ||
        collector->sumsize + collector->ntuples * sizeof(ItemIdData) >
  ...
  if (separateList) {
    ...
    if (metadata->head == InvalidBlockNumber) {
      ...
      if (needWal) XLogRegisterBuffer(1, buffer, REGBUF_STANDARD);
    }
  } else {
    /*
    ...
    for (i = 0; i < collector->ntuples; i++) {
      ...
      l = PageAddItem(page, (Item)collector->tuples[i], tupsize, off, false,
                      false);
  ...
  if (needCleanup) ginInsertCleanup(ginstate, false, NULL);
}
 ...
 */
void ginHeapTupleFastCollect(GinState *ginstate, GinTupleCollector *collector,
   ...
   */
  entries = ginExtractEntries(ginstate, attnum, value, isNull, &nentries,
                              &categories);
  ...
  if (collector->tuples == NULL) {
    ...
    collector->tuples =
        (IndexTuple *)palloc(sizeof(IndexTuple) * collector->lentuples);
  }

  while (collector->ntuples + nentries > collector->lentuples) {
    collector->lentuples *= 2;
    collector->tuples = (IndexTuple *)repalloc(
        collector->tuples, sizeof(IndexTuple) * collector->lentuples);
  }
  ...
  for (i = 0; i < nentries; i++) {
    ...
    itup = GinFormTuple(ginstate, attnum, entries[i], categories[i], NULL, 0, 0,
                        true);
    itup->t_tid = *ht_ctid;
 ...
 */
static bool shiftList(Relation index, Buffer metabuffer, BlockNumber newHead,
  ...
  do {
    ...
    while (data.ndeleted < GIN_NDELETE_AT_ONCE && blknoToDelete != newHead) {
      ...
      if (GinPageIsDeleted(page)) {
        ...
        for (i = 0; i < data.ndeleted; i++) UnlockReleaseBuffer(buffers[i]);
    ...
    if (stats) stats->pages_deleted += data.ndeleted;
     ...
     * critical section.
     */
    if (RelationNeedsWAL(index)) XLogEnsureRecordSpace(data.ndeleted, 0);
...
static void initKeyArray(KeyArray *keys, int32 maxvalues) {
  ...
  keys->categories =
      (GinNullCategory *)palloc(sizeof(GinNullCategory) * maxvalues);
  keys->nvalues = 0;
...
static void addDatum(KeyArray *keys, Datum datum, GinNullCategory category) {
  if (keys->nvalues >= keys->maxvalues) {
    ...
    keys->categories = (GinNullCategory *)repalloc(
        keys->categories, sizeof(GinNullCategory) * keys->maxvalues);
  }
 ...
 * calls.
 */
static void processPendingPage(BuildAccumulator *accum, KeyArray *ka, Page page,
  ...
  for (i = startoff; i <= maxoff; i = OffsetNumberNext(i)) {
    ...
    if (!ItemPointerIsValid(&heapptr)) {
       ...
       * and reset ka.
       */
      ginInsertBAEntries(accum, &heapptr, attrnum, ka->keys, ka->categories,
                         ka->nvalues);
      ka->nvalues = 0;
      heapptr = itup->t_tid;
      attrnum = curattnum;
 ...
 */
void ginInsertCleanup(GinState *ginstate, bool vac_delay,
                      IndexBulkDeleteResult *stats) {
  Relation index = ginstate->index;
  ...
  opCtx = AllocSetContextCreate(
      CurrentMemoryContext, "GIN insert cleanup temporary context",
  ...
  for (;;) {
    ...
    if (GinPageGetOpaque(page)->rightlink == InvalidBlockNumber ||
      ...
      if (PageGetMaxOffsetNumber(page) != maxoff) {
        ...
        ginBeginBAScan(&accum);
        while ((list = ginGetBAEntry(&accum, &attnum, &key, &category,
                                     &nlist)) != NULL)
          ginEntryInsert(ginstate, attnum, key, category, list, nlist, NULL);
      }
