...
GinBtreeStack *ginFindLeafPage(GinBtree btree, bool searchMode) {
  ...
  for (;;) {
    ...
    while (btree->fullScan == FALSE && stack->blkno != btree->rootBlkno &&
      ...
      if (rightlink == InvalidBlockNumber) /* rightmost page */
        break;
...
void freeGinBtreeStack(GinBtreeStack *stack) {
  while (stack) {
    ...
    if (stack->buffer != InvalidBuffer) ReleaseBuffer(stack->buffer);
...
static void ginFindParents(GinBtree btree, GinBtreeStack *stack) {
  ...
  for (;;) {
    LockBuffer(buffer, GIN_EXCLUSIVE);
    page = BufferGetPage(buffer);
    if (GinPageIsLeaf(page)) elog(ERROR, "Lost path");
...
static bool ginPlaceToPage(GinBtree btree, GinBtreeStack *stack,
  ...
  if (GinPageIsData(page)) xlflags |= GIN_INSERT_ISDATA;
  if (GinPageIsLeaf(page)) {
  ...
  if (RelationNeedsWAL(btree->index)) XLogBeginInsert();

  rc = btree->placeToPage(btree, stack->buffer, stack, insertdata, updateblkno,
                          &newlpage, &newrpage);
  if (rc == UNMODIFIED) {
    XLogResetInsertion();
    return true;
    ...
    if (RelationNeedsWAL(btree->index)) {
      ...
      if (childbuf != InvalidBuffer) PageSetLSN(childpage, recptr);
    }
    ...
    if (stack->parent == NULL) {
      ...
      GinInitPage(newrootpg, GinPageGetOpaque(newlpage)->flags &
                                 ~(GIN_LEAF | GIN_COMPRESSED),
                  BLCKSZ);

      btree->fillRoot(btree, newrootpg, BufferGetBlockNumber(lbuffer), newlpage,
    ...
    MarkBufferDirty(stack->buffer);
    if (BufferIsValid(childbuf)) MarkBufferDirty(childbuf);
    ...
    if (RelationNeedsWAL(btree->index)) {
      ...
      if (stack->parent == NULL) {
        ...
        XLogRegisterBuffer(2, stack->buffer,
      ...
      if (BufferIsValid(childbuf)) XLogRegisterBuffer(3, childbuf, 0);
      ...
      if (stack->parent == NULL) PageSetLSN(BufferGetPage(lbuffer), recptr);
      if (BufferIsValid(childbuf)) PageSetLSN(childpage, recptr);
    }
    END_CRIT_SECTION();
    ...
    if (stack->parent == NULL) UnlockReleaseBuffer(lbuffer);

    pfree(newlpage);
    pfree(newrpage);
    if (newrootpg) pfree(newrootpg);
...
static void ginFinishSplit(GinBtree btree, GinBtreeStack *stack, bool freestack,
  ...
  do {
    ...
    while ((parent->off =
      ...
      parent->buffer =
          ginStepRight(parent->buffer, btree->index, GIN_EXCLUSIVE);
    ...
    done = ginPlaceToPage(btree, parent, insertdata, updateblkno, stack->buffer,
                          buildStats);
    pfree(insertdata);
     ...
     */
    if (!first || freestack) LockBuffer(stack->buffer, GIN_UNLOCK);
    if (freestack) {
  ...
  if (freestack) freeGinBtreeStack(stack);
}
