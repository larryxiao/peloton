...
Datum ginbeginscan(PG_FUNCTION_ARGS) {
  ...
  so->nkeys = 0;
  so->tempCtx =
      AllocSetContextCreate(CurrentMemoryContext, "Gin scan temporary context",
                            ALLOCSET_DEFAULT_MINSIZE, ALLOCSET_DEFAULT_INITSIZE,
                            ALLOCSET_DEFAULT_MAXSIZE);
  so->keyCtx = AllocSetContextCreate(
      CurrentMemoryContext, "Gin scan key context", ALLOCSET_DEFAULT_MINSIZE,
...
static GinScanEntry ginFillScanEntry(GinScanOpaque so, OffsetNumber attnum,
  ...
  if (extra_data == NULL) {
    for (i = 0; i < so->totalentries; i++) {
      ...
      if (prevEntry->extra_data == NULL &&
          ...
          prevEntry->searchMode == searchMode && prevEntry->attnum == attnum &&
          ginCompareEntries(ginstate, attnum, prevEntry->queryKey,
                            prevEntry->queryCategory, queryKey,
                            queryCategory) == 0) {
        /* Successful match */
        return prevEntry;
      }
  ...
  if (so->totalentries >= so->allocentries) {
    so->allocentries *= 2;
    so->entries = (GinScanEntry *)repalloc(
        so->entries, so->allocentries * sizeof(GinScanEntry));
  }
...
static void ginFillScanKey(GinScanOpaque so, OffsetNumber attnum,
                           StrategyNumber strategy, int32 searchMode,
                           Datum query, uint32 nQueryValues, Datum *queryValues,
  ...
  if (searchMode != GIN_SEARCH_MODE_DEFAULT) nQueryValues++;
  ...
  for (i = 0; i < nQueryValues; i++) {
    ...
    key->scanEntry[i] =
        ginFillScanEntry(so, attnum, strategy, searchMode, queryKey,
                         queryCategory, isPartialMatch, this_extra);
  }
}
...
void ginNewScanKey(IndexScanDesc scan) {
  ...
  so->keys =
      (GinScanKey)palloc(Max(scan->numberOfKeys, 1) * sizeof(GinScanKeyData));
  so->nkeys = 0;
  ...
  so->allocentries = 32;
  so->entries =
      (GinScanEntry *)palloc0(so->allocentries * sizeof(GinScanEntry));
  ...
  for (i = 0; i < scan->numberOfKeys; i++) {
    ...
    queryValues = (Datum *)DatumGetPointer(FunctionCall7Coll(
        &so->ginstate.extractQueryFn[skey->sk_attno - 1],
    ...
    if (searchMode != GIN_SEARCH_MODE_DEFAULT) hasNullQuery = true;
    ...
    ginFillScanKey(so, skey->sk_attno, skey->sk_strategy, searchMode,
                   skey->sk_argument, nQueryValues, queryValues,
  ...
  if (so->nkeys == 0 && !so->isVoidRes) {
    ...
    ginFillScanKey(so, FirstOffsetNumber, InvalidStrategy,
                   GIN_SEARCH_MODE_EVERYTHING, (Datum)0, 0, NULL, NULL, NULL,
                   NULL);
  }
  ...
  if (hasNullQuery && !so->isVoidRes) {
    ...
    if (ginStats.ginVersion < 1)
      ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                      errmsg(
                          "old GIN indexes do not support whole-index scans "
                          "nor searches for nulls"),
