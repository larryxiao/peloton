  ...
  for (i = 0; i < nitem; i++) {
    if (gvs->callback(items + i, gvs->callback_state)) {
      ...
      if (!tmpitems) {
         ...
         */
        tmpitems =
            static_cast<ItemPointer>(palloc(sizeof(ItemPointerData) * nitem));
        memcpy(tmpitems, items, sizeof(ItemPointerData) * i);
      }
    } else {
      gvs->result->num_index_tuples += 1;
      if (tmpitems) tmpitems[remaining] = items[i];
      remaining++;
    }
  }
...
static bool ginVacuumPostingTreeLeaves(GinVacuumState *gvs, BlockNumber blkno,
                                       bool isRoot, Buffer *rootBuffer) {
  Buffer buffer;
  Page page;
  bool hasVoidPage = FALSE;
  MemoryContext oldCxt;

  buffer = ReadBufferExtended(gvs->index, MAIN_FORKNUM, blkno, RBM_NORMAL,
                              gvs->strategy);
  ...
  if (GinPageIsLeaf(page)) {
    ...
    for (i = FirstOffsetNumber; i <= GinPageGetOpaque(page)->maxoff; i++) {
      ...
      if (ginVacuumPostingTreeLeaves(gvs, PostingItemGetBlockNumber(pitem),
                                     FALSE, NULL))
        isChildHasVoid = TRUE;
    }

    if (isChildHasVoid) hasVoidPage = TRUE;
  }
 ...
 */
static void ginDeletePage(GinVacuumState *gvs, BlockNumber deleteBlkno,
                          BlockNumber leftBlkno, BlockNumber parentBlkno,
   ...
   */
  lBuffer = ReadBufferExtended(gvs->index, MAIN_FORKNUM, leftBlkno, RBM_NORMAL,
                               gvs->strategy);
  ...
  if (!isParentRoot) LockBuffer(pBuffer, GIN_UNLOCK);
 ...
 */
static bool ginScanToDelete(GinVacuumState *gvs, BlockNumber blkno, bool isRoot,
  ...
  if (!meDelete) me->leftBlkno = blkno;
 ...
 */
static Page ginVacuumEntryPage(GinVacuumState *gvs, Buffer buffer,
                               BlockNumber *roots, uint32 *nroot) {
  ...
  *nroot = 0;
  ...
  for (i = FirstOffsetNumber; i <= maxoff; i++) {
    ...
    if (GinIsPostingTree(itup)) {
      ...
      if (free_items_orig) pfree(items_orig);
...
Datum ginbulkdelete(PG_FUNCTION_ARGS) {
  ...
  Buffer buffer;
  BlockNumber
      rootOfPostingTree[BLCKSZ / (sizeof(IndexTupleData) + sizeof(ItemId))];
  uint32 nRoot;

  gvs.tmpCxt = AllocSetContextCreate(
      CurrentMemoryContext, "Gin vacuum temporary context",
...
Datum ginvacuumcleanup(PG_FUNCTION_ARGS) {
  ...
  for (blkno = GIN_ROOT_BLKNO; blkno < npages; blkno++) {
    ...
    if (GinPageIsDeleted(page)) {
      ...
      if (GinPageIsLeaf(page)) idxStat.nEntries += PageGetMaxOffsetNumber(page);
    }
