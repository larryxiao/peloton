...
static GinTernaryValue trueTriConsistentFn(GinScanKey key) { return GIN_TRUE; }
...
static bool directBoolConsistentFn(GinScanKey key) {
  ...
  return DatumGetBool(FunctionCall8Coll(
      key->consistentFmgrInfo, key->collation, PointerGetDatum(key->entryRes),
      UInt16GetDatum(key->strategy), key->query,
      UInt32GetDatum(key->nuserentries), PointerGetDatum(key->extra_data),
      PointerGetDatum(&key->recheckCurItem), PointerGetDatum(key->queryValues),
...
static GinTernaryValue directTriConsistentFn(GinScanKey key) {
  return DatumGetGinTernaryValue(FunctionCall7Coll(
      key->triConsistentFmgrInfo, key->collation,
      PointerGetDatum(key->entryRes), UInt16GetDatum(key->strategy), key->query,
      UInt32GetDatum(key->nuserentries), PointerGetDatum(key->extra_data),
...
static GinTernaryValue shimTriConsistentFn(GinScanKey key) {
  ...
  for (i = 0; i < key->nentries; i++) {
    if (key->entryRes[i] == GIN_MAYBE) {
      if (nmaybe >= MAX_MAYBE_ENTRIES) return GIN_MAYBE;
   ...
   * function as is.
   */
  if (nmaybe == 0) return directBoolConsistentFn(key);

  /* First call consistent function with all the maybe-inputs set FALSE */
  for (i = 0; i < nmaybe; i++) key->entryRes[maybeEntries[i]] = GIN_FALSE;
  ...
  for (;;) {
    ...
    if (curResult != boolResult) return GIN_MAYBE;
  }

  /* TRUE with recheck is taken to mean MAYBE */
  if (curResult == GIN_TRUE && recheck) curResult = GIN_MAYBE;
