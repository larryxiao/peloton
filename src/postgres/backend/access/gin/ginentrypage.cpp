...
static void entrySplitPage(GinBtree btree, Buffer origbuf, GinBtreeStack *stack,
...
IndexTuple GinFormTuple(GinState *ginstate, OffsetNumber attnum, Datum key,
  ...
  if (newsize != IndexTupleSize(itup)) {
     ...
     */
    memset((char *)itup + IndexTupleSize(itup), 0,
           newsize - IndexTupleSize(itup));
 ...
 * in *nitems.
 */
ItemPointer ginReadTuple(GinState *ginstate, OffsetNumber attnum,
  ...
  if (GinItupIsCompressed(itup)) {
    if (nipd > 0) {
      ...
      if (nipd != ndecoded)
        elog(ERROR,
             "number of items mismatch in GIN entry tuple, %d in tuple header, "
             "%d decoded",
             nipd, ndecoded);
    } else {
 ...
 */
static IndexTuple GinFormInteriorTuple(IndexTuple itup, Page page,
...
static bool entryIsMoveRight(GinBtree btree, Page page) {
  ...
  if (ginCompareAttEntries(btree->ginstate, btree->entryAttnum, btree->entryKey,
                           btree->entryCategory, attnum, key, category) > 0)
    return TRUE;
...
static BlockNumber entryLocateEntry(GinBtree btree, GinBtreeStack *stack) {
  ...
  while (high > low) {
    ...
    if (mid == maxoff && GinPageRightMost(page)) {
      ...
      attnum = gintuple_get_attrnum(btree->ginstate, itup);
      key = gintuple_get_key(btree->ginstate, itup, &category);
      result = ginCompareAttEntries(btree->ginstate, btree->entryAttnum,
                                    btree->entryKey, btree->entryCategory,
                                    attnum, key, category);
    }
...
static bool entryLocateLeafEntry(GinBtree btree, GinBtreeStack *stack) {
  ...
  while (high > low) {
    ...
    key = gintuple_get_key(btree->ginstate, itup, &category);
    result = ginCompareAttEntries(btree->ginstate, btree->entryAttnum,
                                  btree->entryKey, btree->entryCategory, attnum,
                                  key, category);
    if (result == 0) {
      stack->off = mid;
      return true;
...
static OffsetNumber entryFindChildPtr(GinBtree btree, Page page,
                                      BlockNumber blkno,
  ...
  if (storedOff >= FirstOffsetNumber && storedOff <= maxoff) {
    ...
    if (GinGetDownlink(itup) == blkno) return storedOff;
    ...
    for (i = storedOff + 1; i <= maxoff; i++) {
      ...
      if (GinGetDownlink(itup) == blkno) return i;
    }
    maxoff = storedOff - 1;
  }
...
static bool entryIsEnoughSpace(GinBtree btree, Buffer buf, OffsetNumber off,
  ...
  if (PageGetFreeSpace(page) + releasedsz >= addedsz) return true;
 ...
 */
static void entryPreparePage(GinBtree btree, Page page, OffsetNumber off,
                             GinBtreeEntryInsertData *insertData,
 ...
 */
static GinPlaceToPageRC entryPlaceToPage(GinBtree btree, Buffer buf,
                                         GinBtreeStack *stack,
                                         void *insertPayload,
                                         BlockNumber updateblkno,
                                         Page *newlpage, Page *newrpage) {
  GinBtreeEntryInsertData *insertData =
      static_cast<GinBtreeEntryInsertData *>(insertPayload);
  Page page = BufferGetPage(buf);
  ...
  if (!entryIsEnoughSpace(btree, buf, off, insertData)) {
    entrySplitPage(btree, buf, stack, insertPayload, updateblkno, newlpage,
                   newrpage);
    return SPLIT;
  }
  ...
  placed = PageAddItem(page, (Item)insertData->entry,
                       IndexTupleSize(insertData->entry), off, false, false);
  if (placed != off)
...
static void entrySplitPage(GinBtree btree, Buffer origbuf, GinBtreeStack *stack,
  ...
  for (i = FirstOffsetNumber; i <= maxoff; i++) {
    ...
    if (lsize > totalsize / 2) {
      if (separator == InvalidOffsetNumber) separator = i - 1;
      page = rpage;
    } else {
...
static void *entryPrepareDownlink(GinBtree btree, Buffer lbuf) {
  ...
  insertData = static_cast<GinBtreeEntryInsertData *>(
      palloc(sizeof(GinBtreeEntryInsertData)));
 ...
 */
void ginEntryFillRoot(GinBtree btree, Page root, BlockNumber lblkno, Page lpage,
                      BlockNumber rblkno, Page rpage) {
  IndexTuple itup;

  itup = GinFormInteriorTuple(getRightMostTuple(lpage), lpage, lblkno);
  if (PageAddItem(root, (Item)itup, IndexTupleSize(itup), InvalidOffsetNumber,
                  false, false) == InvalidOffsetNumber)
 ...
 */
void ginPrepareEntryScan(GinBtree btree, OffsetNumber attnum, Datum key,
                         GinNullCategory category, GinState *ginstate) {
