...
static void dataSplitPageInternal(GinBtree btree, Buffer origbuf,
                                  GinBtreeStack *stack, void *insertdata,
                                  BlockNumber updateblkno, Page *newlpage,
                                  Page *newrpage);
...
static void dataPlaceToPageLeafSplit(Buffer buf, disassembledLeaf *leaf,
                                     ItemPointerData lbound,
                                     ItemPointerData rbound, Page lpage,
                                     Page rpage);
 ...
 */
ItemPointer GinDataLeafPageGetItems(Page page, int *nitems,
  ...
  if (GinPageIsCompressed(page)) {
    ...
    result =
        static_cast<ItemPointer>(palloc((*nitems) * sizeof(ItemPointerData)));
...
int GinDataLeafPageGetItemsToTbm(Page page, TIDBitmap *tbm) {
  ...
  if (GinPageIsCompressed(page)) {
    ...
    if (nitems > 0) tbm_add_tuples(tbm, uncompressed, nitems, false);
  }
...
static bool dataIsMoveRight(GinBtree btree, Page page) {
  ...
  if (GinPageRightMost(page)) return FALSE;
 ...
 */
static OffsetNumber dataFindChildPtr(GinBtree btree, Page page,
                                     BlockNumber blkno,
  ...
  if (storedOff >= FirstOffsetNumber && storedOff <= maxoff) {
    ...
    if (PostingItemGetBlockNumber(pitem) == blkno) return storedOff;
    ...
    for (i = storedOff + 1; i <= maxoff; i++) {
      ...
      if (PostingItemGetBlockNumber(pitem) == blkno) return i;
    }
 ...
 */
void GinDataPageAddPostingItem(Page page, PostingItem *data,
                               OffsetNumber offset) {
 ...
 */
static GinPlaceToPageRC dataPlaceToPageLeaf(GinBtree btree, Buffer buf,
                                            GinBtreeStack *stack,
                                            void *insertdata, Page *newlpage,
                                            Page *newrpage) {
  GinBtreeDataLeafInsertData *items =
      static_cast<GinBtreeDataLeafInsertData *>(insertdata);
   ...
   */
  tmpCxt =
      AllocSetContextCreate(CurrentMemoryContext, "Gin split temporary context",
  ...
  if (!dlist_is_empty(&leaf->segments)) {
    ...
    if (!lastleftinfo->items)
      lastleftinfo->items =
          ginPostingListDecode(lastleftinfo->seg, &lastleftinfo->nitems);
  ...
  if (ItemPointerIsValid(&remaining)) {
    ...
    for (i = 0; i < maxitems; i++) {
      if (ginCompareItemPointers(&newItems[i], &remaining) >= 0) break;
    }
    if (i == 0) elog(ERROR, "could not split GIN page; no new___ items fit");
    maxitems = i;
  }
  ...
  if (!needsplit) {
    ...
    if (!btree->isBuild) {
      while (dlist_has_prev(&leaf->segments, leaf->lastleft)) {
        ...
        if (lastleftinfo->action != GIN_SEGMENT_DELETE) {
           ...
           * more than 75% full.
           */
          if ((leaf->lsize - segsize) - (leaf->rsize + segsize) < 0) break;
          if (append) {
            if ((leaf->lsize - segsize) < (BLCKSZ * 3) / 4) break;
          }
    ...
    *newrpage = static_cast<Page>(MemoryContextAlloc(oldCxt, BLCKSZ));
    ...
    dataPlaceToPageLeafSplit(buf, leaf, lbound, rbound, *newlpage, *newrpage);
    ...
    if (append)
      elog(DEBUG2, "appended %d items to block %u; split %d/%d (%d to go)",
           maxitems, BufferGetBlockNumber(buf), (int)leaf->lsize,
           (int)leaf->rsize, items->nitem - items->curitem - maxitems);
    else
 ...
 */
void ginVacuumPostingTreeLeaf(Relation indexrel, Buffer buffer,
                              GinVacuumState *gvs) {
  ...
  dlist_foreach(iter, &leaf->segments) {
    ...
    cleaned =
        ginVacuumItemPointers(gvs, seginfo->items, seginfo->nitems, &ncleaned);
    pfree(seginfo->items);
    seginfo->items = NULL;
    seginfo->nitems = 0;
    if (cleaned) {
  ...
  if (removedsomething) {
    ...
    dlist_foreach(iter, &leaf->segments) {
      leafSegmentInfo *seginfo =
          dlist_container(leafSegmentInfo, node, iter.cur);

      if (seginfo->action != GIN_SEGMENT_UNMODIFIED) modified = true;
...
static void registerLeafRecompressWALData(Buffer buf, disassembledLeaf *leaf) {
  ...
  dlist_foreach(iter, &leaf->segments) {
    ...
    if (seginfo->action != GIN_SEGMENT_UNMODIFIED) nmodified++;
  }

  walbufbegin = static_cast<char *>(
  ...
  dlist_foreach(iter, &leaf->segments) {
    ...
    *(walbufend++) = action;
    ...
    if (action != GIN_SEGMENT_INSERT) segno++;
  }
...
static void dataPlaceToPageLeafRecompress(Buffer buf, disassembledLeaf *leaf) {
  ...
  dlist_foreach(iter, &leaf->segments) {
    ...
    if (seginfo->action != GIN_SEGMENT_DELETE) {
      ...
      if (modified) memcpy(ptr, seginfo->seg, segsize);
...
static void dataPlaceToPageLeafSplit(Buffer buf, disassembledLeaf *leaf,
  ...
  for (node = dlist_head_node(&leaf->segments); node != firstright;
  ...
  *GinDataPageGetRightBound(lpage) = lbound;
  ...
  for (node = firstright;; node = dlist_next_node(&leaf->segments, node)) {
    ...
    if (!dlist_has_next(&leaf->segments, node)) break;
  }
 ...
 */
static GinPlaceToPageRC dataPlaceToPageInternal(
    GinBtree btree, Buffer buf, GinBtreeStack *stack, void *insertdata,
  ...
  if (GinNonLeafDataPageGetFreeSpace(page) < sizeof(PostingItem)) {
    dataSplitPageInternal(btree, buf, stack, insertdata, updateblkno, newlpage,
                          newrpage);
    return SPLIT;
  }
 ...
 */
static GinPlaceToPageRC dataPlaceToPage(GinBtree btree, Buffer buf,
                                        GinBtreeStack *stack, void *insertdata,
                                        BlockNumber updateblkno, Page *newlpage,
                                        Page *newrpage) {
  Page page = BufferGetPage(buf);
  ...
  if (GinPageIsLeaf(page))
    return dataPlaceToPageLeaf(btree, buf, stack, insertdata, newlpage,
                               newrpage);
  else
    return dataPlaceToPageInternal(btree, buf, stack, insertdata, updateblkno,
                                   newlpage, newrpage);
}
...
static void dataSplitPageInternal(GinBtree btree, Buffer origbuf,
  ...
  memcpy(GinDataPageGetPostingItem(lpage, FirstOffsetNumber), allitems,
...
static disassembledLeaf *disassembleLeaf(Page page) {
  ...
  if (GinPageIsCompressed(page)) {
    ...
    while ((Pointer)seg < segend) {
      leafSegmentInfo *seginfo =
          static_cast<leafSegmentInfo *>(palloc(sizeof(leafSegmentInfo)));
    ...
    seginfo->action = GIN_SEGMENT_REPLACE;
    seginfo->seg = NULL;
    seginfo->items = static_cast<ItemPointer>(
        palloc(nuncompressed * sizeof(ItemPointerData)));
    memcpy(seginfo->items, uncompressed,
           nuncompressed * sizeof(ItemPointerData));
...
static bool addItemsToLeaf(disassembledLeaf *leaf, ItemPointer newItems,
  ...
  dlist_foreach(iter, &leaf->segments) {
    leafSegmentInfo *cur =
        (leafSegmentInfo *)dlist_container(leafSegmentInfo, node, iter.cur);
    int nthis;
    ...
    if (!dlist_has_next(&leaf->segments, iter.cur))
      ...
      next = (leafSegmentInfo *)dlist_container(
          leafSegmentInfo, node, dlist_next_node(&leaf->segments, iter.cur));
      if (next->items)
      ...
      nthis = 0;
      while (nthis < newleft &&
             ginCompareItemPointers(&nextnew[nthis], &next_first) < 0)
        nthis++;
    }
    if (nthis == 0) continue;

    /* Merge the new___ items with the existing items. */
    if (!cur->items) cur->items = ginPostingListDecode(cur->seg, &cur->nitems);
    ...
    tmpitems = ginMergeItemPointers(cur->items, cur->nitems, nextnew, nthis,
                                    &ntmpitems);
...
static bool leafRepackItems(disassembledLeaf *leaf, ItemPointer remaining) {
   ...
   * while iterating.
   */
  for (cur_node = dlist_head_node(&leaf->segments); cur_node != NULL;
       cur_node = next_node) {
    ...
    if (seginfo->action != GIN_SEGMENT_DELETE) {
      if (seginfo->seg == NULL) {
        if (seginfo->nitems > GinPostingListSegmentMaxSize)
          ...
          seginfo->seg =
              ginCompressPostingList(seginfo->items, seginfo->nitems,
    ...
    if (seginfo->action == GIN_SEGMENT_DELETE) continue;
 ...
 */
BlockNumber createPostingTree(Relation index, ItemPointerData *items,
  ...
  while (nrootitems < nitems) {
    ...
    segment = ginCompressPostingList(&items[nrootitems], nitems - nrootitems,
                                     GinPostingListSegmentMaxSize, &npacked);
    segsize = SizeOfGinPostingList(segment);
    if (rootsize + segsize > GinDataPageMaxDataSize) break;
  ...
  if (buildStats) buildStats->nDataPages++;
  ...
  if (nitems > nrootitems) {
    ginInsertItemPointers(index, blkno, items + nrootitems, nitems - nrootitems,
                          buildStats);
  }
 ...
 */
GinBtreeStack *ginScanBeginPostingTree(GinBtree btree, Relation index,
