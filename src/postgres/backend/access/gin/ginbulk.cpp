...
static void ginCombineData(RBNode *existing, const RBNode *newdata, void *arg) {
  ...
  if (eo->count >= eo->maxcount) {
    ...
    eo->maxcount *= 2;
    eo->list = (ItemPointerData *)repalloc(
        eo->list, sizeof(ItemPointerData) * eo->maxcount);
    accum->allocatedMemory += GetMemoryChunkSpace(eo->list);
  }
...
static int cmpEntryAccumulator(const RBNode *a, const RBNode *b, void *arg) {
  ...
  return ginCompareAttEntries(accum->ginstate, ea->attnum, ea->key,
                              ea->category, eb->attnum, eb->key, eb->category);
}
...
static RBNode *ginAllocEntryAccumulator(void *arg) {
  ...
  if (accum->entryallocator == NULL || accum->eas_used >= DEF_NENTRY) {
    accum->entryallocator = static_cast<GinEntryAccumulator *>(
        palloc(sizeof(GinEntryAccumulator) * DEF_NENTRY));
...
void ginInitBA(BuildAccumulator *accum) {
  ...
  accum->eas_used = 0;
  accum->tree = rb_create(sizeof(GinEntryAccumulator), cmpEntryAccumulator,
                          ginCombineData, ginAllocEntryAccumulator,
 ...
 */
static Datum getDatumCopy(BuildAccumulator *accum, OffsetNumber attnum,
                          Datum value) {
 ...
 */
void ginInsertBAEntries(BuildAccumulator *accum, ItemPointer heapptr,
                        OffsetNumber attnum, Datum *entries,
 ...
 */
ItemPointerData *ginGetBAEntry(BuildAccumulator *accum, OffsetNumber *attnum,
  ...
  if (entry == NULL) return NULL; /* no more entries */
