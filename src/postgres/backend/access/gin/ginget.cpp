...
static bool moveRightIfItNeeded(GinBtreeData *btree, GinBtreeStack *stack) {
  ...
  if (stack->off > PageGetMaxOffsetNumber(page)) {
     ...
     */
    if (GinPageRightMost(page)) return false; /* no more pages */
 ...
 */
static void scanPostingTree(Relation index, GinScanEntry scanEntry,
  ...
  for (;;) {
    ...
    if (GinPageRightMost(page)) break; /* no more pages */
...
static bool collectMatchBitmap(GinBtreeData *btree, GinBtreeStack *stack,
  ...
  for (;;) {
     ...
     */
    if (moveRightIfItNeeded(btree, stack) == false) return true;
     ...
     */
    if (gintuple_get_attrnum(btree->ginstate, itup) != attnum) return true;
    ...
    if (scanEntry->isPartialMatch) {
       ...
       */
      if (icategory != GIN_CAT_NORM_KEY) return true;
       ...
       */
      if (icategory == GIN_CAT_NULL_ITEM) return true;
    }
    ...
    if (GinIsPostingTree(itup)) {
      ...
      for (;;) {
        ...
        if (ginCompareEntries(btree->ginstate, attnum, newDatum, newCategory,
                              idatum, icategory) == 0)
          break; /* Found! */
...
restartScanEntry:
   ...
   */
  ginPrepareEntryScan(&btreeEntry, entry->attnum, entry->queryKey,
                      entry->queryCategory, ginstate);
  ...
  if (entry->isPartialMatch || entry->queryCategory == GIN_CAT_EMPTY_QUERY) {
    ...
    if (collectMatchBitmap(&btreeEntry, stackEntry, entry) == false) {
      ...
      if (entry->matchBitmap) {
        if (entry->matchIterator) tbm_end_iterate(entry->matchIterator);
  ...
  if (needUnlock) LockBuffer(stackEntry->buffer, GIN_UNLOCK);
  freeGinBtreeStack(stackEntry);
}
...
static void startScanKey(GinState *ginstate, GinScanOpaque so, GinScanKey key) {
  ...
  if (key->nentries > 1) {
    ...
    for (i = 0; i < key->nentries; i++) entryIndexes[i] = i;
    qsort_arg(entryIndexes, key->nentries, sizeof(int),
              entryIndexByFrequencyCmp, key);

    for (i = 0; i < key->nentries - 1; i++) {
      /* Pass all entries <= i as FALSE, and the rest as MAYBE */
      for (j = 0; j <= i; j++) key->entryRes[entryIndexes[j]] = GIN_FALSE;
      for (j = i + 1; j < key->nentries; j++)
        key->entryRes[entryIndexes[j]] = GIN_MAYBE;

      if (key->triConsistentFn(key) == GIN_FALSE) break;
    }
    ...
    key->requiredEntries = static_cast<GinScanEntryData **>(
        palloc(key->nrequired * sizeof(GinScanEntry)));
    key->additionalEntries = static_cast<GinScanEntryData **>(
        palloc(key->nadditional * sizeof(GinScanEntry)));
    ...
    key->nadditional = 0;
    key->requiredEntries =
        static_cast<GinScanEntryData **>(palloc(1 * sizeof(GinScanEntry)));
...
static void startScan(IndexScanDesc scan) {
  ...
  if (GinFuzzySearchLimit > 0) {
    ...
    for (i = 0; i < so->totalentries; i++) {
      if (so->entries[i]->predictNumberResult <=
          so->totalentries * GinFuzzySearchLimit) {
        reduce = false;
        break;
      }
    }
   ...
   */
  for (i = 0; i < so->nkeys; i++) startScanKey(ginstate, so, so->keys + i);
}
...
static void entryLoadMoreItems(GinState *ginstate, GinScanEntry entry,
  ...
  for (;;) {
     ...
     */
    if (!GinPageRightMost(page) &&
        ginCompareItemPointers(&advancePast, GinDataPageGetRightBound(page)) >=
            0) {
      /*
...
#define dropItem(e) \
  (gin_rand() >     \
   ((double)GinFuzzySearchLimit) / ((double)((e)->predictNumberResult)))
  ...
  if (entry->matchBitmap) {
    ...
    do {
      ...
      if (entry->matchResult->blockno == advancePastBlk) {
         ...
         * offsets.
         */
        if (entry->matchResult->offsets[entry->matchResult->ntuples - 1] <=
            advancePastOff) {
          entry->offset = entry->matchResult->ntuples;
          continue;
        }
      ...
      ItemPointerSet(&entry->curItem, entry->matchResult->blockno,
 ...
 */
static void keyGetItem(GinState *ginstate, MemoryContext tempCtx,
                       GinScanKey key, ItemPointerData advancePast) {
  ItemPointerData minItem;
   ...
   */
  if (ginCompareItemPointers(&key->curItem, &advancePast) > 0) return;
  ...
  for (i = 0; i < key->nentries; i++) {
    entry = key->scanEntry[i];
    if (entry->isFinished) key->entryRes[i] = GIN_FALSE;
#if 0
...
static bool scanGetItem(IndexScanDesc scan, ItemPointerData advancePast,
  ...
  do {
    ...
    for (i = 0; i < so->nkeys && match; i++) {
      ...
      if (key->isFinished) return false;
...
static bool scanGetCandidate(IndexScanDesc scan, pendingPosition *pos) {
  ...
  for (;;) {
    ...
    if (pos->firstOffset > maxoff) {
      ...
      itup =
          (IndexTuple)PageGetItem(page, PageGetItemId(page, pos->firstOffset));
      pos->item = itup->t_tid;
      if (GinPageHasFullRow(page)) {
        /*
         * find itempointer to the next row
         */
        for (pos->lastOffset = pos->firstOffset + 1; pos->lastOffset <= maxoff;
             pos->lastOffset++) {
          itup = (IndexTuple)PageGetItem(page,
                                         PageGetItemId(page, pos->lastOffset));
          if (!ItemPointerEquals(&pos->item, &itup->t_tid)) break;
        }
      } else {
        /*
...
static bool matchPartialInPendingList(GinState *ginstate, Page page,
                                      ...
                                      GinScanEntry entry, Datum *datum,
  ...
  if (entry->queryCategory != GIN_CAT_NORM_KEY) return false;

  while (off < maxoff) {
    itup = (IndexTuple)PageGetItem(page, PageGetItemId(page, off));

    if (gintuple_get_attrnum(ginstate, itup) != entry->attnum) return false;
    ...
    if (category[off - 1] != GIN_CAT_NORM_KEY) return false;
     ...
     */
    cmp = DatumGetInt32(FunctionCall4Coll(
        &ginstate->comparePartialFn[entry->attnum - 1],
...
static bool collectMatchesForHeapRow(IndexScanDesc scan, pendingPosition *pos) {
  ...
  for (;;) {
    ...
    for (i = 0; i < so->nkeys; i++) {
      ...
      for (j = 0; j < key->nentries; j++) {
        ...
        OffsetNumber StopLow = pos->firstOffset, StopHigh = pos->lastOffset,
                     StopMiddle;

        /* If already matched on earlier page, do no extra work */
        if (key->entryRes[j]) continue;
        ...
        while (StopLow < StopHigh) {
          ...
          if (entry->queryCategory == GIN_CAT_EMPTY_QUERY) {
            ...
            res =
                ginCompareEntries(&so->ginstate, entry->attnum, entry->queryKey,
                                  entry->queryCategory, datum[StopMiddle - 1],
                                  category[StopMiddle - 1]);
          }
          ...
          if (res == 0) {
            ...
            if (entry->isPartialMatch)
              key->entryRes[j] = matchPartialInPendingList(
                  &so->ginstate, page, StopMiddle, pos->lastOffset, entry,
        ...
        if (StopLow >= StopHigh && entry->isPartialMatch) {
           ...
           */
          key->entryRes[j] = matchPartialInPendingList(
              &so->ginstate, page, StopHigh, pos->lastOffset, entry, datum,
              category, datumExtracted);
        }
   ...
   */
  for (i = 0; i < so->nkeys; i++) {
    if (pos->hasMatchKey[i] == false) return false;
  }
 ...
 */
static void scanPendingInsert(IndexScanDesc scan, TIDBitmap *tbm,
                              int64 *ntids) {
  GinScanOpaque so = (GinScanOpaque)scan->opaque;
  MemoryContext oldCtx;
  bool recheck, match;
...
Datum gingetbitmap(PG_FUNCTION_ARGS) {
  ...
  if (GinIsVoidRes(scan)) PG_RETURN_INT64(0);
