...
static void ginRedoInsertEntry(Buffer buffer, bool isLeaf,
                               BlockNumber rightblkno, void *rdata) {
  Page page = BufferGetPage(buffer);
  ...
  if (rightblkno != InvalidBlockNumber) {
    ...
    Assert(offset >= FirstOffsetNumber &&
  ...
  if (PageAddItem(page, (Item)itup, IndexTupleSize(itup), offset, false,
    ...
    elog(ERROR, "failed to add item to index page in %u/%u/%u", node.spcNode,
...
static void ginRedoRecompress(Page page, ginxlogRecompressDataLeaf *data) {
  ...
  if (!GinPageIsCompressed(page)) {
    ...
    plist =
        ginCompressPostingList(uncompressed, nuncompressed, BLCKSZ, &npacked);
  ...
  for (actionno = 0; actionno < data->nactions; actionno++) {
    ...
    if (a_action == GIN_SEGMENT_ADDITEMS) {
      ...
      newitems =
          ginMergeItemPointers(items, nitems, olditems, nolditems, &nnewitems);
    ...
    switch (a_action) {
      ...
      case GIN_SEGMENT_REPLACE:
        ...
        memmove(segptr + newsegsize, segptr + segsize, szleft - segsize);
...
static void ginRedoInsertData(Buffer buffer, bool isLeaf,
                              BlockNumber rightblkno, void *rdata) {
  Page page = BufferGetPage(buffer);
...
static void ginRedoSplit(XLogReaderState *record) {
   ...
   * split
   */
  if (!isLeaf) ginRedoClearIncompleteSplit(record, 3);
...
static void ginRedoDeletePage(XLogReaderState *record) {
  ...
  if (BufferIsValid(lbuffer)) UnlockReleaseBuffer(lbuffer);
  if (BufferIsValid(pbuffer)) UnlockReleaseBuffer(pbuffer);
  if (BufferIsValid(dbuffer)) UnlockReleaseBuffer(dbuffer);
}
  ...
  if (data->ntuples > 0) {
    ...
    if (XLogReadBufferForRedo(record, 1, &buffer) == BLK_NEEDS_REDO) {
      ...
      for (i = 0; i < data->ntuples; i++) {
        ...
        if (PageAddItem(page, (Item)tuples, tupsize, off, false, false) ==
            InvalidOffsetNumber)
...
void gin_xlog_startup(void) {
  opCtx = AllocSetContextCreate(
      CurrentMemoryContext, "GIN recovery temporary context",
      ALLOCSET_DEFAULT_MINSIZE, ALLOCSET_DEFAULT_INITSIZE,
      ALLOCSET_DEFAULT_MAXSIZE);
}

void gin_xlog_cleanup(void) { MemoryContextDelete(opCtx); }
