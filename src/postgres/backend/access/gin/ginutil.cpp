...
void initGinState(GinState *state, Relation index) {
  ...
  for (i = 0; i < origTupdesc->natts; i++) {
    if (state->oneCol)
      ...
      TupleDescInitEntry(state->tupdesc[i], (AttrNumber)1, NULL, INT2OID, -1,
                         0);
    ...
    if (state->consistentFn[i].fn_oid == InvalidOid &&
      ...
      elog(ERROR,
           "missing GIN support function (%d or %d) for attribute %d of index "
           "\"%s\"",
           GIN_CONSISTENT_PROC, GIN_TRICONSISTENT_PROC, i + 1,
...
OffsetNumber gintuple_get_attrnum(GinState *ginstate, IndexTuple tuple) {
  ...
  if (ginstate->oneCol) {
     ...
     */
    res =
        index_getattr(tuple, FirstOffsetNumber, ginstate->tupdesc[0], &isnull);
    Assert(!isnull);
 ...
 */
Datum gintuple_get_key(GinState *ginstate, IndexTuple tuple,
  ...
  if (ginstate->oneCol) {
     ...
     */
    res =
        index_getattr(tuple, FirstOffsetNumber, ginstate->origTupdesc, &isnull);
  } else {
    /*
    ...
    res = index_getattr(tuple, OffsetNumberNext(FirstOffsetNumber),
                        ginstate->tupdesc[colN - 1], &isnull);
  }
...
Buffer GinNewBuffer(Relation index) {
  ...
  for (;;) {
    ...
    if (ConditionalLockBuffer(buffer)) {
      ...
      if (GinPageIsDeleted(page)) return buffer; /* OK to use */
 ...
 */
int ginCompareEntries(GinState *ginstate, OffsetNumber attnum, Datum a,
                      GinNullCategory categorya, Datum b,
                      GinNullCategory categoryb) {
  /* if not of same null category, sort by that first */
  if (categorya != categoryb) return (categorya < categoryb) ? -1 : 1;

  /* all null items in same category are equal */
  if (categorya != GIN_CAT_NORM_KEY) return 0;
 ...
 */
int ginCompareAttEntries(GinState *ginstate, OffsetNumber attnuma, Datum a,
                         GinNullCategory categorya, OffsetNumber attnumb,
                         Datum b, GinNullCategory categoryb) {
  /* attribute number is the first sort key */
  if (attnuma != attnumb) return (attnuma < attnumb) ? -1 : 1;
...
static int cmpEntries(const void *a, const void *b, void *arg) {
  ...
  if (aa->isnull) {
    ...
    res = DatumGetInt32(FunctionCall2Coll(data->cmpDatumFunc, data->collation,
                                          aa->datum, bb->datum));
 ...
 */
Datum *ginExtractEntries(GinState *ginstate, OffsetNumber attnum, Datum value,
                         bool isNull, int32 *nentries,
  ...
  entries = (Datum *)DatumGetPointer(FunctionCall3Coll(
      ...
      ginstate->supportCollation[attnum - 1], value, PointerGetDatum(nentries),
  ...
  *categories = (GinNullCategory *)nullFlags;
  ...
  if (*nentries > 1) {
    ...
    arg.haveDups = false;
    qsort_arg(keydata, *nentries, sizeof(keyEntryData), cmpEntries,
              (void *)&arg);
...
Datum ginoptions(PG_FUNCTION_ARGS) {
  ...
  static const relopt_parse_elt tab[] = {
      ...
      {"gin_pending_list_limit", RELOPT_TYPE_INT,
