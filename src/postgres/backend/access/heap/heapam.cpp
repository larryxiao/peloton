...
static HeapScanDesc heap_beginscan_internal(
    Relation relation, Snapshot snapshot, int nkeys, ScanKey key,
    bool allow_strat, bool allow_sync, bool allow_pagemode, bool is_bitmapscan,
    bool is_samplescan, bool temp_snap);
static HeapTuple heap_prepare_insert(Relation relation, HeapTuple tup,
                                     TransactionId xid, CommandId cid,
                                     int options);
static XLogRecPtr log_heap_update(Relation reln, Buffer oldbuf, Buffer newbuf,
                                  HeapTuple oldtup, HeapTuple newtup,
                                  HeapTuple old_key_tup,
                                  bool all_visible_cleared,
                                  bool new_all_visible_cleared);
static void HeapSatisfiesHOTandKeyUpdate(
    Relation relation, Bitmapset *hot_attrs, Bitmapset *key_attrs,
    Bitmapset *id_attrs, bool *satisfies_hot, bool *satisfies_key,
    bool *satisfies_id, HeapTuple oldtup, HeapTuple newtup);
...
static bool ConditionalMultiXactIdWait(MultiXactId multi,
                                       MultiXactStatus status, uint16 infomask,
                                       Relation rel, int *remaining);
...
tupleLockExtraInfo[MaxLockTupleMode + 1] = {
    {
     /* LockTupleKeyShare */
     AccessShareLock, MultiXactStatusForKeyShare,
     -1 /* KeyShare does not allow updating tuples */
    },
    {
     /* LockTupleShare */
     RowShareLock, MultiXactStatusForShare,
...
static void heapgettup(HeapScanDesc scan, ScanDirection dir, int nkeys,
  ...
  if (ScanDirectionIsForward(dir)) {
    ...
    if (page != scan->rs_cblock) heapgetpage(scan, page);
  ...
  for (;;) {
    while (linesleft > 0) {
      if (ItemIdIsNormal(lpp)) {
        ...
        if (valid && key != NULL)
          HeapKeyTest(tuple, RelationGetDescr(scan->rs_rd), nkeys, key, valid);
    ...
    if (backward) {
      finished =
          ...
          (scan->rs_numblocks != InvalidBlockNumber ? --scan->rs_numblocks <= 0
                                                    : false);
       ...
       */
      if (scan->rs_syncscan) ss_report_location(scan->rs_rd, page);
    }
...
Relation try_relation_open(Oid relationId, LOCKMODE lockmode) {
  ...
  if (!SearchSysCacheExists1(RELOID, ObjectIdGetDatum(relationId))) {
    ...
    if (lockmode != NoLock) UnlockRelationOid(relationId, lockmode);
...
Relation relation_openrv_extended(const RangeVar *relation, LOCKMODE lockmode,
  ...
  if (!OidIsValid(relOid)) return NULL;
 ...
 * ----------------
 */
HeapScanDesc heap_beginscan(Relation relation, Snapshot snapshot, int nkeys,
                            ScanKey key) {
  return heap_beginscan_internal(relation, snapshot, nkeys, key, true, true,
...
HeapScanDesc heap_beginscan_strat(Relation relation, Snapshot snapshot,
                                  ...
                                  bool allow_sync) {
  return heap_beginscan_internal(relation, snapshot, nkeys, key, allow_strat,
                                 allow_sync, true, false, false, false);
}

HeapScanDesc heap_beginscan_bm(Relation relation, Snapshot snapshot, int nkeys,
                               ScanKey key) {
  return heap_beginscan_internal(relation, snapshot, nkeys, key, false, false,
                                 true, true, false, false);
}

HeapScanDesc heap_beginscan_sampling(Relation relation, Snapshot snapshot,
                                     int nkeys, ScanKey key, bool allow_strat,
                                     bool allow_pagemode) {
  return heap_beginscan_internal(relation, snapshot, nkeys, key, allow_strat,
                                 false, allow_pagemode, false, true, false);
}
...
static HeapScanDesc heap_beginscan_internal(
   ...
   */
  if (!is_bitmapscan) PredicateLockRelation(relation, snapshot);
...
void heap_endscan(HeapScanDesc scan) {
  ...
  if (scan->rs_key) pfree(scan->rs_key);

  if (scan->rs_strategy != NULL) FreeAccessStrategy(scan->rs_strategy);

  if (scan->rs_temp_snap) UnregisterSnapshot(scan->rs_snapshot);
...
bool heap_fetch(Relation relation, Snapshot snapshot, HeapTuple tuple,
  ...
  if (valid) PredicateLockTuple(relation, tuple, snapshot);
  ...
  if (valid) {
    ...
    *userbuf = buffer;
    ...
    if (stats_relation != NULL) pgstat_count_heap_fetch(stats_relation);
...
bool heap_hot_search_buffer(ItemPointer tid, Relation relation, Buffer buffer,
  ...
  if (all_dead) *all_dead = first_call;
  ...
  for (;;) {
     ...
     */
    if (at_chain_start && HeapTupleIsHeapOnly(heapTuple)) break;
...
bool heap_hot_search(ItemPointer tid, Relation relation, Snapshot snapshot,
  ...
  buffer = ReadBuffer(relation, ItemPointerGetBlockNumber(tid));
  LockBuffer(buffer, BUFFER_LOCK_SHARE);
  result = heap_hot_search_buffer(tid, relation, buffer, snapshot, &heapTuple,
                                  all_dead, true);
  LockBuffer(buffer, BUFFER_LOCK_UNLOCK);
  ReleaseBuffer(buffer);
  return result;
...
void heap_get_latest_tid(Relation relation, Snapshot snapshot,
  ...
  if (!ItemPointerIsValid(tid)) return;
  ...
  if (blk >= RelationGetNumberOfBlocks(relation))
    elog(ERROR, "block number %u is out of range for relation \"%s\"", blk,
...
Oid heap_insert(Relation relation, HeapTuple tup, CommandId cid, int options,
  ...
  if (!(options & HEAP_INSERT_SKIP_WAL) && RelationNeedsWAL(relation)) {
    ...
    Assert(ItemPointerGetBlockNumber(&heaptup->t_self) ==
...
static HeapTuple heap_prepare_insert(Relation relation, HeapTuple tup,
  ...
  if (options & HEAP_INSERT_FROZEN) HeapTupleHeaderSetXminFrozen(tup->t_data);
...
void heap_multi_insert(Relation relation, HeapTuple *tuples, int ntuples,
  ...
  heaptuples =
      static_cast<HeapTupleData **>(palloc(ntuples * sizeof(HeapTuple)));
  for (i = 0; i < ntuples; i++)
   ...
   * beforehand.
   */
  if (needwal) scratch = static_cast<char *>(palloc(BLCKSZ));
  ...
  while (ndone < ntuples) {
    ...
    for (nthispage = 1; ndone + nthispage < ntuples; nthispage++) {
       ...
       * better be prepared...
       */
      if (needwal && need_cids) log_heap_new_cid(relation, heaptup);
    }
    ...
    if (needwal) {
       ...
       */
      init =
          (ItemPointerGetOffsetNumber(&(heaptuples[ndone]->t_self)) ==
               FirstOffsetNumber &&
      ...
      for (i = 0; i < nthispage; i++) {
        ...
        memcpy(scratchptr, (char *)heaptup->t_data + SizeofHeapTupleHeader,
               datalen);
      ...
      if (need_tuple_data) xlrec->flags |= XLH_INSERT_CONTAINS_NEW_TUPLE;
   ...
   */
  for (i = 0; i < ntuples; i++) tuples[i]->t_self = heaptuples[i]->t_self;
 ...
 */
static inline bool xmax_infomask_changed(uint16 new_infomask,
                                         uint16 old_infomask) {
  const uint16 interesting =
      HEAP_XMAX_IS_MULTI | HEAP_XMAX_LOCK_ONLY | HEAP_LOCK_MASK;

  if ((new_infomask & interesting) != (old_infomask & interesting)) return true;
 ...
 */
HTSU_Result heap_delete(Relation relation, ItemPointer tid, CommandId cid,
   ...
   * the lock.
   */
  if (PageIsAllVisible(page)) visibilitymap_pin(relation, block, &vmbuffer);
...
l1:
  ...
  compute_new_xmax_infomask(HeapTupleHeaderGetRawXmax(tp.t_data),
                            tp.t_data->t_infomask, tp.t_data->t_infomask2, xid,
                            LockTupleExclusive, true, &new_xmax, &new_infomask,
                            &new_infomask2);
  ...
  if (old_key_tuple != NULL && old_key_copied) heap_freetuple(old_key_tuple);
...
void simple_heap_delete(Relation relation, ItemPointer tid) {
  ...
  result = heap_delete(relation, tid, GetCurrentCommandId(true),
                       InvalidSnapshot, true /* wait for commit */, &hufd);
  switch (result) {
...
HTSU_Result heap_update(Relation relation, ItemPointer otid, HeapTuple newtup,
  ...
  uint16 infomask_old_tuple, infomask2_old_tuple, infomask_new_tuple,
      infomask2_new_tuple;
...
l2:
  ...
  if (result == HeapTupleInvisible) {
    ...
    if (infomask & HEAP_XMAX_IS_MULTI) {
      ...
      if (DoesMultiXactIdConflict((MultiXactId)xwait, infomask, *lockmode)) {
        ...
        /* wait for multixact */
        MultiXactIdWait((MultiXactId)xwait, mxact_status, infomask, relation,
                        &oldtup.t_self, XLTW_Update, &remain);
       ...
       */
      LockBuffer(buffer, BUFFER_LOCK_UNLOCK);
      heap_acquire_tuplock(relation, &(oldtup.t_self), *lockmode, LockWaitBlock,
                           &have_tuple_lock);
      ...
      if (oldtup.t_data->t_infomask & HEAP_XMAX_INVALID) can_continue = true;
    }
  ...
  compute_new_xmax_infomask(
      HeapTupleHeaderGetRawXmax(oldtup.t_data), oldtup.t_data->t_infomask,
      oldtup.t_data->t_infomask2, xid, *lockmode, true, &xmax_old_tuple,
  ...
  if (need_toast || newtupsize > pagefree) {
    ...
    if (newtupsize > pagefree) {
      ...
      newbuf = RelationGetBufferForTuple(relation, heaptup->t_len, buffer, 0,
                                         NULL, &vmbuffer_new, &vmbuffer);
    } else {
  ...
  if (newbuf == buffer) {
     ...
     */
    if (satisfies_hot) use_hot_update = true;
  } else {
   ...
   */
  old_key_tuple =
      ExtractReplicaIdentity(relation, &oldtup, !satisfies_id, &old_key_copied);
  ...
  if (RelationNeedsWAL(relation)) {
    ...
    recptr = log_heap_update(relation, buffer, newbuf, &oldtup, heaptup,
                             old_key_tuple, all_visible_cleared,
  ...
  if (newbuf != buffer) LockBuffer(newbuf, BUFFER_LOCK_UNLOCK);
  LockBuffer(buffer, BUFFER_LOCK_UNLOCK);
  ...
  if (newbuf != buffer) ReleaseBuffer(newbuf);
  ReleaseBuffer(buffer);
  if (BufferIsValid(vmbuffer_new)) ReleaseBuffer(vmbuffer_new);
  if (BufferIsValid(vmbuffer)) ReleaseBuffer(vmbuffer);
...
static void HeapSatisfiesHOTandKeyUpdate(
  ...
  for (;;) {
    ...
    changed = !heap_tuple_attr_equals(RelationGetDescr(relation), check_now,
                                      oldtup, newtup);
    if (changed) {
      if (check_now == next_hot_attnum) hot_result = false;
      if (check_now == next_key_attnum) key_result = false;
      if (check_now == next_id_attnum) id_result = false;

      /* if all are false now, we can stop checking */
      if (!hot_result && !key_result && !id_result) break;
    }
 ...
 */
HTSU_Result heap_lock_tuple(Relation relation, HeapTuple tuple, CommandId cid,
                            LockTupleMode mode, LockWaitPolicy wait_policy,
                            bool follow_updates, Buffer *buffer,
                            HeapUpdateFailureData *hufd) {
  HTSU_Result result;
  ItemPointer tid = &(tuple->t_self);
  ItemId lp;
  ...
  if (result == HeapTupleInvisible) {
    ...
    if (first_time) {
      ...
      if (infomask & HEAP_XMAX_IS_MULTI) {
        ...
        for (i = 0; i < nmembers; i++) {
          ...
          if (!TransactionIdIsCurrentTransactionId(members[i].xid)) continue;
    ...
    if (require_sleep) {
      ...
      if (infomask & HEAP_XMAX_IS_MULTI) {
        ...
        switch (wait_policy) {
          case LockWaitBlock:
            MultiXactIdWait((MultiXactId)xwait, status, infomask, relation,
                            &tuple->t_self, XLTW_Lock, NULL);
            break;
          case LockWaitSkip:
...
failed:
  if (result != HeapTupleMayBeUpdated) {
    ...
    if (result == HeapTupleSelfUpdated)
      ...
      hufd->cmax = InvalidCommandId;
    LockBuffer(*buffer, BUFFER_LOCK_UNLOCK);
    if (have_tuple_lock) UnlockTupleTuplock(relation, tid, mode);
    return result;
  }
  ...
  compute_new_xmax_infomask(xmax, old_infomask, tuple->t_data->t_infomask2,
                            GetCurrentTransactionId(), mode, false, &xid,
   ...
   * the tuple as well.
   */
  if (HEAP_XMAX_IS_LOCKED_ONLY(new_infomask)) tuple->t_data->t_ctid = *tid;
  ...
  if (RelationNeedsWAL(relation)) {
    ...
    xlrec.infobits_set =
        compute_infobits(new_infomask, tuple->t_data->t_infomask2);
...
static bool heap_acquire_tuplock(Relation relation, ItemPointer tid,
                                 LockTupleMode mode, LockWaitPolicy wait_policy,
                                 bool *have_tuple_lock) {
  if (*have_tuple_lock) return true;
  ...
  switch (wait_policy) {
    ...
    case LockWaitSkip:
      if (!ConditionalLockTupleTuplock(relation, tid, mode)) return false;
      break;
 ...
 * add_to_xmax) acquiring a new___ lock of some mode, compute the new___ Xmax
 *and
...
l5:
  ...
  if (old_infomask & HEAP_XMAX_INVALID) {
    ...
    if (is_update) {
      ...
      if (mode == LockTupleExclusive) new_infomask2 |= HEAP_KEYS_UPDATED;
    } else {
    ...
    if (!MultiXactIdIsRunning(xmax, HEAP_XMAX_IS_LOCKED_ONLY(old_infomask))) {
      if (HEAP_XMAX_IS_LOCKED_ONLY(old_infomask) ||
          !TransactionIdDidCommit(
              MultiXactIdGetUpdateXid(xmax, old_infomask))) {
        /*
    ...
    if (xmax == add_to_xmax) {
      ...
      if (mode < old_mode) mode = old_mode;
 ...
 */
static HTSU_Result test_lockmode_for_conflict(MultiXactStatus status,
                                              TransactionId xid,
                                              LockTupleMode mode,
                                              bool *needwait) {
  ...
  *needwait = false;
  ...
  if (TransactionIdIsCurrentTransactionId(xid)) {
     ...
     * always be checked.
     */
    if (!ISUPDATE_from_mxstatus(status)) return HeapTupleMayBeUpdated;
 ...
 * version as well.
 */
static HTSU_Result heap_lock_updated_tuple_rec(Relation rel, ItemPointer tid,
                                               TransactionId xid,
                                               LockTupleMode mode) {
  ...
  l4:
    ...
    if (!(old_infomask & HEAP_XMAX_INVALID)) {
      ...
      if (old_infomask & HEAP_XMAX_IS_MULTI) {
        ...
        nmembers = GetMultiXactIdMembers(
            rawxmax, &members, false, HEAP_XMAX_IS_LOCKED_ONLY(old_infomask));
        for (i = 0; i < nmembers; i++) {
          HTSU_Result res;

          res = test_lockmode_for_conflict(members[i].status, members[i].xid,
                                           mode, &needwait);
        ...
        res = test_lockmode_for_conflict(status, rawxmax, mode, &needwait);
        if (needwait) {
          LockBuffer(buf, BUFFER_LOCK_UNLOCK);
    ...
    compute_new_xmax_infomask(xmax, old_infomask, mytup.t_data->t_infomask2,
                              xid, mode, false, &new_xmax, &new_infomask,
                              &new_infomask2);
...
void heap_finish_speculative(Relation relation, HeapTuple tuple) {
  ...
  if (PageGetMaxOffsetNumber(page) >= offnum) lp = PageGetItemId(page, offnum);
...
void heap_abort_speculative(Relation relation, HeapTuple tuple) {
  ...
  if (HeapTupleHasExternal(&tp)) toast_delete(relation, &tp);
 ...
 */
static TransactionId FreezeMultiXactId(MultiXactId multi, uint16 t_infomask,
  ...
  *flags = 0;
  ...
  if (!MultiXactIdIsValid(multi)) {
    ...
    *flags |= FRM_INVALIDATE_XMAX;
    ...
    Assert((!(t_infomask & HEAP_LOCK_MASK) &&
           ...
           !MultiXactIdIsRunning(multi, HEAP_XMAX_IS_LOCKED_ONLY(t_infomask)));
  ...
  allow_old =
      !(t_infomask & HEAP_LOCK_MASK) && HEAP_XMAX_IS_LOCKED_ONLY(t_infomask);
   ...
   */
  nnewmembers = 0;
  newmembers = static_cast<MultiXactMember *>(
      palloc(sizeof(MultiXactMember) * nmembers));
  has_lockers = false;
  ...
  if (nnewmembers == 0) {
    ...
    *flags |= FRM_RETURN_IS_XID;
    if (update_committed) *flags |= FRM_MARK_COMMITTED;
    xid = update_xid;
  } else {
    /*
 ...
 */
bool heap_prepare_freeze_tuple(HeapTupleHeader tuple, TransactionId cutoff_xid,
...

{
  ...
  if (tuple->t_infomask & HEAP_XMAX_IS_MULTI) {
    ...
    newxmax = FreezeMultiXactId(xid, tuple->t_infomask, cutoff_xid,
                                cutoff_multi, &flags);
    ...
    if (flags & FRM_INVALIDATE_XMAX)
      ...
      frz->xmax = newxmax;
      if (flags & FRM_MARK_COMMITTED) frz->t_infomask &= HEAP_XMAX_COMMITTED;
      changed = true;
...
bool heap_freeze_tuple(HeapTupleHeader tuple, TransactionId cutoff_xid,
  ...
  if (do_freeze) heap_execute_freeze_tuple(tuple, &frz);
  return do_freeze;
}
...
static void GetMultiXactIdHintBits(MultiXactId multi, uint16 *new_infomask,
  ...
  for (i = 0; i < nmembers; i++) {
    ...
    if (mode > strongest) strongest = mode;
...
static TransactionId MultiXactIdGetUpdateXid(TransactionId xmax,
  ...
  if (nmembers > 0) {
    ...
    for (i = 0; i < nmembers; i++) {
      /* Ignore lockers */
      if (!ISUPDATE_from_mxstatus(members[i].status)) continue;
...
static bool DoesMultiXactIdConflict(MultiXactId multi, uint16 infomask,
  ...
  allow_old =
      !(infomask & HEAP_LOCK_MASK) && HEAP_XMAX_IS_LOCKED_ONLY(infomask);
  nmembers = GetMultiXactIdMembers(multi, &members, allow_old,
                                   HEAP_XMAX_IS_LOCKED_ONLY(infomask));
  if (nmembers >= 0) {
    int i;
    ...
    for (i = 0; i < nmembers; i++) {
      ...
      if (!DoLockModesConflict(memlockmode, wanted)) continue;

      /* ignore members from current xact */
      memxid = members[i].xid;
      if (TransactionIdIsCurrentTransactionId(memxid)) continue;

      if (ISUPDATE_from_mxstatus(members[i].status)) {
        /* ignore aborted updaters */
        if (TransactionIdDidAbort(memxid)) continue;
      } else {
        /* ignore lockers-only that are no longer in progress */
        if (!TransactionIdIsInProgress(memxid)) continue;
      }
...
static bool Do_MultiXactIdWait(MultiXactId multi, MultiXactStatus status,
                               uint16 infomask, bool nowait, Relation rel,
  ...
  if (nmembers >= 0) {
    ...
    for (i = 0; i < nmembers; i++) {
      ...
      if (!DoLockModesConflict(LOCKMODE_from_mxstatus(memstatus),
        ...
        if (remaining && TransactionIdIsInProgress(memxid)) remain++;
        continue;
      }
...
static void MultiXactIdWait(MultiXactId multi, MultiXactStatus status,
                            uint16 infomask, Relation rel, ItemPointer ctid,
                            XLTW_Oper oper, int *remaining) {
  (void)Do_MultiXactIdWait(multi, status, infomask, false, rel, ctid, oper,
                           remaining);
}
...
static bool ConditionalMultiXactIdWait(MultiXactId multi,
                                       MultiXactStatus status, uint16 infomask,
                                       Relation rel, int *remaining) {
  return Do_MultiXactIdWait(multi, status, infomask, true, rel, NULL, XLTW_None,
                            remaining);
}
 ...
 */
XLogRecPtr log_heap_visible(RelFileNode rnode, Buffer heap_buffer,
  ...
  if (!XLogHintBitIsNeeded()) flags |= REGBUF_NO_IMAGE;
...
static XLogRecPtr log_heap_update(Relation reln, Buffer oldbuf, Buffer newbuf,
  ...
  if (oldbuf == newbuf && !need_tuple_data &&
    ...
    for (prefixlen = 0; prefixlen < Min(oldlen, newlen); prefixlen++) {
      if (newp[prefixlen] != oldp[prefixlen]) break;
    }
    ...
    /* Same for suffix */
    for (suffixlen = 0; suffixlen < Min(oldlen, newlen) - prefixlen;
         suffixlen++) {
      if (newp[newlen - suffixlen - 1] != oldp[oldlen - suffixlen - 1]) break;
    }
  ...
  xlrec.old_infobits_set =
      compute_infobits(oldtup->t_data->t_infomask, oldtup->t_data->t_infomask2);
  ...
  if (prefixlen == 0) {
    ...
    XLogRegisterBufData(
        0, ((char *)newtup->t_data) + newtup->t_data->t_hoff + prefixlen,
...
static HeapTuple ExtractReplicaIdentity(Relation relation, HeapTuple tp,
  ...
  *copy = false;
  ...
  if (!RelationIsLogicallyLogged(relation)) return NULL;

  if (replident == REPLICA_IDENTITY_NOTHING) return NULL;
  ...
  for (natt = 0; natt < idx_desc->natts; natt++) {
    ...
    if (attno < 0) {
       ...
       */
      if (attno == ObjectIdAttributeNumber) continue;
...
static void heap_xlog_clean(XLogReaderState *record) {
  ...
  if (action == BLK_NEEDS_REDO) {
    ...
    heap_page_prune_execute(buffer, redirected, nredirected, nowdead, ndead,
                            nowunused, nunused);
  ...
  if (BufferIsValid(buffer)) UnlockReleaseBuffer(buffer);
...
static void heap_xlog_visible(XLogReaderState *record) {
  ...
  if (XLogReadBufferForRedoExtended(record, 0, RBM_ZERO_ON_ERROR, false,
    ...
    if (PageIsNew(vmpage)) PageInit(vmpage, BLCKSZ, 0);
...
static void fix_infomask_from_infobits(uint8 infobits, uint16 *infomask,
  ...
  *infomask2 &= ~HEAP_KEYS_UPDATED;
  ...
  if (infobits & XLHL_XMAX_IS_MULTI) *infomask |= HEAP_XMAX_IS_MULTI;
  if (infobits & XLHL_XMAX_LOCK_ONLY) *infomask |= HEAP_XMAX_LOCK_ONLY;
  if (infobits & XLHL_XMAX_EXCL_LOCK) *infomask |= HEAP_XMAX_EXCL_LOCK;
  /* note HEAP_XMAX_SHR_LOCK isn't considered here */
  if (infobits & XLHL_XMAX_KEYSHR_LOCK) *infomask |= HEAP_XMAX_KEYSHR_LOCK;

  if (infobits & XLHL_KEYS_UPDATED) *infomask2 |= HEAP_KEYS_UPDATED;
}
...
static void heap_xlog_insert(XLogReaderState *record) {
  ...
  if (action == BLK_NEEDS_REDO) {
    ...
    if (PageAddItem(page, (Item)htup, newlen, xlrec->offnum, true, true) ==
        InvalidOffsetNumber)
...
static void heap_xlog_update(XLogReaderState *record, bool hot_update) {
  ...
  oldaction =
      XLogReadBufferForRedo(record, (oldblk == newblk) ? 0 : 1, &obuffer);
  ...
  if (BufferIsValid(obuffer)) UnlockReleaseBuffer(obuffer);
...
static void heap_xlog_inplace(XLogReaderState *record) {
  ...
  if (XLogReadBufferForRedo(record, 0, &buffer) == BLK_NEEDS_REDO) {
    ...
    oldlen = ItemIdGetLength(lp) - htup->t_hoff;
    if (oldlen != newlen) elog(PANIC, "heap_inplace_redo: wrong tuple length");
...
void heap_sync(Relation rel) {
  ...
  if (!RelationNeedsWAL(rel)) return;
