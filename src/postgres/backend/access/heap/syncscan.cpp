 ...
 * a new___ backend beginning a seqscan to begin it close to where other
 *backends
...
static BlockNumber ss_search(RelFileNode relfilenode, BlockNumber location,
                             bool set);

/*
 * SyncScanShmemSize --- report amount of shared memory space needed
 */
Size SyncScanShmemSize(void) { return SizeOfScanLocations(SYNC_SCAN_NELEM); }
...
void SyncScanShmemInit(void) {
  ...
  scan_locations = (ss_scan_locations_t *)ShmemInitStruct(
      "Sync Scan Locations List", SizeOfScanLocations(SYNC_SCAN_NELEM), &found);
  ...
  if (!IsUnderPostmaster) {
    ...
    for (i = 0; i < SYNC_SCAN_NELEM; i++) {
      ...
      item->next =
          (i < SYNC_SCAN_NELEM - 1) ? (&scan_locations->items[i + 1]) : NULL;
    }
  } else
    Assert(found);
}
  ...
  for (;;) {
    ...
    if (match || item->next == NULL) {
      ...
      if (item != scan_locations->head) {
        /* unlink */
        if (item == scan_locations->tail) scan_locations->tail = item->prev;
...
BlockNumber ss_get_location(Relation rel, BlockNumber relnblocks) {
   ...
   * location was saved.
   */
  if (startloc >= relnblocks) startloc = 0;
