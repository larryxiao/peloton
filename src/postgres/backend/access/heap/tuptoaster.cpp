...
static int toast_open_indexes(Relation toastrel, LOCKMODE lock,
...
struct varlena *heap_tuple_fetch_attr(struct varlena *attr) {
  ...
  if (VARATT_IS_EXTERNAL_ONDISK(attr)) {
    ...
    if (VARATT_IS_EXTERNAL(attr)) return heap_tuple_fetch_attr(attr);
...
struct varlena *heap_tuple_untoast_attr_slice(struct varlena *attr,
  ...
  if (VARATT_IS_EXTERNAL_ONDISK(attr)) {
    ...
    return heap_tuple_untoast_attr_slice(redirect.pointer, sliceoffset,
                                         slicelength);
  } else if (VARATT_IS_EXTERNAL_EXPANDED(attr)) {
...
HeapTuple toast_insert_or_update(Relation rel, HeapTuple newtup,
  ...
  for (i = 0; i < numAttrs; i++) {
    ...
    if (att[i]->attlen == -1) {
       ...
       */
      if (att[i]->attstorage == 'p') toast_action[i] = 'p';
  ...
  if (has_nulls) hoff += BITMAPLEN(numAttrs);
  if (newtup->t_data->t_infomask & HEAP_HASOID) hoff += sizeof(Oid);
  hoff = MAXALIGN(hoff);
  ...
  while (heap_compute_data_size(tupleDesc, toast_values, toast_isnull) >
     ...
     */
    for (i = 0; i < numAttrs; i++) {
      if (toast_action[i] != ' ') continue;
      if (VARATT_IS_EXTERNAL(DatumGetPointer(toast_values[i])))
        continue; /* can't happen, toast_action would be 'p' */
      if (VARATT_IS_COMPRESSED(DatumGetPointer(toast_values[i]))) continue;
      if (att[i]->attstorage != 'x' && att[i]->attstorage != 'e') continue;
    ...
    if (att[i]->attstorage == 'x') {
      ...
      if (DatumGetPointer(new_value) != NULL) {
        ...
        if (toast_free[i]) pfree(DatumGetPointer(old_value));
    ...
    if (toast_sizes[i] > maxDataLen &&
      ...
      toast_action[i] = 'p';
      toast_values[i] =
          toast_save_datum(rel, toast_values[i], toast_oldexternal[i], options);
      if (toast_free[i]) pfree(DatumGetPointer(old_value));
      toast_free[i] = true;
      need_change = true;
      need_free = true;
  ...
  while (heap_compute_data_size(tupleDesc, toast_values, toast_isnull) >
    ...
    for (i = 0; i < numAttrs; i++) {
      ...
      if (VARATT_IS_EXTERNAL(DatumGetPointer(toast_values[i])))
        continue; /* can't happen, toast_action would be 'p' */
      if (VARATT_IS_COMPRESSED(DatumGetPointer(toast_values[i]))) continue;
      if (att[i]->attstorage != 'm') continue;
      if (toast_sizes[i] > biggest_size) {
        biggest_attno = i;
        biggest_size = toast_sizes[i];
      }
  ...
  if (need_change) {
    ...
    if (has_nulls) new_header_len += BITMAPLEN(numAttrs);
    if (olddata->t_infomask & HEAP_HASOID) new_header_len += sizeof(Oid);
  ...
  if (need_free)
    for (i = 0; i < numAttrs; i++)
      if (toast_free[i]) pfree(DatumGetPointer(toast_values[i]));
  ...
  if (need_delold)
    for (i = 0; i < numAttrs; i++)
      if (toast_delold[i]) toast_delete_datum(rel, toast_oldvalues[i]);
...
HeapTuple toast_flatten_tuple(HeapTuple tup, TupleDesc tupleDesc) {
   ...
   */
  if (tupleDesc->tdhasoid) HeapTupleSetOid(new_tuple, HeapTupleGetOid(tup));
 ...
 * ----------
 */
Datum toast_flatten_tuple_to_datum(HeapTupleHeader tup, uint32 tup_len,
                                   TupleDesc tupleDesc) {
  HeapTupleHeader new_data;
  int32 new_header_len;
  int32 new_data_len;
  ...
  new_header_len = SizeofHeapTupleHeader;
  if (has_nulls) new_header_len += BITMAPLEN(numAttrs);
  if (tup->t_infomask & HEAP_HASOID) new_header_len += sizeof(Oid);
  new_header_len = MAXALIGN(new_header_len);
...
Datum toast_compress_datum(Datum value) {
  ...
  tmp =
      (struct varlena *)palloc(PGLZ_MAX_OUTPUT(valsize) + TOAST_COMPRESS_HDRSZ);
   ...
   */
  len = pglz_compress(VARDATA_ANY(DatumGetPointer(value)), valsize,
                      TOAST_COMPRESS_RAWDATA(tmp), PGLZ_strategy_default);
    ...
    if (toast_pointer.va_valueid == InvalidOid) {
      ...
      do {
        toast_pointer.va_valueid = GetNewOidWithIndex(
          ...
          toastid_valueid_exists(rel->rd_toastoid, toast_pointer.va_valueid));
    }
  }
  ...
  while (data_todo > 0) {
    ...
    for (i = 0; i < num_indexes; i++) {
      ...
      if (IndexIsReady(toastidxs[i]->rd_index))
        index_insert(toastidxs[i], t_values, t_isnull, &(toasttup->t_self),
                     toastrel,
...
static void toast_delete_datum(Relation rel, Datum value) {
  ...
  while ((toasttup = systable_getnext_ordered(toastscan,
...
static bool toastrel_valueid_exists(Relation toastrel, Oid valueid) {
   ...
   */
  toastscan =
      systable_beginscan(toastrel, RelationGetRelid(toastidxs[validIndex]),
                         true, SnapshotToast, 1, &toastkey);

  if (systable_getnext(toastscan) != NULL) result = true;
...
static struct varlena *toast_fetch_datum(struct varlena *attr) {
  ...
  /* Look for the valid index of the toast relation */
  validIndex =
      toast_open_indexes(toastrel, AccessShareLock, &toastidxs, &num_indexes);
  ...
  while ((ttup = systable_getnext_ordered(toastscan, ForwardScanDirection)) !=
    ...
    if (residx != nextidx)
      elog(ERROR,
           ...
           residx, nextidx, toast_pointer.va_valueid,
           RelationGetRelationName(toastrel));
    if (residx < numchunks - 1) {
      if (chunksize != TOAST_MAX_CHUNK_SIZE)
        elog(ERROR,
             "unexpected chunk size %d (expected %d) in chunk %d of %d for "
             "toast value %u in %s",
      ...
      if ((residx * TOAST_MAX_CHUNK_SIZE + chunksize) != ressize)
        elog(ERROR,
             "unexpected chunk size %d (expected %d) in final chunk %d for "
             "toast value %u in %s",
             chunksize, (int)(ressize - residx * TOAST_MAX_CHUNK_SIZE), residx,
             toast_pointer.va_valueid, RelationGetRelationName(toastrel));
    } else
      elog(ERROR,
           "unexpected chunk number %d (out of range %d..%d) for toast value "
           "%u in %s",
           residx, 0, numchunks - 1, toast_pointer.va_valueid,
           RelationGetRelationName(toastrel));

    /*
     * Copy the data into proper place in our result
     */
    memcpy(VARDATA(result) + residx * TOAST_MAX_CHUNK_SIZE, chunkdata,
           chunksize);
...
static struct varlena *toast_fetch_datum_slice(struct varlena *attr,
  ...
  if (numchunks == 1) {
    ...
    ScanKeyInit(&toastkey[1], (AttrNumber)2, BTGreaterEqualStrategyNumber,
                F_INT4GE, Int32GetDatum(startchunk));
    ScanKeyInit(&toastkey[2], (AttrNumber)2, BTLessEqualStrategyNumber,
                F_INT4LE, Int32GetDatum(endchunk));
    nscankeys = 3;
  }
  ...
  while ((ttup = systable_getnext_ordered(toastscan, ForwardScanDirection)) !=
    ...
    if (residx < totalchunks - 1) {
      if (chunksize != TOAST_MAX_CHUNK_SIZE)
        elog(ERROR,
             "unexpected chunk size %d (expected %d) in chunk %d of %d for "
             "toast value %u in %s when fetching slice",
             chunksize, (int)TOAST_MAX_CHUNK_SIZE, residx, totalchunks,
             toast_pointer.va_valueid, RelationGetRelationName(toastrel));
      ...
      elog(ERROR,
           "unexpected chunk number %d (out of range %d..%d) for toast value "
           "%u in %s",
           residx, 0, totalchunks - 1, toast_pointer.va_valueid,
           RelationGetRelationName(toastrel));
    ...
    memcpy(VARDATA(result) + (residx * TOAST_MAX_CHUNK_SIZE - sliceoffset) +
               chcpystrt,
...
static struct varlena *toast_decompress_datum(struct varlena *attr) {
  ...
  if (pglz_decompress(TOAST_COMPRESS_RAWDATA(attr),
                      VARSIZE(attr) - TOAST_COMPRESS_HDRSZ, VARDATA(result),
...
static void toast_close_indexes(Relation *toastidxs, int num_indexes,
  ...
  for (i = 0; i < num_indexes; i++) index_close(toastidxs[i], lock);
  pfree(toastidxs);
}
