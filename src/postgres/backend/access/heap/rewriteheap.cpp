 ...
 */
RewriteState begin_heap_rewrite(Relation old_heap, Relation new_heap,
  ...
  rw_cxt = AllocSetContextCreate(
      CurrentMemoryContext, "Table rewrite", ALLOCSET_DEFAULT_MINSIZE,
  ...
  state->rs_unresolved_tups = hash_create(
      "Rewrite / Unresolved ctids", 128, /* arbitrary initial size */
  ...
  state->rs_old_new_tid_map = hash_create(
      "Rewrite / Old to new___ tid map", 128, /* arbitrary initial size */
      &hash_ctl, HASH_ELEM | HASH_BLOBS | HASH_CONTEXT);
...
void end_heap_rewrite(RewriteState state) {
  ...
  while ((unresolved = static_cast<UnresolvedTup>(
  ...
  if (state->rs_buffer_valid) {
    if (state->rs_use_wal)
      log_newpage(&state->rs_new_rel->rd_node, MAIN_FORKNUM, state->rs_blockno,
   ...
   */
  if (RelationNeedsWAL(state->rs_new_rel)) heap_sync(state->rs_new_rel);
 ...
 */
void rewrite_heap_tuple(RewriteState state, HeapTuple old_tuple,
                        HeapTuple new_tuple) {
  MemoryContext old_cxt;
  ItemPointerData old_tid;
  TidHashKey hashkey;
  ...
  memcpy(&new_tuple->t_data->t_choice.t_heap,
         &old_tuple->t_data->t_choice.t_heap, sizeof(HeapTupleFields));
  ...
  if (!((old_tuple->t_data->t_infomask & HEAP_XMAX_INVALID) ||
    ...
    if (mapping != NULL) {
      ...
      unresolved = static_cast<UnresolvedTup>(
          hash_search(state->rs_unresolved_tups, &hashkey, HASH_ENTER, &found));
      Assert(!found);
  ...
  for (;;) {
    ...
    if ((new_tuple->t_data->t_infomask & HEAP_UPDATED) &&
      ...
      unresolved = static_cast<UnresolvedTup>(
          hash_search(state->rs_unresolved_tups, &hashkey, HASH_FIND, NULL));
      ...
      if (unresolved != NULL) {
         ...
         */
        if (free_new) heap_freetuple(new_tuple);
        ...
        mapping = static_cast<OldToNewMapping>(hash_search(
            state->rs_old_new_tid_map, &hashkey, HASH_ENTER, &found));
        Assert(!found);
...
static void raw_heap_insert(RewriteState state, HeapTuple tup) {
  ...
  if (heaptup != tup) heap_freetuple(heaptup);
}
...
static void logical_begin_heap_rewrite(RewriteState state) {
  ...
  if (!state->rs_logical_rewrite) return;
  ...
  state->rs_logical_mappings =
      hash_create("Logical rewrite mapping", 128, /* arbitrary initial size */
                  &hash_ctl, HASH_ELEM | HASH_BLOBS | HASH_CONTEXT);
}
...
static void logical_heap_rewrite_flush_mappings(RewriteState state) {
  ...
  if (state->rs_num_rewrite_mappings == 0) return;
  ...
  while ((src = (RewriteMappingFile *)hash_seq_search(&seq_status)) != NULL) {
    ...
    if (src->num_mappings == 0) continue;
    ...
    if (written != len)
      ereport(ERROR,
              (errcode_for_file_access(),
               errmsg("could not write to file \"%s\", wrote %d of %d: %m",
                      src->path, written, len)));
    src->off += len;
 ...
 */
static void logical_rewrite_log_mapping(RewriteState state, TransactionId xid,
  ...
  src = static_cast<RewriteMappingFile *>(
      hash_search(state->rs_logical_mappings, &xid, HASH_ENTER, &found));
  ...
  if (!found) {
    ...
    snprintf(path, MAXPGPATH, "pg_logical/mappings/" LOGICAL_REWRITE_FORMAT,
             dboid, relid, (uint32)(state->rs_begin_lsn >> 32),
             (uint32)state->rs_begin_lsn, xid, GetCurrentTransactionId());
  ...
  pmap = static_cast<RewriteMappingDataEntry *>(
      MemoryContextAlloc(state->rs_cxt, sizeof(RewriteMappingDataEntry)));
...
static void logical_rewrite_heap_tuple(RewriteState state,
  ...
  if (!do_log_xmin && !do_log_xmax) return;
   ...
   * ---
   */
  if (do_log_xmin) logical_rewrite_log_mapping(state, xmin, &map);
...
void heap_xlog_logical_rewrite(XLogReaderState *r) {
  ...
  snprintf(path, MAXPGPATH, "pg_logical/mappings/" LOGICAL_REWRITE_FORMAT,
           ...
           (uint32)(xlrec->start_lsn >> 32), (uint32)xlrec->start_lsn,
  ...
  if (ftruncate(fd, xlrec->offset) != 0)
    ereport(ERROR, (errcode_for_file_access(),
                    errmsg("could not truncate file \"%s\" to %u: %m", path,
                           (uint32)xlrec->offset)));
...
void CheckPointLogicalRewriteHeap(void) {
  ...
  while ((mapping_de = ReadDir(
              mappings_dir,
              static_cast<const char *>("pg_logical/mappings"))) != NULL) {
    struct stat statbuf;
    Oid dboid;
    Oid relid;
    XLogRecPtr lsn;
    ...
    if (lstat(path, &statbuf) == 0 && !S_ISREG(statbuf.st_mode)) continue;

    /* Skip over files that cannot be ours. */
    if (strncmp(mapping_de->d_name, "map-", 4) != 0) continue;

    if (sscanf(mapping_de->d_name, LOGICAL_REWRITE_FORMAT, &dboid, &relid, &hi,
               &lo, &rewrite_xid, &create_xid) != 6)
