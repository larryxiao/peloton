 ...
 */
Datum brininsert(PG_FUNCTION_ARGS) {
  ...
  for (;;) {
    ...
    if (bdesc == NULL) {
      ...
      tupcxt = AllocSetContextCreate(
          CurrentMemoryContext, "brininsert cxt", ALLOCSET_DEFAULT_MINSIZE,
    ...
    for (keyno = 0; keyno < bdesc->bd_tupdesc->natts; keyno++) {
      ...
      result = FunctionCall4Coll(addValue, idxRel->rd_indcollation[keyno],
    ...
    if (!need_insert) {
       ...
       */
      if (!brin_doupdate(idxRel, pagesPerRange, revmap, heapBlk, buf, off,
                         origtup, origsz, newtup, newsz, samepage)) {
        /* no luck; start over */
 ...
 */
Datum brinbeginscan(PG_FUNCTION_ARGS) {
 ...
 * keys.
 */
Datum bringetbitmap(PG_FUNCTION_ARGS) {
   ...
   */
  consistentFn = static_cast<FmgrInfo *>(
      palloc0(sizeof(FmgrInfo) * bdesc->bd_tupdesc->natts));
  ...
  perRangeCxt = AllocSetContextCreate(
      CurrentMemoryContext, "bringetbitmap cxt", ALLOCSET_DEFAULT_MINSIZE,
      ALLOCSET_DEFAULT_INITSIZE, ALLOCSET_DEFAULT_MAXSIZE);
  ...
  for (heapBlk = 0; heapBlk < nblocks; heapBlk += opaque->bo_pagesPerRange) {
    ...
    tup = brinGetTupleForHeapBlock(opaque->bo_rmAccess, heapBlk, &buf, &off,
                                   &size, BUFFER_LOCK_SHARE);
    if (tup) {
      ...
      if (dtup->bt_placeholder) {
        ...
        for (keyno = 0; keyno < scan->numberOfKeys; keyno++) {
          ...
          add = FunctionCall3Coll(&consistentFn[keyattno - 1],
                                  key->sk_collation, PointerGetDatum(bdesc),
                                  PointerGetDatum(bval), PointerGetDatum(key));
    ...
    if (addrange) {
      ...
      for (pageno = heapBlk; pageno <= heapBlk + opaque->bo_pagesPerRange - 1;
           pageno++) {
        MemoryContextSwitchTo(oldcxt);
  ...
  if (buf != InvalidBuffer) ReleaseBuffer(buf);
 ...
 */
Datum brinrescan(PG_FUNCTION_ARGS) {
  IndexScanDesc scan = (IndexScanDesc)PG_GETARG_POINTER(0);
 ...
 */
Datum brinendscan(PG_FUNCTION_ARGS) {
  IndexScanDesc scan = (IndexScanDesc)PG_GETARG_POINTER(0);
...
Datum brinmarkpos(PG_FUNCTION_ARGS) {
  elog(ERROR, "BRIN does not support mark/restore");
  PG_RETURN_VOID();
}

Datum brinrestrpos(PG_FUNCTION_ARGS) {
  elog(ERROR, "BRIN does not support mark/restore");
  PG_RETURN_VOID();
}
 ...
 */
static void brinbuildCallback(Relation index, HeapTuple htup, Datum *values,
                              bool *isnull, bool tupleIsAlive, void *brstate) {
  ...
  for (i = 0; i < state->bs_bdesc->bd_tupdesc->natts; i++) {
    ...
    FunctionCall4Coll(addValue,
                      ...
                      PointerGetDatum(state->bs_bdesc), PointerGetDatum(col),
                      values[i], isnull[i]);
  }
}
 ...
 */
Datum brinbuild(PG_FUNCTION_ARGS) {
...
Datum brinbuildempty(PG_FUNCTION_ARGS) {
 ...
 */
Datum brinbulkdelete(PG_FUNCTION_ARGS) {
 ...
 */
Datum brinvacuumcleanup(PG_FUNCTION_ARGS) {
  IndexVacuumInfo *info = (IndexVacuumInfo *)PG_GETARG_POINTER(0);
  IndexBulkDeleteResult *stats = (IndexBulkDeleteResult *)PG_GETARG_POINTER(1);
  Relation heapRel;

  /* No-op in ANALYZE ONLY mode */
  if (info->analyze_only) PG_RETURN_POINTER(stats);
  ...
  brinsummarize(info->index, heapRel, &stats->num_index_tuples,
 ...
 */
Datum brinoptions(PG_FUNCTION_ARGS) {
  ...
  static const relopt_parse_elt tab[] = {
      {"pages_per_range", RELOPT_TYPE_INT,
       offsetof(BrinOptions, pagesPerRange)}};

  options =
      parseRelOptions(reloptions, validate, RELOPT_KIND_BRIN, &numoptions);

  /* if none set, we're done */
  if (numoptions == 0) PG_RETURN_NULL();

  rdopts = static_cast<BrinOptions *>(
...
Datum brin_summarize_new_values(PG_FUNCTION_ARGS) {
  ...
  heapRel =
      heap_open(IndexGetRelation(indexoid, false), ShareUpdateExclusiveLock);
...
BrinDesc *brin_build_desc(Relation rel) {
  ...
  cxt = AllocSetContextCreate(CurrentMemoryContext, "brin desc cxt",
  ...
  for (keyno = 0; keyno < tupdesc->natts; keyno++) {
    ...
    opcinfo[keyno] = (BrinOpcInfo *)DatumGetPointer(
        FunctionCall1(opcInfoFn, tupdesc->attrs[keyno]->atttypid));
    totalstored += opcinfo[keyno]->oi_nstored;
  }

  /* Allocate our result struct and fill it in */
  totalsize =
      offsetof(BrinDesc, bd_info) + sizeof(BrinOpcInfo *) * tupdesc->natts;
...
static void summarize_range(IndexInfo *indexInfo, BrinBuildState *state,
                            Relation heapRel, BlockNumber heapBlk) {
  Buffer phbuf;
  BrinTuple *phtup;
  Size phsz;
  OffsetNumber offset;
  ...
  offset = brin_doinsert(state->bs_irel, state->bs_pagesPerRange,
                         state->bs_rmAccess, &phbuf, heapBlk, phtup, phsz);
  ...
  IndexBuildHeapRangeScan(heapRel, state->bs_irel, indexInfo, false, heapBlk,
                          state->bs_pagesPerRange, brinbuildCallback,
                          (void *)state);
  ...
  for (;;) {
     ...
     */
    newtup =
        brin_form_tuple(state->bs_bdesc, heapBlk, state->bs_dtuple, &newsize);
    samepage = brin_can_do_samepage_update(phbuf, phsz, newsize);
    didupdate = brin_doupdate(state->bs_irel, state->bs_pagesPerRange,
                              state->bs_rmAccess, heapBlk, phbuf, offset, phtup,
                              phsz, newtup, newsize, samepage);
...
static void brinsummarize(Relation index, Relation heapRel,
  ...
  for (heapBlk = 0; heapBlk < heapNumBlocks; heapBlk += pagesPerRange) {
    ...
    if (tup == NULL) {
        ...
        if (IsolationUsesXactSnapshot() && FirstSnapshotSet)
          ereport(ERROR,
                  (errcode(ERRCODE_INVALID_TRANSACTION_STATE),
                   errmsg(
                       "brin_summarize_new_values() cannot run in a "
                       "transaction that has already obtained a snapshot")));
      }
      ...
      if (numSummarized) *numSummarized += 1.0;
    } else {
      if (numExisting) *numExisting += 1.0;
      LockBuffer(buf, BUFFER_LOCK_UNLOCK);
    }
  }
...
static void form_and_insert_tuple(BrinBuildState *state) {
  ...
  brin_doinsert(state->bs_irel, state->bs_pagesPerRange, state->bs_rmAccess,
                &state->bs_currentInsertBuf, state->bs_currRangeStart, tup,
                size);
  state->bs_numtuples++;
...
static void union_tuples(BrinDesc *bdesc, BrinMemTuple *a, BrinTuple *b) {
  ...
  cxt = AllocSetContextCreate(
      CurrentMemoryContext, "brin union", ALLOCSET_DEFAULT_MINSIZE,
      ALLOCSET_DEFAULT_INITSIZE, ALLOCSET_DEFAULT_MAXSIZE);
  oldcxt = MemoryContextSwitchTo(cxt);
  ...
  for (keyno = 0; keyno < bdesc->bd_tupdesc->natts; keyno++) {
    ...
    FunctionCall3Coll(unionFn, bdesc->bd_index->rd_indcollation[keyno],
                      PointerGetDatum(bdesc), PointerGetDatum(col_a),
