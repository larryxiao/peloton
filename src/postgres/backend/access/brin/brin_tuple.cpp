...
static inline void brin_deconstruct_tuple(BrinDesc *brdesc, char *tp,
                                          bits8 *nullbits, bool nulls,
                                          Datum *values, bool *allnulls,
                                          bool *hasnulls);
    ...
    for (i = 0; i < brdesc->bd_tupdesc->natts; i++) {
      for (j = 0; j < brdesc->bd_info[i]->oi_nstored; j++)
        TupleDescInitEntry(tupdesc, attno++, NULL,
                           brdesc->bd_info[i]->oi_typcache[j]->type_id, -1, 0);
    }
...
BrinTuple *brin_form_tuple(BrinDesc *brdesc, BlockNumber blkno,
  ...
  phony_nullbitmap = static_cast<bits8 *>(
  ...
  for (keyno = 0; keyno < brdesc->bd_tupdesc->natts; keyno++) {
     ...
     */
    if (tuple->bt_columns[keyno].bv_hasnulls) anynulls = true;
  ...
  data_len =
      heap_compute_data_size(brtuple_disk_tupdesc(brdesc), values, nulls);
  ...
  heap_fill_tuple(brtuple_disk_tupdesc(brdesc), values, nulls,
                  (char *)rettuple + hoff, data_len, &phony_infomask,
                  phony_nullbitmap);
  ...
  if (anynulls) {
    ...
    for (keyno = 0; keyno < brdesc->bd_tupdesc->natts; keyno++) {
      ...
      if (!tuple->bt_columns[keyno].bv_allnulls) continue;
    ...
    for (keyno = 0; keyno < brdesc->bd_tupdesc->natts; keyno++) {
      ...
      if (!tuple->bt_columns[keyno].bv_hasnulls) continue;
  ...
  if (tuple->bt_placeholder) rettuple->bt_info |= BRIN_PLACEHOLDER_MASK;
 ...
 */
BrinTuple *brin_form_placeholder_tuple(BrinDesc *brdesc, BlockNumber blkno,
                                       Size *size) {
  Size len;
  Size hoff;
  BrinTuple *rettuple;
 ...
 */
void brin_free_tuple(BrinTuple *tuple) { pfree(tuple); }
 ...
 */
bool brin_tuples_equal(const BrinTuple *a, Size alen, const BrinTuple *b,
                       Size blen) {
...
BrinMemTuple *brin_new_memtuple(BrinDesc *brdesc) {
  ...
  dtup = static_cast<BrinMemTuple *>(
      palloc0(basesize + sizeof(Datum) * brdesc->bd_totalstored));
  ...
  dtup->bt_context = AllocSetContextCreate(
      CurrentMemoryContext, "brin dtuple", ALLOCSET_DEFAULT_MINSIZE,
...
BrinMemTuple *brin_deform_tuple(BrinDesc *brdesc, BrinTuple *tuple) {
  ...
  if (BrinTupleIsPlaceholder(tuple)) dtup->bt_placeholder = true;
  dtup->bt_blkno = tuple->bt_blkno;

  values = static_cast<Datum *>(palloc(sizeof(Datum) * brdesc->bd_totalstored));
  allnulls =
      static_cast<bool *>(palloc(sizeof(bool) * brdesc->bd_tupdesc->natts));
  hasnulls =
      static_cast<bool *>(palloc(sizeof(bool) * brdesc->bd_tupdesc->natts));
  ...
  if (BrinTupleHasNulls(tuple))
    ...
    nullbits = NULL;
  brin_deconstruct_tuple(brdesc, tp, nullbits, BrinTupleHasNulls(tuple), values,
                         allnulls, hasnulls);
  ...
  for (valueno = 0, keyno = 0; keyno < brdesc->bd_tupdesc->natts; keyno++) {
    ...
    for (i = 0; i < brdesc->bd_info[keyno]->oi_nstored; i++)
      dtup->bt_columns[keyno].bv_values[i] = datumCopy(
          values[valueno++], brdesc->bd_info[keyno]->oi_typcache[i]->typbyval,
...
static inline void brin_deconstruct_tuple(BrinDesc *brdesc, char *tp,
  ...
  for (attnum = 0; attnum < brdesc->bd_tupdesc->natts; attnum++) {
    ...
    for (datumno = 0; datumno < brdesc->bd_info[attnum]->oi_nstored;
         datumno++) {
