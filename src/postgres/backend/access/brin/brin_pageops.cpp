 ...
 * the new___ tuple.  If the update is not done for whatever reason, return
 *false.
 * Caller may retry the update if this happens.
 */
bool brin_doupdate(Relation idxrel, BlockNumber pagesPerRange,
                   BrinRevmap *revmap, BlockNumber heapBlk, Buffer oldbuf,
                   OffsetNumber oldoff, const BrinTuple *origtup, Size origsz,
                   const BrinTuple *newtup, Size newsz, bool samepage) {
  Page oldpage;
  ItemId oldlp;
  BrinTuple *oldtup;
  Size oldsz;
  ...
  if (!samepage) {
    ...
    if (extended) FreeSpaceMapVacuum(idxrel);
    if (!BufferIsValid(newbuf)) return false;
     ...
     */
    if (newbuf == oldbuf) newbuf = InvalidBuffer;
  } else {
  ...
  if (!ItemIdIsNormal(oldlp)) {
    ...
    if (BufferIsValid(newbuf)) UnlockReleaseBuffer(newbuf);
    return false;
  }
  ...
  if (((BrinPageFlags(oldpage) & BRIN_EVACUATE_PAGE) == 0) &&
    ...
    if (PageAddItem(oldpage, (Item)newtup, newsz, oldoff, true, false) ==
        InvalidOffsetNumber)
    ...
    PageIndexDeleteNoCompact(oldpage, &oldoff, 1);
    newoff = PageAddItem(newpage, (Item)newtup, newsz, InvalidOffsetNumber,
                         false, false);
    ...
    if (RelationNeedsWAL(idxrel)) {
      ...
      XLogRegisterBuffer(0, newbuf,
                         REGBUF_STANDARD | (extended ? REGBUF_WILL_INIT : 0));
      XLogRegisterBufData(0, (char *)newtup, newsz);
...
OffsetNumber brin_doinsert(Relation idxrel, BlockNumber pagesPerRange,
                           BrinRevmap *revmap, Buffer *buffer,
                           BlockNumber heapBlk, BrinTuple *tup, Size itemsz) {
  Page page;
  BlockNumber blk;
  OffsetNumber off;
  Buffer revmapbuf;
  ...
  BRIN_elog((DEBUG2, "inserted tuple (%u,%u) for range starting at %u", blk,
             off, heapBlk));
...
void brin_evacuate_page(Relation idxRel, BlockNumber pagesPerRange,
  ...
  for (off = FirstOffsetNumber; off <= maxoff; off++) {
    ...
    if (ItemIdIsUsed(lp)) {
      ...
      if (!brin_doupdate(idxRel, pagesPerRange, revmap, tup->bt_blkno, buf, off,
                         tup, sz, tup, sz, false))
        off--; /* retry */
      ...
      if (!BRIN_IS_REGULAR_PAGE(page)) break;
    }
  }
...
static Buffer brin_getinsertbuffer(Relation irel, Buffer oldbuf, Size itemsz,
  ...
  if (newblk == InvalidBlockNumber) newblk = GetPageWithFreeSpace(irel, itemsz);
  for (;;) {
    Buffer buf;
    ...
    if (extensionLockHeld) UnlockRelationForExtension(irel, ExclusiveLock);

    page = BufferGetPage(buf);

    if (extended) brin_page_init(page, BRIN_PAGETYPE_REGULAR);
    ...
    if (freespace >= itemsz) {
       ...
       */
      if (extended)
        RecordPageWithFreeSpace(irel, BufferGetBlockNumber(buf), freespace);
      ...
      ereport(ERROR,
              (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
               errmsg("index row size %lu exceeds maximum %lu for index \"%s\"",
                      (unsigned long)itemsz, (unsigned long)freespace,
                      RelationGetRelationName(irel))));
      return InvalidBuffer; /* keep compiler quiet */
    }

    if (newblk != oldblk) UnlockReleaseBuffer(buf);
    if (BufferIsValid(oldbuf) && oldblk <= newblk)
      LockBuffer(oldbuf, BUFFER_LOCK_UNLOCK);
