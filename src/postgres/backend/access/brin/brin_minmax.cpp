...
Datum brin_minmax_opcinfo(PG_FUNCTION_ARGS) {
  ...
  result = static_cast<BrinOpcInfo *>(
      palloc0(MAXALIGN(SizeofBrinOpcInfo(2)) + sizeof(MinmaxOpaque)));
  result->oi_nstored = 2;
  result->oi_opaque =
      (MinmaxOpaque *)MAXALIGN((char *)result + SizeofBrinOpcInfo(2));
...
Datum brin_minmax_add_value(PG_FUNCTION_ARGS) {
  ...
  if (DatumGetBool(compar)) {
    if (!attr->attbyval) pfree(DatumGetPointer(column->bv_values[1]));
    column->bv_values[1] = datumCopy(newval, attr->attbyval, attr->attlen);
    updated = true;
  }
...
Datum brin_minmax_consistent(PG_FUNCTION_ARGS) {
  ...
  switch (key->sk_strategy) {
    ...
    case BTLessEqualStrategyNumber:
      finfo =
          minmax_get_strategy_procinfo(bdesc, attno, subtype, key->sk_strategy);
...
Datum brin_minmax_union(PG_FUNCTION_ARGS) {
  ...
  if (col_a->bv_allnulls) {
    ...
    col_a->bv_values[0] =
        datumCopy(col_b->bv_values[0], attr->attbyval, attr->attlen);
    col_a->bv_values[1] =
        datumCopy(col_b->bv_values[1], attr->attbyval, attr->attlen);
    PG_RETURN_VOID();
  }
  ...
  needsadj = FunctionCall2Coll(finfo, colloid, col_b->bv_values[1],
                               col_a->bv_values[1]);
  if (needsadj) {
    if (!attr->attbyval) pfree(DatumGetPointer(col_a->bv_values[1]));
    col_a->bv_values[1] =
        datumCopy(col_b->bv_values[1], attr->attbyval, attr->attlen);
  }
