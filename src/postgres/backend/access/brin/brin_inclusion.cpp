...
Datum brin_inclusion_opcinfo(PG_FUNCTION_ARGS) {
   ...
   * to look it up again.
   */
  result = static_cast<BrinOpcInfo *>(
      palloc0(MAXALIGN(SizeofBrinOpcInfo(3)) + sizeof(InclusionOpaque)));
  result->oi_nstored = 3;
  result->oi_opaque =
      (InclusionOpaque *)MAXALIGN((char *)result + SizeofBrinOpcInfo(3));
...
Datum brin_inclusion_add_value(PG_FUNCTION_ARGS) {
  ...
  if (finfo != NULL &&
      DatumGetBool(FunctionCall1Coll(finfo, colloid, newval))) {
  ...
  if (finfo != NULL &&
      DatumGetBool(FunctionCall2Coll(
          finfo, colloid, column->bv_values[INCLUSION_UNION], newval)))
    PG_RETURN_BOOL(false);
  ...
  Assert(finfo != NULL);
  result = FunctionCall2Coll(finfo, colloid, column->bv_values[INCLUSION_UNION],
                             newval);
  if (!attr->attbyval)
...
Datum brin_inclusion_consistent(PG_FUNCTION_ARGS) {
  ...
  if (key->sk_flags & SK_ISNULL) {
    if (key->sk_flags & SK_SEARCHNULL) {
      if (column->bv_allnulls || column->bv_hasnulls) PG_RETURN_BOOL(true);
      PG_RETURN_BOOL(false);
    }
  ...
  if (column->bv_allnulls) PG_RETURN_BOOL(false);
  ...
  switch (key->sk_strategy) {
     ...
     * converse placement operator___; for this to work, the converse
     *operator___
    ...
    case RTSuperEqualStrategyNumber:
      ...
      result = FunctionCall2Coll(finfo, colloid, unionval, query);
      if (DatumGetBool(result)) PG_RETURN_BOOL(true);
...
Datum brin_inclusion_union(PG_FUNCTION_ARGS) {
  ...
  /* Adjust "hasnulls". */
  if (!col_a->bv_hasnulls && col_b->bv_hasnulls) col_a->bv_hasnulls = true;

  /* If there are no values in B, there's nothing left to do. */
  if (col_b->bv_allnulls) PG_RETURN_VOID();
  ...
  if (col_a->bv_allnulls) {
    ...
    col_a->bv_values[INCLUSION_UNION] = datumCopy(
        col_b->bv_values[INCLUSION_UNION], attr->attbyval, attr->attlen);
  ...
  if (DatumGetBool(col_a->bv_values[INCLUSION_UNMERGEABLE])) PG_RETURN_VOID();
  ...
  finfo = inclusion_get_procinfo(bdesc, attno, PROCNUM_MERGE);
  Assert(finfo != NULL);
  result = FunctionCall2Coll(finfo, colloid, col_a->bv_values[INCLUSION_UNION],
                             col_b->bv_values[INCLUSION_UNION]);
  if (!attr->attbyval)
...
static FmgrInfo *inclusion_get_procinfo(BrinDesc *bdesc, uint16 attno,
   ...
   * searching again.
   */
  if (opaque->extra_proc_missing[basenum]) return NULL;
  ...
  if (opaque->strategy_procinfos[strategynum - 1].fn_oid == InvalidOid) {
    ...
    tuple =
        SearchSysCache4(AMOPSTRATEGY, ObjectIdGetDatum(opfamily),
                        ...
                        ObjectIdGetDatum(subtype), Int16GetDatum(strategynum));

    if (!HeapTupleIsValid(tuple))
      elog(ERROR, "missing operator___ %d(%u,%u) in opfamily %u", strategynum,
           attr->atttypid, subtype, opfamily);
