...
static void reform_and_rewrite_tuple(HeapTuple tuple, TupleDesc oldTupDesc,
                                     TupleDesc newTupDesc, Datum *values,
                                     bool *isnull, bool newRelHasOids,
...
void cluster(ClusterStmt *stmt, bool isTopLevel) {
  if (stmt->relation != NULL) {
     ...
     * of error.
     */
    cluster_context = AllocSetContextCreate(
        PortalContext, "Cluster", ALLOCSET_DEFAULT_MINSIZE,
 ...
 * protection here.
 */
void check_index_is_clusterable(Relation OldHeap, Oid indexOid, bool recheck,
                                LOCKMODE lockmode) {
  Relation OldIndex;
...
static void rebuild_relation(Relation OldHeap, Oid indexOid, bool verbose) {
  ...
  if (OidIsValid(indexOid)) mark_index_clustered(OldHeap, indexOid, true);
  ...
  OIDNewHeap =
      make_new_heap(tableOid, tableSpace, OldHeap->rd_rel->relpersistence,
                    AccessExclusiveLock);
 ...
 */
Oid make_new_heap(Oid OIDOldHeap, Oid NewTableSpace, char relpersistence,
                  LOCKMODE lockmode) {
   ...
   * particular, we can't create the new___ heap in a different namescpace___
   *from
  ...
  OIDNewHeap = heap_create_with_catalog(
      NewHeapName, namespaceid, NewTableSpace, InvalidOid, InvalidOid,
      InvalidOid, OldHeap->rd_rel->relowner, OldHeapDesc, NIL, RELKIND_RELATION,
      relpersistence, false, RelationIsMapped(OldHeap), true, 0, ONCOMMIT_NOOP,
      reloptions, false, true, true, NULL);
...
static void copy_heap_data(Oid OIDNewHeap, Oid OIDOldHeap, Oid OIDOldIndex,
   ...
   */
  vacuum_set_xid_limits(OldHeap, 0, 0, 0, 0, &OldestXmin, &FreezeXid, NULL,
  ...
  *pCutoffMulti = MultiXactCutoff;
  ...
  for (;;) {
    ...
    if (tuplesort != NULL)
      ...
      reform_and_rewrite_tuple(tuple, oldTupDesc, newTupDesc, values, isnull,
                               NewHeap->rd_rel->relhasoids, rwstate);
  }

  if (indexScan != NULL) index_endscan(indexScan);
  if (heapScan != NULL) heap_endscan(heapScan);
  ...
  ereport(elevel, (errmsg(
                       ...
                       "versions in %u pages",
                       RelationGetRelationName(OldHeap), tups_vacuumed,
                       num_tuples, RelationGetNumberOfBlocks(OldHeap)),
                   errdetail(
  ...
  if (OldIndex != NULL) index_close(OldIndex, NoLock);
  heap_close(OldHeap, NoLock);
...
static void swap_relation_files(Oid r1, Oid r2, bool target_is_pg_class,
                                ...
                                TransactionId frozenXid,
                                MultiXactId cutoffMulti, Oid *mapped_tables) {
   ...
   */
  InvokeObjectPostAlterHookArg(RelationRelationId, r1, 0, InvalidOid,
                               is_internal);
  ...
  if (swap_toast_by_content && relform1->relkind == RELKIND_TOASTVALUE &&
    ...
    swap_relation_files(
        toastIndex1, toastIndex2, target_is_pg_class, swap_toast_by_content,
        is_internal, InvalidTransactionId, InvalidMultiXactId, mapped_tables);
  }
 ...
 */
void finish_heap_swap(Oid OIDOldHeap, Oid OIDNewHeap, bool is_system_catalog,
                      bool swap_toast_by_content, bool check_constraints,
                      bool is_internal, TransactionId frozenXid,
  ...
  swap_relation_files(OIDOldHeap, OIDNewHeap,
                      (OIDOldHeap == RelationRelationId), swap_toast_by_content,
                      is_internal, frozenXid, cutoffMulti, mapped_tables);
   ...
   */
  if (is_system_catalog) CacheInvalidateCatalog(OIDOldHeap);
  ...
  if (check_constraints) reindex_flags |= REINDEX_REL_CHECK_CONSTRAINTS;
  ...
  if (!swap_toast_by_content) {
    ...
    if (OidIsValid(newrel->rd_rel->reltoastrelid)) {
      ...
      toastidx =
          toast_get_valid_index(newrel->rd_rel->reltoastrelid, AccessShareLock);
...
static List *get_tables_to_cluster(MemoryContext cluster_context) {
  ...
  while ((indexTuple = heap_getnext(scan, ForwardScanDirection)) != NULL) {
    ...
    if (!pg_class_ownercheck(index->indrelid, GetUserId())) continue;
...
static void reform_and_rewrite_tuple(HeapTuple tuple, TupleDesc oldTupDesc,
  ...
  for (i = 0; i < newTupDesc->natts; i++) {
    if (newTupDesc->attrs[i]->attisdropped) isnull[i] = true;
  }

  copiedTuple = heap_form_tuple(newTupDesc, values, isnull);

  /* Preserve OID, if any */
  if (newRelHasOids) HeapTupleSetOid(copiedTuple, HeapTupleGetOid(tuple));
