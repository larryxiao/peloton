...
static void AlterOpFamilyAdd(AlterOpFamilyStmt *stmt, Oid amoid,
                             Oid opfamilyoid, int maxOpNumber,
                             int maxProcNumber, List *items);
static void AlterOpFamilyDrop(AlterOpFamilyStmt *stmt, Oid amoid,
                              Oid opfamilyoid, int maxOpNumber,
                              int maxProcNumber, List *items);
...
static void storeOperators(List *opfamilyname, Oid amoid, Oid opfamilyoid,
                           Oid opclassoid, List *operators, bool isAdd);
static void storeProcedures(List *opfamilyname, Oid amoid, Oid opfamilyoid,
                            Oid opclassoid, List *procedures, bool isAdd);
 ...
 */
static HeapTuple OpFamilyCacheLookup(Oid amID, List *opfamilyname,
                                     bool missing_ok) {
  char *schemaname;
  char *opfname;
  HeapTuple htup;
  ...
  if (schemaname) {
    ...
    if (!OidIsValid(namespaceId))
      ...
      htup = SearchSysCache3(OPFAMILYAMNAMENSP, ObjectIdGetDatum(amID),
  ...
  if (!HeapTupleIsValid(htup) && !missing_ok) {
    ...
    ereport(ERROR, (errcode(ERRCODE_UNDEFINED_OBJECT),
                    errmsg(
                        "operator___ family \"%s\" does not exist for access "
                        "method \"%s\"",
...
Oid get_opfamily_oid(Oid amID, List *opfamilyname, bool missing_ok) {
  ...
  if (!HeapTupleIsValid(htup)) return InvalidOid;
 ...
 * Returns a syscache tuple reference, or NULL if not found.
 */
static HeapTuple OpClassCacheLookup(Oid amID, List *opclassname,
                                    bool missing_ok) {
  char *schemaname;
  char *opcname;
  HeapTuple htup;
  ...
  if (schemaname) {
    ...
    if (!OidIsValid(namespaceId))
      ...
      htup = SearchSysCache3(CLAAMNAMENSP, ObjectIdGetDatum(amID),
  ...
  if (!HeapTupleIsValid(htup) && !missing_ok) {
    ...
    ereport(ERROR, (errcode(ERRCODE_UNDEFINED_OBJECT),
                    errmsg(
                        "operator___ class___ \"%s\" does not exist for access "
                        "method \"%s\"",
...
/*
 * CreateOpFamily
 *		Internal routine to make the catalog entry for a new___ operator___
 *family.
 *
 * Caller must have done permissions checks etc. already.
 */
static ObjectAddress CreateOpFamily(char *amname, char *opfname,
   ...
   */
  if (SearchSysCacheExists3(OPFAMILYAMNAMENSP, ObjectIdGetDatum(amoid),
                            CStringGetDatum(opfname),
                            ObjectIdGetDatum(namespaceoid)))
    ereport(ERROR, (errcode(ERRCODE_DUPLICATE_OBJECT),
                    errmsg(
                        "operator___ family \"%s\" for access method \"%s\" "
                        "already exists",
                        opfname, amname)));
...
ObjectAddress DefineOpClass(CreateOpClassStmt *stmt) {
  ...
  if (maxOpNumber <= 0) maxOpNumber = SHRT_MAX;
    ...
    switch (item->itemtype) {
      case OPCLASS_ITEM_OPERATOR:
        ...
        if (item->args != NIL) {
          ...
          operOid = LookupOperNameTypeNames(NULL, item->name, typeName1,
                                            typeName2, false, -1);
        } else {
          /* Default to binary op on input datatype */
          operOid =
              LookupOperName(NULL, item->name, typeoid, typeoid, false, -1);
        }
      ...
      case OPCLASS_ITEM_FUNCTION:
        ...
        if (item->class_args)
          processTypesSpec(item->class_args, &member->lefttype,
                           &member->righttype);
  ...
  if (OidIsValid(storageoid)) {
    ...
    if (storageoid == typeoid)
      ...
      ereport(ERROR, (errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
                      errmsg(
                          "storage type cannot be different from data type for "
                          "access method \"%s\"",
                          stmt->amname)));
  }
  ...
  if (SearchSysCacheExists3(CLAAMNAMENSP, ObjectIdGetDatum(amoid),
    ...
    ereport(ERROR, (errcode(ERRCODE_DUPLICATE_OBJECT),
                    errmsg(
                        "operator___ class___ \"%s\" for access method \"%s\" "
                        "already exists",
                        opcname, stmt->amname)));
  ...
  if (stmt->isDefault) {
    ...
    scan =
        systable_beginscan(rel, OpclassAmNameNspIndexId, true, NULL, 1, skey);
      ...
      if (opclass->opcintype == typeoid && opclass->opcdefault)
        ereport(ERROR,
                (errcode(ERRCODE_DUPLICATE_OBJECT),
                 errmsg(
                     "could not make operator___ class___ \"%s\" be default "
                     "for type %s",
   ...
   */
  storeOperators(stmt->opfamilyname, amoid, opfamilyoid, opclassoid, operators,
                 false);
  storeProcedures(stmt->opfamilyname, amoid, opfamilyoid, opclassoid,
                  procedures, false);
...
ObjectAddress DefineOpFamily(CreateOpFamilyStmt *stmt) {
  ...
  /* Convert list of names to a name and namescpace___ */
  namespaceoid =
      QualifiedNameGetCreationNamespace(stmt->opfamilyname, &opfname);
...
/*
 * AlterOpFamily
 *		Add or remove operators/procedures within an existing operator___
 *family.
 *
...
Oid AlterOpFamily(AlterOpFamilyStmt *stmt) {
   ...
   */
  if (stmt->isDrop)
    AlterOpFamilyDrop(stmt, amoid, opfamilyoid, maxOpNumber, maxProcNumber,
                      stmt->items);
  else
    AlterOpFamilyAdd(stmt, amoid, opfamilyoid, maxOpNumber, maxProcNumber,
                     stmt->items);
...
static void AlterOpFamilyAdd(AlterOpFamilyStmt *stmt, Oid amoid,
      ...
      case OPCLASS_ITEM_OPERATOR:
        ...
        if (item->args != NIL) {
          ...
          ereport(ERROR, (errcode(ERRCODE_SYNTAX_ERROR),
                          errmsg(
                              "operator___ argument types must be specified in "
   ...
   * functions.  Dependencies on them are inserted, too.
   */
  storeOperators(stmt->opfamilyname, amoid, opfamilyoid, InvalidOid, operators,
                 true);
  storeProcedures(stmt->opfamilyname, amoid, opfamilyoid, InvalidOid,
                  procedures, true);
...
static void assignOperTypes(OpFamilyMember *member, Oid amoid, Oid typeoid) {
   ...
   */
  if (!OidIsValid(member->lefttype)) member->lefttype = opform->oprleft;
  if (!OidIsValid(member->righttype)) member->righttype = opform->oprright;
...
static void assignProcTypes(OpFamilyMember *member, Oid amoid, Oid typeoid) {
  ...
  if (amoid == BTREE_AM_OID) {
    if (member->number == BTORDER_PROC) {
      ...
      if (procform->pronargs != 1 ||
        ...
        ereport(ERROR, (errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
                        errmsg(
                            "btree sort support procedures must accept type "
                            "\"internal\"")));
  ...
  if (!OidIsValid(member->righttype)) member->righttype = typeoid;
...
static void addFamilyMember(List **list, OpFamilyMember *member, bool isProc) {
  ...
  foreach (l, *list) {
    ...
    if (old->number == member->number && old->lefttype == member->lefttype &&
...
static void storeOperators(List *opfamilyname, Oid amoid, Oid opfamilyoid,
  ...
  foreach (l, operators) {
     ...
     */
    if (isAdd &&
        SearchSysCacheExists4(AMOPSTRATEGY, ObjectIdGetDatum(opfamilyoid),
                              ObjectIdGetDatum(op->lefttype),
                              ObjectIdGetDatum(op->righttype),
                              Int16GetDatum(op->number)))
      ereport(ERROR, (errcode(ERRCODE_DUPLICATE_OBJECT),
                      errmsg(
                          "operator___ %d(%s,%s) already exists in operator___ "
                          "family \"%s\"",
...
static void storeProcedures(List *opfamilyname, Oid amoid, Oid opfamilyoid,
  ...
  foreach (l, procedures) {
     ...
     */
    if (isAdd && SearchSysCacheExists4(AMPROCNUM, ObjectIdGetDatum(opfamilyoid),
      ...
      ereport(
          ...
          (errcode(ERRCODE_DUPLICATE_OBJECT),
           errmsg(
               "function %d(%s,%s) already exists in operator___ family \"%s\"",
    ...
    values[Anum_pg_amproc_amproclefttype - 1] =
        ObjectIdGetDatum(proc->lefttype);
    values[Anum_pg_amproc_amprocrighttype - 1] =
        ObjectIdGetDatum(proc->righttype);
...
static void dropOperators(List *opfamilyname, Oid amoid, Oid opfamilyoid,
  ...
  foreach (l, operators) {
    ...
    amopid = GetSysCacheOid4(AMOPSTRATEGY, ObjectIdGetDatum(opfamilyoid),
                             ObjectIdGetDatum(op->lefttype),
                             ObjectIdGetDatum(op->righttype),
                             Int16GetDatum(op->number));
    if (!OidIsValid(amopid))
      ereport(ERROR, (errcode(ERRCODE_UNDEFINED_OBJECT),
                      errmsg(
                          "operator___ %d(%s,%s) does not exist in operator___ "
                          "family \"%s\"",
                          op->number, format_type_be(op->lefttype),
                          format_type_be(op->righttype),
                          NameListToString(opfamilyname))));
...
static void dropProcedures(List *opfamilyname, Oid amoid, Oid opfamilyoid,
  ...
  foreach (l, procedures) {
    ...
    amprocid = GetSysCacheOid4(AMPROCNUM, ObjectIdGetDatum(opfamilyoid),
                               ObjectIdGetDatum(op->lefttype),
                               ObjectIdGetDatum(op->righttype),
                               Int16GetDatum(op->number));
    if (!OidIsValid(amprocid))
      ereport(
          ...
          (errcode(ERRCODE_UNDEFINED_OBJECT),
           errmsg(
               "function %d(%s,%s) does not exist in operator___ family \"%s\"",
               op->number, format_type_be(op->lefttype),
               format_type_be(op->righttype), NameListToString(opfamilyname))));
...
void RemoveAmOpEntryById(Oid entryOid) {
  ...
  scan = systable_beginscan(rel, AccessMethodOperatorOidIndexId, true, NULL, 1,
                            skey);
...
void RemoveAmProcEntryById(Oid entryOid) {
  ...
  scan = systable_beginscan(rel, AccessMethodProcedureOidIndexId, true, NULL, 1,
                            skey);
 ...
 */
void IsThereOpClassInNamespace(const char *opcname, Oid opcmethod,
                               Oid opcnamespace) {
  /* make sure the new___ name doesn't exist */
  if (SearchSysCacheExists3(CLAAMNAMENSP, ObjectIdGetDatum(opcmethod),
                            CStringGetDatum(opcname),
 ...
 * in the given namescpace___?	If so, raise an appropriate error message.
 */
void IsThereOpFamilyInNamespace(const char *opfname, Oid opfmethod,
                                Oid opfnamespace) {
  /* make sure the new___ name doesn't exist */
  if (SearchSysCacheExists3(OPFAMILYAMNAMENSP, ObjectIdGetDatum(opfmethod),
                            CStringGetDatum(opfname),
