...
Oid CreateSchemaCommand(CreateSchemaStmt *stmt, const char *queryString) {
  ...
  if (stmt->if_not_exists &&
    ...
    ereport(NOTICE,
            (errcode(ERRCODE_DUPLICATE_SCHEMA),
             errmsg("schema \"%s\" already exists, skipping", schemaName)));
    return InvalidOid;
  }
  ...
  /*
   * Temporarily make the new___ namescpace___ be the front of the search path,
   * as
  ...
  foreach (parsetree_item, parsetree_list) {
    ...
    /* do this step */
    ProcessUtility(stmt, queryString, PROCESS_UTILITY_SUBCOMMAND, NULL,
                   None_Receiver, NULL);
...
static void AlterSchemaOwner_internal(HeapTuple tup, Relation rel,
  ...
  if (nspForm->nspowner != newOwnerId) {
     ...
     */
    aclDatum =
        SysCacheGetAttr(NAMESPACENAME, tup, Anum_pg_namespace_nspacl, &isNull);
    if (!isNull) {
      newAcl =
          aclnewowner(DatumGetAclP(aclDatum), nspForm->nspowner, newOwnerId);
