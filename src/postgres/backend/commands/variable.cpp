...
bool check_datestyle(char **newval, void **extra, GucSource source) {
  ...
  foreach (l, elemlist) {
    ...
    if (pg_strcasecmp(tok, "ISO") == 0) {
      ...
      if (!have_style) newDateStyle = myextra[0];
      if (!have_order) newDateOrder = myextra[1];
      free(subval);
      free(subextra);
    } else {
  ...
  if (pg_strncasecmp(*newval, "interval", 8) == 0) {
    ...
    *endptr = '\0';
    ...
    interval = DatumGetIntervalP(
        DirectFunctionCall3(interval_in, CStringGetDatum(val),
...
bool check_transaction_deferrable(bool *newval, void **extra,
                                  GucSource source) {
  if (IsSubTransaction()) {
    GUC_check_errcode(ERRCODE_ACTIVE_SQL_TRANSACTION);
    GUC_check_errmsg(
...
void assign_random_seed(double newval, void *extra) {
  ...
  if (*((int *)extra)) DirectFunctionCall1(setseed, Float8GetDatum(newval));
  *((int *)extra) = 0;
}

const char *show_random_seed(void) { return "unavailable"; }
...
bool check_session_authorization(char **newval, void **extra,
                                 GucSource source) {
  HeapTuple roleTup;
  Oid roleid;
  bool is_superuser;
