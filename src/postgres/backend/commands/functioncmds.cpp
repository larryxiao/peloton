 ...
 * condition.)
 */
static void compute_return_type(TypeName *returnType, Oid languageOid,
  ...
  if (typtup) {
    ...
    namespaceId =
        QualifiedNameGetCreationNamespace(returnType->names, &typname);
  ...
  if (aclresult != ACLCHECK_OK) aclcheck_error_type(aclresult, rettype);
 ...
 */
void interpret_function_parameter_list(
    List *parameters, Oid languageOid, bool is_aggregate,
    const char *queryString, oidvector **parameterTypes,
    ArrayType **allParameterTypes, ArrayType **parameterModes,
    ArrayType **parameterNames, List **parameterDefaults, Oid *variadicArgType,
  ...
  *parameterDefaults = NIL;
  ...
  foreach (x, parameters) {
    ...
    if (aclresult != ACLCHECK_OK) aclcheck_error_type(aclresult, toid);
    ...
    if (fp->defexpr) {
      ...
      *parameterDefaults = lappend(*parameterDefaults, def);
      ...
      if (isinput && have_defaults)
        ereport(ERROR, (errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
                        errmsg(
                            "input parameters after one with a default value "
  ...
  *parameterTypes = buildoidvector(inTypes, inCount);
  ...
  if (outCount > 0 || varCount > 0) {
    *allParameterTypes = construct_array(allTypes, parameterCount, OIDOID,
                                         sizeof(Oid), true, 'i');
    *parameterModes =
        construct_array(paramModes, parameterCount, CHAROID, 1, true, 'c');
    if (outCount > 1) *requiredResultType = RECORDOID;
  ...
  if (have_names) {
    ...
    *parameterNames =
        construct_array(paramNames, parameterCount, TEXTOID, -1, false, 'i');
  } else
    *parameterNames = NULL;
}
 ...
 */
static bool compute_common_attribute(DefElem *defel, DefElem **volatility_item,
                                     DefElem **strict_item,
                                     DefElem **security_item,
                                     DefElem **leakproof_item, List **set_items,
                                     DefElem **cost_item, DefElem **rows_item) {
  if (strcmp(defel->defname, "volatility") == 0) {
    if (*volatility_item) goto duplicate_error;

    *volatility_item = defel;
  } else if (strcmp(defel->defname, "strict") == 0) {
    if (*strict_item) goto duplicate_error;

    *strict_item = defel;
  } else if (strcmp(defel->defname, "security") == 0) {
    if (*security_item) goto duplicate_error;

    *security_item = defel;
  } else if (strcmp(defel->defname, "leakproof") == 0) {
    if (*leakproof_item) goto duplicate_error;

    *leakproof_item = defel;
  } else if (strcmp(defel->defname, "set") == 0) {
    *set_items = lappend(*set_items, defel->arg);
  } else if (strcmp(defel->defname, "cost") == 0) {
    if (*cost_item) goto duplicate_error;

    *cost_item = defel;
  } else if (strcmp(defel->defname, "rows") == 0) {
    if (*rows_item) goto duplicate_error;
 ...
 * attributes.
 */
static void compute_attributes_sql_style(List *options, List **as,
                                         char **language, Node **transform,
                                         bool *windowfunc_p, char *volatility_p,
                                         bool *strict_p, bool *security_definer,
                                         bool *leakproof_p,
                                         ArrayType **proconfig, float4 *procost,
                                         float4 *prorows) {
  ListCell *option;
  ...
  foreach (option, options) {
    ...
    } else if (compute_common_attribute(defel, &volatility_item, &strict_item,
                                        &security_item, &leakproof_item,
  ...
  if (windowfunc_item) *windowfunc_p = intVal(windowfunc_item->arg);
  if (volatility_item)
    *volatility_p = interpret_func_volatility(volatility_item);
  if (strict_item) *strict_p = intVal(strict_item->arg);
  if (security_item) *security_definer = intVal(security_item->arg);
  if (leakproof_item) *leakproof_p = intVal(leakproof_item->arg);
 ...
 *------------
 */
static void compute_attributes_with_style(List *parameters, bool *isStrict_p,
                                          char *volatility_p) {
  ListCell *pl;
  ...
  foreach (pl, parameters) {
    ...
    if (pg_strcasecmp(param->defname, "isstrict") == 0)
      *isStrict_p = defGetBoolean(param);
      ...
      if (defGetBoolean(param)) *volatility_p = PROVOLATILE_IMMUTABLE;
    } else
...
static void interpret_AS_clause(Oid languageOid, const char *languageName,
                                char *funcname, List *as, char **prosrc_str_p,
                                char **probin_str_p) {
  Assert(as != NIL);
  ...
  if (languageOid == ClanguageId) {
    ...
    *probin_str_p = strVal(linitial(as));
    if (list_length(as) == 1)
      ...
      *prosrc_str_p = strVal(lsecond(as));
      if (strcmp(*prosrc_str_p, "-") == 0) *prosrc_str_p = funcname;
    }
  } else {
    ...
    *probin_str_p = NULL;
    ...
    if (languageOid == INTERNALlanguageId) {
       ...
       */
      if (strlen(*prosrc_str_p) == 0) *prosrc_str_p = funcname;
    }
  }
}
 ...
 */
ObjectAddress CreateFunction(CreateFunctionStmt *stmt,
  ...
  compute_attributes_sql_style(stmt->options, &as_clause, &language,
                               &transformDefElem, &isWindowFunc, &volatility,
                               &isStrict, &security, &isLeakProof, &proconfig,
                               &procost, &prorows);
  ...
  if (!HeapTupleIsValid(languageTuple))
    ereport(ERROR, (errcode(ERRCODE_UNDEFINED_OBJECT),
                    ...
                    (PLTemplateExists(language)
                         ? errhint(
                               "Use CREATE LANGUAGE to load the language into "
                               "the database.")
                         : 0)));
  ...
  if (transformDefElem) {
    ...
    foreach (lc, (List *)transformDefElem) {
      Oid typeid___ =
          typenameTypeId(NULL, static_cast<const TypeName *>(lfirst(lc)));
  ...
  interpret_function_parameter_list(
      stmt->parameters, languageOid, false, /* not an aggregate */
      queryString, &parameterTypes, &allParameterTypes, &parameterModes,
      &parameterNames, &parameterDefaults, &variadicArgType,
      &requiredResultType);

  if (stmt->returnType) {
    /* explicit RETURNS clause */
    compute_return_type(stmt->returnType, languageOid, &prorettype,
                        &returnsSet);
  ...
  if (list_length(trftypes_list) > 0) {
    ...
    arr = static_cast<Datum *>(
        palloc(list_length(trftypes_list) * sizeof(Datum)));
    i = 0;
    foreach (lc, trftypes_list)
      arr[i++] = ObjectIdGetDatum(lfirst_oid(lc));
    trftypes = construct_array(arr, list_length(trftypes_list), OIDOID,
                               sizeof(Oid), true, 'i');
  } else {
  ...
  interpret_AS_clause(languageOid, language, funcname, as_clause, &prosrc_str,
                      &probin_str);
  ...
  return ProcedureCreate(
      funcname, namespaceId, stmt->replace, returnsSet, prorettype, GetUserId(),
      languageOid, languageValidator, prosrc_str, /* converted to text later */
      ...
      PointerGetDatum(parameterNames), parameterDefaults,
      PointerGetDatum(trftypes), PointerGetDatum(proconfig), procost, prorows);
}
...
void RemoveFunctionById(Oid funcOid) {
  ...
  if (isagg) {
    ...
    if (!HeapTupleIsValid(tup)) /* should not happen */
      elog(ERROR, "cache lookup failed for pg_aggregate tuple for function %u",
           funcOid);
...
ObjectAddress AlterFunction(AlterFunctionStmt *stmt) {
  ...
  funcOid = LookupFuncNameTypeNames(stmt->func->funcname, stmt->func->funcargs,
                                    false);
  ...
  foreach (l, stmt->actions) {
    ...
    if (compute_common_attribute(defel, &volatility_item, &strict_item,
                                 &security_def_item, &leakproof_item,
                                 &set_items, &cost_item, &rows_item) == false)
      elog(ERROR, "option \"%s\" not recognized", defel->defname);
  }

  if (volatility_item)
    procForm->provolatile = interpret_func_volatility(volatility_item);
  if (strict_item) procForm->proisstrict = intVal(strict_item->arg);
  if (security_def_item) procForm->prosecdef = intVal(security_def_item->arg);
  if (leakproof_item) {
...
ObjectAddress CreateCast(CreateCastStmt *stmt) {
  ...
  if (aclresult != ACLCHECK_OK) aclcheck_error_type(aclresult, sourcetypeid);

  aclresult = pg_type_aclcheck(targettypeid, GetUserId(), ACL_USAGE);
  if (aclresult != ACLCHECK_OK) aclcheck_error_type(aclresult, targettypeid);
  ...
  if (castmethod == COERCION_METHOD_FUNCTION) {
    ...
    funcid = LookupFuncNameTypeNames(stmt->func->funcname, stmt->func->funcargs,
                                     false);
    ...
    if (!IsBinaryCoercible(sourcetypeid, procstruct->proargtypes.values[0]))
      ereport(ERROR, (errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
                      errmsg(
                          "argument of cast function must match or be "
                          "binary-coercible from source data type")));
    ...
    if (!IsBinaryCoercible(procstruct->prorettype, targettypeid))
      ereport(ERROR, (errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
                      errmsg(
                          "return data type of cast function must match or be "
                          "binary-coercible to target data type")));
 ...
 */
   ...
   * conditions).
   */
  tuple = SearchSysCache2(CASTSOURCETARGET, ObjectIdGetDatum(sourcetypeid),
...
Oid get_cast_oid(Oid sourcetypeid, Oid targettypeid, bool missing_ok) {
  ...
  oid = GetSysCacheOid2(CASTSOURCETARGET, ObjectIdGetDatum(sourcetypeid),
                        ObjectIdGetDatum(targettypeid));
 ...
 * CREATE TRANSFORM
 */
Oid CreateTransform(CreateTransformStmt *stmt) {
  Oid typeid___;
  char typtype;
  Oid langid;
  Oid fromsqlfuncid;
  ...
  if (aclresult != ACLCHECK_OK) aclcheck_error_type(aclresult, typeid___);
   ...
   */
  if (stmt->fromsql) {
    fromsqlfuncid = LookupFuncNameTypeNames(stmt->fromsql->funcname,
                                            stmt->fromsql->funcargs, false);
  ...
  if (stmt->tosql) {
    tosqlfuncid = LookupFuncNameTypeNames(stmt->tosql->funcname,
                                          stmt->tosql->funcargs, false);

    if (!pg_proc_ownercheck(tosqlfuncid, GetUserId()))
      aclcheck_error(ACLCHECK_NOT_OWNER, ACL_KIND_PROC,
                     NameListToString(stmt->tosql->funcname));
    ...
    if (procstruct->prorettype != typeid___)
      ereport(ERROR, (errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
                      errmsg(
                          "return data type of TO SQL function must be the "
                          "transform data type")));
    check_transform_function(procstruct);
    ReleaseSysCache(tuple);
  } else
  ...
  tuple = SearchSysCache2(TRFTYPELANG, ObjectIdGetDatum(typeid___),
      ...
      ereport(ERROR,
              (errcode(ERRCODE_DUPLICATE_OBJECT),
               errmsg("transform for type %s language \"%s\" already exists",
                      format_type_be(typeid___), stmt->lang)));
...
Oid get_transform_oid(Oid type_id, Oid lang_id, bool missing_ok) {
  ...
  oid = GetSysCacheOid2(TRFTYPELANG, ObjectIdGetDatum(type_id),
    ...
    ereport(
        ...
        (errcode(ERRCODE_UNDEFINED_OBJECT),
         errmsg("transform for type %s language \"%s\" does not exist",
                format_type_be(type_id), get_language_name(lang_id, false))));
  return oid;
}
...
void DropTransformById(Oid transformOid) {
  ...
  scan = systable_beginscan(relation, TransformOidIndexId, true, NULL, 1,
                            &scankey);
...
void IsThereFunctionInNamespace(const char *proname, int pronargs,
  ...
  if (SearchSysCacheExists3(PROCNAMEARGSNSP, CStringGetDatum(proname),
                            PointerGetDatum(proargtypes),
                            ObjectIdGetDatum(nspOid)))
    ereport(ERROR, (errcode(ERRCODE_DUPLICATE_FUNCTION),
                    errmsg("function %s already exists in schema \"%s\"",
                           funcname_signature_string(proname, pronargs, NIL,
                                                     proargtypes->values),
...
void ExecuteDoStmt(DoStmt *stmt) {
  ...
  if (languageStruct->lanpltrusted) {
    ...
    aclresult =
        pg_language_aclcheck(codeblock->langOid, GetUserId(), ACL_USAGE);
    if (aclresult != ACLCHECK_OK)
      aclcheck_error(aclresult, ACL_KIND_LANGUAGE,
                     NameStr(languageStruct->lanname));
  } else {
