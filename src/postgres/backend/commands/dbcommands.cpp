...
static bool get_db_info(const char *name, LOCKMODE lockmode, Oid *dbIdP,
                        Oid *ownerIdP, int *encodingP, bool *dbIsTemplateP,
...
Oid createdb(const CreatedbStmt *stmt) {
  ...
  if (dtemplate && dtemplate->arg) dbtemplate = defGetString(dtemplate);
  ...
  if (dcollate && dcollate->arg) dbcollate = defGetString(dcollate);
  if (dctype && dctype->arg) dbctype = defGetString(dctype);
  ...
  if (!get_db_info(dbtemplate, ShareLock, &src_dboid, &src_owner, &src_encoding,
    ...
    ereport(ERROR,
            (errcode(ERRCODE_UNDEFINED_DATABASE),
             errmsg("ctemplate database \"%s\" does not exist", dbtemplate)));
  ...
  if (dbcollate == NULL) dbcollate = src_collate;
  if (dbctype == NULL) dbctype = src_ctype;
  ...
  if (strcmp(dbtemplate, "template0") != 0) {
    if (encoding != src_encoding)
      ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                      errmsg(
                          "new___ encoding (%s) is incompatible with the "
                          "encoding of the ctemplate database (%s)",
                          pg_encoding_to_char(encoding),
                          pg_encoding_to_char(src_encoding)),
                      errhint(
                          "Use the same encoding as in the ctemplate database, "
                          "or use template0 as ctemplate.")));

    if (strcmp(dbcollate, src_collate) != 0)
      ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                      errmsg(
                          "new___ collation (%s) is incompatible with the "
                          "collation of the ctemplate database (%s)",
                          dbcollate, src_collate),
                      errhint(
                          "Use the same collation as in the ctemplate "
                          "database, or use template0 as ctemplate.")));

    if (strcmp(dbctype, src_ctype) != 0)
      ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                      errmsg(
                          "new___ LC_CTYPE (%s) is incompatible with the "
                          "LC_CTYPE of the ctemplate database (%s)",
                          dbctype, src_ctype),
                      errhint(
                          "Use the same LC_CTYPE as in the ctemplate database, "
                          "or use template0 as ctemplate.")));
  }
  ...
  if (dtablespacename && dtablespacename->arg) {
    ...
    /* check permissions */
    aclresult =
        pg_tablespace_aclcheck(dst_deftablespace, GetUserId(), ACL_CREATE);
      ...
      if (stat(srcpath, &st) == 0 && S_ISDIR(st.st_mode) &&
        ...
        ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                        errmsg("cannot assign new___ default tablespace \"%s\"",
                               tablespacename),
                        errdetail(
                            "There is a conflict because database \"%s\" "
  ...
  tuple = heap_form_tuple(RelationGetDescr(pg_database_rel), new_record,
                          new_record_nulls);
  ...
  PG_ENSURE_ERROR_CLEANUP(createdb_failure_callback, PointerGetDatum(&fparms));
  {
    ...
    while ((tuple = heap_getnext(scan, ForwardScanDirection)) != NULL) {
      ...
      if (srctablespace == GLOBALTABLESPACE_OID) continue;
 ...
 */
void check_encoding_locale_matches(int encoding, const char *collate,
                                   const char *ctype) {
  int ctype_encoding = pg_get_encoding_from_locale(ctype, true);
  int collate_encoding = pg_get_encoding_from_locale(collate, true);

  if (!(ctype_encoding == encoding || ctype_encoding == PG_SQL_ASCII ||
        ctype_encoding == -1 ||
#ifdef WIN32
        encoding == PG_UTF8 ||
#endif
    ...
    ereport(ERROR,
            (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
             errmsg("encoding \"%s\" does not match locale \"%s\"",
                    pg_encoding_to_char(encoding), ctype),
             errdetail("The chosen LC_CTYPE setting requires encoding \"%s\".",
                       pg_encoding_to_char(ctype_encoding))));

  if (!(collate_encoding == encoding || collate_encoding == PG_SQL_ASCII ||
        collate_encoding == -1 ||
#ifdef WIN32
        encoding == PG_UTF8 ||
#endif
    ...
    ereport(
        ...
        (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
         errmsg("encoding \"%s\" does not match locale \"%s\"",
                pg_encoding_to_char(encoding), collate),
...
static void createdb_failure_callback(int code, Datum arg) {
  createdb_failure_params *fparms =
      (createdb_failure_params *)DatumGetPointer(arg);
...
void dropdb(const char *dbname, bool missing_ok) {
  ...
  if (!get_db_info(dbname, AccessExclusiveLock, &db_id, NULL, NULL,
    ...
    if (!missing_ok) {
      ...
      ereport(NOTICE,
              (errmsg("database \"%s\" does not exist, skipping", dbname)));
      return;
    }
  }
    ...
    ereport(ERROR,
            (errcode(ERRCODE_OBJECT_IN_USE),
             ...
             errdetail_plural("There is %d slot, %d of them active.",
                              "There are %d slots, %d of them active.", nslots,
...
static void movedb(const char *dbname, const char *tblspcname) {
  ...
  if (!get_db_info(dbname, AccessExclusiveLock, &db_id, NULL, NULL, NULL, NULL,
                   NULL, NULL, NULL, &src_tblspcoid, NULL, NULL))
    ereport(ERROR, (errcode(ERRCODE_UNDEFINED_DATABASE),
                    errmsg("database \"%s\" does not exist", dbname)));
  ...
  if (dstdir != NULL) {
    while ((xlde = ReadDir(dstdir, dst_dbpath)) != NULL) {
      ...
      ereport(ERROR, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                      errmsg(
                          "some relations of database \"%s\" are already in "
                          "tablespace \"%s\"",
                          dbname, tblspcname),
                      errhint(
                          "You must move them back to the database's default "
                          "tablespace before using this command.")));
    }
  ...
  PG_ENSURE_ERROR_CLEANUP(movedb_failure_callback, PointerGetDatum(&fparms));
  {
     ...
     */
    ScanKeyInit(&scankey, Anum_pg_database_datname, BTEqualStrategyNumber,
                F_NAMEEQ, NameGetDatum(dbname));
    sysscan = systable_beginscan(pgdbrel, DatabaseNameIndexId, true, NULL, 1,
                                 &scankey);
    ...
    InvokeObjectPostAlterHook(DatabaseRelationId, HeapTupleGetOid(newtuple), 0);
  ...
  if (!rmtree(src_dbpath, true))
    ereport(WARNING, (errmsg(
                         "some useless files may be left behind in old "
...
ObjectAddress AlterDatabaseOwner(const char *dbname, Oid newOwnerId) {
  ...
  if (datForm->datdba != newOwnerId) {
    ...
    newtuple = heap_modify_tuple(tuple, RelationGetDescr(rel), repl_val,
                                 repl_null, repl_repl);
...
static bool get_db_info(const char *name, LOCKMODE lockmode, Oid *dbIdP,
  ...
  for (;;) {
    ...
    scan = systable_beginscan(relation, DatabaseNameIndexId, true, NULL, 1,
                              &scanKey);
    ...
    if (HeapTupleIsValid(tuple)) {
      ...
      if (strcmp(name, NameStr(dbform->datname)) == 0) {
        ...
        if (dbIsTemplateP) *dbIsTemplateP = dbform->datistemplate;
        /* allowing connections? */
        if (dbAllowConnP) *dbAllowConnP = dbform->datallowconn;
        /* last system OID used in database */
        if (dbLastSysOidP) *dbLastSysOidP = dbform->datlastsysoid;
        /* limit of frozen XIDs */
        if (dbFrozenXidP) *dbFrozenXidP = dbform->datfrozenxid;
        /* minimum MultixactId */
        if (dbMinMultiP) *dbMinMultiP = dbform->datminmxid;
        /* default tablespace for this database */
        if (dbTablespace) *dbTablespace = dbform->dattablespace;
        /* default locale settings for this database */
        if (dbCollate) *dbCollate = pstrdup(NameStr(dbform->datcollate));
        if (dbCtype) *dbCtype = pstrdup(NameStr(dbform->datctype));
...
static void remove_dbtablespaces(Oid db_id) {
  ...
  while ((tuple = heap_getnext(scan, ForwardScanDirection)) != NULL) {
    ...
    if (dsttablespace == GLOBALTABLESPACE_OID) continue;
...
static int errdetail_busy_db(int notherbackends, int npreparedxacts) {
  if (notherbackends > 0 && npreparedxacts > 0)
     ...
     */
    errdetail(
        "There are %d other session(s) and %d prepared transaction(s) using "
        "the database.",
...
Oid get_database_oid(const char *dbname, bool missing_ok) {
  ...
  ScanKeyInit(&entry[0], Anum_pg_database_datname, BTEqualStrategyNumber,
              F_NAMEEQ, CStringGetDatum(dbname));
  scan = systable_beginscan(pg_database, DatabaseNameIndexId, true, NULL, 1,
                            entry);
...
void dbase_redo(XLogReaderState *record) {
  ...
  if (info == XLOG_DBASE_CREATE) {
    ...
    if (InHotStandby) {
       ...
       */
      LockSharedObjectForSession(DatabaseRelationId, xlrec->db_id, 0,
                                 AccessExclusiveLock);
    ...
    if (InHotStandby) {
       ...
       */
      UnlockSharedObjectForSession(DatabaseRelationId, xlrec->db_id, 0,
                                   AccessExclusiveLock);
    }
  } else
