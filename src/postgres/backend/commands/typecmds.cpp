...
ObjectAddress DefineType(List *names, List *parameters) {
   ...
   */
  typoid = GetSysCacheOid2(TYPENAMENSP, CStringGetDatum(typeName),
  ...
  if (OidIsValid(typoid) && get_typisdefined(typoid)) {
    if (moveArrayTypeName(typoid, typeName, typeNamespace)) typoid = InvalidOid;
  }
  ...
  if (!OidIsValid(typoid)) {
     ...
     */
    if (parameters == NIL) return address;
  } else {
  ...
  if (internalLengthEl) internalLength = defGetTypeLength(internalLengthEl);
  if (inputNameEl) inputName = defGetQualifiedName(inputNameEl);
  if (outputNameEl) outputName = defGetQualifiedName(outputNameEl);
  if (receiveNameEl) receiveName = defGetQualifiedName(receiveNameEl);
  if (sendNameEl) sendName = defGetQualifiedName(sendNameEl);
  if (typmodinNameEl) typmodinName = defGetQualifiedName(typmodinNameEl);
  if (typmodoutNameEl) typmodoutName = defGetQualifiedName(typmodoutNameEl);
  if (analyzeNameEl) analyzeName = defGetQualifiedName(analyzeNameEl);
  if (categoryEl) {
  ...
  if (preferredEl) preferred = defGetBoolean(preferredEl);
  if (delimiterEl) {
  ...
  if (defaultValueEl) defaultValue = defGetString(defaultValueEl);
  if (byValueEl) byValue = defGetBoolean(byValueEl);
  if (alignmentEl) {
  ...
  if (collatableEl)
    collation =
        defGetBoolean(collatableEl) ? DEFAULT_COLLATION_OID : InvalidOid;
  ...
  if (typmodinName == NIL && typmodoutName != NIL)
    ereport(ERROR, (errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
                    errmsg(
                        "type modifier output function is useless without a "
  ...
  if (receiveName) receiveOid = findTypeReceiveFunction(receiveName, typoid);
  if (sendName) sendOid = findTypeSendFunction(sendName, typoid);
   ...
   */
  if (typmodinName) typmodinOid = findTypeTypmodinFunction(typmodinName);
  if (typmodoutName) typmodoutOid = findTypeTypmodoutFunction(typmodoutName);

  /*
   * Convert analysis function proc name to an OID. If no analysis function
   * is specified, we'll use zero to select the built-in default algorithm.
   */
  if (analyzeName) analyzeOid = findTypeAnalyzeFunction(analyzeName, typoid);
...
ObjectAddress DefineDomain(CreateDomainStmt *stmt) {
  ...
  /* Convert list of names to a name and namescpace___ */
  domainNamespace =
      QualifiedNameGetCreationNamespace(stmt->domainname, &domainName);
   ...
   */
  old_type_oid = GetSysCacheOid2(TYPENAMENSP, CStringGetDatum(domainName),
  ...
  if (typtype != TYPTYPE_BASE && typtype != TYPTYPE_DOMAIN &&
      typtype != TYPTYPE_ENUM && typtype != TYPTYPE_RANGE)
    ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH),
                    errmsg("\"%s\" is not a valid base type for a domain",
                           TypeNameToString(stmt->typeName))));

  aclresult = pg_type_aclcheck(basetypeoid, GetUserId(), ACL_USAGE);
  if (aclresult != ACLCHECK_OK) aclcheck_error_type(aclresult, basetypeoid);
  ...
  if (!isnull) defaultValue = TextDatumGetCString(datum);

  /* Inherited default binary value */
  datum =
      SysCacheGetAttr(TYPEOID, typeTup, Anum_pg_type_typdefaultbin, &isnull);
  if (!isnull) defaultValueBin = TextDatumGetCString(datum);
    ...
    switch (constr->contype) {
      case CONSTR_DEFAULT:
        ...
        if (constr->raw_expr) {
           ...
           */
          defaultExpr = cookDefault(pstate, constr->raw_expr, basetypeoid,
                                    basetypeMod, domainName);
           ...
           * might have.
           */
          if (defaultExpr == NULL || (IsA(defaultExpr, Const) &&
      ...
      case CONSTR_CHECK:
        ...
        if (constr->is_no_inherit)
          ereport(ERROR, (errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
                          errmsg(
                              "check constraints for domains cannot be marked "
                              "NO INHERIT")));
        break;
      ...
      case CONSTR_ATTR_IMMEDIATE:
        ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                        errmsg(
                            "specifying constraint deferrability not supported "
                            "for domains")));
        break;
  ...
  foreach (listptr, schema) {
    ...
    switch (constr->contype) {
      case CONSTR_CHECK:
        domainAddConstraint(address.objectId, domainNamespace, basetypeoid,
                            basetypeMod, constr, domainName, NULL);
        break;
...
ObjectAddress DefineEnum(CreateEnumStmt *stmt) {
   ...
   * it's an autogenerated array, we can rename it out of the way.
   */
  old_type_oid = GetSysCacheOid2(TYPENAMENSP, CStringGetDatum(enumName),
...
ObjectAddress AlterEnum(AlterEnumStmt *stmt, bool isTopLevel) {
  ...
  AddEnumLabel(enum_type_oid, stmt->newVal, stmt->newValNeighbor,
               stmt->newValIsAfter, stmt->skipIfExists);
...
ObjectAddress DefineRange(CreateRangeStmt *stmt) {
  ...
  if (type_is_collatable(rangeSubtype)) {
    ...
    if (rangeCollationName != NIL)
      ereport(ERROR, (errcode(ERRCODE_WRONG_OBJECT_TYPE),
                      errmsg(
                          "range collation specified but subtype does not "
                          "support collation")));
...
static void makeRangeConstructors(const char *name, Oid namescpace___,
  ...
  for (i = 0; i < lengthof(prosrc); i++) {
    ...
    constructorArgTypesVector =
        buildoidvector(constructorArgTypes, pronargs[i]);
  ...
  if (opcname != NIL) {
      ...
      ereport(
          ...
          (errcode(ERRCODE_DATATYPE_MISMATCH),
           errmsg("operator___ class___ \"%s\" does not accept data type %s",
                  NameListToString(opcname), format_type_be(subtype))));
  } else {
    ...
    if (!OidIsValid(opcid)) {
      ...
      ereport(ERROR, (errcode(ERRCODE_UNDEFINED_OBJECT),
                      errmsg(
                          "data type %s has no default operator___ class___ "
                          "for access method \"%s\"",
                          format_type_be(subtype), "btree"),
                      errhint(
                          "You must specify an operator___ class___ for the "
                          "range type or define a default operator___ class___ "
                          "for the subtype.")));
    }
  }
...
ObjectAddress DefineCompositeType(RangeVar *typevar, List *coldeflist) {
  ...
  old_type_oid = GetSysCacheOid2(TYPENAMENSP,
                                 ...
                                 ObjectIdGetDatum(typeNamespace));
  if (OidIsValid(old_type_oid)) {
    if (!moveArrayTypeName(old_type_oid, createStmt->relation->relname,
                           typeNamespace))
      ereport(ERROR, (errcode(ERRCODE_DUPLICATE_OBJECT),
...
ObjectAddress AlterDomainDefault(List *names, Node *defaultRaw) {
  ...
  if (defaultRaw) {
    ...
    defaultExpr = cookDefault(pstate, defaultRaw, typTup->typbasetype,
                              typTup->typtypmod, NameStr(typTup->typname));
  ...
  newtuple = heap_modify_tuple(tup, RelationGetDescr(rel), new_record,
  ...
  GenerateTypeDependencies(
      typTup->typnamespace, domainoid, InvalidOid, /* typrelid is n/a */
      0,                                           /* relation kind is n/a */
      typTup->typowner, typTup->typinput, typTup->typoutput, typTup->typreceive,
      typTup->typsend, typTup->typmodin, typTup->typmodout, typTup->typanalyze,
      InvalidOid, false, /* a domain isn't an implicit array */
      typTup->typbasetype, typTup->typcollation, defaultExpr,
 ...
 */
ObjectAddress AlterDomainDropConstraint(List *names, const char *constrName,
  ...
  ScanKeyInit(&key[0], Anum_pg_constraint_contypid, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(HeapTupleGetOid(tup)));

  conscan =
      systable_beginscan(conrel, ConstraintTypidIndexId, true, NULL, 1, key);
...
ObjectAddress AlterDomainAddConstraint(List *names, Node *newConstraint,
  ...
  ccbin = domainAddConstraint(domainoid, typTup->typnamespace,
                              typTup->typbasetype, typTup->typtypmod, constr,
   ...
   */
  if (!constr->skip_validation) validateDomainConstraint(domainoid, ccbin);
...
ObjectAddress AlterDomainValidateConstraint(List *names, char *constrName) {
   ...
   */
  conrel = heap_open(ConstraintRelationId, RowExclusiveLock);
  ScanKeyInit(&key, Anum_pg_constraint_contypid, BTEqualStrategyNumber, F_OIDEQ,
  ...
  InvokeObjectPostAlterHook(ConstraintRelationId, HeapTupleGetOid(copyTuple),
                            0);
  ...
  foreach (rt, rels) {
    ...
    while ((tuple = heap_getnext(scan, ForwardScanDirection)) != NULL) {
      ...
      for (i = 0; i < rtc->natts; i++) {
        ...
        conResult =
            ExecEvalExprSwitchContext(exprstate, econtext, &isNull, NULL);
        ...
        if (!isNull && !DatumGetBool(conResult)) {
          ...
          ereport(ERROR, (errcode(ERRCODE_CHECK_VIOLATION),
                          errmsg(
                              "column \"%s\" of table \"%s\" contains values "
                              "that violate the new___ constraint",
                              NameStr(tupdesc->attrs[attnum - 1]->attname),
                              RelationGetRelationName(testrel)),
                          errtablecol(testrel, attnum)));
        }
...
static List *get_rels_with_domain(Oid domainOid, LOCKMODE lockmode) {
  ...
  while (HeapTupleIsValid(depTup = systable_getnext(depScan))) {
    ...
    if (pg_depend->classid == TypeRelationId) {
       ...
       */
      result =
          list_concat(result, get_rels_with_domain(pg_depend->objid, lockmode));
      continue;
    }
    ...
    if (rtc == NULL) {
      ...
      if (OidIsValid(rel->rd_rel->reltype))
        find_composite_type_dependencies(rel->rd_rel->reltype, NULL,
     ...
     * have a problem.
     */
    if (pg_depend->objsubid > RelationGetNumberOfAttributes(rtc->rel)) continue;
    pg_att = rtc->rel->rd_att->attrs[pg_depend->objsubid - 1];
    if (pg_att->attisdropped || pg_att->atttypid != domainOid) continue;
...
static char *domainAddConstraint(Oid domainOid, Oid domainNamespace,
   ...
   */
  if (constr->conname) {
    if (ConstraintNameIsUsed(CONSTRAINT_DOMAIN, domainOid, domainNamespace,
                             constr->conname))
      ereport(ERROR,
    ...
    constr->conname =
        ChooseConstraintName(domainName, NULL, "check", domainNamespace, NIL);
  ...
  ccoid = CreateConstraintEntry(constr->conname,          /* Constraint Name */
                                ...
                                NULL, 0, domainOid, /* domain constraint */
                                ...
                                false,  /* connoinherit */
                                false); /* is_internal */
  if (constrAddr) ObjectAddressSet(*constrAddr, ConstraintRelationId, ccoid);
 ...
 */
ObjectAddress AlterTypeOwner(List *names, Oid newOwnerId,
  ...
  if (typTup->typowner != newOwnerId) {
    ...
    if (!superuser()) {
      ...
      aclresult =
          pg_namespace_aclcheck(typTup->typnamespace, newOwnerId, ACL_CREATE);
      if (aclresult != ACLCHECK_OK)
        aclcheck_error(aclresult, ACL_KIND_NAMESPACE,
    ...
    if (typTup->typtype == TYPTYPE_COMPOSITE)
      ...
      aclDatum = heap_getattr(tup, Anum_pg_type_typacl, RelationGetDescr(rel),
                              &isNull);
      /* Null ACLs do not require changes */
      if (!isNull) {
        newAcl =
            aclnewowner(DatumGetAclP(aclDatum), typTup->typowner, newOwnerId);
...
void AlterTypeOwnerInternal(Oid typeOid, Oid newOwnerId, bool hasDependEntry) {
  ...
  aclDatum =
      heap_getattr(tup, Anum_pg_type_typacl, RelationGetDescr(rel), &isNull);
  /* Null ACLs do not require changes */
  if (!isNull) {
 ...
 */
ObjectAddress AlterTypeNamespace(List *names, const char *newschema,
...
Oid AlterTypeNamespace_oid(Oid typeOid, Oid nspOid,
                           ObjectAddresses *objsMoved) {
  Oid elemOid;
 ...
 */
Oid AlterTypeNamespaceInternal(Oid typeOid, Oid nspOid, bool isImplicitArray,
  ...
  if (object_address_present(&thisobj, objsMoved)) return InvalidOid;
    ...
    ereport(ERROR,
            (errcode(ERRCODE_DUPLICATE_OBJECT),
             errmsg("type \"%s\" already exists in schema \"%s\"",
                    NameStr(typform->typname), get_namespace_name(nspOid))));
  ...
  if (isCompositeType) {
     ...
     */
    AlterConstraintNamespaces(typform->typrelid, oldNspOid, nspOid, false,
                              objsMoved);
  } else {
  ...
  if ((isCompositeType || typform->typtype != TYPTYPE_COMPOSITE) &&
      !isImplicitArray)
    if (changeDependencyFor(TypeRelationId, typeOid, NamespaceRelationId,
                            oldNspOid, nspOid) != 1)
