...
static void AlterEventTriggerOwner_internal(Relation rel, HeapTuple tup,
                                            Oid newOwnerId);
...
Oid CreateEventTrigger(CreateEventTrigStmt *stmt) {
  ...
  foreach (lc, stmt->whenclause) {
    ...
    if (strcmp(def->defname, "tag") == 0) {
      if (tags != NULL) error_duplicate_filter_variable(def->defname);
      tags = (List *)def->arg;
    } else
      ereport(ERROR,
  ...
  return insert_event_trigger_tuple(stmt->trigname, stmt->eventname, evtowner,
                                    funcoid, tags);
}
...
static event_trigger_command_tag_check_result check_ddl_tag(const char *tag) {
  ...
  if (pg_strcasecmp(tag, "CREATE TABLE AS") == 0 ||
      ...
      pg_strcasecmp(tag, "COMMENT") == 0 || pg_strcasecmp(tag, "GRANT") == 0 ||
  ...
  for (etsd = event_trigger_support; etsd->obtypename != NULL; etsd++)
    if (pg_strcasecmp(etsd->obtypename, obtypename) == 0) break;
  if (etsd->obtypename == NULL) return EVENT_TRIGGER_COMMAND_TAG_NOT_RECOGNIZED;
  if (!etsd->supported) return EVENT_TRIGGER_COMMAND_TAG_NOT_SUPPORTED;
  return EVENT_TRIGGER_COMMAND_TAG_OK;
}
...
static Datum filter_list_to_array(List *filterlist) {
  ...
  foreach (lc, filterlist) {
    ...
    for (p = result; *p; p++) *p = pg_ascii_toupper((unsigned char)*p);
...
Oid AlterEventTrigger(AlterEventTrigStmt *stmt) {
  ...
  if (!pg_event_trigger_ownercheck(trigoid, GetUserId()))
    aclcheck_error(ACLCHECK_NOT_OWNER, ACL_KIND_EVENT_TRIGGER, stmt->trigname);
...
static void AlterEventTriggerOwner_internal(Relation rel, HeapTuple tup,
  ...
  changeDependencyOnOwner(EventTriggerRelationId, HeapTupleGetOid(tup),
                          newOwnerId);
...
static bool filter_event_trigger(const char **tag,
  ...
  if (SessionReplicationRole == SESSION_REPLICATION_ROLE_REPLICA) {
    if (item->enabled == TRIGGER_FIRES_ON_ORIGIN) return false;
  } else {
    if (item->enabled == TRIGGER_FIRES_ON_REPLICA) return false;
  }

  /* Filter by tags, if any were specified. */
  if (item->ntags != 0 &&
      bsearch(tag, item->tag, item->ntags, sizeof(char *), pg_qsort_strcmp) ==
          NULL)
    return false;
 ...
 */
static List *EventTriggerCommonSetup(Node *parsetree, EventTriggerEvent event,
                                     const char *eventstr,
 ...
 */
  ...
  foreach (lc, cachelist) {
    EventTriggerCacheItem *item =
        static_cast<EventTriggerCacheItem *>(lfirst(lc));
...
void EventTriggerDDLCommandStart(Node *parsetree) {
  ...
  runlist = EventTriggerCommonSetup(parsetree, EVT_DDLCommandStart,
                                    "ddl_command_start", &trigdata);
...
void EventTriggerDDLCommandEnd(Node *parsetree) {
  ...
  runlist = EventTriggerCommonSetup(parsetree, EVT_DDLCommandEnd,
                                    "ddl_command_end", &trigdata);
  if (runlist == NIL) return;
...
void EventTriggerSQLDrop(Node *parsetree) {
  ...
  runlist =
      EventTriggerCommonSetup(parsetree, EVT_SQLDrop, "sql_drop", &trigdata);
...
static void EventTriggerInvoke(List *fn_oid_list, EventTriggerData *trigdata) {
  ...
  context = AllocSetContextCreate(
      CurrentMemoryContext, "event trigger context", ALLOCSET_DEFAULT_MINSIZE,
  ...
  foreach (lc, fn_oid_list) {
    ...
    InitFunctionCallInfoData(fcinfo, &flinfo, 0, InvalidOid, (Node *)trigdata,
                             NULL);
...
bool EventTriggerBeginCompleteQuery(void) {
   ...
   * install one.
   */
  if (!trackDroppedObjectsNeeded()) return false;

  cxt = AllocSetContextCreate(
      TopMemoryContext, "event trigger state", ALLOCSET_DEFAULT_MINSIZE,
      ALLOCSET_DEFAULT_INITSIZE, ALLOCSET_DEFAULT_MAXSIZE);
  state = static_cast<EventTriggerQueryState *>(
      MemoryContextAlloc(cxt, sizeof(EventTriggerQueryState)));
  state->cxt = cxt;
  slist_init(&(state->SQLDropList));
  state->in_sql_drop = false;
  state->table_rewrite_oid = InvalidOid;

  state->commandCollectionInhibited =
 ...
 */
void EventTriggerSQLDropAddObject(const ObjectAddress *object, bool original,
                                  bool normal) {
  SQLDropObject *obj;
  ...
  if (is_objectclass_supported(object->classId)) {
    ...
    if (tuple) {
      ...
      if (get_object_namensp_unique(obj->address.classId) &&
        ...
        if (attnum != InvalidAttrNumber) {
          datum =
              heap_getattr(tuple, attnum, RelationGetDescr(catalog), &isnull);
          if (!isnull) obj->objname = pstrdup(NameStr(*DatumGetName(datum)));
        }
      }
    }
...
Datum pg_event_trigger_dropped_objects(PG_FUNCTION_ARGS) {
  ...
  if (!(rsinfo->allowedModes & SFRM_Materialize))
    ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                    errmsg(
                        "materialize mode required, but it is not allowed in "
                        "this context")));
...
void EventTriggerCollectAlterTableSubcmd(Node *subcmd, ObjectAddress address) {
  ...
  currentEventTriggerState->currentCommand->d.alterTable.subcmds = lappend(
      currentEventTriggerState->currentCommand->d.alterTable.subcmds, newsub);
...
void EventTriggerCollectAlterOpFam(AlterOpFamilyStmt *stmt, Oid opfamoid,
  ...
  command->in_extension = creating_extension;
  ObjectAddressSet(command->d.opfam.address, OperatorFamilyRelationId,
                   opfamoid);
...
void EventTriggerCollectCreateOpClass(CreateOpClassStmt *stmt, Oid opcoid,
  ...
  command->in_extension = creating_extension;
  ObjectAddressSet(command->d.createopc.address, OperatorClassRelationId,
                   opcoid);
    ...
    switch (cmd->type) {
      ...
      case SCT_AlterTSConfig: {
        ...
        if (cmd->type == SCT_Simple)
          ...
          ObjectAddressSet(addr, cmd->d.alterTable.classId,
        ...
        if (is_objectclass_supported(addr.classId)) {
          ...
          if (nspAttnum != InvalidAttrNumber) {
            ...
            objtup = get_catalog_object_by_oid(catalog, addr.objectId);
            if (!HeapTupleIsValid(objtup))
              elog(ERROR, "cache lookup failed for object %u/%u", addr.classId,
                   addr.objectId);
      ...
      case SCT_AlterDefaultPrivileges:
        ...
        values[i++] = CStringGetTextDatum(
            stringify_adefprivs_objtype(cmd->d.defprivs.objtype));
        /* schema */
        nulls[i++] = true;
        /* identity */
        nulls[i++] = true;
      ...
      case SCT_Grant:
        ...
        /* command tag */
        values[i++] = CStringGetTextDatum(
            cmd->d.grant.istmt->is_grant ? "GRANT" : "REVOKE");
        /* object_type */
        values[i++] = CStringGetTextDatum(
            stringify_grantobjtype(cmd->d.grant.istmt->objtype));
        /* schema */
        nulls[i++] = true;
        /* identity */
        nulls[i++] = true;
