...
#define GetUpdatedColumns(relinfo, estate)                           \
  (rt_fetch((relinfo)->ri_RangeTableIndex, (estate)->es_range_table) \
       ->updatedCols)

/* Local function prototypes */
static void ConvertTriggerToFK(CreateTrigStmt *stmt, Oid funcoid);
static void SetTriggerFlags(TriggerDesc *trigdesc, Trigger *trigger);
static HeapTuple GetTupleForTrigger(EState *estate, EPQState *epqstate,
                                    ResultRelInfo *relinfo, ItemPointer tid,
                                    LockTupleMode lockmode,
                                    TupleTableSlot **newSlot);
static bool TriggerEnabled(EState *estate, ResultRelInfo *relinfo,
                           Trigger *trigger, TriggerEvent event,
                           Bitmapset *modifiedCols, HeapTuple oldtup,
                           HeapTuple newtup);
static HeapTuple ExecCallTriggerFunc(TriggerData *trigdata, int tgindx,
                                     FmgrInfo *finfo, Instrumentation *instr,
                                     MemoryContext per_tuple_context);
static void AfterTriggerSaveEvent(EState *estate, ResultRelInfo *relinfo,
                                  int event, bool row_trigger, HeapTuple oldtup,
                                  HeapTuple newtup, List *recheckIndexes,
...
ObjectAddress CreateTrigger(CreateTrigStmt *stmt, const char *queryString,
  ...
  if (!isInternal) {
    aclresult =
        pg_class_aclcheck(RelationGetRelid(rel), GetUserId(), ACL_TRIGGER);
  ...
  if (stmt->row) TRIGGER_SETT_ROW(tgtype);
  tgtype |= stmt->timing;
  tgtype |= stmt->events;
  ...
  if (stmt->whenClause) {
     ...
     */
    rte = addRangeTableEntryForRelation(pstate, rel, makeAlias("old", NIL),
                                        false, false);
    addRTEtoQuery(pstate, rte, false, true, true);
    rte = addRangeTableEntryForRelation(pstate, rel, makeAlias("new", NIL),
                                        false, false);
    addRTEtoQuery(pstate, rte, false, true, true);

    /* Transform expression.  Copy to be sure we don't modify original */
    whenClause = transformWhereClause(
        pstate, static_cast<Node *>(copyObject(stmt->whenClause)),
     ...
     * of subselects.
     */
    varList = pull_var_clause(whenClause, PVC_REJECT_AGGREGATES,
      ...
      switch (var->varno) {
        case PRS2_OLD_VARNO:
          if (!TRIGGER_FOR_ROW(tgtype))
            ereport(ERROR, (errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
                            errmsg(
                                "statement trigger's WHEN condition cannot "
        ...
        case PRS2_NEW_VARNO:
          ...
          if (TRIGGER_FOR_DELETE(tgtype))
            ereport(ERROR, (errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
                            errmsg(
                                "DELETE trigger's WHEN condition cannot "
                                "reference NEW values"),
                            parser_errposition(pstate, var->location)));
          if (var->varattno < 0 && TRIGGER_FOR_BEFORE(tgtype))
            ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                            errmsg(
                                "BEFORE trigger's WHEN condition cannot "
                                "reference NEW system columns"),
                            parser_errposition(pstate, var->location)));
          break;
        default:
          /* can't happen without add_missing_from, so just elog */
          elog(ERROR,
               "trigger WHEN condition cannot contain references to other "
               "relations");
          break;
      }
    }
  ...
  if (stmt->isconstraint && !OidIsValid(constraintOid)) {
    ...
    Assert(!isInternal);
    constraintOid = CreateConstraintEntry(
        stmt->trigname, RelationGetNamespace(rel), CONSTRAINT_TRIGGER,
        stmt->deferrable, stmt->initdeferred, true, RelationGetRelid(rel),
        NULL,                                           /* no conkey */
        0, InvalidOid,                                  /* no domain */
        InvalidOid,                                     /* no index */
   ...
   */
  if (isInternal) {
    snprintf(internaltrigname, sizeof(internaltrigname), "%s_%u",
  ...
  if (!isInternal) {
    ...
    tgscan =
        systable_beginscan(tgrel, TriggerRelidNameIndexId, true, NULL, 1, &key);
  ...
  values[Anum_pg_trigger_tgname - 1] =
      DirectFunctionCall1(namein, CStringGetDatum(trigname));
  ...
  if (stmt->args) {
    ...
    values[Anum_pg_trigger_tgargs - 1] =
        DirectFunctionCall1(byteain, CStringGetDatum(args));
  } else {
      ...
      if (attnum == InvalidAttrNumber)
        ereport(ERROR,
                (errcode(ERRCODE_UNDEFINED_COLUMN),
                 errmsg("column \"%s\" of relation \"%s\" does not exist", name,
                        RelationGetRelationName(rel))));
...
static void ConvertTriggerToFK(CreateTrigStmt *stmt, Oid funcoid) {
  ...
  if (l == NULL) {
    ...
    ProcessUtility((Node *)atstmt,
                   ...
                   PROCESS_UTILITY_SUBCOMMAND, NULL, None_Receiver, NULL);
...
void RemoveTriggerById(Oid trigOid) {
   ...
   */
  ScanKeyInit(&skey[0], ObjectIdAttributeNumber, BTEqualStrategyNumber, F_OIDEQ,
...
Oid get_trigger_oid(Oid relid, const char *trigname, bool missing_ok) {
  ...
  ScanKeyInit(&skey[0], Anum_pg_trigger_tgrelid, BTEqualStrategyNumber, F_OIDEQ,
              ObjectIdGetDatum(relid));
  ScanKeyInit(&skey[1], Anum_pg_trigger_tgname, BTEqualStrategyNumber, F_NAMEEQ,
              CStringGetDatum(trigname));

  tgscan =
      systable_beginscan(tgrel, TriggerRelidNameIndexId, true, NULL, 2, skey);
 ...
 */
static void RangeVarCallbackForRenameTrigger(const RangeVar *rv, Oid relid,
...
ObjectAddress renametrig(RenameStmt *stmt) {
   ...
   */
  relid =
      RangeVarGetRelidExtended(stmt->relation, AccessExclusiveLock, false,
                               false, RangeVarCallbackForRenameTrigger, NULL);
  ...
  tgscan =
      systable_beginscan(tgrel, TriggerRelidNameIndexId, true, NULL, 2, key);
 ...
 * system triggers
 */
void EnableDisableTrigger(Relation rel, const char *tgname, char fires_when,
                          bool skip_system) {
  Relation tgrel;
  int nkeys;
  ScanKeyData keys[2];
  ...
  ScanKeyInit(&keys[0], Anum_pg_trigger_tgrelid, BTEqualStrategyNumber, F_OIDEQ,
              ObjectIdGetDatum(RelationGetRelid(rel)));
  if (tgname) {
  ...
  tgscan = systable_beginscan(tgrel, TriggerRelidNameIndexId, true, NULL, nkeys,
                              keys);
   ...
   */
  if (changed) CacheInvalidateRelcache(rel);
}
...
void RelationBuildTriggers(Relation relation) {
  ...
  tgrel = heap_open(TriggerRelationId, AccessShareLock);
  tgscan =
      systable_beginscan(tgrel, TriggerRelidNameIndexId, true, NULL, 1, &skey);
  ...
  while (HeapTupleIsValid(htup = systable_getnext(tgscan))) {
    ...
    build->tgname = DatumGetCString(
        DirectFunctionCall1(nameout, NameGetDatum(&pg_trigger->tgname)));
    ...
    if (build->tgnargs > 0) {
      ...
      val = DatumGetByteaP(
          fastgetattr(htup, Anum_pg_trigger_tgargs, tgrel->rd_att, &isnull));
      if (isnull)
...
static void SetTriggerFlags(TriggerDesc *trigdesc, Trigger *trigger) {
  ...
  trigdesc->trig_insert_before_row |= TRIGGER_TYPE_MATCHES(
      tgtype, TRIGGER_TYPE_ROW, TRIGGER_TYPE_BEFORE, TRIGGER_TYPE_INSERT);
  trigdesc->trig_insert_after_row |= TRIGGER_TYPE_MATCHES(
      tgtype, TRIGGER_TYPE_ROW, TRIGGER_TYPE_AFTER, TRIGGER_TYPE_INSERT);
  trigdesc->trig_insert_instead_row |= TRIGGER_TYPE_MATCHES(
      tgtype, TRIGGER_TYPE_ROW, TRIGGER_TYPE_INSTEAD, TRIGGER_TYPE_INSERT);
  trigdesc->trig_insert_before_statement |= TRIGGER_TYPE_MATCHES(
      tgtype, TRIGGER_TYPE_STATEMENT, TRIGGER_TYPE_BEFORE, TRIGGER_TYPE_INSERT);
  trigdesc->trig_insert_after_statement |= TRIGGER_TYPE_MATCHES(
      tgtype, TRIGGER_TYPE_STATEMENT, TRIGGER_TYPE_AFTER, TRIGGER_TYPE_INSERT);
  trigdesc->trig_update_before_row |= TRIGGER_TYPE_MATCHES(
      tgtype, TRIGGER_TYPE_ROW, TRIGGER_TYPE_BEFORE, TRIGGER_TYPE_UPDATE);
  trigdesc->trig_update_after_row |= TRIGGER_TYPE_MATCHES(
      tgtype, TRIGGER_TYPE_ROW, TRIGGER_TYPE_AFTER, TRIGGER_TYPE_UPDATE);
  trigdesc->trig_update_instead_row |= TRIGGER_TYPE_MATCHES(
      tgtype, TRIGGER_TYPE_ROW, TRIGGER_TYPE_INSTEAD, TRIGGER_TYPE_UPDATE);
  trigdesc->trig_update_before_statement |= TRIGGER_TYPE_MATCHES(
      tgtype, TRIGGER_TYPE_STATEMENT, TRIGGER_TYPE_BEFORE, TRIGGER_TYPE_UPDATE);
  trigdesc->trig_update_after_statement |= TRIGGER_TYPE_MATCHES(
      tgtype, TRIGGER_TYPE_STATEMENT, TRIGGER_TYPE_AFTER, TRIGGER_TYPE_UPDATE);
  trigdesc->trig_delete_before_row |= TRIGGER_TYPE_MATCHES(
      tgtype, TRIGGER_TYPE_ROW, TRIGGER_TYPE_BEFORE, TRIGGER_TYPE_DELETE);
  trigdesc->trig_delete_after_row |= TRIGGER_TYPE_MATCHES(
      tgtype, TRIGGER_TYPE_ROW, TRIGGER_TYPE_AFTER, TRIGGER_TYPE_DELETE);
  trigdesc->trig_delete_instead_row |= TRIGGER_TYPE_MATCHES(
      tgtype, TRIGGER_TYPE_ROW, TRIGGER_TYPE_INSTEAD, TRIGGER_TYPE_DELETE);
  trigdesc->trig_delete_before_statement |= TRIGGER_TYPE_MATCHES(
      tgtype, TRIGGER_TYPE_STATEMENT, TRIGGER_TYPE_BEFORE, TRIGGER_TYPE_DELETE);
  trigdesc->trig_delete_after_statement |= TRIGGER_TYPE_MATCHES(
      tgtype, TRIGGER_TYPE_STATEMENT, TRIGGER_TYPE_AFTER, TRIGGER_TYPE_DELETE);
  /* there are no row-level truncate triggers */
  trigdesc->trig_truncate_before_statement |=
      TRIGGER_TYPE_MATCHES(tgtype, TRIGGER_TYPE_STATEMENT, TRIGGER_TYPE_BEFORE,
                           TRIGGER_TYPE_TRUNCATE);
  trigdesc->trig_truncate_after_statement |=
      TRIGGER_TYPE_MATCHES(tgtype, TRIGGER_TYPE_STATEMENT, TRIGGER_TYPE_AFTER,
                           TRIGGER_TYPE_TRUNCATE);
}
...
TriggerDesc *CopyTriggerDesc(TriggerDesc *trigdesc) {
  ...
  if (trigdesc == NULL || trigdesc->numtriggers <= 0) return NULL;
  ...
  for (i = 0; i < trigdesc->numtriggers; i++) {
    ...
    if (trigger->tgqual) trigger->tgqual = pstrdup(trigger->tgqual);
    trigger++;
  }
...
void FreeTriggerDesc(TriggerDesc *trigdesc) {
  ...
  for (i = 0; i < trigdesc->numtriggers; i++) {
    ...
    if (trigger->tgnattr > 0) pfree(trigger->tgattr);
    if (trigger->tgnargs > 0) {
      while (--(trigger->tgnargs) >= 0)
        pfree(trigger->tgargs[trigger->tgnargs]);
      pfree(trigger->tgargs);
    }
    if (trigger->tgqual) pfree(trigger->tgqual);
    trigger++;
  }
...
bool equalTriggerDescs(TriggerDesc *trigdesc1, TriggerDesc *trigdesc2) {
   ...
   */
  if (trigdesc1 != NULL) {
    if (trigdesc2 == NULL) return false;
    if (trigdesc1->numtriggers != trigdesc2->numtriggers) return false;
    for (i = 0; i < trigdesc1->numtriggers; i++) {
      Trigger *trig1 = trigdesc1->triggers + i;
      Trigger *trig2 = trigdesc2->triggers + i;

      if (trig1->tgoid != trig2->tgoid) return false;
      if (strcmp(trig1->tgname, trig2->tgname) != 0) return false;
      if (trig1->tgfoid != trig2->tgfoid) return false;
      if (trig1->tgtype != trig2->tgtype) return false;
      if (trig1->tgenabled != trig2->tgenabled) return false;
      if (trig1->tgisinternal != trig2->tgisinternal) return false;
      if (trig1->tgconstrrelid != trig2->tgconstrrelid) return false;
      if (trig1->tgconstrindid != trig2->tgconstrindid) return false;
      if (trig1->tgconstraint != trig2->tgconstraint) return false;
      if (trig1->tgdeferrable != trig2->tgdeferrable) return false;
      if (trig1->tginitdeferred != trig2->tginitdeferred) return false;
      if (trig1->tgnargs != trig2->tgnargs) return false;
      if (trig1->tgnattr != trig2->tgnattr) return false;
      if (trig1->tgnattr > 0 &&
          memcmp(trig1->tgattr, trig2->tgattr,
                 trig1->tgnattr * sizeof(int16)) != 0)
        return false;
      for (j = 0; j < trig1->tgnargs; j++)
        if (strcmp(trig1->tgargs[j], trig2->tgargs[j]) != 0) return false;
...
static HeapTuple ExecCallTriggerFunc(TriggerData *trigdata, int tgindx,
   ...
   */
  InitFunctionCallInfoData(fcinfo, finfo, 0, InvalidOid, (Node *)trigdata,
                           NULL);
...
void ExecBSInsertTriggers(EState *estate, ResultRelInfo *relinfo) {
  ...
  if (trigdesc == NULL) return;
  if (!trigdesc->trig_insert_before_statement) return;
  ...
  for (i = 0; i < trigdesc->numtriggers; i++) {
    ...
    if (!TRIGGER_TYPE_MATCHES(trigger->tgtype, TRIGGER_TYPE_STATEMENT,
      ...
      continue;
    if (!TriggerEnabled(estate, relinfo, trigger, LocTriggerData.tg_event, NULL,
                        NULL, NULL))
      continue;

    LocTriggerData.tg_trigger = trigger;
    newtuple = ExecCallTriggerFunc(
        &LocTriggerData, i, relinfo->ri_TrigFunctions,
        relinfo->ri_TrigInstrument, GetPerTupleMemoryContext(estate));
...
void ExecASInsertTriggers(EState *estate, ResultRelInfo *relinfo) {
  ...
  if (trigdesc && trigdesc->trig_insert_after_statement)
    AfterTriggerSaveEvent(estate, relinfo, TRIGGER_EVENT_INSERT, false, NULL,
                          NULL, NIL, NULL);
}

TupleTableSlot *ExecBRInsertTriggers(EState *estate, ResultRelInfo *relinfo,
                                     TupleTableSlot *slot) {
  TriggerDesc *trigdesc = relinfo->ri_TrigDesc;
  ...
  for (i = 0; i < trigdesc->numtriggers; i++) {
    ...
    newtuple = ExecCallTriggerFunc(
        &LocTriggerData, i, relinfo->ri_TrigFunctions,
        relinfo->ri_TrigInstrument, GetPerTupleMemoryContext(estate));
    if (oldtuple != newtuple && oldtuple != slottuple) heap_freetuple(oldtuple);
    if (newtuple == NULL) return NULL; /* "do nothing" */
  }
...
void ExecARInsertTriggers(EState *estate, ResultRelInfo *relinfo,
                          HeapTuple trigtuple, List *recheckIndexes) {
  TriggerDesc *trigdesc = relinfo->ri_TrigDesc;

  if (trigdesc && trigdesc->trig_insert_after_row)
    AfterTriggerSaveEvent(estate, relinfo, TRIGGER_EVENT_INSERT, true, NULL,
                          trigtuple, recheckIndexes, NULL);
}

TupleTableSlot *ExecIRInsertTriggers(EState *estate, ResultRelInfo *relinfo,
                                     TupleTableSlot *slot) {
  TriggerDesc *trigdesc = relinfo->ri_TrigDesc;
  HeapTuple slottuple = ExecMaterializeSlot(slot);
  HeapTuple newtuple = slottuple;
  ...
  LocTriggerData.type = T_TriggerData;
  LocTriggerData.tg_event =
      TRIGGER_EVENT_INSERT | TRIGGER_EVENT_ROW | TRIGGER_EVENT_INSTEAD;
  LocTriggerData.tg_relation = relinfo->ri_RelationDesc;
  LocTriggerData.tg_newtuple = NULL;
  LocTriggerData.tg_newtuplebuf = InvalidBuffer;
  for (i = 0; i < trigdesc->numtriggers; i++) {
...
void ExecBSDeleteTriggers(EState *estate, ResultRelInfo *relinfo) {
  ...
  if (trigdesc == NULL) return;
  if (!trigdesc->trig_delete_before_statement) return;
...
void ExecASDeleteTriggers(EState *estate, ResultRelInfo *relinfo) {
  ...
  if (trigdesc && trigdesc->trig_delete_after_statement)
    AfterTriggerSaveEvent(estate, relinfo, TRIGGER_EVENT_DELETE, false, NULL,
                          NULL, NIL, NULL);
}
...
bool ExecBRDeleteTriggers(EState *estate, EPQState *epqstate,
  ...
  if (fdw_trigtuple == NULL) {
    ...
    if (trigtuple == NULL) return false;
  } else
  ...
  for (i = 0; i < trigdesc->numtriggers; i++) {
    ...
    if (newtuple != trigtuple) heap_freetuple(newtuple);
  }
  if (trigtuple != fdw_trigtuple) heap_freetuple(trigtuple);
...
void ExecARDeleteTriggers(EState *estate, ResultRelInfo *relinfo,
  ...
  if (trigdesc && trigdesc->trig_delete_after_row) {
    ...
    Assert(HeapTupleIsValid(fdw_trigtuple) ^ ItemPointerIsValid(tupleid));
    if (fdw_trigtuple == NULL)
      trigtuple = GetTupleForTrigger(estate, NULL, relinfo, tupleid,
                                     LockTupleExclusive, NULL);
    else
      trigtuple = fdw_trigtuple;

    AfterTriggerSaveEvent(estate, relinfo, TRIGGER_EVENT_DELETE, true,
                          trigtuple, NULL, NIL, NULL);
    if (trigtuple != fdw_trigtuple) heap_freetuple(trigtuple);
  }
}
...
bool ExecIRDeleteTriggers(EState *estate, ResultRelInfo *relinfo,
  ...
  LocTriggerData.type = T_TriggerData;
  LocTriggerData.tg_event =
      TRIGGER_EVENT_DELETE | TRIGGER_EVENT_ROW | TRIGGER_EVENT_INSTEAD;
  LocTriggerData.tg_relation = relinfo->ri_RelationDesc;
  LocTriggerData.tg_newtuple = NULL;
  LocTriggerData.tg_newtuplebuf = InvalidBuffer;
  for (i = 0; i < trigdesc->numtriggers; i++) {
    ...
    LocTriggerData.tg_trigtuplebuf = InvalidBuffer;
    LocTriggerData.tg_trigger = trigger;
    rettuple = ExecCallTriggerFunc(
        &LocTriggerData, i, relinfo->ri_TrigFunctions,
        relinfo->ri_TrigInstrument, GetPerTupleMemoryContext(estate));
    if (rettuple == NULL) return false; /* Delete was suppressed */
    if (rettuple != trigtuple) heap_freetuple(rettuple);
  }
  return true;
}
...
void ExecBSUpdateTriggers(EState *estate, ResultRelInfo *relinfo) {
  ...
  if (trigdesc == NULL) return;
  if (!trigdesc->trig_update_before_statement) return;
...
void ExecASUpdateTriggers(EState *estate, ResultRelInfo *relinfo) {
  ...
  if (trigdesc && trigdesc->trig_update_after_statement)
    AfterTriggerSaveEvent(estate, relinfo, TRIGGER_EVENT_UPDATE, false, NULL,
                          NULL, NIL, GetUpdatedColumns(relinfo, estate));
}

TupleTableSlot *ExecBRUpdateTriggers(EState *estate, EPQState *epqstate,
  ...
  if (fdw_trigtuple == NULL) {
    ...
    trigtuple = GetTupleForTrigger(estate, epqstate, relinfo, tupleid, lockmode,
                                   &newSlot);
    if (trigtuple == NULL) return NULL; /* cancel the update action */
  } else {
    trigtuple = fdw_trigtuple;
    newSlot = NULL;
  }
...
void ExecARUpdateTriggers(EState *estate, ResultRelInfo *relinfo,
                          ItemPointer tupleid, HeapTuple fdw_trigtuple,
                          HeapTuple newtuple, List *recheckIndexes) {
  TriggerDesc *trigdesc = relinfo->ri_TrigDesc;
  ...
  if (trigdesc && trigdesc->trig_update_after_row) {
    ...
    AfterTriggerSaveEvent(estate, relinfo, TRIGGER_EVENT_UPDATE, true,
                          trigtuple, newtuple, recheckIndexes,
                          GetUpdatedColumns(relinfo, estate));
    if (trigtuple != fdw_trigtuple) heap_freetuple(trigtuple);
  }
}

TupleTableSlot *ExecIRUpdateTriggers(EState *estate, ResultRelInfo *relinfo,
                                     HeapTuple trigtuple,
                                     TupleTableSlot *slot) {
  TriggerDesc *trigdesc = relinfo->ri_TrigDesc;
  HeapTuple slottuple = ExecMaterializeSlot(slot);
  ...
  LocTriggerData.type = T_TriggerData;
  LocTriggerData.tg_event =
      TRIGGER_EVENT_UPDATE | TRIGGER_EVENT_ROW | TRIGGER_EVENT_INSTEAD;
  LocTriggerData.tg_relation = relinfo->ri_RelationDesc;
  for (i = 0; i < trigdesc->numtriggers; i++) {
    Trigger *trigger = &trigdesc->triggers[i];
...
void ExecASTruncateTriggers(EState *estate, ResultRelInfo *relinfo) {
  ...
  if (trigdesc && trigdesc->trig_truncate_after_statement)
    AfterTriggerSaveEvent(estate, relinfo, TRIGGER_EVENT_TRUNCATE, false, NULL,
                          NULL, NIL, NULL);
}
...
static HeapTuple GetTupleForTrigger(EState *estate, EPQState *epqstate,
  ...
  ltrmark:
    ;
    tuple.t_self = *tid;
    test = heap_lock_tuple(relation, &tuple, estate->es_output_cid, lockmode,
    ...
    switch (test) {
        ...
        if (hufd.cmax != estate->es_output_cid)
          ereport(ERROR,
                  (errcode(ERRCODE_TRIGGERED_DATA_CHANGE_VIOLATION),
                   errmsg(
                       "tuple to be updated was already modified by an "
                       "operation triggered by the current command"),
                   errhint(
                       "Consider using an AFTER trigger instead of a BEFORE "
      ...
      case HeapTupleUpdated:
        ...
        if (!ItemPointerEquals(&hufd.ctid, &tuple.t_self)) {
          ...
          epqslot = EvalPlanQual(estate, epqstate, relation,
                                 relinfo->ri_RangeTableIndex, lockmode,
                                 &hufd.ctid, hufd.xmax);
...
static bool TriggerEnabled(EState *estate, ResultRelInfo *relinfo,
  ...
  if (trigger->tgqual) {
    ...
    if (!ExecQual(*predicate, econtext, false)) return false;
  }
...
#define GetTriggerSharedData(evt)       \
  ((AfterTriggerShared)((char *)(evt) + \
                        ((evt)->ate_flags & AFTER_TRIGGER_OFFSET)))
...
static void AfterTriggerExecute(AfterTriggerEvent event, Relation rel,
                                TriggerDesc *trigdesc, FmgrInfo *finfo,
...
static bool afterTriggerCheckState(AfterTriggerShared evtshared) {
   ...
   */
  if ((evtshared->ats_event & AFTER_TRIGGER_DEFERRABLE) == 0) return false;
  ...
  if (state != NULL) {
    ...
    if (state->all_isset) return state->all_isdeferred;
  }
  ...
  if (chunk == NULL || chunk->endfree - chunk->freeptr < needed) {
    ...
    if (afterTriggers.event_cxt == NULL)
      afterTriggers.event_cxt = AllocSetContextCreate(
          TopTransactionContext, "AfterTriggerEvents", ALLOCSET_DEFAULT_MINSIZE,
...
#error MAX_CHUNK_SIZE must not exceed AFTER_TRIGGER_OFFSET
    ...
    chunk = static_cast<AfterTriggerEventChunk *>(
        MemoryContextAlloc(afterTriggers.event_cxt, chunksize));
    chunk->next = NULL;
  ...
  for (newshared = ((AfterTriggerShared)chunk->endptr) - 1;
       (char *)newshared >= chunk->endfree; newshared--) {
 ...
 * ----------
 */
static void afterTriggerRestoreEventList(
    AfterTriggerEventList *events, const AfterTriggerEventList *old_events) {
  AfterTriggerEventChunk *chunk;
  AfterTriggerEventChunk *next_chunk;
...
static void AfterTriggerExecute(AfterTriggerEvent event, Relation rel,
  ...
  switch (event->ate_flags & AFTER_TRIGGER_TUP_BITS) {
    ...
    case AFTER_TRIGGER_FDW_REUSE:
       ...
       */
      LocTriggerData.tg_trigtuple = ExecMaterializeSlot(trig_tuple_slot1);
      LocTriggerData.tg_trigtuplebuf = InvalidBuffer;
   ...
   */
  rettuple = ExecCallTriggerFunc(&LocTriggerData, tgindx, finfo, NULL,
                                 per_tuple_context);
   ...
   * Release buffers
   */
  if (buffer1 != InvalidBuffer) ReleaseBuffer(buffer1);
  if (buffer2 != InvalidBuffer) ReleaseBuffer(buffer2);
...
static bool afterTriggerInvokeEvents(AfterTriggerEventList *events,
                                     CommandId firing_id, EState *estate,
                                     bool delete_ok) {
  bool all_fired = true;
  AfterTriggerEventChunk *chunk;
  MemoryContext per_tuple_context;
  ...
  per_tuple_context =
      AllocSetContextCreate(CurrentMemoryContext, "AfterTriggerTupleContext",
...
#define DEFTRIG_INITALLOC 8
      afterTriggers.state_stack = (SetConstraintState *)palloc(
          DEFTRIG_INITALLOC * sizeof(SetConstraintState));
      afterTriggers.events_stack = (AfterTriggerEventList *)palloc(
          DEFTRIG_INITALLOC * sizeof(AfterTriggerEventList));
      afterTriggers.depth_stack =
          (int *)palloc(DEFTRIG_INITALLOC * sizeof(int));
      afterTriggers.firing_stack =
          (CommandId *)palloc(DEFTRIG_INITALLOC * sizeof(CommandId));
      ...
      afterTriggers.state_stack = (SetConstraintState *)repalloc(
          afterTriggers.state_stack, new_alloc * sizeof(SetConstraintState));
      afterTriggers.events_stack = (AfterTriggerEventList *)repalloc(
          afterTriggers.events_stack,
          new_alloc * sizeof(AfterTriggerEventList));
      afterTriggers.depth_stack =
          (int *)repalloc(afterTriggers.depth_stack, new_alloc * sizeof(int));
      afterTriggers.firing_stack = (CommandId *)repalloc(
          afterTriggers.firing_stack, new_alloc * sizeof(CommandId));
...
void AfterTriggerEndSubXact(bool isCommit) {
  ...
  if (isCommit) {
     ...
     */
    if (my_level >= afterTriggers.maxtransdepth) return;
...
static void AfterTriggerEnlargeQueryState(void) {
  ...
  if (afterTriggers.maxquerydepth == 0) {
    ...
    afterTriggers.query_stack = (AfterTriggerEventList *)MemoryContextAlloc(
        TopTransactionContext, new_alloc * sizeof(AfterTriggerEventList));
    afterTriggers.fdw_tuplestores = (Tuplestorestate **)MemoryContextAllocZero(
        TopTransactionContext, new_alloc * sizeof(Tuplestorestate *));
    ...
    afterTriggers.query_stack = (AfterTriggerEventList *)repalloc(
        afterTriggers.query_stack, new_alloc * sizeof(AfterTriggerEventList));
    afterTriggers.fdw_tuplestores = (Tuplestorestate **)repalloc(
        afterTriggers.fdw_tuplestores, new_alloc * sizeof(Tuplestorestate *));
    /* Clear newly-allocated slots for subsequent lazy initialization. */
    memset(afterTriggers.fdw_tuplestores + old_alloc, 0,
           (new_alloc - old_alloc) * sizeof(Tuplestorestate *));
    afterTriggers.maxquerydepth = new_alloc;
  }
...
static SetConstraintState SetConstraintStateCreate(int numalloc) {
  ...
  if (numalloc <= 0) numalloc = 1;

  /*
   * We assume that zeroing will correctly initialize the state values.
   */
  state = (SetConstraintState)MemoryContextAllocZero(
      TopTransactionContext, offsetof(SetConstraintStateData, trigstates) +
...
static SetConstraintState SetConstraintStateAddItem(SetConstraintState state,
  ...
  if (state->numstates >= state->numalloc) {
    ...
    state = (SetConstraintState)repalloc(
        state, offsetof(SetConstraintStateData, trigstates) +
  ...
  if (stmt->constraints == NIL) {
    ...
    foreach (lc, stmt->constraints) {
      ...
      if (constraint->catalogname) {
        if (strcmp(constraint->catalogname, get_database_name(MyDatabaseId)) !=
            0)
          ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                          errmsg(
      ...
      if (constraint->schemaname) {
        Oid namespaceId =
            LookupExplicitNamespace(constraint->schemaname, false);
      ...
      foreach (nslc, namespacelist) {
        ...
        ScanKeyInit(&skey[0], Anum_pg_constraint_conname, BTEqualStrategyNumber,
                    F_NAMEEQ, CStringGetDatum(constraint->relname));
        ScanKeyInit(&skey[1], Anum_pg_constraint_connamespace,
    ...
    foreach (lc, conoidlist) {
      ...
      ScanKeyInit(&skey, Anum_pg_trigger_tgconstraint, BTEqualStrategyNumber,
                  F_OIDEQ, ObjectIdGetDatum(conoid));

      tgscan = systable_beginscan(tgrel, TriggerConstraintIndexId, true, NULL,
                                  1, &skey);
...
bool AfterTriggerPendingOnRel(Oid relid) {
  ...
  for_each_event_chunk(event, chunk, afterTriggers.events) {
     ...
     */
    if (event->ate_flags & AFTER_TRIGGER_DONE) continue;

    if (evtshared->ats_relid == relid) return true;
  }
  ...
  for (depth = 0; depth <= afterTriggers.query_depth &&
                      depth < afterTriggers.maxquerydepth;
       depth++) {
...
static void AfterTriggerSaveEvent(EState *estate, ResultRelInfo *relinfo,
  ...
  for (i = 0; i < trigdesc->numtriggers; i++) {
    ...
    if (!TRIGGER_TYPE_MATCHES(trigger->tgtype, tgtype_level, TRIGGER_TYPE_AFTER,
                              tgtype_event))
      continue;
    if (!TriggerEnabled(estate, relinfo, trigger, event, modifiedCols, oldtup,
                        newtup))
      continue;
    ...
    new_shared.ats_event =
        (event & TRIGGER_EVENT_OPMASK) | (row_trigger ? TRIGGER_EVENT_ROW : 0) |
   ...
   */
  if (fdw_tuplestore) {
    if (oldtup != NULL) tuplestore_puttuple(fdw_tuplestore, oldtup);
    if (newtup != NULL) tuplestore_puttuple(fdw_tuplestore, newtup);
  }
}

Datum pg_trigger_depth(PG_FUNCTION_ARGS) { PG_RETURN_INT32(MyTriggerDepth); }
