...
Oid get_extension_oid(const char *extname, bool missing_ok) {
  ...
  scandesc =
      systable_beginscan(rel, ExtensionNameIndexId, true, NULL, 1, entry);
...
static void check_valid_extension_name(const char *extensionname) {
  ...
  if (first_dir_separator(extensionname) != NULL)
    ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                    errmsg("invalid extension name: \"%s\"", extensionname),
                    errdetail(
                        "Extension names must not contain directory separator "
                        "characters.")));
}
  ...
  if (first_dir_separator(versionname) != NULL)
    ereport(ERROR,
            (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
             errmsg("invalid extension version name: \"%s\"", versionname),
             errdetail(
                 "Version names must not contain directory separator "
                 "characters.")));
}
...
static char *get_extension_aux_control_filename(ExtensionControlFile *control,
  ...
  result = (char *)palloc(MAXPGPATH);
  snprintf(result, MAXPGPATH, "%s/%s--%s.control", scriptdir, control->name,
           version);
...
static char *get_extension_script_filename(ExtensionControlFile *control,
  ...
  result = (char *)palloc(MAXPGPATH);
  if (from_version)
    snprintf(result, MAXPGPATH, "%s/%s--%s--%s.sql", scriptdir, control->name,
             from_version, version);
  else
    snprintf(result, MAXPGPATH, "%s/%s--%s.sql", scriptdir, control->name,
             version);
...
static void parse_extension_control_file(ExtensionControlFile *control,
    ...
    if (strcmp(item->name, "directory") == 0) {
      if (version)
        ereport(ERROR, (errcode(ERRCODE_SYNTAX_ERROR),
                        errmsg(
                            "parameter \"%s\" cannot be set in a secondary "
      ...
      if (control->encoding < 0)
        ereport(ERROR,
                (errcode(ERRCODE_UNDEFINED_OBJECT),
                 errmsg("\"%s\" is not a valid encoding name", item->value)));
  ...
  if (control->relocatable && control->schema != NULL)
    ereport(ERROR, (errcode(ERRCODE_SYNTAX_ERROR),
                    errmsg(
                        "parameter \"schema\" cannot be specified when "
                        "\"relocatable\" is true")));
 ...
 */
static ExtensionControlFile *read_extension_aux_control_file(
    const ExtensionControlFile *pcontrol, const char *version) {
  ...
  foreach (lc1, raw_parsetree_list) {
    ...
    foreach (lc2, stmt_list) {
      ...
      if (IsA(stmt, PlannedStmt) &&
        ...
        qdesc = CreateQueryDesc((PlannedStmt *)stmt, sql, GetActiveSnapshot(),
                                NULL, dest, NULL, 0);
        ...
        ProcessUtility(stmt, sql, PROCESS_UTILITY_QUERY, NULL, dest, NULL);
      }
...
static void execute_extension_script(Oid extensionOid,
                                     ExtensionControlFile *control,
                                     const char *from_version,
                                     const char *version, List *requiredSchemas,
  ...
  if (client_min_messages < WARNING)
    (void)set_config_option("client_min_messages", "warning", PGC_USERSET,
                            PGC_S_SESSION, GUC_ACTION_SAVE, true, 0, false);
  if (log_min_messages < WARNING)
    (void)set_config_option("log_min_messages", "warning", PGC_SUSET,
                            PGC_S_SESSION, GUC_ACTION_SAVE, true, 0, false);
  ...
  PG_TRY();
  {
     ...
     */
    t_sql = DirectFunctionCall4Coll(textregexreplace, C_COLLATION_OID, t_sql,
    ...
    if (!control->relocatable) {
      ...
      t_sql = DirectFunctionCall3(replace_text, t_sql,
    ...
    if (control->module_pathname) {
      t_sql = DirectFunctionCall3(
          replace_text, t_sql, CStringGetTextDatum("MODULE_PATHNAME"),
...
static ExtensionVersionInfo *get_ext_ver_info(const char *versionname,
  ...
  foreach (lc, *evi_list) {
    ...
    if (strcmp(evi->name, versionname) == 0) return evi;
  }
...
static ExtensionVersionInfo *get_nearest_unprocessed_vertex(List *evi_list) {
  ...
  foreach (lc, evi_list) {
    ...
    if (evi2->distance_known) continue;
    /* remember the closest such vertex */
    if (evi == NULL || evi->distance > evi2->distance) evi = evi2;
  }
...
static List *get_ext_ver_list(ExtensionControlFile *control) {
  ...
  while ((de = ReadDir(dir, location)) != NULL) {
    ...
    /* must be a .sql file ... */
    if (!is_extension_script_filename(de->d_name)) continue;

    /* ... matching extension name followed by separator */
    if (strncmp(de->d_name, control->name, extnamelen) != 0 ||
        de->d_name[extnamelen] != '-' || de->d_name[extnamelen + 1] != '-')
      continue;
...
static List *identify_update_path(ExtensionControlFile *control,
  ...
  if (result == NIL)
    ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                    errmsg(
                        "extension \"%s\" has no update path from version "
                        "\"%s\" to version \"%s\"",
...
static List *find_update_path(List *evi_list, ExtensionVersionInfo *evi_start,
  ...
  while ((evi = get_nearest_unprocessed_vertex(evi_list)) != NULL) {
    ...
    if (evi == evi_target) break; /* found shortest path to target */
  ...
  if (!evi_target->distance_known) return NIL;
...
ObjectAddress CreateExtension(CreateExtensionStmt *stmt) {
  ...
  if (d_old_version && d_old_version->arg) {
    ...
    if (strcmp(oldVersionName, versionName) == 0)
      ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                      errmsg(
                          "FROM version must be different from installation "
                          "target version \"%s\"",
                          versionName)));

    updateVersions =
        identify_update_path(pcontrol, oldVersionName, versionName);
  ...
  foreach (lc, control->requires) {
    ...
    if (!OidIsValid(reqext))
      ereport(ERROR,
              (errcode(ERRCODE_UNDEFINED_OBJECT),
               errmsg("required extension \"%s\" is not installed", curreq)));
   ...
   */
  address = InsertExtensionTuple(
      control->name, extowner, schemaOid, control->relocatable, versionName,
      PointerGetDatum(NULL), PointerGetDatum(NULL), requiredExtensions);
 ...
 *
 * Insert the new___ pg_extension row, and create extension's dependency
 *entries.
...
void RemoveExtensionById(Oid extId) {
  ...
  /* We assume that there can be at most one matching tuple */
  if (HeapTupleIsValid(tuple)) simple_heap_delete(rel, &tuple->t_self);
...
Datum pg_available_extensions(PG_FUNCTION_ARGS) {
  ...
  if (dir == NULL && errno == ENOENT) {
    ...
    while ((de = ReadDir(dir, location)) != NULL) {
      ...
      if (!is_extension_control_filename(de->d_name)) continue;
...
Datum pg_extension_config_dump(PG_FUNCTION_ARGS) {
  ...
  if (!HeapTupleIsValid(extTup)) /* should not happen */
    elog(ERROR, "extension with oid %u does not exist", CurrentExtensionObject);
  ...
  if (isnull) {
    ...
    a = array_set(a, 1, &arrayIndex, elementDatum, false,
                  -1 /* varlena array */, sizeof(Oid) /* OID's typlen */,
                  true /* OID's typbyval */, 'i' /* OID's typalign */);
  }
  ...
  if (isnull) {
    ...
    if (ARR_NDIM(a) != 1 || ARR_LBOUND(a)[0] != 1 || ARR_HASNULL(a) ||
    ...
    a = array_set(a, 1, &arrayIndex, elementDatum, false,
                  -1 /* varlena array */, -1 /* TEXT's typlen */,
                  false /* TEXT's typbyval */, 'i' /* TEXT's typalign */);
  }
  repl_val[Anum_pg_extension_extcondition - 1] = PointerGetDatum(a);
  repl_repl[Anum_pg_extension_extcondition - 1] = true;

  extTup = heap_modify_tuple(extTup, RelationGetDescr(extRel), repl_val,
                             repl_null, repl_repl);
...
static void extension_config_remove(Oid extensionoid, Oid tableoid) {
  ...
  if (arrayLength <= 1) {
    ...
    deconstruct_array(a, OIDOID, sizeof(Oid), true, 'i', &dvalues, &dnulls,
                      &nelems);

    /* We already checked there are no nulls, so ignore dnulls */
    for (i = arrayIndex; i < arrayLength - 1; i++) dvalues[i] = dvalues[i + 1];

    a = construct_array(dvalues, arrayLength - 1, OIDOID, sizeof(Oid), true,
                        'i');
...
ObjectAddress AlterExtensionNamespace(List *names, const char *newschema,
  ...
  changeDependencyFor(ExtensionRelationId, extensionOid, NamespaceRelationId,
                      oldNspOid, nspOid);
...
ObjectAddress ExecAlterExtensionStmt(AlterExtensionStmt *stmt) {
  ...
  ScanKeyInit(&key[0], Anum_pg_extension_extname, BTEqualStrategyNumber,
              F_NAMEEQ, CStringGetDatum(stmt->extname));

  extScan =
      systable_beginscan(extRel, ExtensionNameIndexId, true, NULL, 1, key);

  extTup = systable_getnext(extScan);

  if (!HeapTupleIsValid(extTup))
    ereport(ERROR, (errcode(ERRCODE_UNDEFINED_OBJECT),
                    errmsg("extension \"%s\" does not exist", stmt->extname)));
  ...
  datum = heap_getattr(extTup, Anum_pg_extension_extversion,
                       RelationGetDescr(extRel), &isnull);
  if (isnull) elog(ERROR, "extversion is null");
...
static void ApplyExtensionUpdates(Oid extensionOid,
  ...
  foreach (lcv, updateVersions) {
    ...
    extScan =
        systable_beginscan(extRel, ExtensionOidIndexId, true, NULL, 1, key);
    ...
    extTup = heap_modify_tuple(extTup, RelationGetDescr(extRel), values, nulls,
                               repl);
...
ObjectAddress ExecAlterExtensionContentsStmt(AlterExtensionContentsStmt *stmt,
  ...
  if (objAddr) *objAddr = object;

  /* Permission check: must own target object, too */
  check_object_ownership(GetUserId(), stmt->objtype, object, stmt->objname,
                         stmt->objargs, relation);
    ...
    if (oldExtension != extension.objectId)
      ereport(ERROR, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                      errmsg("%s is not a member of extension \"%s\"",
                             getObjectDescription(&object), stmt->extname)));
