...
static void show_grouping_sets(PlanState *planstate, Agg *agg, List *ancestors,
                               ExplainState *es);
static void show_grouping_set_keys(PlanState *planstate, Agg *aggnode,
                                   Sort *sortnode, List *context,
                                   bool useprefix, List *ancestors,
                                   ExplainState *es);
...
static void show_sort_group_keys(PlanState *planstate, const char *qlabel,
                                 ...
                                 bool *nullsFirst, List *ancestors,
                                 ExplainState *es);
...
void ExplainQuery(ExplainStmt *stmt, const char *queryString,
  ...
  if (rewritten == NIL) {
    ...
    foreach (l, rewritten) {
      ...
      if (lnext(l) != NULL) ExplainSeparatePlans(es);
    }
  }
...
void ExplainOneUtility(Node *utilityStmt, IntoClause *into, ExplainState *es,
  ...
  if (IsA(utilityStmt, CreateTableAsStmt)) {
    ...
    ExplainOneQuery((Query *)linitial(rewritten), ctas->into, es, queryString,
                    params);
...
void ExplainOnePlan(PlannedStmt *plannedstmt, IntoClause *into,
  ...
  if (es->buffers) instrument_option |= INSTRUMENT_BUFFERS;
  ...
  if (es->analyze) ExplainPrintTriggers(es, queryDesc);
...
void ExplainPrintPlan(ExplainState *es, QueryDesc *queryDesc) {
  ...
  es->deparse_cxt =
      deparse_context_for_plan_rtable(es->rtable, es->rtable_names);
...
static void report_triggers(ResultRelInfo *rInfo, bool show_relname,
  ...
  if (!rInfo->ri_TrigDesc || !rInfo->ri_TrigInstrument) return;
  for (nt = 0; nt < rInfo->ri_TrigDesc->numtriggers; nt++) {
    ...
    if (es->format == EXPLAIN_FORMAT_TEXT) {
      ...
      if (conname) appendStringInfo(es->str, " for constraint %s", conname);
      if (show_relname) appendStringInfo(es->str, " on %s", relname);
      appendStringInfo(es->str, ": time=%.3f calls=%.0f\n",
                       1000.0 * instr->total, instr->ntuples);
    } else {
      ExplainPropertyText("Trigger Name", trig->tgname, es);
      if (conname) ExplainPropertyText("Constraint Name", conname, es);
...
static void ExplainPreScanNode(PlanState *planstate, Bitmapset **rels_used) {
  ...
  switch (nodeTag(plan)) {
    ...
    case T_ForeignScan:
      *rels_used =
          bms_add_members(*rels_used, ((ForeignScan *)plan)->fs_relids);
      break;
    case T_CustomScan:
      *rels_used =
          bms_add_members(*rels_used, ((CustomScan *)plan)->custom_relids);
      break;
    case T_ModifyTable:
      *rels_used =
          bms_add_member(*rels_used, ((ModifyTable *)plan)->nominalRelation);
      if (((ModifyTable *)plan)->exclRelRTI)
        *rels_used =
            bms_add_member(*rels_used, ((ModifyTable *)plan)->exclRelRTI);
      break;
    default:
      break;
  }
  ...
  /* subPlan-s */
  if (planstate->subPlan) ExplainPreScanSubPlans(planstate->subPlan, rels_used);
}
...
static void ExplainPreScanMemberNodes(List *plans, PlanState **planstates,
  ...
  for (j = 0; j < nplans; j++) ExplainPreScanNode(planstates[j], rels_used);
}
...
static void ExplainNode(PlanState *planstate, List *ancestors,
  ...
  if (es->format == EXPLAIN_FORMAT_TEXT) {
    ...
    if (strategy) ExplainPropertyText("Strategy", strategy, es);
    if (operation) ExplainPropertyText("Operation", operation, es);
    if (relationship)
      ExplainPropertyText("Parent Relationship", relationship, es);
    if (plan_name) ExplainPropertyText("Subplan Name", plan_name, es);
    if (custom_name)
   ...
   */
  if (planstate->instrument) InstrEndLoop(planstate->instrument);
  ...
  if (es->analyze && planstate->instrument &&
    ...
    if (es->format == EXPLAIN_FORMAT_TEXT) {
      if (es->timing)
        ...
        appendStringInfo(es->str, " (actual rows=%.0f loops=%.0f)", rows,
                         nloops);
    } else {
      if (es->timing) {
  ...
  /* target list */
  if (es->verbose) show_plan_tlist(planstate, ancestors, es);
  ...
  switch (nodeTag(plan)) {
    ...
    case T_BitmapIndexScan:
      ...
      break;
    case T_BitmapHeapScan:
      show_scan_qual(((BitmapHeapScan *)plan)->bitmapqualorig, "Recheck Cond",
                     planstate, ancestors, es);
    ...
    case T_FunctionScan:
      if (es->verbose) {
        ...
        show_expression((Node *)fexprs, "Function Call", planstate, ancestors,
                        es->verbose, es);
      }
      show_scan_qual(plan->qual, "Filter", planstate, ancestors, es);
      if (plan->qual)
    ...
    case T_NestLoop:
      ...
      if (((NestLoop *)plan)->join.joinqual)
        show_instrumentation_count("Rows Removed by Join Filter", 1, planstate,
                                   es);
    ...
    case T_MergeJoin:
      ...
      break;
    case T_HashJoin:
      show_upper_qual(((HashJoin *)plan)->hashclauses, "Hash Cond", planstate,
                      ancestors, es);
    ...
    case T_Result:
      ...
      break;
    case T_ModifyTable:
      show_modifytable_info((ModifyTableState *)planstate, ancestors, es);
      break;
    case T_Hash:
  ...
  if (es->buffers && planstate->instrument) {
    ...
    if (es->format == EXPLAIN_FORMAT_TEXT) {
      bool has_shared =
          (usage->shared_blks_hit > 0 || usage->shared_blks_read > 0 ||
           usage->shared_blks_dirtied > 0 || usage->shared_blks_written > 0);
      bool has_local =
          (usage->local_blks_hit > 0 || usage->local_blks_read > 0 ||
           usage->local_blks_dirtied > 0 || usage->local_blks_written > 0);
      bool has_temp =
          (usage->temp_blks_read > 0 || usage->temp_blks_written > 0);
      ...
      ExplainPropertyLong("Shared Dirtied Blocks", usage->shared_blks_dirtied,
                          es);
      ExplainPropertyLong("Shared Written Blocks", usage->shared_blks_written,
                          es);
      ExplainPropertyLong("Local Hit Blocks", usage->local_blks_hit, es);
      ExplainPropertyLong("Local Read Blocks", usage->local_blks_read, es);
      ExplainPropertyLong("Local Dirtied Blocks", usage->local_blks_dirtied,
                          es);
      ExplainPropertyLong("Local Written Blocks", usage->local_blks_written,
                          es);
      ExplainPropertyLong("Temp Read Blocks", usage->temp_blks_read, es);
      ExplainPropertyLong("Temp Written Blocks", usage->temp_blks_written, es);
      ExplainPropertyFloat("I/O Read Time",
                           INSTR_TIME_GET_MILLISEC(usage->blk_read_time), 3,
                           es);
      ExplainPropertyFloat("I/O Write Time",
                           INSTR_TIME_GET_MILLISEC(usage->blk_write_time), 3,
                           es);
    }
  }
  ...
  if (es->format == EXPLAIN_FORMAT_TEXT) es->indent = save_indent;
...
static void show_plan_tlist(PlanState *planstate, List *ancestors,
  ...
  if (plan->targetlist == NIL) return;
  /* The tlist of an Append isn't real helpful, so suppress it */
  if (IsA(plan, Append)) return;
  /* Likewise for MergeAppend and RecursiveUnion */
  if (IsA(plan, MergeAppend)) return;
  if (IsA(plan, RecursiveUnion)) return;

  /* Set up deparsing context */
  context = set_deparse_context_planstate(es->deparse_cxt, (Node *)planstate,
                                          ancestors);
  ...
  foreach (lc, plan->targetlist) {
    ...
    result = lappend(result, deparse_expression((Node *)tle->expr, context,
                                                useprefix, false));
  }
...
static void show_sort_keys(SortState *sortstate, List *ancestors,
  ...
  show_sort_group_keys((PlanState *)sortstate, "Sort Key", plan->numCols,
                       plan->sortColIdx, plan->sortOperators, plan->collations,
  ...
  if (plan->numCols > 0 || plan->groupingSets) {
    ...
    if (plan->groupingSets)
      ...
      show_sort_group_keys(outerPlanState(astate), "Group Key", plan->numCols,
                           plan->grpColIdx, NULL, NULL, NULL, ancestors, es);
...
static void show_grouping_sets(PlanState *planstate, Agg *agg, List *ancestors,
  ...
  show_grouping_set_keys(planstate, agg, NULL, context, useprefix, ancestors,
                         es);
  ...
  foreach (lc, agg->chain) {
    ...
    show_grouping_set_keys(planstate, aggnode, sortnode, context, useprefix,
                           ancestors, es);
  }
...
static void show_grouping_set_keys(PlanState *planstate, Agg *aggnode,
  ...
  if (sortnode) {
    show_sort_group_keys(planstate, "Sort Key", sortnode->numCols,
                         sortnode->sortColIdx, sortnode->sortOperators,
                         sortnode->collations, sortnode->nullsFirst, ancestors,
                         es);
    if (es->format == EXPLAIN_FORMAT_TEXT) es->indent++;
  }
  ...
  foreach (lc, gsets) {
    ...
    foreach (lc2, (List *)lfirst(lc)) {
      ...
      exprstr =
          deparse_expression((Node *)target->expr, context, useprefix, true);
  ...
  if (sortnode && es->format == EXPLAIN_FORMAT_TEXT) es->indent--;
  ...
  for (keyno = 0; keyno < nkeys; keyno++) {
    ...
    if (sortOperators != NULL)
      show_sortorder_options(&sortkeybuf, (Node *)target->expr,
                             sortOperators[keyno], collations[keyno],
                             nullsFirst[keyno]);
...
static void show_hash_info(HashState *hashstate, ExplainState *es) {
  ...
  if (hashtable) {
    ...
    if (es->format != EXPLAIN_FORMAT_TEXT) {
      ...
      ExplainPropertyLong("Original Hash Buckets", hashtable->nbuckets_original,
                          es);
      ExplainPropertyLong("Hash Batches", hashtable->nbatch, es);
      ExplainPropertyLong("Original Hash Batches", hashtable->nbatch_original,
                          es);
      ...
      appendStringInfoSpaces(es->str, es->indent * 2);
      appendStringInfo(es->str,
                       "Buckets: %d (originally %d)  Batches: %d (originally "
                       "%d)  Memory Usage: %ldkB\n",
...
static void show_instrumentation_count(const char *qlabel, int which,
  ...
  if (!es->analyze || !planstate->instrument) return;
...
static void ExplainTargetRel(Plan *plan, Index rti, ExplainState *es) {
    ...
    case T_FunctionScan: {
      ...
      if (list_length(fscan->functions) == 1) {
        ...
        if (IsA(rtfunc->funcexpr, FuncExpr)) {
          ...
          if (es->verbose)
            namescpace___ = get_namespace_name(get_func_namespace(funcid));
        }
      }
  ...
  if (es->format == EXPLAIN_FORMAT_TEXT) {
    ...
    if (namescpace___ != NULL) ExplainPropertyText("Schema", namescpace___, es);
...
static void show_modifytable_info(ModifyTableState *mtstate, List *ancestors,
  ...
  for (j = 0; j < mtstate->mt_nplans; j++) {
    ...
    if (fdwroutine && fdwroutine->ExplainForeignModify != NULL) {
      ...
      fdwroutine->ExplainForeignModify(mtstate, resultRelInfo, fdw_private, j,
                                       es);
    }
  ...
  if (node->onConflictAction != ONCONFLICT_NONE) {
     ...
     */
    if (idxNames) ExplainPropertyList("Conflict Arbiter Indexes", idxNames, es);
...
void ExplainPropertyList(const char *qlabel, List *data, ExplainState *es) {
  ...
  switch (es->format) {
    case EXPLAIN_FORMAT_TEXT:
      ...
      foreach (lc, data) {
        if (!first) appendStringInfoString(es->str, ", ");
 ...
 */
void ExplainPropertyListNested(const char *qlabel, List *data,
                               ExplainState *es) {
  ListCell *lc;
  bool first = true;
...
static void ExplainXMLTag(const char *tagname, int flags, ExplainState *es) {
  ...
  if ((flags & X_CLOSING) != 0) appendStringInfoCharMacro(es->str, '/');
  for (s = tagname; *s; s++)
    appendStringInfoCharMacro(es->str, (*s == ' ') ? '-' : *s);
  if ((flags & X_CLOSE_IMMEDIATE) != 0) appendStringInfoString(es->str, " /");
  appendStringInfoCharMacro(es->str, '>');
  if ((flags & X_NOWHITESPACE) == 0) appendStringInfoCharMacro(es->str, '\n');
}
