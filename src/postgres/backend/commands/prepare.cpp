...
void PrepareQuery(PrepareStmt *stmt, const char *queryString) {
  ...
  if (!stmt->name || stmt->name[0] == '\0')
    ereport(ERROR, (errcode(ERRCODE_INVALID_PSTATEMENT_DEFINITION),
   ...
   */
  plansource =
      CreateCachedPlan(stmt->query, queryString, CreateCommandTag(stmt->query));
   ...
   */
  query = parse_analyze_varparams((Node *)copyObject(stmt->query), queryString,
                                  &argtypes, &nargs);
  ...
  CompleteCachedPlan(plansource, query_list, NULL, argtypes, nargs, NULL, NULL,
...
void ExecuteQuery(ExecuteStmt *stmt, IntoClause *intoClause,
  ...
  if (intoClause) {
    ...
    if (!IsA(pstmt, PlannedStmt) || pstmt->commandType != CMD_SELECT ||
...
static ParamListInfo EvaluateParams(PreparedStatement *pstmt, List *params,
  ...
  foreach (l, params) {
    ...
    expr =
        coerce_to_target_type(pstate, expr, given_type_id, expected_type_id, -1,
    ...
    if (expr == NULL)
      ereport(ERROR,
              (errcode(ERRCODE_DATATYPE_MISMATCH),
               errmsg(
                   "parameter $%d of type %s cannot be coerced to the expected "
                   "type %s",
 ...
 * table entry.
 */
void StorePreparedStatement(const char *stmt_name, CachedPlanSource *plansource,
                            bool from_sql) {
  PreparedStatement *entry;
...
void DropAllPreparedStatements(void) {
  ...
  /* nothing cached */
  if (!prepared_queries) return;

  /* walk over cache */
  hash_seq_init(&seq, prepared_queries);
  while ((entry = static_cast<PreparedStatement *>(hash_seq_search(&seq))) !=
         NULL) {
    /* Release the plancache entry */
    DropCachedPlan(entry->plansource);
 ...
 */
void ExplainExecuteQuery(ExecuteStmt *execstmt, IntoClause *into,
  ...
  foreach (p, plan_list) {
    ...
    if (lnext(p) != NULL) ExplainSeparatePlans(es);
  }
...
Datum pg_prepared_statement(PG_FUNCTION_ARGS) {
  ...
  TupleDescInitEntry(tupdesc, (AttrNumber)3, "prepare_time", TIMESTAMPTZOID, -1,
                     0);
  TupleDescInitEntry(tupdesc, (AttrNumber)4, "parameter_types", REGTYPEARRAYOID,
                     -1, 0);
  ...
  if (prepared_queries) {
    ...
    while ((prep_stmt = static_cast<PreparedStatement *>(
