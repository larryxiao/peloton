...
void validateWithCheckOption(char *value) {
  if (value == NULL || (pg_strcasecmp(value, "local") != 0 &&
...
static void DefineViewRules(Oid viewOid, Query *viewParse, bool replace) {
   ...
   */
  DefineQueryRewrite(pstrdup(ViewSelectRuleName), viewOid, NULL, CMD_SELECT,
...
static Query *UpdateRangeTableOfViewParse(Oid viewOid, Query *viewParse) {
   ...
   * OLD first, then NEW....
   */
  rt_entry1 = addRangeTableEntryForRelation(
      pstate, viewRel, makeAlias("old", NIL), false, false);
  rt_entry2 = addRangeTableEntryForRelation(
      pstate, viewRel, makeAlias("new", NIL), false, false);
 ...
 */
ObjectAddress DefineView(ViewStmt *stmt, const char *queryString) {
  Query *viewParse;
  RangeVar *view;
  ListCell *cell;
  bool check_option;
   ...
   */
  viewParse =
      parse_analyze((Node *)copyObject(stmt->query), queryString, NULL, 0);

  /*
   * The grammar should ensure that the result is a single SELECT Query.
   * However, it doesn't forbid SELECT INTO, so we have to check for that.
   */
  if (!IsA(viewParse, Query)) elog(ERROR, "unexpected parse analysis result");
  ...
  if (stmt->withCheckOption == LOCAL_CHECK_OPTION)
    stmt->options =
        lappend(stmt->options,
                makeDefElem("check_option", (Node *)makeString("local")));
  else if (stmt->withCheckOption == CASCADED_CHECK_OPTION)
    stmt->options =
        lappend(stmt->options,
                makeDefElem("check_option", (Node *)makeString("cascaded")));
  ...
  foreach (cell, stmt->options) {
    ...
    if (pg_strcasecmp(defel->defname, "check_option") == 0) check_option = true;
  }
  ...
  if (view->relpersistence == RELPERSISTENCE_PERMANENT &&
    ...
    ereport(NOTICE,
            (errmsg("view \"%s\" will be a temporary view", view->relname)));
  }
   ...
   * aborted.
   */
  address = DefineVirtualRelation(view, viewParse->targetList, stmt->replace,
                                  stmt->options);
