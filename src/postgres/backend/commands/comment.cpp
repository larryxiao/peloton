...
void CreateComments(Oid oid, Oid classoid, int32 subid, char *comment) {
  ...
  ScanKeyInit(&skey[0], Anum_pg_description_objoid, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(oid));
  ScanKeyInit(&skey[1], Anum_pg_description_classoid, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(classoid));
  ScanKeyInit(&skey[2], Anum_pg_description_objsubid, BTEqualStrategyNumber,
              F_INT4EQ, Int32GetDatum(subid));

  description = heap_open(DescriptionRelationId, RowExclusiveLock);

  sd = systable_beginscan(description, DescriptionObjIndexId, true, NULL, 3,
                          skey);
...
void CreateSharedComments(Oid oid, Oid classoid, char *comment) {
  ...
  ScanKeyInit(&skey[0], Anum_pg_shdescription_objoid, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(oid));
  ScanKeyInit(&skey[1], Anum_pg_shdescription_classoid, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(classoid));
...
void DeleteComments(Oid oid, Oid classoid, int32 subid) {
  ...
  sd = systable_beginscan(description, DescriptionObjIndexId, true, NULL, nkeys,
                          skey);
...
char *GetComment(Oid oid, Oid classoid, int32 subid) {
  ...
  while ((tuple = systable_getnext(sd)) != NULL) {
    ...
    value =
        heap_getattr(tuple, Anum_pg_description_description, tupdesc, &isnull);
    if (!isnull) comment = TextDatumGetCString(value);
    break; /* Assume there can be only one match */
  }
