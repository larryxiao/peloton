 ...
 */
ObjectAddress DefineAggregate(List *name, List *args, bool oldstyle,
  ...
  if (mtransType != NULL) {
    ...
    if (minvtransfuncName == NIL)
      ereport(ERROR, (errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
                      errmsg(
                          "aggregate minvfunc must be specified when mstype is "
                          "specified")));
  } else {
    if (mtransfuncName != NIL)
      ereport(
  ...
  if (oldstyle) {
    ...
    if (baseType != NULL)
      ereport(ERROR, (errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
                      errmsg(
                          "basetype is redundant with aggregate input type "
                          "specification")));

    numArgs = list_length(args);
    interpret_function_parameter_list(
        args, InvalidOid, true, /* is an aggregate */
  ...
  return AggregateCreate(
      ...
      aggKind, numArgs, numDirectArgs, parameterTypes,
      PointerGetDatum(allParameterTypes), PointerGetDatum(parameterModes),
      PointerGetDatum(parameterNames), parameterDefaults, variadicArgType,
