...
ObjectAddress ExecSecLabelStmt(SecLabelStmt *stmt) {
  ...
  if (stmt->provider == NULL) {
    ...
    if (lnext(list_head(label_provider_list)) != NULL)
      ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                      errmsg(
                          "must specify provider when multiple security label "
  ...
  check_object_ownership(GetUserId(), stmt->objtype, address, stmt->objname,
...
static char *GetSharedSecurityLabel(const ObjectAddress *object,
  ...
  ScanKeyInit(&keys[0], Anum_pg_shseclabel_objoid, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(object->objectId));
  ScanKeyInit(&keys[1], Anum_pg_shseclabel_classoid, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(object->classId));
  ScanKeyInit(&keys[2], Anum_pg_shseclabel_provider, BTEqualStrategyNumber,
...
char *GetSecurityLabel(const ObjectAddress *object, const char *provider) {
  ...
  scan = systable_beginscan(pg_seclabel, SecLabelObjectIndexId, true, NULL, 4,
                            keys);
 ...
 */
void SetSecurityLabel(const ObjectAddress *object, const char *provider,
                      const char *label) {
  Relation pg_seclabel;
  ScanKeyData keys[4];
  SysScanDesc scan;
  ...
  if (HeapTupleIsValid(oldtup)) {
    if (label == NULL)
      ...
      newtup = heap_modify_tuple(oldtup, RelationGetDescr(pg_seclabel), values,
                                 nulls, replaces);
...
void DeleteSharedSecurityLabel(Oid objectId, Oid classId) {
  ...
  ScanKeyInit(&skey[0], Anum_pg_shseclabel_objoid, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(objectId));
  ScanKeyInit(&skey[1], Anum_pg_shseclabel_classoid, BTEqualStrategyNumber,
...
void DeleteSecurityLabel(const ObjectAddress *object) {
  ...
  ScanKeyInit(&skey[0], Anum_pg_seclabel_objoid, BTEqualStrategyNumber, F_OIDEQ,
              ObjectIdGetDatum(object->objectId));
  ScanKeyInit(&skey[1], Anum_pg_seclabel_classoid, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(object->classId));
  if (object->objectSubId != 0) {
    ScanKeyInit(&skey[2], Anum_pg_seclabel_objsubid, BTEqualStrategyNumber,
                F_INT4EQ, Int32GetDatum(object->objectSubId));
    nkeys = 3;
  } else
    nkeys = 2;
