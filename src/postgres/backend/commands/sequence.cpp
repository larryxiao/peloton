...
typedef struct sequence_magic { uint32 magic; } sequence_magic;
...
static Form_pg_sequence read_seq_tuple(SeqTable elm, Relation rel, Buffer *buf,
                                       HeapTuple seqtuple);
static void init_params(List *options, bool isInit, Form_pg_sequence new___,
                        List **owned_by);
 ...
 */
ObjectAddress DefineSequence(CreateSeqStmt *seq) {
  ...
  if (owned_by) process_owned_by(rel, owned_by);
...
static void fill_seq_with_data(Relation rel, HeapTuple tuple) {
  ...
  if (RelationNeedsWAL(rel)) GetTopTransactionId();
 ...
 */
ObjectAddress AlterSequence(AlterSeqStmt *stmt) {
  Oid relid;
  SeqTable elm;
  Relation seqrel;
  Buffer buf;
  ...
  /* check the comment above nextval_internal()'s equivalent call. */
  if (RelationNeedsWAL(seqrel)) GetTopTransactionId();
...
static int64 nextval_internal(Oid relid) {
  ...
  if (!seqrel->rd_islocaltemp) PreventCommandIfReadOnly("nextval()");
  ...
  while (fetch) /* try to fetch cache [+ log ] numbers */
  {
    ...
    if (incby > 0) {
      ...
      if ((maxv >= 0 && next > maxv - incby) ||
        ...
        if (rescnt > 0) break; /* stop fetching */
        if (!seq->is_cycled) {
          char buf[100];
   ...
   */
  if (logit && RelationNeedsWAL(seqrel)) GetTopTransactionId();
...
Datum currval_oid(PG_FUNCTION_ARGS) {
  ...
  if (pg_class_aclcheck(elm->relid, GetUserId(), ACL_SELECT | ACL_USAGE) !=
      ACLCHECK_OK)
    ereport(ERROR, (errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
                    errmsg("permission denied for sequence %s",
                           RelationGetRelationName(seqrel))));
...
static void do_setval(Oid relid, int64 next, bool iscalled) {
  ...
  /* read-only transactions may only modify temp sequences */
  if (!seqrel->rd_islocaltemp) PreventCommandIfReadOnly("setval()");
    ...
    ereport(
        ...
        (errcode(ERRCODE_NUMERIC_VALUE_OUT_OF_RANGE),
         errmsg(
             ...
             bufv, RelationGetRelationName(seqrel), bufm, bufx)));
  }
...
static void init_sequence(Oid relid, SeqTable *p_elm, Relation *p_rel) {
  ...
  if (seqhashtab == NULL) create_seq_hashtable();
...
static void process_owned_by(Relation seqrel, List *owned_by) {
  ...
  if (nnames == 1) {
    ...
    if (strcmp(strVal(linitial(owned_by)), "none") != 0)
      ereport(ERROR,
              (errcode(ERRCODE_SYNTAX_ERROR), errmsg("invalid OWNED BY option"),
  ...
  if (tablerel) relation_close(tablerel, NoLock);
}
...
void seq_redo(XLogReaderState *record) {
  ...
  if (info != XLOG_SEQ_LOG) elog(PANIC, "seq_redo: unknown op code %u", info);
  ...
  if (PageAddItem(localpage, (Item)item, itemsz, FirstOffsetNumber, false,
