...
static void RangeVarCallbackForPolicy(const RangeVar *rv, Oid relid,
...
static ArrayType *policy_role_list_to_array(List *roles) {
  ...
  foreach (cell, roles) {
    ...
    if (spec->roletype == ROLESPEC_PUBLIC) {
      ...
      temp_array[i++] = ObjectIdGetDatum(get_rolespec_oid((Node *)spec, false));
  }

  role_ids =
      construct_array(temp_array, num_roles, OIDOID, sizeof(Oid), true, 'i');
...
void RelationBuildRowSecurity(Relation relation) {
  ...
  PG_TRY();
  {
    ...
    rsdesc = static_cast<RowSecurityDesc *>(
        MemoryContextAllocZero(rscxt, sizeof(RowSecurityDesc)));
    rsdesc->rscxt = rscxt;
    ...
    while (HeapTupleIsValid(tuple = systable_getnext(sscan))) {
      ...
      if (isnull) elog(ERROR, "unexpected null value in pg_policy.polroles");
      ...
      policy->with_check_qual =
          static_cast<Expr *>(copyObject(with_check_qual));
      ...
      if (qual_expr != NULL) pfree(qual_expr);
      if (with_check_qual != NULL) pfree(with_check_qual);
    }
...
void RemovePolicyById(Oid policy_id) {
  ...
  sscan =
      systable_beginscan(pg_policy_rel, PolicyOidIndexId, true, NULL, 1, skey);
...
ObjectAddress CreatePolicy(CreatePolicyStmt *stmt) {
  ...
  table_id =
      RangeVarGetRelidExtended(stmt->table, AccessExclusiveLock, false, false,
  ...
  rte = addRangeTableEntryForRelation(qual_pstate, target_table, NULL, false,
                                      false);
  addRTEtoQuery(qual_pstate, rte, false, true, true);

  /* Add for the with-check quals */
  rte = addRangeTableEntryForRelation(with_check_pstate, target_table, NULL,
                                      false, false);
  addRTEtoQuery(with_check_pstate, rte, false, true, true);

  qual = transformWhereClause(qual_pstate,
                              static_cast<Node *>(copyObject(stmt->qual)),
                              EXPR_KIND_WHERE, "POLICY");

  with_check_qual = transformWhereClause(
      with_check_pstate, static_cast<Node *>(copyObject(stmt->with_check)),
      EXPR_KIND_WHERE, "POLICY");
  ...
  ScanKeyInit(&skey[0], Anum_pg_policy_polrelid, BTEqualStrategyNumber, F_OIDEQ,
              ObjectIdGetDatum(table_id));

  /* Set key - policy's name. */
  ScanKeyInit(&skey[1], Anum_pg_policy_polname, BTEqualStrategyNumber, F_NAMEEQ,
              CStringGetDatum(stmt->policy_name));

  sscan = systable_beginscan(pg_policy_rel, PolicyPolrelidPolnameIndexId, true,
                             NULL, 2, skey);
...
ObjectAddress AlterPolicy(AlterPolicyStmt *stmt) {
  ...
  /* Parse role_ids */
  if (stmt->roles != NULL) role_ids = policy_role_list_to_array(stmt->roles);
...
ObjectAddress rename_policy(RenameStmt *stmt) {
  ...
  table_id =
      RangeVarGetRelidExtended(stmt->relation, AccessExclusiveLock, false,
                               false, RangeVarCallbackForPolicy, (void *)stmt);
  ...
  InvokeObjectPostAlterHook(PolicyRelationId, HeapTupleGetOid(policy_tuple), 0);
