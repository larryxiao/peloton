...
void LockTableCommand(LockStmt *lockstmt) {
  ...
  foreach (p, lockstmt->relations) {
    ...
    reloid = RangeVarGetRelidExtended(
        rv, lockstmt->mode, false, lockstmt->nowait,
        RangeVarCallbackForLockTable, (void *)&lockstmt->mode);

    if (recurse) LockTableRecurse(reloid, lockstmt->mode, lockstmt->nowait);
  }
}
...
static void RangeVarCallbackForLockTable(const RangeVar *rv, Oid relid,
  ...
  if (!OidIsValid(relid)) return; /* doesn't exist, so no permissions check */
...
static void LockTableRecurse(Oid reloid, LOCKMODE lockmode, bool nowait) {
  ...
  foreach (lc, children) {
    ...
    if (aclresult != ACLCHECK_OK) {
      ...
      if (!relname) continue; /* child concurrently dropped, just skip it */
    ...
    if (!nowait)
      ...
      ereport(ERROR,
              (errcode(ERRCODE_LOCK_NOT_AVAILABLE),
               errmsg("could not obtain lock on relation \"%s\"", relname)));
    }
