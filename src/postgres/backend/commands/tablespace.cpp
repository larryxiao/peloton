...
void TablespaceCreateDbspace(Oid spcNode, Oid dbNode, bool isRedo) {
   ...
   */
  if (spcNode == GLOBALTABLESPACE_OID) return;
...
void DropTableSpace(DropTableSpaceStmt *stmt) {
  ...
  if (!HeapTupleIsValid(tuple)) {
    if (!stmt->missing_ok) {
      ...
      ereport(NOTICE, (errmsg("tablespace \"%s\" does not exist, skipping",
                              tablespacename)));
...
static void create_tablespace_directories(const char *location,
  ...
  location_with_version_dir =
      psprintf("%s/%s", location, TABLESPACE_VERSION_DIRECTORY);
    ...
    if (errno == ENOENT)
      ereport(
          ...
          (errcode(ERRCODE_UNDEFINED_FILE),
           ...
           InRecovery ? errhint(
                            "Create this directory for the tablespace before "
  ...
  while ((de = ReadDir(dirdesc, linkloc_with_version_dir)) != NULL) {
    ...
    if (rmdir(subfile) < 0)
      ereport(redo ? LOG : ERROR,
              (errcode_for_file_access(),
               errmsg("could not remove directory \"%s\": %m", subfile)));
...
ObjectAddress RenameTableSpace(const char *oldname, const char *newname) {
  ...
  ScanKeyInit(&entry[0], Anum_pg_tablespace_spcname, BTEqualStrategyNumber,
              F_NAMEEQ, CStringGetDatum(oldname));
  scan = heap_beginscan_catalog(rel, 1, entry);
  tup = heap_getnext(scan, ForwardScanDirection);
  if (!HeapTupleIsValid(tup))
    ereport(ERROR, (errcode(ERRCODE_UNDEFINED_OBJECT),
                    errmsg("tablespace \"%s\" does not exist", oldname)));
  ...
  ScanKeyInit(&entry[0], Anum_pg_tablespace_spcname, BTEqualStrategyNumber,
              F_NAMEEQ, CStringGetDatum(newname));
  scan = heap_beginscan_catalog(rel, 1, entry);
  tup = heap_getnext(scan, ForwardScanDirection);
  if (HeapTupleIsValid(tup))
    ereport(ERROR, (errcode(ERRCODE_DUPLICATE_OBJECT),
                    errmsg("tablespace \"%s\" already exists", newname)));
...
Oid AlterTableSpaceOptions(AlterTableSpaceOptionsStmt *stmt) {
  ...
  ScanKeyInit(&entry[0], Anum_pg_tablespace_spcname, BTEqualStrategyNumber,
              F_NAMEEQ, CStringGetDatum(stmt->tablespacename));
  scandesc = heap_beginscan_catalog(rel, 1, entry);
  tup = heap_getnext(scandesc, ForwardScanDirection);
  if (!HeapTupleIsValid(tup))
    ereport(ERROR,
            (errcode(ERRCODE_UNDEFINED_OBJECT),
             errmsg("tablespace \"%s\" does not exist", stmt->tablespacename)));
  ...
  newOptions = transformRelOptions(isnull ? (Datum)0 : datum, stmt->options,
                                   NULL, NULL, false, stmt->isReset);
  (void)tablespace_reloptions(newOptions, true);
  ...
  newtuple = heap_modify_tuple(tup, RelationGetDescr(rel), repl_val, repl_null,
                               repl_repl);
...
Oid GetDefaultTablespace(char relpersistence) {
   ...
   */
  if (result == MyDatabaseTableSpace) result = InvalidOid;
  return result;
}
...
bool check_temp_tablespaces(char **newval, void **extra, GucSource source) {
  ...
  if (IsTransactionState()) {
    ...
    myextra = static_cast<temp_tablespaces_extra *>(malloc(
        offsetof(temp_tablespaces_extra, tblSpcs) + numSpcs * sizeof(Oid)));
    if (!myextra) return false;
...
void PrepareTempTablespaces(void) {
  ...
  if (TempTablespacesAreSet()) return;
  ...
  foreach (l, namelist) {
    ...
    aclresult = pg_tablespace_aclcheck(curoid, GetUserId(), ACL_CREATE);
    if (aclresult != ACLCHECK_OK) continue;
...
char *get_tablespace_name(Oid spc_oid) {
  ...
  ScanKeyInit(&entry[0], ObjectIdAttributeNumber, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(spc_oid));
  scandesc = heap_beginscan_catalog(rel, 1, entry);
  tuple = heap_getnext(scandesc, ForwardScanDirection);
...
void tblspc_redo(XLogReaderState *record) {
  ...
  if (info == XLOG_TBLSPC_CREATE) {
    xl_tblspc_create_rec *xlrec =
        (xl_tblspc_create_rec *)XLogRecGetData(record);
