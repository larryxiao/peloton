...
static void intorel_startup(DestReceiver *self, int operation,
                            TupleDesc typeinfo,
...
int GetIntoRelEFlags(IntoClause *intoClause) {
  ...
  if (intoClause->skipData) flags |= EXEC_FLAG_WITH_NO_DATA;
...
static void intorel_startup(DestReceiver *self, int operation,
    ...
    if (!OidIsValid(col->collOid) && type_is_collatable(coltype->typeOid))
      ereport(
          ...
          (errcode(ERRCODE_INDETERMINATE_COLLATION),
           errmsg(
               "no collation was derived for column \"%s\" with collatable "
               "type %s",
  ...
  for (attnum = 1; attnum <= intoRelationDesc->rd_att->natts; attnum++)
    rte->insertedCols = bms_add_member(
        rte->insertedCols, attnum - FirstLowInvalidHeapAttributeNumber);
   ...
   */
  if (check_enable_rls(intoRelationAddr.objectId, InvalidOid, false) ==
      RLS_ENABLED)
   ...
   */
  myState->hi_options =
      HEAP_INSERT_SKIP_FSM | (XLogIsNeeded() ? 0 : HEAP_INSERT_SKIP_WAL);
...
static void intorel_receive(TupleTableSlot *slot, DestReceiver *self,
   ...
   */
  if (myState->rel->rd_rel->relhasoids) HeapTupleSetOid(tuple, InvalidOid);

  heap_insert(myState->rel, tuple, myState->output_cid, myState->hi_options,
              myState->bistate);
...
static void intorel_shutdown(DestReceiver *self) {
  ...
  if (myState->hi_options & HEAP_INSERT_SKIP_WAL) heap_sync(myState->rel);
 ...
 */
static void intorel_destroy(DestReceiver *self) { pfree(self); }
