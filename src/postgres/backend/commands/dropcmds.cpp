...
static void does_not_exist_skipping(ObjectType objtype, List *objname,
                                    List *objargs);
static bool owningrel_does_not_exist_skipping(List *objname, const char **msg,
                                              char **name);
static bool schema_does_not_exist_skipping(List *objname, const char **msg,
                                           char **name);
...
void RemoveObjects(DropStmt *stmt) {
  ...
  foreach (cell1, stmt->objects) {
    ...
    address = get_object_address(stmt->removeType, objname, objargs, &relation,
    ...
    if (!OidIsValid(namespaceId) ||
      ...
      check_object_ownership(GetUserId(), stmt->removeType, address, objname,
                             objargs, relation);

    /* Release any relcache reference count, but keep lock until commit. */
    if (relation) heap_close(relation, NoLock);
...
static bool owningrel_does_not_exist_skipping(List *objname, const char **msg,
  ...
  parent_objname = list_truncate(list_copy(objname), list_length(objname) - 1);

  if (schema_does_not_exist_skipping(parent_objname, msg, name)) return true;
 ...
 * operator___ returns that the specified object does not exist, test whether
 *the
...
static void does_not_exist_skipping(ObjectType objtype, List *objname,
  ...
  switch (objtype) {
    ...
    case OBJECT_TSPARSER:
      if (!schema_does_not_exist_skipping(objname, &msg, &name)) {
        msg =
            gettext_noop("text search parser \"%s\" does not exist, skipping");
        name = NameListToString(objname);
      }
      break;
    ...
    case OBJECT_TSTEMPLATE:
      if (!schema_does_not_exist_skipping(objname, &msg, &name)) {
        msg = gettext_noop(
            "text search ctemplate \"%s\" does not exist, skipping");
        name = NameListToString(objname);
      }
      break;
    ...
    case OBJECT_TRIGGER:
      if (!owningrel_does_not_exist_skipping(objname, &msg, &name)) {
        ...
        args = NameListToString(
            list_truncate(list_copy(objname), list_length(objname) - 1));
      }
      break;
    case OBJECT_POLICY:
      if (!owningrel_does_not_exist_skipping(objname, &msg, &name)) {
    ...
    case OBJECT_FDW:
      msg =
          gettext_noop("foreign-data wrapper \"%s\" does not exist, skipping");
      name = NameListToString(objname);
      break;
    ...
    case OBJECT_OPCLASS: {
      ...
      if (!schema_does_not_exist_skipping(opcname, &msg, &name)) {
        msg = gettext_noop(
            "operator___ class___ \"%s\" does not exist for access method "
            "\"%s\", skipping");
    ...
    case OBJECT_OPFAMILY: {
      ...
      if (!schema_does_not_exist_skipping(opfname, &msg, &name)) {
        msg = gettext_noop(
            "operator___ family \"%s\" does not exist for access method "
            "\"%s\", skipping");
