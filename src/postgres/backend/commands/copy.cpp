...
static CopyState BeginCopy(bool is_from, Relation rel, Node *raw_query,
                           const char *queryString, const Oid queryRelId,
                           List *attnamelist, List *options);
static void EndCopy(CopyState cstate);
static void ClosePipeToProgram(CopyState cstate);
static CopyState BeginCopyTo(Relation rel, Node *query, const char *queryString,
                             const Oid queryRelId, const char *filename,
...
static Datum CopyReadBinaryAttribute(CopyState cstate, int column_no,
                                     FmgrInfo *flinfo, Oid typioparam,
 ...
 */
static void CopySendData(CopyState cstate, const void *databuf, int datasize) {
  appendBinaryStringInfo(cstate->fe_msgbuf, static_cast<const char *>(databuf),
                         datasize);
}
...
/* Default line termination depends on platform */
    ...
    case COPY_OLD_FE:
      ...
      if (!cstate->binary) CopySendChar(cstate, '\n');
    ...
    case COPY_OLD_FE:
      ...
      if (pq_getbytes((char *)databuf, minread)) {
        ...
        ereport(ERROR, (errcode(ERRCODE_CONNECTION_FAILURE),
                        errmsg(
                            "unexpected EOF on client connection with an open "
                            "transaction")));
      }
      bytesread = minread;
      break;
...
static bool CopyLoadRawBuf(CopyState cstate) {
  ...
  inbytes =
      CopyGetData(cstate, cstate->raw_buf + nbytes, 1, RAW_BUF_SIZE - nbytes);
  nbytes += inbytes;
...
Oid DoCopy(const CopyStmt *stmt, const char *queryString, uint64 *processed) {
  ...
  if (is_from) {
    ...
    cstate = BeginCopyFrom(rel, stmt->filename, stmt->is_program, stmt->attlist,
                           stmt->options);
   ...
   */
  if (rel != NULL) heap_close(rel, (is_from ? NoLock : AccessShareLock));
...
void ProcessCopyOptions(CopyState cstate, bool is_from, List *options) {
  ...
  if (cstate == NULL) cstate = (CopyStateData *)palloc0(sizeof(CopyStateData));
  ...
  if (!cstate->delim)
    cstate->delim =
        cstate->csv_mode ? static_cast<char *>(",") : static_cast<char *>("\t");

  if (!cstate->null_print)
    cstate->null_print =
        cstate->csv_mode ? static_cast<char *>("") : static_cast<char *>("\\N");
  cstate->null_print_len = strlen(cstate->null_print);

  if (cstate->csv_mode) {
    if (!cstate->quote) cstate->quote = "\"";
    if (!cstate->escape) cstate->escape = cstate->quote;
  }
  ...
  if (strchr(cstate->null_print, '\r') != NULL ||
    ...
    ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                    errmsg(
                        "COPY null representation cannot use newline or "
                        "carriage return")));
   ...
   */
  if (!cstate->csv_mode &&
      strchr("\\.abcdefghijklmnopqrstuvwxyz0123456789", cstate->delim[0]) !=
          NULL)
    ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
...
static CopyState BeginCopy(bool is_from, Relation rel, Node *raw_query,
  ...
  cstate->copycontext = AllocSetContextCreate(
      CurrentMemoryContext, "COPY", ALLOCSET_DEFAULT_MINSIZE,
  ...
  if (rel) {
    ...
    cstate->queryDesc = CreateQueryDesc(plan, queryString, GetActiveSnapshot(),
  ...
  if (cstate->force_quote_all) {
    ...
    for (i = 0; i < num_phys_attrs; i++) cstate->force_quote_flags[i] = true;
...
static void ClosePipeToProgram(CopyState cstate) {
  ...
  if (pclose_rc == -1)
    ereport(ERROR, (errmsg("could not close pipe to external command: %m")));
  else if (pclose_rc != 0)
    ereport(ERROR, (errmsg("program \"%s\" failed", cstate->filename),
...
static CopyState BeginCopyTo(Relation rel, Node *query, const char *queryString,
                             const Oid queryRelId, const char *filename,
                             bool is_program, List *attnamelist,
                             List *options) {
  CopyState cstate;
  ...
  if (pipe) {
    ...
    if (whereToSendOutput != DestRemote) cstate->copy_file = stdout;
  } else {
...
static uint64 DoCopyTo(CopyState cstate) {
  ...
  PG_TRY();
  {
    if (fe_copy) SendCopyBegin(cstate);

    processed = CopyTo(cstate);

    if (fe_copy) SendCopyEnd(cstate);
  }
  PG_CATCH();
  {
    /*
...
static uint64 CopyTo(CopyState cstate) {
  ...
  cstate->rowcontext = AllocSetContextCreate(
      CurrentMemoryContext, "COPY TO", ALLOCSET_DEFAULT_MINSIZE,
      ALLOCSET_DEFAULT_INITSIZE, ALLOCSET_DEFAULT_MAXSIZE);
  ...
  if (cstate->binary) {
    ...
    if (cstate->need_transcoding)
      cstate->null_print_client = pg_server_to_any(
          cstate->null_print, cstate->null_print_len, cstate->file_encoding);
    ...
    if (cstate->header_line) {
      ...
      foreach (cur, cstate->attnumlist) {
        ...
        if (hdr_delim) CopySendChar(cstate, cstate->delim[0]);
        hdr_delim = true;
...
static void CopyOneRowTo(CopyState cstate, Oid tupleOid, Datum *values,
  ...
  foreach (cur, cstate->attnumlist) {
    ...
    if (!cstate->binary) {
      if (need_delim) CopySendChar(cstate, cstate->delim[0]);
      need_delim = true;
    }
...
void CopyFromErrorCallback(void *arg) {
  ...
  if (cstate->binary) {
    ...
    if (cstate->cur_attname)
      errcontext("COPY %s, line %d, column %s", cstate->cur_relname,
                 cstate->cur_lineno, cstate->cur_attname);
    else
      errcontext("COPY %s, line %d", cstate->cur_relname, cstate->cur_lineno);
  } else {
    if (cstate->cur_attname && cstate->cur_attval) {
      ...
      errcontext("COPY %s, line %d, column %s: \"%s\"", cstate->cur_relname,
                 cstate->cur_lineno, cstate->cur_attname, attval);
      pfree(attval);
    } else if (cstate->cur_attname) {
      /* error is relevant to a particular column, value is NULL */
      errcontext("COPY %s, line %d, column %s: null input", cstate->cur_relname,
                 cstate->cur_lineno, cstate->cur_attname);
    } else {
      /*
      ...
      if (cstate->line_buf_valid &&
        ...
        errcontext("COPY %s, line %d: \"%s\"", cstate->cur_relname,
...
#define MAX_COPY_DATA_DISPLAY 100
  ...
  if (slen <= MAX_COPY_DATA_DISPLAY) return pstrdup(str);
...
#define MAX_BUFFERED_TUPLES 1000
   ...
   * must be the case if we have a new___ table or new___ relfilenode, so we
   *need
  ...
  if ((resultRelInfo->ri_TrigDesc != NULL &&
    ...
    bufferedTuples = static_cast<HeapTupleData **>(
        palloc(MAX_BUFFERED_TUPLES * sizeof(HeapTuple)));
  }
  ...
  for (;;) {
    ...
    if (!NextCopyFrom(cstate, econtext, values, nulls, &loaded_oid)) break;

    /* And now we can form the input tuple. */
    tuple = heap_form_tuple(tupDesc, values, nulls);

    if (loaded_oid != InvalidOid) HeapTupleSetOid(tuple, loaded_oid);
    ...
    if (!skip_tuple) {
      ...
      if (useHeapMultiInsert) {
        ...
        if (nBufferedTuples == 0) firstBufferedLineNo = cstate->cur_lineno;
        ...
        if (resultRelInfo->ri_NumIndices > 0)
          recheckIndexes = ExecInsertIndexTuples(slot, &(tuple->t_self), estate,
                                                 false, NULL, NIL);
   ...
   */
  if (hi_options & HEAP_INSERT_SKIP_WAL) heap_sync(cstate->rel);
...
static void CopyFromInsertBatch(CopyState cstate, EState *estate,
  ...
  if (resultRelInfo->ri_NumIndices > 0) {
    for (i = 0; i < nBufferedTuples; i++) {
      ...
      recheckIndexes = ExecInsertIndexTuples(
          myslot, &(bufferedTuples[i]->t_self), estate, false, NULL, NIL);
   ...
   */
    ...
    for (i = 0; i < nBufferedTuples; i++) {
      cstate->cur_lineno = firstBufferedLineNo + i;
      ExecARInsertTriggers(estate, resultRelInfo, bufferedTuples[i], NIL);
    }
  }
...
bool NextCopyFromRawFields(CopyState cstate, char ***fields, int *nfields) {
  ...
  if (cstate->cur_lineno == 0 && cstate->header_line) {
    cstate->cur_lineno++;
    if (CopyReadLine(cstate)) return false; /* done */
  }
   ...
   */
  if (done && cstate->line_buf.len == 0) return false;
...
bool NextCopyFrom(CopyState cstate, ExprContext *econtext, Datum *values,
  ...
  if (!cstate->binary) {
    ...
    if (!NextCopyFromRawFields(cstate, &field_strings, &fldct)) return false;
    ...
    foreach (cur, cstate->attnumlist) {
      ...
      cstate->cur_attval = string;
      values[m] = InputFunctionCall(&in_functions[m], string, typioparams[m],
                                    attr[m]->atttypmod);
    ...
    if (file_has_oids) {
      ...
      if (isnull || loaded_oid == InvalidOid)
        ereport(ERROR, (errcode(ERRCODE_BAD_COPY_FILE_FORMAT),
                        errmsg("invalid OID in COPY data")));
      cstate->cur_attname = NULL;
      if (cstate->oids && tupleOid != NULL) *tupleOid = loaded_oid;
    }
    ...
    foreach (cur, cstate->attnumlist) {
      ...
      i++;
      values[m] =
          CopyReadBinaryAttribute(cstate, i, &in_functions[m], typioparams[m],
                                  attr[m]->atttypmod, &nulls[m]);
      cstate->cur_attname = NULL;
    }
  }
  ...
  for (i = 0; i < num_defaults; i++) {
    ...
    values[defmap[i]] =
        ExecEvalExpr(defexprs[i], econtext, &nulls[defmap[i]], NULL);
  }
...
static bool CopyReadLineText(CopyState cstate) {
  ...
  for (;;) {
    ...
    if (raw_buf_ptr >= copy_buf_len || need_data) {
       ...
       */
      if (!CopyLoadRawBuf(cstate)) hit_eof = true;
      raw_buf_ptr = 0;
      copy_buf_len = cstate->raw_buf_len;
            ...
            ereport(ERROR,
                    (errcode(ERRCODE_BAD_COPY_FILE_FORMAT),
                     ...
                     !cstate->csv_mode
                         ...
                         : errhint(
                               "Use quoted CSV field to represent carriage "
                               "return.")));
    ...
    if (c == '\n' && (!cstate->csv_mode || !in_quote)) {
      if (cstate->eol_type == EOL_CR || cstate->eol_type == EOL_CRNL)
        ereport(ERROR,
                (errcode(ERRCODE_BAD_COPY_FILE_FORMAT),
                 !cstate->csv_mode ? errmsg("literal newline found in data")
        ...
        if (cstate->eol_type == EOL_CRNL) {
          ...
          if (c2 == '\n') {
            if (!cstate->csv_mode)
              ereport(ERROR, (errcode(ERRCODE_BAD_COPY_FILE_FORMAT),
                              errmsg(
                                  "end-of-copy marker does not match previous "
                                  "newline style")));
            else
              NO_END_OF_COPY_GOTO;
          } else if (c2 != '\r') {
...
static int CopyReadAttributesText(CopyState cstate) {
  ...
  for (;;) {
    ...
    if (fieldno >= cstate->max_fields) {
      cstate->max_fields *= 2;
      cstate->raw_fields = static_cast<char **>(
      ...
      if (c == '\\') {
        ...
        switch (c) {
          ...
          case '7': {
            ...
            c = val & 0377;
            if (c == '\0' || IS_HIGHBIT_SET(c)) saw_non_ascii = true;
          } break;
          case 'x':
            /* Handle \x3F */
            if (cur_ptr < line_end_ptr) {
...
static Datum CopyReadBinaryAttribute(CopyState cstate, int column_no,
  ...
  result =
      ReceiveFunctionCall(flinfo, &cstate->attribute_buf, typioparam, typmod);
...
#define DUMPSOFAR()                                            \
  do {                                                         \
    if (ptr > start) CopySendData(cstate, start, ptr - start); \
  } while (0)
...
static void CopyAttributeOutCSV(CopyState cstate, char *string, bool use_quote,
  ...
  if (!use_quote && strcmp(string, cstate->null_print) == 0) use_quote = true;
...
static List *CopyGetAttnums(TupleDesc tupDesc, Relation rel,
  ...
  if (attnamelist == NIL) {
    ...
    for (i = 0; i < attr_count; i++) {
      if (attr[i]->attisdropped) continue;
 ...
 */
static void copy_dest_shutdown(DestReceiver *self) { /* no-op */ }

/*
 * copy_dest_destroy --- release DestReceiver object
 */
static void copy_dest_destroy(DestReceiver *self) { pfree(self); }
