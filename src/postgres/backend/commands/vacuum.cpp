 ...
 */
void vacuum(int options, RangeVar *relation, Oid relid, VacuumParams *params,
  ...
  if (false) {
    ...
    vac_context = AllocSetContextCreate(
        PortalContext, "Vacuum", ALLOCSET_DEFAULT_MINSIZE,
    ...
    if (use_own_xacts) {
      ...
      if (ActiveSnapshotSet()) PopActiveSnapshot();
    ...
    PG_TRY();
    {
      ...
      foreach (cur, relations) {
        ...
        if (options & VACOPT_VACUUM) {
          if (!vacuum_rel(relid, relation, options, params)) continue;
        }
        ...
        if (options & VACOPT_ANALYZE) {
          ...
          analyze_rel(relid, relation, options, params, va_cols, in_outer_xact,
                      vac_strategy);
 ...
 * not interested.
 */
void vacuum_set_xid_limits(Relation rel, int freeze_min_age,
                           int freeze_table_age, int multixact_freeze_min_age,
  ...
  *oldestXmin = GetOldestXmin(rel, true);
  ...
  if (freezemin < 0) freezemin = vacuum_freeze_min_age;
  ...
  if (!TransactionIdIsNormal(limit)) limit = FirstNormalTransactionId;
  ...
  if (!TransactionIdIsNormal(safeLimit)) safeLimit = FirstNormalTransactionId;
  ...
  *freezeLimit = limit;
  ...
  if (mxid_freezemin < 0) mxid_freezemin = vacuum_multixact_freeze_min_age;
  mxid_freezemin = Min(mxid_freezemin, effective_multixact_freeze_max_age / 2);
  Assert(mxid_freezemin >= 0);

  /* compute the cutoff multi, being careful to generate a valid value */
  mxactLimit = GetOldestMultiXactId() - mxid_freezemin;
  if (mxactLimit < FirstMultiXactId) mxactLimit = FirstMultiXactId;

  safeMxactLimit = ReadNextMultiXactId() - effective_multixact_freeze_max_age;
  if (safeMxactLimit < FirstMultiXactId) safeMxactLimit = FirstMultiXactId;

  if (MultiXactIdPrecedes(mxactLimit, safeMxactLimit)) {
    ereport(WARNING, (errmsg("oldest multixact is far in the past"),
                      errhint(
                          "Close open transactions with multixacts soon to "
  ...
  *multiXactCutoff = mxactLimit;
  ...
  if (xidFullScanLimit != NULL) {
    ...
    if (freezetable < 0) freezetable = vacuum_freeze_table_age;
    ...
    *xidFullScanLimit = limit;
    ...
    if (freezetable < 0) freezetable = vacuum_multixact_freeze_table_age;
 ...
 */
double vac_estimate_reltuples(Relation relation, bool is_analyze,
   ...
   * provided.)
   */
  if (scanned_pages == 0) return old_rel_tuples;
 ...
 *		This routine is shared by VACUUM and ANALYZE.
 */
void vac_update_relstats(Relation relation, BlockNumber num_pages,
  ...
  if (TransactionIdIsNormal(frozenxid) && pgcform->relfrozenxid != frozenxid &&
      (TransactionIdPrecedes(pgcform->relfrozenxid, frozenxid) ||
       TransactionIdPrecedes(ReadNewTransactionId(), pgcform->relfrozenxid))) {
...
void vac_update_datfrozenxid(void) {
  ...
  if (dirty) heap_inplace_update(relation, tuple);
  ...
  if (dirty || ForceTransactionIdLimitUpdate())
    vac_truncate_clog(newFrozenXid, newMinMulti, lastSaneFrozenXid,
                      lastSaneMinMulti);
}
...
static void vac_truncate_clog(TransactionId frozenXID, MultiXactId minMulti,
  ...
  if (frozenAlreadyWrapped) {
    ereport(WARNING, (errmsg(
                          "some databases have not been vacuumed in over 2 "
                          "billion transactions"),
                      errdetail(
                          "You might have already suffered "
...
static bool vacuum_rel(Oid relid, RangeVar *relation, int options,
   ...
   */
  lmode =
      (options & VACOPT_FULL) ? AccessExclusiveLock : ShareUpdateExclusiveLock;
  ...
  if (onerel->rd_rel->relkind != RELKIND_RELATION &&
    ...
    ereport(WARNING, (errmsg(
                         "skipping \"%s\" --- cannot vacuum non-tables or "
  ...
  if (options & VACOPT_FULL) {
    ...
    cluster_rel(relid, InvalidOid, false, (options & VACOPT_VERBOSE) != 0);
  } else
  ...
  if (onerel) relation_close(onerel, NoLock);
 ...
 */
void vac_open_indexes(Relation relation, LOCKMODE lockmode, int *nindexes,
                      Relation **Irel) {
  List *indexoidlist;
