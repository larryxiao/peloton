...
static Datum optionListToArray(List *options) {
  ...
  if (astate) return makeArrayResult(astate, CurrentMemoryContext);
 ...
 * FOREIGN TABLE.
 */
Datum transformGenericOptions(Oid catalogId, Datum oldOptions, List *options,
                              Oid fdwvalidator) {
 ...
 * superuser.
 */
static void AlterForeignDataWrapperOwner_internal(Relation rel, HeapTuple tup,
                                                  Oid newOwnerId) {
  ...
  if (form->fdwowner != newOwnerId) {
    ...
    repl_val[Anum_pg_foreign_data_wrapper_fdwowner - 1] =
        ObjectIdGetDatum(newOwnerId);

    aclDatum = heap_getattr(tup, Anum_pg_foreign_data_wrapper_fdwacl,
    ...
    if (!isNull) {
      ...
      repl_val[Anum_pg_foreign_data_wrapper_fdwacl - 1] =
    ...
    changeDependencyOnOwner(ForeignDataWrapperRelationId, HeapTupleGetOid(tup),
                            newOwnerId);
  }

  InvokeObjectPostAlterHook(ForeignDataWrapperRelationId, HeapTupleGetOid(tup),
                            0);
}
 ...
 */
static void AlterForeignServerOwner_internal(Relation rel, HeapTuple tup,
                                             Oid newOwnerId) {
  ...
  if (form->srvowner != newOwnerId) {
    ...
    repl_val[Anum_pg_foreign_server_srvowner - 1] =
        ObjectIdGetDatum(newOwnerId);

    aclDatum = heap_getattr(tup, Anum_pg_foreign_server_srvacl,
                            RelationGetDescr(rel), &isNull);
    /* Null ACLs do not require changes */
    if (!isNull) {
 ...
 */
static void parse_func_options(List *func_options, bool *handler_given,
                               Oid *fdwhandler, bool *validator_given,
                               Oid *fdwvalidator) {
  ListCell *cell;
...
ObjectAddress CreateForeignDataWrapper(CreateFdwStmt *stmt) {
  ...
  parse_func_options(stmt->func_options, &handler_given, &fdwhandler,
                     &validator_given, &fdwvalidator);

  values[Anum_pg_foreign_data_wrapper_fdwhandler - 1] =
      ObjectIdGetDatum(fdwhandler);
  values[Anum_pg_foreign_data_wrapper_fdwvalidator - 1] =
      ObjectIdGetDatum(fdwvalidator);

  nulls[Anum_pg_foreign_data_wrapper_fdwacl - 1] = true;

  fdwoptions = transformGenericOptions(ForeignDataWrapperRelationId,
                                       PointerGetDatum(NULL), stmt->options,
                                       fdwvalidator);
...
ObjectAddress AlterForeignDataWrapper(AlterFdwStmt *stmt) {
  ...
  if (handler_given) {
    repl_val[Anum_pg_foreign_data_wrapper_fdwhandler - 1] =
        ObjectIdGetDatum(fdwhandler);
    ...
    ereport(WARNING, (errmsg(
                         "changing the foreign-data wrapper handler can change "
                         "behavior of existing foreign tables")));
  }

  if (validator_given) {
    repl_val[Anum_pg_foreign_data_wrapper_fdwvalidator - 1] =
        ObjectIdGetDatum(fdwvalidator);
  ...
  if (stmt->options) {
    ...
    datum = SysCacheGetAttr(FOREIGNDATAWRAPPEROID, tp,
  ...
  tp = heap_modify_tuple(tp, RelationGetDescr(rel), repl_val, repl_null,
                         repl_repl);
...
ObjectAddress CreateForeignServer(CreateForeignServerStmt *stmt) {
  ...
  /* Add server options */
  srvoptions =
      transformGenericOptions(ForeignServerRelationId, PointerGetDatum(NULL),
...
ObjectAddress AlterForeignServer(AlterForeignServerStmt *stmt) {
  ...
  tp =
      SearchSysCacheCopy1(FOREIGNSERVERNAME, CStringGetDatum(stmt->servername));
 ...
 */
static void user_mapping_ddl_aclcheck(Oid umuserid, Oid serverid,
...
ObjectAddress CreateUserMapping(CreateUserMappingStmt *stmt) {
   ...
   */
  umId = GetSysCacheOid2(USERMAPPINGUSERSERVER, ObjectIdGetDatum(useId),
  ...
  /* Add user options */
  useoptions =
      transformGenericOptions(UserMappingRelationId, PointerGetDatum(NULL),
                              stmt->options, fdw->fdwvalidator);
...
ObjectAddress AlterUserMapping(AlterUserMappingStmt *stmt) {
  ...
  if (stmt->options) {
    ...
    /* Prepare the options array */
    datum = transformGenericOptions(UserMappingRelationId, datum, stmt->options,
                                    fdw->fdwvalidator);
...
void CreateForeignTable(CreateForeignTableStmt *stmt, Oid relid) {
  ...
  values[Anum_pg_foreign_table_ftserver - 1] =
      ObjectIdGetDatum(server->serverid);
  /* Add table generic options */
  ftoptions =
      transformGenericOptions(ForeignTableRelationId, PointerGetDatum(NULL),
                              stmt->options, fdw->fdwvalidator);
...
void ImportForeignSchema(ImportForeignSchemaStmt *stmt) {
  ...
  aclresult =
      pg_foreign_server_aclcheck(server->serverid, GetUserId(), ACL_USAGE);
  if (aclresult != ACLCHECK_OK)
    aclcheck_error(aclresult, ACL_KIND_FOREIGN_SERVER, server->servername);
  ...
  if (fdw_routine->ImportForeignSchema == NULL)
    ereport(ERROR, (errcode(ERRCODE_FDW_NO_SCHEMAS),
                    errmsg(
                        "foreign-data wrapper \"%s\" does not support IMPORT "
                        "FOREIGN SCHEMA",
                        fdw->fdwname)));
  ...
  foreach (lc, cmd_list) {
    ...
    foreach (lc2, raw_parsetree_list) {
      CreateForeignTableStmt *cstmt =
          static_cast<CreateForeignTableStmt *>(lfirst(lc2));
      ...
      /* Execute statement */
      ProcessUtility((Node *)cstmt, cmd, PROCESS_UTILITY_SUBCOMMAND, NULL,
                     None_Receiver, NULL);
