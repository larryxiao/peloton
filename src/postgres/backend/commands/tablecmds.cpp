 ...
 * Phase 3 copy (this could be either a new___ column with a non-null default,
 * or
...
static const struct dropmsgstrings dropmsgstringarray[] = {
    {RELKIND_RELATION, ERRCODE_UNDEFINED_TABLE,
    ...
    {RELKIND_VIEW, ERRCODE_UNDEFINED_TABLE,
     gettext_noop("view \"%s\" does not exist"),
     gettext_noop("view \"%s\" does not exist, skipping"),
     gettext_noop("\"%s\" is not a view"),
     gettext_noop("Use DROP VIEW to remove a view.")},
    {RELKIND_MATVIEW, ERRCODE_UNDEFINED_TABLE,
    ...
    {RELKIND_INDEX, ERRCODE_UNDEFINED_OBJECT,
     gettext_noop("index \"%s\" does not exist"),
     gettext_noop("index \"%s\" does not exist, skipping"),
     gettext_noop("\"%s\" is not an index"),
     gettext_noop("Use DROP INDEX to remove an index.")},
    {RELKIND_COMPOSITE_TYPE, ERRCODE_UNDEFINED_OBJECT,
     gettext_noop("type \"%s\" does not exist"),
     gettext_noop("type \"%s\" does not exist, skipping"),
     gettext_noop("\"%s\" is not a type"),
     gettext_noop("Use DROP TYPE to remove a type.")},
    {RELKIND_FOREIGN_TABLE, ERRCODE_UNDEFINED_OBJECT,
...
static void AlterIndexNamespaces(Relation classRel, Relation rel, Oid oldNspOid,
                                 Oid newNspOid, ObjectAddresses *objsMoved);
static void AlterSeqNamespaces(Relation classRel, Relation rel, Oid oldNspOid,
                               Oid newNspOid, ObjectAddresses *objsMoved,
                               LOCKMODE lockmode);
...
static ObjectAddress ATExecValidateConstraint(Relation rel, char *constrName,
                                              bool recurse, bool recursing,
                                              LOCKMODE lockmode);
static int transformColumnNameList(Oid relId, List *colList, int16 *attnums,
                                   Oid *atttypids);
static int transformFkeyGetPrimaryKey(Relation pkrel, Oid *indexOid,
                                      List **attnamelist, int16 *attnums,
                                      Oid *atttypids, Oid *opclasses);
...
static void validateForeignKeyConstraint(char *conname, Relation rel,
                                         Relation pkrel, Oid pkindOid,
                                         Oid constraintOid);
...
static void ATSimpleRecursion(List **wqueue, Relation rel, AlterTableCmd *cmd,
                              bool recurse, LOCKMODE lockmode);
static void ATTypedTableRecursion(List **wqueue, Relation rel,
                                  AlterTableCmd *cmd, LOCKMODE lockmode);
static List *find_typed_table_dependencies(Oid typeOid, const char *typeName,
                                           DropBehavior behavior);
static void ATPrepAddColumn(List **wqueue, Relation rel, bool recurse,
                            bool recursing, bool is_view, AlterTableCmd *cmd,
                            LOCKMODE lockmode);
static ObjectAddress ATExecAddColumn(List **wqueue, AlteredTableInfo *tab,
                                     Relation rel, ColumnDef *colDef,
                                     bool isOid, bool recurse, bool recursing,
                                     LOCKMODE lockmode);
static void check_for_column_name_collision(Relation rel, const char *colname);
static void add_column_datatype_dependency(Oid relid, int32 attnum, Oid typid);
static void add_column_collation_dependency(Oid relid, int32 attnum,
                                            Oid collid);
static void ATPrepAddOids(List **wqueue, Relation rel, bool recurse,
                          AlterTableCmd *cmd, LOCKMODE lockmode);
static ObjectAddress ATExecDropNotNull(Relation rel, const char *colName,
                                       LOCKMODE lockmode);
...
static void ATPrepDropColumn(List **wqueue, Relation rel, bool recurse,
                             bool recursing, AlterTableCmd *cmd,
                             LOCKMODE lockmode);
static ObjectAddress ATExecDropColumn(List **wqueue, Relation rel,
                                      const char *colName,
                                      DropBehavior behavior, bool recurse,
                                      bool recursing, bool missing_ok,
                                      LOCKMODE lockmode);
static ObjectAddress ATExecAddIndex(AlteredTableInfo *tab, Relation rel,
                                    IndexStmt *stmt, bool is_rebuild,
                                    LOCKMODE lockmode);
static ObjectAddress ATExecAddConstraint(List **wqueue, AlteredTableInfo *tab,
                                         Relation rel,
                                         Constraint *newConstraint,
                                         bool recurse, bool is_readd,
                                         LOCKMODE lockmode);
static ObjectAddress ATExecAddIndexConstraint(AlteredTableInfo *tab,
                                              Relation rel, IndexStmt *stmt,
                                              LOCKMODE lockmode);
static ObjectAddress ATAddCheckConstraint(List **wqueue, AlteredTableInfo *tab,
...
static void ATExecDropConstraint(Relation rel, const char *constrName,
                                 ...
                                 bool recursing, bool missing_ok,
                                 LOCKMODE lockmode);
static void ATPrepAlterColumnType(List **wqueue, AlteredTableInfo *tab,
                                  Relation rel, bool recurse, bool recursing,
                                  AlterTableCmd *cmd, LOCKMODE lockmode);
...
static void change_owner_fix_column_acls(Oid relationOid, Oid oldOwnerId,
                                         Oid newOwnerId);
...
static void ATExecSetRelOptions(Relation rel, List *defList,
                                AlterTableType operation, LOCKMODE lockmode);
...
static ObjectAddress ATExecAddInherit(Relation child_rel, RangeVar *parent,
                                      LOCKMODE lockmode);
static ObjectAddress ATExecDropInherit(Relation rel, RangeVar *parent,
                                       LOCKMODE lockmode);
static void drop_parent_dependency(Oid relid, Oid refclassid, Oid refobjid);
static ObjectAddress ATExecAddOf(Relation rel, const TypeName *ofTypename,
                                 LOCKMODE lockmode);
static void ATExecDropOf(Relation rel, LOCKMODE lockmode);
static void ATExecReplicaIdentity(Relation rel, ReplicaIdentityStmt *stmt,
                                  LOCKMODE lockmode);
...
ObjectAddress DefineRelation(CreateStmt *stmt, char relkind, Oid ownerId,
  ...
  if (stmt->relation->relpersistence == RELPERSISTENCE_TEMP &&
    ...
    ereport(ERROR, (errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
                    errmsg(
                        "cannot create temporary table within "
  ...
  if (!OidIsValid(ownerId)) ownerId = GetUserId();
  ...
  if (stmt->ofTypename) {
    ...
    if (aclresult != ACLCHECK_OK) aclcheck_error_type(aclresult, ofTypeId);
  } else
    ofTypeId = InvalidOid;
  ...
  schema = MergeAttributes(schema, stmt->inhRelations,
                           stmt->relation->relpersistence, &inheritOids,
  ...
  relationId = heap_create_with_catalog(
      relname, namespaceId, tablespaceId, InvalidOid, InvalidOid, ofTypeId,
      ownerId, descriptor, list_concat(cookedDefaults, old_constraints),
      relkind, stmt->relation->relpersistence, false, false, localHasOids,
      parentOidCount, stmt->oncommit, reloptions, true, allowSystemTableMods,
      false, typaddress);
  ...
  if (rawDefaults || stmt->constraints)
    AddRelationNewConstraints(rel, rawDefaults, stmt->constraints, true, true,
                              false);
 ...
 */
static void DropErrorMsgNonExistent(RangeVar *rel, char rightkind,
                                    bool missing_ok) {
  ...
  if (rel->schemaname != NULL &&
    ...
    if (!missing_ok) {
      ...
      ereport(NOTICE, (errmsg("schema \"%s\" does not exist, skipping",
                              rel->schemaname)));
    }
    return;
  }
...
static void DropErrorMsgWrongType(const char *relname, char wrongkind,
  ...
  ereport(
      ERROR,
      (errcode(ERRCODE_WRONG_OBJECT_TYPE), errmsg(rentry->nota_msg, relname),
  ...
  if (drop->concurrent) {
    ...
    if (list_length(drop->objects) != 1)
      ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                      errmsg(
                          "DROP INDEX CONCURRENTLY does not support dropping "
                          "multiple objects")));
...
void PelotonRemoveRelations(DropStmt *drop) {
  ObjectAddresses *objects;
  char relkind;
  ListCell *cell;
  int flags = 0;
...
void ExecuteTruncate(TruncateStmt *stmt) {
  ...
  foreach (cell, stmt->relations) {
    ...
    if (recurse) {
      ...
      foreach (child, children) {
        ...
        if (list_member_oid(relids, childrelid)) continue;
  ...
  if (stmt->behavior == DROP_CASCADE) {
    for (;;) {
      ...
      if (newrelids == NIL) break; /* nothing else to add */
      ...
      foreach (cell, newrelids) {
        ...
        ereport(NOTICE, (errmsg("truncate cascades to table \"%s\"",
 ...
 */
...
#else
  if (stmt->behavior == DROP_RESTRICT) heap_truncate_check_FKs(rels, false);
#endif
  ...
  resultRelInfos =
      (ResultRelInfo *)palloc(list_length(rels) * sizeof(ResultRelInfo));
  resultRelInfo = resultRelInfos;
  foreach (cell, rels) {
    Relation rel = (Relation)lfirst(cell);
  ...
  foreach (cell, rels) {
    ...
    if (rel->rd_createSubid == mySubid ||
       ...
       * deletion at commit.
       */
      RelationSetNewRelfilenode(rel, rel->rd_rel->relpersistence, RecentXmin,
                                minmulti);
...
static void truncate_check_rel(Relation rel) {
  ...
  /* Permissions checks */
  aclresult =
      pg_class_aclcheck(RelationGetRelid(rel), GetUserId(), ACL_TRUNCATE);
  if (aclresult != ACLCHECK_OK)
...
static List *MergeAttributes(List *schema, List *supers, char relpersistence,
  ...
  static Node bogus_marker = {
      static_cast<NodeTag>(0)}; /* marks conflicting defaults */
  ...
  foreach (entry, schema) {
    ...
    if (coldef->typeName == NULL)
      ...
      ereport(ERROR, (errcode(ERRCODE_UNDEFINED_COLUMN),
                      errmsg("column \"%s\" does not exist", coldef->colname)));
  ...
  foreach (entry, supers) {
    ...
    if (relation->rd_rel->relhasoids) parentsWithOids++;
    ...
    for (parent_attno = 1; parent_attno <= tupleDesc->natts; parent_attno++) {
       ...
       */
      if (attribute->attisdropped) continue; /* leave newattno entry as zero */
          ...
          ereport(ERROR,
                  (errcode(ERRCODE_DATATYPE_MISMATCH),
                   errmsg("inherited column \"%s\" has a type conflict",
                          attributeName),
                   errdetail("%s versus %s", TypeNameToString(def->typeName),
          ...
          ereport(
              ...
              (errcode(ERRCODE_COLLATION_MISMATCH),
               errmsg("inherited column \"%s\" has a collation conflict",
                      attributeName),
               errdetail("\"%s\" versus \"%s\"", get_collation_name(defCollId),
    ...
    if (constr && constr->num_check > 0) {
      ...
      for (i = 0; i < constr->num_check; i++) {
        ...
        if (check[i].ccnoinherit) continue;

        /* Adjust Vars to match new___ table's column numbering */
        expr = map_variable_attnos(
            static_cast<Node *>(stringToNode(check[i].ccbin)), 1, 0, newattno,
        ...
        if (found_whole_row)
          ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                          ...
                          errdetail(
                              "Constraint \"%s\" contains a whole-row "
      ...
      if (exist_attno > 0) {
        ...
        if (exist_attno == schema_attno)
          ...
          ereport(
              NOTICE,
              (errmsg(
                   "moving and merging column \"%s\" with inherited definition",
                   attributeName),
               errdetail(
                   "User-specified column moved to the position of the "
                   "inherited column.")));
        def = (ColumnDef *)list_nth(inhSchema, exist_attno - 1);
        typenameTypeIdAndMod(NULL, def->typeName, &defTypeId, &deftypmod);
...
static bool MergeCheckConstraint(List *constraints, char *name, Node *expr) {
  ...
  foreach (lc, constraints) {
    ...
    if (strcmp(ccon->name, name) != 0) continue;
    ...
    ereport(ERROR, (errcode(ERRCODE_DUPLICATE_OBJECT),
                    errmsg(
                        "check constraint name \"%s\" appears multiple times "
...
static void StoreCatalogInheritance1(Oid relationId, Oid parentOid,
   ...
   */
  InvokeObjectPostAlterHookArg(InheritsRelationId, relationId, 0, parentOid,
                               false);
...
static int findAttrByName(const char *attributeName, List *schema) {
  ...
  foreach (s, schema) {
    ...
    if (strcmp(attributeName, def->colname) == 0) return i;
 ...
 */
static void renameatt_check(Oid myrelid, Form_pg_class classform,
                            bool recursing) {
  ...
  if (relkind != RELKIND_RELATION && relkind != RELKIND_VIEW &&
    ...
    ereport(ERROR, (errcode(ERRCODE_WRONG_OBJECT_TYPE),
                    errmsg(
                        "\"%s\" is not a table, view, materialized view, "
                        "composite type, index, or foreign table",
 ...
 */
static AttrNumber renameatt_internal(Oid myrelid, const char *oldattname,
  ...
  if (recurse) {
     ...
     */
    child_oids =
        find_all_inheritors(myrelid, AccessExclusiveLock, &child_numparents);
    ...
    forboth(lo, child_oids, li, child_numparents) {
      ...
      renameatt_internal(childrelid, oldattname, newattname, false, true,
                         numparents, behavior);
    }
  } else {
    /*
  ...
  if (targetrelation->rd_rel->relkind == RELKIND_COMPOSITE_TYPE) {
    ...
    foreach (lo, child_oids)
      renameatt_internal(lfirst_oid(lo), oldattname, newattname, true, true, 0,
                         behavior);
  }
 ...
 */
static void RangeVarCallbackForRenameAttribute(const RangeVar *rv, Oid relid,
                                               Oid oldrelid, void *arg) {
  HeapTuple tuple;
  Form_pg_class form;

  tuple = SearchSysCache1(RELOID, ObjectIdGetDatum(relid));
  if (!HeapTupleIsValid(tuple)) return; /* concurrently dropped */
 ...
 */
static ObjectAddress rename_constraint_internal(Oid myrelid, Oid mytypid,
  ...
  if (myrelid && con->contype == CONSTRAINT_CHECK && !con->connoinherit) {
    if (recurse) {
      ...
      if (expected_parents == 0 &&
        ...
        ereport(ERROR, (errcode(ERRCODE_INVALID_TABLE_DEFINITION),
                        errmsg(
                            "inherited constraint \"%s\" must be renamed in "
                            "child tables too",
                            oldconname)));
    }
...
ObjectAddress RenameConstraint(RenameStmt *stmt) {
  ...
  return rename_constraint_internal(
      relid, typid, stmt->subname, stmt->newname,
...
ObjectAddress RenameRelation(RenameStmt *stmt) {
  ...
  relid = RangeVarGetRelidExtended(
      stmt->relation, AccessExclusiveLock, stmt->missing_ok, false,
      RangeVarCallbackForAlterRelation, (void *)stmt);
...
void RenameRelationInternal(Oid myrelid, const char *newrelname,
  ...
  InvokeObjectPostAlterHookArg(RelationRelationId, myrelid, 0, InvalidOid,
                               is_internal);
...
Oid AlterTableLookupRelation(AlterTableStmt *stmt, LOCKMODE lockmode) {
  return RangeVarGetRelidExtended(stmt->relation, lockmode, stmt->missing_ok,
...
void AlterTable(Oid relid, LOCKMODE lockmode, AlterTableStmt *stmt) {
  ...
  ATController(stmt, rel, stmt->cmds,
               interpretInhOption(stmt->relation->inhOpt), lockmode);
}
...
AlterTableGetLockLevel(List *cmds) {
  ...
  foreach (lcmd, cmds) {
     ...
     */
    if (cmd_lockmode > lockmode) lockmode = cmd_lockmode;
  }
...
static void ATPrepCmd(List **wqueue, Relation rel, AlterTableCmd *cmd,
  ...
  switch (cmd->subtype) {
    ...
    case AT_ResetOptions: /* ALTER COLUMN RESET ( options ) */
      ATSimplePermissions(
          rel, ATT_TABLE | ATT_MATVIEW | ATT_INDEX | ATT_FOREIGN_TABLE);
    ...
    case AT_AddConstraint: /* ADD CONSTRAINT */
      ...
      if (recurse) cmd->subtype = AT_AddConstraintRecurse;
      pass = AT_PASS_ADD_CONSTR;
      break;
    ...
    case AT_DropConstraint: /* DROP CONSTRAINT */
      ...
      /* Recursion occurs during execution phase */
      /* No command-specific prep needed except saving recurse flag */
      if (recurse) cmd->subtype = AT_DropConstraintRecurse;
      pass = AT_PASS_DROP;
      break;
    ...
    case AT_ValidateConstraint: /* VALIDATE CONSTRAINT */
      ...
      /* Recursion occurs during execution phase */
      /* No command-specific prep needed except saving recurse flag */
      if (recurse) cmd->subtype = AT_ValidateConstraintRecurse;
      pass = AT_PASS_MISC;
      break;
...
static void ATExecCmd(List **wqueue, AlteredTableInfo *tab, Relation rel,
  ...
  switch (cmd->subtype) {
    ...
    case AT_AddColumnToView: /* add column via CREATE OR REPLACE
                              * VIEW */
      address = ATExecAddColumn(wqueue, tab, rel, (ColumnDef *)cmd->def, false,
    ...
    case AT_SetStorage: /* ALTER COLUMN SET STORAGE */
      ...
      break;
    case AT_DropColumn: /* DROP COLUMN */
      address = ATExecDropColumn(wqueue, rel, cmd->name, cmd->behavior, false,
                                 false, cmd->missing_ok, lockmode);
      break;
    case AT_DropColumnRecurse: /* DROP COLUMN with recursion */
      address = ATExecDropColumn(wqueue, rel, cmd->name, cmd->behavior, true,
                                 false, cmd->missing_ok, lockmode);
      break;
    case AT_AddIndex: /* ADD INDEX */
      address =
    ...
    case AT_ChangeOwner: /* ALTER OWNER */
      ATExecChangeOwner(RelationGetRelid(rel),
                        get_rolespec_oid(cmd->newowner, false), false,
                        lockmode);
      break;
    case AT_ClusterOn: /* CLUSTER ON */
    ...
    case AT_AddOids: /* SET WITH OIDS */
      ...
      if (cmd->def != NULL)
        address = ATExecAddColumn(wqueue, tab, rel, (ColumnDef *)cmd->def, true,
                                  false, false, lockmode);
      break;
    ...
    case AT_EnableTrig: /* ENABLE TRIGGER name */
      ATExecEnableDisableTrigger(rel, cmd->name, TRIGGER_FIRES_ON_ORIGIN, false,
                                 lockmode);
      break;
    case AT_EnableAlwaysTrig: /* ENABLE ALWAYS TRIGGER name */
      ATExecEnableDisableTrigger(rel, cmd->name, TRIGGER_FIRES_ALWAYS, false,
                                 lockmode);
      break;
    case AT_EnableReplicaTrig: /* ENABLE REPLICA TRIGGER name */
      ATExecEnableDisableTrigger(rel, cmd->name, TRIGGER_FIRES_ON_REPLICA,
                                 false, lockmode);
      break;
    case AT_DisableTrig: /* DISABLE TRIGGER name */
      ATExecEnableDisableTrigger(rel, cmd->name, TRIGGER_DISABLED, false,
                                 lockmode);
      break;
    case AT_EnableTrigAll: /* ENABLE TRIGGER ALL */
      ATExecEnableDisableTrigger(rel, NULL, TRIGGER_FIRES_ON_ORIGIN, false,
                                 lockmode);
      break;
    ...
    case AT_EnableTrigUser: /* ENABLE TRIGGER USER */
      ATExecEnableDisableTrigger(rel, NULL, TRIGGER_FIRES_ON_ORIGIN, true,
                                 lockmode);
      break;
...
static void ATRewriteTables(AlterTableStmt *parsetree, List **wqueue,
  ...
  foreach (ltab, *wqueue) {
    ...
    if (tab->relkind == RELKIND_FOREIGN_TABLE) continue;
    ...
    if (tab->rewrite > 0) {
       ...
       */
      persistence = tab->chgPersistence ? tab->newrelpersistence
       ...
       */
      OIDNewHeap =
          make_new_heap(tab->relid, NewTableSpace, persistence, lockmode);
...
static void ATRewriteTable(AlteredTableInfo *tab, Oid OIDNewHeap,
  ...
  if (newrel) {
    ...
    if (!XLogIsNeeded()) hi_options |= HEAP_INSERT_SKIP_WAL;
  } else {
  ...
  if (newrel || tab->new_notnull) {
    ...
    if (notnull_attrs) needscan = true;
  }
  ...
  if (newrel || needscan) {
    ...
    if (newrel)
      ereport(DEBUG1, (errmsg("rewriting table \"%s\"",
                              RelationGetRelationName(oldrel))));
    else
      ereport(DEBUG1, (errmsg("verifying table \"%s\"",
                              RelationGetRelationName(oldrel))));
    ...
    while ((tuple = heap_getnext(scan, ForwardScanDirection)) != NULL) {
      if (tab->rewrite > 0) {
        ...
        if (oldTupDesc->tdhasoid) tupOid = HeapTupleGetOid(tuple);
        ...
        foreach (l, tab->newvals) {
          ...
          values[ex->attnum - 1] = ExecEvalExpr(ex->exprstate, econtext,
        ...
        /* Preserve OID, if any */
        if (newTupDesc->tdhasoid) HeapTupleSetOid(tuple, tupOid);
      ...
      if (newrel) heap_insert(newrel, tuple, mycid, hi_options, bistate);
  ...
  if (newrel) {
    ...
    if (hi_options & HEAP_INSERT_SKIP_WAL) heap_sync(newrel);
...
static void ATWrongRelkindError(Relation rel, int allowed_targets) {
  ...
  switch (allowed_targets) {
    ...
    case ATT_TABLE | ATT_MATVIEW | ATT_INDEX | ATT_FOREIGN_TABLE:
      msg =
          _("\"%s\" is not a table, materialized view, composite type, or "
            "foreign table");
      break;
    case ATT_VIEW:
...
static void ATTypedTableRecursion(List **wqueue, Relation rel,
  ...
  children = find_typed_table_dependencies(
      rel->rd_rel->reltype, RelationGetRelationName(rel), cmd->behavior);
...
void find_composite_type_dependencies(Oid typeOid, Relation origRelation,
  ...
  while (HeapTupleIsValid(depTup = systable_getnext(depScan))) {
    ...
    if (rel->rd_rel->relkind == RELKIND_RELATION ||
        ...
        ereport(
            ...
            (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
             errmsg("cannot alter type \"%s\" because column \"%s.%s\" uses it",
                    origTypeName, RelationGetRelationName(rel),
                    NameStr(att->attname))));
        ...
        ereport(
            ...
            (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
             errmsg("cannot alter type \"%s\" because column \"%s.%s\" uses it",
                    ...
                    RelationGetRelationName(rel), NameStr(att->attname))));
      else if (origRelation->rd_rel->relkind == RELKIND_FOREIGN_TABLE)
        ereport(ERROR,
                (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                 errmsg(
                     "cannot alter foreign table \"%s\" because column "
                     "\"%s.%s\" uses its row type",
                     RelationGetRelationName(origRelation),
                     RelationGetRelationName(rel), NameStr(att->attname))));
      else
        ereport(ERROR,
                (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                 errmsg(
                     "cannot alter table \"%s\" because column \"%s.%s\" uses "
                     "its row type",
                     RelationGetRelationName(origRelation),
                     RelationGetRelationName(rel), NameStr(att->attname))));
       ...
       */
      find_composite_type_dependencies(rel->rd_rel->reltype, origRelation,
                                       origTypeName);
    }
    ...
    if (behavior == DROP_RESTRICT)
      ereport(
          ...
          (errcode(ERRCODE_DEPENDENT_OBJECTS_STILL_EXIST),
           errmsg(
               "cannot alter type \"%s\" because it is the type of a typed "
               "table",
               typeName),
...
static void ATPrepAddColumn(List **wqueue, Relation rel, bool recurse,
  ...
  if (recurse && !is_view) cmd->subtype = AT_AddColumnRecurse;
}
...
static ObjectAddress ATExecAddColumn(List **wqueue, AlteredTableInfo *tab,
  ...
  if (recursing) ATSimplePermissions(rel, ATT_TABLE | ATT_FOREIGN_TABLE);
        ...
        ereport(
            ...
            (errcode(ERRCODE_COLLATION_MISMATCH),
             errmsg(
                 ...
                 RelationGetRelationName(rel), colDef->colname),
             errdetail("\"%s\" versus \"%s\"", get_collation_name(ccollid),
  ...
  if (aclresult != ACLCHECK_OK) aclcheck_error_type(aclresult, typeOid);
  ...
  if (relkind != RELKIND_VIEW && relkind != RELKIND_COMPOSITE_TYPE &&
    ...
    if (!defval && DomainHasConstraints(typeOid)) {
      ...
      defval = (Expr *)coerce_to_target_type(
          NULL, (Node *)defval, baseTypeId, typeOid, typmod,
   ...
   * table to fix that.
   */
  if (isOid) tab->rewrite |= AT_REWRITE_ALTER_OID;
  ...
  foreach (child, children) {
    ...
    ATExecAddColumn(wqueue, childtab, childrel, colDef, isOid, recurse, true,
                    lockmode);
...
static void check_for_column_name_collision(Relation rel, const char *colname) {
   ...
   */
  attTuple = SearchSysCache2(ATTNAME, ObjectIdGetDatum(RelationGetRelid(rel)),
                             PointerGetDatum(colname));
  if (!HeapTupleIsValid(attTuple)) return;
...
static void ATPrepAddOids(List **wqueue, Relation rel, bool recurse,
  ...
  if (recurse) cmd->subtype = AT_AddOidsRecurse;
}
...
static void ATPrepSetStatistics(Relation rel, const char *colName,
  ...
  if (rel->rd_rel->relkind != RELKIND_RELATION &&
    ...
    ereport(ERROR, (errcode(ERRCODE_WRONG_OBJECT_TYPE),
                    errmsg(
                        "\"%s\" is not a table, materialized view, index, or "
                        "foreign table",
                        RelationGetRelationName(rel))));
...
static ObjectAddress ATExecSetOptions(Relation rel, const char *colName,
  ...
  newOptions = transformRelOptions(isnull ? (Datum)0 : datum, (List *)options,
...
static void ATPrepDropColumn(List **wqueue, Relation rel, bool recurse,
  ...
  if (recurse) cmd->subtype = AT_DropColumnRecurse;
}
...
static ObjectAddress ATExecDropColumn(List **wqueue, Relation rel,
  ...
  if (attnum <= 0 && attnum != ObjectIdAttributeNumber)
    ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                    errmsg("cannot drop system column \"%s\"", colName)));

  /* Don't drop inherited columns */
  if (targetatt->attinhcount > 0 && !recursing)
    ereport(ERROR, (errcode(ERRCODE_INVALID_TABLE_DEFINITION),
                    errmsg("cannot drop inherited column \"%s\"", colName)));
...
static ObjectAddress ATExecAddIndex(AlteredTableInfo *tab, Relation rel,
  ...
  address = DefineIndex(RelationGetRelid(rel), stmt,
                        InvalidOid, /* no predefined OID */
                        true,       /* is_alter_table */
                        check_rights, skip_build, quiet);
...
static ObjectAddress ATExecAddIndexConstraint(AlteredTableInfo *tab,
  ...
  if (constraintName == NULL)
    ...
    ereport(NOTICE, (errmsg(
                        "ALTER TABLE / ADD CONSTRAINT USING INDEX will rename "
                        "index \"%s\" to \"%s\"",
  ...
  if (stmt->primary) index_check_primary_key(rel, indexInfo, true);
  ...
  address = index_constraint_create(
      rel, index_oid, indexInfo, constraintName, constraintType,
      stmt->deferrable, stmt->initdeferred, stmt->primary,
...
static ObjectAddress ATExecAddConstraint(List **wqueue, AlteredTableInfo *tab,
  ...
  switch (newConstraint->contype) {
    case CONSTR_CHECK:
      address = ATAddCheckConstraint(wqueue, tab, rel, newConstraint, recurse,
                                     false, is_readd, lockmode);
      break;
...
static ObjectAddress ATAddCheckConstraint(List **wqueue, AlteredTableInfo *tab,
  ...
  foreach (lcon, newcons) {
    ...
    if (constr->conname == NULL) constr->conname = ccon->name;
   ...
   */
  if (constr->is_no_inherit || is_readd) return address;
...
/*
 * Add a foreign-key constraint to a single table; return the new___
 *constraint's
 * address.
 *
...
static ObjectAddress ATAddForeignKeyConstraint(AlteredTableInfo *tab,
  ...
  switch (rel->rd_rel->relpersistence) {
    ...
    case RELPERSISTENCE_UNLOGGED:
      if (pkrel->rd_rel->relpersistence != RELPERSISTENCE_PERMANENT &&
        ...
        ereport(ERROR, (errcode(ERRCODE_INVALID_TABLE_DEFINITION),
                        errmsg(
                            "constraints on unlogged tables may reference only "
                            "permanent or unlogged tables")));
      break;
    case RELPERSISTENCE_TEMP:
      ...
      if (!pkrel->rd_islocaltemp || !rel->rd_islocaltemp)
        ereport(ERROR, (errcode(ERRCODE_INVALID_TABLE_DEFINITION),
                        errmsg(
                            "constraints on temporary tables must involve "
                            "temporary tables of this session")));
      break;
  }
  ...
  numfks = transformColumnNameList(RelationGetRelid(rel),
                                   fkconstraint->fk_attrs, fkattnum, fktypoid);
  ...
  if (fkconstraint->pk_attrs == NIL) {
    numpks =
        transformFkeyGetPrimaryKey(pkrel, &indexOid, &fkconstraint->pk_attrs,
                                   pkattnum, pktypoid, opclasses);
  } else {
    numpks = transformColumnNameList(
        RelationGetRelid(pkrel), fkconstraint->pk_attrs, pkattnum, pktypoid);
  ...
  if (numfks != numpks)
    ereport(ERROR, (errcode(ERRCODE_INVALID_FOREIGN_KEY),
                    errmsg(
                        "number of referencing and referenced columns for "
  ...
  for (i = 0; i < numpks; i++) {
    ...
    if (!OidIsValid(ppeqop))
      elog(ERROR, "missing operator___ %d(%u,%u) in opfamily %u", eqstrategy,
    ...
    if (!(OidIsValid(pfeqop) && OidIsValid(ffeqop)))
      ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH),
                      ...
                      errdetail(
                          ...
                          format_type_be(fktype), format_type_be(pktype))));
  ...
  constrOid = CreateConstraintEntry(
      ...
      fkconstraint->initially_valid, RelationGetRelid(rel), fkattnum, numfks,
      InvalidOid, /* not a domain
                   * constraint */
      indexOid, RelationGetRelid(pkrel), pkattnum, pfeqoperators, ppeqoperators,
      ffeqoperators, numpks, fkconstraint->fk_upd_action,
      fkconstraint->fk_del_action, fkconstraint->fk_matchtype,
...
static ObjectAddress ATExecAlterConstraint(Relation rel, AlterTableCmd *cmd,
  ...
  scan =
      systable_beginscan(conrel, ConstraintRelidIndexId, true, NULL, 1, &key);
  ...
  if (currcon->contype != CONSTRAINT_FOREIGN)
    ereport(ERROR, (errcode(ERRCODE_WRONG_OBJECT_TYPE),
                    errmsg(
                        "constraint \"%s\" of relation \"%s\" is not a foreign "
                        "key constraint",
                        cmdcon->conname, RelationGetRelationName(rel))));
  ...
  if (currcon->condeferrable != cmdcon->deferrable ||
    ...
    InvokeObjectPostAlterHook(ConstraintRelationId, HeapTupleGetOid(contuple),
                              0);
    ...
    ScanKeyInit(&tgkey, Anum_pg_trigger_tgconstraint, BTEqualStrategyNumber,
                F_OIDEQ, ObjectIdGetDatum(HeapTupleGetOid(contuple)));

    tgscan = systable_beginscan(tgrel, TriggerConstraintIndexId, true, NULL, 1,
                                &tgkey);
    ...
    while (HeapTupleIsValid(tgtuple = systable_getnext(tgscan))) {
      ...
      if (copy_tg->tgrelid != RelationGetRelid(rel))
        otherrelids = list_append_unique_oid(otherrelids, copy_tg->tgrelid);
    ...
    ObjectAddressSet(address, ConstraintRelationId, HeapTupleGetOid(contuple));
  } else
    address = InvalidObjectAddress;
...
static ObjectAddress ATExecValidateConstraint(Relation rel, char *constrName,
  ...
  if (!con->convalidated) {
    ...
    if (con->contype == CONSTRAINT_FOREIGN) {
      ...
      validateForeignKeyConstraint(constrName, rel, refrel, con->conindid,
      ...
      foreach (child, children) {
        ...
        if (childoid == RelationGetRelid(rel)) continue;
    ...
    ObjectAddressSet(address, ConstraintRelationId, HeapTupleGetOid(tuple));
  } else
...
static int transformColumnNameList(Oid relId, List *colList, int16 *attnums,
  ...
  foreach (l, colList) {
    ...
    if (!HeapTupleIsValid(atttuple))
      ereport(ERROR, (errcode(ERRCODE_UNDEFINED_COLUMN),
                      errmsg(
                          "column \"%s\" referenced in foreign key constraint "
                          "does not exist",
                          attname)));
...
static int transformFkeyGetPrimaryKey(Relation pkrel, Oid *indexOid,
  ...
  *indexOid = InvalidOid;
    ...
    if (indexStruct->indisprimary && IndexIsValid(indexStruct)) {
      ...
      if (!indexStruct->indimmediate)
        ereport(ERROR, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                        errmsg(
                            "cannot use a deferrable primary key for "
                            "referenced table \"%s\"",
                            RelationGetRelationName(pkrel))));
  ...
  indclassDatum =
      SysCacheGetAttr(INDEXRELID, indexTuple, Anum_pg_index_indclass, &isnull);
  Assert(!isnull);
                                   ...
                                   Oid *opclasses) /* output parameter */
{
  ...
  for (i = 0; i < numattrs; i++) {
    for (j = i + 1; j < numattrs; j++) {
      if (attnums[i] == attnums[j])
        ereport(ERROR, (errcode(ERRCODE_INVALID_FOREIGN_KEY),
                        errmsg(
                            "foreign key referenced-columns list must not "
                            "contain duplicates")));
    }
  }
  ...
  foreach (indexoidscan, indexoidlist) {
     ...
     */
    if (indexStruct->indnatts == numattrs && indexStruct->indisunique &&
  ...
  if (!found) {
    if (found_deferrable)
      ereport(ERROR, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                      errmsg(
                          "cannot use a deferrable unique constraint for "
                          "referenced table \"%s\"",
                          RelationGetRelationName(pkrel))));
    else
      ereport(ERROR, (errcode(ERRCODE_INVALID_FOREIGN_KEY),
...
static CoercionPathType findFkeyCast(Oid targetTypeId, Oid sourceTypeId,
  ...
  if (targetTypeId == sourceTypeId) {
    ...
    *funcid = InvalidOid;
    ...
    ret = find_coercion_pathway(targetTypeId, sourceTypeId, COERCION_IMPLICIT,
                                funcid);
    if (ret == COERCION_PATH_NONE)
      /* A previously-relied-upon cast is now gone. */
      elog(ERROR, "could not find cast from %u to %u", sourceTypeId,
           targetTypeId);
  }
...
static void checkFkeyPermissions(Relation rel, int16 *attnums, int natts) {
  ...
  for (i = 0; i < natts; i++) {
    aclresult = pg_attribute_aclcheck(RelationGetRelid(rel), attnums[i], roleid,
                                      ACL_REFERENCES);
    if (aclresult != ACLCHECK_OK)
      aclcheck_error(aclresult, ACL_KIND_CLASS, RelationGetRelationName(rel));
  }
...
static void validateCheckConstraint(Relation rel, HeapTuple constrtup) {
  ...
  if (rel->rd_rel->relkind == RELKIND_FOREIGN_TABLE) return;
   ...
   */
  val =
      SysCacheGetAttr(CONSTROID, constrtup, Anum_pg_constraint_conbin, &isnull);
  if (isnull)
    elog(ERROR, "null conbin for constraint %u", HeapTupleGetOid(constrtup));
  conbin = TextDatumGetCString(val);
  origexpr = (Expr *)stringToNode(conbin);
  exprstate =
      (List *)ExecPrepareExpr((Expr *)make_ands_implicit(origexpr), estate);
...
static void validateForeignKeyConstraint(char *conname, Relation rel,
   ...
   */
  if (RI_Initial_Check(&trig, rel, pkrel)) return;
...
static void ATExecDropConstraint(Relation rel, const char *constrName,
  ...
  while (HeapTupleIsValid(tuple = systable_getnext(scan))) {
    ...
    if (strcmp(NameStr(con->conname), constrName) != 0) continue;
  ...
  foreach (child, children) {
    ...
    ScanKeyInit(&key, Anum_pg_constraint_conrelid, BTEqualStrategyNumber,
                F_OIDEQ, ObjectIdGetDatum(childrelid));
    scan =
        systable_beginscan(conrel, ConstraintRelidIndexId, true, NULL, 1, &key);
    ...
    while (HeapTupleIsValid(tuple = systable_getnext(scan))) {
      ...
      if (con->contype != CONSTRAINT_CHECK) continue;

      if (strcmp(NameStr(con->conname), constrName) == 0) break;
    }
    ...
    if (con->coninhcount <= 0) /* shouldn't happen */
      elog(ERROR, "relation %u has non-inherited constraint \"%s\"", childrelid,
           constrName);
    ...
    if (recurse) {
      ...
      if (con->coninhcount == 1 && !con->conislocal) {
        ...
        ATExecDropConstraint(childrel, constrName, behavior, true, true, false,
                             lockmode);
      } else {
...
static void ATPrepAlterColumnType(List **wqueue, AlteredTableInfo *tab,
  ...
  if (aclresult != ACLCHECK_OK) aclcheck_error_type(aclresult, targettype);
  ...
  if (tab->relkind == RELKIND_RELATION) {
    ...
    transform = coerce_to_target_type(
        pstate, transform, exprType(transform), targettype, targettypmod,
        COERCION_ASSIGNMENT, COERCE_IMPLICIT_CAST, -1);
    if (transform == NULL)
      ereport(
          ERROR,
  ...
  if (recurse)
    ...
    ereport(ERROR, (errcode(ERRCODE_INVALID_TABLE_DEFINITION),
                    errmsg(
                        "type of inherited column \"%s\" must be changed in "
                        "child tables too",
                        colName)));
...
static bool ATColumnChangeRequiresRewrite(Node *expr, AttrNumber varattno) {
  ...
  for (;;) {
    ...
    if (IsA(expr, Var) && ((Var *)expr)->varattno == varattno)
      ...
      if (DomainHasConstraints(d->resulttype)) return true;
      expr = (Node *)d->arg;
    } else
      return true;
  }
...
static ObjectAddress ATExecAlterColumnType(AlteredTableInfo *tab, Relation rel,
  ...
  if (attTup->atthasdef) {
    ...
    defaultexpr = coerce_to_target_type(
        NULL, /* no UNKNOWN params */
        defaultexpr, exprType(defaultexpr), targettype, targettypmod,
        COERCION_ASSIGNMENT, COERCE_IMPLICIT_CAST, -1);
    if (defaultexpr == NULL)
      ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH),
                      errmsg(
                          "default for column \"%s\" cannot be cast "
  ...
  while (HeapTupleIsValid(depTup = systable_getnext(scan))) {
    ...
    switch (getObjectClass(&foundObject)) {
        ...
        if (relKind == RELKIND_INDEX) {
          ...
          if (!list_member_oid(tab->changedIndexOids, foundObject.objectId)) {
            tab->changedIndexOids =
                lappend_oid(tab->changedIndexOids, foundObject.objectId);
        ...
        if (!list_member_oid(tab->changedConstraintOids,
          ...
          if (foundDep->deptype == DEPENDENCY_NORMAL) {
            tab->changedConstraintOids =
                lcons_oid(foundObject.objectId, tab->changedConstraintOids);
            tab->changedConstraintDefs =
                lcons(defstring, tab->changedConstraintDefs);
          } else {
            tab->changedConstraintOids =
                lappend_oid(tab->changedConstraintOids, foundObject.objectId);
            tab->changedConstraintDefs =
        ...
        ereport(ERROR,
                (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                 ...
                 errdetail("%s depends on column \"%s\"",
                           getObjectDescription(&foundObject), colName)));
        break;
      ...
      default:
        elog(ERROR, "unrecognized object class___: %u", foundObject.classId);
    }
  }
...
static ObjectAddress ATExecAlterColumnGenericOptions(Relation rel,
  ...
  datum =
      SysCacheGetAttr(ATTNAME, tuple, Anum_pg_attribute_attfdwoptions, &isnull);
...
static void ATPostAlterTypeCleanup(List **wqueue, AlteredTableInfo *tab,
  ...
  forboth(oid_item, tab->changedIndexOids, def_item, tab->changedIndexDefs) {
    ...
    ATPostAlterTypeParse(oldId, relid, InvalidOid, (char *)lfirst(def_item),
                         wqueue, lockmode, tab->rewrite);
  }
  ...
  foreach (list_item, raw_parsetree_list) {
    ...
    if (IsA(stmt, IndexStmt))
      querytree_list = lappend(
          querytree_list, transformIndexStmt(oldRelId, (IndexStmt *)stmt, cmd));
    else if (IsA(stmt, AlterTableStmt))
      querytree_list = list_concat(
          querytree_list,
          transformAlterTableStmt(oldRelId, (AlterTableStmt *)stmt, cmd));
    else
  ...
  foreach (list_item, querytree_list) {
    ...
    switch (nodeTag(stm)) {
      case T_IndexStmt: {
        ...
        if (!rewrite) TryReuseIndex(oldId, stmt);
...
static void TryReuseForeignKey(Oid oldId, Constraint *con) {
  ...
  if (isNull) elog(ERROR, "null conpfeqop for constraint %u", oldId);
...
void ATExecChangeOwner(Oid relationOid, Oid newOwnerId, bool recursing,
  ...
  if (tuple_class->relowner != newOwnerId) {
    ...
    newtuple = heap_modify_tuple(tuple, RelationGetDescr(class_rel), repl_val,
                                 repl_null, repl_repl);
...
static void change_owner_fix_column_acls(Oid relationOid, Oid oldOwnerId,
  ...
  while (HeapTupleIsValid(attributeTuple = systable_getnext(scan))) {
    ...
    newtuple = heap_modify_tuple(attributeTuple, RelationGetDescr(attRelation),
...
static void change_owner_recurse_to_sequences(Oid relationOid, Oid newOwnerId,
  ...
  while (HeapTupleIsValid(tup = systable_getnext(scan))) {
    ...
    if (depForm->refobjsubid == 0 || depForm->classid != RelationRelationId ||
        depForm->objsubid != 0 || depForm->deptype != DEPENDENCY_AUTO)
      continue;
...
static void ATExecSetRelOptions(Relation rel, List *defList,
  ...
  newOptions =
      transformRelOptions(isnull ? (Datum)0 : datum, defList, NULL, validnsps,
  ...
  if (rel->rd_rel->relkind == RELKIND_VIEW) {
    ...
    if (check_option) {
      ...
      if (view_updatable_error)
        ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                        errmsg(
                            "WITH CHECK OPTION is supported only on "
  ...
  if (OidIsValid(rel->rd_rel->reltoastrelid)) {
    ...
    newOptions =
        transformRelOptions(isnull ? (Datum)0 : datum, defList, "toast",
                            validnsps, false, operation == AT_ResetRelOptions);
    ...
    InvokeObjectPostAlterHookArg(RelationRelationId, RelationGetRelid(toastrel),
                                 0, InvalidOid, true);
 ...
 * The user must have CREATE rights on the new___ tablespace, as usual.   The
 *main
...
Oid AlterTableMoveAll(AlterTableMoveAllStmt *stmt) {
  ...
  if (stmt->objtype != OBJECT_TABLE && stmt->objtype != OBJECT_INDEX &&
    ...
    ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                    errmsg(
                        "only tables, indexes, and materialized views exist in "
                        "tablespaces")));
  ...
  if (new_tablespaceoid == MyDatabaseTableSpace) new_tablespaceoid = InvalidOid;

  /* no-op */
  if (orig_tablespaceoid == new_tablespaceoid) return new_tablespaceoid;
  ...
  while ((tuple = heap_getnext(scan, ForwardScanDirection)) != NULL) {
    ...
    if ((stmt->objtype == OBJECT_TABLE &&
        ...
        (stmt->objtype == OBJECT_INDEX && relForm->relkind != RELKIND_INDEX) ||
        (stmt->objtype == OBJECT_MATVIEW &&
    ...
    if (stmt->nowait &&
      ...
      ereport(ERROR, (errcode(ERRCODE_OBJECT_IN_USE),
                      errmsg(
                          "aborting because lock on relation \"%s\".\"%s\" is "
                          "not available",
 ...
 */
static void copy_relation_data(SMgrRelation src, SMgrRelation dst,
                               ForkNumber forkNum, char relpersistence) {
  char *buf;
  Page page;
  bool use_wal;
   ...
   */
  if (relpersistence == RELPERSISTENCE_PERMANENT) smgrimmedsync(dst, forkNum);
}
 ...
 */
static void ATExecEnableDisableRule(Relation rel, char *trigname,
                                    char fires_when, LOCKMODE lockmode) {
...
static ObjectAddress ATExecAddInherit(Relation child_rel, RangeVar *parent,
    ...
    if (inh->inhparent == RelationGetRelid(parent_rel))
      ereport(ERROR,
              (errcode(ERRCODE_DUPLICATE_TABLE),
               errmsg("relation \"%s\" would be inherited from more than once",
                      RelationGetRelationName(parent_rel))));
    if (inh->inhseqno > inhseqno) inhseqno = inh->inhseqno;
  }
  systable_endscan(scan);
  ...
  if (list_member_oid(children, RelationGetRelid(parent_rel)))
    ereport(ERROR,
            (errcode(ERRCODE_DUPLICATE_TABLE),
             ...
             errdetail("\"%s\" is already a child of \"%s\".", parent->relname,
  ...
  if (parent_rel->rd_rel->relhasoids && !child_rel->rd_rel->relhasoids)
    ereport(ERROR, (errcode(ERRCODE_WRONG_OBJECT_TYPE),
                    errmsg(
                        "table \"%s\" without OIDs cannot inherit from table "
                        "\"%s\" with OIDs",
                        RelationGetRelationName(child_rel),
                        RelationGetRelationName(parent_rel))));
 ...
 */
static bool constraints_equivalent(HeapTuple a, HeapTuple b,
                                   TupleDesc tupleDesc) {
  ...
  if (acon->condeferrable != bcon->condeferrable ||
      ...
      strcmp(decompile_conbin(a, tupleDesc), decompile_conbin(b, tupleDesc)) !=
          0)
    return false;
  else
    return true;
...
static void MergeAttributesIntoExisting(Relation child_rel,
  ...
  for (parent_attno = 1; parent_attno <= parent_natts; parent_attno++) {
    ...
    /* Ignore dropped columns in the parent. */
    if (attribute->attisdropped) continue;

    /* Find same column in child (matching on column name). */
    tuple =
        SearchSysCacheCopyAttName(RelationGetRelid(child_rel), attributeName);
    if (HeapTupleIsValid(tuple)) {
      ...
      if (attribute->atttypid != childatt->atttypid ||
        ...
        ereport(
            ...
            (errcode(ERRCODE_DATATYPE_MISMATCH),
             errmsg("child table \"%s\" has different type for column \"%s\"",
                    RelationGetRelationName(child_rel), attributeName)));
      ...
      ereport(ERROR,
              (errcode(ERRCODE_DATATYPE_MISMATCH),
               errmsg("child table is missing column \"%s\"", attributeName)));
    }
  }
...
static void MergeConstraintsIntoExisting(Relation child_rel,
  ...
  while (HeapTupleIsValid(parent_tuple = systable_getnext(parent_scan))) {
    ...
    if (parent_con->contype != CONSTRAINT_CHECK) continue;

    /* if the parent's constraint is marked NO INHERIT, it's not inherited */
    if (parent_con->connoinherit) continue;

    /* Search for a child constraint matching this one */
    ScanKeyInit(&child_key, Anum_pg_constraint_conrelid, BTEqualStrategyNumber,
    ...
    while (HeapTupleIsValid(child_tuple = systable_getnext(child_scan))) {
      ...
      if (child_con->contype != CONSTRAINT_CHECK) continue;
      ...
      if (!constraints_equivalent(parent_tuple, child_tuple, tuple_desc))
        ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH),
                        errmsg(
                            "child table \"%s\" has different definition for "
                            "check constraint \"%s\"",
                            RelationGetRelationName(child_rel),
      ...
      if (child_con->connoinherit)
        ereport(ERROR, (errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
                        errmsg(
                            "constraint \"%s\" conflicts with non-inherited "
...
static ObjectAddress ATExecDropInherit(Relation rel, RangeVar *parent,
  ...
  while (HeapTupleIsValid(attributeTuple = systable_getnext(scan))) {
    ...
    if (SearchSysCacheExistsAttName(RelationGetRelid(parent_rel),
      ...
      if (copy_att->attinhcount == 0) copy_att->attislocal = true;
  ...
  ScanKeyInit(&key[0], Anum_pg_constraint_conrelid, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(RelationGetRelid(parent_rel)));
  scan = systable_beginscan(catalogRelation, ConstraintRelidIndexId, true, NULL,
                            1, key);
  ...
  while (HeapTupleIsValid(constraintTuple = systable_getnext(scan))) {
    ...
    if (match) {
      ...
      if (copy_con->coninhcount == 0) copy_con->conislocal = true;
  ...
  drop_parent_dependency(RelationGetRelid(rel), RelationRelationId,
   ...
   */
  InvokeObjectPostAlterHookArg(InheritsRelationId, RelationGetRelid(rel), 0,
...
static void drop_parent_dependency(Oid relid, Oid refclassid, Oid refobjid) {
  ...
  scan = systable_beginscan(catalogRelation, DependDependerIndexId, true, NULL,
                            3, key);
  ...
  while (HeapTupleIsValid(depTuple = systable_getnext(scan))) {
    ...
    if (dep->refclassid == refclassid && dep->refobjid == refobjid &&
        dep->refobjsubid == 0 && dep->deptype == DEPENDENCY_NORMAL)
...
static ObjectAddress ATExecAddOf(Relation rel, const TypeName *ofTypename,
  ...
  for (type_attno = 1; type_attno <= typeTupleDesc->natts; type_attno++) {
    ...
    if (type_attr->attisdropped) continue;
    ...
    do {
      if (table_attno > tableTupleDesc->natts)
        ereport(ERROR,
                (errcode(ERRCODE_DATATYPE_MISMATCH),
                 errmsg("table is missing column \"%s\"", type_attname)));
...
static void relation_mark_replica_identity(Relation rel, char ri_type,
  ...
  foreach (index, RelationGetIndexList(rel)) {
    ...
    if (dirty) {
      ...
      InvokeObjectPostAlterHookArg(IndexRelationId, thisIndexOid, 0, InvalidOid,
                                   is_internal);
    }
...
static void ATExecReplicaIdentity(Relation rel, ReplicaIdentityStmt *stmt,
  ...
  for (key = 0; key < indexRel->rd_index->indnatts; key++) {
    ...
    if (attno <= 0 && attno != ObjectIdAttributeNumber)
      elog(ERROR, "internal column %u in unique index \"%s\"", attno,
           RelationGetRelationName(indexRel));
    ...
    if (!attr->attnotnull)
      ereport(ERROR,
              (errcode(ERRCODE_WRONG_OBJECT_TYPE),
               errmsg(
                   "index \"%s\" cannot be used as replica identity because "
...
static void ATExecGenericOptions(Relation rel, List *options) {
  ...
  tuple = heap_modify_tuple(tuple, RelationGetDescr(ftrel), repl_val, repl_null,
                            repl_repl);
...
static bool ATPrepChangePersistence(Relation rel, bool toLogged) {
  ...
  while (HeapTupleIsValid(tuple = systable_getnext(scan))) {
    ...
    if (con->contype == CONSTRAINT_FOREIGN) {
      ...
      if (RelationGetRelid(rel) == foreignrelid) continue;
...
ObjectAddress AlterTableNamespace(AlterObjectSchemaStmt *stmt, Oid *oldschema) {
    ...
    if (sequenceIsOwned(relid, &tableId, &colId))
      ereport(ERROR,
              (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
               ...
               errdetail("Sequence \"%s\" is linked to table \"%s\".",
                         RelationGetRelationName(rel), get_rel_name(tableId))));
  }
  ...
  if (oldschema) *oldschema = oldNspOid;
...
void AlterTableNamespaceInternal(Relation rel, Oid oldNspOid, Oid nspOid,
  ...
  if (rel->rd_rel->relkind == RELKIND_RELATION ||
    ...
    AlterSeqNamespaces(classRel, rel, oldNspOid, nspOid, objsMoved,
                       AccessExclusiveLock);
    AlterConstraintNamespaces(RelationGetRelid(rel), oldNspOid, nspOid, false,
                              objsMoved);
  }
...
static void AlterIndexNamespaces(Relation classRel, Relation rel, Oid oldNspOid,
  ...
  foreach (l, indexList) {
     ...
     */
    if (!object_address_present(&thisobj, objsMoved)) {
      AlterRelationNamespaceInternal(classRel, indexOid, oldNspOid, newNspOid,
                                     false, objsMoved);
      add_exact_object_address(&thisobj, objsMoved);
    }
  }
...
void register_on_commit_action(Oid relid, OnCommitAction action) {
   ...
   */
  if (action == ONCOMMIT_NOOP || action == ONCOMMIT_PRESERVE_ROWS) return;
...
void PreCommit_on_commit_actions(void) {
  ...
  foreach (l, on_commits) {
    ...
    if (oc->deleting_subid != InvalidSubTransactionId) continue;
...
void AtEOSubXact_on_commit_actions(bool isCommit, SubTransactionId mySubid,
  ...
  while (cur_item != NULL) {
    ...
    if (!isCommit && oc->creating_subid == mySubid) {
      ...
      /* cur_item must be preserved */
      if (oc->creating_subid == mySubid) oc->creating_subid = parentSubid;
 ...
 * used by all.
 */
void RangeVarCallbackOwnsTable(const RangeVar *relation, Oid relId,
                               Oid oldRelId, void *arg) {
  char relkind;
 ...
 */
void RangeVarCallbackOwnsRelation(const RangeVar *relation, Oid relId,
                                  Oid oldRelId, void *arg) {
  HeapTuple tuple;
...
static void RangeVarCallbackForAlterRelation(const RangeVar *rv, Oid relid,
  ...
  if (IsA(stmt, RenameStmt)) {
    aclresult =
        pg_namespace_aclcheck(classform->relnamespace, GetUserId(), ACL_CREATE);
    if (aclresult != ACLCHECK_OK)
      aclcheck_error(aclresult, ACL_KIND_NAMESPACE,
