...
ObjectAddress AlterTSDictionary(AlterTSDictionaryStmt *stmt) {
  ...
  opt =
      SysCacheGetAttr(TSDICTOID, tup, Anum_pg_ts_dict_dictinitoption, &isnull);
  if (isnull)
    dictoptions = NIL;
  else
  ...
  foreach (pl, stmt->options) {
     ...
     */
    if (defel->arg) dictoptions = lappend(dictoptions, defel);
  }
  ...
  newtup = heap_modify_tuple(tup, RelationGetDescr(rel), repl_val, repl_null,
                             repl_repl);
...
ObjectAddress DefineTSTemplate(List *names, List *parameters) {
  ...
  values[Anum_pg_ts_template_tmplnamespace - 1] =
...
static HeapTuple GetTSConfigTuple(List *names) {
  ...
  if (!OidIsValid(cfgId)) return NULL;
...
static ObjectAddress makeConfigurationDependencies(HeapTuple tuple,
  ...
  if (mapRel) {
    ...
    ScanKeyInit(&skey, Anum_pg_ts_config_map_mapcfg, BTEqualStrategyNumber,
                F_OIDEQ, ObjectIdGetDatum(myself.objectId));
 ...
 */
ObjectAddress DefineTSConfiguration(List *names, List *parameters,
  ...
  if (mapRel) heap_close(mapRel, RowExclusiveLock);
...
void RemoveTSConfigurationById(Oid cfgId) {
  ...
  ScanKeyInit(&skey, Anum_pg_ts_config_map_mapcfg, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(cfgId));
...
ObjectAddress AlterTSConfiguration(AlterTSConfigurationStmt *stmt) {
  ...
  InvokeObjectPostAlterHook(TSConfigMapRelationId, HeapTupleGetOid(tup), 0);
...
static int *getTokenTypes(Oid prsId, List *tokennames) {
  ...
  list =
      (LexDescr *)DatumGetPointer(OidFunctionCall1(prs->lextypeOid, (Datum)0));
  ...
  foreach (tn, tokennames) {
    ...
    if (!found)
      ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                      errmsg("token type \"%s\" does not exist", strVal(val))));
    i++;
  }
...
static void MakeConfigurationMapping(AlterTSConfigurationStmt *stmt,
  ...
  if (stmt->override) {
     ...
     */
    for (i = 0; i < ntoken; i++) {
      ScanKeyInit(&skey[0], Anum_pg_ts_config_map_mapcfg, BTEqualStrategyNumber,
                  F_OIDEQ, ObjectIdGetDatum(cfgId));
      ScanKeyInit(&skey[1], Anum_pg_ts_config_map_maptokentype,
                  BTEqualStrategyNumber, F_INT4EQ, Int32GetDatum(tokens[i]));

      scan =
          systable_beginscan(relMap, TSConfigMapIndexId, true, NULL, 2, skey);
  ...
  if (stmt->replace) {
    ...
    while (HeapTupleIsValid((maptup = systable_getnext(scan)))) {
      ...
      if (cfgmap->mapdict == dictOld) {
        ...
        newtup = heap_modify_tuple(maptup, RelationGetDescr(relMap), repl_val,
                                   repl_null, repl_repl);
    ...
    for (i = 0; i < ntoken; i++) {
      for (j = 0; j < ndict; j++) {
        ...
        values[Anum_pg_ts_config_map_maptokentype - 1] =
            Int32GetDatum(tokens[i]);
        values[Anum_pg_ts_config_map_mapseqno - 1] = Int32GetDatum(j + 1);
        values[Anum_pg_ts_config_map_mapdict - 1] =
            ObjectIdGetDatum(dictIds[j]);
...
text *serialize_deflist(List *deflist) {
  ...
  foreach (l, deflist) {
    ...
    if (strchr(val, '\\')) appendStringInfoChar(&buf, ESCAPE_STRING_SYNTAX);
    appendStringInfoChar(&buf, '\'');
    while (*val) {
      char ch = *val++;

      if (SQL_STR_DOUBLE(ch, true)) appendStringInfoChar(&buf, ch);
      appendStringInfoChar(&buf, ch);
    }
    appendStringInfoChar(&buf, '\'');
    if (lnext(l) != NULL) appendStringInfoString(&buf, ", ");
  }
...
List *deserialize_deflist(Datum txt) {
      ...
      case CS_INSQVALUE:
        if (*ptr == '\'') {
          if (ptr + 1 < endptr && ptr[1] == '\'') {
            ...
            *wsptr++ = '\0';
            result = lappend(
                result, makeDefElem(pstrdup(workspace),
  ...
  if (state == CS_INWVALUE) {
    *wsptr++ = '\0';
    result =
        lappend(result, makeDefElem(pstrdup(workspace),
                                    (Node *)makeString(pstrdup(startvalue))));
