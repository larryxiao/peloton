...
static bool create_toast_table(Relation rel, Oid toastOid, Oid toastIndexOid,
   ...
   */
  if (rel->rd_rel->reltoastrelid != InvalidOid) return false;

  if (!IsBinaryUpgrade) {
    if (!needs_toast_table(rel)) return false;
  } else {
    /*
     ...
     */
    if (!OidIsValid(binary_upgrade_next_toast_pg_class_oid)) return false;
  ...
  toast_relid = heap_create_with_catalog(
      toast_relname, namespaceid, rel->rd_rel->reltablespace, toastOid,
      toast_typid, InvalidOid, rel->rd_rel->relowner, tupdesc, NIL,
      RELKIND_TOASTVALUE, rel->rd_rel->relpersistence, shared_relation,
      mapped_relation, true, 0, ONCOMMIT_NOOP, reloptions, false, true, true,
      NULL);
  ...
  index_create(
      toast_rel, toast_idxname, toastIndexOid, InvalidOid, indexInfo,
      list_make2(static_cast<void *>(static_cast<char *>("chunk_id")),
                 static_cast<void *>(static_cast<char *>("chunk_seq"))),
      BTREE_AM_OID, rel->rd_rel->reltablespace, collationObjectId,
      classObjectId, coloptions, (Datum)0, true, false, false, false, true,
...
static bool needs_toast_table(Relation rel) {
  ...
  for (i = 0; i < tupdesc->natts; i++) {
    if (att[i]->attisdropped) continue;
    ...
    if (att[i]->attlen > 0) {
      ...
      if (maxlen < 0)
        ...
        data_length += maxlen;
      if (att[i]->attstorage != 'p') has_toastable_attrs = true;
    }
  }
  if (!has_toastable_attrs) return false; /* nothing to toast? */
  if (maxlength_unknown) return true;     /* any unlimited-length attrs? */
