...
static void AddNewRelationTuple(Relation pg_class_desc, Relation new_rel_desc,
                                Oid new_rel_oid, Oid new_type_oid,
                                Oid reloftype, Oid relowner, char relkind,
                                Datum relacl, Datum reloptions);
static ObjectAddress AddNewRelationType(const char *typeName, Oid typeNamespace,
                                        Oid new_rel_oid, char new_rel_kind,
                                        Oid ownerid, Oid new_row_type,
                                        Oid new_array_type);
...
static Node *cookConstraint(ParseState *pstate, Node *raw_constraint,
                            char *relname);
...
Form_pg_attribute SystemAttributeByName(const char *attname, bool relhasoids) {
  ...
  for (j = 0; j < (int)lengthof(SysAtt); j++) {
    ...
    if (relhasoids || att->attnum != ObjectIdAttributeNumber) {
      if (strcmp(NameStr(att->attname), attname) == 0) return att;
    }
  }
...
Relation heap_create(const char *relname, Oid relnamespace, Oid reltablespace,
                     Oid relid, Oid relfilenode, TupleDesc tupDesc,
                     char relkind, char relpersistence, bool shared_relation,
                     bool mapped_relation, bool allow_system_table_mods) {
  bool create_storage;
  Relation rel;
   ...
   */
  if (reltablespace == MyDatabaseTableSpace) reltablespace = InvalidOid;

  /*
   * build the relcache entry.
   */
  rel = RelationBuildLocalRelation(relname, relnamespace, tupDesc, relid,
                                   relfilenode, reltablespace, shared_relation,
                                   mapped_relation, relpersistence, relkind);
 ...
 * --------------------------------
 */
void CheckAttributeType(const char *attname, Oid atttypid, Oid attcollation,
  ...
  if (atttypid == UNKNOWNOID) {
    ...
    if (atttypid != ANYARRAYOID || !allow_system_table_mods)
      ereport(ERROR, (errcode(ERRCODE_INVALID_TABLE_DEFINITION),
                      errmsg("column \"%s\" has pseudo-type %s", attname,
    ...
    CheckAttributeType(attname, getBaseType(atttypid), attcollation,
                       containing_rowtypes, allow_system_table_mods);
    ...
    for (i = 0; i < tupdesc->natts; i++) {
      ...
      CheckAttributeType(NameStr(attr->attname), attr->atttypid,
                         attr->attcollation, containing_rowtypes,
                         allow_system_table_mods);
    }
     ...
     */
    CheckAttributeType(attname, att_typelem, attcollation, containing_rowtypes,
                       allow_system_table_mods);
  }
  ...
  if (!OidIsValid(attcollation) && type_is_collatable(atttypid))
    ereport(
        ...
        (errcode(ERRCODE_INVALID_TABLE_DEFINITION),
         errmsg(
             "no collation was derived for column \"%s\" with collatable type "
             "%s",
 ...
 * --------------------------------
 */
void InsertPgClassTuple(Relation pg_class_desc, Relation new_rel_desc,
  ...
  values[Anum_pg_class_reltablespace - 1] =
  ...
  values[Anum_pg_class_reltoastrelid - 1] =
  ...
  values[Anum_pg_class_relhassubclass - 1] =
      BoolGetDatum(rd_rel->relhassubclass);
  values[Anum_pg_class_relispopulated - 1] =
...
Oid heap_create_with_catalog(const char *relname, Oid relnamespace,
                             Oid reltablespace, Oid relid, Oid reltypeid,
                             Oid reloftypeid, Oid ownerid, TupleDesc tupdesc,
                             List *cooked_constraints, char relkind,
                             char relpersistence, bool shared_relation,
                             bool mapped_relation, bool oidislocal,
                             int oidinhcount, OnCommitAction oncommit,
                             Datum reloptions, bool use_user_acl,
   ...
   */
  old_type_oid = GetSysCacheOid2(TYPENAMENSP, CStringGetDatum(relname),
    ...
    if (IsBinaryUpgrade &&
      ...
      if (!OidIsValid(binary_upgrade_next_heap_pg_class_oid))
        ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                        errmsg(
                            "pg_class heap OID value not set when in binary "
                            "upgrade mode")));
  ...
  if (use_user_acl) {
    switch (relkind) {
      ...
      case RELKIND_SEQUENCE:
        relacl =
            get_user_default_acl(ACL_OBJECT_SEQUENCE, ownerid, relnamespace);
        break;
      default:
        relacl = NULL;
        break;
   ...
   */
  new_rel_desc =
      heap_create(relname, relnamespace, reltablespace, relid, InvalidOid,
                  tupdesc, relkind, relpersistence, shared_relation,
   ...
   */
  new_type_addr = AddNewRelationType(relname, relnamespace, relid, relkind,
                                     ownerid, reltypeid, new_array_oid);
  new_type_oid = new_type_addr.objectId;
  if (typaddress) *typaddress = new_type_addr;
  ...
  AddNewRelationTuple(pg_class_desc, new_rel_desc, relid, new_type_oid,
                      reloftypeid, ownerid, relkind, PointerGetDatum(relacl),
                      reloptions);
   ...
   * need a namescpace___ dependency (they live in a pinned namescpace___) nor
   *an
  ...
  if (relkind != RELKIND_COMPOSITE_TYPE && relkind != RELKIND_TOASTVALUE &&
    ...
    if (relacl != NULL) {
      ...
      updateAclDependencies(RelationRelationId, relid, 0, ownerid, 0, NULL,
   ...
   */
  if (oncommit != ONCOMMIT_NOOP) register_on_commit_action(relid, oncommit);
...
void DeleteAttributeTuples(Oid relid) {
  ...
  scan =
      systable_beginscan(attrel, AttributeRelidNumIndexId, true, NULL, 1, key);
...
void DeleteSystemAttributeTuples(Oid relid) {
  ...
  ScanKeyInit(&key[0], Anum_pg_attribute_attrelid, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(relid));
  ScanKeyInit(&key[1], Anum_pg_attribute_attnum, BTLessEqualStrategyNumber,
              F_INT2LE, Int16GetDatum(0));
...
void RemoveAttributeById(Oid relid, AttrNumber attnum) {
  ...
  tuple = SearchSysCacheCopy2(ATTNUM, ObjectIdGetDatum(relid),
  ...
  if (attnum < 0) {
     ...
     */
    snprintf(newattname, sizeof(newattname), "........pg.dropped.%d........",
             attnum);
  ...
  if (attnum > 0) RemoveStatistics(relid, attnum);
 ...
 */
void RemoveAttrDefault(Oid relid, AttrNumber attnum, DropBehavior behavior,
  ...
  ScanKeyInit(&scankeys[0], Anum_pg_attrdef_adrelid, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(relid));
  ScanKeyInit(&scankeys[1], Anum_pg_attrdef_adnum, BTEqualStrategyNumber,
              F_INT2EQ, Int16GetDatum(attnum));

  scan = systable_beginscan(attrdef_rel, AttrDefaultIndexId, true, NULL, 2,
                            scankeys);
  ...
  if (complain && !found)
    elog(ERROR, "could not find attrdef tuple for relation %u attnum %d", relid,
         attnum);
}
...
void RemoveAttrDefaultById(Oid attrdefId) {
  ...
  ScanKeyInit(&scankeys[0], ObjectIdAttributeNumber, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(attrdefId));

  scan = systable_beginscan(attrdef_rel, AttrDefaultOidIndexId, true, NULL, 1,
                            scankeys);
  ...
  if (!HeapTupleIsValid(tuple)) /* shouldn't happen */
    elog(ERROR, "cache lookup failed for attribute %d of relation %u", myattnum,
         myrelid);
 ...
 */
Oid StoreAttrDefault(Relation rel, AttrNumber attnum, Node *expr,
                     bool is_internal) {
  char *adbin;
  char *adsrc;
  Relation adrel;
  ...
  adsrc = deparse_expression(
      expr,
      deparse_context_for(RelationGetRelationName(rel), RelationGetRelid(rel)),
      false, false);
  ...
  atttup = SearchSysCacheCopy2(ATTNUM, ObjectIdGetDatum(RelationGetRelid(rel)),
                               Int16GetDatum(attnum));
...
static Oid StoreRelCheck(Relation rel, char *ccname, Node *expr,
   ...
   */
  varList =
      pull_var_clause(expr, PVC_REJECT_AGGREGATES, PVC_REJECT_PLACEHOLDERS);
  ...
  if (keycount > 0) {
    ...
    foreach (vl, varList) {
      ...
      for (j = 0; j < i; j++)
        if (attNos[j] == var->varattno) break;
      if (j == i) attNos[i++] = var->varattno;
    }
    keycount = i;
  } else
    attNos = NULL;
 ...
 *
 * Each CookedConstraint struct is modified to store the new___ catalog tuple
 *OID.
 *
...
static void StoreConstraints(Relation rel, List *cooked_constraints,
  ...
  if (cooked_constraints == NIL) return; /* nothing to do */
    ...
    switch (con->contype) {
      ...
      case CONSTR_CHECK:
        con->conoid = StoreRelCheck(
            rel, con->name, con->expr, !con->skip_validation, con->is_local,
            con->inhcount, con->is_no_inherit, is_internal);
        numchecks++;
        break;
      default:
  ...
  if (numchecks > 0) SetRelationNumChecks(rel, numchecks);
}
...
List *AddRelationNewConstraints(Relation rel, List *newColDefaults,
  ...
  foreach (cell, newColDefaults) {
    ...
    expr = cookDefault(pstate, colDef->raw_default, atp->atttypid,
                       atp->atttypmod, NameStr(atp->attname));
  ...
  foreach (cell, newConstraints) {
    ...
    if (cdef->contype != CONSTR_CHECK) continue;
    ...
    if (cdef->raw_expr != NULL) {
       ...
       */
      expr =
          cookConstraint(pstate, cdef->raw_expr, RelationGetRelationName(rel));
    } else {
      Assert(cdef->cooked_expr != NULL);
    ...
    if (cdef->conname != NULL) {
        ...
        if (strcmp((char *)lfirst(cell2), ccname) == 0)
          ereport(ERROR,
                  (errcode(ERRCODE_DUPLICATE_OBJECT),
                   errmsg("check constraint \"%s\" already exists", ccname)));
      }
       ...
       */
      if (MergeWithExistingConstraint(rel, ccname, expr, allow_merge, is_local,
                                      cdef->is_no_inherit))
        continue;
    } else {
      /*
      ...
      ccname =
          ChooseConstraintName(RelationGetRelationName(rel), colname, "check",
...
static bool MergeWithExistingConstraint(Relation rel, char *ccname, Node *expr,
  ...
  ScanKeyInit(&skey[0], Anum_pg_constraint_conname, BTEqualStrategyNumber,
              F_NAMEEQ, CStringGetDatum(ccname));

  ScanKeyInit(&skey[1], Anum_pg_constraint_connamespace, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(RelationGetNamespace(rel)));

  conscan = systable_beginscan(conDesc, ConstraintNameNspIndexId, true, NULL, 2,
                               skey);
  ...
  while (HeapTupleIsValid(tup = systable_getnext(conscan))) {
    ...
    if (con->conrelid == RelationGetRelid(rel)) {
      ...
      if (con->contype == CONSTRAINT_CHECK) {
        ...
        val = fastgetattr(tup, Anum_pg_constraint_conbin, conDesc->rd_att,
                          &isnull);
        if (isnull)
 ...
 */
Node *cookDefault(ParseState *pstate, Node *raw_default, Oid atttypid,
                  int32 atttypmod, char *attname) {
  Node *expr;
  ...
  if (OidIsValid(atttypid)) {
    ...
    expr = coerce_to_target_type(pstate, expr, type_id, atttypid, atttypmod,
        ...
        if (tempTables)
          ereport(ERROR,
                  (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                   ...
                   errdetail(
                       "Table \"%s\" references \"%s\", but they do not have "
                       "the same ON COMMIT setting.",
                       relname2, relname)));
        else
          ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                          errmsg(
                              ...
                              "key constraint"),
                          errdetail("Table \"%s\" references \"%s\".", relname2,
                                    relname),
                          errhint(
...
List *heap_truncate_find_FKs(List *relationIds) {
  ...
  while (HeapTupleIsValid(tuple = systable_getnext(fkeyScan))) {
    ...
    /* Not a foreign key */
    if (con->contype != CONSTRAINT_FOREIGN) continue;

    /* Not referencing one of our list of tables */
    if (!list_member_oid(relationIds, con->confrelid)) continue;
...
static List *insert_ordered_unique_oid(List *list, Oid datum) {
  ...
  if (list == NIL || datum < linitial_oid(list)) return lcons_oid(datum, list);
  /* Does it match the first entry? */
  if (datum == linitial_oid(list)) return list; /* duplicate, so don't insert */
  ...
  for (;;) {
    ...
    if (datum == lfirst_oid(curr)) return list; /* duplicate, so don't insert */
