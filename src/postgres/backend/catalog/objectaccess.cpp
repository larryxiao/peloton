 ...
 */
void RunObjectPostCreateHook(Oid classId, Oid objectId, int subId,
                             bool is_internal) {
  ...
  (*object_access_hook)(OAT_POST_CREATE, classId, objectId, subId,
                        (void *)&pc_arg);
}
...
bool RunNamespaceSearchHook(Oid objectId, bool ereport_on_violation) {
  ...
  (*object_access_hook)(OAT_NAMESPACE_SEARCH, NamespaceRelationId, objectId, 0,
                        (void *)&ns_arg);
...
void RunFunctionExecuteHook(Oid objectId) {
  ...
  (*object_access_hook)(OAT_FUNCTION_EXECUTE, ProcedureRelationId, objectId, 0,
                        NULL);
}
