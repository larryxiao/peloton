...
static AclMode restrict_and_check_grant(
    bool is_grant, AclMode avail_goptions, bool all_privs, AclMode privileges,
    Oid objectId, Oid grantorId, AclObjectKind objkind, const char *objname,
    AttrNumber att_number, const char *colname);
static AclMode pg_aclmask(AclObjectKind objkind, Oid table_oid,
                          AttrNumber attnum, Oid roleid, AclMode mask,
                          AclMaskHow how);
...
static void dumpacl(Acl *acl) {
  ...
  for (i = 0; i < ACL_NUM(acl); ++i)
    elog(DEBUG2, "	acl[%d]: %s", i,
         DatumGetCString(
             DirectFunctionCall1(aclitemout, PointerGetDatum(aip + i))));
}
#endif /* ACLDEBUG */
 ...
 */
static Acl *merge_acl_with_grant(Acl *old_acl, bool is_grant, bool grant_option,
                                 DropBehavior behavior, List *grantees,
                                 AclMode privileges, Oid grantorId,
                                 Oid ownerId) {
  unsigned modechg;
  ListCell *j;
  Acl *new_acl;
  ...
  foreach (j, grantees) {
    ...
    ACLITEM_SET_PRIVS_GOPTIONS(
        aclitem, (is_grant || !grant_option) ? privileges : ACL_NO_RIGHTS,
    ...
    if (this_privileges == 0) {
      if (objkind == ACL_KIND_COLUMN && colname)
        ereport(WARNING, (errcode(ERRCODE_WARNING_PRIVILEGE_NOT_GRANTED),
                          errmsg(
                              "no privileges were granted for column \"%s\" of "
                              "relation \"%s\"",
                              colname, objname)));
      else
        ereport(WARNING,
      ...
      if (objkind == ACL_KIND_COLUMN && colname)
        ereport(WARNING, (errcode(ERRCODE_WARNING_PRIVILEGE_NOT_GRANTED),
                          errmsg(
                              "not all privileges were granted for column "
                              "\"%s\" of relation \"%s\"",
                              colname, objname)));
      else
        ereport(WARNING, (errcode(ERRCODE_WARNING_PRIVILEGE_NOT_GRANTED),
    ...
    if (this_privileges == 0) {
      if (objkind == ACL_KIND_COLUMN && colname)
        ereport(WARNING, (errcode(ERRCODE_WARNING_PRIVILEGE_NOT_REVOKED),
                          errmsg(
                              "no privileges could be revoked for column "
                              "\"%s\" of relation \"%s\"",
                              colname, objname)));
      else
        ereport(WARNING,
      ...
      if (objkind == ACL_KIND_COLUMN && colname)
        ereport(WARNING, (errcode(ERRCODE_WARNING_PRIVILEGE_NOT_REVOKED),
                          errmsg(
                              "not all privileges could be revoked for column "
                              "\"%s\" of relation \"%s\"",
                              colname, objname)));
      else
        ereport(WARNING,
  ...
  foreach (cell, nspnames) {
    ...
    switch (objtype) {
      ...
      case ACL_OBJECT_FUNCTION: {
        ...
        ScanKeyInit(&key[0], Anum_pg_proc_pronamespace, BTEqualStrategyNumber,
...
static List *getRelationsInNamespace(Oid namespaceId, char relkind) {
  ...
  ScanKeyInit(&key[0], Anum_pg_class_relnamespace, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(namespaceId));
  ScanKeyInit(&key[1], Anum_pg_class_relkind, BTEqualStrategyNumber, F_CHAREQ,
...
void ExecAlterDefaultPrivilegesStmt(AlterDefaultPrivilegesStmt *stmt) {
  ...
  if (dnspnames) nspnames = (List *)dnspnames->arg;
  if (drolespecs) rolespecs = (List *)drolespecs->arg;
...
static void SetDefaultACL(InternalDefaultACL *iacls) {
  ...
  tuple =
      SearchSysCache3(DEFACLROLENSPOBJ, ObjectIdGetDatum(iacls->roleid),
                      ObjectIdGetDatum(iacls->nspid), CharGetDatum(objtype));
  ...
  if (aclequal(new_acl, def_acl)) {
    ...
    if (isNew) {
      /* insert new___ entry */
      values[Anum_pg_default_acl_defaclrole - 1] =
          ObjectIdGetDatum(iacls->roleid);
      values[Anum_pg_default_acl_defaclnamespace - 1] =
    ...
    if (isNew) {
      /* dependency on role */
      recordDependencyOnOwner(DefaultAclRelationId, HeapTupleGetOid(newtuple),
                              iacls->roleid);
    ...
    updateAclDependencies(DefaultAclRelationId, HeapTupleGetOid(newtuple), 0,
                          iacls->roleid, noldmembers, oldmembers, nnewmembers,
                          newmembers);
    ...
    if (isNew)
      ...
      InvokeObjectPostAlterHook(DefaultAclRelationId, HeapTupleGetOid(newtuple),
                                0);
  }

  if (HeapTupleIsValid(tuple)) ReleaseSysCache(tuple);
...
static void expand_all_col_privileges(Oid table_oid, Form_pg_class classForm,
  ...
  Assert(classForm->relnatts - FirstLowInvalidHeapAttributeNumber <
         num_col_privileges);
  for (curr_att = FirstLowInvalidHeapAttributeNumber + 1;
       curr_att <= classForm->relnatts; curr_att++) {
    HeapTuple attTuple;
    bool isdropped;

    if (curr_att == InvalidAttrNumber) continue;

    /* Skip OID column if it doesn't exist */
    if (curr_att == ObjectIdAttributeNumber && !classForm->relhasoids) continue;

    /* Views don't have any system columns at all */
    if (classForm->relkind == RELKIND_VIEW && curr_att < 0) continue;
    ...
    col_privileges[curr_att - FirstLowInvalidHeapAttributeNumber] |=
        this_privileges;
  }
}
 ...
 */
static void ExecGrant_Attribute(InternalGrant *istmt, Oid relOid,
                                const char *relname, AttrNumber attnum,
                                Oid ownerId, AclMode col_privileges,
  ...
  attr_tuple =
      SearchSysCache2(ATTNUM, ObjectIdGetDatum(relOid), Int16GetDatum(attnum));
  if (!HeapTupleIsValid(attr_tuple))
    elog(ERROR, "cache lookup failed for attribute %d of relation %u", attnum,
         relOid);
   ...
   */
  aclDatum =
      SysCacheGetAttr(ATTNUM, attr_tuple, Anum_pg_attribute_attacl, &isNull);
  if (isNull) {
   ...
   */
  col_privileges = restrict_and_check_grant(
      istmt->is_grant, avail_goptions,
      (col_privileges == ACL_ALL_RIGHTS_COLUMN), col_privileges, relOid,
      grantorId, ACL_KIND_COLUMN, relname, attnum,
  ...
  new_acl = merge_acl_with_grant(old_acl, istmt->is_grant, istmt->grant_option,
                                 ...
                                 col_privileges, grantorId, ownerId);
...
static void ExecGrant_Relation(InternalGrant *istmt) {
  ...
  foreach (cell, istmt->objects) {
      ...
      if (pg_class_tuple->relkind == RELKIND_SEQUENCE) {
        ...
        if (this_privileges & ~((AclMode)ACL_ALL_RIGHTS_SEQUENCE)) {
          ...
          ereport(WARNING, (errcode(ERRCODE_INVALID_GRANT_OPERATION),
                            errmsg(
                                "sequence \"%s\" only supports USAGE, SELECT, "
     ...
     */
    num_col_privileges =
        pg_class_tuple->relnatts - FirstLowInvalidHeapAttributeNumber + 1;
    ...
    if (this_privileges != ACL_NO_RIGHTS) {
      ...
      this_privileges = restrict_and_check_grant(
          istmt->is_grant, avail_goptions, istmt->all_privs, this_privileges,
          relOid, grantorId, aclkind, NameStr(pg_class_tuple->relname), 0,
          NULL);
      ...
      new_acl = merge_acl_with_grant(
          old_acl, istmt->is_grant, istmt->grant_option, istmt->behavior,
          istmt->grantees, this_privileges, grantorId, ownerId);
      ...
      /* Update the shared dependency ACL info */
      updateAclDependencies(RelationRelationId, relOid, 0, ownerId, noldmembers,
    ...
    if (have_col_privileges) {
      ...
      for (i = 0; i < num_col_privileges; i++) {
        if (col_privileges[i] == ACL_NO_RIGHTS) continue;
        ExecGrant_Attribute(istmt, relOid, NameStr(pg_class_tuple->relname),
                            i + FirstLowInvalidHeapAttributeNumber, ownerId,
                            col_privileges[i], attRelation, old_rel_acl);
      }
    }
...
static void ExecGrant_Database(InternalGrant *istmt) {
  ...
  foreach (cell, istmt->objects) {
     ...
     */
    this_privileges = restrict_and_check_grant(
        istmt->is_grant, avail_goptions, istmt->all_privs, istmt->privileges,
        datId, grantorId, ACL_KIND_DATABASE,
        NameStr(pg_database_tuple->datname), 0, NULL);
    ...
    updateAclDependencies(DatabaseRelationId, HeapTupleGetOid(tuple), 0,
                          ownerId, noldmembers, oldmembers, nnewmembers,
                          newmembers);
...
static void ExecGrant_Fdw(InternalGrant *istmt) {
  ...
  foreach (cell, istmt->objects) {
    ...
    this_privileges = restrict_and_check_grant(
        istmt->is_grant, avail_goptions, istmt->all_privs, istmt->privileges,
        fdwid, grantorId, ACL_KIND_FDW, NameStr(pg_fdw_tuple->fdwname), 0,
        NULL);
...
static void ExecGrant_ForeignServer(InternalGrant *istmt) {
  ...
  foreach (cell, istmt->objects) {
    ...
    this_privileges = restrict_and_check_grant(
        ...
        NameStr(pg_server_tuple->srvname), 0, NULL);
    ...
    /* Update the shared dependency ACL info */
    updateAclDependencies(ForeignServerRelationId, HeapTupleGetOid(tuple), 0,
                          ownerId, noldmembers, oldmembers, nnewmembers,
                          newmembers);
...
static void ExecGrant_Function(InternalGrant *istmt) {
  ...
  foreach (cell, istmt->objects) {
    ...
    this_privileges = restrict_and_check_grant(
        istmt->is_grant, avail_goptions, istmt->all_privs, istmt->privileges,
        funcId, grantorId, ACL_KIND_PROC, NameStr(pg_proc_tuple->proname), 0,
        NULL);
    ...
    /* Update the shared dependency ACL info */
    updateAclDependencies(ProcedureRelationId, funcId, 0, ownerId, noldmembers,
                          oldmembers, nnewmembers, newmembers);
...
static void ExecGrant_Language(InternalGrant *istmt) {
  ...
  foreach (cell, istmt->objects) {
    ...
    aclDatum =
        SysCacheGetAttr(LANGNAME, tuple, Anum_pg_language_lanacl, &isNull);
    if (isNull) {
    ...
    this_privileges = restrict_and_check_grant(
        ...
        NameStr(pg_language_tuple->lanname), 0, NULL);
...
static void ExecGrant_Largeobject(InternalGrant *istmt) {
  ...
  foreach (cell, istmt->objects) {
    ...
    scan = systable_beginscan(relation, LargeObjectMetadataOidIndexId, true,
                              NULL, 1, entry);
    ...
    this_privileges = restrict_and_check_grant(
        istmt->is_grant, avail_goptions, istmt->all_privs, istmt->privileges,
        loid, grantorId, ACL_KIND_LARGEOBJECT, loname, 0, NULL);
...
static void ExecGrant_Namespace(InternalGrant *istmt) {
  ...
  foreach (cell, istmt->objects) {
    ...
    this_privileges = restrict_and_check_grant(
        ...
        NameStr(pg_namespace_tuple->nspname), 0, NULL);
...
static void ExecGrant_Tablespace(InternalGrant *istmt) {
  ...
  foreach (cell, istmt->objects) {
    ...
    this_privileges = restrict_and_check_grant(
        ...
        NameStr(pg_tablespace_tuple->spcname), 0, NULL);
    ...
    /* Update the shared dependency ACL info */
    updateAclDependencies(TableSpaceRelationId, tblId, 0, ownerId, noldmembers,
                          oldmembers, nnewmembers, newmembers);
...
static void ExecGrant_Type(InternalGrant *istmt) {
  ...
  foreach (cell, istmt->objects) {
    ...
    this_privileges = restrict_and_check_grant(
        istmt->is_grant, avail_goptions, istmt->all_privs, istmt->privileges,
        typId, grantorId, ACL_KIND_TYPE, NameStr(pg_type_tuple->typname), 0,
        NULL);
    ...
    /* Update the shared dependency ACL info */
    updateAclDependencies(TypeRelationId, typId, 0, ownerId, noldmembers,
                          oldmembers, nnewmembers, newmembers);
...
static AclMode string_to_privilege(const char *privname) {
  ...
  if (strcmp(privname, "temporary") == 0) return ACL_CREATE_TEMP;
  if (strcmp(privname, "temp") == 0) return ACL_CREATE_TEMP;
  if (strcmp(privname, "connect") == 0) return ACL_CONNECT;
  if (strcmp(privname, "rule") == 0) return 0; /* ignore old RULE privileges */
  ereport(ERROR, (errcode(ERRCODE_SYNTAX_ERROR),
...
void aclcheck_error(AclResult aclerr, AclObjectKind objectkind,
...
void aclcheck_error_col(AclResult aclerr, AclObjectKind objectkind,
...
void aclcheck_error_type(AclResult aclerr, Oid typeOid) {
  ...
  aclcheck_error(aclerr, ACL_KIND_TYPE,
                 format_type_be(element_type ? element_type : typeOid));
}
...
static AclMode pg_aclmask(AclObjectKind objkind, Oid table_oid,
  ...
  switch (objkind) {
    ...
    case ACL_KIND_LARGEOBJECT:
      return pg_largeobject_aclmask_snapshot(table_oid, roleid, mask, how,
                                             NULL);
    case ACL_KIND_NAMESPACE:
    ...
    default:
      elog(ERROR, "unrecognized objkind: %d", (int)objkind);
      /* not reached, but keep compiler quiet */
      return ACL_NO_RIGHTS;
  }
}
 ...
 * superuser-ness here.)
 */
AclMode pg_attribute_aclmask(Oid table_oid, AttrNumber attnum, Oid roleid,
  ...
  aclDatum =
      SysCacheGetAttr(ATTNUM, attTuple, Anum_pg_attribute_attacl, &isNull);
  ...
  if (acl && (Pointer)acl != DatumGetPointer(aclDatum)) pfree(acl);
 ...
 */
AclMode pg_class_aclmask(Oid table_oid, Oid roleid, AclMode mask,
                         AclMaskHow how) {
  AclMode result;
  HeapTuple tuple;
  Form_pg_class classForm;
  ...
  if ((mask &
      ...
      classForm->relkind != RELKIND_VIEW && !superuser_arg(roleid) &&
 ...
 */
AclMode pg_database_aclmask(Oid db_oid, Oid roleid, AclMode mask,
                            AclMaskHow how) {
  AclMode result;
  HeapTuple tuple;
  Datum aclDatum;
  ...
  if (superuser_arg(roleid)) return mask;
  ...
  aclDatum =
      SysCacheGetAttr(DATABASEOID, tuple, Anum_pg_database_datacl, &isNull);
  if (isNull) {
    /* No ACL, so build default ACL */
 ...
 */
AclMode pg_proc_aclmask(Oid proc_oid, Oid roleid, AclMode mask,
                        AclMaskHow how) {
  AclMode result;
  HeapTuple tuple;
  Datum aclDatum;
 ...
 */
AclMode pg_language_aclmask(Oid lang_oid, Oid roleid, AclMode mask,
                            AclMaskHow how) {
  AclMode result;
  HeapTuple tuple;
  Datum aclDatum;
 ...
 */
AclMode pg_largeobject_aclmask_snapshot(Oid lobj_oid, Oid roleid, AclMode mask,
                                        AclMaskHow how, Snapshot snapshot) {
  AclMode result;
  Relation pg_lo_meta;
  ScanKeyData entry[1];
  SysScanDesc scan;
  ...
  ScanKeyInit(&entry[0], ObjectIdAttributeNumber, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(lobj_oid));
 ...
 */
AclMode pg_namespace_aclmask(Oid nsp_oid, Oid roleid, AclMode mask,
                             AclMaskHow how) {
  AclMode result;
  HeapTuple tuple;
  Datum aclDatum;
  ...
  /*
   * If we have been assigned this namescpace___ as a temp namescpace___, check
   *to
  ...
  if (isTempNamespace(nsp_oid)) {
    if (pg_database_aclcheck(MyDatabaseId, roleid, ACL_CREATE_TEMP) ==
        ACLCHECK_OK)
  ...
  aclDatum =
      SysCacheGetAttr(NAMESPACEOID, tuple, Anum_pg_namespace_nspacl, &isNull);
  if (isNull) {
    /* No ACL, so build default ACL */
 ...
 */
AclMode pg_tablespace_aclmask(Oid spc_oid, Oid roleid, AclMode mask,
                              AclMaskHow how) {
  AclMode result;
  HeapTuple tuple;
  Datum aclDatum;
  ...
  aclDatum =
      SysCacheGetAttr(TABLESPACEOID, tuple, Anum_pg_tablespace_spcacl, &isNull);
 ...
 * data wrapper
 */
AclMode pg_foreign_data_wrapper_aclmask(Oid fdw_oid, Oid roleid, AclMode mask,
                                        AclMaskHow how) {
  AclMode result;
  HeapTuple tuple;
  Datum aclDatum;
  ...
  if (!HeapTupleIsValid(tuple))
    ereport(ERROR, (errmsg("foreign-data wrapper with OID %u does not exist",
                           fdw_oid)));
 ...
 * server.
 */
AclMode pg_foreign_server_aclmask(Oid srv_oid, Oid roleid, AclMode mask,
                                  AclMaskHow how) {
  AclMode result;
  HeapTuple tuple;
  Datum aclDatum;
  ...
  if (!HeapTupleIsValid(tuple))
    ereport(ERROR,
            (errmsg("foreign server with OID %u does not exist", srv_oid)));
 ...
 */
AclMode pg_type_aclmask(Oid type_oid, Oid roleid, AclMode mask,
                        AclMaskHow how) {
  AclMode result;
  HeapTuple tuple;
  Datum aclDatum;
  ...
  if (!HeapTupleIsValid(tuple))
    ereport(ERROR, (errmsg("type with OID %u does not exist", type_oid)));
 ...
 */
AclResult pg_attribute_aclcheck(Oid table_oid, AttrNumber attnum, Oid roleid,
                                AclMode mode) {
 ...
 */
AclResult pg_attribute_aclcheck_all(Oid table_oid, Oid roleid, AclMode mode,
                                    AclMaskHow how) {
  AclResult result;
  HeapTuple classTuple;
  Form_pg_class classForm;
   ...
   */
  classTuple = SearchSysCache1(RELOID, ObjectIdGetDatum(table_oid));
  if (!HeapTupleIsValid(classTuple)) return ACLCHECK_NO_PRIV;
  classForm = (Form_pg_class)GETSTRUCT(classTuple);
  ...
  for (curr_att = 1; curr_att <= nattrs; curr_att++) {
    ...
    attTuple = SearchSysCache2(ATTNUM, ObjectIdGetDatum(table_oid),
                               Int16GetDatum(curr_att));
    if (!HeapTupleIsValid(attTuple)) continue;
    ...
    if (heap_attisnull(attTuple, Anum_pg_attribute_attacl))
      ...
      attmask =
          pg_attribute_aclmask(table_oid, curr_att, roleid, mode, ACLMASK_ANY);

    ReleaseSysCache(attTuple);

    if (attmask != 0) {
      result = ACLCHECK_OK;
      if (how == ACLMASK_ANY) break; /* succeed on any success */
    } else {
      result = ACLCHECK_NO_PRIV;
      if (how == ACLMASK_ALL) break; /* fail on any failure */
    }
  }
...
AclResult pg_largeobject_aclcheck_snapshot(Oid lobj_oid, Oid roleid,
  ...
  if (pg_largeobject_aclmask_snapshot(lobj_oid, roleid, mode, ACLMASK_ANY,
                                      snapshot) != 0)
    return ACLCHECK_OK;
  else
    return ACLCHECK_NO_PRIV;
 ...
 * data wrapper
 */
AclResult pg_foreign_data_wrapper_aclcheck(Oid fdw_oid, Oid roleid,
                                           AclMode mode) {
  ...
  if (!HeapTupleIsValid(tuple))
    ereport(ERROR,
            (errcode(ERRCODE_UNDEFINED_OBJECT),
             errmsg("operator___ family with OID %u does not exist", opf_oid)));
...
bool pg_event_trigger_ownercheck(Oid et_oid, Oid roleid) {
  ...
  if (!HeapTupleIsValid(tuple))
    ereport(ERROR,
            (errcode(ERRCODE_UNDEFINED_OBJECT),
             errmsg("event trigger with OID %u does not exist", et_oid)));
...
bool pg_extension_ownercheck(Oid ext_oid, Oid roleid) {
  ...
  scan = systable_beginscan(pg_extension, ExtensionOidIndexId, true, NULL, 1,
                            entry);
...
static Acl *get_default_acl_internal(Oid roleId, Oid nsp_oid, char objtype) {
  ...
  tuple = SearchSysCache3(DEFACLROLENSPOBJ, ObjectIdGetDatum(roleId),
                          ObjectIdGetDatum(nsp_oid), CharGetDatum(objtype));
...
Acl *get_user_default_acl(GrantObjectType objtype, Oid ownerId, Oid nsp_oid) {
  ...
  if (glob_acl == NULL && schema_acl == NULL) return NULL;
  ...
  if (glob_acl == NULL) glob_acl = def_acl;
  ...
  aclitemsort(result);
  aclitemsort(def_acl);
  if (aclequal(result, def_acl)) result = NULL;
