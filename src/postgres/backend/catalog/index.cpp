...
static TupleDesc ConstructTupleDescriptor(
    Relation heapRelation, IndexInfo *indexInfo, List *indexColNames,
...
static void UpdateIndexRelation(Oid indexoid, Oid heapoid, IndexInfo *indexInfo,
                                Oid *collationOids, Oid *classOids,
                                int16 *coloptions, bool primary,
                                bool isexclusion, bool immediate, bool isvalid);
static void index_update_stats(Relation rel, bool hasindex, bool isprimary,
                               double reltuples);
static void IndexCheckExclusion(Relation heapRelation, Relation indexRelation,
                                IndexInfo *indexInfo);
 ...
 */
void index_check_primary_key(Relation heapRel, IndexInfo *indexInfo,
                             bool is_alter_table) {
  List *cmds;
  int i;
  ...
  for (i = 0; i < indexInfo->ii_NumIndexAttrs; i++) {
    ...
    atttuple =
        SearchSysCache2(ATTNUM, ObjectIdGetDatum(RelationGetRelid(heapRel)),
   ...
   * unduly.
   */
  if (cmds) AlterTableInternal(RelationGetRelid(heapRel), cmds, false);
}
  ...
  for (i = 0; i < numatts; i++) {
    ...
    if (atnum != 0) {
      ...
      if (atnum < 0) {
         ...
         */
        from =
            SystemAttributeDefinition(atnum, heapRelation->rd_rel->relhasoids);
      } else {
        /*
       ...
       */
      CheckAttributeType(NameStr(to->attname), to->atttypid, to->attcollation,
                         NIL, false);
    }
...
static void UpdateIndexRelation(Oid indexoid, Oid heapoid, IndexInfo *indexInfo,
                                Oid *collationOids, Oid *classOids,
                                int16 *coloptions, bool primary,
                                bool isexclusion, bool immediate,
                                bool isvalid) {
  int2vector *indkey;
  ...
  if (exprsDatum == (Datum)0) nulls[Anum_pg_index_indexprs - 1] = true;
  values[Anum_pg_index_indpred - 1] = predDatum;
  if (predDatum == (Datum)0) nulls[Anum_pg_index_indpred - 1] = true;
...
Oid index_create(Relation heapRelation, const char *indexRelationName,
                 Oid indexRelationId, Oid relFileNode, IndexInfo *indexInfo,
                 List *indexColNames, Oid accessMethodObjectId,
                 Oid tableSpaceId, Oid *collationObjectId, Oid *classObjectId,
                 int16 *coloptions, Datum reloptions, bool isprimary,
                 bool isconstraint, bool deferrable, bool initdeferred,
                 bool allow_system_table_mods, bool skip_build, bool concurrent,
  ...
  if (!allow_system_table_mods && IsSystemRelation(heapRelation) &&
    ...
    ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                    errmsg(
                        "user-defined indexes on system catalog tables are not "
                        "supported")));
  ...
  if (concurrent && IsSystemRelation(heapRelation))
    ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                    errmsg(
                        "concurrent index creation on system catalog tables is "
                        "not supported")));
  ...
  if (concurrent && is_exclusion)
    ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                    errmsg_internal(
                        "concurrent index creation for exclusion constraints "
                        "is not supported")));
   ...
   */
  indexTupDesc = ConstructTupleDescriptor(heapRelation, indexInfo,
                                          indexColNames, accessMethodObjectId,
    ...
    if (IsBinaryUpgrade) {
      if (!OidIsValid(binary_upgrade_next_index_pg_class_oid))
        ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                        errmsg(
                            "pg_class index OID value not set when in binary "
                            "upgrade mode")));
   ...
   */
  indexRelation =
      heap_create(indexRelationName, namespaceId, tableSpaceId, indexRelationId,
                  relFileNode, indexTupDesc, RELKIND_INDEX, relpersistence,
                  shared_relation, mapped_relation, allow_system_table_mods);
   ...
   */
  InsertPgClassTuple(pg_class, indexRelation, RelationGetRelid(indexRelation),
                     (Datum)0, reloptions);
   ...
   */
  InitializeAttributeOids(indexRelation, indexInfo->ii_NumIndexAttrs,
                          indexRelationId);
  ...
  UpdateIndexRelation(indexRelationId, heapRelationId, indexInfo,
                      collationObjectId, classObjectId, coloptions, isprimary,
                      is_exclusion, !deferrable, !concurrent);
  ...
  if (!IsBootstrapProcessingMode()) {
    ...
    if (isconstraint) {
      ...
      index_constraint_create(heapRelation, indexRelationId, indexInfo,
                              indexRelationName, constraintType, deferrable,
                              initdeferred, false, /* already marked primary */
    ...
    if (indexInfo->ii_Expressions) {
      recordDependencyOnSingleRelExpr(
          &myself, (Node *)indexInfo->ii_Expressions, heapRelationId,
          DEPENDENCY_NORMAL, DEPENDENCY_AUTO);
    }

    /* Store dependencies on anything mentioned in predicate */
    if (indexInfo->ii_Predicate) {
      recordDependencyOnSingleRelExpr(&myself, (Node *)indexInfo->ii_Predicate,
                                      heapRelationId, DEPENDENCY_NORMAL,
                                      DEPENDENCY_AUTO);
    }
  } else {
  ...
  InvokeObjectPostCreateHookArg(RelationRelationId, indexRelationId, 0,
                                is_internal);
 ...
 */
ObjectAddress index_constraint_create(
    Relation heapRelation, Oid indexRelationId, IndexInfo *indexInfo,
  ...
  conOid = CreateConstraintEntry(
      constraintName, namespaceId, constraintType, deferrable, initdeferred,
      true, RelationGetRelid(heapRelation), indexInfo->ii_KeyAttrNumbers,
      indexInfo->ii_NumIndexAttrs, InvalidOid, /* no domain */
      indexRelationId,                         /* index OID */
      InvalidOid,                              /* no foreign key */
      NULL, NULL, NULL, NULL, 0, ' ', ' ', ' ', indexInfo->ii_ExclusionOps,
   ...
   */
  if (mark_as_primary) index_update_stats(heapRelation, true, true, -1.0);
  ...
  if (update_pgindex && (mark_as_primary || deferrable)) {
    ...
    indexTuple =
        SearchSysCacheCopy1(INDEXRELID, ObjectIdGetDatum(indexRelationId));
...
void index_drop(Oid indexId, bool concurrent) {
   ...
   * them.
   */
  if (hasexprs) RemoveStatistics(indexId, 0);
...
IndexInfo *BuildIndexInfo(Relation index) {
  ...
  if (numKeys < 1 || numKeys > INDEX_MAX_KEYS)
    elog(ERROR, "invalid indnatts %d for index %u", numKeys,
  ...
  if (indexStruct->indisexclusion) {
    RelationGetExclusionInfo(index, &ii->ii_ExclusionOps,
                             &ii->ii_ExclusionProcs, &ii->ii_ExclusionStrats);
  } else {
 ...
 * ----------------
 */
void FormIndexDatum(IndexInfo *indexInfo, TupleTableSlot *slot, EState *estate,
  ...
  if (indexInfo->ii_Expressions != NIL &&
    ...
    indexInfo->ii_ExpressionsState =
        (List *)ExecPrepareExpr((Expr *)indexInfo->ii_Expressions, estate);
...
void index_build(Relation heapRelation, Relation indexRelation,
                 IndexInfo *indexInfo, bool isprimary, bool isreindex) {
  ...
  ereport(DEBUG1, (errmsg("building index \"%s\" on table \"%s\"",
  ...
  stats = (IndexBuildResult *)DatumGetPointer(OidFunctionCall3(
      procedure, PointerGetDatum(heapRelation), PointerGetDatum(indexRelation),
 ...
 * chain tip.
 */
double IndexBuildHeapScan(Relation heapRelation, Relation indexRelation,
  ...
  return IndexBuildHeapRangeScan(heapRelation, indexRelation, indexInfo,
                                 allow_sync, 0, InvalidBlockNumber, callback,
                                 callback_state);
}
 ...
 */
double IndexBuildHeapRangeScan(Relation heapRelation, Relation indexRelation,
                               IndexInfo *indexInfo, bool allow_sync,
  ...
  while ((heapTuple = heap_getnext(scan, ForwardScanDirection)) != NULL) {
        ...
        case HEAPTUPLE_INSERT_IN_PROGRESS:
          ...
          if (!TransactionIdIsCurrentTransactionId(xwait)) {
            ...
            if (checking_uniqueness) {
               ...
               */
              LockBuffer(scan->rs_cbuf, BUFFER_LOCK_UNLOCK);
              XactLockTableWait(xwait, heapRelation, &heapTuple->t_self,
    ...
    if (HeapTupleIsHeapOnly(heapTuple)) {
      ...
      if (!OffsetNumberIsValid(root_offsets[offnum - 1]))
        elog(ERROR,
             "failed to find parent tuple for heap-only tuple at (%u,%u) in "
             "table \"%s\"",
             ItemPointerGetBlockNumber(&heapTuple->t_self), offnum,
             RelationGetRelationName(heapRelation));
...
static void IndexCheckExclusion(Relation heapRelation, Relation indexRelation,
  ...
  while ((heapTuple = heap_getnext(scan, ForwardScanDirection)) != NULL) {
    ...
    check_exclusion_constraint(heapRelation, indexRelation, indexInfo,
                               &(heapTuple->t_self), values, isnull, estate,
                               true);
  }
...
void validate_index(Oid heapId, Oid indexId, Snapshot snapshot) {
  ...
  state.tuplesort = tuplesort_begin_datum(TIDOID, TIDLessOperator, InvalidOid,
                                          false, maintenance_work_mem, false);
  state.htups = state.itups = state.tups_inserted = 0;

  (void)index_bulk_delete(&ivinfo, NULL, validate_index_callback,
                          (void *)&state);
   ...
   */
  validate_index_heapscan(heapRelation, indexRelation, indexInfo, snapshot,
                          &state);
  ...
  elog(DEBUG2,
       "validate_index found %.0f heap tuples, %.0f index tuples; inserted "
       "%.0f missing tuples",
...
static void validate_index_heapscan(Relation heapRelation,
  ...
  while ((heapTuple = heap_getnext(scan, ForwardScanDirection)) != NULL) {
    ...
    while (!tuplesort_empty &&
      ...
      tuplesort_empty =
          !tuplesort_getdatum(state->tuplesort, true, &ts_val, &ts_isnull);
...
Oid IndexGetRelation(Oid indexId, bool missing_ok) {
  ...
  tuple = SearchSysCache1(INDEXRELID, ObjectIdGetDatum(indexId));
  if (!HeapTupleIsValid(tuple)) {
    if (missing_ok) return InvalidOid;
    elog(ERROR, "cache lookup failed for index %u", indexId);
  }
 ...
 */
void reindex_index(Oid indexId, bool skip_constraint_checks, char persistence,
                   int options) {
  ...
  if (options & REINDEXOPT_VERBOSE)
    ereport(INFO, (errmsg("index \"%s\" was reindexed", get_rel_name(indexId)),
...
bool reindex_relation(Oid relid, int flags, int options) {
  ...
  if (is_pg_class) (void)RelationGetIndexAttrBitmap(rel, INDEX_ATTR_BITMAP_ALL);
  ...
  PG_TRY();
  {
    ...
    foreach (indexId, indexIds) {
      ...
      if (is_pg_class) RelationSetIndexList(rel, doneIndexes, InvalidOid);
      ...
      if (is_pg_class) doneIndexes = lappend_oid(doneIndexes, indexOid);
    }
  }
  PG_CATCH();
  {
  ...
  if (is_pg_class) RelationSetIndexList(rel, indexIds, ClassOidIndexId);
...
static void SetReindexPending(List *indexes) {
  /* Reindexing is not re-entrant. */
  if (pendingReindexedIndexes) elog(ERROR, "cannot reindex while reindexing");
 ...
 */
static void ResetReindexPending(void) { pendingReindexedIndexes = NIL; }
