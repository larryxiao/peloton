...
void AlterSetting(Oid databaseid, Oid roleid, VariableSetStmt *setstmt) {
  ...
  scan = systable_beginscan(rel, DbRoleSettingDatidRolidIndexId, true, NULL, 2,
                            scankey);
  ...
  if (setstmt->kind == VAR_RESET_ALL) {
    if (HeapTupleIsValid(tuple)) {
      ...
      if (!isnull) new___ = GUCArrayReset(DatumGetArrayTypeP(datum));
  ...
  InvokeObjectPostAlterHookArg(DbRoleSettingRelationId, databaseid, 0, roleid,
                               false);
...
void ApplySetting(Snapshot snapshot, Oid databaseid, Oid roleid,
  ...
  ScanKeyInit(&keys[0], Anum_pg_db_role_setting_setdatabase,
              BTEqualStrategyNumber, F_OIDEQ, ObjectIdGetDatum(databaseid));
  ScanKeyInit(&keys[1], Anum_pg_db_role_setting_setrole, BTEqualStrategyNumber,
