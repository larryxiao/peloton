...
ObjectAddress ConversionCreate(const char *conname, Oid connamespace,
                               Oid conowner, int32 conforencoding,
                               int32 contoencoding, Oid conproc, bool def) {
  int i;
  Relation rel;
  TupleDesc tupDesc;
  HeapTuple tup;
  ...
  if (SearchSysCacheExists2(CONNAMENSP, PointerGetDatum(conname),
...
void RemoveConversionById(Oid conversionOid) {
  ...
  ScanKeyInit(&scanKeyData, ObjectIdAttributeNumber, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(conversionOid));
 ...
 */
Oid FindDefaultConversion(Oid name_space, int32 for_encoding,
                          int32 to_encoding) {
  CatCList *catlist;
  HeapTuple tuple;
  ...
  catlist = SearchSysCacheList3(CONDEFAULT, ObjectIdGetDatum(name_space),
