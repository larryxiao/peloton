...
static void shdepChangeDep(Relation sdepRel, Oid classid, Oid objid,
                           int32 objsubid, Oid refclassid, Oid refobjid,
...
void recordSharedDependencyOn(ObjectAddress *depender,
  ...
  if (!isSharedObjectPinned(referenced->classId, referenced->objectId,
    ...
    shdepAddDependency(sdepRel, depender->classId, depender->objectId,
                       depender->objectSubId, referenced->classId,
                       referenced->objectId, deptype);
  }
...
static void shdepChangeDep(Relation sdepRel, Oid classid, Oid objid,
  ...
  ScanKeyInit(&key[3], Anum_pg_shdepend_objsubid, BTEqualStrategyNumber,
              F_INT4EQ, Int32GetDatum(objsubid));

  scan = systable_beginscan(sdepRel, SharedDependDependerIndexId, true, NULL, 4,
                            key);

  while ((scantup = systable_getnext(scan)) != NULL) {
    /* Ignore if not of the target dependency type */
    if (((Form_pg_shdepend)GETSTRUCT(scantup))->deptype != deptype) continue;
  ...
  if (isSharedObjectPinned(refclassid, refobjid, sdepRel)) {
    ...
    if (oldtup) simple_heap_delete(sdepRel, &oldtup->t_self);
  } else if (oldtup) {
  ...
  if (oldtup) heap_freetuple(oldtup);
}
...
void changeDependencyOnOwner(Oid classId, Oid objectId, Oid newOwnerId) {
  ...
  shdepChangeDep(sdepRel, classId, objectId, 0, AuthIdRelationId, newOwnerId,
   ...
   *----------
   */
  shdepDropDependency(sdepRel, classId, objectId, 0, true, AuthIdRelationId,
                      newOwnerId, SHARED_DEPENDENCY_ACL);
...
void updateAclDependencies(Oid classId, Oid objectId, int32 objsubId,
                           Oid ownerId, int noldmembers, Oid *oldmembers,
  ...
  if (noldmembers > 0 || nnewmembers > 0) {
    ...
    for (i = 0; i < nnewmembers; i++) {
      ...
      if (isSharedObjectPinned(AuthIdRelationId, roleid, sdepRel)) continue;

      shdepAddDependency(sdepRel, classId, objectId, objsubId, AuthIdRelationId,
                         roleid, SHARED_DEPENDENCY_ACL);
    }
  ...
  if (oldmembers) pfree(oldmembers);
  if (newmembers) pfree(newmembers);
}
 ...
 */
bool checkSharedDependencies(Oid classId, Oid objectId, char **detail_msg,
                             char **detail_log_msg) {
  Relation sdepRel;
  ScanKeyData key[2];
  SysScanDesc scan;
...
#define MAX_REPORTED_DEPS 100
  ...
  ScanKeyInit(&key[0], Anum_pg_shdepend_refclassid, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(classId));
  ScanKeyInit(&key[1], Anum_pg_shdepend_refobjid, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(objectId));

  scan = systable_beginscan(sdepRel, SharedDependReferenceIndexId, true, NULL,
                            2, key);
    ...
    if (sdepForm->dbid == MyDatabaseId) {
      if (numReportedDeps < MAX_REPORTED_DEPS) {
        ...
        storeObjectDescription(
            &descs, LOCAL_OBJECT, &object,
            static_cast<SharedDependencyType>(sdepForm->deptype), 0);
      } else
        numNotReportedDeps++;
      storeObjectDescription(
          &alldescs, LOCAL_OBJECT, &object,
          static_cast<SharedDependencyType>(sdepForm->deptype), 0);
    } else if (sdepForm->dbid == InvalidOid) {
      if (numReportedDeps < MAX_REPORTED_DEPS) {
        numReportedDeps++;
        storeObjectDescription(
            &descs, SHARED_OBJECT, &object,
            static_cast<SharedDependencyType>(sdepForm->deptype), 0);
      } else
        numNotReportedDeps++;
      storeObjectDescription(
          &alldescs, SHARED_OBJECT, &object,
          static_cast<SharedDependencyType>(sdepForm->deptype), 0);
    } else {
...
void copyTemplateDependencies(Oid templateDbId, Oid newDbId) {
  ...
  scan = systable_beginscan(sdepRel, SharedDependDependerIndexId, true, NULL, 1,
                            key);
...
void deleteSharedDependencyRecordsFor(Oid classId, Oid objectId,
  ...
  shdepDropDependency(sdepRel, classId, objectId, objectSubId,
                      (objectSubId == 0), InvalidOid, InvalidOid,
...
static void shdepDropDependency(Relation sdepRel, Oid classId, Oid objectId,
  ...
  scan = systable_beginscan(sdepRel, SharedDependDependerIndexId, true, NULL,
                            nkeys, key);
  ...
  while (HeapTupleIsValid(tup = systable_getnext(scan))) {
    ...
    if (OidIsValid(refclassId) && shdepForm->refclassid != refclassId) continue;
    if (OidIsValid(refobjId) && shdepForm->refobjid != refobjId) continue;
    ...
    case AuthIdRelationId:
      if (!SearchSysCacheExists1(AUTHOID, ObjectIdGetDatum(objectId)))
        ereport(ERROR, (errcode(ERRCODE_UNDEFINED_OBJECT),
                        errmsg("role %u was concurrently dropped", objectId)));
      break;
 ...
 */
      ...
      if (tablespace == NULL)
        ereport(ERROR,
                (errcode(ERRCODE_UNDEFINED_OBJECT),
                 errmsg("tablespace %u was concurrently dropped", objectId)));
      pfree(tablespace);
      break;
    }
#endif
    ...
    case DatabaseRelationId: {
      ...
      if (database == NULL)
        ereport(ERROR,
                (errcode(ERRCODE_UNDEFINED_OBJECT),
                 errmsg("database %u was concurrently dropped", objectId)));
      pfree(database);
      break;
    }
...
static void storeObjectDescription(StringInfo descs, objectType type,
  ...
  if (descs->len != 0) appendStringInfoChar(descs, '\n');
  ...
  switch (type) {
    ...
    case REMOTE_OBJECT:
      ...
      appendStringInfo(descs,
                       ngettext("%d object in %s", "%d objects in %s", count),
                       count, objdesc);
      break;
...
static bool isSharedObjectPinned(Oid classId, Oid objectId, Relation sdepRel) {
  ...
  if (HeapTupleIsValid(tup)) {
    ...
    if (shdepForm->deptype == SHARED_DEPENDENCY_PIN) result = true;
  }
...
void shdepDropOwned(List *roleids, DropBehavior behavior) {
  ...
  foreach (cell, roleids) {
    ...
    ScanKeyInit(&key[0], Anum_pg_shdepend_refclassid, BTEqualStrategyNumber,
                F_OIDEQ, ObjectIdGetDatum(AuthIdRelationId));
    ScanKeyInit(&key[1], Anum_pg_shdepend_refobjid, BTEqualStrategyNumber,
...
void shdepReassignOwned(List *roleids, Oid newrole) {
  ...
  foreach (cell, roleids) {
    ...
    if (isSharedObjectPinned(AuthIdRelationId, roleid, sdepRel)) {
      ...
      ereport(ERROR, (errcode(ERRCODE_DEPENDENT_OBJECTS_STILL_EXIST),
                      errmsg(
                          "cannot reassign ownership of objects owned by %s "
    ...
    while ((tuple = systable_getnext(scan)) != NULL) {
      ...
      if (sdepForm->deptype != SHARED_DEPENDENCY_OWNER) continue;
