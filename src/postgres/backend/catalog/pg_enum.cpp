...
void EnumValuesDelete(Oid enumTypeOid) {
  ...
  ScanKeyInit(&key[0], Anum_pg_enum_enumtypid, BTEqualStrategyNumber, F_OIDEQ,
 ...
 */
void AddEnumLabel(Oid enumTypeOid, const char *newVal, const char *neighbor,
    ...
    if (skipIfExists) {
      ereport(NOTICE,
              (errcode(ERRCODE_DUPLICATE_OBJECT),
               errmsg("enum label \"%s\" already exists, skipping", newVal)));
      return;
    } else
...
restart:
  ...
  for (i = 0; i < nelems; i++) existing[i] = &(list->members[i]->tuple);
  ...
  if (neighbor == NULL) {
    ...
    if (other_nbr_index < 0)
      ...
      newelemorder = (nbr_en->enumsortorder + other_nbr_en->enumsortorder) / 2;
  ...
  if (IsBinaryUpgrade) {
    ...
    if (neighbor != NULL)
      ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                      errmsg(
                          "ALTER TYPE ADD BEFORE/AFTER is incompatible with "
                          "binary upgrade")));
    ...
    for (;;) {
      ...
      for (i = 0; i < nelems; i++) {
        ...
        if (exists_oid & 1) continue; /* ignore odd Oids */
 ...
 */
static void RenumberEnumType(Relation pg_enum, HeapTuple *existing,
                             int nelems) {
  int i;
