...
static void findDependentObjects(const ObjectAddress *object, int flags,
                                 ObjectAddressStack *stack,
                                 ObjectAddresses *targetObjects,
                                 const ObjectAddresses *pendingObjects,
                                 Relation *depRel);
static void reportDependentObjects(const ObjectAddresses *targetObjects,
                                   DropBehavior behavior, int msglevel,
...
static bool object_address_present_add_flags(const ObjectAddress *object,
                                             int flags, ObjectAddresses *addrs);
...
static void deleteObjectsInList(ObjectAddresses *targetObjects,
  ...
  if (trackDroppedObjectsNeeded() && !(flags & PERFORM_DELETION_INTERNAL)) {
    for (i = 0; i < targetObjects->numrefs; i++) {
      ...
      if (extra->flags & DEPFLAG_ORIGINAL) original = true;
      if (extra->flags & DEPFLAG_NORMAL) normal = true;
      if (extra->flags & DEPFLAG_REVERSE) normal = true;
 ...
 * a column default is dropped as an intermediate step while adding a new___
 *one,
 ...
 */
void performDeletion(const ObjectAddress *object, DropBehavior behavior,
                     int flags) {
  Relation depRel;
  ObjectAddresses *targetObjects;
...
void performMultipleDeletions(const ObjectAddresses *objects,
  ...
  for (i = 0; i < objects->numrefs; i++) {
    ...
    findDependentObjects(thisobj, DEPFLAG_ORIGINAL, NULL, /* empty stack */
   ...
   */
  reportDependentObjects(targetObjects, behavior, NOTICE,
...
void peloton_performMultipleDeletions(const ObjectAddresses *objects,
                                      DropBehavior behavior, int flags) {
  Relation depRel;
  ObjectAddresses *targetObjects;
  int i;
...
void deleteWhatDependsOn(const ObjectAddress *object, bool showNotices) {
   ...
   * Check if deletion is allowed, and report about cascaded deletes.
   */
  reportDependentObjects(targetObjects, DROP_CASCADE,
                         showNotices ? NOTICE : DEBUG2, object);
  ...
  for (i = 0; i < targetObjects->numrefs; i++) {
    ...
    if (thisextra->flags & DEPFLAG_ORIGINAL) continue;
...
static void findDependentObjects(const ObjectAddress *object, int flags,
   ...
   */
  if (stack_address_present_add_flags(object, flags, stack)) return;
   ...
   */
  if (object_address_present_add_flags(object, flags, targetObjects)) return;
  ...
  scan = systable_beginscan(*depRel, DependDependerIndexId, true, NULL, nkeys,
                            key);
  ...
  while (HeapTupleIsValid(tup = systable_getnext(scan))) {
    ...
    switch (foundDep->deptype) {
      ...
      case DEPENDENCY_EXTENSION:
         ...
         */
        if (stack_address_present_add_flags(&otherObject, 0, stack)) break;
         ...
         */
        findDependentObjects(&otherObject, DEPFLAG_REVERSE, stack,
  ...
  scan = systable_beginscan(*depRel, DependReferenceIndexId, true, NULL, nkeys,
                            key);
  ...
  while (HeapTupleIsValid(tup = systable_getnext(scan))) {
    ...
    findDependentObjects(&otherObject, subflags, &mystack, targetObjects,
...
#define MAX_REPORTED_DEPS 100
  ...
  for (i = targetObjects->numrefs - 1; i >= 0; i--) {
    ...
    if (extra->flags & DEPFLAG_ORIGINAL) continue;
    ...
    if (extra->flags & (DEPFLAG_AUTO | DEPFLAG_INTERNAL | DEPFLAG_EXTENSION)) {
      ...
      if (numReportedClient < MAX_REPORTED_DEPS) {
        ...
        if (clientdetail.len != 0) appendStringInfoChar(&clientdetail, '\n');
        appendStringInfo(&clientdetail, _("%s depends on %s"), objDesc,
                         otherDesc);
        numReportedClient++;
      } else
        numNotReportedClient++;
      /* separate entries with a newline */
      if (logdetail.len != 0) appendStringInfoChar(&logdetail, '\n');
  ...
  if (!ok) {
    if (origObject)
      ...
      ereport(
          ...
          (errcode(ERRCODE_DEPENDENT_OBJECTS_STILL_EXIST),
           errmsg(
               "cannot drop desired object(s) because other objects depend on "
               "them"),
           errdetail("%s", clientdetail.data),
           errdetail_log("%s", logdetail.data),
           errhint("Use DROP ... CASCADE to drop the dependent objects too.")));
 ...
 */
void recordDependencyOnSingleRelExpr(const ObjectAddress *depender, Node *expr,
                                     Oid relId, DependencyType behavior,
  ...
  if (behavior != self_behavior && context.addrs->numrefs > 0) {
    ...
    recordMultipleDependencies(depender, self_addrs->refs, self_addrs->numrefs,
                               self_behavior);
...
static bool find_expr_references_walker(Node *node,
  ...
  if (IsA(node, Var)) {
     ...
     */
    if (var->varattno == InvalidAttrNumber) return false;
    if (rte->rtekind == RTE_RELATION) {
      ...
      find_expr_references_walker(
          (Node *)list_nth(rte->joinaliasvars, var->varattno - 1), context);
    ...
    result = query_tree_walker(query, reinterpret_cast<query_tree_walker_fptr>(
                                          find_expr_references_walker),
...
static void eliminate_duplicate_dependencies(ObjectAddresses *addrs) {
  ...
  if (addrs->numrefs <= 1) return; /* nothing to do */
...
static void add_object_address(ObjectClass oclass, Oid objectId, int32 subId,
  ...
  if (addrs->numrefs >= addrs->maxrefs) {
    addrs->maxrefs *= 2;
    addrs->refs = (ObjectAddress *)repalloc(
        addrs->refs, addrs->maxrefs * sizeof(ObjectAddress));
    Assert(!addrs->extras);
  }
  /* record this item */
...
static void add_exact_object_address_extra(const ObjectAddress *object,
  ...
  if (!addrs->extras)
    addrs->extras = (ObjectAddressExtra *)palloc(addrs->maxrefs *
  ...
  if (addrs->numrefs >= addrs->maxrefs) {
    ...
    addrs->refs = (ObjectAddress *)repalloc(
        addrs->refs, addrs->maxrefs * sizeof(ObjectAddress));
    addrs->extras = (ObjectAddressExtra *)repalloc(
        addrs->extras, addrs->maxrefs * sizeof(ObjectAddressExtra));
  }
  /* record this item */
  item = addrs->refs + addrs->numrefs;
  *item = *object;
...
void free_object_addresses(ObjectAddresses *addrs) {
  pfree(addrs->refs);
  if (addrs->extras) pfree(addrs->extras);
  pfree(addrs);
}
