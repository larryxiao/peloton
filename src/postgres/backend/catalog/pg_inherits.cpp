...
List *find_inheritance_children(Oid parentrelId, LOCKMODE lockmode) {
   ...
   * subclass.
   */
  if (!has_subclass(parentrelId)) return NIL;
  ...
  ScanKeyInit(&key[0], Anum_pg_inherits_inhparent, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(parentrelId));

  scan =
      systable_beginscan(relation, InheritsParentIndexId, true, NULL, 1, key);
 ...
 * against possible DROPs of child relations.
 */
List *find_all_inheritors(Oid parentrelId, LOCKMODE lockmode,
                          List **numparents) {
...
bool typeInheritsFrom(Oid subclassTypeId, Oid superclassTypeId) {
  ...
  if (subclassRelid == InvalidOid) return false; /* not a complex type */
  superclassRelid = typeidTypeRelid(superclassTypeId);
  if (superclassRelid == InvalidOid) return false; /* not a complex type */

  /* No point in searching if the superclass has no subclasses */
  if (!has_subclass(superclassRelid)) return false;
  ...
  foreach (queue_item, queue) {
    ...
    inhscan = systable_beginscan(inhrel, InheritsRelidSeqnoIndexId, true, NULL,
                                 1, &skey);
