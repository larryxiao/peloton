 ...
 * ----------------------------------------------------------------
 */
ObjectAddress ProcedureCreate(
    const char *procedureName, Oid procNamespace, bool replace, bool returnsSet,
    Oid returnType, Oid proowner, Oid languageObjectId, Oid languageValidator,
    const char *prosrc, const char *probin, bool isAgg, bool isWindowFunc,
    bool security_definer, bool isLeakProof, bool isStrict, char volatility,
    oidvector *parameterTypes, Datum allParameterTypes, Datum parameterModes,
    Datum parameterNames, List *parameterDefaults, Datum trftypes,
  ...
  if (parameterModes != PointerGetDatum(NULL)) {
    ...
    if (ARR_NDIM(modesArray) != 1 || ARR_DIMS(modesArray)[0] != allParamCount ||
  ...
  if (allParameterTypes != PointerGetDatum(NULL)) {
    for (i = 0; i < allParamCount; i++) {
      if (paramModes == NULL || paramModes[i] == PROARGMODE_IN ||
  ...
  if ((IsPolymorphicType(returnType) || genericOutParam) && !genericInParam)
    ereport(ERROR, (errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
                    ...
                    errdetail(
                        "A function returning a polymorphic type must have at "
  ...
  if ((returnType == ANYRANGEOID || anyrangeOutParam) && !anyrangeInParam)
    ereport(ERROR, (errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
                    errmsg("cannot determine result data type"),
                    errdetail(
                        "A function returning \"anyrange\" must have at least "
                        "one \"anyrange\" argument.")));
  ...
  if (parameterCount == 1 && OidIsValid(parameterTypes->values[0]) &&
    ...
    ereport(ERROR,
            (errcode(ERRCODE_DUPLICATE_COLUMN),
             errmsg("\"%s\" is already an attribute of type %s", procedureName,
  ...
  if (parameterDefaults != NIL)
    values[Anum_pg_proc_proargdefaults - 1] =
        CStringGetTextDatum(nodeToString(parameterDefaults));
  else
  ...
  oldtup = SearchSysCache3(PROCNAMEARGSNSP, PointerGetDatum(procedureName),
  ...
  if (HeapTupleIsValid(oldtup)) {
    ...
    if (!isnull) {
      ...
      n_old_arg_names =
          get_func_input_arg_names(proargnames, proargmodes, &old_arg_names);
      n_new_arg_names = get_func_input_arg_names(parameterNames, parameterModes,
                                                 &new_arg_names);
      for (j = 0; j < n_old_arg_names; j++) {
        if (old_arg_names[j] == NULL) continue;
    ...
    if (oldproc->pronargdefaults != 0) {
      ...
      for (i = list_length(parameterDefaults) - oldproc->pronargdefaults; i > 0;
           i--)
        newlc = lnext(newlc);
      ...
      foreach (oldlc, oldDefaults) {
        ...
        if (exprType(oldDef) != exprType(newDef))
          ereport(ERROR, (errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
                          errmsg(
                              "cannot change data type of existing parameter "
                              "default value"),
                          errhint("Use DROP FUNCTION %s first.",
                                  format_procedure(HeapTupleGetOid(oldtup)))));
        newlc = lnext(newlc);
   ...
   */
  if (is_update) deleteDependencyRecordsFor(ProcedureRelationId, retval, true);
  ...
  if (!is_update && proacl != NULL) {
    ...
    updateAclDependencies(ProcedureRelationId, retval, 0, proowner, 0, NULL,
  ...
  if (OidIsValid(languageValidator)) {
    ...
    if (check_function_bodies) {
      ...
      if (set_items) /* Need a new___ GUC nesting level */
      {
        ...
        ProcessGUCArray(set_items, (superuser() ? PGC_SUSET : PGC_USERSET),
    ...
    if (set_items) AtEOXact_GUC(true, save_nestlevel);
  }
...
Datum fmgr_c_validator(PG_FUNCTION_ARGS) {
  ...
  tmp = SysCacheGetAttr(PROCOID, tuple, Anum_pg_proc_prosrc, &isnull);
  if (isnull) elog(ERROR, "null prosrc for C function %u", funcoid);
  prosrc = TextDatumGetCString(tmp);

  tmp = SysCacheGetAttr(PROCOID, tuple, Anum_pg_proc_probin, &isnull);
  if (isnull) elog(ERROR, "null probin for C function %u", funcoid);
...
Datum fmgr_sql_validator(PG_FUNCTION_ARGS) {
  ...
  if (get_typtype(proc->prorettype) == TYPTYPE_PSEUDO &&
      proc->prorettype != RECORDOID && proc->prorettype != VOIDOID &&
  ...
  if (check_function_bodies) {
    ...
    if (!haspolyarg) {
      ...
      (void)check_sql_fn_retval(funcoid, proc->prorettype, querytree_list, NULL,
                                NULL);
    }
...
bool function_parse_error_transpose(const char *prosrc) {
  ...
  if (origerrposition <= 0) {
    ...
    if (origerrposition <= 0) return false;
  }
...
static int match_prosrc_to_query(const char *prosrc, const char *queryText,
  ...
  for (curpos = 0; curpos < querylen - prosrclen; curpos++) {
    if (queryText[curpos] == '$' &&
       ...
       */
      if (matchpos) return 0; /* multiple matches, fail */
...
static bool match_prosrc_to_literal(const char *prosrc, const char *literal,
  ...
  while (*prosrc) {
    ...
    if (*literal == '\\') {
      literal++;
      if (cursorpos > 0) newcp++;
    } else if (*literal == '\'') {
      if (literal[1] != '\'') goto fail;
      literal++;
      if (cursorpos > 0) newcp++;
    }
    chlen = pg_mblen(prosrc);
    if (strncmp(prosrc, literal, chlen) != 0) goto fail;
    prosrc += chlen;
    literal += chlen;
  }
...
List *oid_array_to_list(Datum datum) {
  ...
  deconstruct_array(array, OIDOID, sizeof(Oid), true, 'i', &values, NULL,
                    &nelems);
  for (i = 0; i < nelems; i++) result = lappend_oid(result, values[i]);
  return result;
}
