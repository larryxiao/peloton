 ...
 * AggregateCreate
 */
ObjectAddress AggregateCreate(
    const char *aggName, Oid aggNamespace, char aggKind, int numArgs,
    int numDirectArgs, oidvector *parameterTypes, Datum allParameterTypes,
    Datum parameterModes, Datum parameterNames, List *parameterDefaults,
    Oid variadicArgType, List *aggtransfnName, List *aggfinalfnName,
    List *aggmtransfnName, List *aggminvtransfnName, List *aggmfinalfnName,
    bool finalfnExtraArgs, bool mfinalfnExtraArgs, List *aggsortopName,
    Oid aggTransType, int32 aggTransSpace, Oid aggmTransType,
    int32 aggmTransSpace, const char *agginitval, const char *aggminitval) {
  Relation aggdesc;
  HeapTuple tup;
  ...
  if (IsPolymorphicType(aggTransType) && !hasPolyArg)
    ereport(ERROR, (errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
                    ...
                    errdetail(
                        "An aggregate using a polymorphic transition type must "
                        "have at least one polymorphic argument.")));

  /*
   * Likewise for moving-aggregate transtype, if any
   */
  if (OidIsValid(aggmTransType) && IsPolymorphicType(aggmTransType) &&
      !hasPolyArg)
    ereport(ERROR, (errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
                    errmsg("cannot determine transition data type"),
                    errdetail(
  ...
  if (aggKind == AGGKIND_HYPOTHETICAL && numDirectArgs < numArgs) {
    ...
    if (OidIsValid(variadicArgType) || numDirectArgs < numAggregatedArgs ||
      ...
      ereport(ERROR, (errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
                      errmsg(
                          "a hypothetical-set aggregate must have direct "
                          "arguments matching its aggregated arguments")));
  }
  ...
  if (proc->proisstrict && agginitval == NULL) {
    if (numArgs < 1 || !IsBinaryCoercible(aggArgTypes[0], aggTransType))
      ereport(ERROR, (errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
                      errmsg(
                          "must not omit initial value when transition "
                          "function is strict and transition type is not "
  ...
  if (aggmtransfnName) {
    ...
    mtransfn = lookup_agg_function(aggmtransfnName, nargs_transfn, fnArgs,
                                   variadicArgType, &rettype);
  ...
  if (aggminvtransfnName) {
    ...
    minvtransfn = lookup_agg_function(aggminvtransfnName, nargs_transfn, fnArgs,
                                      variadicArgType, &rettype);
  ...
  if (aggfinalfnName) {
    ...
    finalfn = lookup_agg_function(aggfinalfnName, nargs_finalfn, fnArgs,
                                  ffnVariadicArgType, &finaltype);
    ...
    if (finalfnExtraArgs && func_strict(finalfn))
      ereport(ERROR, (errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
                      errmsg(
                          "final function with extra arguments must not be "
                          "declared STRICT")));
  } else {
    /*
  ...
  if (IsPolymorphicType(finaltype) && !hasPolyArg)
    ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH),
                    ...
                    errdetail(
                        "An aggregate returning a polymorphic type "
  ...
  if (finaltype == INTERNALOID && !hasInternalArg)
    ereport(ERROR, (errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
                    ...
                    errdetail(
                        "A function returning \"internal\" must have at least "
  ...
  if (OidIsValid(aggmTransType)) {
    ...
    if (aggmfinalfnName) {
      ...
      mfinalfn = lookup_agg_function(aggmfinalfnName, nargs_finalfn, fnArgs,
                                     ffnVariadicArgType, &rettype);
    ...
    if (rettype != finaltype)
      ereport(ERROR, (errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
                      errmsg(
                          "moving-aggregate implementation returns type %s, "
                          "but plain implementation returns type %s",
                          format_type_be(aggmTransType),
                          format_type_be(aggTransType))));
  }
  ...
  if (aggsortopName) {
    if (numArgs != 1)
      ereport(ERROR, (errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
                      errmsg(
                          "sort operator___ can only be specified for "
                          "single-argument aggregates")));
    sortop = LookupOperName(NULL, aggsortopName, aggArgTypes[0], aggArgTypes[0],
                            false, -1);
  }
  ...
  if (aclresult != ACLCHECK_OK) aclcheck_error_type(aclresult, aggTransType);

  if (OidIsValid(aggmTransType)) {
    aclresult = pg_type_aclcheck(aggmTransType, GetUserId(), ACL_USAGE);
    if (aclresult != ACLCHECK_OK) aclcheck_error_type(aclresult, aggmTransType);
  }

  aclresult = pg_type_aclcheck(finaltype, GetUserId(), ACL_USAGE);
  if (aclresult != ACLCHECK_OK) aclcheck_error_type(aclresult, finaltype);
  ...
  myself = ProcedureCreate(aggName, aggNamespace, false, /* no replacement */
  ...
  values[Anum_pg_aggregate_aggmfinalextra - 1] =
      BoolGetDatum(mfinalfnExtraArgs);
  ...
  if (aggminitval)
    values[Anum_pg_aggregate_aggminitval - 1] =
        CStringGetTextDatum(aggminitval);
  else
...
static Oid lookup_agg_function(List *fnName, int nargs, Oid *input_types,
  ...
  fdresult = func_get_detail(fnName, NIL, NIL, nargs, input_types, false, false,
                             &fnOid, rettype, &retset, &nvargs, &vatype,
                             &true_oid_array, NULL);
  ...
  if (variadicArgType == ANYOID && vatype != ANYOID)
    ereport(ERROR,
            (errcode(ERRCODE_DATATYPE_MISMATCH),
             errmsg(
                 "function %s must accept VARIADIC ANY to be used in this "
                 "aggregate",
                 func_signature_string(fnName, nargs, NIL, input_types))));
