...
Oid CreateConstraintEntry(
    const char *constraintName, Oid constraintNamespace, char constraintType,
    bool isDeferrable, bool isDeferred, bool isValidated, Oid relId,
    const int16 *constraintKey, int constraintNKeys, Oid domainId,
    Oid indexRelId, Oid foreignRelId, const int16 *foreignKey,
    const Oid *pfEqOp, const Oid *ppEqOp, const Oid *ffEqOp, int foreignNKeys,
    char foreignUpdateType, char foreignDeleteType, char foreignMatchType,
    const Oid *exclOp, Node *conExpr, const char *conBin, const char *conSrc,
    bool conIsLocal, int conInhCount, bool conNoInherit, bool is_internal) {
  Relation conDesc;
  Oid conOid;
  HeapTuple tup;
  ...
  if (constraintNKeys > 0) {
    ...
    conkeyArray =
        construct_array(conkey, constraintNKeys, INT2OID, 2, true, 's');
  } else
    conkeyArray = NULL;
  ...
  if (foreignNKeys > 0) {
    ...
    confkeyArray =
        construct_array(fkdatums, foreignNKeys, INT2OID, 2, true, 's');
    for (i = 0; i < foreignNKeys; i++)
      fkdatums[i] = ObjectIdGetDatum(pfEqOp[i]);
    conpfeqopArray =
        construct_array(fkdatums, foreignNKeys, OIDOID, sizeof(Oid), true, 'i');
    for (i = 0; i < foreignNKeys; i++)
  ...
  if (exclOp != NULL) {
    ...
    conexclopArray = construct_array(opdatums, constraintNKeys, OIDOID,
                                     sizeof(Oid), true, 'i');
  } else
    conexclopArray = NULL;
  ...
  values[Anum_pg_constraint_connamespace - 1] =
 ...
 */
bool ConstraintNameIsUsed(ConstraintCategory conCat, Oid objId,
...
char *ChooseConstraintName(const char *name1, const char *name2,
                           const char *label, Oid namespaceid, List *others) {
  int pass = 0;
  char *conname = NULL;
...
void RenameConstraintById(Oid conId, const char *newname) {
   ...
   */
  if (OidIsValid(con->conrelid) &&
      ConstraintNameIsUsed(CONSTRAINT_RELATION, con->conrelid,
                           con->connamespace, newname))
    ereport(ERROR,
            (errcode(ERRCODE_DUPLICATE_OBJECT),
             errmsg("constraint \"%s\" for relation \"%s\" already exists",
                    newname, get_rel_name(con->conrelid))));
  if (OidIsValid(con->contypid) &&
      ConstraintNameIsUsed(CONSTRAINT_DOMAIN, con->contypid, con->connamespace,
                           newname))
 ...
 */
void AlterConstraintNamespaces(Oid ownerId, Oid oldNspId, Oid newNspId,
  ...
  if (isType) {
    ScanKeyInit(&key[0], Anum_pg_constraint_contypid, BTEqualStrategyNumber,
                F_OIDEQ, ObjectIdGetDatum(ownerId));
 ...
 */
void get_constraint_relation_oids(Oid constraint_oid, Oid *conrelid,
                                  Oid *confrelid) {
  HeapTuple tup;
  Form_pg_constraint con;
 ...
 */
Oid get_relation_constraint_oid(Oid relid, const char *conname,
                                bool missing_ok) {
...
Oid get_domain_constraint_oid(Oid typid, const char *conname, bool missing_ok) {
  ...
  ScanKeyInit(&skey[0], Anum_pg_constraint_contypid, BTEqualStrategyNumber,
              F_OIDEQ, ObjectIdGetDatum(typid));

  scan = systable_beginscan(pg_constraint, ConstraintTypidIndexId, true, NULL,
                            1, skey);
 ...
 * in pg_constraint.
 */
bool check_functional_grouping(Oid relid, Index varno, Index varlevelsup,
  ...
  while (HeapTupleIsValid(tuple = systable_getnext(scan))) {
    ...
    if (con->contype != CONSTRAINT_PRIMARY) continue;
    /* Constraint must be non-deferrable */
    if (con->condeferrable) continue;
    ...
    for (i = 0; i < numkeys; i++) {
      ...
      foreach (gl, grouping_columns) {
        ...
        if (IsA(gvar, Var) && gvar->varno == varno &&
            gvar->varlevelsup == varlevelsup && gvar->varattno == attnum) {
          found_col = true;
          break;
        }
      }
