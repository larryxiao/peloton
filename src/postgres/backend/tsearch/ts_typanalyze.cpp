...
static void compute_tsvector_stats(VacAttrStats *stats,
  ...
  lexemes_tab =
      hash_create("Analyzed lexemes table", num_mcelem, &hash_ctl,
  ...
  for (vector_no = 0; vector_no < samplerows; vector_no++) {
    ...
    for (j = 0; j < vector->size; j++) {
      ...
      item = (TrackItem *)hash_search(lexemes_tab, (const void *)&hash_key,
                                      HASH_ENTER, &found);
  ...
  if (null_cnt < samplerows) {
    ...
    if (num_mcelem > 0) {
      ...
      for (i = 0; i < num_mcelem; i++) {
        ...
        mcelem_values[i] = PointerGetDatum(
            cstring_to_text_with_len(item->key.lexeme, item->key.length));
...
static void prune_lexemes_hashtable(HTAB *lexemes_tab, int b_current) {
  ...
  while ((item = (TrackItem *)hash_seq_search(&scan_status)) != NULL) {
    if (item->frequency + item->delta <= b_current) {
      if (hash_search(lexemes_tab, (const void *)&item->key, HASH_REMOVE,
                      NULL) == NULL)
