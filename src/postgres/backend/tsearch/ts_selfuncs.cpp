...
static Selectivity mcelem_tsquery_selec(TSQuery query, Datum *mcelem,
                                        int nmcelem, float4 *numbers,
                                        int nnumbers);
static Selectivity tsquery_opr_selec(QueryItem *item, char *operand,
                                     TextFreq *lookup, int length,
                                     float4 minfreq);
...
static Selectivity tsquerysel(VariableStatData *vardata, Datum constval) {
  ...
  if (query->size == 0) return (Selectivity)0.0;
  ...
  if (HeapTupleIsValid(vardata->statsTuple)) {
    ...
    if (get_attstatsslot(vardata->statsTuple, TEXTOID, -1,
                         STATISTIC_KIND_MCELEM, InvalidOid, NULL, &values,
...
static Selectivity mcelem_tsquery_selec(TSQuery query, Datum *mcelem,
   ...
   */
  if (nnumbers != nmcelem + 2) return tsquery_opr_selec_no_stats(query);
  ...
  selec = tsquery_opr_selec(GETQUERY(query), GETOPERAND(query), lookup, nmcelem,
                            minfreq);
...
static Selectivity tsquery_opr_selec(QueryItem *item, char *operand,
  ...
  if (item->type == QI_VAL) {
    ...
    if (oper->prefix) {
      ...
      if (lookup == NULL) return (Selectivity)DEFAULT_TS_MATCH_SEL;

      searchres = (TextFreq *)bsearch(&key, lookup, length, sizeof(TextFreq),
    ...
    switch (item->qoperator.oper) {
      case OP_NOT:
        selec =
            1.0 - tsquery_opr_selec(item + 1, operand, lookup, length, minfreq);
        break;

      case OP_AND:
        s1 = tsquery_opr_selec(item + 1, operand, lookup, length, minfreq);
        s2 = tsquery_opr_selec(item + item->qoperator.left, operand, lookup,
                               length, minfreq);
        selec = s1 * s2;
        break;
