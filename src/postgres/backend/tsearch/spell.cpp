...
void NIStartBuild(IspellDict *Conf) {
  ...
  Conf->buildCxt = AllocSetContextCreate(
      CurTransactionContext, "Ispell dictionary init context",
...
static void *compact_palloc0(IspellDict *Conf, size_t size) {
  ...
  if (size > COMPACT_MAX_REQ) return palloc0(size);
...
static int cmpspellaffix(const void *s1, const void *s2) {
  return (strncmp((*(SPELL * const *)s1)->p.flag,
                  (*(SPELL * const *)s2)->p.flag, MAXFLAGLEN));
}

static char *findchar(char *str, int c) {
  while (*str) {
    if (t_iseq(str, c)) return str;
    str += pg_mblen(str);
  }
...
static int strbncmp(const unsigned char *s1, const unsigned char *s2,
                    size_t count) {
  int l1 = strlen((const char *)s1) - 1, l2 = strlen((const char *)s2) - 1,
      l = count;
...
static void NIAddSpell(IspellDict *Conf, const char *word, const char *flag) {
  if (Conf->nspell >= Conf->mspell) {
    if (Conf->mspell) {
      Conf->mspell *= 2;
      Conf->Spell =
          (SPELL **)repalloc(Conf->Spell, Conf->mspell * sizeof(SPELL *));
    } else {
      Conf->mspell = 1024 * 20;
  ...
  if (!tsearch_readline_begin(&trst, filename))
    ereport(ERROR,
            (errcode(ERRCODE_CONFIG_FILE_ERROR),
             errmsg("could not open dictionary file \"%s\": %m", filename)));
...
static int FindWord(IspellDict *Conf, const char *word, int affixflag,
                    int flag) {
  ...
  while (node && *ptr) {
    ...
    while (StopLow < StopHigh) {
      ...
      if (StopMiddle->val == *ptr) {
        if (*(ptr + 1) == '\0' && StopMiddle->isword) {
          if (flag == 0) {
            if (StopMiddle->compoundflag & FF_COMPOUNDONLY) return 0;
          } else if ((flag & StopMiddle->compoundflag) == 0)
            return 0;

          if ((affixflag == 0) ||
              (strchr(Conf->AffixData[StopMiddle->affix], affixflag) != NULL))
            return 1;
        }
    ...
    if (StopLow >= StopHigh) break;
  }
  return 0;
}

static void NIAddAffix(IspellDict *Conf, int flag, char flagflags,
                       const char *mask, const char *find, const char *repl,
                       int type) {
  AFFIX *Affix;
  ...
  if ((Affix->flagflags & FF_COMPOUNDONLY) ||
      (Affix->flagflags & FF_COMPOUNDPERMITFLAG)) {
  ...
  while (*str) {
    if (state == PAE_WAIT_MASK) {
      ...
      if (t_iseq(str, '-')) {
        ...
        ereport(ERROR,
                (errcode(ERRCODE_CONFIG_FILE_ERROR), errmsg("syntax error")));
...
static void addFlagValue(IspellDict *Conf, char *s, uint32 val) {
...
static void NIImportOOAffixes(IspellDict *Conf, const char *filename) {
  ...
  while ((recoded = tsearch_readline(&trst)) != NULL) {
    ...
    if (STRNCMP(recoded, "COMPOUNDFLAG") == 0)
      addFlagValue(Conf, recoded + strlen("COMPOUNDFLAG"), FF_COMPOUNDFLAG);
    else if (STRNCMP(recoded, "COMPOUNDBEGIN") == 0)
      addFlagValue(Conf, recoded + strlen("COMPOUNDBEGIN"), FF_COMPOUNDBEGIN);
    else if (STRNCMP(recoded, "COMPOUNDLAST") == 0)
      addFlagValue(Conf, recoded + strlen("COMPOUNDLAST"), FF_COMPOUNDLAST);
    /* COMPOUNDLAST and COMPOUNDEND are synonyms */
    else if (STRNCMP(recoded, "COMPOUNDEND") == 0)
      addFlagValue(Conf, recoded + strlen("COMPOUNDEND"), FF_COMPOUNDLAST);
    else if (STRNCMP(recoded, "COMPOUNDMIDDLE") == 0)
      addFlagValue(Conf, recoded + strlen("COMPOUNDMIDDLE"), FF_COMPOUNDMIDDLE);
    else if (STRNCMP(recoded, "ONLYINCOMPOUND") == 0)
      addFlagValue(Conf, recoded + strlen("ONLYINCOMPOUND"), FF_COMPOUNDONLY);
  ...
  sprintf(scanbuf, "%%6s %%%ds %%%ds %%%ds %%%ds", BUFSIZ / 5, BUFSIZ / 5,
          BUFSIZ / 5, BUFSIZ / 5);
  ...
  while ((recoded = tsearch_readline(&trst)) != NULL) {
    ...
    if (scanread == 4) {
      if (strlen(sflag) != 1) goto nextline;
      flag = *sflag;
      ...
      if (strlen(sflag) != 1 || flag != *sflag || flag == 0) goto nextline;
...
void NIImportAffixes(IspellDict *Conf, const char *filename) {
  ...
  while ((recoded = tsearch_readline(&trst)) != NULL) {
    ...
    if (STRNCMP(recoded, "COMPOUNDFLAG") == 0 ||
        STRNCMP(recoded, "COMPOUNDMIN") == 0 || STRNCMP(recoded, "PFX") == 0 ||
        STRNCMP(recoded, "SFX") == 0)
      goto isnewformat;

    if ((!suffixes) && (!prefixes)) goto nextline;

    if (!parse_affentry(pstr, mask, find, repl)) goto nextline;
...
isnewformat:
  if (oldformat)
    ereport(
        ...
        (errcode(ERRCODE_CONFIG_FILE_ERROR),
         errmsg(
             "affix file contains both old-style and new___-style commands")));
  tsearch_readline_end(&trst);
...
static int MergeAffix(IspellDict *Conf, int a1, int a2) {
  ...
  while (Conf->nAffixData + 1 >= Conf->lenAffixData) {
    Conf->lenAffixData *= 2;
    Conf->AffixData =
        (char **)repalloc(Conf->AffixData, sizeof(char *) * Conf->lenAffixData);
  }
    ...
    if (Conf->Spell[i]->p.d.len > level) {
      ...
      if (Conf->Spell[i]->p.d.len == level + 1) {
        ...
        if (data->isword && data->affix != Conf->Spell[i]->p.d.affix) {
          ...
          clearCompoundOnly =
              (FF_COMPOUNDONLY & data->compoundflag &
               makeCompoundFlags(Conf, Conf->Spell[i]->p.d.affix))
                  ? false
                  : true;
          data->affix =
              MergeAffix(Conf, data->affix, Conf->Spell[i]->p.d.affix);
        } else
        ...
        if (clearCompoundOnly) data->compoundflag &= ~FF_COMPOUNDONLY;
      }
    }
...
void NISortDictionary(IspellDict *Conf) {
  ...
  for (i = 0; i < Conf->nspell; i++) {
    if (i == 0 ||
        strncmp(Conf->Spell[i]->p.flag, Conf->Spell[i - 1]->p.flag, MAXFLAGLEN))
      naffix++;
  }
  ...
  curaffix = -1;
  for (i = 0; i < Conf->nspell; i++) {
    if (i == 0 || strncmp(Conf->Spell[i]->p.flag, Conf->AffixData[curaffix],
                          MAXFLAGLEN)) {
      curaffix++;
...
static AffixNode *mkANode(IspellDict *Conf, int low, int high, int level,
                          int type) {
  int i;
  int nchar = 0;
  uint8 lastchar = '\0';
...
static bool isAffixInUse(IspellDict *Conf, char flag) {
  int i;

  for (i = 0; i < Conf->nAffixData; i++)
    if (strchr(Conf->AffixData[i], flag) != NULL) return true;
...
void NISortAffixes(IspellDict *Conf) {
  ...
  if (Conf->naffixes == 0) return;

  if (Conf->naffixes > 1)
    qsort((void *)Conf->Affix, Conf->naffixes, sizeof(AFFIX), cmpaffix);
  Conf->CompoundAffix = ptr =
      (CMPDAffix *)palloc(sizeof(CMPDAffix) * Conf->naffixes);
  ptr->affix = NULL;

  for (i = 0; i < Conf->naffixes; i++) {
    Affix = &(((AFFIX *)Conf->Affix)[i]);
    if (Affix->type == FF_SUFFIX && i < firstsuffix) firstsuffix = i;

    if ((Affix->flagflags & FF_COMPOUNDFLAG) && Affix->replen > 0 &&
        isAffixInUse(Conf, (char)Affix->flag)) {
      if (ptr == Conf->CompoundAffix || ptr->issuffix != (ptr - 1)->issuffix ||
          strbncmp((const unsigned char *)(ptr - 1)->affix,
                   (const unsigned char *)Affix->repl, (ptr - 1)->len)) {
  ...
  ptr->affix = NULL;
  Conf->CompoundAffix = (CMPDAffix *)repalloc(
      Conf->CompoundAffix, sizeof(CMPDAffix) * (ptr - Conf->CompoundAffix + 1));
...
static AffixNodeData *FindAffixes(AffixNode *node, const char *word, int wrdlen,
                                  int *level, int type) {
  AffixNodeData *StopLow, *StopHigh, *StopMiddle;
  uint8 symbol;

  if (node->isvoid) { /* search void affixes */
    if (node->data->naff) return node->data;
    node = node->data->node;
  }
  ...
  while (node && *level < wrdlen) {
    ...
    while (StopLow < StopHigh) {
      ...
      if (StopMiddle->val == symbol) {
        (*level)++;
        if (StopMiddle->naff) return StopMiddle;
        node = StopMiddle->node;
        break;
...
static char *CheckAffix(const char *word, size_t len, AFFIX *Affix,
                        int flagflags, char *newword, int *baselen) {
  /*
   * Check compound allow flags
   */

  if (flagflags == 0) {
    if (Affix->flagflags & FF_COMPOUNDONLY) return NULL;
  } else if (flagflags & FF_COMPOUNDBEGIN) {
    if (Affix->flagflags & FF_COMPOUNDFORBIDFLAG) return NULL;
    if ((Affix->flagflags & FF_COMPOUNDBEGIN) == 0)
      if (Affix->type == FF_SUFFIX) return NULL;
    ...
    if ((Affix->flagflags & FF_COMPOUNDLAST) == 0)
      if (Affix->type == FF_PREFIX) return NULL;
  }
  ...
  if (Affix->type == FF_SUFFIX) {
     ...
     */
    if (baselen && *baselen + strlen(Affix->find) <= Affix->replen) return NULL;
  ...
  if (Affix->issimple)
    ...
    if (RS_execute(&(Affix->reg.regis), newword)) return newword;
  } else {
    int err;
    pg_wchar *data;
    size_t data_len;
    ...
    if (!(err = pg_regexec(&(Affix->reg.regex), data, data_len, 0, NULL, 0,
                           NULL, 0))) {
      pfree(data);
      return newword;
    }
...
static int addToResult(char **forms, char **cur, char *word) {
  if (cur - forms >= MAX_NORM - 1) return 0;
        ...
        while (pnode) {
          ...
          for (j = 0; j < prefix->naff; j++) {
            if (CheckAffix(newword, swrdlen, prefix->aff[j], flag, pnewword,
                           &baselen)) {
              /* prefix success */
              int ff = (prefix->aff[j]->flagflags & suffix->aff[i]->flagflags &
                        FF_CROSSPRODUCT)
                           ? 0
                           : prefix->aff[j]->flag;
...
static int CheckCompoundAffixes(CMPDAffix **ptr, char *word, int len,
                                bool CheckInPlace) {
  bool issuffix;
  ...
  if (CheckInPlace) {
    ...
    while ((*ptr)->affix) {
      if (len > (*ptr)->len &&
          (affbegin = strstr(word, (*ptr)->affix)) != NULL) {
...
static SplitVar *SplitToVariants(IspellDict *Conf, SPNode *snode,
                                 SplitVar *orig, char *word, int wordlen,
                                 int startpos, int minpos) {
  SplitVar *var = NULL;
  ...
  while (level < wordlen) {
    ...
    while (level > startpos &&
           (lenaff = CheckCompoundAffixes(&caff, word + level, wordlen - level,
                                          (node) ? true : false)) >= 0) {
      /*
      ...
      if (!notprobed[startpos + lenaff - 1]) continue;

      if (level + lenaff - 1 <= minpos) continue;

      if (lenaff >= MAXNORMLEN) continue; /* skip too big value */
      if (lenaff > 0) memcpy(buf, word + startpos, lenaff);
      buf[lenaff] = '\0';
      ...
      if (subres) {
        ...
        ptr->next = SplitToVariants(Conf, NULL, new___, word, wordlen,
    ...
    if (StopLow < StopHigh) {
      ...
      /* find infinitive */
      if (StopMiddle->isword && (StopMiddle->compoundflag & compoundflag) &&
          notprobed[level]) {
        ...
        if (level > minpos) {
          ...
          if (wordlen == level + 1) {
            ...
            while (ptr->next) ptr = ptr->next;
            ptr->next = SplitToVariants(Conf, node, var, word, wordlen,
                                        startpos, level);
...
static void addNorm(TSLexeme **lres, TSLexeme **lcur, char *word, int flags,
                    uint16 NVariant) {
  if (*lres == NULL)
...
TSLexeme *NINormalizeWord(IspellDict *Conf, char *word) {
  ...
  if (Conf->usecompound) {
    ...
    while (var) {
      if (var->nstem > 1) {
        char **subres =
            NormalizeSubWord(Conf, var->stem[var->nstem - 1], FF_COMPOUNDLAST);
        ...
        if (subres) {
          ...
          while (*subptr) {
            for (i = 0; i < var->nstem - 1; i++) {
              addNorm(&lres, &lcur,
                      (subptr == subres) ? var->stem[i] : pstrdup(var->stem[i]),
                      0, NVariant);
            }
      ...
      for (i = 0; i < var->nstem && var->stem[i]; i++) pfree(var->stem[i]);
      ptr = var->next;
      pfree(var->stem);
      pfree(var);
      var = ptr;
