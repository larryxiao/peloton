...
char *get_tsearch_config_filename(const char *basename, const char *extension) {
   ...
   */
  if (strspn(basename, "abcdefghijklmnopqrstuvwxyz0123456789_") !=
      strlen(basename))
  ...
  snprintf(result, MAXPGPATH, "%s/tsearch_data/%s.%s", sharepath, basename,
           extension);
 ...
 */
void readstoplist(const char *fname, StopList *s,
  ...
  if (fname && *fname) {
    ...
    while ((line = tsearch_readline(&trst)) != NULL) {
      ...
      /* Trim trailing space */
      while (*pbuf && !t_isspace(pbuf)) pbuf += pg_mblen(pbuf);
      *pbuf = '\0';
