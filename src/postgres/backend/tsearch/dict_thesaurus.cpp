...
static void newLexeme(DictThesaurus *d, char *b, char *e, uint32 idsubst,
                      uint16 posinsubst) {
  TheLexeme *ptr;
...
static void addWrd(DictThesaurus *d, char *b, char *e, uint32 idsubst,
                   uint16 nwrd, uint16 posinsubst, bool useasis) {
  static int nres = 0;
  static int ntres = 0;
  TheSubstitute *ptr;
  ...
  if (nwrd == 0) {
    ...
    if (idsubst >= d->nsubst) {
      if (d->nsubst == 0) {
        ...
        d->nsubst *= 2;
        d->subst = (TheSubstitute *)repalloc(d->subst,
                                             sizeof(TheSubstitute) * d->nsubst);
      }
    }
  }
...
static void thesaurusRead(char *filename, DictThesaurus *d) {
  ...
  while ((line = tsearch_readline(&trst)) != NULL) {
    ...
    /* is it a comment? */
    while (*ptr && t_isspace(ptr)) ptr += pg_mblen(ptr);
...
static TheLexeme *addCompiledLexeme(TheLexeme *newwrds, int *nnw, int *tnm,
                                    TSLexeme *lexeme, LexemeInfo *src,
                                    uint16 tnvariant) {
  if (*nnw >= *tnm) {
    *tnm *= 2;
...
static int cmpLexemeInfo(LexemeInfo *a, LexemeInfo *b) {
  ...
  if (a->idsubst == b->idsubst) {
    if (a->posinsubst == b->posinsubst) {
      if (a->tnvariant == b->tnvariant) return 0;
...
static int cmpTheLexeme(const void *a, const void *b) {
  ...
  if ((res = cmpLexeme(la, lb)) != 0) return res;
...
static void compileTheLexeme(DictThesaurus *d) {
  ...
  for (i = 0; i < d->nwrds; i++) {
    ...
    if (strcmp(d->wrds[i].lexeme, "?") == 0) /* Is stop word marker? */
      newwrds =
          addCompiledLexeme(newwrds, &nnw, &tnm, NULL, d->wrds[i].entries, 0);
    else {
      ptr = (TSLexeme *)DatumGetPointer(FunctionCall4(
          &(d->subdict->lexize), PointerGetDatum(d->subdict->dictData),
          PointerGetDatum(d->wrds[i].lexeme),
          Int32GetDatum(strlen(d->wrds[i].lexeme)), PointerGetDatum(NULL)));

      if (!ptr)
        ereport(ERROR,
                (errcode(ERRCODE_CONFIG_FILE_ERROR),
                 errmsg(
                     "thesaurus sample word \"%s\" isn't recognized by "
                     "subdictionary (rule %d)",
        ...
        ereport(ERROR,
                (errcode(ERRCODE_CONFIG_FILE_ERROR),
                 errmsg("thesaurus sample word \"%s\" is a stop word (rule %d)",
                        d->wrds[i].lexeme, d->wrds[i].entries->idsubst + 1),
                 errhint(
                     "Use \"?\" to represent a stop word within a sample "
                     "phrase.")));
      else {
        while (ptr->lexeme) {
          ...
          while (remptr->lexeme) {
            if (remptr->nvariant != (remptr - 1)->nvariant) break;
            tnvar++;
            remptr++;
          }
          ...
          while (remptr->lexeme && remptr->nvariant == curvar) {
            newwrds = addCompiledLexeme(newwrds, &nnw, &tnm, remptr,
                                        d->wrds[i].entries, tnvar);
            remptr++;
          }
  ...
  if (d->nwrds > 1) {
    ...
    while (ptrwrds - d->wrds < d->nwrds) {
      if (cmpLexeme(ptrwrds, newwrds) == 0) {
        ...
        if (ptrwrds->lexeme) pfree(ptrwrds->lexeme);
      } else {
        newwrds++;
        *newwrds = *ptrwrds;
      }
  ...
  for (i = 0; i < d->nsubst; i++) {
    ...
    while (inptr && inptr->lexeme) {
      ...
      if (inptr->flags & DT_USEASIS) { /* do not lexize */
        ...
        lexized = (TSLexeme *)DatumGetPointer(FunctionCall4(
            &(d->subdict->lexize), PointerGetDatum(d->subdict->dictData),
            PointerGetDatum(inptr->lexeme),
            Int32GetDatum(strlen(inptr->lexeme)), PointerGetDatum(NULL)));
      }

      if (lexized && lexized->lexeme) {
        int toset = (lexized->lexeme && outptr != d->subst[i].res)
                        ? (outptr - d->subst[i].res)
                        : -1;
        ...
        while (lexized->lexeme) {
          if (outptr - d->subst[i].res + 1 >= n) {
            ...
            n *= 2;
            d->subst[i].res =
                (TSLexeme *)repalloc(d->subst[i].res, sizeof(TSLexeme) * n);
        ...
        if (toset > 0) d->subst[i].res[toset].flags |= TSL_ADDPOS;
      } else if (lexized) {
        ereport(
            ERROR,
            (errcode(ERRCODE_CONFIG_FILE_ERROR),
        ...
        ereport(ERROR, (errcode(ERRCODE_CONFIG_FILE_ERROR),
                        errmsg(
                            "thesaurus substitute word \"%s\" isn't recognized "
                            "by subdictionary (rule %d)",
                            inptr->lexeme, i + 1)));
      }

      if (inptr->lexeme) pfree(inptr->lexeme);
      inptr++;
    }
...
Datum thesaurus_init(PG_FUNCTION_ARGS) {
  ...
  d->subdictOid =
      get_ts_dict_oid(stringToQualifiedNameList(subdictname), false);
...
static LexemeInfo *findTheLexeme(DictThesaurus *d, char *lexeme) {
  ...
  if (d->nwrds == 0) return NULL;

  key.lexeme = lexeme;
  key.entries = NULL;

  res = static_cast<TheLexeme *>(
      bsearch(&key, d->wrds, d->nwrds, sizeof(TheLexeme), cmpLexemeQ));
...
static LexemeInfo *findVariant(LexemeInfo *in, LexemeInfo *stored,
                               uint16 curpos, LexemeInfo **newin, int newn) {
  for (;;) {
    int i;
    ...
    /* step forward */
    for (i = 0; i < newn; i++) newin[i] = newin[i]->nextentry;
  }
}
...
static TSLexeme *checkMatch(DictThesaurus *d, LexemeInfo *info, uint16 curpos,
                            bool *moreres) {
  *moreres = false;
  while (info) {
    Assert(info->idsubst < d->nsubst);
    if (info->nextvariant) *moreres = true;
...
Datum thesaurus_lexize(PG_FUNCTION_ARGS) {
  ...
  res = (TSLexeme *)DatumGetPointer(FunctionCall4(
      &(d->subdict->lexize), PointerGetDatum(d->subdict->dictData),
      PG_GETARG_DATUM(1), PG_GETARG_DATUM(2), PointerGetDatum(NULL)));
  ...
  if (res && res->lexeme) {
    ...
    while (ptr->lexeme) {
      ...
      for (i = 0; i < nlex; i++)
        if ((infos[i] = findTheLexeme(d, basevar[i].lexeme)) == NULL) break;
