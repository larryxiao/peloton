...
static int compareWORD(const void *a, const void *b) {
  ...
  res = tsCompareString(
      ...
      ((const ParsedWord *)b)->word, ((const ParsedWord *)b)->len, false);

  if (res == 0) {
    if (((const ParsedWord *)a)->pos.pos == ((const ParsedWord *)b)->pos.pos)
      return 0;

    res = (((const ParsedWord *)a)->pos.pos > ((const ParsedWord *)b)->pos.pos)
              ? 1
              : -1;
  }
    ...
    if (!(ptr->len == res->len &&
      ...
      if (res->pos.apos[0] < MAXNUMPOS - 1 &&
        ...
        if (res->pos.apos[0] + 1 >= res->alen) {
          res->alen *= 2;
          res->pos.apos =
              (uint16 *)repalloc(res->pos.apos, sizeof(uint16) * res->alen);
        }
        if (res->pos.apos[0] == 0 ||
            res->pos.apos[res->pos.apos[0]] != LIMITPOS(ptr->pos.pos)) {
...
TSVector make_tsvector(ParsedText *prs) {
  ...
  for (i = 0; i < prs->curwords; i++) {
    ...
    if (prs->words[i].alen) {
      ...
      lenstr +=
          sizeof(uint16) + prs->words[i].pos.apos[0] * sizeof(WordEntryPos);
    }
  }
  ...
  for (i = 0; i < prs->curwords; i++) {
    ...
    if (prs->words[i].alen) {
      ...
      if (k > 0xFFFF) elog(ERROR, "positions array too long");
...
Datum to_tsvector_byid(PG_FUNCTION_ARGS) {
  ...
  prs.lenwords = (VARSIZE(in) - VARHDRSZ) / 6; /* just estimation of
                                                * word's number */
  if (prs.lenwords == 0) prs.lenwords = 2;
  prs.curwords = 0;
  prs.pos = 0;
...
Datum to_tsvector(PG_FUNCTION_ARGS) {
  ...
  PG_RETURN_DATUM(DirectFunctionCall2(to_tsvector_byid, ObjectIdGetDatum(cfgId),
                                      PointerGetDatum(in)));
}
 ...
 */
static void pushval_morph(Datum opaque, TSQueryParserState state, char *strval,
    ...
    while (count < prs.curwords) {
      ...
      while (count < prs.curwords && pos == prs.words[count].pos.pos) {
        ...
        while (count < prs.curwords && pos == prs.words[count].pos.pos &&
               variant == prs.words[count].nvariant) {
          pushValue(
              state, prs.words[count].word, prs.words[count].len, weight,
              ((prs.words[count].flags & TSL_PREFIX) || prefix) ? true : false);
          pfree(prs.words[count].word);
          if (cnt) pushOperator(state, OP_AND);
          cnt++;
          count++;
        }

        if (cntvar) pushOperator(state, OP_OR);
        cntvar++;
      }

      if (cntpos) pushOperator(state, OP_AND);
...
Datum to_tsquery_byid(PG_FUNCTION_ARGS) {
  ...
  query = parse_tsquery(text_to_cstring(in), pushval_morph,
                        ObjectIdGetDatum(cfgid), false);

  if (query->size == 0) PG_RETURN_TSQUERY(query);
  ...
  if (len != query->size) {
    ...
    query->size = len;
    memmove((void *)GETOPERAND(query), oldoperand,
...
Datum to_tsquery(PG_FUNCTION_ARGS) {
  ...
  cfgId = getTSCurrentConfig(true);
  PG_RETURN_DATUM(DirectFunctionCall2(to_tsquery_byid, ObjectIdGetDatum(cfgId),
                                      PointerGetDatum(in)));
}
