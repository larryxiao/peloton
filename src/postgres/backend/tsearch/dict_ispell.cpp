...
Datum dispell_init(PG_FUNCTION_ARGS) {
  ...
  foreach (l, dictoptions) {
    ...
    if (pg_strcasecmp(defel->defname, "DictFile") == 0) {
      ...
      NIImportDictionary(
          &(d->obj), get_tsearch_config_filename(defGetString(defel), "dict"));
      dictloaded = true;
      ...
      NIImportAffixes(
          &(d->obj), get_tsearch_config_filename(defGetString(defel), "affix"));
      affloaded = true;
...
Datum dispell_lexize(PG_FUNCTION_ARGS) {
  ...
  if (len <= 0) PG_RETURN_POINTER(NULL);

  txt = lowerstr_with_len(in, len);
  res = NINormalizeWord(&(d->obj), txt);

  if (res == NULL) PG_RETURN_POINTER(NULL);
  ...
  for (ptr = cptr; ptr->lexeme; ptr++) {
    if (searchstoplist(&(d->stoplist), ptr->lexeme)) {
      ...
      if (cptr != ptr) memcpy(cptr, ptr, sizeof(TSLexeme));
      cptr++;
    }
  }
  cptr->lexeme = NULL;
