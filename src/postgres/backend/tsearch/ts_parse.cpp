...
static void setNewTmpRes(LexizeData *ld, ParsedLex *lex, TSLexeme *res) {
  if (ld->tmpRes) {
    ...
    for (ptr = ld->tmpRes; ptr->lexeme; ptr++) pfree(ptr->lexeme);
    pfree(ld->tmpRes);
  }
  ld->tmpRes = res;
  ld->lastRes = lex;
...
static TSLexeme *LexizeExec(LexizeData *ld, ParsedLex **correspondLexem) {
  ...
  if (ld->curDictId == InvalidOid) {
    ...
    while (ld->towork.head) {
      ...
      if (curVal->type == 0 || curVal->type >= ld->cfg->lenmap ||
          map->len == 0) {
      ...
      for (i = ld->posDict; i < map->len; i++) {
        ...
        res = (TSLexeme *)DatumGetPointer(FunctionCall4(
            &(dict->lexize), PointerGetDatum(dict->dictData),
            PointerGetDatum(curValLemm), Int32GetDatum(curValLenLemm),
        ...
        if (ld->dictState.getnext) {
          ...
          if (res) setNewTmpRes(ld, curVal, res);
    ...
    while (ld->curSub) {
      ...
      res = (TSLexeme *)DatumGetPointer(FunctionCall4(
          &(dict->lexize), PointerGetDatum(dict->dictData),
          PointerGetDatum(curVal->lemm), Int32GetDatum(curVal->lenlemm),
          PointerGetDatum(&ld->dictState)));
...
void parsetext(Oid cfgId, ParsedText *prs, char *buf, int buflen) {
  ...
  prsdata = (void *)DatumGetPointer(FunctionCall2(
      &prsobj->prsstart, PointerGetDatum(buf), Int32GetDatum(buflen)));

  LexizeInit(&ldata, cfg);

  do {
    type = DatumGetInt32(
        FunctionCall3(&(prsobj->prstoken), PointerGetDatum(prsdata),
                      PointerGetDatum(&lemm), PointerGetDatum(&lenlemm)));
    ...
    while ((norms = LexizeExec(&ldata, NULL)) != NULL) {
      ...
      while (ptr->lexeme) {
        if (prs->curwords == prs->lenwords) {
          prs->lenwords *= 2;
          prs->words = (ParsedWord *)repalloc(
              (void *)prs->words, prs->lenwords * sizeof(ParsedWord));
        }

        if (ptr->flags & TSL_ADDPOS) prs->pos++;
 ...
 * Headline framework
 */
static void hladdword(HeadlineParsedText *prs, char *buf, int buflen,
                      int type) {
  while (prs->curwords >= prs->lenwords) {
    prs->lenwords *= 2;
    prs->words = (HeadlineWordEntry *)repalloc(
        (void *)prs->words, prs->lenwords * sizeof(HeadlineWordEntry));
  }
...
static void hlfinditem(HeadlineParsedText *prs, TSQuery query, char *buf,
                       int buflen) {
  int i;
  ...
  for (i = 0; i < query->size; i++) {
    if (item->type == QI_VAL &&
        tsCompareString(GETOPERAND(query) + item->qoperand.distance,
                        item->qoperand.length, buf, buflen,
                        item->qoperand.prefix) == 0) {
      if (word->item) {
...
static void addHLParsedLex(HeadlineParsedText *prs, TSQuery query,
                           ParsedLex *lexs, TSLexeme *norms) {
  ParsedLex *tmplexs;
  TSLexeme *ptr;

  while (lexs) {
    if (lexs->type > 0) hladdword(prs, lexs->lemm, lexs->lenlemm, lexs->type);
...
void hlparsetext(Oid cfgId, HeadlineParsedText *prs, TSQuery query, char *buf,
                 int buflen) {
  int type, lenlemm;
  char *lemm = NULL;
  LexizeData ldata;
