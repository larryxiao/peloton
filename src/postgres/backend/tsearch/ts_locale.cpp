...
int t_isdigit(const char *ptr) {
  ...
  if (clen == 1 || lc_ctype_is_c(collation)) return isdigit(TOUCHAR(ptr));
...
int t_isspace(const char *ptr) {
  ...
  if (clen == 1 || lc_ctype_is_c(collation)) return isspace(TOUCHAR(ptr));
...
int t_isprint(const char *ptr) {
  ...
  if (clen == 1 || lc_ctype_is_c(collation)) return isprint(TOUCHAR(ptr));
...
bool tsearch_readline_begin(tsearch_readline_state *stp, const char *filename) {
  if ((stp->fp = AllocateFile(filename, "r")) == NULL) return false;
...
static void tsearch_readline_callback(void *arg) {
   ...
   */
  if (stp->curline)
    errcontext("line %d of configuration file \"%s\": \"%s\"", stp->lineno,
               stp->filename, stp->curline);
  else
    errcontext("line %d of configuration file \"%s\"", stp->lineno,
               stp->filename);
}
 ...
 */
char *lowerstr(const char *str) { return lowerstr_with_len(str, strlen(str)); }
