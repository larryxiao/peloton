...
ForeignDataWrapper *GetForeignDataWrapperByName(const char *fdwname,
  ...
  if (!OidIsValid(fdwId)) return NULL;
...
ForeignServer *GetForeignServer(Oid serverid) {
  ...
  /* Extract server type */
  datum = SysCacheGetAttr(FOREIGNSERVEROID, tp, Anum_pg_foreign_server_srvtype,
                          &isnull);
...
ForeignServer *GetForeignServerByName(const char *srvname, bool missing_ok) {
  ...
  if (!OidIsValid(serverid)) return NULL;
...
UserMapping *GetUserMapping(Oid userid, Oid serverid) {
  ...
  if (!HeapTupleIsValid(tp)) {
    ...
    tp = SearchSysCache2(USERMAPPINGUSERSERVER, ObjectIdGetDatum(InvalidOid),
                         ObjectIdGetDatum(serverid));
  }
...
ForeignTable *GetForeignTable(Oid relid) {
  ...
  datum = SysCacheGetAttr(FOREIGNTABLEREL, tp, Anum_pg_foreign_table_ftoptions,
                          &isnull);
  if (isnull)
    ft->options = NIL;
  else
...
FdwRoutine *GetFdwRoutine(Oid fdwhandler) {
  ...
  if (routine == NULL || !IsA(routine, FdwRoutine))
    elog(ERROR,
         "foreign-data wrapper handler function %u did not return an "
         "FdwRoutine struct",
         fdwhandler);
  ...
  switch (stmt->list_type) {
    ...
    case FDW_IMPORT_SCHEMA_LIMIT_TO:
      foreach (lc, stmt->table_list) {
        ...
        if (strcmp(tablename, rv->relname) == 0) return true;
      }
      return false;
    ...
    case FDW_IMPORT_SCHEMA_EXCEPT:
      foreach (lc, stmt->table_list) {
        ...
        if (strcmp(tablename, rv->relname) == 0) return false;
      }
      return true;
  }
