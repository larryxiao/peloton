...
static void set_errdata_field(MemoryContextData *cxt, char **ptr,
                              const char *str);
...
thread_local static int errordata_stack_depth =
    -1; /* index of topmost active frame */
...
bool errstart(int elevel, const char *filename, int lineno,
  ...
  if (elevel >= ERROR) {
     ...
     */
    if (CritSectionCount > 0) elevel = PANIC;
  ...
  if (elevel < ERROR && !output_to_server && !output_to_client) return false;
  ...
  if (ErrorContext == NULL) {
    ...
    write_stderr(
        "error occurred at %s:%d before error message processing is "
        "available\n",
...
void errfinish(int dummy, ...) {
   ...
   */
  for (econtext = error_context_stack; econtext != NULL;
   ...
   */
  if (elevel >= FATAL && whereToSendOutput == DestRemote) pq_endcopyout(true);
...
#define EVALUATE_MESSAGE(domain, targetfield, appendval, translateit) \
  {                                                                   \
    ...
    if (edata->targetfield) pfree(edata->targetfield);                \
 ...
 * with support for pluralization of the message text
 */
int errdetail_log_plural(const char *fmt_singular, const char *fmt_plural,
                         unsigned long n, ...) {
  ErrorData *edata = &errordata[errordata_stack_depth];
  MemoryContext oldcontext;
...
char *format_elog_string(const char *fmt, ...) {
  ...
  /* the default text domain is the backend's */
  edata->domain =
      save_format_domain ? save_format_domain : PG_TEXTDOMAIN("postgres");
...
void EmitErrorReport(void) {
   ...
   * mechanisms.
   */
  if (edata->output_to_server && emit_log_hook) (*emit_log_hook)(edata);

  /* Send to server log, if enabled */
  if (edata->output_to_server) send_message_to_server_log(edata);

  /* Send to client, if enabled */
  if (edata->output_to_client) send_message_to_frontend(edata);
...
void ThrowErrorData(ErrorData *edata) {
  ...
  if (edata->sqlerrcode > 0) newedata->sqlerrcode = edata->sqlerrcode;
  if (edata->message) newedata->message = pstrdup(edata->message);
  if (edata->detail) newedata->detail = pstrdup(edata->detail);
  if (edata->detail_log) newedata->detail_log = pstrdup(edata->detail_log);
  if (edata->hint) newedata->hint = pstrdup(edata->hint);
  if (edata->context) newedata->context = pstrdup(edata->context);
  if (edata->schema_name) newedata->schema_name = pstrdup(edata->schema_name);
  if (edata->table_name) newedata->table_name = pstrdup(edata->table_name);
  if (edata->column_name) newedata->column_name = pstrdup(edata->column_name);
...
void set_syslog_parameters(const char *ident, int facility) {
  ...
  if (syslog_ident == NULL || strcmp(syslog_ident, ident) != 0 ||
    ...
    if (syslog_ident) free(syslog_ident);
...
static void write_syslog(int level, const char *line) {
  ...
  if (!openlog_done) {
    openlog(syslog_ident ? syslog_ident : "postgres",
            LOG_PID | LOG_NDELAY | LOG_NOWAIT, syslog_facility);
    openlog_done = true;
  }
...
static int GetACPEncoding(void) {
  ...
  if (encoding == -2) encoding = pg_codepage_to_encoding(GetACP());
...
static void write_eventlog(int level, const char *line, int len) {
  ...
  if (evtHandle == INVALID_HANDLE_VALUE) {
    evtHandle = RegisterEventSource(
        NULL, event_source ? event_source : DEFAULT_EVENT_SOURCE);
    if (evtHandle == NULL) {
...
static void write_console(const char *line, int len) {
   ...
   */
  if (!in_error_recursion_trouble() && !redirection_done &&
...
static const char *process_log_prefix_padding(const char *p, int *ppadding) {
  ...
  while (*p >= '0' && *p <= '9') padding = padding * 10 + (*p++ - '0');
...
static void log_line_prefix(StringInfo buf, ErrorData *edata) {
  ...
  if (Log_line_prefix == NULL) return; /* in case guc hasn't run yet */
    ...
    switch (*p) {
      case 'a':
        if (MyProcPort) {
          ...
          if (appname == NULL || *appname == '\0') appname = _("[unknown]");
          if (padding != 0)
      ...
      case 'c':
        if (padding != 0) {
          ...
          snprintf(strfbuf, sizeof(strfbuf) - 1, "%lx.%x", (long)(MyStartTime),
                   MyProcPid);
      ...
      case 's':
        if (formatted_start_time[0] == '\0') setup_formatted_start_time();
        if (padding != 0)
        ...
        if (MyProcPort && MyProcPort->remote_host) {
          if (padding != 0) {
            if (MyProcPort->remote_port && MyProcPort->remote_port[0] != '\0') {
              ...
              hostport = psprintf("%s(%s)", MyProcPort->remote_host,
      ...
      case 'v':
        ...
        if (MyProc != NULL && MyProc->backendId != InvalidBackendId) {
          if (padding != 0) {
            ...
            snprintf(strfbuf, sizeof(strfbuf) - 1, "%d/%u", MyProc->backendId,
                     MyProc->lxid);
            appendStringInfo(buf, "%*s", padding, strfbuf);
          } else
...
static inline void appendCSVLiteral(StringInfo buf, const char *data) {
  ...
  while ((c = *p++) != '\0') {
    if (c == '"') appendStringInfoCharMacro(buf, '"');
...
static void write_csvlog(ErrorData *edata) {
   ...
   */
  if (formatted_log_time[0] == '\0') setup_formatted_log_time();

  appendStringInfoString(&buf, formatted_log_time);
  appendStringInfoChar(&buf, ',');

  /* username */
  if (MyProcPort) appendCSVLiteral(&buf, MyProcPort->user_name);
  appendStringInfoChar(&buf, ',');

  /* database name */
  if (MyProcPort) appendCSVLiteral(&buf, MyProcPort->database_name);
  appendStringInfoChar(&buf, ',');

  /* Process id  */
  if (MyProcPid != 0) appendStringInfo(&buf, "%d", MyProcPid);
  appendStringInfoChar(&buf, ',');
  ...
  /* errcontext */
  if (!edata->hide_ctx) appendCSVLiteral(&buf, edata->context);
  appendStringInfoChar(&buf, ',');
  ...
  if (is_log_level_output(edata->elevel, log_min_error_statement) &&
    ...
    print_stmt = true;
  if (print_stmt) appendCSVLiteral(&buf, debug_query_string);
  appendStringInfoChar(&buf, ',');
  ...
  /* application name */
  if (application_name) appendCSVLiteral(&buf, application_name);
  ...
  for (cp = fmt; *cp; cp++) {
    if (cp[0] == '%' && cp[1] != '\0') {
      ...
      if (*cp == 'm') {
        ...
        for (; *cp2; cp2++) {
          if (*cp2 == '%') appendStringInfoCharMacro(&buf, '%');
...
int trace_recovery(int trace_level) {
  if (trace_level < LOG && trace_level >= trace_recovery_messages) return LOG;
