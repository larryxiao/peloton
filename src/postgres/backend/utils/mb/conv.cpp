 ...
 */
void latin2mic(const unsigned char *l, unsigned char *p, int len, int lc,
               int encoding) {
  int c1;

  while (len > 0) {
    c1 = *l;
    if (c1 == 0) report_invalid_encoding(encoding, (const char *)l, len);
    if (IS_HIGHBIT_SET(c1)) *p++ = lc;
    *p++ = c1;
    l++;
    len--;
  }
 ...
 * encoding is the PG identifier for the local encoding
 */
void mic2latin(const unsigned char *mic, unsigned char *p, int len, int lc,
               int encoding) {
  int c1;
 ...
 */
void latin2mic_with_table(const unsigned char *l, unsigned char *p, int len,
      ...
      *p++ = c1;
      ...
      if (c2) {
        ...
        *p++ = c2;
        ...
        report_untranslatable_char(encoding, PG_MULE_INTERNAL, (const char *)l,
                                   len);
    }
    l++;
    len--;
 ...
 */
void mic2latin_with_table(const unsigned char *mic, unsigned char *p, int len,
                          int lc, int encoding, const unsigned char *tab) {
  unsigned char c1, c2;
 ...
 */
static inline unsigned char *store_coded_char(unsigned char *dest,
                                              uint32 code) {
  if (code & 0xff000000) *dest++ = code >> 24;
  if (code & 0x00ff0000) *dest++ = code >> 16;
  if (code & 0x0000ff00) *dest++ = code >> 8;
  if (code & 0x000000ff) *dest++ = code;
  return dest;
}
 ...
 */
void UtfToLocal(const unsigned char *utf, int len, unsigned char *iso,
  ...
  for (; len > 0; len -= l) {
    ...
    if (!pg_utf8_islegal(utf, l)) break;
    ...
    if (cmap && len > l) {
      ...
      if (l > 1) {
        ...
        cp = static_cast<pg_utf_to_local_combined *>(bsearch(
            cutf, cmap, cmapsize, sizeof(pg_utf_to_local_combined), compare3));
    ...
    p = static_cast<pg_utf_to_local *>(
        bsearch(&iutf, map, mapsize, sizeof(pg_utf_to_local), compare1));
  ...
  if (len > 0) report_invalid_encoding(PG_UTF8, (const char *)utf, len);
 ...
 * See pg_wchar.h for more details about the data structures used here.
 */
void LocalToUtf(const unsigned char *iso, int len, unsigned char *utf,
  ...
  for (; len > 0; len -= l) {
    ...
    p = static_cast<pg_local_to_utf *>(
        bsearch(&iiso, map, mapsize, sizeof(pg_local_to_utf), compare2));
    ...
    if (cmap) {
      cp = static_cast<pg_local_to_utf_combined *>(bsearch(
          &iiso, cmap, cmapsize, sizeof(pg_local_to_utf_combined), compare4));
  ...
  /* if we broke out of loop early, must be invalid input */
  if (len > 0) report_invalid_encoding(encoding, (const char *)iso, len);
