...
/* Internal functions */
static char *perform_default_encoding_conversion(const char *src, int len,
...
int PrepareClientEncoding(int encoding) {
  ...
  if (!PG_VALID_FE_ENCODING(encoding)) return -1;

  /* Can't do anything during startup, per notes above */
  if (!backend_startup_complete) return 0;
  ...
  if (IsTransactionState()) {
    ...
    to_server_proc =
        FindDefaultConversionProc(encoding, current_server_encoding);
    if (!OidIsValid(to_server_proc)) return -1;
    to_client_proc =
        FindDefaultConversionProc(current_server_encoding, encoding);
    if (!OidIsValid(to_client_proc)) return -1;
 ...
 */
int pg_get_client_encoding(void) { return ClientEncoding->encoding; }

/*
 * returns the current client encoding name
 */
const char *pg_get_client_encoding_name(void) { return ClientEncoding->name; }
...
unsigned char *pg_do_encoding_conversion(unsigned char *src, int len,
  ...
  if (!OidIsValid(proc))
    ereport(ERROR, (errcode(ERRCODE_UNDEFINED_FUNCTION),
                    errmsg(
                        "default conversion function for encoding \"%s\" to "
                        "\"%s\" does not exist",
  ...
  result =
      static_cast<unsigned char *>(palloc(len * MAX_CONVERSION_GROWTH + 1));

  OidFunctionCall5(proc, Int32GetDatum(src_encoding),
                   Int32GetDatum(dest_encoding), CStringGetDatum(src),
                   CStringGetDatum(result), Int32GetDatum(len));
  return result;
}
...
Datum pg_convert_to(PG_FUNCTION_ARGS) {
   ...
   */
  result = DirectFunctionCall3(pg_convert, string, src_encoding_name,
                               dest_encoding_name);
...
Datum pg_convert(PG_FUNCTION_ARGS) {
  ...
  /* perform conversion */
  dest_str = (char *)pg_do_encoding_conversion((unsigned char *)src_str, len,
...
char *pg_any_to_server(const char *s, int len, int encoding) {
  if (len <= 0) return (char *)s; /* empty string is always valid */
  ...
  return (char *)pg_do_encoding_conversion((unsigned char *)s, len, encoding,
...
static char *perform_default_encoding_conversion(const char *src, int len,
  ...
  if (flinfo == NULL) return (char *)src;
  ...
  FunctionCall5(flinfo, Int32GetDatum(src_encoding),
                Int32GetDatum(dest_encoding), CStringGetDatum(src),
                CStringGetDatum(result), Int32GetDatum(len));
  return result;
}
...
int pg_encoding_mb2wchar_with_len(int encoding, const char *from, pg_wchar *to,
                                  int len) {
  return (*pg_wchar_table[encoding].mb2wchar_with_len)(
      (const unsigned char *)from, to, len);
}
...
/* same, with any encoding */
int pg_encoding_wchar2mb_with_len(int encoding, const pg_wchar *from, char *to,
                                  int len) {
  return (*pg_wchar_table[encoding].wchar2mb_with_len)(
      from, (unsigned char *)to, len);
}

/* returns the byte length of a multibyte character */
int pg_mblen(const char *mbstr) {
  return ((*pg_wchar_table[DatabaseEncoding->encoding].mblen)(
      (const unsigned char *)mbstr));
}

/* returns the display length of a multibyte character */
int pg_dsplen(const char *mbstr) {
  return ((*pg_wchar_table[DatabaseEncoding->encoding].dsplen)(
      (const unsigned char *)mbstr));
}
...
int pg_mbstrlen(const char *mbstr) {
  ...
  if (pg_database_encoding_max_length() == 1) return strlen(mbstr);
...
int pg_mbstrlen_with_len(const char *mbstr, int limit) {
  ...
  /* optimization for single byte encoding */
  if (pg_database_encoding_max_length() == 1) return limit;
...
int pg_encoding_mbcliplen(int encoding, const char *mbstr, int len, int limit) {
  ...
  /* optimization for single byte encoding */
  if (pg_encoding_max_length(encoding) == 1) return cliplen(mbstr, len, limit);

  mblen_fn = pg_wchar_table[encoding].mblen;

  while (len > 0 && *mbstr) {
    l = (*mblen_fn)((const unsigned char *)mbstr);
    if ((clen + l) > limit) break;
    clen += l;
    if (clen == limit) break;
    len -= l;
    mbstr += l;
...
int pg_mbcharcliplen(const char *mbstr, int len, int limit) {
  ...
  /* optimization for single byte encoding */
  if (pg_database_encoding_max_length() == 1) return cliplen(mbstr, len, limit);
 ...
 */
static bool raw_pg_bind_textdomain_codeset(const char *domainname,
                                           int encoding) {
  ...
  for (i = 0; pg_enc2gettext_tbl[i].name != NULL; i++) {
    if (pg_enc2gettext_tbl[i].encoding == encoding) {
      if (bind_textdomain_codeset(domainname, pg_enc2gettext_tbl[i].name) !=
          NULL)
        return true;
...
int pg_bind_textdomain_codeset(const char *domainname) {
  ...
  if (new_msgenc < 0) new_msgenc = PG_SQL_ASCII;
 ...
 */
int GetDatabaseEncoding(void) { return DatabaseEncoding->encoding; }

const char *GetDatabaseEncodingName(void) { return DatabaseEncoding->name; }
 ...
 */
int GetMessageEncoding(void) { return MessageEncoding->encoding; }
...
WCHAR *pgwin32_message_to_UTF16(const char *str, int len, int *utf16len) {
  ...
  if (codepage != 0) {
    ...
    utf8 = (char *)pg_do_encoding_conversion((unsigned char *)str, len,
                                             GetMessageEncoding(), PG_UTF8);
    if (utf8 != str) len = strlen(utf8);
