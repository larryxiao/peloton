...
static const struct typinfo TypInfo[] = {
    {"bool", BOOLOID, 0, 1, true, 'c', 'p', InvalidOid, F_BOOLIN, F_BOOLOUT},
    {"bytea", BYTEAOID, 0, -1, false, 'i', 'x', InvalidOid, F_BYTEAIN,
     F_BYTEAOUT},
    ...
    {"name", NAMEOID, CHAROID, NAMEDATALEN, false, 'c', 'p', InvalidOid,
     F_NAMEIN, F_NAMEOUT},
    {"regclass", REGCLASSOID, 0, 4, true, 'i', 'p', InvalidOid, F_REGCLASSIN,
     F_REGCLASSOUT},
    {"regproc", REGPROCOID, 0, 4, true, 'i', 'p', InvalidOid, F_REGPROCIN,
     F_REGPROCOUT},
    {"regtype", REGTYPEOID, 0, 4, true, 'i', 'p', InvalidOid, F_REGTYPEIN,
     F_REGTYPEOUT},
    {"regrole", REGROLEOID, 0, 4, true, 'i', 'p', InvalidOid, F_REGROLEIN,
     F_REGROLEOUT},
    {"regnamespace", REGNAMESPACEOID, 0, 4, true, 'i', 'p', InvalidOid,
     F_REGNAMESPACEIN, F_REGNAMESPACEOUT},
    {"text", TEXTOID, 0, -1, false, 'i', 'x', DEFAULT_COLLATION_OID, F_TEXTIN,
     F_TEXTOUT},
    {"oid", OIDOID, 0, 4, true, 'i', 'p', InvalidOid, F_OIDIN, F_OIDOUT},
    {"tid", TIDOID, 0, 6, false, 's', 'p', InvalidOid, F_TIDIN, F_TIDOUT},
    {"xid", XIDOID, 0, 4, true, 'i', 'p', InvalidOid, F_XIDIN, F_XIDOUT},
    {"cid", CIDOID, 0, 4, true, 'i', 'p', InvalidOid, F_CIDIN, F_CIDOUT},
    {"pg_node_tree", PGNODETREEOID, 0, -1, false, 'i', 'x',
     DEFAULT_COLLATION_OID, F_PG_NODE_TREE_IN, F_PG_NODE_TREE_OUT},
    ...
    {"_text", 1009, TEXTOID, -1, false, 'i', 'x', DEFAULT_COLLATION_OID,
     F_ARRAY_IN, F_ARRAY_OUT},
    {"_oid", 1028, OIDOID, -1, false, 'i', 'x', InvalidOid, F_ARRAY_IN,
     F_ARRAY_OUT},
    {"_char", 1002, CHAROID, -1, false, 'i', 'x', InvalidOid, F_ARRAY_IN,
     F_ARRAY_OUT},
    {"_aclitem", 1034, ACLITEMOID, -1, false, 'i', 'x', InvalidOid, F_ARRAY_IN,
     F_ARRAY_OUT}};
...
void AuxiliaryProcessMain(int argc, char *argv[]) {
  ...
  while ((flag = getopt(argc, argv, "B:c:d:D:Fkr:x:-:")) != -1) {
    switch (flag) {
      ...
      case 'd': {
        ...
        SetConfigOption("log_min_messages", debugstr, PGC_POSTMASTER,
                        PGC_S_ARGV);
        SetConfigOption("client_min_messages", debugstr, PGC_POSTMASTER,
                        PGC_S_ARGV);
        pfree(debugstr);
      } break;
      case 'F':
  ...
  if (!IsUnderPostmaster) ChangeToDataDir();

  /* If standalone, create lockfile for data directory */
  if (!IsUnderPostmaster) CreateDataDirLockFile(false);

  SetProcessingMode(BootstrapProcessing);
  IgnoreSystemIndexes = true;

  /* Initialize MaxBackends (if under postmaster, was done already) */
  if (!IsUnderPostmaster) InitializeMaxBackends();
 ...
 */
static void CheckerModeMain(void) { proc_exit(0); }
...
void boot_openrel(char *relname) {
  ...
  if (Typ == NULL) {
    ...
    i = 0;
    while ((tup = heap_getnext(scan, ForwardScanDirection)) != NULL) ++i;
    heap_endscan(scan);
    app = Typ = ALLOC(struct typmap *, i + 1);
    while (i-- > 0) *app++ = ALLOC(struct typmap, 1);
    *app = NULL;
    scan = heap_beginscan_catalog(rel, 0, NULL);
    app = Typ;
    while ((tup = heap_getnext(scan, ForwardScanDirection)) != NULL) {
      (*app)->am_oid = HeapTupleGetOid(tup);
      memcpy((char *)&(*app)->am_typ, (char *)GETSTRUCT(tup),
             sizeof((*app)->am_typ));
      app++;
    }
    heap_endscan(scan);
  ...
  elog(DEBUG4, "open relation %s, attrsize %d", relname,
       (int)ATTRIBUTE_FIXED_PART_SIZE);

  boot_reldesc = heap_openrv(makeRangeVar(NULL, relname, -1), NoLock);
  numattr = boot_reldesc->rd_rel->relnatts;
  for (i = 0; i < numattr; i++) {
    if (attrtypes[i] == NULL) attrtypes[i] = AllocateAttribute();
    memmove((char *)attrtypes[i], (char *)boot_reldesc->rd_att->attrs[i],
            ATTRIBUTE_FIXED_PART_SIZE);

    {
      Form_pg_attribute at = attrtypes[i];

      elog(DEBUG4, "create attribute %d name %s len %d num %d type %u", i,
           NameStr(at->attname), at->attlen, at->attnum, at->atttypid);
    }
  }
}
  ...
  if (name) {
    if (boot_reldesc) {
      if (strcmp(RelationGetRelationName(boot_reldesc), name) != 0)
        elog(ERROR, "close of %s when %s was expected", name,
...
void DefineAttr(char *name, char *type, int attnum, int nullness) {
  ...
  if (attrtypes[attnum] == NULL) attrtypes[attnum] = AllocateAttribute();
...
#define MARKNOTNULL(att)                                   \
  ((att)->attlen > 0 || (att)->atttypid == OIDVECTOROID || \
   (att)->atttypid == INT2VECTOROID)
    ...
    if (MARKNOTNULL(attrtypes[attnum])) {
      ...
      for (i = 0; i < attnum; i++) {
        if (!attrtypes[i]->attnotnull) break;
      }
      if (i == attnum) attrtypes[attnum]->attnotnull = true;
    }
  }
}
...
void InsertOneTuple(Oid objectid) {
  ...
  tupDesc = CreateTupleDesc(numattr, RelationGetForm(boot_reldesc)->relhasoids,
                            attrtypes);
  tuple = heap_form_tuple(tupDesc, values, Nulls);
  if (objectid != (Oid)0) HeapTupleSetOid(tuple, objectid);
   ...
   */
  for (i = 0; i < numattr; i++) Nulls[i] = false;
}
...
void InsertOneValue(char *value, int i) {
  ...
  boot_get_type_io_data(typoid, &typlen, &typbyval, &typalign, &typdelim,
...
static Oid gettype(char *type) {
  ...
  if (Typ != NULL) {
    ...
    for (i = 0; i < n_types; i++) {
      if (strncmp(type, TypInfo[i].name, NAMEDATALEN) == 0) return i;
    }
...
void boot_get_type_io_data(Oid typid, int16 *typlen, bool *typbyval,
  ...
  if (Typ != NULL) {
    ...
    app = Typ;
    while (*app && (*app)->am_oid != typid) ++app;
    ap = *app;
    if (ap == NULL) elog(ERROR, "type OID %u not found in Typ list", typid);
    ...
    *typoutput = ap->am_typ.typoutput;
    ...
    for (typeindex = 0; typeindex < n_types; typeindex++) {
      if (TypInfo[typeindex].oid == typid) break;
    }
...
static Form_pg_attribute AllocateAttribute(void) {
  Form_pg_attribute attribute =
      (Form_pg_attribute)malloc(ATTRIBUTE_FIXED_PART_SIZE);

  if (!PointerIsValid(attribute)) elog(FATAL, "out of memory");
...
char *MapArrayTypeName(const char *s) {
  ...
  newStr[0] = '_';
  j = 1;
  for (i = 0; i < NAMEDATALEN - 2 && s[i] != '['; i++, j++) newStr[j] = s[i];
...
void index_register(Oid heap, Oid ind, IndexInfo *indexInfo) {
  ...
  if (nogc == NULL)
    nogc = AllocSetContextCreate(
        NULL, "BootstrapNoGC", ALLOCSET_DEFAULT_MINSIZE,
  ...
  newind->il_info->ii_Expressions =
      (List *)copyObject(indexInfo->ii_Expressions);
