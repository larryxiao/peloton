...
static void expandRelation(Oid relid, Alias *eref, int rtindex,
                           int sublevels_up, int location, bool include_dropped,
                           List **colnames, List **colvars);
static void expandTupleDesc(TupleDesc tupdesc, Alias *eref, int count,
                            int offset, int rtindex, int sublevels_up,
 ...
 */
RangeTblEntry *refnameRangeTblEntry(ParseState *pstate, const char *schemaname,
  ...
  if (schemaname != NULL) {
    ...
    if (!OidIsValid(relId)) return NULL;
  }
...
static RangeTblEntry *scanNameSpaceForRefname(ParseState *pstate,
  ...
  foreach (l, pstate->p_namespace) {
    ...
    if (!nsitem->p_rel_visible) continue;
    /* If not inside LATERAL, ignore lateral-only items */
    if (nsitem->p_lateral_only && !pstate->p_lateral_active) continue;

    if (strcmp(rte->eref->aliasname, refname) == 0) {
      if (result)
        ereport(ERROR, (errcode(ERRCODE_AMBIGUOUS_ALIAS),
 ...
 */
CommonTableExpr *scanNameSpaceForCTE(ParseState *pstate, const char *refname,
                                     Index *ctelevelsup) {
  Index levelsup;

  for (levelsup = 0; pstate != NULL;
...
static bool isFutureCTE(ParseState *pstate, const char *refname) {
  for (; pstate != NULL; pstate = pstate->parentParseState) {
    ...
    foreach (lc, pstate->p_future_ctes) {
      ...
      if (strcmp(cte->ctename, refname) == 0) return true;
    }
  }
  return false;
}
...
static RangeTblEntry *searchRangeTableForRel(ParseState *pstate,
  ...
  if (!cte) relId = RangeVarGetRelid(relation, NoLock, true);
  ...
  for (levelsup = 0; pstate != NULL;
    ...
    foreach (l, pstate->p_rtable) {
      ...
      if (rte->rtekind == RTE_RELATION && OidIsValid(relId) &&
          rte->relid == relId)
        return rte;
      if (rte->rtekind == RTE_CTE && cte != NULL &&
        ...
        return rte;
      if (strcmp(rte->eref->aliasname, refname) == 0) return rte;
    }
  }
  return NULL;
}
...
/*
 * Complain if a namescpace___ item is currently disallowed as a LATERAL
 *reference.
    ...
    ereport(ERROR,
            (errcode(ERRCODE_INVALID_COLUMN_REFERENCE),
             ...
             (rte == pstate->p_target_rangetblentry)
                 ? errhint(
                       "There is an entry for table \"%s\", but it cannot be "
                       "referenced from this part of the query.",
                       refname)
                 : errdetail(
                       "The combining JOIN type must be INNER or LEFT for a "
                       "LATERAL reference."),
             parser_errposition(pstate, location)));
  }
}
 ...
 */
int RTERangeTablePosn(ParseState *pstate, RangeTblEntry *rte,
                      int *sublevels_up) {
  int index;
  ListCell *l;
  ...
  while (pstate != NULL) {
    index = 1;
    foreach (l, pstate->p_rtable) {
      if (rte == (RangeTblEntry *)lfirst(l)) return index;
      index++;
    }
    pstate = pstate->parentParseState;
    if (sublevels_up)
 ...
 */
RangeTblEntry *GetRTEByRangeTablePosn(ParseState *pstate, int varno,
                                      int sublevels_up) {
 ...
 * may pass -1 instead.
 */
CommonTableExpr *GetCTEForRTE(ParseState *pstate, RangeTblEntry *rte,
                              int rtelevelsup) {
  Index levelsup;
  ListCell *lc;

  /* Determine RTE's levelsup if caller didn't know it */
  if (rtelevelsup < 0) (void)RTERangeTablePosn(pstate, rte, &rtelevelsup);
  ...
  foreach (lc, pstate->p_ctenamespace) {
    ...
    if (strcmp(cte->ctename, rte->ctename) == 0) return cte;
  }
  /* shouldn't happen */
...
static void updateFuzzyAttrMatchState(int fuzzy_rte_penalty,
  ...
  columndistance = varstr_levenshtein_less_equal(
      actual, strlen(actual), match, matchlen, 1, 1, 1,
   ...
   */
  if (columndistance > matchlen / 2) return;
 ...
 */
Node *scanRTEForColumn(ParseState *pstate, RangeTblEntry *rte, char *colname,
  ...
  foreach (c, rte->eref->colnames) {
    ...
    if (fuzzystate != NULL)
      updateFuzzyAttrMatchState(fuzzy_rte_penalty, fuzzystate, rte, attcolname,
                                colname, attnum);
  }
 ...
 */
Node *colNameToVar(ParseState *pstate, char *colname, bool localonly,
                   int location) {
  Node *result = NULL;
  ...
  while (pstate != NULL) {
    ...
    foreach (l, pstate->p_namespace) {
      ...
      if (!nsitem->p_cols_visible) continue;
      /* If not inside LATERAL, ignore lateral-only items */
      if (nsitem->p_lateral_only && !pstate->p_lateral_active) continue;

      /* use orig_pstate here to get the right sublevels_up */
      newresult =
          scanRTEForColumn(orig_pstate, rte, colname, location, 0, NULL);
...
static FuzzyAttrMatchState *searchRangeTableForCol(ParseState *pstate,
  ...
  ParseState *orig_pstate = pstate;
  FuzzyAttrMatchState *fuzzystate =
      static_cast<FuzzyAttrMatchState *>(palloc(sizeof(FuzzyAttrMatchState)));
  ...
  while (pstate != NULL) {
    ...
    foreach (l, pstate->p_rtable) {
      ...
      if (alias != NULL)
        fuzzy_rte_penalty =
            varstr_levenshtein(alias, strlen(alias), rte->eref->aliasname,
                               strlen(rte->eref->aliasname), 1, 1, 1);
...
static void markRTEForSelectPriv(ParseState *pstate, RangeTblEntry *rte,
                                 int rtindex, AttrNumber col) {
  if (rte == NULL) rte = rt_fetch(rtindex, pstate->p_rtable);
  ...
  if (rte->rtekind == RTE_RELATION) {
    ...
    rte->selectedCols = bms_add_member(
        rte->selectedCols, col - FirstLowInvalidHeapAttributeNumber);
...
void markVarForSelectPriv(ParseState *pstate, Var *var, RangeTblEntry *rte) {
  ...
  for (lv = 0; lv < var->varlevelsup; lv++) pstate = pstate->parentParseState;
...
static void buildRelationAliases(TupleDesc tupdesc, Alias *alias, Alias *eref) {
  ...
  for (varattno = 0; varattno < maxattrs; varattno++) {
    ...
    if (attr->attisdropped) {
      ...
      if (aliaslc) alias->colnames = lappend(alias->colnames, attrname);
      numdropped++;
    } else if (aliaslc) {
 ...
 */
static char *chooseScalarFunctionAlias(Node *funcexpr, char *funcname,
   ...
   */
  if (nfuncs == 1 && alias) return alias->aliasname;
 ...
 */
Relation parserOpenTable(ParseState *pstate, const RangeVar *relation,
                         int lockmode) {
  Relation rel;
      ...
      if (isFutureCTE(pstate, relation->relname))
        ereport(ERROR,
                (errcode(ERRCODE_UNDEFINED_TABLE),
                 ...
                 errdetail(
                     "There is a WITH item named \"%s\", but it cannot be "
                     "referenced from this part of the query.",
                     relation->relname),
                 errhint(
 ...
 */
RangeTblEntry *addRangeTableEntryForRelation(ParseState *pstate, Relation rel,
...
RangeTblEntry *addRangeTableEntryForFunction(ParseState *pstate,
  ...
  forthree(lc1, funcexprs, lc2, funcnames, lc3, coldeflists) {
        ...
        ereport(ERROR,
                (errcode(ERRCODE_SYNTAX_ERROR),
                 errmsg(
                     ...
                     "returning \"record\""),
                 parser_errposition(pstate, exprLocation((Node *)coldeflist))));
    } else {
      if (functypclass == TYPEFUNC_RECORD)
    ...
    if (functypclass == TYPEFUNC_COMPOSITE) {
      ...
      foreach (col, coldeflist) {
        ...
        TupleDescInitEntry(tupdesc, (AttrNumber)i, attrname, attrtype,
                           attrtypmod, 0);
        TupleDescInitEntryCollation(tupdesc, (AttrNumber)i, attrcollation);
        rtfunc->funccolnames =
            lappend(rtfunc->funccolnames, makeString(pstrdup(attrname)));
        rtfunc->funccoltypes = lappend_oid(rtfunc->funccoltypes, attrtype);
        rtfunc->funccoltypmods =
            lappend_int(rtfunc->funccoltypmods, attrtypmod);
        rtfunc->funccolcollations =
            lappend_oid(rtfunc->funccolcollations, attrcollation);
  ...
  if (nfuncs > 1 || rangefunc->ordinality) {
    ...
    if (rangefunc->ordinality)
      TupleDescInitEntry(tupdesc, (AttrNumber)++natts, "ordinality", INT8OID,
                         -1, 0);
 ...
 */
RangeTblEntry *addRangeTableEntryForValues(ParseState *pstate, List *exprs,
                                           List *collations, Alias *alias,
                                           bool lateral, bool inFromCl) {
  RangeTblEntry *rte = makeNode(RangeTblEntry);
  ...
  eref =
      alias ? static_cast<Alias *>(copyObject(alias)) : makeAlias(refname, NIL);
  ...
  if (numcolumns < numaliases)
    ereport(ERROR, (errcode(ERRCODE_INVALID_COLUMN_REFERENCE),
                    errmsg(
                        "VALUES lists \"%s\" have %d columns available but %d "
                        "columns specified",
...
RangeTblEntry *addRangeTableEntryForJoin(ParseState *pstate, List *colnames,
                                         JoinType jointype, List *aliasvars,
  ...
  if (list_length(aliasvars) > MaxAttrNumber)
    ereport(ERROR,
            (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
             errmsg("joins can have at most %d columns", MaxAttrNumber)));
...
RangeTblEntry *addRangeTableEntryForCTE(ParseState *pstate,
  ...
  if (!rte->self_reference) cte->cterefcount++;
...
bool isLockedRefname(ParseState *pstate, const char *refname) {
   ...
   */
  if (pstate->p_locked_from_parent) return true;
  ...
  foreach (l, pstate->p_locking_clause) {
    ...
    if (lc->lockedRels == NIL) {
      ...
      foreach (l2, lc->lockedRels) {
        ...
        if (strcmp(refname, thisrel->relname) == 0) return true;
      }
    }
  }
  return false;
 ...
 */
void addRTEtoQuery(ParseState *pstate, RangeTblEntry *rte, bool addToJoinList,
 ...
 */
void expandRTE(RangeTblEntry *rte, int rtindex, int sublevels_up, int location,
  ...
  switch (rte->rtekind) {
    case RTE_RELATION:
      ...
      expandRelation(rte->relid, rte->eref, rtindex, sublevels_up, location,
      ...
      foreach (tlistitem, rte->subquery->targetList) {
        ...
        if (colvars) {
          ...
          varnode = makeVar(rtindex, varattno, exprType((Node *)te->expr),
                            ...
                            exprCollation((Node *)te->expr), sublevels_up);
    ...
    case RTE_FUNCTION: {
      ...
      foreach (lc, rte->functions) {
        ...
        if (functypclass == TYPEFUNC_COMPOSITE) {
          /* Composite data type, e.g. a table's row type */
          Assert(tupdesc);
          expandTupleDesc(tupdesc, rte->eref, rtfunc->funccolcount, atts_done,
          ...
          if (colvars) {
            ...
            varnode = makeVar(rtindex, atts_done + 1, funcrettype, -1,
                              exprCollation(rtfunc->funcexpr), sublevels_up);
            varnode->location = location;
          ...
          if (colvars) {
            ...
            forthree(l1, rtfunc->funccoltypes, l2, rtfunc->funccoltypmods, l3,
                     rtfunc->funccolcollations) {
              ...
              attnum++;
              varnode = makeVar(rtindex, attnum, attrtype, attrtypmod,
                                attrcollation, sublevels_up);
              varnode->location = location;
              *colvars = lappend(*colvars, varnode);
            }
          }
      ...
      if (rte->funcordinality) {
        ...
        if (colvars) {
          Var *varnode = makeVar(rtindex, atts_done + 1, INT8OID, -1,
    ...
    case RTE_VALUES: {
      ...
      varattno = 0;
      forboth(lcv, (List *)linitial(rte->values_lists), lcc,
              rte->values_collations) {
        ...
        if (colvars) {
          ...
          varnode = makeVar(rtindex, varattno, exprType(col), exprTypmod(col),
                            colcollation, sublevels_up);
          varnode->location = location;
          *colvars = lappend(*colvars, varnode);
        }
      }
    ...
    case RTE_JOIN: {
      ...
      Assert(list_length(rte->eref->colnames) ==
      ...
      forboth(colname, rte->eref->colnames, aliasvar, rte->joinaliasvars) {
        ...
        if (avar == NULL) {
          if (include_dropped) {
            ...
            if (colvars) {
               ...
               */
              *colvars =
                  lappend(*colvars, makeNullConst(INT4OID, -1, InvalidOid));
            }
          }
          continue;
        }
        ...
        if (colvars) {
          ...
          varnode = makeVar(rtindex, varattno, exprType(avar), exprTypmod(avar),
                            exprCollation(avar), sublevels_up);
          varnode->location = location;
    ...
    case RTE_CTE: {
      ...
      varattno = 0;
      forthree(lct, rte->ctecoltypes, lcm, rte->ctecoltypmods, lcc,
        ...
        if (colvars) {
          ...
          varnode = makeVar(rtindex, varattno, coltype, coltypmod, colcoll,
                            sublevels_up);
          *colvars = lappend(*colvars, varnode);
        }
      }
...
static void expandRelation(Oid relid, Alias *eref, int rtindex,
  ...
  expandTupleDesc(rel->rd_att, eref, rel->rd_att->natts, 0, rtindex,
                  sublevels_up, location, include_dropped, colnames, colvars);
...
static void expandTupleDesc(TupleDesc tupdesc, Alias *eref, int count,
  ...
  if (colnames) {
    ...
    for (i = 0; i < offset; i++) {
      if (aliascell) aliascell = lnext(aliascell);
    }
  }
  ...
  for (varattno = 0; varattno < count; varattno++) {
    ...
    if (attr->attisdropped) {
      if (include_dropped) {
        if (colnames) *colnames = lappend(*colnames, makeString(pstrdup("")));
        if (colvars) {
          /*
    ...
    if (colvars) {
      ...
      varnode = makeVar(rtindex, varattno + offset + 1, attr->atttypid,
                        attr->atttypmod, attr->attcollation, sublevels_up);
      varnode->location = location;
 ...
 */
List *expandRelAttrs(ParseState *pstate, RangeTblEntry *rte, int rtindex,
...
char *get_rte_attribute_name(RangeTblEntry *rte, AttrNumber attnum) {
  if (attnum == InvalidAttrNumber) return "*";
  ...
  elog(ERROR, "invalid attnum %d for rangetable entry %s", attnum,
       rte->eref->aliasname);
  return NULL; /* keep compiler quiet */
}
...
bool get_rte_attribute_is_dropped(RangeTblEntry *rte, AttrNumber attnum) {
  ...
  switch (rte->rtekind) {
    ...
    case RTE_FUNCTION: {
      ...
      /* If we get here, must be looking for the ordinality column */
      if (rte->funcordinality && attnum == atts_done + 1) return false;
...
int attnameAttNum(Relation rd, const char *attname, bool sysColOK) {
  ...
  if (sysColOK) {
    if ((i = specialAttNum(attname)) != InvalidAttrNumber) {
      if (i != ObjectIdAttributeNumber || rd->rd_rel->relhasoids) return i;
    }
  }
...
void errorMissingRTE(ParseState *pstate, RangeVar *relation) {
  ...
  if (rte && rte->alias &&
      ...
      refnameRangeTblEntry(pstate, NULL, rte->eref->aliasname,
                           relation->location, &sublevels_up) == rte)
 ...
 * produce a helpful message.
 */
void errorMissingColumn(ParseState *pstate, char *relname, char *colname,
                        int location) {
  ...
  if (state->rfirst && AttributeNumberIsValid(state->first))
    closestfirst =
        strVal(list_nth(state->rfirst->eref->colnames, state->first - 1));
  ...
  if (!state->rsecond) {
    ...
    ereport(
        ERROR,
        (errcode(ERRCODE_UNDEFINED_COLUMN),
         relname ? errmsg("column %s.%s does not exist", relname, colname)
         ...
         state->rfirst
             ? closestfirst
                   ...
                   : errhint(
                         "There is a column named \"%s\" in table \"%s\", but "
    ...
    closestsecond =
        strVal(list_nth(state->rsecond->eref->colnames, state->second - 1));
    ...
    ereport(ERROR,
            (errcode(ERRCODE_UNDEFINED_COLUMN),
             relname ? errmsg("column %s.%s does not exist", relname, colname)
                     : errmsg("column \"%s\" does not exist", colname),
             errhint(
                 "Perhaps you meant to reference the column \"%s\".\"%s\" or "
                 "the column \"%s\".\"%s\".",
                 state->rfirst->eref->aliasname, closestfirst,
  ...
  if (IsA(node, Query)) {
    ...
    foreach (rtable, query->rtable) {
      ...
      if (rte->rtekind == RTE_RELATION) {
        ...
        if (relpersistence == RELPERSISTENCE_TEMP) return true;
      }
    }

    return query_tree_walker(query, isQueryUsingTempRelation_walker, context,
