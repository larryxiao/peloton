 ...
 */
void parse_fixed_parameters(ParseState *pstate, Oid *paramTypes,
                            int numParams) {
  FixedParamState *parstate =
      static_cast<FixedParamState *>(palloc(sizeof(FixedParamState)));
 ...
 */
void parse_variable_parameters(ParseState *pstate, Oid **paramTypes,
                               int *numParams) {
  VarParamState *parstate =
      static_cast<VarParamState *>(palloc(sizeof(VarParamState)));
...
static Node *variable_paramref_hook(ParseState *pstate, ParamRef *pref) {
  ...
  if (paramno > *parstate->numParams) {
    ...
    if (*parstate->paramTypes)
      *parstate->paramTypes =
          (Oid *)repalloc(*parstate->paramTypes, paramno * sizeof(Oid));
    else
      *parstate->paramTypes = (Oid *)palloc(paramno * sizeof(Oid));
    /* Zero out the previously-unreferenced slots */
    MemSet(*parstate->paramTypes + *parstate->numParams, 0,
  ...
  if (*pptype == InvalidOid) *pptype = UNKNOWNOID;
...
void check_variable_parameters(ParseState *pstate, Query *query) {
  ...
  if (*parstate->numParams > 0)
    (void)query_tree_walker(query, reinterpret_cast<query_tree_walker_fptr>(
...
static bool query_contains_extern_params_walker(Node *node, void *context) {
  ...
  if (IsA(node, Query)) {
    /* Recurse into RTE subquery or not-yet-planned sublink subquery */
    return query_tree_walker((Query *)node, query_contains_extern_params_walker,
                             context, 0);
  }
