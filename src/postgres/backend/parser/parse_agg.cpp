 ...
 * pstate level.
 */
void transformAggregateCall(ParseState *pstate, Aggref *agg, List *args,
  ...
  if (AGGKIND_IS_ORDERED_SET(agg->aggkind)) {
    ...
    torder = transformSortClause(pstate, aggorder, &tlist, EXPR_KIND_ORDER_BY,
                                 true /* fix unknowns */,
        ...
        if (!OidIsValid(sortcl->sortop)) {
          ...
          ereport(
              ...
              (errcode(ERRCODE_UNDEFINED_FUNCTION),
               ...
               errdetail(
                   "Aggregates with DISTINCT must be able to sort their "
                   "inputs."),
...
static void check_agglevels_and_constraints(ParseState *pstate, Node *expr) {
  ...
  *p_levelsup = min_varlevel;
  ...
  while (min_varlevel-- > 0) pstate = pstate->parentParseState;
  ...
  switch (pstate->p_expr_kind) {
    ...
    case EXPR_KIND_FROM_SUBSELECT:
      ...
      if (isAgg)
        err =
            _("aggregate functions are not allowed in FROM clause of their own "
              "query level");
      else
        err =
            _("grouping operations are not allowed in FROM clause of their own "
              "query level");
    ...
    case EXPR_KIND_TRIGGER_WHEN:
      if (isAgg)
        err =
            _("aggregate functions are not allowed in trigger WHEN conditions");
      else
        err =
            _("grouping operations are not allowed in trigger WHEN conditions");
  ...
  if (errkind) {
    if (isAgg) /* translator: %s is name of a SQL construct, eg GROUP BY */
...
static int check_agg_arguments(ParseState *pstate, List *directargs, List *args,
  ...
  if (agglevel == context.min_agglevel) {
    ...
    if (aggloc < 0) aggloc = locate_agg_of_level((Node *)filter, agglevel);
  ...
  if (directargs) {
    ...
    if (context.min_varlevel >= 0 && context.min_varlevel < agglevel)
      ereport(ERROR, (errcode(ERRCODE_GROUPING_ERROR),
                      errmsg(
                          "outer-level aggregate cannot contain a lower-level "
                          "variable in its direct arguments"),
                      parser_errposition(
...
void transformWindowFuncCall(ParseState *pstate, WindowFunc *wfunc,
  ...
  if (pstate->p_hasWindowFuncs && contain_windowfuncs((Node *)wfunc->args))
    ereport(ERROR, (errcode(ERRCODE_WINDOWING_ERROR),
                    ...
                    parser_errposition(
                        pstate, locate_windowfunc((Node *)wfunc->args))));
  ...
  if (err)
    ereport(ERROR,
            (errcode(ERRCODE_WINDOWING_ERROR), errmsg_internal("%s", err),
  ...
  if (windef->name) {
    ...
    Assert(windef->refname == NULL && windef->partitionClause == NIL &&
...
void parseCheckAggregates(ParseState *pstate, Query *qry) {
  ...
  if (qry->groupingSets) {
    ...
    if (gset_common) {
      for_each_cell(l, lnext(list_head(gsets))) {
        gset_common = list_intersection_int(
            gset_common, static_cast<const List *>(lfirst(l)));
        if (!gset_common) break;
      }
    }
     ...
     * a normal GROUP BY.
     */
    if (list_length(gsets) == 1 && qry->groupClause) qry->groupingSets = NIL;
  }
  ...
  foreach (l, qry->groupClause) {
    ...
    if (expr == NULL) continue; /* probably cannot happen */
  ...
  if (hasJoinRTEs) clause = flatten_join_alias_vars(root, clause);
  ...
  if (pstate->p_hasAggs && hasSelfRefRTEs)
    ereport(ERROR,
            (errcode(ERRCODE_INVALID_RECURSION),
             errmsg(
                 "aggregate functions are not allowed in a recursive query's "
                 "recursive term"),
...
static bool check_ungrouped_columns_walker(
  ...
  if (IsA(node, Aggref)) {
     ...
     * levels, however.
     */
    if ((int)agg->agglevelsup > context->sublevels_up) return false;
  }
  ...
  if (IsA(node, GroupingFunc)) {
    ...
    if ((int)grp->agglevelsup >= context->sublevels_up) return false;
  }
  ...
  if (IsA(node, Var)) {
    ...
    if (context->sublevels_up == 0)
      ereport(ERROR, (errcode(ERRCODE_GROUPING_ERROR),
                      errmsg(
                          "column \"%s.%s\" must appear in the GROUP BY clause "
                          "or be used in an aggregate function",
                          rte->eref->aliasname, attname),
                      context->in_agg_direct_args
                          ? errdetail(
                                "Direct arguments of an ordered-set aggregate "
  ...
  if (IsA(node, GroupingFunc)) {
    ...
    if ((int)grp->agglevelsup == context->sublevels_up) {
      ...
      foreach (lc, grp->args) {
        ...
        if (context->root) expr = flatten_join_alias_vars(context->root, expr);
        ...
        if (ref == 0)
          ereport(ERROR,
                  (errcode(ERRCODE_GROUPING_ERROR),
                   errmsg(
                       "arguments to GROUPING must be grouping expressions of "
                       "the associated query level"),
  ...
  switch (gs->kind) {
    ...
    case GROUPING_SET_ROLLUP: {
      ...
      while (curgroup_size > 0) {
        ...
        foreach (lc, rollup_val) {
          ...
          current_result =
              list_concat(current_result, list_copy(gs_current->content));
    ...
    case GROUPING_SET_SETS: {
      ...
      foreach (lc, gs->content) {
        List *current_result =
            expand_groupingset_node(static_cast<GroupingSet *>(lfirst(lc)));
...
List *expand_grouping_sets(List *groupingSets, int limit) {
  ...
  if (groupingSets == NIL) return NIL;
  ...
  foreach (lc, groupingSets) {
    ...
    if (limit >= 0 && numsets > limit) return NIL;
  ...
  for_each_cell(lc, lnext(list_head(expanded_groups))) {
    ...
    foreach (lc2, result) {
      ...
      foreach (lc3, p) {
        new_result =
            lappend(new_result, list_union_int(q, (List *)lfirst(lc3)));
      }
    }
    result = new_result;
  }
  ...
  if (list_length(result) > 1) {
    ...
    foreach (lc, result) { *ptr++ = static_cast<List *>(lfirst(lc)); }
    ...
    while (result_len-- > 0) result = lappend(result, *ptr++);
...
Oid resolve_aggregate_transtype(Oid aggfuncid, Oid aggtranstype,
  ...
  if (IsPolymorphicType(aggtranstype)) {
    ...
    aggtranstype = enforce_generic_type_consistency(
        inputTypes, declaredArgTypes, agg_nargs, aggtranstype, false);
 ...
 */
void build_aggregate_fnexprs(Oid *agg_input_types, int agg_num_inputs,
                             int agg_num_direct_inputs, int num_finalfn_inputs,
                             bool agg_variadic, Oid agg_state_type,
                             Oid agg_result_type, Oid agg_input_collation,
                             Oid transfn_oid, Oid invtransfn_oid,
                             Oid finalfn_oid, Expr **transfnexpr,
                             Expr **invtransfnexpr, Expr **finalfnexpr) {
  Param *argp;
  List *args;
  FuncExpr *fexpr;
  int i;
  ...
  fexpr = makeFuncExpr(transfn_oid, agg_state_type, args, InvalidOid,
                       agg_input_collation, COERCE_EXPLICIT_CALL);
  ...
  *transfnexpr = (Expr *)fexpr;
  ...
  if (invtransfnexpr != NULL) {
    if (OidIsValid(invtransfn_oid)) {
      fexpr = makeFuncExpr(invtransfn_oid, agg_state_type, args, InvalidOid,
                           agg_input_collation, COERCE_EXPLICIT_CALL);
      fexpr->funcvariadic = agg_variadic;
  ...
  *finalfnexpr =
      (Expr *)makeFuncExpr(finalfn_oid, agg_result_type, args, InvalidOid,
                           agg_input_collation, COERCE_EXPLICIT_CALL);
