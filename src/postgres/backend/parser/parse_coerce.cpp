...
static Node *coerce_type_typmod(Node *node, Oid targetTypeId,
                                int32 targetTypMod, CoercionForm cformat,
                                int location, bool isExplicit,
...
Node *coerce_to_target_type(ParseState *pstate, Node *expr, Oid exprtype,
  ...
  result = coerce_type(pstate, expr, exprtype, targettype, targettypmod,
   ...
   */
  result = coerce_type_typmod(result, targettype, targettypmod, cformat,
...
Node *coerce_type(ParseState *pstate, Node *node, Oid inputTypeId,
  ...
  if (targetTypeId == ANYARRAYOID || targetTypeId == ANYENUMOID ||
    ...
    if (inputTypeId != UNKNOWNOID) {
      ...
      if (baseTypeId != inputTypeId) {
        RelabelType *r =
            makeRelabelType((Expr *)node, baseTypeId, -1, InvalidOid, cformat);
  ...
  if (inputTypeId == UNKNOWNOID && IsA(node, Const)) {
    ...
    if (!con->constisnull)
      newcon->constvalue = stringTypeDatum(
          baseType, DatumGetCString(con->constvalue), inputTypeMod);
    else
  ...
  if (IsA(node, Param) && pstate != NULL &&
     ...
     */
    result = (*pstate->p_coerce_param_hook)(pstate, (Param *)node, targetTypeId,
                                            targetTypeMod, location);
  ...
  if (IsA(node, CollateExpr)) {
    ...
    newcoll->arg = (Expr *)coerce_type(pstate, (Node *)coll->arg, inputTypeId,
                                       targetTypeId, targetTypeMod, ccontext,
                                       cformat, location);
    newcoll->collOid = coll->collOid;
    newcoll->location = coll->location;
    return (Node *)newcoll;
  ...
  pathtype =
      find_coercion_pathway(targetTypeId, inputTypeId, ccontext, &funcId);
  if (pathtype != COERCION_PATH_NONE) {
    if (pathtype != COERCION_PATH_RELABELTYPE) {
      ...
      result = build_coercion_expression(node, pathtype, funcId, baseTypeId,
                                         baseTypeMod, cformat, location,
      ...
      if (result == node) {
         ...
         */
        RelabelType *r = makeRelabelType((Expr *)result, targetTypeId, -1,
                                         InvalidOid, cformat);
  ...
  if (inputTypeId == RECORDOID && ISCOMPLEX(targetTypeId)) {
    ...
    return coerce_record_to_complex(pstate, node, targetTypeId, ccontext,
                                    cformat, location);
  }
...
bool can_coerce_type(int nargs, Oid *input_typeids, Oid *target_typeids,
  ...
  for (i = 0; i < nargs; i++) {
    ...
    pathtype =
        find_coercion_pathway(targetTypeId, inputTypeId, ccontext, &funcId);
    if (pathtype != COERCION_PATH_NONE) continue;

    /*
     * If input is RECORD and target is a composite type, assume we can
     * coerce (may need tighter checking here)
     */
    if (inputTypeId == RECORDOID && ISCOMPLEX(targetTypeId)) continue;

    /*
     * If input is a composite type and target is RECORD, accept
     */
    if (targetTypeId == RECORDOID && ISCOMPLEX(inputTypeId)) continue;
...
Node *coerce_to_domain(Node *arg, Oid baseTypeId, int32 baseTypeMod, Oid typeId,
  ...
  if (baseTypeId == typeId) return arg;

  /* Suppress display of nested coercion steps */
  if (hideInputCoercion) hide_coercion_node(arg);
...
static Node *coerce_type_typmod(Node *node, Oid targetTypeId,
   ...
   */
  if (targetTypMod < 0 || targetTypMod == exprTypmod(node)) return node;

  pathtype = find_typmod_coercion_function(targetTypeId, &funcId);

  if (pathtype != COERCION_PATH_NONE) {
    /* Suppress display of nested coercion steps */
    if (hideInputCoercion) hide_coercion_node(node);

    node =
        build_coercion_expression(node, pathtype, funcId, targetTypeId,
                                  targetTypMod, cformat, location, isExplicit);
  }
...
static Node *build_coercion_expression(Node *node, CoercionPathType pathtype,
  ...
  if (pathtype == COERCION_PATH_FUNC) {
    ...
    if (nargs >= 2) {
      ...
      cons = makeConst(INT4OID, -1, InvalidOid, sizeof(int32),
    ...
    if (nargs == 3) {
      ...
      cons = makeConst(BOOLOID, -1, InvalidOid, sizeof(bool),
                       BoolGetDatum(isExplicit), false, true);

      args = lappend(args, cons);
    }

    fexpr = makeFuncExpr(funcId, targetTypeId, args, InvalidOid, InvalidOid,
                         cformat);
...
static Node *coerce_record_to_complex(ParseState *pstate, Node *node,
  ...
  if (node && IsA(node, RowExpr)) {
    ...
    ereport(ERROR,
            (errcode(ERRCODE_CANNOT_COERCE),
             errmsg("cannot cast type %s to %s", format_type_be(RECORDOID),
  ...
  for (i = 0; i < tupdesc->natts; i++) {
    ...
    cexpr = coerce_to_target_type(
        pstate, expr, exprtype, tupdesc->attrs[i]->atttypid,
        tupdesc->attrs[i]->atttypmod, ccontext, COERCE_IMPLICIT_CAST, -1);
    if (cexpr == NULL)
      ereport(ERROR,
              (errcode(ERRCODE_CANNOT_COERCE),
               errmsg("cannot cast type %s to %s", format_type_be(RECORDOID),
               ...
               errdetail("Cannot cast type %s to %s in column %d.",
                         ...
                         format_type_be(tupdesc->attrs[i]->atttypid), ucolno),
 ...
 */
Node *coerce_to_boolean(ParseState *pstate, Node *node,
  ...
  if (inputTypeId != BOOLOID) {
    ...
    newnode =
        coerce_to_target_type(pstate, node, inputTypeId, BOOLOID, -1,
 ...
 * processing is wanted.
 */
Node *coerce_to_specific_type(ParseState *pstate, Node *node, Oid targetTypeId,
                              const char *constructName) {
  Oid inputTypeId = exprType(node);
  ...
  if (inputTypeId != targetTypeId) {
    ...
    newnode =
        coerce_to_target_type(pstate, node, inputTypeId, targetTypeId, -1,
                              COERCION_ASSIGNMENT, COERCE_IMPLICIT_CAST, -1);
    if (newnode == NULL)
      ereport(
          ERROR,
          (errcode(ERRCODE_DATATYPE_MISMATCH),
           ...
           errmsg("argument of %s must be type %s, not type %s", constructName,
                  format_type_be(targetTypeId), format_type_be(inputTypeId)),
           parser_errposition(pstate, exprLocation(node))));
    node = newnode;
  }
 ...
 */
int parser_coercion_errposition(ParseState *pstate, int coerce_location,
                                Node *input_expr) {
...
Oid select_common_type(ParseState *pstate, List *exprs, const char *context,
      ...
      if (ptype == UNKNOWNOID) {
        ...
        ereport(ERROR,
                (errcode(ERRCODE_DATATYPE_MISMATCH),
                 ...
                 errmsg("%s types %s and %s cannot be matched", context,
   ...
   * now.
   */
  if (ptype == UNKNOWNOID) ptype = TEXTOID;
 ...
 * processing is wanted.
 */
Node *coerce_to_common_type(ParseState *pstate, Node *node, Oid targetTypeId,
                            const char *context) {
  Oid inputTypeId = exprType(node);

  if (inputTypeId == targetTypeId) return node; /* no work */
  if (can_coerce_type(1, &inputTypeId, &targetTypeId, COERCION_IMPLICIT))
    ...
    ereport(ERROR,
            (errcode(ERRCODE_CANNOT_COERCE),
             /* translator: first %s is name of a SQL construct, eg CASE */
             errmsg("%s could not convert type %s to %s", context,
                    format_type_be(inputTypeId), format_type_be(targetTypeId)),
             parser_errposition(pstate, exprLocation(node))));
  return node;
}
...
bool check_generic_type_consistency(Oid *actual_arg_types,
  ...
  for (j = 0; j < nargs; j++) {
    ...
    if (decl_type == ANYELEMENTOID || decl_type == ANYNONARRAYOID ||
      ...
      if (OidIsValid(elem_typeid) && actual_type != elem_typeid) return false;
      elem_typeid = actual_type;
    } else if (decl_type == ANYARRAYOID) {
      if (actual_type == UNKNOWNOID) continue;
      actual_type = getBaseType(actual_type); /* flatten domains */
      if (OidIsValid(array_typeid) && actual_type != array_typeid) return false;
      array_typeid = actual_type;
    } else if (decl_type == ANYRANGEOID) {
      if (actual_type == UNKNOWNOID) continue;
      actual_type = getBaseType(actual_type); /* flatten domains */
      if (OidIsValid(range_typeid) && actual_type != range_typeid) return false;
  ...
  if (OidIsValid(array_typeid)) {
    if (array_typeid == ANYARRAYOID) {
      ...
      if (have_anyelement) return false;
      return true;
    }
  ...
  if (have_anynonarray) {
    ...
    if (type_is_array_domain(elem_typeid)) return false;
  }

  if (have_anyenum) {
    /* require the element type to be an enum */
    if (!type_is_enum(elem_typeid)) return false;
  }
...
Oid enforce_generic_type_consistency(Oid *actual_arg_types,
  ...
  bool have_anyelement = (rettype == ANYELEMENTOID ||
                          rettype == ANYNONARRAYOID || rettype == ANYENUMOID);
      ...
      if (OidIsValid(elem_typeid) && actual_type != elem_typeid)
        ereport(ERROR,
                (errcode(ERRCODE_DATATYPE_MISMATCH),
                 ...
                 errdetail("%s versus %s", format_type_be(elem_typeid),
      ...
      if (OidIsValid(array_typeid) && actual_type != array_typeid)
        ereport(ERROR,
                (errcode(ERRCODE_DATATYPE_MISMATCH),
                 ...
                 errdetail("%s versus %s", format_type_be(array_typeid),
                           format_type_be(actual_type))));
      array_typeid = actual_type;
    } else if (decl_type == ANYRANGEOID) {
      have_generics = true;
      ...
      if (OidIsValid(range_typeid) && actual_type != range_typeid)
        ereport(ERROR,
                (errcode(ERRCODE_DATATYPE_MISMATCH),
                 ...
                 errdetail("%s versus %s", format_type_be(range_typeid),
                           format_type_be(actual_type))));
      range_typeid = actual_type;
    }
  }
   ...
   */
  if (!have_generics) return rettype;
  ...
  if (OidIsValid(array_typeid)) {
    ...
    if (!OidIsValid(elem_typeid)) {
      ...
      ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH),
                      errmsg(
                          "argument declared \"anyarray\" is not consistent "
                          "with argument declared \"anyelement\""),
                      errdetail("%s versus %s", format_type_be(array_typeid),
  ...
  if (OidIsValid(range_typeid)) {
    if (range_typeid == ANYRANGEOID && !have_anyelement) {
      ...
      if (!OidIsValid(range_typelem))
        ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH),
                        errmsg(
                            "argument declared \"anyrange\" is not a range "
                            "type but type %s",
    ...
    if (!OidIsValid(elem_typeid)) {
      ...
      ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH),
                      errmsg(
                          "argument declared \"anyrange\" is not consistent "
                          "with argument declared \"anyelement\""),
                      errdetail("%s versus %s", format_type_be(range_typeid),
                                format_type_be(elem_typeid))));
    }
  ...
  if (rettype == ANYELEMENTOID || rettype == ANYNONARRAYOID ||
      rettype == ANYENUMOID)
    return elem_typeid;
 ...
 */
Oid resolve_generic_type(Oid declared_type, Oid context_actual_type,
  ...
  if (declared_type == ANYARRAYOID) {
    if (context_declared_type == ANYARRAYOID) {
      ...
      return array_typeid;
             ...
             declared_type == ANYNONARRAYOID || declared_type == ANYENUMOID ||
...
bool IsBinaryCoercible(Oid srctype, Oid targettype) {
  ...
  if (targettype == ANYOID || targettype == ANYELEMENTOID) return true;

  /* If srctype is a domain, reduce to its base type */
  if (OidIsValid(srctype)) srctype = getBaseType(srctype);
  ...
  if (targettype == ANYARRAYOID)
    if (type_is_array(srctype)) return true;
  ...
  if (targettype == ANYENUMOID)
    if (type_is_enum(srctype)) return true;

  /* Also accept any range type as coercible to ANYRANGE */
  if (targettype == ANYRANGEOID)
    if (type_is_range(srctype)) return true;

  /* Also accept any composite type as coercible to RECORD */
  if (targettype == RECORDOID)
    if (ISCOMPLEX(srctype)) return true;

  /* Also accept any composite array type as coercible to RECORD[] */
  if (targettype == RECORDARRAYOID)
    if (is_complex_array(srctype)) return true;

  /* Else look in pg_cast */
  tuple = SearchSysCache2(CASTSOURCETARGET, ObjectIdGetDatum(srctype),
                          ObjectIdGetDatum(targettype));
  if (!HeapTupleIsValid(tuple)) return false; /* no cast */
...
CoercionPathType find_coercion_pathway(Oid targetTypeId, Oid sourceTypeId,
  ...
  *funcid = InvalidOid;
  ...
  if (OidIsValid(sourceTypeId)) sourceTypeId = getBaseType(sourceTypeId);
  if (OidIsValid(targetTypeId)) targetTypeId = getBaseType(targetTypeId);

  /* Domains are always coercible to and from their base type */
  if (sourceTypeId == targetTypeId) return COERCION_PATH_RELABELTYPE;
  ...
  if (HeapTupleIsValid(tuple)) {
    ...
    if (targetTypeId != OIDVECTOROID && targetTypeId != INT2VECTOROID) {
      ...
      if ((targetElem = get_element_type(targetTypeId)) != InvalidOid &&
        ...
        elempathtype = find_coercion_pathway(targetElem, sourceElem, ccontext,
                                             &elemfuncid);
...
CoercionPathType find_typmod_coercion_function(Oid typeId, Oid *funcid) {
  ...
  *funcid = InvalidOid;
  ...
  /* Look in pg_cast */
  tuple = SearchSysCache2(CASTSOURCETARGET, ObjectIdGetDatum(typeId),
  ...
  if (!OidIsValid(*funcid)) result = COERCION_PATH_NONE;
...
static bool typeIsOfTypedTable(Oid reltypeId, Oid reloftypeId) {
  ...
  if (relid) {
    ...
    if (reltup->reloftype == reloftypeId) result = true;
