...
static OnConflictExpr *transformOnConflictClause(
 ...
 */
Query *parse_analyze(Node *parseTree, const char *sourceText, Oid *paramTypes,
                     int numParams) {
...
static Query *transformInsertStmt(ParseState *pstate, InsertStmt *stmt) {
  ...
  if (isOnConflictUpdate) targetPerms |= ACL_UPDATE;
  qry->resultRelation =
  ...
  if (selectStmt == NULL) {
     ...
     */
    rte = addRangeTableEntryForValues(pstate, exprsLists, collations, NULL,
                                      lateral, true);
    rtr = makeNode(RangeTblRef);
    /* assume new___ rte is at end */
    rtr->rtindex = list_length(pstate->p_rtable);
  ...
  foreach (lc, exprList) {
    ...
    rte->insertedCols = bms_add_member(
        rte->insertedCols, attr_num - FirstLowInvalidHeapAttributeNumber);
...
static List *transformInsertRow(ParseState *pstate, List *exprlist,
  ...
  if (list_length(exprlist) > list_length(icolumns))
    ereport(ERROR, (errcode(ERRCODE_SYNTAX_ERROR),
                    ...
                    parser_errposition(
                        pstate, exprLocation(static_cast<const Node *>(list_nth(
                                    exprlist, list_length(icolumns)))))));
            ...
            (errcode(ERRCODE_SYNTAX_ERROR),
             ...
             ((list_length(exprlist) == 1 &&
               count_rowexpr_columns(pstate,
                                     static_cast<Node *>(linitial(exprlist))) ==
                   list_length(icolumns))
                  ? errhint(
                        "The insertion source is a row expression containing "
                        "the same number of columns expected by the INSERT. "
                        "Did you accidentally use extra parentheses?")
                  : 0),
             parser_errposition(
                 pstate, exprLocation(static_cast<const Node *>(
                             list_nth(icolumns, list_length(exprlist)))))));
  }
...
static OnConflictExpr *transformOnConflictClause(
  ...
  if (onConflictClause->action == ONCONFLICT_UPDATE) {
    exclRte =
        addRangeTableEntryForRelation(pstate, pstate->p_target_relation,
     ...
     */
    exclRelTlist = expandRelAttrs(pstate, exclRte, exclRelIndex, 0, -1);
...
static int count_rowexpr_columns(ParseState *pstate, Node *expr) {
  ...
  if (IsA(expr, RowExpr)) return list_length(((RowExpr *)expr)->args);
  if (IsA(expr, Var)) {
    Var *var = (Var *)expr;
    ...
    if (attnum > 0 && var->vartype == RECORDOID) {
      ...
      if (rte->rtekind == RTE_SUBQUERY) {
        ...
        if (ste == NULL || ste->resjunk) return -1;
        expr = (Node *)ste->expr;
        if (IsA(expr, RowExpr)) return list_length(((RowExpr *)expr)->args);
      }
    }
...
static Query *transformSelectStmt(ParseState *pstate, SelectStmt *stmt) {
  ...
  qry->sortClause = transformSortClause(
      pstate, stmt->sortClause, &qry->targetList, EXPR_KIND_ORDER_BY,
      true /* fix unknowns */, false /* allow SQL92 rules */);
  ...
  qry->limitCount =
      transformLimitClause(pstate, stmt->limitCount, EXPR_KIND_LIMIT, "LIMIT");
    ...
    ereport(ERROR,
            (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
             ...
             errmsg("%s cannot be applied to VALUES",
                    LCS_asString(((LockingClause *)linitial(
                                      stmt->lockingClause))->strength))));
...
static Query *transformSetOperationStmt(ParseState *pstate, SelectStmt *stmt) {
    ...
    ereport(ERROR,
            (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
             ...
             errmsg("%s is not allowed with UNION/INTERSECT/EXCEPT",
                    LCS_asString(((LockingClause *)linitial(lockingClause))
                                     ->strength))));
   ...
   */
  sostmt =
      (SetOperationStmt *)transformSetOperationTree(pstate, stmt, true, NULL);
  ...
  forthree(lct, sostmt->colTypes, lcm, sostmt->colTypmods, lcc,
           sostmt->colCollations) {
  ...
  jrte = addRangeTableEntryForJoin(pstate, targetnames, JOIN_INNER, targetvars,
                                   NULL, false);
  ...
  qry->sortClause = transformSortClause(
      pstate, sortClause, &qry->targetList, EXPR_KIND_ORDER_BY,
      false /* no unknowns expected */, false /* allow SQL92 rules */);
  ...
  if (tllen != list_length(qry->targetList))
    ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                    ...
                    errdetail(
                        "Only result column names can be used, not expressions "
                        "or functions."),
                    errhint(
                        "Add the expression/function to every SELECT, or move "
                        "the UNION into a FROM clause."),
                    parser_errposition(
                        pstate, exprLocation(static_cast<const Node *>(
                                    list_nth(qry->targetList, tllen))))));

  qry->limitOffset =
      transformLimitClause(pstate, limitOffset, EXPR_KIND_OFFSET, "OFFSET");
  qry->limitCount =
      transformLimitClause(pstate, limitCount, EXPR_KIND_LIMIT, "LIMIT");
      ...
      if (contain_vars_of_level((Node *)selectQuery, 1))
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_COLUMN_REFERENCE),
                 errmsg(
                     "UNION/INTERSECT/EXCEPT member statement cannot refer to "
    ...
    if (targetlist) {
      *targetlist = NIL;
      foreach (tl, selectQuery->targetList) {
        ...
        if (!tle->resjunk) *targetlist = lappend(*targetlist, tle);
      }
    }
...
static List *transformUpdateTargetList(ParseState *pstate, List *origTlist) {
  ...
  foreach (tl, tlist) {
    ...
    target_rte->updatedCols = bms_add_member(
        target_rte->updatedCols, attrno - FirstLowInvalidHeapAttributeNumber);
  ...
  if (rlist == NIL)
    ereport(ERROR,
            (errcode(ERRCODE_SYNTAX_ERROR),
             ...
             parser_errposition(pstate, exprLocation(static_cast<const Node *>(
                                            linitial(returningList))))));
...
static Query *transformDeclareCursorStmt(ParseState *pstate,
  ...
  if (result->hasModifyingCTE)
    ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                    errmsg(
                        "DECLARE CURSOR must not contain data-modifying "
                        "statements in WITH")));
    ...
    ereport(ERROR,
            (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
             ...
             errmsg("DECLARE CURSOR WITH HOLD ... %s is not supported",
                    LCS_asString(((RowMarkClause *)linitial(result->rowMarks))
                                     ->strength)),
...
static Query *transformCreateTableAsStmt(ParseState *pstate,
  ...
  if (stmt->relkind == OBJECT_MATVIEW) {
    ...
    if (query->hasModifyingCTE)
      ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                      errmsg(
                          "materialized views must not use data-modifying "
                          "statements in WITH")));
    ...
    if (query_contains_extern_params(query))
      ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                      errmsg(
                          "materialized views may not be defined using bound "
                          "parameters")));
    ...
    ereport(ERROR,
            (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
             /*------
               translator: %s is a SQL row locking clause such as FOR UPDATE */
             errmsg(
                 "%s is not allowed with set-returning functions in the target "
                 "list",
                 LCS_asString(strength))));
}
...
void applyLockingClause(Query *qry, Index rtindex, LockClauseStrength strength,
  ...
  if (!pushedDown) qry->hasForUpdate = true;
