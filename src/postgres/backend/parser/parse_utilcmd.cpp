...
List *transformCreateStmt(CreateStmt *stmt, const char *queryString) {
  ...
  namespaceid = RangeVarGetAndCheckCreationNamespace(stmt->relation, NoLock,
                                                     &existing_relid);
  ...
  if (stmt->ofTypename) transformOfType(&cxt, stmt->ofTypename);
...
static void transformColumnDefinition(CreateStmtContext *cxt,
  ...
  if (column->typeName) transformColumnType(cxt, column);
  ...
  if (is_serial) {
    ...
    castnode->location = -1;
    funccallnode =
        makeFuncCall(SystemFuncName("nextval"), list_make1(castnode), -1);
      ...
      case CONSTR_DEFAULT:
        if (saw_default)
          ereport(ERROR,
                  (errcode(ERRCODE_SYNTAX_ERROR),
                   errmsg(
                       "multiple default values specified for column \"%s\" of "
                       "table \"%s\"",
                       column->colname, cxt->relation->relname),
                   parser_errposition(cxt->pstate, constraint->location)));
      ...
      case CONSTR_PRIMARY:
        if (cxt->isforeign)
          ereport(ERROR,
                  (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                   errmsg(
                       "primary key constraints are not supported on foreign "
                       "tables"),
                   parser_errposition(cxt->pstate, constraint->location)));
      /* FALL THRU */
      ...
      case CONSTR_FOREIGN:
        if (cxt->isforeign)
          ereport(ERROR,
                  (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                   errmsg(
                       "foreign key constraints are not supported on foreign "
                       "tables"),
                   parser_errposition(cxt->pstate, constraint->location)));
...
static void transformTableLikeClause(CreateStmtContext *cxt,
  ...
  if (relation->rd_rel->relkind == RELKIND_COMPOSITE_TYPE) {
    aclresult =
        pg_type_aclcheck(relation->rd_rel->reltype, GetUserId(), ACL_USAGE);
    if (aclresult != ACLCHECK_OK)
      aclcheck_error(aclresult, ACL_KIND_TYPE,
                     RelationGetRelationName(relation));
  } else {
    aclresult =
        pg_class_aclcheck(RelationGetRelid(relation), GetUserId(), ACL_SELECT);
    if (aclresult != ACLCHECK_OK)
  ...
  if ((table_like_clause->options & CREATE_TABLE_LIKE_CONSTRAINTS) &&
    ...
    for (ccnum = 0; ccnum < tupleDesc->constr->num_check; ccnum++) {
      ...
      ccbin_node =
          map_variable_attnos(static_cast<Node *>(stringToNode(ccbin)), 1, 0,
                              attmap, tupleDesc->natts, &found_whole_row);
      ...
      if (found_whole_row)
        ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                        ...
                        errdetail(
                            "Constraint \"%s\" contains a whole-row reference "
                            "to table \"%s\".",
                            ccname, RelationGetRelationName(relation))));
      ...
      if ((table_like_clause->options & CREATE_TABLE_LIKE_COMMENTS) &&
          (comment =
               GetComment(get_relation_constraint_oid(
                              RelationGetRelid(relation), n->conname, false),
  ...
  if ((table_like_clause->options & CREATE_TABLE_LIKE_INDEXES) &&
    ...
    foreach (l, parent_indexes) {
      ...
      index_stmt =
          generateClonedIndexStmt(cxt, parent_index, attmap, tupleDesc->natts);
...
static IndexStmt *generateClonedIndexStmt(CreateStmtContext *cxt,
  ...
  datum =
      SysCacheGetAttr(INDEXRELID, ht_idx, Anum_pg_index_indcollation, &isnull);
  Assert(!isnull);
  ...
  if (index->primary || index->unique || idxrec->indisexclusion) {
    ...
    if (OidIsValid(constraintId)) {
      ...
      if (idxrec->indisexclusion) {
        ...
        deconstruct_array(DatumGetArrayTypeP(datum), OIDOID, sizeof(Oid), true,
                          'i', &elems, NULL, &nElems);
  ...
  for (keyno = 0; keyno < idxrec->indnatts; keyno++) {
    ...
    if (AttributeNumberIsValid(attnum)) {
      ...
      indexkey = map_variable_attnos(indexkey, 1, 0, attmap, attmap_length,
                                     &found_whole_row);
  ...
  if (!isnull) index->options = untransformRelOptions(datum);
  ...
  if (!isnull) {
    ...
    /* Adjust Vars to match new___ table's column numbering */
    pred_tree = map_variable_attnos(pred_tree, 1, 0, attmap, attmap_length,
                                    &found_whole_row);
...
static List *get_collation(Oid collation, Oid actual_datatype) {
  ...
  if (!OidIsValid(collation)) return NIL; /* easy case */
  ...
  foreach (lc, indexlist) {
    ...
    foreach (k, cxt->alist) {
      ...
      if (equal(index->indexParams, priorindex->indexParams) &&
         ...
         */
        if (priorindex->idxname == NULL) priorindex->idxname = index->idxname;
        keep = false;
        break;
      }
    }

    if (keep) cxt->alist = lappend(cxt->alist, index);
  }
}
...
static IndexStmt *transformIndexConstraint(Constraint *constraint,
  ...
  index->accessMethod = constraint->access_method
                            ...
                            : static_cast<char *>(DEFAULT_INDEX_TYPE);
  ...
  if (constraint->indexname != NULL) {
    ...
    if (!index_form->indisunique)
      ereport(ERROR, (errcode(ERRCODE_WRONG_OBJECT_TYPE),
                      ...
                      errdetail(
                          "Cannot create a primary key or unique constraint "
                          "using such an index."),
                      parser_errposition(cxt->pstate, constraint->location)));
    ...
    if (!index_form->indimmediate && !constraint->deferrable)
      ereport(ERROR, (errcode(ERRCODE_WRONG_OBJECT_TYPE),
                      ...
                      errdetail(
                          "Cannot create a non-deferrable constraint using a "
                          "deferrable index."),
                      parser_errposition(cxt->pstate, constraint->location)));
    ...
    for (i = 0; i < index_form->indnatts; i++) {
      ...
      if (attnum > 0) {
        ...
        attform =
            SystemAttributeDefinition(attnum, heap_rel->rd_rel->relhasoids);
       ...
       */
      defopclass =
          GetDefaultOpClass(attform->atttypid, index_rel->rd_rel->relam);
      if (indclass->values[i] != defopclass || index_rel->rd_indoption[i] != 0)
        ereport(ERROR,
                (errcode(ERRCODE_WRONG_OBJECT_TYPE),
                 errmsg("index \"%s\" does not have default sorting behavior",
                        index_name),
                 errdetail(
                     "Cannot create a primary key or unique constraint using "
                     "such an index."),
                 parser_errposition(cxt->pstate, constraint->location)));
  ...
  foreach (lc, constraint->keys) {
    ...
    if (found) {
      ...
      if (constraint->contype == CONSTR_PRIMARY) column->is_not_null = TRUE;
 ...
 * relation.
 */
IndexStmt *transformIndexStmt(Oid relid, IndexStmt *stmt,
  ...
  if (stmt->transformed) return stmt;
  ...
  if (stmt->whereClause) {
    stmt->whereClause = transformWhereClause(
        pstate, stmt->whereClause, EXPR_KIND_INDEX_PREDICATE, "WHERE");
  ...
  foreach (l, stmt->indexParams) {
    ...
    if (ielem->expr) {
      ...
      ielem->expr =
          transformExpr(pstate, ielem->expr, EXPR_KIND_INDEX_EXPRESSION);
  ...
  if (list_length(pstate->p_rtable) != 1)
    ereport(ERROR, (errcode(ERRCODE_INVALID_COLUMN_REFERENCE),
                    errmsg(
                        "index expressions and predicates can refer only to "
 ...
 */
void transformRuleStmt(RuleStmt *stmt, const char *queryString, List **actions,
                       Node **whereClause) {
  Relation rel;
  ParseState *pstate;
  ...
  *whereClause = transformWhereClause(
      pstate, (Node *)copyObject(stmt->whereClause), EXPR_KIND_WHERE, "WHERE");
  /* we have to fix its collations too */
  ...
  if (list_length(pstate->p_rtable) != 2) /* naughty, naughty... */
    ereport(ERROR, (errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
                    errmsg(
                        "rule WHERE condition cannot contain references to "
                        "other relations")));
    ...
    *actions = list_make1(nothing_qry);
    ...
    foreach (l, stmt->actions) {
      ...
      if (top_subqry->commandType == CMD_UTILITY && *whereClause != NULL)
        ereport(ERROR, (errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
                        errmsg(
                            "rules with WHERE conditions can only have SELECT, "
       ...
       */
      if (rangeTableEntry_used((Node *)top_subqry->cteList, PRS2_OLD_VARNO,
                               0) ||
          rangeTableEntry_used((Node *)sub_qry->cteList, PRS2_OLD_VARNO, 0))
        ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                        errmsg("cannot refer to OLD within WITH query")));
      if (rangeTableEntry_used((Node *)top_subqry->cteList, PRS2_NEW_VARNO,
                               0) ||
 ...
 * relation.
 */
List *transformAlterTableStmt(Oid relid, AlterTableStmt *stmt,
                              const char *queryString) {
  Relation rel;
  ParseState *pstate;
  CreateStmtContext cxt;
  ...
  foreach (lcmd, stmt->cmds) {
    ...
    switch (cmd->subtype) {
      ...
      case AT_AddColumnToView: {
         ...
         */
        if (def->raw_default != NULL) skipValidation = false;
  ...
  foreach (l, cxt.alist) {
    ...
    newcmd->subtype =
        OidIsValid(idxstmt->indexOid) ? AT_AddIndexConstraint : AT_AddIndex;
...
#define SUPPORTS_ATTRS(node)                                                 \
  ((node) != NULL &&                                                         \
   ((node)->contype == CONSTR_PRIMARY || (node)->contype == CONSTR_UNIQUE || \
