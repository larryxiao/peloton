 ...
 */
static const int oldprecedence_l[] = {0, 10, 10, 3, 2, 8, 4, 5,
                                      6, 4,  5,  6, 7, 8, 9};
static const int oldprecedence_r[] = {0, 10, 10, 3, 2, 8, 4, 5,
                                      6, 1,  1,  1, 7, 8, 9};
...
static Expr *make_distinct_op(ParseState *pstate, List *opname, Node *ltree,
                              Node *rtree, int location);
static int operator_precedence_group(Node *node, const char **nodename);
static void emit_precedence_warnings(ParseState *pstate, int opgroup,
                                     const char *opname, Node *lchild,
                                     Node *rchild, int location);
...
static Node *transformExprRecurse(ParseState *pstate, Node *expr) {
  ...
  switch (nodeTag(expr)) {
    ...
    case T_A_ArrayExpr:
      result = transformArrayExpr(pstate, (A_ArrayExpr *)expr, InvalidOid,
                                  InvalidOid, -1);
      break;
...
static void unknown_attribute(ParseState *pstate, Node *relref, char *attname,
  ...
  if (IsA(relref, Var) && ((Var *)relref)->varattno == InvalidAttrNumber) {
    ...
    rte = GetRTEByRangeTablePosn(pstate, ((Var *)relref)->varno,
                                 ((Var *)relref)->varlevelsup);
    ereport(ERROR, (errcode(ERRCODE_UNDEFINED_COLUMN),
                    errmsg("column %s.%s does not exist", rte->eref->aliasname,
                           attname),
    ...
    if (ISCOMPLEX(relTypeId))
      ereport(ERROR, (errcode(ERRCODE_UNDEFINED_COLUMN),
                      errmsg("column \"%s\" not found in data type %s", attname,
...
static Node *transformIndirection(ParseState *pstate, Node *basenode,
    ...
    if (IsA(n, A_Indices))
      ...
      if (subscripts)
        result = (Node *)transformArraySubscripts(
            pstate, result, exprType(result), InvalidOid, exprTypmod(result),
            subscripts, NULL);
      subscripts = NIL;

      newresult = ParseFuncOrColumn(pstate, list_make1(n), list_make1(result),
                                    NULL, location);
      if (newresult == NULL)
  ...
  if (subscripts)
    result = (Node *)transformArraySubscripts(pstate, result, exprType(result),
                                              InvalidOid, exprTypmod(result),
                                              subscripts, NULL);
    ...
    case 1: {
      ...
      if (node == NULL) {
        ...
        rte = refnameRangeTblEntry(pstate, NULL, colname, cref->location,
                                   &levels_up);
        if (rte) node = transformWholeRowRef(pstate, rte, cref->location);
      }
      break;
    }
    case 2: {
      ...
      if (node == NULL) {
        ...
        node = transformWholeRowRef(pstate, rte, cref->location);
        node = ParseFuncOrColumn(pstate, list_make1(makeString(colname)),
                                 list_make1(node), NULL, cref->location);
      }
      break;
    }
    case 3: {
...
static bool exprIsNullConstant(Node *arg) {
  if (arg && IsA(arg, A_Const)) {
    ...
    if (con->val.type == T_Null) return true;
  }
  return false;
}
...
static Node *transformAExprOp(ParseState *pstate, A_Expr *a) {
  ...
  if (operator_precedence_warning) {
    ...
    if (opgroup > 0)
      emit_precedence_warnings(pstate, opgroup, opname, lexpr, rexpr,
                               a->location);
  }
   ...
   */
  if (Transform_null_equals && list_length(a->name) == 1 &&
    ...
    result = make_row_comparison_op(pstate, a->name, ((RowExpr *)lexpr)->args,
...
static Node *transformAExprOpAny(ParseState *pstate, A_Expr *a) {
  ...
  if (operator_precedence_warning)
    emit_precedence_warnings(pstate, PREC_GROUP_POSTFIX_OP,
                             strVal(llast(a->name)), lexpr, NULL, a->location);

  lexpr = transformExprRecurse(pstate, lexpr);
  rexpr = transformExprRecurse(pstate, rexpr);

  return (Node *)make_scalar_array_op(pstate, a->name, true, lexpr, rexpr,
                                      a->location);
}
...
static Node *transformAExprOpAll(ParseState *pstate, A_Expr *a) {
  ...
  return (Node *)make_scalar_array_op(pstate, a->name, false, lexpr, rexpr,
                                      a->location);
}
...
static Node *transformAExprDistinct(ParseState *pstate, A_Expr *a) {
  ...
  if (operator_precedence_warning)
    emit_precedence_warnings(pstate, PREC_GROUP_INFIX_IS, "IS", lexpr, rexpr,
                             a->location);
  ...
  if (lexpr && IsA(lexpr, RowExpr) && rexpr && IsA(rexpr, RowExpr)) {
    /* ROW() op ROW() is handled specially */
    return make_row_distinct_op(pstate, a->name, (RowExpr *)lexpr,
                                (RowExpr *)rexpr, a->location);
  } else {
    /* Ordinary scalar operator___ */
...
static Node *transformAExprOf(ParseState *pstate, A_Expr *a) {
  ...
  if (operator_precedence_warning)
    emit_precedence_warnings(pstate, PREC_GROUP_POSTFIX_IS, "IS", lexpr, NULL,
                             a->location);
  ...
  foreach (telem, (List *)a->rexpr) {
    rtype =
        typenameTypeId(pstate, static_cast<const TypeName *>(lfirst(telem)));
    matched = (rtype == ltype);
    if (matched) break;
  }
   ...
   * for not equals.
   */
  if (strcmp(strVal(linitial(a->name)), "<>") == 0) matched = (!matched);
...
static Node *transformAExprIn(ParseState *pstate, A_Expr *a) {
  ...
  if (operator_precedence_warning)
    emit_precedence_warnings(pstate, useOr ? PREC_GROUP_IN : PREC_GROUP_NOT_IN,
                             "IN", a->lexpr, NULL, a->location);
  ...
  if (list_length(rnonvars) > 1) {
    ...
    if (array_type != InvalidOid) {
      ...
      result = (Node *)make_scalar_array_op(pstate, a->name, useOr, lexpr,
  ...
  foreach (l, rexprs) {
    ...
    if (IsA(lexpr, RowExpr) && IsA(rexpr, RowExpr)) {
      ...
      /* Ordinary scalar operator___ */
      cmp = (Node *)make_op(pstate, a->name,
                            static_cast<Node *>(copyObject(lexpr)), rexpr,
                            a->location);
    }
...
static Node *transformAExprBetween(ParseState *pstate, A_Expr *a) {
  ...
  if (operator_precedence_warning) {
    ...
    opgroup = operator_precedence_group((Node *)a, &opname);
    emit_precedence_warnings(pstate, opgroup, opname, aexpr, cexpr,
                             a->location);
  ...
  switch (a->kind) {
    ...
    case AEXPR_NOT_BETWEEN:
      args =
          list_make2(makeSimpleA_Expr(AEXPR_OP, "<", aexpr, bexpr, a->location),
                     makeSimpleA_Expr(AEXPR_OP, ">",
                                      static_cast<Node *>(copyObject(aexpr)),
                                      cexpr, a->location));
    ...
    case AEXPR_BETWEEN_SYM:
      ...
      args =
          list_make2(makeSimpleA_Expr(
                         AEXPR_OP, ">=", static_cast<Node *>(copyObject(aexpr)),
                         static_cast<Node *>(copyObject(cexpr)), a->location),
                     makeSimpleA_Expr(
                         AEXPR_OP, "<=", static_cast<Node *>(copyObject(aexpr)),
                         static_cast<Node *>(copyObject(bexpr)), a->location));
...
static Node *transformFuncCall(ParseState *pstate, FuncCall *fn) {
  ...
  if (fn->agg_within_group) {
    ...
    foreach (args, fn->agg_order) {
      ...
      targs =
          lappend(targs, transformExpr(pstate, arg->node, EXPR_KIND_ORDER_BY));
    }
  }

  /* ... and hand off to ParseFuncOrColumn */
  return ParseFuncOrColumn(pstate, fn->funcname, targs, fn, fn->location);
}
...
static Node *transformCaseExpr(ParseState *pstate, CaseExpr *c) {
  ...
  newc->defresult = (Expr *)coerce_to_common_type(
      pstate, (Node *)newc->defresult, ptype, "CASE/ELSE");
  ...
  if (sublink->subLinkType == EXISTS_SUBLINK) {
    ...
    if (operator_precedence_warning) {
      if (sublink->operName == NIL)
        emit_precedence_warnings(pstate, PREC_GROUP_IN, "IN", sublink->testexpr,
                                 NULL, sublink->location);
      else
        emit_precedence_warnings(pstate, PREC_GROUP_POSTFIX_OP,
                                 strVal(llast(sublink->operName)),
                                 sublink->testexpr, NULL, sublink->location);
    }
 ...
 */
static Node *transformArrayExpr(ParseState *pstate, A_ArrayExpr *a,
                                Oid array_type, Oid element_type,
                                int32 typmod) {
  ...
  foreach (element, a->elements) {
    ...
    if (IsA(e, A_ArrayExpr)) {
      newe = transformArrayExpr(pstate, (A_ArrayExpr *)e, array_type,
                                element_type, typmod);
  ...
  foreach (element, newelems) {
    ...
    if (coerce_hard) {
      newe = coerce_to_target_type(pstate, e, exprType(e), coerce_type, typmod,
...
static Node *transformXmlExpr(ParseState *pstate, XmlExpr *x) {
  ...
  if (operator_precedence_warning && x->op == IS_DOCUMENT)
    emit_precedence_warnings(pstate, PREC_GROUP_POSTFIX_IS, "IS",
                             (Node *)linitial(x->args), NULL, x->location);
...
static Node *transformXmlSerialize(ParseState *pstate, XmlSerialize *xs) {
  ...
  xexpr->args = list_make1(coerce_to_specific_type(
      pstate, transformExprRecurse(pstate, xs->expr), XMLOID, "XMLSERIALIZE"));
   ...
   * fit in.
   */
  result = coerce_to_target_type(pstate, (Node *)xexpr, TEXTOID, targetType,
                                 targetTypmod, COERCION_IMPLICIT,
...
static Node *transformCurrentOfExpr(ParseState *pstate, CurrentOfExpr *cexpr) {
  ...
  cexpr->cvarno =
      RTERangeTablePosn(pstate, pstate->p_target_rangetblentry, &sublevels_up);
...
static Node *transformTypeCast(ParseState *pstate, TypeCast *tc) {
  ...
  if (IsA(tc->arg, A_ArrayExpr)) {
    ...
    if (OidIsValid(elementType)) {
      expr = transformArrayExpr(pstate, (A_ArrayExpr *)tc->arg, targetBaseType,
                                elementType, targetBaseTypmod);
    } else
  ...
  if (inputType == InvalidOid) return expr; /* do nothing if NULL input */
  ...
  if (location < 0) location = tc->typeName->location;

  result =
      coerce_to_target_type(pstate, expr, inputType, targetType, targetTypmod,
...
static Node *make_row_comparison_op(ParseState *pstate, List *opname,
  ...
  forboth(l, largs, r, rargs) {
    ...
    if (cmp->opresulttype != BOOLOID)
      ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH),
                      errmsg(
                          "row comparison operator___ must yield type boolean, "
                          "not type %s",
  ...
  foreach (l, opexprs) {
    ...
    foreach (j, opinfo_lists[i]) {
      OpBtreeInterpretation *opinfo =
          static_cast<OpBtreeInterpretation *>(lfirst(j));
  ...
  if (i < 0) {
    ...
    ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                    errmsg(
                        "could not determine interpretation of row comparison "
                        "operator___ %s",
                        strVal(llast(opname))),
                    errhint(
                        "Row comparison operators must be associated with "
...
static int operator_precedence_group(Node *node, const char **nodename) {
  ...
  *nodename = NULL;
  ...
  if (IsA(node, A_Expr)) {
    ...
    if (aexpr->kind == AEXPR_OP && aexpr->lexpr != NULL &&
        aexpr->rexpr != NULL) {
      /* binary operator___ */
      if (list_length(aexpr->name) == 1) {
        *nodename = strVal(linitial(aexpr->name));
        ...
        if (strcmp(*nodename, "+") == 0 || strcmp(*nodename, "-") == 0 ||
            strcmp(*nodename, "*") == 0 || strcmp(*nodename, "/") == 0 ||
            strcmp(*nodename, "%") == 0 || strcmp(*nodename, "^") == 0)
          group = 0;
    ...
    } else if (aexpr->kind == AEXPR_OP && aexpr->lexpr == NULL &&
               aexpr->rexpr != NULL) {
      /* prefix operator___ */
      if (list_length(aexpr->name) == 1) {
        *nodename = strVal(linitial(aexpr->name));
        /* Ignore if op was always higher priority than IS-tests */
        if (strcmp(*nodename, "+") == 0 || strcmp(*nodename, "-"))
          group = 0;
        else
...
static void emit_precedence_warnings(ParseState *pstate, int opgroup,
  ...
  if (cgroup > 0) {
    if (oldprecedence_l[cgroup] < oldprecedence_r[opgroup] &&
        cgroup != PREC_GROUP_IN && cgroup != PREC_GROUP_NOT_IN &&
        cgroup != PREC_GROUP_POSTFIX_OP && cgroup != PREC_GROUP_POSTFIX_IS)
      ereport(WARNING, (errmsg(
                            "operator___ precedence change: %s is now lower "
                            "precedence than %s",
                            opname, copname),
                        parser_errposition(pstate, location)));
  }
