...
static void merge_collation_state(Oid collation, CollateStrength strength,
                                  int location, Oid collation2, int location2,
                                  assign_collations_context *context);
static void assign_aggregate_collations(Aggref *aggref,
                                        assign_collations_context *loccontext);
static void assign_ordered_set_collations(
    Aggref *aggref, assign_collations_context *loccontext);
static void assign_hypothetical_collations(
    Aggref *aggref, assign_collations_context *loccontext);
...
void assign_query_collations(ParseState *pstate, Query *query) {
   ...
   */
  (void)query_tree_walker(
      query,
      reinterpret_cast<query_tree_walker_fptr>(assign_query_collations_walker),
      (void *)pstate, QTW_IGNORE_RANGE_TABLE | QTW_IGNORE_CTE_SUBQUERIES);
}
...
static bool assign_query_collations_walker(Node *node, ParseState *pstate) {
   ...
   */
  if (IsA(node, SetOperationStmt)) return false;
  ...
  if (context.strength == COLLATE_CONFLICT) {
    ...
    ereport(
        ...
        (errcode(ERRCODE_COLLATION_MISMATCH),
         ...
         errhint(
             "You can choose the collation by applying the COLLATE clause to "
...
static bool assign_collations_walker(Node *node,
  ...
  switch (nodeTag(node)) {
    case T_CollateExpr: {
      ...
      (void)expression_tree_walker(
          node, reinterpret_cast<expression_tree_walker_fptr>(
                    assign_collations_walker),
          (void *)&loccontext);
    ...
    case T_RowCompareExpr: {
      ...
      forboth(l, expr->largs, r, expr->rargs) {
        ...
        coll =
            select_common_collation(context->pstate, list_make2(le, re), true);
    ...
    case T_TargetEntry:
      ...
      if (strength == COLLATE_CONFLICT &&
        ...
        ereport(ERROR,
                (errcode(ERRCODE_COLLATION_MISMATCH),
                 errmsg(
                     "collation mismatch between implicit collations \"%s\" "
                     "and \"%s\"",
                     get_collation_name(loccontext.collation),
                     get_collation_name(loccontext.collation2)),
                 errhint(
                     "You can choose the collation by applying the COLLATE "
                     "clause to one or both expressions."),
    ...
    case T_Query: {
      ...
      if (qtree->targetList == NIL) return false;
      tent = (TargetEntry *)linitial(qtree->targetList);
      Assert(IsA(tent, TargetEntry));
      if (tent->resjunk) return false;
   ...
   */
  merge_collation_state(collation, strength, location, loccontext.collation2,
                        loccontext.location2, context);
  ...
  while (h_cell && s_cell) {
    ...
    if (paircontext.strength == COLLATE_CONFLICT)
      ereport(ERROR,
              (errcode(ERRCODE_COLLATION_MISMATCH),
               errmsg(
                   "collation mismatch between implicit collations \"%s\" and "
                   "\"%s\"",
               ...
               errhint(
                   "You can choose the collation by applying the COLLATE "
                   "clause to one or both expressions."),
               parser_errposition(paircontext.pstate, paircontext.location2)));
    ...
    if (OidIsValid(paircontext.collation) &&
      ...
      s_tle->expr =
          (Expr *)makeRelabelType(s_tle->expr, exprType((Node *)s_tle->expr),
                                  exprTypmod((Node *)s_tle->expr),
                                  paircontext.collation, COERCE_IMPLICIT_CAST);
    }
     ...
     */
    if (merge_sort_collations)
      merge_collation_state(paircontext.collation, paircontext.strength,
                            paircontext.location, paircontext.collation2,
                            paircontext.location2, loccontext);
