...
Node *ParseFuncOrColumn(ParseState *pstate, List *funcname, List *fargs,
  ...
  fdresult =
      func_get_detail(funcname, fargs, argnames, nargs, actual_arg_types,
                      !func_variadic, true, &funcid, &rettype, &retset, &nvargs,
                      &vatype, &declared_arg_types, &argdefaults);
  ...
  if (fdresult == FUNCDETAIL_COERCION) {
    ...
    return coerce_type(pstate, static_cast<Node *>(linitial(fargs)),
                       actual_arg_types[0], rettype, -1, COERCION_EXPLICIT,
    ...
    if (over)
      ereport(ERROR, (errcode(ERRCODE_WRONG_OBJECT_TYPE),
                      errmsg(
                          "OVER specified, but %s is not a window function nor "
    ...
    if (AGGKIND_IS_ORDERED_SET(aggkind)) {
              ...
              (errcode(ERRCODE_UNDEFINED_FUNCTION),
               errmsg("function %s does not exist",
                      func_signature_string(funcname, nargs, argnames,
                                            actual_arg_types)),
               errhint(
                   "There is an ordered-set aggregate %s, but it requires %d "
                   "direct arguments, not %d.",
                   NameListToString(funcname), catDirectArgs, numDirectArgs),
               parser_errposition(pstate, location)));
      } else {
        /*
                      ...
                      (errcode(ERRCODE_UNDEFINED_FUNCTION),
                       errmsg("function %s does not exist",
                              func_signature_string(funcname, nargs, argnames,
                                                    actual_arg_types)),
                       errhint(
                           "There is an ordered-set aggregate %s, but it "
                           "requires at least %d direct arguments.",
                           NameListToString(funcname), catDirectArgs),
                       parser_errposition(pstate, location)));
          }
        }
      }
      ...
      if (agg_within_group)
        ereport(ERROR, (errcode(ERRCODE_WRONG_OBJECT_TYPE),
                        errmsg(
                            "%s is not an ordered-set aggregate, so it cannot "
                            "have WITHIN GROUP",
                            NameListToString(funcname)),
                        parser_errposition(pstate, location)));
    }
              ...
              (errcode(ERRCODE_UNDEFINED_FUNCTION),
               errmsg("function %s does not exist",
                      func_signature_string(funcname, nargs, argnames,
                                            actual_arg_types)),
               errhint(
                   "No aggregate function matches the given name and argument "
                   "types. "
 ...
 */
TableSampleClause *ParseTableSample(ParseState *pstate, char *samplemethod,
                                    Node *repeatable, List *sampleargs,
                                    int location) {
  HeapTuple tuple;
  ...
  if (!HeapTupleIsValid(tuple))
    ereport(ERROR,
            (errcode(ERRCODE_UNDEFINED_OBJECT),
             errmsg("tablesample method \"%s\" does not exist", samplemethod),
             parser_errposition(pstate, location)));
  ...
  tuple = SearchSysCache1(PROCOID, ObjectIdGetDatum(tablesample->tsminit));

  if (!HeapTupleIsValid(tuple)) /* should not happen */
    elog(ERROR, "cache lookup failed for function %u", tablesample->tsminit);
  ...
  if (list_length(sampleargs) != initnargs)
    ereport(ERROR,
            (errcode(ERRCODE_TOO_MANY_ARGUMENTS),
             errmsg_plural(
                 "tablesample method \"%s\" expects %d argument got %d",
                 "tablesample method \"%s\" expects %d arguments got %d",
                 initnargs, samplemethod, initnargs, list_length(sampleargs)),
             parser_errposition(pstate, location)));
 ...
 */
int func_match_argtypes(int nargs, Oid *input_typeids,
                        ...
                        FuncCandidateList *candidates) /* return value */
{
  ...
  *candidates = NULL;
  ...
  for (current_candidate = raw_candidates; current_candidate != NULL;
 ...
 * - ay 6/95
 */
FuncCandidateList func_select_candidate(int nargs, Oid *input_typeids,
  ...
  for (i = 0; i < nargs; i++) {
    ...
    for (current_candidate = candidates; current_candidate != NULL;
      ...
      get_type_category_preferred(current_type, &current_category,
  ...
  if (nunknowns < nargs) {
    ...
    if (known_type != UNKNOWNOID) {
      ...
      for (i = 0; i < nargs; i++) input_base_typeids[i] = known_type;
      ncandidates = 0;
      last_candidate = NULL;
      for (current_candidate = candidates; current_candidate != NULL;
           current_candidate = current_candidate->next) {
                               ...
                               List **argdefaults) /* optional return value */
{
  ...
  *vatype = InvalidOid;
  *true_typeids = NULL;
  if (argdefaults) *argdefaults = NIL;

  /* Get list of possible candidates from namescpace___ search */
  raw_candidates = FuncnameGetCandidates(
      funcname, nargs, fargnames, expand_variadic, expand_defaults, false);
  ...
  for (best_candidate = raw_candidates; best_candidate != NULL;
    ...
    if (memcmp(argtypes, best_candidate->args, nargs * sizeof(Oid)) == 0) break;
  }
  ...
  if (best_candidate == NULL) {
    ...
    if (raw_candidates != NULL) {
      ...
      ncandidates = func_match_argtypes(nargs, argtypes, raw_candidates,
                                        &current_candidates);

      /* one match only? then run with it... */
      if (ncandidates == 1) best_candidate = current_candidates;
       ...
       */
        ...
        best_candidate =
            func_select_candidate(nargs, argtypes, current_candidates);

        /*
         * If we were able to choose a best candidate, we're done.
         * Otherwise, ambiguous function call.
         */
        if (!best_candidate) return FUNCDETAIL_MULTIPLE;
      }
    }
  }
  ...
  if (best_candidate) {
     ...
     */
    if (!OidIsValid(best_candidate->oid)) return FUNCDETAIL_MULTIPLE;
    ...
    *vatype = pform->provariadic;
    ...
    if (argdefaults && best_candidate->ndargs > 0) {
      ...
      proargdefaults =
          SysCacheGetAttr(PROCOID, ftup, Anum_pg_proc_proargdefaults, &isnull);
      Assert(!isnull);
      ...
      if (best_candidate->argnumbers != NULL) {
        ...
        firstdefarg =
            &best_candidate
                 ->argnumbers[best_candidate->nargs - best_candidate->ndargs];
...
static void unify_hypothetical_args(ParseState *pstate, List *fargs,
  ...
  i = 0;
  foreach (lc, fargs) { args[i++] = (Node *)lfirst(lc); }
  ...
  for (i = numNonHypotheticalArgs; i < numDirectArgs; i++) {
    ...
    if (declared_arg_types[i] != declared_arg_types[aargpos])
      elog(ERROR,
           "hypothetical-set aggregate has inconsistent declared argument "
           "types");

    /* No need to unify if make_fn_arguments will coerce */
    if (declared_arg_types[i] != ANYOID) continue;
     ...
     */
    commontype = select_common_type(pstate, list_make2(args[aargpos], args[i]),
                                    "WITHIN GROUP", NULL);
     ...
     */
    args[i] = coerce_type(pstate, args[i], actual_arg_types[i], commontype, -1,
                          COERCION_IMPLICIT, COERCE_IMPLICIT_CAST, -1);
    actual_arg_types[i] = commontype;
    args[aargpos] = coerce_type(pstate, args[aargpos],
                                actual_arg_types[aargpos], commontype, -1,
                                COERCION_IMPLICIT, COERCE_IMPLICIT_CAST, -1);
  ...
  i = 0;
  foreach (lc, fargs) { lfirst(lc) = args[i++]; }
}
 ...
 */
void make_fn_arguments(ParseState *pstate, List *fargs, Oid *actual_arg_types,
                       Oid *declared_arg_types) {
  ...
  foreach (current_fargs, fargs) {
    ...
    if (actual_arg_types[i] != declared_arg_types[i]) {
      ...
      if (IsA(node, NamedArgExpr)) {
        ...
        node = coerce_type(pstate, (Node *)na->arg, actual_arg_types[i],
                           declared_arg_types[i], -1, COERCION_IMPLICIT,
                           COERCE_IMPLICIT_CAST, -1);
        na->arg = (Expr *)node;
      } else {
        node = coerce_type(pstate, node, actual_arg_types[i],
                           declared_arg_types[i], -1, COERCION_IMPLICIT,
                           COERCE_IMPLICIT_CAST, -1);
...
static Oid FuncNameAsType(List *funcname) {
  ...
  typtup =
      LookupTypeName(NULL, makeTypeNameFromNameList(funcname), NULL, false);
  if (typtup == NULL) return InvalidOid;
...
static Node *ParseComplexProjection(ParseState *pstate, char *funcname,
  ...
  if (IsA(first_arg, Var) &&
    ...
    rte = GetRTEByRangeTablePosn(pstate, ((Var *)first_arg)->varno,
  ...
  else if (get_expr_result_type(first_arg, NULL, &tupdesc) !=
           TYPEFUNC_COMPOSITE)
 ...
 * messages.
 */
const char *funcname_signature_string(const char *funcname, int nargs,
  ...
  for (i = 0; i < nargs; i++) {
    if (i) appendStringInfoString(&argbuf, ", ");
    if (i >= numposargs) {
 ...
 */
const char *func_signature_string(List *funcname, int nargs, List *argnames,
                                  const Oid *argtypes) {
  return funcname_signature_string(NameListToString(funcname), nargs, argnames,
                                   argtypes);
}
 ...
 * else raise an error.
 */
Oid LookupFuncName(List *funcname, int nargs, const Oid *argtypes,
                   bool noError) {
  ...
  if (!OidIsValid(oid)) {
    ...
    if (argcount == 0)
      ...
      ereport(ERROR,
              (errcode(ERRCODE_UNDEFINED_FUNCTION),
               errmsg("aggregate %s does not exist",
                      func_signature_string(aggname, argcount, NIL, argoids))));
  }
