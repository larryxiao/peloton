...
static const char *const recursion_errormsgs[] = {
    ...
    gettext_noop(
        "recursive reference to query \"%s\" must not appear within its "
        "non-recursive term"),
    /* RECURSION_SUBLINK */
    gettext_noop(
        "recursive reference to query \"%s\" must not appear within a "
        "subquery"),
    /* RECURSION_OUTERJOIN */
    gettext_noop(
        "recursive reference to query \"%s\" must not appear within an outer "
        "join"),
    /* RECURSION_INTERSECT */
    gettext_noop(
...
List *transformWithClause(ParseState *pstate, WithClause *withClause) {
  ...
  foreach (lc, withClause->ctes) {
    ...
    if (!IsA(cte->ctequery, SelectStmt)) {
      ...
      Assert(IsA(cte->ctequery, InsertStmt) || IsA(cte->ctequery, UpdateStmt) ||
...
static void analyzeCTE(ParseState *pstate, CommonTableExpr *cte) {
   ...
   */
  if (!IsA(query, Query)) elog(ERROR, "unexpected non-Query statement in WITH");
  ...
  if (query->commandType != CMD_SELECT && pstate->parentParseState != NULL)
    ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                    errmsg(
                        "WITH clause containing a data-modifying statement "
  ...
  if (!cte->cterecursive) {
    ...
    foreach (lctlist, GetCTETargetList(cte)) {
      ...
      if (lctyp == NULL || lctypmod == NULL ||
          lccoll == NULL) /* shouldn't happen */
      ...
      if (exprType(texpr) != lfirst_oid(lctyp) ||
        ...
        ereport(
            ...
            (errcode(ERRCODE_DATATYPE_MISMATCH),
             errmsg(
                 "recursive query \"%s\" column %d has type %s in "
                 "non-recursive term but type %s overall",
                 cte->ctename, varattno,
                 format_type_with_typemod(lfirst_oid(lctyp),
                                          lfirst_int(lctypmod)),
                 format_type_with_typemod(exprType(texpr), exprTypmod(texpr))),
             errhint(
                 "Cast the output of the non-recursive term to the correct "
                 "type."),
      ...
      if (exprCollation(texpr) != lfirst_oid(lccoll))
        ereport(
            ...
            (errcode(ERRCODE_COLLATION_MISMATCH),
             errmsg(
                 "recursive query \"%s\" column %d has collation \"%s\" in "
                 "non-recursive term but collation \"%s\" overall",
                 cte->ctename, varattno, get_collation_name(lfirst_oid(lccoll)),
                 get_collation_name(exprCollation(texpr))),
             errhint(
                 "Use the COLLATE clause to set the collation of the "
                 "non-recursive term."),
             parser_errposition(pstate, exprLocation(texpr))));
      lctyp = lnext(lctyp);
 ...
 */
void analyzeCTETargetList(ParseState *pstate, CommonTableExpr *cte,
                          List *tlist) {
  int numaliases;
  int varattno;
  ListCell *tlistitem;
  ...
  foreach (tlistitem, tlist) {
    ...
    if (cte->cterecursive && coltype == UNKNOWNOID) {
      ...
      if (!OidIsValid(colcoll)) colcoll = DEFAULT_COLLATION_OID;
    }
  ...
  if (varattno < numaliases)
    ereport(ERROR, (errcode(ERRCODE_INVALID_COLUMN_REFERENCE),
                    errmsg(
                        "WITH query \"%s\" has %d columns available but %d "
                        "columns specified",
      ...
      for (i = 0; i < cstate->numitems; i++) {
        ...
        if (strcmp(rv->relname, cte->ctename) == 0) {
          ...
          if (i != myindex) {
            ...
            cstate->items[myindex].depends_on = bms_add_member(
                cstate->items[myindex].depends_on, cstate->items[i].id);
          } else {
  ...
  if (IsA(node, SelectStmt)) {
    ...
    if (stmt->withClause) {
      if (stmt->withClause->recursive) {
        ...
        (void)raw_expression_tree_walker(
            node, reinterpret_cast<raw_expression_tree_walker_fptr>(
  ...
  for (i = 0; i < cstate->numitems; i++) {
    ...
    if (!IsA(stmt, SelectStmt))
      ereport(ERROR, (errcode(ERRCODE_INVALID_RECURSION),
                      errmsg(
                          "recursive query \"%s\" must not contain "
    ...
    if (stmt->op != SETOP_UNION)
      ereport(ERROR, (errcode(ERRCODE_INVALID_RECURSION),
                      errmsg(
                          "recursive query \"%s\" does not have the form "
...
static bool checkWellFormedRecursionWalker(Node *node, CteState *cstate) {
  ...
  if (IsA(node, JoinExpr)) {
    ...
    switch (j->jointype) {
      ...
      case JOIN_LEFT:
        checkWellFormedRecursionWalker(j->larg, cstate);
        if (save_context == RECURSION_OK) cstate->context = RECURSION_OUTERJOIN;
        checkWellFormedRecursionWalker(j->rarg, cstate);
...
static void checkWellFormedSelectStmt(SelectStmt *stmt, CteState *cstate) {
  ...
  if (save_context != RECURSION_OK) {
    ...
    raw_expression_tree_walker(
        (Node *)stmt, reinterpret_cast<raw_expression_tree_walker_fptr>(
                          checkWellFormedRecursionWalker),
        (void *)cstate);
  } else {
    switch (stmt->op) {
      ...
      case SETOP_INTERSECT:
        if (stmt->all) cstate->context = RECURSION_INTERSECT;
        checkWellFormedRecursionWalker((Node *)stmt->larg, cstate);
        checkWellFormedRecursionWalker((Node *)stmt->rarg, cstate);
        cstate->context = save_context;
        ...
        break;
      case SETOP_EXCEPT:
        if (stmt->all) cstate->context = RECURSION_EXCEPT;
        checkWellFormedRecursionWalker((Node *)stmt->larg, cstate);
