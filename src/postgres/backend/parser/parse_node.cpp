...
int parser_errposition(ParseState *pstate, int location) {
  ...
  if (pstate == NULL || pstate->p_sourcetext == NULL) return 0;
 ...
 */
Var *make_var(ParseState *pstate, RangeTblEntry *rte, int attrno,
              int location) {
  Var *result;
 ...
 */
ArrayRef *transformArraySubscripts(ParseState *pstate, Node *arrayBase,
                                   Oid arrayType, Oid elementType,
                                   int32 arrayTypMod, List *indirection,
                                   Node *assignFrom) {
  bool isSlice = false;
  ...
  foreach (idx, indirection) {
    ...
    /* If it's not int4 already, try to coerce */
    subexpr =
        coerce_to_target_type(pstate, subexpr, exprType(subexpr), INT4OID, -1,
  ...
  if (assignFrom != NULL) {
    ...
    newFrom = coerce_to_target_type(pstate, assignFrom, typesource, typeneeded,
                                    arrayTypMod, COERCION_ASSIGNMENT,
                                    COERCE_IMPLICIT_CAST, -1);
    if (newFrom == NULL)
      ereport(ERROR,
              (errcode(ERRCODE_DATATYPE_MISMATCH),
...
Const *make_const(ParseState *pstate, Value *value, int location) {
  ...
  switch (nodeTag(value)) {
    ...
    case T_Float:
      ...
      if (scanint8(strVal(value), true, &val64)) {
        ...
        setup_parser_errposition_callback(&pcbstate, pstate, location);
        val = DirectFunctionCall3(numeric_in, CStringGetDatum(strVal(value)),
    ...
    case T_BitString:
      ...
      setup_parser_errposition_callback(&pcbstate, pstate, location);
      val =
          DirectFunctionCall3(bit_in, CStringGetDatum(strVal(value)),
  ...
  con = makeConst(typeid___, -1, /* typmod -1 is OK for all cases */
