...
static void markTargetListOrigin(ParseState *pstate, TargetEntry *tle, Var *var,
                                 int levelsup);
static Node *transformAssignmentIndirection(
    ParseState *pstate, Node *basenode, const char *targetName,
    bool targetIsArray, Oid targetTypeId, int32 targetTypMod,
    Oid targetCollation, ListCell *indirection, Node *rhs, int location);
static Node *transformAssignmentSubscripts(
    ParseState *pstate, Node *basenode, const char *targetName,
    Oid targetTypeId, int32 targetTypMod, Oid targetCollation, List *subscripts,
    bool isSlice, ListCell *next_indirection, Node *rhs, int location);
 ...
 */
TargetEntry *transformTargetEntry(ParseState *pstate, Node *node, Node *expr,
                                  ParseExprKind exprKind, char *colname,
                                  bool resjunk) {
  /* Transform the node if caller didn't do it already */
  if (expr == NULL) expr = transformExpr(pstate, node, exprKind);
  ...
  foreach (o_target, targetlist) {
    ...
    if (IsA(res->val, ColumnRef)) {
      ...
      if (IsA(llast(cref->fields), A_Star)) {
        ...
        p_target =
            list_concat(p_target, ExpandColumnRefStar(pstate, cref, true));
        continue;
      }
      ...
      if (IsA(llast(ind->indirection), A_Star)) {
        ...
        p_target = list_concat(
            p_target, ExpandIndirectionStar(pstate, ind, true, exprKind));
        continue;
      }
    }
     ...
     */
    p_target =
        lappend(p_target, transformTargetEntry(pstate, res->val, NULL, exprKind,
                                               res->name, false));
  }
  ...
  foreach (lc, exprlist) {
    ...
    if (IsA(e, ColumnRef)) {
      ...
      if (IsA(llast(ind->indirection), A_Star)) {
        /* It is something.*, expand into multiple items */
        result = list_concat(
            result, ExpandIndirectionStar(pstate, ind, false, exprKind));
        continue;
      }
    }
...
static void markTargetListOrigin(ParseState *pstate, TargetEntry *tle, Var *var,
  ...
  if (var == NULL || !IsA(var, Var)) return;
...
Expr *transformAssignedExpr(ParseState *pstate, Expr *expr,
  ...
  if (indirection) {
    ...
    if (pstate->p_is_insert) {
       ...
       */
      colVar = (Node *)make_var(pstate, pstate->p_target_rangetblentry, attrno,
                                location);
    }

    expr = (Expr *)transformAssignmentIndirection(
        pstate, colVar, colname, false, attrtype, attrtypmod, attrcollation,
        list_head(indirection), (Node *)expr, location);
  } else {
    /*
    ...
    expr = (Expr *)coerce_to_target_type(pstate, orig_expr, type_id, attrtype,
                                         attrtypmod, COERCION_ASSIGNMENT,
 ...
 */
void updateTargetListEntry(ParseState *pstate, TargetEntry *tle, char *colname,
                           int attrno, List *indirection, int location) {
...
static Node *transformAssignmentIndirection(
  ...
  for_each_cell(i, indirection) {
    ...
    if (IsA(n, A_Indices)) {
      ...
      if (((A_Indices *)n)->lidx != NULL) isSlice = true;
      ...
      if (subscripts) {
        ...
        return transformAssignmentSubscripts(
            pstate, basenode, targetName, targetTypeId, targetTypMod,
            targetCollation, subscripts, isSlice, i, rhs, location);
      }
      ...
      if (!typrelid)
        ereport(ERROR,
                (errcode(ERRCODE_DATATYPE_MISMATCH),
                 errmsg(
                     "cannot assign to field \"%s\" of column \"%s\" because "
                     "its type %s is not a composite type",
                     strVal(n), targetName, format_type_be(targetTypeId)),
                 parser_errposition(pstate, location)));
      ...
      if (attnum == InvalidAttrNumber)
        ereport(ERROR,
                (errcode(ERRCODE_UNDEFINED_COLUMN),
                 errmsg(
                     "cannot assign to field \"%s\" of column \"%s\" because "
                     "there is no such column in data type %s",
                     strVal(n), targetName, format_type_be(targetTypeId)),
                 parser_errposition(pstate, location)));
      if (attnum < 0)
        ereport(ERROR,
      ...
      get_atttypetypmodcoll(typrelid, attnum, &fieldTypeId, &fieldTypMod,
                            &fieldCollation);

      /* recurse to create appropriate RHS for field assign */
      rhs = transformAssignmentIndirection(
          pstate, NULL, strVal(n), false, fieldTypeId, fieldTypMod,
          fieldCollation, lnext(i), rhs, location);
  ...
  if (subscripts) {
    ...
    return transformAssignmentSubscripts(
        pstate, basenode, targetName, targetTypeId, targetTypMod,
        targetCollation, subscripts, isSlice, NULL, rhs, location);
  }

  /* base case: just coerce RHS to match target type ID */

  result = coerce_to_target_type(pstate, rhs, exprType(rhs), targetTypeId,
      ...
      ereport(ERROR,
              (errcode(ERRCODE_DATATYPE_MISMATCH),
               errmsg(
                   ...
                   " but expression is of type %s",
                   targetName, format_type_be(targetTypeId),
...
static Node *transformAssignmentSubscripts(
  ...
  rhs = transformAssignmentIndirection(pstate, NULL, targetName, true,
                                       typeNeeded, arrayTypMod, collationNeeded,
                                       next_indirection, rhs, location);

  /* process subscripts */
  result = (Node *)transformArraySubscripts(
      pstate, basenode, arrayType, elementTypeId, arrayTypMod, subscripts, rhs);

  /* If target was a domain over array, need to coerce up to the domain */
  if (arrayType != targetTypeId) {
    Oid resulttype = exprType(result);

    result = coerce_to_target_type(pstate, result, resulttype, targetTypeId,
                                   targetTypMod, COERCION_ASSIGNMENT,
                                   COERCE_IMPLICIT_CAST, -1);
    ...
    if (result == NULL)
      ereport(ERROR,
              (errcode(ERRCODE_CANNOT_COERCE),
               errmsg("cannot cast type %s to %s", format_type_be(resulttype),
...
static List *ExpandAllTables(ParseState *pstate, int location) {
  ...
  foreach (l, pstate->p_namespace) {
    ...
    target =
        list_concat(target, expandRelAttrs(pstate, rte,
                                           RTERangeTablePosn(pstate, rte, NULL),
                                           0, location));
  }
...
static List *ExpandIndirectionStar(ParseState *pstate, A_Indirection *ind,
  ...
  ind->indirection =
      list_truncate(ind->indirection, list_length(ind->indirection) - 1);
...
static List *ExpandRowReference(ParseState *pstate, Node *expr,
  ...
  for (i = 0; i < numAttrs; i++) {
    ...
    if (make_target_entry) {
      ...
      te = makeTargetEntry((Expr *)fselect, (AttrNumber)pstate->p_next_resno++,
...
TupleDesc expandRecordVariable(ParseState *pstate, Var *var, int levelsup) {
  ...
  if (attnum == InvalidAttrNumber) {
    ...
    forboth(lname, names, lvar, vars) {
      ...
      TupleDescInitEntry(tupleDesc, i, label, exprType(varnode),
                         exprTypmod(varnode), 0);
    ...
    case RTE_CTE:
      ...
      if (!rte->self_reference) {
        ...
        if (IsA(expr, Var)) {
          ...
          for (levelsup = 0; levelsup < rte->ctelevelsup + netlevelsup;
               levelsup++)
  ...
  switch (nodeTag(node)) {
    case T_ColumnRef: {
      ...
      foreach (l, ((ColumnRef *)node)->fields) {
        ...
        if (IsA(i, String)) fname = strVal(i);
      }
      if (fname) {
        *name = fname;
        return 2;
    ...
    case T_CaseExpr:
      strength =
          FigureColnameInternal((Node *)((CaseExpr *)node)->defresult, name);
      if (strength <= 1) {
        *name = "case";
        return 1;
      }
