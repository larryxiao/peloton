...
TupleTableSlot *ExecSort(SortState *node) {
  ...
  if (!node->sort_Done) {
    ...
    tuplesortstate = tuplesort_begin_heap(
        tupDesc, plannode->numCols, plannode->sortColIdx,
        plannode->sortOperators, plannode->collations, plannode->nullsFirst,
        work_mem, node->randomAccess);
    if (node->bounded) tuplesort_set_bound(tuplesortstate, node->bound);
...
void ExecReScanSort(SortState *node) {
   ...
   */
  if (outerPlan->chgParam != NULL || node->bounded != node->bounded_Done ||
