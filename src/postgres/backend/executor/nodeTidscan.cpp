  ...
  foreach (l, evalList) {
    ...
    if (is_opclause(expr)) {
      ...
      itemptr = (ItemPointer)DatumGetPointer(
          ExecEvalExprSwitchContext(exstate, econtext, &isNull, NULL));
      if (!isNull && ItemPointerIsValid(itemptr) &&
          ItemPointerGetBlockNumber(itemptr) < nblocks) {
        if (numTids >= numAllocTids) {
          numAllocTids *= 2;
          tidList = (ItemPointerData *)repalloc(
              tidList, numAllocTids * sizeof(ItemPointerData));
        }
      ...
      arraydatum = ExecEvalExprSwitchContext(exstate, econtext, &isNull, NULL);
      if (isNull) continue;
      itemarray = DatumGetArrayTypeP(arraydatum);
      deconstruct_array(itemarray, TIDOID, SizeOfIptrData, false, 's',
...
static TupleTableSlot *TidNext(TidScanState *node) {
   ...
   */
  if (node->tss_TidList == NULL) TidListCreate(node);
...
TidScanState *ExecInitTidScan(TidScan *node, EState *estate, int eflags) {
   ...
   */
  tidstate->ss.ps.targetlist = (List *)ExecInitExpr(
      (Expr *)node->scan.plan.targetlist, (PlanState *)tidstate);
  tidstate->ss.ps.qual =
      (List *)ExecInitExpr((Expr *)node->scan.plan.qual, (PlanState *)tidstate);

  tidstate->tss_tidquals =
      (List *)ExecInitExpr((Expr *)node->tidquals, (PlanState *)tidstate);
