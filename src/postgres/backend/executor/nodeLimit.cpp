  ...
  switch (node->lstate) {
    ...
    case LIMIT_RESCAN:
      ...
      for (;;) {
        ...
        node->subSlot = slot;
        if (++node->position > node->offset) break;
      }
    ...
    case LIMIT_SUBPLANEOF:
       ...
       */
      slot = ExecProcNode(outerPlan);
      if (TupIsNull(slot)) elog(ERROR, "LIMIT subplan failed to run backwards");
      node->subSlot = slot;
      node->lstate = LIMIT_INWINDOW;
...
static void recompute_limits(LimitState *node) {
  ...
  if (node->limitCount) {
    ...
    if (isNull) {
      ...
      if (node->count < 0)
        ereport(ERROR, (errcode(ERRCODE_INVALID_ROW_COUNT_IN_LIMIT_CLAUSE),
...
LimitState *ExecInitLimit(Limit *node, EState *estate, int eflags) {
  ...
  limitstate->limitOffset =
      ExecInitExpr((Expr *)node->limitOffset, (PlanState *)limitstate);
  limitstate->limitCount =
      ExecInitExpr((Expr *)node->limitCount, (PlanState *)limitstate);
