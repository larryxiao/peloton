 ...
 */
static void tstoreStartupReceiver(DestReceiver *self, int operation,
                                  TupleDesc typeinfo,
                                  MemcachedState *mc_state = nullptr) {
  ...
  if (myState->detoast) {
    for (i = 0; i < natts; i++) {
      if (attrs[i]->attisdropped) continue;
      if (attrs[i]->attlen == -1) {
        needtoast = true;
        break;
      }
  ...
  if (needtoast) {
    ...
    /* Create workspace */
    myState->outvalues =
        (Datum *)MemoryContextAlloc(myState->cxt, natts * sizeof(Datum));
    myState->tofree =
        (Datum *)MemoryContextAlloc(myState->cxt, natts * sizeof(Datum));
  } else {
...
static void tstoreReceiveSlot_detoast(TupleTableSlot *slot, DestReceiver *self,
  ...
  for (i = 0; i < natts; i++) {
    ...
    if (!attrs[i]->attisdropped && attrs[i]->attlen == -1 &&
        !slot->tts_isnull[i]) {
      if (VARATT_IS_EXTERNAL(DatumGetPointer(val))) {
        val = PointerGetDatum(
            heap_tuple_fetch_attr((struct varlena *)DatumGetPointer(val)));
  ...
  tuplestore_putvalues(myState->tstore, typeinfo, myState->outvalues,
                       slot->tts_isnull);
  MemoryContextSwitchTo(oldcxt);

  /* And release any temporary detoasted values */
  for (i = 0; i < nfree; i++) pfree(DatumGetPointer(myState->tofree[i]));
}
...
static void tstoreShutdownReceiver(DestReceiver *self) {
  ...
  if (myState->outvalues) pfree(myState->outvalues);
  myState->outvalues = NULL;
  if (myState->tofree) pfree(myState->tofree);
  myState->tofree = NULL;
}
 ...
 */
static void tstoreDestroyReceiver(DestReceiver *self) { pfree(self); }
