...
static TupleTableSlot *BitmapHeapNext(BitmapHeapScanState *node) {
  ...
  for (;;) {
     ...
     */
    ExecStoreTuple(&scan->rs_ctup, slot, scan->rs_cbuf, false);
...
void ExecReScanBitmapHeapScan(BitmapHeapScanState *node) {
  ...
  if (node->prefetch_iterator) tbm_end_iterate(node->prefetch_iterator);
...
BitmapHeapScanState *ExecInitBitmapHeapScan(BitmapHeapScan *node,
  ...
  scanstate->ss.ps.qual = (List *)ExecInitExpr((Expr *)node->scan.plan.qual,
                                               (PlanState *)scanstate);
  scanstate->bitmapqualorig = (List *)ExecInitExpr((Expr *)node->bitmapqualorig,
                                                   (PlanState *)scanstate);
