...
static TupleTableSlot *IndexOnlyNext(IndexOnlyScanState *node) {
  ...
  while ((tid = index_getnext_tid(scandesc, direction)) != NULL) {
    ...
    if (!visibilitymap_test(scandesc->heapRelation,
      ...
      if (tuple == NULL) continue; /* no visible tuple, try next index entry */
    ...
    if (scandesc->numberOfOrderBys > 0 && scandesc->xs_recheckorderby)
      ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                      errmsg(
                          "lossy distance functions are not supported in "
                          "index-only scans")));
...
TupleTableSlot *ExecIndexOnlyScan(IndexOnlyScanState *node) {
  ...
  return ExecScan(&node->ss, (ExecScanAccessMtd)IndexOnlyNext,
...
void ExecReScanIndexOnlyScan(IndexOnlyScanState *node) {
  ...
  if (node->ioss_NumRuntimeKeys != 0) {
    ...
    ResetExprContext(econtext);
    ExecIndexEvalRuntimeKeys(econtext, node->ioss_RuntimeKeys,
 ...
 * ----------------------------------------------------------------
 */
IndexOnlyScanState *ExecInitIndexOnlyScan(IndexOnlyScan *node, EState *estate,
                                          int eflags) {
  ...
  indexstate->indexqual =
      (List *)ExecInitExpr((Expr *)node->indexqual, (PlanState *)indexstate);
  ...
  ExecIndexBuildScanKeys(
      (PlanState *)indexstate, indexstate->ioss_RelationDesc, node->indexqual,
      false, &indexstate->ioss_ScanKeys, &indexstate->ioss_NumScanKeys,
      &indexstate->ioss_RuntimeKeys, &indexstate->ioss_NumRuntimeKeys,
      NULL, /* no ArrayKeys */
      NULL);
   ...
   */
  ExecIndexBuildScanKeys(
      (PlanState *)indexstate, indexstate->ioss_RelationDesc,
      node->indexorderby, true, &indexstate->ioss_OrderByKeys,
      &indexstate->ioss_NumOrderByKeys, &indexstate->ioss_RuntimeKeys,
      &indexstate->ioss_NumRuntimeKeys, NULL, /* no ArrayKeys */
      NULL);
   ...
   */
  indexstate->ioss_ScanDesc = index_beginscan(
      currentRelation, indexstate->ioss_RelationDesc, estate->es_snapshot,
      indexstate->ioss_NumScanKeys, indexstate->ioss_NumOrderByKeys);
   ...
   */
  if (indexstate->ioss_NumRuntimeKeys == 0)
    index_rescan(indexstate->ioss_ScanDesc, indexstate->ioss_ScanKeys,
                 indexstate->ioss_NumScanKeys, indexstate->ioss_OrderByKeys,
