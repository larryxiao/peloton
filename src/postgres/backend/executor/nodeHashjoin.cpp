  ...
  for (;;) {
    switch (node->hj_JoinState) {
      case HJ_BUILD_HASHTABLE:
        ...
        } else if (HJ_FILL_OUTER(node) || (outerNode->plan->startup_cost <
        ...
        hashtable =
            ExecHashTableCreate((Hash *)hashNode->ps.plan,
                                node->hj_HashOperators, HJ_FILL_INNER(node));
         ...
         * outer relation.
         */
        if (hashtable->totalTuples == 0 && !HJ_FILL_OUTER(node)) return NULL;
      ...
      case HJ_NEED_NEW_OUTER:
        ...
        ExecHashGetBucketAndBatch(hashtable, hashvalue, &node->hj_CurBucketNo,
                                  &batchno);
        ...
        if (batchno != hashtable->curbatch &&
          ...
          ExecHashJoinSaveTuple(ExecFetchSlotMinimalTuple(outerTupleSlot),
                                hashvalue, &hashtable->outerBatchFile[batchno]);
      ...
      case HJ_NEED_NEW_BATCH:
         ...
         */
        if (!ExecHashJoinNewBatch(node)) return NULL; /* end of join */
        node->hj_JoinState = HJ_NEED_NEW_OUTER;
        break;
...
HashJoinState *ExecInitHashJoin(HashJoin *node, EState *estate, int eflags) {
   ...
   */
  hjstate->js.ps.targetlist = (List *)ExecInitExpr(
      (Expr *)node->join.plan.targetlist, (PlanState *)hjstate);
  hjstate->js.ps.qual =
      (List *)ExecInitExpr((Expr *)node->join.plan.qual, (PlanState *)hjstate);
  hjstate->js.jointype = node->join.jointype;
  hjstate->js.joinqual =
      (List *)ExecInitExpr((Expr *)node->join.joinqual, (PlanState *)hjstate);
  hjstate->hashclauses =
      (List *)ExecInitExpr((Expr *)node->hashclauses, (PlanState *)hjstate);
  ...
  switch (node->join.jointype) {
    ...
    case JOIN_LEFT:
    case JOIN_ANTI:
      hjstate->hj_NullInnerTupleSlot = ExecInitNullTupleSlot(
          estate, ExecGetResultType(innerPlanState(hjstate)));
      break;
    case JOIN_RIGHT:
      hjstate->hj_NullOuterTupleSlot = ExecInitNullTupleSlot(
          estate, ExecGetResultType(outerPlanState(hjstate)));
      break;
    case JOIN_FULL:
      hjstate->hj_NullOuterTupleSlot = ExecInitNullTupleSlot(
          estate, ExecGetResultType(outerPlanState(hjstate)));
...
static bool ExecHashJoinNewBatch(HashJoinState *hjstate) {
  ...
  if (curbatch >= nbatch) return false; /* no more batches */
...
void ExecReScanHashJoin(HashJoinState *node) {
  ...
  if (node->hj_HashTable != NULL) {
    if (node->hj_HashTable->nbatch == 1 &&
       ...
       */
      if (HJ_FILL_INNER(node)) ExecHashTableResetMatchFlags(node->hj_HashTable);
