...
static void eval_windowfunction(WindowAggState *winstate,
                                WindowStatePerFunc perfuncstate, Datum *result,
                                bool *isnull);
...
static void initialize_windowaggregate(WindowAggState *winstate,
  ...
  if (peraggstate->initValueIsNull)
    ...
    peraggstate->transValue =
        datumCopy(peraggstate->initValue, peraggstate->transtypeByVal,
...
static void advance_windowaggregate(WindowAggState *winstate,
  ...
  if (peraggstate->transfn.fn_strict) {
    ...
    if (peraggstate->transValueCount == 0 && peraggstate->transValueIsNull) {
      ...
      peraggstate->transValue =
          datumCopy(fcinfo->arg[1], peraggstate->transtypeByVal,
                    peraggstate->transtypeLen);
...
static bool advance_windowaggregate_base(WindowAggState *winstate,
  ...
  if (peraggstate->transValueCount == 1) {
    MemoryContextSwitchTo(oldContext);
    initialize_windowaggregate(
        winstate, &winstate->perfunc[peraggstate->wfuncno], peraggstate);
    return true;
  }
...
static void finalize_windowaggregate(WindowAggState *winstate,
  ...
  if (!peraggstate->resulttypeByVal && !*isnull &&
    ...
    *result = datumCopy(*result, peraggstate->resulttypeByVal,
...
static void eval_windowaggregates(WindowAggState *winstate) {
  ...
  if (numaggs == 0) return; /* nothing to do */
  ...
  while (numaggs_restart < numaggs &&
    ...
    for (i = 0; i < numaggs; i++) {
      ...
      peraggstate = &winstate->peragg[i];
      if (peraggstate->restart) continue;

      wfuncno = peraggstate->wfuncno;
      ok = advance_windowaggregate_base(winstate, &winstate->perfunc[wfuncno],
                                        peraggstate);
      if (!ok) {
  ...
  for (i = 0; i < numaggs; i++) {
    ...
    Assert(peraggstate->aggcontext != winstate->aggcontext ||
           numaggs_restart == 0 || peraggstate->restart);

    if (peraggstate->restart) {
      wfuncno = peraggstate->wfuncno;
      initialize_windowaggregate(winstate, &winstate->perfunc[wfuncno],
                                 peraggstate);
  ...
  for (i = 0; i < numaggs; i++) {
    ...
    finalize_windowaggregate(winstate, &winstate->perfunc[wfuncno], peraggstate,
                             result, isnull);
...
static void eval_windowfunction(WindowAggState *winstate,
  ...
  InitFunctionCallInfoData(
      fcinfo, &(perfuncstate->flinfo), perfuncstate->numArguments,
      perfuncstate->winCollation,
  ...
  *isnull = fcinfo.isnull;
  ...
  if (!perfuncstate->resulttypeByVal && !fcinfo.isnull &&
      !MemoryContextContains(CurrentMemoryContext, DatumGetPointer(*result)))
    *result = datumCopy(*result, perfuncstate->resulttypeByVal,
...
static void begin_partition(WindowAggState *winstate) {
  ...
  if (winstate->numaggs > 0) {
    ...
    agg_winobj->readptr =
        tuplestore_alloc_read_pointer(winstate->buffer, readptr_flags);
...
static void spool_tuples(WindowAggState *winstate, int64 pos) {
  ...
  if (!winstate->buffer) return;           /* just a safety check */
  if (winstate->partition_spooled) return; /* whole partition done already */
   ...
   */
  if (!tuplestore_in_memory(winstate->buffer)) pos = -1;
  ...
  while (winstate->spooled_rows <= pos || pos == -1) {
    ...
    if (node->partNumCols > 0) {
      ...
      if (!execTuplesMatch(winstate->first_part_slot, outerslot,
...
static void release_partition(WindowAggState *winstate) {
  ...
  for (i = 0; i < winstate->numfuncs; i++) {
    ...
    if (perfuncstate->winobj) perfuncstate->winobj->localmem = NULL;
  }
  ...
  if (winstate->buffer) tuplestore_end(winstate->buffer);
...
static bool row_is_in_frame(WindowAggState *winstate, int64 pos,
  ...
  if (frameOptions & FRAMEOPTION_START_CURRENT_ROW) {
    if (frameOptions & FRAMEOPTION_ROWS) {
      ...
      if (pos < winstate->currentpos) return false;
    ...
    if (frameOptions & FRAMEOPTION_ROWS) {
      ...
      if (pos < winstate->currentpos + offset) return false;
    } else if (frameOptions & FRAMEOPTION_RANGE) {
  ...
  if (frameOptions & FRAMEOPTION_END_CURRENT_ROW) {
    ...
    if (frameOptions & FRAMEOPTION_ROWS) {
      ...
      if (frameOptions & FRAMEOPTION_END_VALUE_PRECEDING) offset = -offset;
  ...
  if (frameOptions & FRAMEOPTION_START_UNBOUNDED_PRECEDING) {
    ...
    if (frameOptions & FRAMEOPTION_ROWS) {
      ...
      for (;;) {
        ...
        if (fhprev < winstate->frameheadpos) break;
...
TupleTableSlot *ExecWindowAgg(WindowAggState *winstate) {
  ...
  if (winstate->all_done) return NULL;
  ...
  if (winstate->ss.ps.ps_TupFromTlist) {
    ...
    if (isDone == ExprMultipleResult) return result;
  ...
  if (winstate->all_first) {
    ...
    if (frameOptions & FRAMEOPTION_START_VALUE) {
      ...
      value = ExecEvalExprSwitchContext(winstate->startOffset, econtext,
                                        &isnull, NULL);
      if (isnull)
        ereport(ERROR, (errcode(ERRCODE_NULL_VALUE_NOT_ALLOWED),
                        errmsg("frame starting offset must not be null")));
      /* copy value into query-lifespan context */
      get_typlenbyval(exprType((Node *)winstate->startOffset->expr), &len,
                      &byval);
    ...
    if (frameOptions & FRAMEOPTION_END_VALUE) {
      ...
      /* copy value into query-lifespan context */
      get_typlenbyval(exprType((Node *)winstate->endOffset->expr), &len,
                      &byval);
...
restart:
   ...
   * Evaluate aggregates
   */
  if (winstate->numaggs > 0) eval_windowaggregates(winstate);
  ...
  winstate->ss.ps.ps_TupFromTlist = (isDone == ExprMultipleResult);
  return result;
}
...
WindowAggState *ExecInitWindowAgg(WindowAgg *node, EState *estate, int eflags) {
  ...
  winstate->partcontext = AllocSetContextCreate(
      CurrentMemoryContext, "WindowAgg_Partition", ALLOCSET_DEFAULT_MINSIZE,
  ...
  winstate->aggcontext = AllocSetContextCreate(
      CurrentMemoryContext, "WindowAgg_Aggregates", ALLOCSET_DEFAULT_MINSIZE,
      ALLOCSET_DEFAULT_INITSIZE, ALLOCSET_DEFAULT_MAXSIZE);
  ...
  winstate->ss.ps.targetlist = (List *)ExecInitExpr(
      (Expr *)node->plan.targetlist, (PlanState *)winstate);
  ...
  if (node->partNumCols > 0)
    winstate->partEqfunctions =
        execTuplesMatchPrepare(node->partNumCols, node->partOperators);
  if (node->ordNumCols > 0)
    winstate->ordEqfunctions =
        execTuplesMatchPrepare(node->ordNumCols, node->ordOperators);
   ...
   */
  perfunc =
      (WindowStatePerFunc)palloc0(sizeof(WindowStatePerFuncData) * numfuncs);
  ...
  foreach (l, winstate->funcs) {
    ...
    get_typlenbyval(wfunc->wintype, &perfuncstate->resulttypeLen,
  ...
  winstate->startOffset =
      ExecInitExpr((Expr *)node->startOffset, (PlanState *)winstate);
  winstate->endOffset =
      ExecInitExpr((Expr *)node->endOffset, (PlanState *)winstate);
...
static WindowStatePerAggData *initialize_peragg(WindowAggState *winstate,
  ...
  i = 0;
  foreach (lc, wfunc->args) { inputTypes[i++] = exprType((Node *)lfirst(lc)); }
  ...
  /* Check that aggregate owner has permission to call component fns */
  {
    ...
    if (OidIsValid(invtransfn_oid)) {
      ...
      if (aclresult != ACLCHECK_OK)
        aclcheck_error(aclresult, ACL_KIND_PROC, get_func_name(invtransfn_oid));
  ...
  aggtranstype = resolve_aggregate_transtype(wfunc->winfnoid, aggtranstype,
                                             inputTypes, numArguments);

  /* build expression trees using actual argument & result types */
  build_aggregate_fnexprs(
      inputTypes, numArguments, 0, /* no ordered-set window functions yet */
      peraggstate->numFinalArgs, false, /* no variadic window functions yet */
      aggtranstype, wfunc->wintype, wfunc->inputcollid, transfn_oid,
      invtransfn_oid, finalfn_oid, &transfnexpr, &invtransfnexpr, &finalfnexpr);
  ...
  get_typlenbyval(wfunc->wintype, &peraggstate->resulttypeLen,
                  &peraggstate->resulttypeByVal);
  get_typlenbyval(aggtranstype, &peraggstate->transtypeLen,
  ...
  if (peraggstate->transfn.fn_strict && peraggstate->initValueIsNull) {
    if (numArguments < 1 || !IsBinaryCoercible(inputTypes[0], aggtranstype))
      ereport(ERROR, (errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
                      errmsg(
                          "aggregate %u needs to have compatible input type "
                          "and transition type",
                          wfunc->winfnoid)));
  }
  ...
  if (OidIsValid(invtransfn_oid) &&
    ...
    ereport(ERROR, (errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
                    errmsg(
                        "strictness of aggregate's forward and inverse "
                        "transition functions must match")));
  ...
  if (OidIsValid(invtransfn_oid))
    peraggstate->aggcontext = AllocSetContextCreate(
        CurrentMemoryContext, "WindowAgg_AggregatePrivate",
...
static bool are_peers(WindowAggState *winstate, TupleTableSlot *slot1,
  ...
  /* If no ORDER BY, all rows are peers with each other */
  if (node->ordNumCols == 0) return true;

  return execTuplesMatch(slot1, slot2, node->ordNumCols, node->ordColIdx,
                         winstate->ordEqfunctions,
                         winstate->tmpcontext->ecxt_per_tuple_memory);
}
...
static bool window_gettupleslot(WindowObject winobj, int64 pos,
  ...
  if (pos >= winstate->spooled_rows) return false;
  ...
  if (winobj->seekpos < pos - 1) {
    if (!tuplestore_skiptuples(winstate->buffer, pos - 1 - winobj->seekpos,
                               true))
      elog(ERROR, "unexpected end of tuplestore");
    winobj->seekpos = pos - 1;
  } else if (winobj->seekpos > pos + 1) {
    if (!tuplestore_skiptuples(winstate->buffer, winobj->seekpos - (pos + 1),
                               false))
      elog(ERROR, "unexpected end of tuplestore");
    winobj->seekpos = pos + 1;
 ...
 */
Datum WinGetFuncArgInPartition(WindowObject winobj, int argno, int relpos,
                               int seektype, bool set_mark, bool *isnull,
                               bool *isout) {
  WindowAggState *winstate;
  ExprContext *econtext;
  TupleTableSlot *slot;
 ...
 * (plus setting *isout true, if isout isn't NULL).
 */
Datum WinGetFuncArgInFrame(WindowObject winobj, int argno, int relpos,
                           int seektype, bool set_mark, bool *isnull,
                           bool *isout) {
  WindowAggState *winstate;
  ExprContext *econtext;
  ...
  gottuple = window_gettupleslot(winobj, abs_pos, slot);
  if (gottuple) gottuple = row_is_in_frame(winstate, abs_pos, slot);
...
Datum WinGetFuncArgCurrent(WindowObject winobj, int argno, bool *isnull) {
  ...
  econtext->ecxt_outertuple = winstate->ss.ss_ScanTupleSlot;
  return ExecEvalExpr((ExprState *)list_nth(winobj->argstates, argno), econtext,
                      isnull, NULL);
}
