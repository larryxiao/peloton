 ...
 */
bool execCurrentOf(CurrentOfExpr *cexpr, ExprContext *econtext, Oid table_oid,
  ...
  if (queryDesc->estate->es_rowMarks) {
    ...
    foreach (lc, queryDesc->estate->es_rowMarks) {
      ...
      if (thiserm->relid == table_oid) {
        if (erm)
          ereport(ERROR, (errcode(ERRCODE_INVALID_CURSOR_STATE),
                          errmsg(
                              "cursor \"%s\" has multiple FOR UPDATE/SHARE "
    ...
    if (erm == NULL)
      ereport(ERROR, (errcode(ERRCODE_INVALID_CURSOR_STATE),
                      errmsg(
                          "cursor \"%s\" does not have a FOR UPDATE/SHARE "
    ...
    if (TupIsNull(scanstate->ss_ScanTupleSlot)) return false;

    /* Use slot_getattr to catch any possible mistakes */
    tuple_tableoid = DatumGetObjectId(slot_getattr(
        scanstate->ss_ScanTupleSlot, TableOidAttributeNumber, &lisnull));
    Assert(!lisnull);
    tuple_tid = (ItemPointer)DatumGetPointer(slot_getattr(
        scanstate->ss_ScanTupleSlot, SelfItemPointerAttributeNumber, &lisnull));
    Assert(!lisnull);
      ...
      if (prm->ptype != REFCURSOROID)
        ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH),
                        errmsg(
                            ...
                            paramId, format_type_be(prm->ptype),
  ...
  switch (nodeTag(node)) {
    ...
    case T_AppendState: {
      ...
      for (i = 0; i < astate->as_nplans; i++) {
        ...
        if (!elem) continue;
        if (result) return NULL; /* multiple matches */
        result = elem;
      }
      return result;
    }
