...
static TupleTableSlot *FunctionNext(FunctionScanState *node) {
  ...
  if (node->simple) {
     ...
     */
    if (tstore == NULL) {
      node->funcstates[0].tstore = tstore = ExecMakeTableFunctionResult(
          node->funcstates[0].funcexpr, node->ss.ps.ps_ExprContext,
          node->argcontext, node->funcstates[0].tupdesc,
  ...
  for (funcno = 0; funcno < node->nfuncs; funcno++) {
    ...
    if (fs->tstore == NULL) {
      fs->tstore = ExecMakeTableFunctionResult(
          fs->funcexpr, node->ss.ps.ps_ExprContext, node->argcontext,
          fs->tupdesc, node->eflags & EXEC_FLAG_BACKWARD);
    ...
    if (fs->rowcount != -1 && fs->rowcount < oldpos)
      ...
      (void)tuplestore_gettupleslot(
          fs->tstore, ScanDirectionIsForward(direction), false, fs->func_slot);
 ...
 * ----------------------------------------------------------------
 */
FunctionScanState *ExecInitFunctionScan(FunctionScan *node, EState *estate,
                                        int eflags) {
  ...
  scanstate->ss.ps.targetlist = (List *)ExecInitExpr(
      (Expr *)node->scan.plan.targetlist, (PlanState *)scanstate);
  scanstate->ss.ps.qual = (List *)ExecInitExpr((Expr *)node->scan.plan.qual,
                                               (PlanState *)scanstate);

  scanstate->funcstates = static_cast<FunctionScanPerFuncState *>(
      palloc(nfuncs * sizeof(FunctionScanPerFuncState)));
  ...
  foreach (lc, node->functions) {
    ...
    if (functypclass == TYPEFUNC_COMPOSITE) {
      ...
      TupleDescInitEntry(tupdesc, (AttrNumber)1,
                         ...
                         funcrettype, -1, 0);
      TupleDescInitEntryCollation(tupdesc, (AttrNumber)1,
                                  exprCollation(funcexpr));
    } else if (functypclass == TYPEFUNC_RECORD) {
      tupdesc =
          BuildDescFromLists(rtfunc->funccolnames, rtfunc->funccoltypes,
                             rtfunc->funccoltypmods, rtfunc->funccolcollations);
  ...
  if (scanstate->simple) {
    ...
    if (node->funcordinality) {
      TupleDescInitEntry(scan_tupdesc, ++attno,
                         NULL, /* don't care about the name here */
                         INT8OID, -1, 0);
    }
   ...
   */
  scanstate->argcontext =
      AllocSetContextCreate(CurrentMemoryContext, "Table function arguments",
