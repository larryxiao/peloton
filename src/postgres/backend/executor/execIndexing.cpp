...
static bool check_exclusion_or_unique_constraint(
    Relation heap, Relation index, IndexInfo *indexInfo, ItemPointer tupleid,
    Datum *values, bool *isnull, EState *estate, bool newIndex,
    CEOUC_WAIT_MODE waitMode, bool errorOK, ItemPointer conflictTid);

static bool index_recheck_constraint(Relation index, Oid *constr_procs,
                                     Datum *existing_values,
                                     bool *existing_isnull, Datum *new_values);
...
void ExecOpenIndices(ResultRelInfo *resultRelInfo, bool speculative) {
  ...
  foreach (l, indexoidlist) {
     ...
     */
    if (speculative && ii->ii_Unique) BuildSpeculativeIndexInfo(indexDesc, ii);
...
void ExecCloseIndices(ResultRelInfo *resultRelInfo) {
  ...
  for (i = 0; i < numIndices; i++) {
    if (indexDescs[i] == NULL) continue; /* shouldn't happen? */
 ...
 * ----------------------------------------------------------------
 */
List *ExecInsertIndexTuples(TupleTableSlot *slot, ItemPointer tupleid,
  ...
  for (i = 0; i < numIndices; i++) {
    ...
    if (!indexInfo->ii_ReadyForInserts) continue;
    ...
    if (indexInfo->ii_ExclusionOps != NULL) {
      ...
      satisfiesConstraint = check_exclusion_or_unique_constraint(
          heapRelation, indexRelation, indexInfo, tupleid, values, isnull,
          estate, false, waitMode, violationOK, NULL);
    }
...
bool ExecCheckIndexConstraints(TupleTableSlot *slot, EState *estate,
  ...
  for (i = 0; i < numIndices; i++) {
    ...
    if (!indexInfo->ii_Unique && !indexInfo->ii_ExclusionOps) continue;
    ...
    if (!indexRelation->rd_index->indimmediate)
      ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                      errmsg(
                          "ON CONFLICT does not support deferred unique "
    ...
    satisfiesConstraint = check_exclusion_or_unique_constraint(
        heapRelation, indexRelation, indexInfo, &invalidItemPtr, values, isnull,
        estate, false, CEOUC_WAIT, true, conflictTid);
    if (!satisfiesConstraint) return false;
  }
...
static bool check_exclusion_or_unique_constraint(
  ...
  for (i = 0; i < index_natts; i++) {
    ScanKeyEntryInitialize(&scankeys[i], 0, i + 1, constr_strats[i], InvalidOid,
                           index_collations[i], constr_procs[i], values[i]);
  }
...
retry:
  ...
  while ((tup = index_getnext(index_scan, ForwardScanDirection)) != NULL) {
    ...
    if (violationOK) {
      conflict = true;
      if (conflictTid) *conflictTid = tup->t_self;
      break;
    }

    error_new = BuildIndexValueDescription(index, values, isnull);
    error_existing =
        BuildIndexValueDescription(index, existing_values, existing_isnull);
    if (newIndex)
      ereport(ERROR,
      ...
      ereport(
          ...
          (errcode(ERRCODE_EXCLUSION_VIOLATION),
           errmsg("conflicting key value violates exclusion constraint \"%s\"",
                  RelationGetRelationName(index)),
           error_new && error_existing
               ? errdetail("Key %s conflicts with existing key %s.", error_new,
                           error_existing)
 ...
 */
void check_exclusion_constraint(Relation heap, Relation index,
                                IndexInfo *indexInfo, ItemPointer tupleid,
                                Datum *values, bool *isnull, EState *estate,
                                bool newIndex) {
...
static bool index_recheck_constraint(Relation index, Oid *constr_procs,
  ...
  for (i = 0; i < index_natts; i++) {
    ...
    if (existing_isnull[i]) return false;

    if (!DatumGetBool(OidFunctionCall2Coll(constr_procs[i],
                                           index->rd_indcollation[i],
                                           existing_values[i], new_values[i])))
      return false;
  }
