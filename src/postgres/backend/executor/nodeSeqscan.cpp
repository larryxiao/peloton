...
static void InitScanRelation(SeqScanState *node, EState *estate, int eflags) {
  ...
  currentRelation = ExecOpenScanRelation(
      estate, ((SeqScan *)node->ps.plan)->scanrelid, eflags);

  /* initialize a heapscan */
  currentScanDesc =
      heap_beginscan(currentRelation, estate->es_snapshot, 0, NULL);
...
SeqScanState *ExecInitSeqScan(SeqScan *node, EState *estate, int eflags) {
   ...
   */
  scanstate->ps.targetlist = (List *)ExecInitExpr((Expr *)node->plan.targetlist,
                                                  (PlanState *)scanstate);
  scanstate->ps.qual =
