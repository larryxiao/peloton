...
TupleTableSlot * /* result tuple from subplan */
   ...
   */
  eof_tuplestore =
      (tuplestorestate == NULL) || tuplestore_ateof(tuplestorestate);
  ...
  if (!eof_tuplestore) {
    if (tuplestore_gettupleslot(tuplestorestate, forward, false, slot))
      return slot;
    if (forward) eof_tuplestore = true;
  }
  ...
  if (eof_tuplestore && !node->eof_underlying) {
     ...
     */
    if (tuplestorestate) tuplestore_puttupleslot(tuplestorestate, outerslot);
...
MaterialState *ExecInitMaterial(Material *node, EState *estate, int eflags) {
   ...
   */
  if (eflags & EXEC_FLAG_BACKWARD) matstate->eflags |= EXEC_FLAG_REWIND;
...
void ExecEndMaterial(MaterialState *node) {
   ...
   */
  if (node->tuplestorestate != NULL) tuplestore_end(node->tuplestorestate);
...
void ExecMaterialMarkPos(MaterialState *node) {
   ...
   */
  if (!node->tuplestorestate) return;
