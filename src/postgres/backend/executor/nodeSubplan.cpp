...
static Datum ExecSubPlan(SubPlanState *node, ExprContext *econtext,
                         bool *isNull, ExprDoneCond *isDone);
static Datum ExecAlternativeSubPlan(AlternativeSubPlanState *node,
                                    ExprContext *econtext, bool *isNull,
                                    ExprDoneCond *isDone);
static Datum ExecHashSubPlan(SubPlanState *node, ExprContext *econtext,
                             bool *isNull);
static Datum ExecScanSubPlan(SubPlanState *node, ExprContext *econtext,
                             bool *isNull);
...
static Datum ExecHashSubPlan(SubPlanState *node, ExprContext *econtext,
   ...
   */
  *isNull = false;
  if (!node->havehashrows && !node->havenullrows) return BoolGetDatum(false);
  ...
  if (slotNoNulls(slot)) {
    if (node->havehashrows &&
        FindTupleHashEntry(node->hashtable, slot, node->cur_eq_funcs,
...
static Datum ExecScanSubPlan(SubPlanState *node, ExprContext *econtext,
  ...
  if (subLinkType == ARRAY_SUBLINK)
    astate =
        initArrayResultAny(subplan->firstColType, CurrentMemoryContext, true);
  ...
  *isNull = false;
  ...
  for (slot = ExecProcNode(planstate); !TupIsNull(slot);
    ...
    rowresult =
        ExecEvalExprSwitchContext(node->testexpr, econtext, &rownull, NULL);
...
static void buildSubPlanHash(SubPlanState *node, ExprContext *econtext) {
  ...
  node->hashtable = BuildTupleHashTable(
      ncols, node->keyColIdx, node->tab_eq_funcs, node->tab_hash_funcs,
      nbuckets, sizeof(TupleHashEntryData), node->hashtablecxt,
      node->hashtempcxt);
...
static bool slotAllNulls(TupleTableSlot *slot) {
  ...
  for (i = 1; i <= ncols; i++) {
    if (!slot_attisnull(slot, i)) return false;
  }
  return true;
}
...
SubPlanState *ExecInitSubPlan(SubPlan *subplan, PlanState *parent) {
  ...
  sstate->planstate =
      (PlanState *)list_nth(estate->es_subplanstates, subplan->plan_id - 1);
  ...
  if (subplan->useHashTable) {
    ...
    sstate->hashtablecxt = AllocSetContextCreate(
        CurrentMemoryContext, "Subplan HashTable Context",
        ALLOCSET_DEFAULT_MINSIZE, ALLOCSET_DEFAULT_INITSIZE,
        ALLOCSET_DEFAULT_MAXSIZE);
    /* and a small one for the hash tables to use as temp storage */
    sstate->hashtempcxt = AllocSetContextCreate(
        CurrentMemoryContext, "Subplan HashTable Temp Context",
    ...
    for (i = 0; i < ncols; i++) sstate->keyColIdx[i] = i + 1;
    ...
    foreach (l, oplist) {
      ...
      if (!get_compatible_hash_operators(opexpr->opno, NULL, &rhs_eq_oper))
        elog(ERROR,
             "could not find compatible hash operator___ for operator___ %u",
             opexpr->opno);
    ...
    ExecSetSlotDescriptor(slot, tupDesc);
    sstate->projRight =
        ExecBuildProjectionInfo(righttlist, sstate->innerecontext, slot, NULL);
  }
...
void ExecReScanSetParamPlan(SubPlanState *node, PlanState *parent) {
  ...
  foreach (l, subplan->setParam) {
    ...
    if (subplan->subLinkType != CTE_SUBLINK) prm->execPlan = node;
...
static Datum ExecAlternativeSubPlan(AlternativeSubPlanState *node,
  ...
  SubPlanState *activesp =
      (SubPlanState *)list_nth(node->subplans, node->active);
