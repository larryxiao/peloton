 ...
 * --------------------------------
 */
void ExecResetTupleTable(List *tupleTable, /* tuple table */
                         bool shouldFree)  /* true if we should free memory */
{
  ...
  if (shouldFree) list_free(tupleTable);
}
                           ...
                           TupleDesc tupdesc)    /* new___ tuple descriptor */
{
   ...
   */
  slot->tts_values = (Datum *)MemoryContextAlloc(
      slot->tts_mcxt, tupdesc->natts * sizeof(Datum));
  slot->tts_isnull =
      (bool *)MemoryContextAlloc(slot->tts_mcxt, tupdesc->natts * sizeof(bool));
}
 ...
 * --------------------------------
 */
TupleTableSlot *ExecStoreTuple(HeapTuple tuple, TupleTableSlot *slot,
   ...
   */
  if (slot->tts_shouldFree) heap_freetuple(slot->tts_tuple);
  if (slot->tts_shouldFreeMin) heap_free_minimal_tuple(slot->tts_mintuple);
 ...
 * --------------------------------
 */
TupleTableSlot *ExecStoreMinimalTuple(MinimalTuple mtup, TupleTableSlot *slot,
                                      bool shouldFree) {
  /*
   * sanity checks
   */
  ...
  slot->tts_minhdr.t_data =
      (HeapTupleHeader)((char *)mtup - MINIMAL_TUPLE_OFFSET);
...
HeapTuple ExecCopySlotTuple(TupleTableSlot *slot) {
   ...
   */
  if (TTS_HAS_PHYSICAL_TUPLE(slot)) return heap_copytuple(slot->tts_tuple);
...
MinimalTuple ExecCopySlotMinimalTuple(TupleTableSlot *slot) {
   ...
   */
  if (slot->tts_mintuple) return heap_copy_minimal_tuple(slot->tts_mintuple);
  if (slot->tts_tuple) return minimal_tuple_from_heap_tuple(slot->tts_tuple);
...
HeapTuple ExecFetchSlotTuple(TupleTableSlot *slot) {
   ...
   */
  if (TTS_HAS_PHYSICAL_TUPLE(slot)) return slot->tts_tuple;
...
MinimalTuple ExecFetchSlotMinimalTuple(TupleTableSlot *slot) {
   ...
   */
  if (slot->tts_mintuple) return slot->tts_mintuple;
...
HeapTuple ExecMaterializeSlot(TupleTableSlot *slot) {
   ...
   * nothing to do.
   */
  if (slot->tts_tuple && slot->tts_shouldFree) return slot->tts_tuple;
   ...
   */
  if (!slot->tts_shouldFreeMin) slot->tts_mintuple = NULL;
 ...
 * --------------------------------
 */
TupleTableSlot *ExecMakeSlotContentsReadOnly(TupleTableSlot *slot) {
  /*
   * sanity checks
   */
  Assert(slot != NULL);
  ...
  if (slot->tts_tuple == NULL) {
    ...
    for (attnum = 0; attnum < slot->tts_nvalid; attnum++) {
      slot->tts_values[attnum] = MakeExpandedObjectReadOnly(
          slot->tts_values[attnum], slot->tts_isnull[attnum],
          att[attnum]->attlen);
    }
  }
...
static TupleDesc ExecTypeFromTLInternal(List *targetList, bool hasoid,
  ...
  foreach (l, targetList) {
    ...
    if (skipjunk && tle->resjunk) continue;
...
TupleDesc ExecTypeFromExprList(List *exprList) {
  ...
  foreach (lc, exprList) {
    ...
    TupleDescInitEntry(typeInfo, cur_resno, NULL, exprType(e), exprTypmod(e),
                       0);
...
HeapTuple BuildTupleFromCStrings(AttInMetadata *attinmeta, char **values) {
  ...
  for (i = 0; i < natts; i++) {
    if (!tupdesc->attrs[i]->attisdropped) {
      ...
      dvalues[i] = InputFunctionCall(&attinmeta->attinfuncs[i], values[i],
...
Datum HeapTupleHeaderGetDatum(HeapTupleHeader tuple) {
  ...
  if (!HeapTupleHeaderHasExternal(tuple)) return PointerGetDatum(tuple);
  ...
  result = toast_flatten_tuple_to_datum(
      tuple, HeapTupleHeaderGetDatumLength(tuple), tupDesc);
