...
TupleTableSlot *ExecGroup(GroupState *node) {
  ...
  for (;;) {
    ...
    for (;;) {
       ...
       */
      if (!execTuplesMatch(firsttupleslot, outerslot, numCols, grpColIdx,
                           node->eqfunctions, econtext->ecxt_per_tuple_memory))
        break;
    }
...
GroupState *ExecInitGroup(Group *node, EState *estate, int eflags) {
   ...
   */
  grpstate->ss.ps.targetlist = (List *)ExecInitExpr(
      (Expr *)node->plan.targetlist, (PlanState *)grpstate);
  grpstate->ss.ps.qual =
      (List *)ExecInitExpr((Expr *)node->plan.qual, (PlanState *)grpstate);
