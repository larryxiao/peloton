...
CustomScanState *ExecInitCustomScan(CustomScan *cscan, EState *estate,
                                    int eflags) {
  CustomScanState *css;
  ...
  css->ss.ps.targetlist = (List *)ExecInitExpr(
      (Expr *)cscan->scan.plan.targetlist, (PlanState *)css);
  css->ss.ps.qual =
      (List *)ExecInitExpr((Expr *)cscan->scan.plan.qual, (PlanState *)css);
