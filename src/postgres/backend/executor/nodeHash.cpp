...
HashState *ExecInitHash(Hash *node, EState *estate, int eflags) {
   ...
   */
  hashstate->ps.targetlist = (List *)ExecInitExpr((Expr *)node->plan.targetlist,
                                                  (PlanState *)hashstate);
  hashstate->ps.qual =
      (List *)ExecInitExpr((Expr *)node->plan.qual, (PlanState *)hashstate);
...
HashJoinTable ExecHashTableCreate(Hash *node, List *hashOperators,
  ...
  hashtable->hashCxt = AllocSetContextCreate(
      CurrentMemoryContext, "HashTableContext", ALLOCSET_DEFAULT_MINSIZE,
      ALLOCSET_DEFAULT_INITSIZE, ALLOCSET_DEFAULT_MAXSIZE);

  hashtable->batchCxt = AllocSetContextCreate(
      hashtable->hashCxt, "HashBatchContext", ALLOCSET_DEFAULT_MINSIZE,
      ALLOCSET_DEFAULT_INITSIZE, ALLOCSET_DEFAULT_MAXSIZE);
   ...
   */
  if (nbatch > 1) ExecHashBuildSkewHash(hashtable, node, num_skew_mcvs);
...
void ExecChooseHashTableSize(double ntuples, int tupwidth, bool useskew,
   ...
   */
  tupsize = HJTUPLE_OVERHEAD + MAXALIGN(SizeofMinimalTupleHeader) +
            MAXALIGN(tupwidth);
  ...
  if (useskew) {
     ...
     */
    *num_skew_mcvs =
        skew_table_bytes / (tupsize + (8 * sizeof(HashSkewBucket *)) +
                            sizeof(int) + SKEW_BUCKET_OVERHEAD);
    if (*num_skew_mcvs > 0) hash_table_bytes -= skew_table_bytes;
  } else
    *num_skew_mcvs = 0;
  ...
  if (inner_rel_bytes + bucket_bytes > hash_table_bytes) {
    ...
    minbatch = (int)dbatch;
    nbatch = 2;
    while (nbatch < minbatch) nbatch <<= 1;
  }
...
static void ExecHashIncreaseNumBatches(HashJoinTable hashtable) {
  ...
  if (!hashtable->growEnabled) return;

  /* safety check to avoid overflow */
  if (oldnbatch > Min(INT_MAX / 2, MaxAllocSize / (sizeof(void *) * 2))) return;
  ...
  printf("Increasing nbatch to %d because space = %lu\n", nbatch,
         (unsigned long)hashtable->spaceUsed);
#endif
  ...
  if (hashtable->innerBatchFile == NULL) {
    ...
    hashtable->innerBatchFile = (BufFile **)repalloc(
        hashtable->innerBatchFile, nbatch * sizeof(BufFile *));
    hashtable->outerBatchFile = (BufFile **)repalloc(
        hashtable->outerBatchFile, nbatch * sizeof(BufFile *));
  ...
  if (hashtable->nbuckets_optimal != hashtable->nbuckets) {
    ...
    hashtable->buckets = static_cast<HashJoinTupleData **>(repalloc(
  ...
  while (oldchunks != NULL) {
    ...
    while (idx < oldchunks->used) {
      ...
      ninmemory++;
      ExecHashGetBucketAndBatch(hashtable, hashTuple->hashvalue, &bucketno,
                                &batchno);
      ...
      if (batchno == curbatch) {
        ...
        ExecHashJoinSaveTuple(HJTUPLE_MINTUPLE(hashTuple), hashTuple->hashvalue,
...
static void ExecHashIncreaseNumBuckets(HashJoinTable hashtable) {
  ...
  if (hashtable->nbuckets >= hashtable->nbuckets_optimal) return;
   ...
   */
  hashtable->buckets = (HashJoinTuple *)repalloc(
      hashtable->buckets, hashtable->nbuckets * sizeof(HashJoinTuple));
 ...
 */
void ExecHashTableInsert(HashJoinTable hashtable, TupleTableSlot *slot,
                         uint32 hashvalue) {
  ...
  if (batchno == hashtable->curbatch) {
     ...
     */
    if ((hashtable->nbatch == 1) && (hashtable->nbuckets_optimal <=
                                     INT_MAX / 2) && /* overflow protection */
 ...
 */
bool ExecHashGetHashValue(HashJoinTable hashtable, ExprContext *econtext,
                          List *hashkeys, bool outer_tuple, bool keep_nulls,
                          uint32 *hashvalue) {
  uint32 hashkey = 0;
 ...
 */
void ExecHashGetBucketAndBatch(HashJoinTable hashtable, uint32 hashvalue,
...
static void ExecHashBuildSkewHash(HashJoinTable hashtable, Hash *node,
  ...
  if (get_attstatsslot(statsTuple, node->skewColType, node->skewColTypmod,
    ...
    if (mcvsToUse > nvalues) mcvsToUse = nvalues;
     ...
     */
    frac = 0;
    for (i = 0; i < mcvsToUse; i++) frac += numbers[i];
     ...
     */
    nbuckets = 2;
    while (nbuckets <= mcvsToUse) nbuckets <<= 1;
     ...
     */
    hashtable->skewBucket = (HashSkewBucket **)MemoryContextAllocZero(
        hashtable->batchCxt, nbuckets * sizeof(HashSkewBucket *));
    hashtable->skewBucketNums = (int *)MemoryContextAllocZero(
        hashtable->batchCxt, mcvsToUse * sizeof(int));

    hashtable->spaceUsed +=
        nbuckets * sizeof(HashSkewBucket *) + mcvsToUse * sizeof(int);
    hashtable->spaceUsedSkew +=
        nbuckets * sizeof(HashSkewBucket *) + mcvsToUse * sizeof(int);
    if (hashtable->spaceUsed > hashtable->spacePeak)
      hashtable->spacePeak = hashtable->spaceUsed;
    ...
    for (i = 0; i < mcvsToUse; i++) {
       ...
       */
      if (hashtable->skewBucket[bucket] != NULL) continue;

      /* Okay, create a new___ skew bucket for this hashvalue. */
      hashtable->skewBucket[bucket] = (HashSkewBucket *)MemoryContextAlloc(
          hashtable->batchCxt, sizeof(HashSkewBucket));
      hashtable->skewBucket[bucket]->hashvalue = hashvalue;
...
int ExecHashGetSkewBucket(HashJoinTable hashtable, uint32 hashvalue) {
   ...
   */
  if (!hashtable->skewEnabled) return INVALID_SKEW_BUCKET_NO;
   ...
   */
  if (hashtable->skewBucket[bucket] != NULL) return bucket;
...
static void ExecHashSkewTableInsert(HashJoinTable hashtable,
  ...
  hashTupleSize = HJTUPLE_OVERHEAD + tuple->t_len;
  hashTuple =
      (HashJoinTuple)MemoryContextAlloc(hashtable->batchCxt, hashTupleSize);
  hashTuple->hashvalue = hashvalue;
  memcpy(HJTUPLE_MINTUPLE(hashTuple), tuple, tuple->t_len);
  HeapTupleHeaderClearMatch(HJTUPLE_MINTUPLE(hashTuple));
...
static void *dense_alloc(HashJoinTable hashtable, Size size) {
  ...
  if (size > HASH_CHUNK_THRESHOLD) {
    ...
    newChunk = (HashMemoryChunk)MemoryContextAlloc(
        hashtable->batchCxt, offsetof(HashMemoryChunkData, data) + size);
