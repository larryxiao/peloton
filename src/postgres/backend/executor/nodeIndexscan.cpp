...
static TupleTableSlot *IndexNextWithReorder(IndexScanState *node) {
  ...
  next_indextuple:
     ...
     * queue already.
     */
    if (!was_exact || (topmost &&
                       cmp_orderbyvals(lastfetched_vals, lastfetched_nulls,
                                       topmost->orderbyvals,
                                       topmost->orderbynulls, node) > 0)) {
...
static void EvalOrderByExpressions(IndexScanState *node,
  ...
  foreach (l, node->indexorderbyorig) {
    ...
    node->iss_OrderByValues[i] =
        ExecEvalExpr(orderby, econtext, &node->iss_OrderByNulls[i], NULL);
    i++;
  }
...
static int reorderqueue_cmp(const pairingheap_node *a,
  ...
  return -cmp_orderbyvals(rta->orderbyvals, rta->orderbynulls, rtb->orderbyvals,
                          rtb->orderbynulls, node);
}
...
static void reorderqueue_push(IndexScanState *node, HeapTuple tuple,
  ...
  for (i = 0; i < node->iss_NumOrderByKeys; i++) {
    if (!orderbynulls[i])
      rt->orderbyvals[i] =
          datumCopy(orderbyvals[i], node->iss_OrderByTypByVals[i],
...
TupleTableSlot *ExecIndexScan(IndexScanState *node) {
  ...
  if (node->iss_NumOrderByKeys > 0)
    return ExecScan(&node->ss, (ExecScanAccessMtd)IndexNextWithReorder,
...
void ExecReScanIndexScan(IndexScanState *node) {
  ...
  if (node->iss_NumRuntimeKeys != 0) {
    ...
    ResetExprContext(econtext);
    ExecIndexEvalRuntimeKeys(econtext, node->iss_RuntimeKeys,
...
void ExecIndexEvalRuntimeKeys(ExprContext *econtext,
  ...
  for (j = 0; j < numRuntimeKeys; j++) {
     ...
     */
    scanvalue = ExecEvalExpr(key_expr, econtext, &isNull, NULL);
    if (isNull) {
 ...
 */
bool ExecIndexEvalArrayKeys(ExprContext *econtext, IndexArrayKeyInfo *arrayKeys,
                            int numArrayKeys) {
  bool result = true;
  int j;
  MemoryContext oldContext;
  ...
  for (j = 0; j < numArrayKeys; j++) {
     ...
     */
    arraydatum = ExecEvalExpr(array_expr, econtext, &isNull, NULL);
    if (isNull) {
      result = false;
 ...
 */
  ...
  if (node->iss_RuntimeContext) FreeExprContext(node->iss_RuntimeContext, true);
#endif
...
IndexScanState *ExecInitIndexScan(IndexScan *node, EState *estate, int eflags) {
   ...
   */
  indexstate->ss.ps.targetlist = (List *)ExecInitExpr(
      (Expr *)node->scan.plan.targetlist, (PlanState *)indexstate);
  indexstate->ss.ps.qual = (List *)ExecInitExpr((Expr *)node->scan.plan.qual,
                                                (PlanState *)indexstate);
  indexstate->indexqualorig = (List *)ExecInitExpr((Expr *)node->indexqualorig,
                                                   (PlanState *)indexstate);
  indexstate->indexorderbyorig = (List *)ExecInitExpr(
      (Expr *)node->indexorderbyorig, (PlanState *)indexstate);
  ...
  ExecIndexBuildScanKeys(
      (PlanState *)indexstate, indexstate->iss_RelationDesc, node->indexqual,
      false, &indexstate->iss_ScanKeys, &indexstate->iss_NumScanKeys,
      &indexstate->iss_RuntimeKeys, &indexstate->iss_NumRuntimeKeys,
      NULL, /* no ArrayKeys */
      NULL);
   ...
   */
  ExecIndexBuildScanKeys(
      (PlanState *)indexstate, indexstate->iss_RelationDesc, node->indexorderby,
      true, &indexstate->iss_OrderByKeys, &indexstate->iss_NumOrderByKeys,
      &indexstate->iss_RuntimeKeys, &indexstate->iss_NumRuntimeKeys,
      NULL, /* no ArrayKeys */
      NULL);
  ...
  if (indexstate->iss_NumOrderByKeys > 0) {
    ...
    indexstate->iss_SortSupport =
        (SortSupportData *)palloc0(numOrderByKeys * sizeof(SortSupportData));
    ...
    forboth(lco, node->indexorderbyops, lcx, node->indexorderbyorig) {
      ...
      get_typlenbyval(orderbyType, &indexstate->iss_OrderByTypLens[i],
    ...
    indexstate->iss_OrderByValues =
        (Datum *)palloc(numOrderByKeys * sizeof(Datum));
    indexstate->iss_OrderByNulls =
        (bool *)palloc(numOrderByKeys * sizeof(bool));

    /* and initialize the reorder queue */
    indexstate->iss_ReorderQueue =
        pairingheap_allocate(reorderqueue_cmp, indexstate);
  }
   ...
   */
  indexstate->iss_ScanDesc = index_beginscan(
   ...
   */
  if (indexstate->iss_NumRuntimeKeys == 0)
    index_rescan(indexstate->iss_ScanDesc, indexstate->iss_ScanKeys,
                 indexstate->iss_NumScanKeys, indexstate->iss_OrderByKeys,
 ...
 *
 * 1. Simple operator___ with constant comparison value ("indexkey op
 *constant").
 ...
 */
void ExecIndexBuildScanKeys(PlanState *planstate, Relation index, List *quals,
                            bool isorderby, ScanKey *scanKeys, int *numScanKeys,
                            IndexRuntimeKeyInfo **runtimeKeys,
                            int *numRuntimeKeys, IndexArrayKeyInfo **arrayKeys,
                            int *numArrayKeys) {
  ListCell *qual_cell;
  ScanKey scan_keys;
  ...
  array_keys =
      (IndexArrayKeyInfo *)palloc0(n_scan_keys * sizeof(IndexArrayKeyInfo));
  n_array_keys = 0;
  ...
  foreach (qual_cell, quals) {
    ...
    if (IsA(clause, OpExpr)) {
      ...
      first_sub_key =
          (ScanKey)palloc(list_length(rc->opnos) * sizeof(ScanKeyData));
      n_sub_key = 0;
      ...
      while (opnos_cell != NULL) {
        ...
        if (index->rd_rel->relam != BTREE_AM_OID || varattno < 1 ||
            varattno > index->rd_index->indnatts)
        ...
        if (op_strategy != rc->rctype)
          elog(ERROR,
               "RowCompare index qualification contains wrong operator___");

        opfuncid = get_opfamily_proc(opfamily, op_lefttype, op_righttype,
                                     BTORDER_PROC);
