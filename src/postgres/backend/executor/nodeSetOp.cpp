...
static void build_hash_table(SetOpState *setopstate) {
  ...
  setopstate->hashtable = BuildTupleHashTable(
      node->numCols, node->dupColIdx, setopstate->eqfunctions,
      setopstate->hashfunctions, node->numGroups, sizeof(SetOpHashEntryData),
      setopstate->tableContext, setopstate->tempContext);
}
...
static TupleTableSlot *setop_retrieve_direct(SetOpState *setopstate) {
  ...
  while (!setopstate->setop_done) {
     ...
     * slot.
     */
    ExecStoreTuple(setopstate->grp_firstTuple, resultTupleSlot, InvalidBuffer,
                   true);
    ...
    for (;;) {
       ...
       */
      if (!execTuplesMatch(resultTupleSlot, outerslot, node->numCols,
                           node->dupColIdx, setopstate->eqfunctions,
...
static void setop_fill_hash_table(SetOpState *setopstate) {
  ...
  Assert(firstFlag == 0 ||
         (firstFlag == 1 && (node->cmd == SETOPCMD_INTERSECT ||
  ...
  for (;;) {
    ...
    if (flag == firstFlag) {
      ...
      if (isnew) initialize_counts(&entry->pergroup);
      ...
      if (entry) advance_counts(&entry->pergroup, flag);
    }
...
static TupleTableSlot *setop_retrieve_hash_table(SetOpState *setopstate) {
  ...
  while (!setopstate->setop_done) {
    ...
    if (setopstate->numOutput > 0) {
      setopstate->numOutput--;
      return ExecStoreMinimalTuple(entry->shared.firstTuple, resultTupleSlot,
                                   false);
    }
  }
...
SetOpState *ExecInitSetOp(SetOp *node, EState *estate, int eflags) {
  ...
  setopstate->tempContext = AllocSetContextCreate(
      CurrentMemoryContext, "SetOp", ALLOCSET_DEFAULT_MINSIZE,
  ...
  if (node->strategy == SETOP_HASHED)
    setopstate->tableContext = AllocSetContextCreate(
        CurrentMemoryContext, "SetOp hash table", ALLOCSET_DEFAULT_MINSIZE,
        ALLOCSET_DEFAULT_INITSIZE, ALLOCSET_DEFAULT_MAXSIZE);
   ...
   */
  if (node->strategy == SETOP_HASHED) eflags &= ~EXEC_FLAG_REWIND;
  ...
  if (node->strategy == SETOP_HASHED)
    ...
    setopstate->eqfunctions =
        execTuplesMatchPrepare(node->numCols, node->dupOperators);
