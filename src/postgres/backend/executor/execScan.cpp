...
static inline TupleTableSlot *ExecScanFetch(ScanState *node,
  ...
  if (estate->es_epqTuple != NULL) {
    ...
    if (estate->es_epqTupleSet[scanrelid - 1]) {
      ...
      if (estate->es_epqScanDone[scanrelid - 1]) return ExecClearTuple(slot);
      ...
      ExecStoreTuple(estate->es_epqTuple[scanrelid - 1], slot, InvalidBuffer,
                     false);
...
TupleTableSlot *ExecScan(
  ...
  if (node->ps.ps_TupFromTlist) {
    ...
    if (isDone == ExprMultipleResult) return resultSlot;
...
void ExecAssignScanProjectionInfoWithVarno(ScanState *node, Index varno) {
  ...
  if (tlist_matches_tupdesc(&node->ps, scan->plan.targetlist, varno,
...
static bool tlist_matches_tupdesc(PlanState *ps, List *tlist, Index varno,
  ...
  for (attrno = 1; attrno <= numattrs; attrno++) {
    ...
    if (tlist_item == NULL) return false; /* tlist too short */
    var = (Var *)((TargetEntry *)lfirst(tlist_item))->expr;
    if (!var || !IsA(var, Var)) return false; /* tlist item not a Var */
    /* if these Asserts fail, planner messed up */
    Assert(var->varno == varno);
    Assert(var->varlevelsup == 0);
    if (var->varattno != attrno) return false; /* out of order */
