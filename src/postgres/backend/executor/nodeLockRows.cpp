...
lnext:
  ...
  foreach (lc, node->lr_arowMarks) {
    ...
    if (*testTuple != NULL) heap_freetuple(*testTuple);
    *testTuple = NULL;
    ...
    if (erm->rti != erm->prti) {
      ...
      if (isNull) elog(ERROR, "tableoid is NULL");
    ...
    if (erm->relation->rd_rel->relkind == RELKIND_FOREIGN_TABLE) {
      ...
      copyTuple = fdwroutine->RefetchForeignRow(estate, erm, datum, &updated);
      if (copyTuple == NULL) {
    ...
    test = heap_lock_tuple(erm->relation, &tuple, estate->es_output_cid,
                           lockmode, erm->waitPolicy, true, &buffer, &hufd);
  ...
  if (epq_needed) {
    ...
    for (i = 0; i < node->lr_ntables; i++) {
      EvalPlanQualSetTuple(&node->lr_epqstate, i + 1, node->lr_curtuples[i]);
    ...
    foreach (lc, node->lr_arowMarks) {
      ...
      if (!heap_fetch(erm->relation, SnapshotAny, &tuple, &buffer, false, NULL))
...
LockRowsState *ExecInitLockRows(LockRows *node, EState *estate, int eflags) {
  ...
  lrstate->lr_curtuples =
      (HeapTuple *)palloc0(lrstate->lr_ntables * sizeof(HeapTuple));
