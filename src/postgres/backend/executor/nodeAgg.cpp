...
static void initialize_aggregates(AggState *aggstate, AggStatePerAgg peragg,
                                  AggStatePerGroup pergroup, int numReset);
...
static void finalize_aggregate(AggState *aggstate, AggStatePerAgg peraggstate,
                               ...
                               bool *resultIsNull);
static void prepare_projection_slot(AggState *aggstate, TupleTableSlot *slot,
                                    int currentSet);
static void finalize_aggregates(AggState *aggstate, AggStatePerAgg peragg,
...
static void initialize_phase(AggState *aggstate, int newphase) {
  ...
  if (newphase < aggstate->numphases - 1) {
    ...
    aggstate->sort_out =
        tuplesort_begin_heap(tupDesc, sortnode->numCols, sortnode->sortColIdx,
                             sortnode->sortOperators, sortnode->collations,
                             sortnode->nullsFirst, work_mem, false);
  }
 ...
 */
static void initialize_aggregate(AggState *aggstate, AggStatePerAgg peraggstate,
                                 AggStatePerGroup pergroupstate) {
  /*
  ...
  if (peraggstate->numSortCols > 0) {
    ...
    if (peraggstate->numInputs == 1)
      peraggstate->sortstates[aggstate->current_set] = tuplesort_begin_datum(
          peraggstate->evaldesc->attrs[0]->atttypid,
          peraggstate->sortOperators[0], peraggstate->sortCollations[0],
          peraggstate->sortNullsFirst[0], work_mem, false);
    else
      peraggstate->sortstates[aggstate->current_set] = tuplesort_begin_heap(
          peraggstate->evaldesc, peraggstate->numSortCols,
          peraggstate->sortColIdx, peraggstate->sortOperators,
          peraggstate->sortCollations, peraggstate->sortNullsFirst, work_mem,
          false);
  }
...
static void initialize_aggregates(AggState *aggstate, AggStatePerAgg peragg,
  ...
  if (numReset < 1) numReset = numGroupingSets;
 ...
 *
 * The new___ values (and null flags) have been preloaded into argument
 *positions
...
static void advance_aggregates(AggState *aggstate, AggStatePerGroup pergroup) {
  ...
  for (aggno = 0; aggno < numAggs; aggno++) {
    ...
    if (filter) {
      ...
      res = ExecEvalExprSwitchContext(filter, aggstate->tmpcontext, &isnull,
                                      NULL);
      if (isnull || !DatumGetBool(res)) continue;
    }
    ...
    if (peraggstate->numSortCols > 0) {
      ...
      if (peraggstate->transfn.fn_strict) {
        for (i = 0; i < numTransInputs; i++) {
          if (slot->tts_isnull[i]) break;
        }
        if (i < numTransInputs) continue;
      }
      ...
      for (setno = 0; setno < numGroupingSets; setno++) {
        ...
        if (peraggstate->numInputs == 1)
          tuplesort_putdatum(peraggstate->sortstates[setno],
                             slot->tts_values[0], slot->tts_isnull[0]);
        else
  ...
  while (tuplesort_getdatum(peraggstate->sortstates[aggstate->current_set],
    ...
    if (isDistinct && haveOldVal &&
        ((oldIsNull && *isNull) ||
         (!oldIsNull && !*isNull &&
          DatumGetBool(
              FunctionCall2(&peraggstate->equalfns[0], oldVal, *newVal))))) {
...
static void process_ordered_aggregate_multi(AggState *aggstate,
  ...
  while (tuplesort_gettupleslot(peraggstate->sortstates[aggstate->current_set],
    ...
    if (numDistinctCols == 0) MemoryContextReset(workcontext);
...
static void finalize_aggregate(AggState *aggstate, AggStatePerAgg peraggstate,
  ...
  foreach (lc, peraggstate->aggrefstate->aggdirectargs) {
    ...
    fcinfo.arg[i] = ExecEvalExpr(expr, aggstate->ss.ps.ps_ExprContext,
...
static void finalize_aggregates(AggState *aggstate, AggStatePerAgg peragg,
  ...
  for (aggno = 0; aggno < aggstate->numaggs; aggno++) {
    ...
    finalize_aggregate(aggstate, peraggstate, pergroupstate, &aggvalues[aggno],
                       &aggnulls[aggno]);
  }
}
...
static bool find_unaggregated_cols_walker(Node *node, Bitmapset **colnos) {
  ...
  return expression_tree_walker(node,
                                reinterpret_cast<expression_tree_walker_fptr>(
...
static void build_hash_table(AggState *aggstate) {
  ...
  aggstate->hashtable = BuildTupleHashTable(
      node->numCols, node->grpColIdx, aggstate->phase->eqfunctions,
      aggstate->hashfunctions, node->numGroups, entrysize,
      aggstate->aggcontexts[0]->ecxt_per_tuple_memory, tmpmem);
}
...
static List *find_hash_columns(AggState *aggstate) {
  ...
  collist = NIL;
  while ((i = bms_first_member(colnos)) >= 0) collist = lcons_int(i, collist);
  bms_free(colnos);
...
static AggHashEntry lookup_hash_entry(AggState *aggstate,
  ...
  entry =
      (AggHashEntry)LookupTupleHashEntry(aggstate->hashtable, hashslot, &isnew);
...
TupleTableSlot *ExecAgg(AggState *node) {
  ...
  if (!node->agg_done) {
    ...
    switch (node->phase->aggnode->aggstrategy) {
      case AGG_HASHED:
        if (!node->table_filled) agg_fill_hash_table(node);
...
static TupleTableSlot *agg_retrieve_direct(AggState *aggstate) {
  ...
  while (!aggstate->agg_done) {
    ...
    if (aggstate->input_done ||
        (node->aggstrategy == AGG_SORTED && aggstate->projected_set != -1 &&
         aggstate->projected_set < (numGroupingSets - 1) && nextSetSize > 0 &&
         !execTuplesMatch(econtext->ecxt_outertuple,
                          tmpcontext->ecxt_outertuple, nextSetSize,
                          node->grpColIdx, aggstate->phase->eqfunctions,
      ...
      if (aggstate->grp_firstTuple == NULL) {
        ...
        if (!TupIsNull(outerslot)) {
          ...
          if (hasGroupingSets) {
            ...
            if (aggstate->projected_set >= numGroupingSets) continue;
          } else {
            aggstate->agg_done = true;
            /* If we are grouping, we should produce no tuples too */
            if (node->aggstrategy != AGG_PLAIN) return NULL;
          }
        }
      }
...
AggState *ExecInitAgg(Agg *node, EState *estate, int eflags) {
  ...
  aggstate->aggcontexts =
      (ExprContext **)palloc0(sizeof(ExprContext *) * numGroupingSets);
   ...
   * particular order.
   */
  aggstate->ss.ps.targetlist = (List *)ExecInitExpr(
      (Expr *)node->plan.targetlist, (PlanState *)aggstate);
  aggstate->ss.ps.qual =
      (List *)ExecInitExpr((Expr *)node->plan.qual, (PlanState *)aggstate);
   ...
   */
  if (node->aggstrategy == AGG_HASHED) eflags &= ~EXEC_FLAG_REWIND;
  ...
  aggstate->phases = static_cast<AggStatePerPhase>(
      palloc0(numPhases * sizeof(AggStatePerPhaseData)));
  ...
  for (phase = 0; phase < numPhases; ++phase) {
    ...
    if (num_sets) {
      phasedata->gset_lengths =
          static_cast<int *>(palloc(num_sets * sizeof(int)));
      phasedata->grouped_cols =
          static_cast<Bitmapset **>(palloc(num_sets * sizeof(Bitmapset *)));
      ...
      all_grouped_cols =
          bms_add_members(all_grouped_cols, phasedata->grouped_cols[0]);
    } else {
      Assert(phase == 0);
    ...
    if (aggnode->aggstrategy == AGG_SORTED) {
      ...
      phasedata->eqfunctions =
          execTuplesMatchPrepare(aggnode->numCols, aggnode->grpOperators);
    }
  ...
  foreach (l, aggstate->aggs) {
    ...
    aggtranstype = resolve_aggregate_transtype(
        aggref->aggfnoid, aggform->aggtranstype, inputTypes, numArguments);

    /* build expression trees using actual argument & result types */
    build_aggregate_fnexprs(
        inputTypes, numArguments, numDirectArgs, peraggstate->numFinalArgs,
        aggref->aggvariadic, aggtranstype, aggref->aggtype, aggref->inputcollid,
        transfn_oid, InvalidOid, /* invtrans is not needed here */
        finalfn_oid, &transfnexpr, NULL, &finalfnexpr);
    ...
    get_typlenbyval(aggref->aggtype, &peraggstate->resulttypeLen,
     ...
     */
    textInitVal =
        SysCacheGetAttr(AGGFNOID, aggTuple, Anum_pg_aggregate_agginitval,
    ...
    peraggstate->evalproj = ExecBuildProjectionInfo(
        aggrefstate->args, aggstate->tmpcontext, peraggstate->evalslot, NULL);
    ...
    if (numSortCols > 0) {
      ...
      if (numInputs == 1) {
        get_typlenbyval(inputTypes[numDirectArgs], &peraggstate->inputtypeLen,
...
void ExecEndAgg(AggState *node) {
  ...
  if (node->sort_in) tuplesort_end(node->sort_in);
  if (node->sort_out) tuplesort_end(node->sort_out);
...
Aggref *AggGetAggref(FunctionCallInfo fcinfo) {
  if (fcinfo->context && IsA(fcinfo->context, AggState)) {
    ...
    if (curperagg) return curperagg->aggref;
  }
  return NULL;
}
...
void AggRegisterCallback(FunctionCallInfo fcinfo,
                         ExprContextCallbackFunction func, Datum arg) {
  if (fcinfo->context && IsA(fcinfo->context, AggState)) {
    AggState *aggstate = (AggState *)fcinfo->context;
    ExprContext *cxt = aggstate->aggcontexts[aggstate->current_set];
