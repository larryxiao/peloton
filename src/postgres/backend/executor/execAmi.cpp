...
void ExecReScan(PlanState *node) {
  ...
  if (node->instrument) InstrEndLoop(node->instrument);
  ...
  if (node->ps_ExprContext) ReScanExprContext(node->ps_ExprContext);
  ...
  switch (nodeTag(node)) {
    ...
    case T_Append: {
      ...
      foreach (l, ((Append *)node)->appendplans) {
        if (!ExecSupportsBackwardScan((Plan *)lfirst(l))) return false;
      }
 ...
 */
static bool TargetListSupportsBackwardScan(List *targetlist) {
  if (expression_returns_set((Node *)targetlist)) return false;
  return true;
}
