...
static Datum ExecEvalArrayRef(ArrayRefExprState *astate, ExprContext *econtext,
                              bool *isNull, ExprDoneCond *isDone);
static bool isAssignmentIndirectionExpr(ExprState *exprstate);
static Datum ExecEvalAggref(AggrefExprState *aggref, ExprContext *econtext,
                            bool *isNull, ExprDoneCond *isDone);
...
static Datum ExecEvalCase(CaseExprState *caseExpr, ExprContext *econtext,
                          bool *isNull, ExprDoneCond *isDone);
static Datum ExecEvalCaseTestExpr(ExprState *exprstate, ExprContext *econtext,
                                  bool *isNull, ExprDoneCond *isDone);
static Datum ExecEvalArray(ArrayExprState *astate, ExprContext *econtext,
                           bool *isNull, ExprDoneCond *isDone);
static Datum ExecEvalRow(RowExprState *rstate, ExprContext *econtext,
                         bool *isNull, ExprDoneCond *isDone);
...
static Datum ExecEvalCoalesce(CoalesceExprState *coalesceExpr,
                              ExprContext *econtext, bool *isNull,
                              ExprDoneCond *isDone);
static Datum ExecEvalMinMax(MinMaxExprState *minmaxExpr, ExprContext *econtext,
                            bool *isNull, ExprDoneCond *isDone);
static Datum ExecEvalXml(XmlExprState *xmlExpr, ExprContext *econtext,
                         bool *isNull, ExprDoneCond *isDone);
static Datum ExecEvalNullIf(FuncExprState *nullIfExpr, ExprContext *econtext,
                            bool *isNull, ExprDoneCond *isDone);
static Datum ExecEvalNullTest(NullTestState *nstate, ExprContext *econtext,
                              bool *isNull, ExprDoneCond *isDone);
...
static Datum ExecEvalFieldSelect(FieldSelectState *fstate,
                                 ExprContext *econtext, bool *isNull,
                                 ExprDoneCond *isDone);
static Datum ExecEvalFieldStore(FieldStoreState *fstate, ExprContext *econtext,
                                bool *isNull, ExprDoneCond *isDone);
  ...
  foreach (l, astate->refupperindexpr) {
    ...
    if (i >= MAXDIM)
      ereport(ERROR, (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
                      errmsg(
                          "number of array dimensions (%d) exceeds the maximum "
                          "allowed (%d)",
                          i + 1, MAXDIM)));

    upper.indx[i++] =
        DatumGetInt32(ExecEvalExpr(eltstate, econtext, &eisnull, NULL));
  ...
  if (isAssignment) {
    ...
    if (isAssignmentIndirectionExpr(astate->refassgnexpr)) {
      if (*isNull) {
        ...
        econtext->caseValue_datum = array_get_element(
            array_source, i, upper.indx, astate->refattrlength,
            astate->refelemlength, astate->refelembyval, astate->refelemalign,
            &econtext->caseValue_isNull);
      } else {
        econtext->caseValue_datum = array_get_slice(
            array_source, i, upper.indx, lower.indx, astate->refattrlength,
            astate->refelemlength, astate->refelembyval, astate->refelemalign);
    ...
    if (astate->refattrlength > 0) /* fixed-length array? */
      if (eisnull || *isNull) return array_source;
    ...
    if (*isNull) {
      array_source =
          PointerGetDatum(construct_empty_array(arrayRef->refelemtype));
      *isNull = false;
    }

    if (lIndex == NULL)
      return array_set_element(array_source, i, upper.indx, sourceData, eisnull,
                               astate->refattrlength, astate->refelemlength,
                               astate->refelembyval, astate->refelemalign);
    else
      return array_set_slice(array_source, i, upper.indx, lower.indx,
                             sourceData, eisnull, astate->refattrlength,
                             astate->refelemlength, astate->refelembyval,
                             astate->refelemalign);
  }

  if (lIndex == NULL)
    return array_get_element(array_source, i, upper.indx, astate->refattrlength,
                             astate->refelemlength, astate->refelembyval,
                             astate->refelemalign, isNull);
  else
    return array_get_slice(array_source, i, upper.indx, lower.indx,
                           astate->refattrlength, astate->refelemlength,
                           astate->refelembyval, astate->refelemalign);
}
 ...
 */
static bool isAssignmentIndirectionExpr(ExprState *exprstate) {
  if (exprstate == NULL) return false; /* just paranoia */
...
static Datum ExecEvalScalarVar(ExprState *exprstate, ExprContext *econtext,
  ...
  if (attnum > 0) {
    ...
    if (attnum > slot_tupdesc->natts) /* should never happen */
      elog(ERROR, "attribute number %d exceeds number of columns %d", attnum,
           slot_tupdesc->natts);
    ...
    if (!attr->attisdropped) {
      if (variable->vartype != attr->atttypid)
        ereport(ERROR, (errmsg("attribute %d has wrong type", attnum),
...
static Datum ExecEvalWholeRowVar(WholeRowVarExprState *wrvstate,
  ...
  if (wrvstate->parent) {
    ...
    if (subplan) {
      ...
      if (junk_filter_needed) {
        ...
        wrvstate->wrv_junkFilter = ExecInitJunkFilter(
            subplan->plan->targetlist, ExecGetResultType(subplan)->tdhasoid,
    ...
    if (var_tupdesc->natts != slot_tupdesc->natts)
      ereport(
          ...
          (errcode(ERRCODE_DATATYPE_MISMATCH),
           ...
           errdetail_plural(
               "Table row contains %d attribute, but query expects %d.",
               "Table row contains %d attributes, but query expects %d.",
               slot_tupdesc->natts, slot_tupdesc->natts, var_tupdesc->natts)));

    for (i = 0; i < var_tupdesc->natts; i++) {
      Form_pg_attribute vattr = var_tupdesc->attrs[i];
      Form_pg_attribute sattr = slot_tupdesc->attrs[i];

      if (vattr->atttypid == sattr->atttypid) continue; /* no worries */
      if (!vattr->attisdropped)
        ereport(
            ...
            (errcode(ERRCODE_DATATYPE_MISMATCH),
             errmsg("table row type and query-specified row type do not match"),
             errdetail(
                 "Table has type %s at ordinal position %d, but query expects "
                 "%s.",
                 format_type_be(sattr->atttypid), i + 1,
  ...
  if (econtext->ecxt_estate &&
    ...
    RangeTblEntry *rte =
        rt_fetch(variable->varno, econtext->ecxt_estate->es_range_table);
  ...
  /* Fetch the value */
  return (*wrvstate->xprstate.evalfunc)((ExprState *)wrvstate, econtext, isNull,
                                        isDone);
}
...
static Datum ExecEvalWholeRowSlow(WholeRowVarExprState *wrvstate,
  ...
  *isNull = false;
  ...
  for (i = 0; i < var_tupdesc->natts; i++) {
    ...
    if (!vattr->attisdropped) continue; /* already checked non-dropped cols */
    if (heap_attisnull(tuple, i + 1)) continue; /* null is always okay */
    if (vattr->attlen != sattr->attlen || vattr->attalign != sattr->attalign)
      ereport(
          ERROR,
          (errcode(ERRCODE_DATATYPE_MISMATCH),
           errmsg("table row type and query-specified row type do not match"),
           errdetail(
               "Physical storage mismatch on dropped attribute at ordinal "
               "position %d.",
               i + 1)));
  }
      ...
      if (prm->ptype != expression->paramtype)
        ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH),
                        errmsg(
                            ...
                            thisParamId, format_type_be(prm->ptype),
...
Datum GetAttributeByName(HeapTupleHeader tuple, const char *attname,
                         bool *isNull) {
  AttrNumber attrno;
  Datum result;
  Oid tupType;
...
static void init_fcache(Oid foid, Oid input_collation, FuncExprState *fcache,
  ...
  InitFunctionCallInfoData(fcache->fcinfo_data, &(fcache->func),
                           list_length(fcache->args), input_collation, NULL,
                           NULL);
  ...
  if (fcache->func.fn_retset && needDescForSets) {
    ...
    functypclass =
        get_expr_result_type(fcache->func.fn_expr, &funcrettype, &tupdesc);
...
static TupleDesc get_cached_rowtype(Oid type_id, int32 typmod,
  ...
  if (tupDesc == NULL || type_id != tupDesc->tdtypeid ||
    ...
    if (*cache_field) {
      ...
      RegisterExprContextCallback(econtext, ShutdownTupleDescRef,
...
static ExprDoneCond ExecEvalFuncArgs(FunctionCallInfo fcinfo, List *argList,
  ...
  foreach (arg, argList) {
    ...
    fcinfo->arg[i] =
        ExecEvalExpr(argstate, econtext, &fcinfo->argnull[i], &thisArgIsDone);
    ...
    ereport(
        ...
        (errcode(ERRCODE_DATATYPE_MISMATCH),
         ...
         errdetail_plural(
             ...
             src_tupdesc->natts, src_tupdesc->natts, dst_tupdesc->natts)));
  ...
  for (i = 0; i < dst_tupdesc->natts; i++) {
    ...
    if (!dattr->attisdropped)
      ereport(
          ...
          (errcode(ERRCODE_DATATYPE_MISMATCH),
           errmsg(
               "function return row and query-specified return row do not "
               "match"),
           errdetail(
    ...
    if (dattr->attlen != sattr->attlen || dattr->attalign != sattr->attalign)
      ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH),
                      errmsg(
                          "function return row and query-specified return row "
                          "do not match"),
                      errdetail(
                          "Physical storage mismatch on dropped attribute at "
                          "ordinal position %d.",
                          i + 1)));
  }
}
...
restart:
  ...
  if (fcache->func.fn_retset || hasSetArg) {
    ...
    for (;;) {
      ...
      if (rsinfo.returnMode == SFRM_ValuePerCall) {
        ...
        if (rsinfo.isDone != ExprSingleResult)
          ereport(ERROR, (errcode(ERRCODE_E_R_I_E_SRF_PROTOCOL_VIOLATED),
                          errmsg(
                              "table-function protocol for materialize mode "
                              "was not followed")));
        if (rsinfo.setResult != NULL) {
          /* prepare to return values from the tuplestore */
          ExecPrepareTuplestoreResult(fcache, econtext, rsinfo.setResult,
                                      rsinfo.setDesc);
      ...
      if (!hasSetArg) break; /* input not a set, so done */
...
static Datum ExecMakeFunctionResultNoSets(FuncExprState *fcache,
  ...
  foreach (arg, fcache->args) {
    ...
    fcinfo->arg[i] =
        ExecEvalExpr(argstate, econtext, &fcinfo->argnull[i], NULL);
    i++;
  }
...
Tuplestorestate *ExecMakeTableFunctionResult(ExprState *funcexpr,
  ...
  rsinfo.allowedModes =
      (int)(SFRM_ValuePerCall | SFRM_Materialize | SFRM_Materialize_Preferred);
  ...
  if (funcexpr && IsA(funcexpr, FuncExprState) &&
    ...
    InitFunctionCallInfoData(fcinfo, &(fcache->func), list_length(fcache->args),
                             fcache->fcinfo_data.fncollation, NULL,
                             (Node *)&rsinfo);
    ...
    if (rsinfo.returnMode == SFRM_ValuePerCall) {
      ...
      if (first_time) {
        ...
        if (returnsTuple) {
           ...
           */
          tupdesc = CreateTemplateTupleDesc(1, false);
          TupleDescInitEntry(tupdesc, (AttrNumber)1, "column", funcrettype, -1,
                             0);
        }
      ...
      if (returnsTuple) {
        ...
        if (HeapTupleHeaderGetTypeId(td) != tupdesc->tdtypeid ||
          ...
          ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH),
                          errmsg(
                              "rows returned by function are not all of the "
                              "same row type")));
      ...
      if (!first_time || rsinfo.isDone != ExprSingleResult)
        ereport(ERROR, (errcode(ERRCODE_E_R_I_E_SRF_PROTOCOL_VIOLATED),
                        errmsg(
                            "table-function protocol for materialize mode was "
                            "not followed")));
...
static Datum ExecEvalScalarArrayOp(ScalarArrayOpExprState *sstate,
  ...
  ScalarArrayOpExpr *opexpr =
      (ScalarArrayOpExpr *)sstate->fxprstate.xprstate.expr;
  ...
  *isNull = false;
  ...
  if (sstate->element_type != ARR_ELEMTYPE(arr)) {
    get_typlenbyvalalign(ARR_ELEMTYPE(arr), &sstate->typlen, &sstate->typbyval,
                         &sstate->typalign);
...
static Datum ExecEvalConvertRowtype(ConvertRowtypeExprState *cstate,
  ...
  if (cstate->indesc == NULL) {
    get_cached_rowtype(exprType((Node *)convert->arg), -1, &cstate->indesc,
                       econtext);
  ...
  if (!cstate->initialized) {
    ...
    cstate->map =
        convert_tuples_by_name(cstate->indesc, cstate->outdesc,
...
static Datum ExecEvalCase(CaseExprState *caseExpr, ExprContext *econtext,
  ...
  if (caseExpr->arg) {
    econtext->caseValue_datum = ExecEvalExpr(caseExpr->arg, econtext,
                                             &econtext->caseValue_isNull, NULL);
  }
...
static Datum ExecEvalArray(ArrayExprState *astate, ExprContext *econtext,
  ...
  *isNull = false;
  ...
  if (!arrayExpr->multidims) {
    ...
    result = construct_md_array(dvalues, dnulls, ndims, dims, lbs, element_type,
                                astate->elemlength, astate->elembyval,
                                astate->elemalign);
  } else {
    ...
    foreach (element, astate->elements) {
      ...
      if (firstone) {
        ...
        if (elem_ndims != this_ndims ||
            memcmp(elem_dims, ARR_DIMS(array), elem_ndims * sizeof(int)) != 0 ||
    ...
    for (i = 0; i < outer_nelems; i++) {
      ...
      dat += subbytes[i];
      if (havenulls)
        array_bitmap_copy(ARR_NULLBITMAP(result), iitem, subbitmaps[i], 0,
                          subnitems[i]);
      iitem += subnitems[i];
    }
  }
...
static Datum ExecEvalRowCompare(RowCompareExprState *rstate,
  ...
  *isNull = true; /* until we get a result */
  ...
  forboth(l, rstate->largs, r, rstate->rargs) {
    ...
    if (locfcinfo.isnull) return (Datum)0; /* force NULL result */
    if (cmpresult != 0) break; /* no need to compare remaining columns */
    i++;
  }
...
static Datum ExecEvalMinMax(MinMaxExprState *minmaxExpr, ExprContext *econtext,
  ...
  *isNull = true; /* until we get a result */
  ...
  InitFunctionCallInfoData(locfcinfo, &minmaxExpr->cfunc, 2, collation, NULL,
                           NULL);
  ...
  foreach (arg, minmaxExpr->args) {
    ...
    if (valueIsNull) continue; /* ignore NULL inputs */
  ...
  switch (xexpr->op) {
        ...
        if (!isnull) {
          appendStringInfo(&buf, "<%s>%s</%s>", argname,
                           map_sql_value_to_xml_value(
                               value, exprType((Node *)e->expr), true),
                           argname);
          *isNull = false;
        }
      }
    ...
    case IS_XMLSERIALIZE: {
      ...
      *isNull = false;
      ...
      return PointerGetDatum(
          xmltotext_with_xmloption(DatumGetXmlP(value), xexpr->xmloption));
    } break;
...
static Datum ExecEvalNullTest(NullTestState *nstate, ExprContext *econtext,
  ...
  if (isDone && *isDone == ExprEndResult) return result; /* nothing to check */
  ...
  if (ntest->argisrow && !(*isNull)) {
    ...
    tupDesc =
        get_cached_rowtype(tupType, tupTypmod, &nstate->argdesc, econtext);
        ...
        if (!conIsNull && !DatumGetBool(conResult))
          ereport(
              ...
              (errcode(ERRCODE_CHECK_VIOLATION),
               ...
               errdomainconstraint(ctest->resulttype, con->name)));
...
static Datum ExecEvalFieldSelect(FieldSelectState *fstate,
  ...
  if (fieldnum > tupDesc->natts) /* should never happen */
    elog(ERROR, "attribute number %d exceeds number of columns %d", fieldnum,
         tupDesc->natts);
  ...
  if (fselect->resulttype != attr->atttypid)
    ereport(ERROR, (errmsg("attribute %d has wrong type", fieldnum),
                    errdetail("Table has type %s, but query expects %s.",
                              format_type_be(attr->atttypid),
...
static Datum ExecEvalFieldStore(FieldStoreState *fstate, ExprContext *econtext,
  ...
  tupDesc =
      get_cached_rowtype(fstore->resulttype, -1, &fstate->argdesc, econtext);
  ...
  *isNull = false;
  ...
  forboth(l1, fstate->newvals, l2, fstore->fieldnums) {
    ...
    values[fieldnum - 1] =
        ExecEvalExpr(newval, econtext, &isnull[fieldnum - 1], NULL);
  }
...
static Datum ExecEvalCoerceViaIO(CoerceViaIOState *iostate,
  ...
  if (isDone && *isDone == ExprEndResult) return inputval; /* nothing to do */
  ...
  result =
      InputFunctionCall(&iostate->infunc, string, iostate->intypioparam, -1);
...
static Datum ExecEvalArrayCoerceExpr(ArrayCoerceExprState *astate,
  ...
  if (isDone && *isDone == ExprEndResult) return result; /* nothing to do */
  if (*isNull) return result;                            /* nothing to do */
   ...
   */
  InitFunctionCallInfoData(locfcinfo, &(astate->elemfunc), 3, InvalidOid, NULL,
                           NULL);
  locfcinfo.arg[0] = result;
 ...
 */
Datum ExecEvalExprSwitchContext(ExprState *expression, ExprContext *econtext,
                                bool *isNull, ExprDoneCond *isDone) {
  Datum retDatum;
  MemoryContext oldContext;
  ...
  switch (nodeTag(node)) {
    ...
    case T_Aggref: {
      ...
      if (parent && IsA(parent, AggState)) {
        ...
        astate->aggdirectargs =
            (List *)ExecInitExpr((Expr *)aggref->aggdirectargs, parent);
        astate->args = (List *)ExecInitExpr((Expr *)aggref->args, parent);
        astate->aggfilter = ExecInitExpr(aggref->aggfilter, parent);
    ...
    case T_ArrayRef: {
      ...
      astate->refupperindexpr =
          (List *)ExecInitExpr((Expr *)aref->refupperindexpr, parent);
      astate->reflowerindexpr =
          (List *)ExecInitExpr((Expr *)aref->reflowerindexpr, parent);
      astate->refexpr = ExecInitExpr(aref->refexpr, parent);
      astate->refassgnexpr = ExecInitExpr(aref->refassgnexpr, parent);
      /* do one-time catalog lookups for type info */
      astate->refattrlength = get_typlen(aref->refarraytype);
      get_typlenbyvalalign(aref->refelemtype, &astate->refelemlength,
                           &astate->refelembyval, &astate->refelemalign);
      state = (ExprState *)astate;
    } break;
    case T_FuncExpr: {
    ...
    case T_ScalarArrayOpExpr: {
      ...
      sstate->fxprstate.xprstate.evalfunc =
          (ExprStateEvalFunc)ExecEvalScalarArrayOp;
      sstate->fxprstate.args =
          (List *)ExecInitExpr((Expr *)opexpr->args, parent);
    ...
    case T_ArrayExpr: {
      ...
      get_typlenbyvalalign(arrayexpr->element_typeid, &astate->elemlength,
                           &astate->elembyval, &astate->elemalign);
      state = (ExprState *)astate;
    } break;
    case T_RowExpr: {
    ...
    case T_RowCompareExpr: {
      ...
      i = 0;
      forthree(l, rcexpr->opnos, l2, rcexpr->opfamilies, l3,
               rcexpr->inputcollids) {
    ...
    case T_CoerceToDomain: {
      ...
      cstate->constraint_ref =
          (DomainConstraintRef *)palloc(sizeof(DomainConstraintRef));
      InitDomainConstraintRef(ctest->resulttype, cstate->constraint_ref,
 ...
 */
static bool ExecTargetList(List *targetlist, ExprContext *econtext,
  ...
  foreach (tl, targetlist) {
    ...
    values[resind] = ExecEvalExpr(gstate->arg, econtext, &isnull[resind],
                                  &itemIsDone[resind]);
      ...
      if (*isDone == ExprEndResult) {
        foreach (tl, targetlist) {
          ...
          while (itemIsDone[resind] == ExprMultipleResult) {
            values[resind] = ExecEvalExpr(gstate->arg, econtext,
                                          &isnull[resind], &itemIsDone[resind]);
          }
        }
...
TupleTableSlot *ExecProject(ProjectionInfo *projInfo, ExprDoneCond *isDone) {
  ...
  if (projInfo->pi_targetlist) {
    if (!ExecTargetList(projInfo->pi_targetlist, econtext, slot->tts_values,
