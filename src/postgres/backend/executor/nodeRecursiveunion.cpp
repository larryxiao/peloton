...
static void build_hash_table(RecursiveUnionState *rustate) {
  ...
  rustate->hashtable = BuildTupleHashTable(
      node->numCols, node->dupColIdx, rustate->eqfunctions,
      rustate->hashfunctions, node->numGroups, sizeof(RUHashEntryData),
...
RecursiveUnionState *ExecInitRecursiveUnion(RecursiveUnion *node,
  ...
  if (node->numCols > 0) {
    rustate->tempContext = AllocSetContextCreate(
        CurrentMemoryContext, "RecursiveUnion", ALLOCSET_DEFAULT_MINSIZE,
        ALLOCSET_DEFAULT_INITSIZE, ALLOCSET_DEFAULT_MAXSIZE);
    rustate->tableContext = AllocSetContextCreate(
        CurrentMemoryContext, "RecursiveUnion hash table",
  ...
  if (node->numCols > 0) {
    execTuplesHashPrepare(node->numCols, node->dupOperators,
                          &rustate->eqfunctions, &rustate->hashfunctions);
...
void ExecReScanRecursiveUnion(RecursiveUnionState *node) {
  ...
  if (plan->numCols > 0) build_hash_table(node);
