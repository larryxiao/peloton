...
static MergeJoinClause MJExamineQuals(List *mergeclauses, Oid *mergefamilies,
  ...
  foreach (cl, mergeclauses) {
    ...
    get_op_opfamily_properties(qual->opno, opfamily, false, &op_strategy,
                               &op_lefttype, &op_righttype);
    if (op_strategy != BTEqualStrategyNumber) /* should not happen */
      elog(ERROR, "cannot merge using non-equality operator___ %u", qual->opno);
...
static MJEvalResult MJEvalOuterValues(MergeJoinState *mergestate) {
  ...
  if (TupIsNull(mergestate->mj_OuterTupleSlot)) return MJEVAL_ENDOFJOIN;
  ...
  for (i = 0; i < mergestate->mj_NumClauses; i++) {
    ...
    clause->ldatum =
        ExecEvalExpr(clause->lexpr, econtext, &clause->lisnull, NULL);
    if (clause->lisnull) {
...
static MJEvalResult MJEvalInnerValues(MergeJoinState *mergestate,
  ...
  if (TupIsNull(innerslot)) return MJEVAL_ENDOFJOIN;
  ...
  for (i = 0; i < mergestate->mj_NumClauses; i++) {
    ...
    clause->rdatum =
        ExecEvalExpr(clause->rexpr, econtext, &clause->risnull, NULL);
    if (clause->risnull) {
      /* match is impossible; can we end the join early? */
...
static int MJCompare(MergeJoinState *mergestate) {
  ...
  for (i = 0; i < mergestate->mj_NumClauses; i++) {
    ...
    result =
        ApplySortComparator(clause->ldatum, clause->lisnull, clause->rdatum,
                            clause->risnull, &clause->ssup);
   ...
   */
  if (result == 0 && (nulleqnull || mergestate->mj_ConstFalseJoin)) result = 1;
    ...
    switch (node->mj_JoinState) {
      ...
      case EXEC_MJ_INITIALIZE_INNER:
        ...
        switch (MJEvalInnerValues(node, innerTupleSlot)) {
          ...
          case MJEVAL_NONMATCHABLE:
            ...
            if (node->mj_ExtraMarks) ExecMarkPos(innerPlan);
      ...
      case EXEC_MJ_JOINTUPLES:
        ...
        if (qualResult) {
          ...
          qualResult =
              (otherqual == NIL || ExecQual(otherqual, econtext, false));
...
MergeJoinState *ExecInitMergeJoin(MergeJoin *node, EState *estate, int eflags) {
  ...
  mergestate->js.ps.qual = (List *)ExecInitExpr((Expr *)node->join.plan.qual,
                                                (PlanState *)mergestate);
  mergestate->js.jointype = node->join.jointype;
  mergestate->js.joinqual = (List *)ExecInitExpr((Expr *)node->join.joinqual,
                                                 (PlanState *)mergestate);
  ...
  innerPlanState(mergestate) =
      ExecInitNode(innerPlan(node), estate, eflags | EXEC_FLAG_MARK);
  ...
  switch (node->join.jointype) {
    ...
    case JOIN_ANTI:
      ...
      mergestate->mj_FillInner = false;
      mergestate->mj_NullInnerTupleSlot = ExecInitNullTupleSlot(
          estate, ExecGetResultType(innerPlanState(mergestate)));
      break;
    case JOIN_RIGHT:
      mergestate->mj_FillOuter = false;
      mergestate->mj_FillInner = true;
      mergestate->mj_NullOuterTupleSlot = ExecInitNullTupleSlot(
          estate, ExecGetResultType(outerPlanState(mergestate)));
      ...
      if (!check_constant_qual(node->join.joinqual,
        ...
        ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                        errmsg(
                            "RIGHT JOIN is only supported with merge-joinable "
                            "join conditions")));
      break;
    case JOIN_FULL:
      mergestate->mj_FillOuter = true;
      ...
      if (!check_constant_qual(node->join.joinqual,
        ...
        ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                        errmsg(
                            "FULL JOIN is only supported with merge-joinable "
                            "join conditions")));
      break;
    default:
  ...
  mergestate->mj_Clauses = MJExamineQuals(
      node->mergeclauses, node->mergeFamilies, node->mergeCollations,
...
void ExecReScanMergeJoin(MergeJoinState *node) {
   ...
   * first ExecProcNode.
   */
  if (node->js.ps.lefttree->chgParam == NULL) ExecReScan(node->js.ps.lefttree);
