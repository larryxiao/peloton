...
static Node *sql_fn_post_column_ref(ParseState *pstate, ColumnRef *cref,
                                    Node *var);
static Node *sql_fn_make_param(SQLFunctionParseInfoPtr pinfo, int paramno,
                               int location);
static Node *sql_fn_resolve_param_name(SQLFunctionParseInfoPtr pinfo,
                                       const char *paramname, int location);
static List *init_execution_state(List *queryTree_list,
                                  SQLFunctionCachePtr fcache, bool lazyEvalOK);
...
static void sqlfunction_startup(DestReceiver *self, int operation,
                                TupleDesc typeinfo,
...
SQLFunctionParseInfoPtr prepare_sql_fn_parse_info(HeapTuple procedureTuple,
  ...
  if (nargs > 0) {
    ...
    memcpy(argOidVect, procedureStruct->proargtypes.values,
           nargs * sizeof(Oid));
  ...
  if (nargs > 0) {
    ...
    if (isNull) proargnames = PointerGetDatum(NULL); /* just to be sure */

    proargmodes = SysCacheGetAttr(PROCNAMEARGSNSP, procedureTuple,
                                  Anum_pg_proc_proargmodes, &isNull);
    if (isNull) proargmodes = PointerGetDatum(NULL); /* just to be sure */

    n_arg_names =
        get_func_input_arg_names(proargnames, proargmodes, &pinfo->argnames);

    /* Paranoia: ignore the result if too few array entries */
    if (n_arg_names < nargs) pinfo->argnames = NULL;
  } else
    pinfo->argnames = NULL;
...
static Node *sql_fn_post_column_ref(ParseState *pstate, ColumnRef *cref,
                                    Node *var) {
  SQLFunctionParseInfoPtr pinfo =
      (SQLFunctionParseInfoPtr)pstate->p_ref_hook_state;
  int nnames;
  Node *field1;
  Node *subfield = NULL;
  const char *name1;
  ...
  if (nnames == 3) {
     ...
     */
    if (strcmp(name1, pinfo->fname) != 0) return NULL;
  ...
  if (subfield) {
     ...
     * caller.
     */
    param = ParseFuncOrColumn(pstate, list_make1(subfield), list_make1(param),
                              NULL, cref->location);
  }
...
static List *init_execution_state(List *queryTree_list,
  ...
  foreach (lc1, queryTree_list) {
    ...
    foreach (lc2, qtlist) {
      ...
      if (queryTree->canSetTag) lasttages = newes;
  ...
  if (lasttages && fcache->junkFilter) {
    ...
    if (lazyEvalOK && IsA(lasttages->stmt, PlannedStmt)) {
      PlannedStmt *ps = (PlannedStmt *)lasttages->stmt;

      if (ps->commandType == CMD_SELECT && ps->utilityStmt == NULL &&
          !ps->hasModifyingCTE)
...
static void init_sql_fcache(FmgrInfo *finfo, Oid collation, bool lazyEvalOK) {
   ...
   */
  fcontext = AllocSetContextCreate(
      finfo->fn_mcxt, "SQL function data", ALLOCSET_DEFAULT_MINSIZE,
  ...
  if (IsPolymorphicType(rettype)) {
    ...
    if (rettype == InvalidOid) /* this probably should not happen */
      ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH),
                      errmsg(
                          "could not determine actual result type for function "
                          "declared to return type %s",
   ...
   */
  fcache->pinfo =
      prepare_sql_fn_parse_info(procedureTuple, finfo->fn_expr, collation);

  /*
   * And of course we need the function body text.
   */
  tmp = SysCacheGetAttr(PROCOID, procedureTuple, Anum_pg_proc_prosrc, &isNull);
  if (isNull) elog(ERROR, "null prosrc for function %u", foid);
  ...
  foreach (lc, raw_parsetree_list) {
    ...
    queryTree_sublist = pg_analyze_and_rewrite_params(
        parsetree, fcache->src, (ParserSetupHook)sql_fn_parser_setup,
        fcache->pinfo);
    queryTree_list = lappend(queryTree_list, queryTree_sublist);
    flat_query_list =
        list_concat(flat_query_list, list_copy(queryTree_sublist));
  }
   ...
   */
  fcache->returnsTuple = check_sql_fn_retval(foid, rettype, flat_query_list,
                                             NULL, &fcache->junkFilter);
...
static void postquel_start(execution_state *es, SQLFunctionCachePtr fcache) {
  ...
  if (IsA(es->stmt, PlannedStmt))
    es->qd = CreateQueryDesc((PlannedStmt *)es->stmt, fcache->src,
                             GetActiveSnapshot(), InvalidSnapshot, dest,
                             fcache->paramLI, 0);
  else
    es->qd = CreateUtilityQueryDesc(es->stmt, fcache->src, GetActiveSnapshot(),
                                    dest, fcache->paramLI);
...
static bool postquel_getnext(execution_state *es, SQLFunctionCachePtr fcache) {
  ...
  if (es->qd->utilitystmt) {
    ...
    ProcessUtility((es->qd->plannedstmt ? (Node *)es->qd->plannedstmt
                                        : es->qd->utilitystmt),
                   fcache->src, PROCESS_UTILITY_QUERY, es->qd->params,
                   es->qd->dest, NULL);
...
Datum fmgr_sql(PG_FUNCTION_ARGS) {
  ...
  while (es) {
     ...
     */
    if (es->status != F_EXEC_DONE) break;
  ...
  if (fcache->returnsSet) {
    ...
    if (es) {
      ...
      if (!fcache->shutdown_reg) {
        RegisterExprContextCallback(rsi->econtext, ShutdownSQLFunction,
      ...
      if (fcache->shutdown_reg) {
        UnregisterExprContextCallback(rsi->econtext, ShutdownSQLFunction,
                                      PointerGetDatum(fcache));
  ...
  if (pushed_snapshot) PopActiveSnapshot();
...
static void sql_exec_error_callback(void *arg) {
   ...
   */
  if (fcache == NULL || fcache->fname == NULL) return;
  ...
  if (fcache->func_state) {
    ...
    foreach (lc, fcache->func_state) {
      ...
      while (es) {
        if (es->qd) {
          errcontext("SQL function \"%s\" statement %d", fcache->fname,
                     query_num);
          break;
        }
        es = es->next;
...
static void ShutdownSQLFunction(Datum arg) {
  ...
  foreach (lc, fcache->func_state) {
    ...
    while (es) {
      ...
      if (es->status == F_EXEC_RUN) {
        ...
        if (!fcache->readonly_func) PushActiveSnapshot(es->qd->snapshot);

        postquel_end(es);

        if (!fcache->readonly_func) PopActiveSnapshot();
      }
  ...
  if (fcache->tstore) tuplestore_end(fcache->tstore);
  fcache->tstore = NULL;
 ...
 * set to NULL.
 */
bool check_sql_fn_retval(Oid func_id, Oid rettype, List *queryTreeList,
                         bool *modifyTargetList, JunkFilter **junkFilter) {
  Query *parse;
  List **tlist_ptr;
  List *tlist;
  int tlistlen;
  ...
  if (junkFilter) *junkFilter = NULL; /* initialize in case of VOID result */
   ...
   * entities.
   */
  if (parse && parse->commandType == CMD_SELECT && parse->utilityStmt == NULL) {
  ...
  if (fn_typtype == TYPTYPE_BASE || fn_typtype == TYPTYPE_DOMAIN ||
    ...
    if (!IsBinaryCoercible(restype, rettype))
      ereport(
          ...
          (errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
           errmsg("return type mismatch in function declared to return %s",
                  format_type_be(rettype)),
           errdetail("Actual return type is %s.", format_type_be(restype))));
    if (modifyTargetList && restype != rettype) {
      tle->expr = (Expr *)makeRelabelType(tle->expr, rettype, -1,
    ...
    if (junkFilter) *junkFilter = ExecInitJunkFilter(tlist, false, NULL);
    ...
    foreach (lc, tlist) {
      ...
      if (tle->resjunk) {
        if (modifyTargetList) junkattrs = lappend(junkattrs, tle);
        continue;
      }
      ...
      do {
        ...
        if (attr->attisdropped && modifyTargetList) {
          ...
          null_expr = (Expr *)makeConst(INT4OID, -1, InvalidOid, sizeof(int32),
                                        (Datum)0, true, /* isnull */
                                        true /* byval */);
          newtlist = lappend(newtlist,
                             makeTargetEntry(null_expr, colindex, NULL, false));
          /* NULL insertion is dangerous in a setop */
          if (parse->setOperations) *modifyTargetList = true;
        }
        ...
        ereport(
            ...
            (errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
             ...
             errdetail("Final statement returns %s instead of %s at column %d.",
                       format_type_be(tletype), format_type_be(atttype),
                       tuplogcols)));
      if (modifyTargetList) {
        if (tletype != atttype) {
          tle->expr = (Expr *)makeRelabelType(tle->expr, atttype, -1,
    ...
    /* Set up junk filter if needed */
    if (junkFilter)
      *junkFilter = ExecInitJunkFilterConversion(
          tlist, CreateTupleDescCopy(tupdesc), NULL);
 ...
 */
static void sqlfunction_shutdown(DestReceiver *self) { /* no-op */ }

/*
 * sqlfunction_destroy --- release DestReceiver object
 */
static void sqlfunction_destroy(DestReceiver *self) { pfree(self); }
