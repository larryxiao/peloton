...
void ExecReScanBitmapIndexScan(BitmapIndexScanState *node) {
  ...
  if (node->biss_NumArrayKeys != 0)
    node->biss_RuntimeKeysReady = ExecIndexEvalArrayKeys(
        econtext, node->biss_ArrayKeys, node->biss_NumArrayKeys);
  else
...
BitmapIndexScanState *ExecInitBitmapIndexScan(BitmapIndexScan *node,
  ...
  ExecIndexBuildScanKeys(
      (PlanState *)indexstate, indexstate->biss_RelationDesc, node->indexqual,
      false, &indexstate->biss_ScanKeys, &indexstate->biss_NumScanKeys,
