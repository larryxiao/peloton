 ...
 */
static void base_backup_cleanup(int code, Datum arg) { do_pg_abort_backup(); }
...
static void perform_base_backup(basebackup_options *opt, DIR *tblspcdir) {
  ...
  startptr = do_pg_start_backup(
      opt->label, opt->fastcheckpoint, &starttli, &labelfile, tblspcdir,
      &tablespaces, &tblspc_map_file, opt->progress, opt->sendtblspcmapfile);
  /*
  ...
  PG_ENSURE_ERROR_CLEANUP(base_backup_cleanup, (Datum)0);
  {
    ...
    if (is_absolute_path(pgstat_stat_directory) &&
      ...
      statrelpath =
          psprintf("./%s", pgstat_stat_directory + datadirpathlen + 1);
  ...
  if (opt->includewal) {
     ...
     */
    if (nWalFiles < 1) ereport(ERROR, (errmsg("could not find any WAL files")));
    ...
    for (i = 0; i < nWalFiles; i++) {
      ...
      while ((cnt = fread(buf, 1, Min(sizeof(buf), XLogSegSize - len), fp)) >
             0) {
        CheckXLogRemoved(segno, tli);
...
static void parse_basebackup_options(List *options, basebackup_options *opt) {
      ...
      if (maxrate < MAX_RATE_LOWER || maxrate > MAX_RATE_UPPER)
        ereport(ERROR, (errcode(ERRCODE_NUMERIC_VALUE_OUT_OF_RANGE),
                        errmsg(
                            ...
                            "\"%s\" (%d .. %d)",
                            (int)maxrate, "MAX_RATE", MAX_RATE_LOWER,
                            MAX_RATE_UPPER)));
  ...
  if (opt->label == NULL) opt->label = "base backup";
}
...
int64 sendTablespace(char *path, bool sizeonly) {
  ...
  if (!sizeonly) _tarWriteHeader(TABLESPACE_VERSION_DIRECTORY, NULL, &statbuf);
...
static int64 sendDir(char *path, int basepathlen, bool sizeonly,
  ...
  while ((de = ReadDir(dir, path)) != NULL) {
    ...
    if (strncmp(de->d_name, PG_TEMP_FILE_PREFIX, strlen(PG_TEMP_FILE_PREFIX)) ==
        0)
      continue;

    /* skip auto conf temporary file */
    if (strncmp(de->d_name, PG_AUTOCONF_FILENAME ".tmp",
     ...
     */
    if (strcmp(de->d_name, BACKUP_LABEL_FILE) == 0) continue;

    if (strcmp(de->d_name, TABLESPACE_MAP) == 0) continue;
    ...
    if (strcmp(pathbuf, "./global/pg_control") == 0) continue;
    ...
    if ((statrelpath != NULL && strcmp(pathbuf, statrelpath) == 0) ||
      ...
      if (!sizeonly) _tarWriteHeader(pathbuf + basepathlen + 1, NULL, &statbuf);
      size += 512;
      continue;
    }
 ...
 */
#define MAX_TAR_MEMBER_FILELEN \
  (((int64)1 << Min(33, sizeof(pgoff_t) * 8 - 1)) - 1)
...
static bool sendFile(char *readfilename, char *tarfilename,
  ...
  if (fp == NULL) {
    if (errno == ENOENT && missing_ok) return false;
    ereport(ERROR, (errcode_for_file_access(),
  ...
  if (statbuf->st_size > MAX_TAR_MEMBER_FILELEN)
    ereport(ERROR, (errmsg("archive member \"%s\" too large for tar format",
                           tarfilename)));
  ...
  switch (rc) {
    ...
    case TAR_SYMLINK_TOO_LONG:
      ereport(ERROR, (errmsg(
                         "symbolic link target too long for tar format: file "
                         "name \"%s\", target \"%s\"",
                         filename, linktarget)));
      break;
    default:
 ...
 */
static void throttle(size_t increment) {
  int64 elapsed, elapsed_min, sleep;
  int wait_result;

  if (throttling_counter < 0) return;

  throttling_counter += increment;
  if (throttling_counter < throttling_sample) return;
  ...
  if (sleep > 0) {
    ...
    if (wait_result & WL_LATCH_SET) CHECK_FOR_INTERRUPTS();
  } else {
    /*
