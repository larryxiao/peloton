...
void SyncRepWaitForLSN(XLogRecPtr XactCommitLSN) {
  ...
  for (;;) {
    ...
    if (syncRepState == SYNC_REP_WAITING) syncRepState = MyProc->syncRepState;
    ...
    if (ProcDiePending) {
      ereport(WARNING,
              (errcode(ERRCODE_ADMIN_SHUTDOWN),
               errmsg(
                   "canceling the wait for synchronous replication and "
    ...
    if (QueryCancelPending) {
      ...
      ereport(WARNING,
              (errmsg(
                   "canceling wait for synchronous replication due to user "
                   "request"),
               errdetail(
                   "The transaction has already committed locally, but might "
                   "not have been replicated to the standby.")));
...
static void SyncRepQueueInsert(int mode) {
  ...
  while (proc) {
     ...
     * is ordered by LSN.
     */
    if (proc->waitLSN < MyProc->waitLSN) break;
...
WalSnd *SyncRepGetSynchronousStandby(void) {
  ...
  for (i = 0; i < max_wal_senders; i++) {
    ...
    /* Must be streaming */
    if (walsnd->state != WALSNDSTATE_STREAMING) continue;
    ...
    if (result != NULL && result_priority <= this_priority) continue;

    /* Must have a valid flush position */
    if (XLogRecPtrIsInvalid(walsnd->flush)) continue;
     ...
     */
    if (this_priority == 1) return result;
  }
...
void SyncRepReleaseWaiters(void) {
  ...
  if (announce_next_takeover) {
    ...
    ereport(
        LOG,
        (errmsg(
            "standby \"%s\" is now the synchronous standby with priority %u",
...
static int SyncRepWakeQueue(bool all, int mode) {
  ...
  while (proc) {
     ...
     */
    if (!all && walsndctl->lsn[mode] < proc->waitLSN) return numprocs;
...
void SyncRepUpdateSyncStandbysDefined(void) {
  ...
  if (sync_standbys_defined != WalSndCtl->sync_standbys_defined) {
    ...
    if (!sync_standbys_defined) {
      ...
      for (i = 0; i < NUM_SYNC_REP_WAIT_MODE; i++) SyncRepWakeQueue(true, i);
    }
...
static bool SyncRepQueueIsOrderedByLSN(int mode) {
  ...
  while (proc) {
     ...
     * have matching LSNs
     */
    if (proc->waitLSN <= lastLSN) return false;
...
bool check_synchronous_standby_names(char **newval, void **extra,
                                     GucSource source) {
  char *rawstring;
  List *elemlist;
