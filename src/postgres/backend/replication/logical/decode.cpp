...
void LogicalDecodingProcessRecord(LogicalDecodingContext *ctx,
  ...
  switch ((RmgrIds)XLogRecGetRmid(record)) {
    ...
    case RM_REPLORIGIN_ID:
      break;
    case RM_NEXT_ID:
      elog(ERROR, "unexpected RM_NEXT_ID rmgr_id: %u",
           (RmgrIds)XLogRecGetRmid(buf.record));
  }
}
...
static void DecodeXactOp(LogicalDecodingContext *ctx, XLogRecordBuffer *buf) {
  ...
  if (SnapBuildCurrentState(builder) < SNAPBUILD_FULL_SNAPSHOT) return;
  ...
  switch (info) {
    ...
    case XLOG_XACT_ASSIGNMENT: {
      ...
      for (i = 0; i < xlrec->nsubxacts; i++) {
        ReorderBufferAssignChild(reorder, xlrec->xtop, *(sub_xid++),
                                 buf->origptr);
      }
      break;
    }
 ...
 */
static void DecodeStandbyOp(LogicalDecodingContext *ctx,
                            XLogRecordBuffer *buf) {
  SnapBuild *builder = ctx->snapshot_builder;
  XLogReaderState *r = buf->record;
...
static void DecodeHeapOp(LogicalDecodingContext *ctx, XLogRecordBuffer *buf) {
  ...
  switch (info) {
    ...
    case XLOG_HEAP_INPLACE:
       ...
       */
      if (!TransactionIdIsValid(xid)) break;
...
static inline bool FilterByOrigin(LogicalDecodingContext *ctx,
                                  RepOriginId origin_id) {
  if (ctx->callbacks.filter_by_origin_cb == NULL) return false;
...
static void DecodeCommit(LogicalDecodingContext *ctx, XLogRecordBuffer *buf,
  ...
  ReorderBufferCommit(ctx->reorder, xid, buf->origptr, buf->endptr, commit_time,
...
static void DecodeInsert(LogicalDecodingContext *ctx, XLogRecordBuffer *buf) {
  ...
  if (target_node.dbNode != ctx->slot->data.database) return;

  /* output plugin doesn't look for this origin, no need to queue */
  if (FilterByOrigin(ctx, XLogRecGetOrigin(r))) return;
...
static void DecodeDelete(LogicalDecodingContext *ctx, XLogRecordBuffer *buf) {
   ...
   */
  if (xlrec->flags & XLH_DELETE_IS_SUPER) return;
...
static void DecodeMultiInsert(LogicalDecodingContext *ctx,
  ...
  if (rnode.dbNode != ctx->slot->data.database) return;
