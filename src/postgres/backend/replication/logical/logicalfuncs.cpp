...
static void LogicalOutputWrite(LogicalDecodingContext *ctx, XLogRecPtr lsn,
   ...
   */
  if (!p->binary_output)
    Assert(pg_verify_mbstr(GetDatabaseEncoding(), ctx->out->data, ctx->out->len,
                           false));

  /* ick, but cstring_to_text_with_len works for bytea perfectly fine */
  values[2] =
      PointerGetDatum(cstring_to_text_with_len(ctx->out->data, ctx->out->len));
 ...
 */
static void XLogRead(char *buf, TimeLineID tli, XLogRecPtr startptr,
                     Size count) {
  char *p;
  XLogRecPtr recptr;
  Size nbytes;
...
static void check_permissions(void) {
  if (!superuser() && !has_rolreplication(GetUserId()))
    ereport(ERROR, (errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
                    (errmsg(
                        "must be superuser or replication role to use "
                        "replication slots"))));
}
 ...
 * loop for now.
 */
int logical_read_local_xlog_page(XLogReaderState *state,
...
static Datum pg_logical_slot_get_changes_guts(FunctionCallInfo fcinfo,
  ...
  PG_TRY();
  {
    ctx = CreateDecodingContext(InvalidXLogRecPtr, options,
    ...
    if (!binary && ctx->options.output_type != OUTPUT_PLUGIN_TEXTUAL_OUTPUT)
      ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                      errmsg(
                          "logical decoding output plugin \"%s\" produces "
                          "binary output, but \"%s\" expects textual data",
    ...
    while ((startptr != InvalidXLogRecPtr && startptr < end_of_wal) ||
       ...
       */
      if (record != NULL) LogicalDecodingProcessRecord(ctx, ctx->reader);

      /* check limits */
      if (upto_lsn != InvalidXLogRecPtr && upto_lsn <= ctx->reader->EndRecPtr)
        break;
      if (upto_nchanges != 0 && upto_nchanges <= p->returned_rows) break;
      CHECK_FOR_INTERRUPTS();
    }
  }
  PG_CATCH();
