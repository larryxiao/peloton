 ...
 * ---------------------------------------
 */
static ReorderBufferIterTXNState *ReorderBufferIterTXNInit(
    ReorderBuffer *rb, ReorderBufferTXN *txn);
static ReorderBufferChange *ReorderBufferIterTXNNext(
    ReorderBuffer *rb, ReorderBufferIterTXNState *state);
...
ReorderBuffer *ReorderBufferAllocate(void) {
  ...
  new_ctx = AllocSetContextCreate(
      CurrentMemoryContext, "ReorderBuffer", ALLOCSET_DEFAULT_MINSIZE,
...
static ReorderBufferTXN *ReorderBufferGetTXN(ReorderBuffer *rb) {
  ...
  if (rb->nr_cached_transactions > 0) {
    ...
    txn = (ReorderBufferTXN *)dlist_container(
        ReorderBufferTXN, node, dlist_pop_head_node(&rb->cached_transactions));
  } else {
    txn = (ReorderBufferTXN *)MemoryContextAlloc(rb->context,
...
ReorderBufferChange *ReorderBufferGetChange(ReorderBuffer *rb) {
  ...
  if (rb->nr_cached_changes) {
    ...
    change = (ReorderBufferChange *)dlist_container(
        ReorderBufferChange, node, dlist_pop_head_node(&rb->cached_changes));
  } else {
    change = (ReorderBufferChange *)MemoryContextAlloc(
        rb->context, sizeof(ReorderBufferChange));
  }
...
ReorderBufferTupleBuf *ReorderBufferGetTupleBuf(ReorderBuffer *rb) {
  ...
  if (rb->nr_cached_tuplebufs) {
    ...
    tuple = (ReorderBufferTupleBuf *)MemoryContextAlloc(
        rb->context, sizeof(ReorderBufferTupleBuf));
  }
...
static ReorderBufferTXN *ReorderBufferTXNByXid(ReorderBuffer *rb,
  ...
  ent = (ReorderBufferTXNByIdEnt *)hash_search(
      rb->by_txn, (void *)&xid, create ? HASH_ENTER : HASH_FIND, &found);
  if (found)
    txn = ent->txn;
  else if (create) {
...
ReorderBufferTXN *ReorderBufferGetOldestTXN(ReorderBuffer *rb) {
  ...
  if (dlist_is_empty(&rb->toplevel_by_lsn)) return NULL;
...
void ReorderBufferCommitChild(ReorderBuffer *rb, TransactionId xid,
  ...
  subtxn =
      ReorderBufferTXNByXid(rb, subxid, false, NULL, InvalidXLogRecPtr, false);
...
static ReorderBufferIterTXNState *ReorderBufferIterTXNInit(
  ...
  state = (ReorderBufferIterTXNState *)MemoryContextAllocZero(
      rb->context, sizeof(ReorderBufferIterTXNState) +
  ...
  /* allocate heap */
  state->heap =
      binaryheap_allocate(state->nr_txns, ReorderBufferIterCompare, state);
  ...
  dlist_foreach(cur_txn_i, &txn->subtxns) {
    ...
    if (cur_txn->nentries > 0) {
      ...
      if (txn->nentries != txn->nentries_mem)
        ReorderBufferRestoreChanges(rb, cur_txn, &state->entries[off].fd,
                                    &state->entries[off].segno);

      cur_change =
          dlist_head_element(ReorderBufferChange, node, &cur_txn->changes);
...
static ReorderBufferChange *ReorderBufferIterTXNNext(
  ...
  if (state->heap->bh_size == 0) return NULL;
...
static void ReorderBufferCleanupTXN(ReorderBuffer *rb, ReorderBufferTXN *txn) {
  ...
  if (txn->nentries != txn->nentries_mem) ReorderBufferRestoreCleanup(rb, txn);
...
static void ReorderBufferBuildTupleCidHash(ReorderBuffer *rb,
  ...
  if (!txn->has_catalog_changes || dlist_is_empty(&txn->tuplecids)) return;
  ...
  dlist_foreach(iter, &txn->tuplecids) {
    ...
    ent = (ReorderBufferTupleCidEnt *)hash_search(
        txn->tuplecid_hash, (void *)&key,
        static_cast<HASHACTION>(HASH_ENTER | HASH_FIND), &found);
    if (!found) {
...
static Snapshot ReorderBufferCopySnap(ReorderBuffer *rb, Snapshot orig_snap,
  ...
  size = sizeof(SnapshotData) + sizeof(TransactionId) * orig_snap->xcnt +
...
void ReorderBufferCommit(ReorderBuffer *rb, TransactionId xid,
  ...
  if (txn->nentries_mem != txn->nentries) ReorderBufferSerializeTXN(rb, txn);
  ...
  PG_TRY();
  {
    ...
    while ((change = ReorderBufferIterTXNNext(rb, iterstate)) != NULL) {
      ...
      switch (change->action) {
        ...
        case REORDER_BUFFER_CHANGE_DELETE:
           ...
           */
          if (reloid == InvalidOid && change->data.tp.newtuple == NULL &&
              change->data.tp.oldtuple == NULL)
            goto change_done;
           ...
           * few cases we do.
           */
          if (relation->rd_rel->relkind == RELKIND_SEQUENCE) goto change_done;
        ...
        case REORDER_BUFFER_CHANGE_INTERNAL_SNAPSHOT:
          ...
          if (snapshot_now->copied) {
            ...
            snapshot_now = ReorderBufferCopySnap(rb, change->data.snapshot, txn,
                                                 command_id);
          }
        ...
        case REORDER_BUFFER_CHANGE_INTERNAL_COMMAND_ID:
          ...
          if (command_id < change->data.command_id) {
            ...
            if (!snapshot_now->copied) {
              ...
              snapshot_now =
                  ReorderBufferCopySnap(rb, snapshot_now, txn, command_id);
            }
    ...
    if (snapshot_now->copied) ReorderBufferFreeSnap(rb, snapshot_now);
  ...
  PG_CATCH();
  {
    ...
    if (iterstate) ReorderBufferIterTXNFinish(rb, iterstate);
...
void ReorderBufferForget(ReorderBuffer *rb, TransactionId xid, XLogRecPtr lsn) {
  ...
  if (txn->base_snapshot != NULL && txn->ninvalidations > 0) {
    ...
    if (use_subtxn) BeginInternalSubTransaction("replay");
     ...
     */
    if (use_subtxn) AbortCurrentTransaction();

    ReorderBufferExecuteInvalidations(rb, txn);

    if (use_subtxn) RollbackAndReleaseCurrentSubTransaction();
  } else
    Assert(txn->ninvalidations == 0);
 ...
 */
bool ReorderBufferIsXidKnown(ReorderBuffer *rb, TransactionId xid) {
  ReorderBufferTXN *txn;
...
void ReorderBufferAddInvalidations(ReorderBuffer *rb, TransactionId xid,
  ...
  txn->invalidations = (SharedInvalidationMessage *)MemoryContextAlloc(
      rb->context, sizeof(SharedInvalidationMessage) * nmsgs);
 ...
 */
void ReorderBufferXidSetCatalogChanges(ReorderBuffer *rb, TransactionId xid,
                                       XLogRecPtr lsn) {
  ReorderBufferTXN *txn;
...
static void ReorderBufferSerializeTXN(ReorderBuffer *rb,
  ...
  elog(DEBUG2, "spill %u changes in XID %u to disk", (uint32)txn->nentries_mem,
       txn->xid);
...
static void ReorderBufferSerializeChange(ReorderBuffer *rb,
  ...
  switch (change->action) {
    ...
    case REORDER_BUFFER_CHANGE_INTERNAL_SPEC_INSERT: {
      ...
      if (oldtup)
        oldlen = offsetof(ReorderBufferTupleBuf, t_data) + oldtup->tuple.t_len;

      if (newtup)
        newlen = offsetof(ReorderBufferTupleBuf, t_data) + newtup->tuple.t_len;
    ...
    case REORDER_BUFFER_CHANGE_INTERNAL_SNAPSHOT: {
      ...
      sz += sizeof(SnapshotData) + sizeof(TransactionId) * snap->xcnt +
  ...
  while (restored < max_changes_in_memory && *segno <= last_segno) {
    ...
    if (readBytes == 0) {
      ...
      *fd = -1;
      ...
      ereport(ERROR, (errcode_for_file_access(),
                      errmsg(
                          "could not read from reorderbuffer spill file: read "
    ...
    ReorderBufferSerializeReserve(
        rb, sizeof(ReorderBufferDiskChange) + ondisk->size);
    ondisk = (ReorderBufferDiskChange *)rb->outbuf;
    ...
    if (readBytes < 0)
      ...
      ereport(ERROR,
              (errcode_for_file_access(),
               errmsg(
                   "could not read from reorderbuffer spill file: read %d "
                   "instead of %u bytes",
                   readBytes,
  ...
  switch (change->action) {
    ...
    case REORDER_BUFFER_CHANGE_INTERNAL_SNAPSHOT: {
      ...
      change->data.snapshot =
          static_cast<Snapshot>(MemoryContextAllocZero(rb->context, size));

      newsnap = change->data.snapshot;

      memcpy(newsnap, data, size);
      newsnap->xip =
          (TransactionId *)(((char *)newsnap) + sizeof(SnapshotData));
...
void StartupReorderBuffer(void) {
  ...
  while ((logical_de = ReadDir(logical_dir, "pg_replslot")) != NULL) {
    ...
    if (lstat(path, &statbuf) == 0 && !S_ISDIR(statbuf.st_mode)) continue;
...
static void ReorderBufferToastAppendChunk(ReorderBuffer *rb,
  ...
  if (txn->toast_hash == NULL) ReorderBufferToastInitHash(rb, txn);
  ...
  ent = (ReorderBufferToastEnt *)hash_search(txn->toast_hash, (void *)&chunk_id,
                                             HASH_ENTER, &found);
...
static void ReorderBufferToastReplace(ReorderBuffer *rb, ReorderBufferTXN *txn,
  ...
  for (natt = 0; natt < desc->natts; natt++) {
    ...
    if (attr->attlen != -1) continue;

    /* no data */
    if (isnull[natt]) continue;

    /* ok, we know we have a toast datum */
    varlena = (struct varlena *)DatumGetPointer(attrs[natt]);

    /* no need to do anything if the tuple isn't external */
    if (!VARATT_IS_EXTERNAL(varlena)) continue;
     ...
     */
    ent = (ReorderBufferToastEnt *)hash_search(
        txn->toast_hash, (void *)&toast_pointer.va_valueid, HASH_FIND, NULL);
    ...
    reconstructed =
        static_cast<struct varlena *>(palloc0(toast_pointer.va_rawsize));
    ...
    dlist_foreach(it, &ent->chunks) {
      ...
      chunk =
          DatumGetPointer(fastgetattr(&ctup->tuple, 3, toast_desc, &isnull));
  ...
  pfree(tmphtup);
  for (natt = 0; natt < desc->natts; natt++) {
    if (free[natt]) pfree(DatumGetPointer(attrs[natt]));
  }
  pfree(attrs);
  pfree(free);
  pfree(isnull);
...
static void ReorderBufferToastReset(ReorderBuffer *rb, ReorderBufferTXN *txn) {
  ...
  while ((ent = (ReorderBufferToastEnt *)hash_seq_search(&hstat)) != NULL) {
    ...
    if (ent->reconstructed != NULL) pfree(ent->reconstructed);
...
static void DisplayMapping(HTAB *tuplecid_data) {
  ...
  while ((ent = (ReorderBufferTupleCidEnt *)hash_seq_search(&hstat)) != NULL) {
    elog(DEBUG3, "mapping: node: %u/%u/%u tid: %u/%u cmin: %u, cmax: %u",
         ent->key.relnode.dbNode, ent->key.relnode.spcNode,
         ent->key.relnode.relNode,
         BlockIdGetBlockNumber(&ent->key.tid.ip_blkid), ent->key.tid.ip_posid,
         ent->cmin, ent->cmax);
  }
}
#endif
 ...
 */
static void ApplyLogicalMappingFile(HTAB *tuplecid_data, Oid relid,
                                    const char *fname) {
  char path[MAXPGPATH];
  int fd;
  int readBytes;
  ...
  fd = OpenTransientFile(path, O_RDONLY | PG_BINARY, 0);
  if (fd < 0) ereport(ERROR, (errmsg("could not open file \"%s\": %m", path)));
  ...
  while (true) {
    ...
    ent = (ReorderBufferTupleCidEnt *)hash_search(tuplecid_data, (void *)&key,
                                                  HASH_FIND, NULL);
    ...
    new_ent = (ReorderBufferTupleCidEnt *)hash_search(
        tuplecid_data, (void *)&key, HASH_ENTER, &found);
 ...
 */
static bool TransactionIdInArray(TransactionId xid, TransactionId *xip,
                                 Size num) {
 ...
 */
static void UpdateLogicalMappings(HTAB *tuplecid_data, Oid relid,
                                  Snapshot snapshot) {
  DIR *mapping_dir;
  ...
  while ((mapping_de = ReadDir(mapping_dir, "pg_logical/mappings")) != NULL) {
    ...
    if (strncmp(mapping_de->d_name, "map-", 4) != 0) continue;

    if (sscanf(mapping_de->d_name, LOGICAL_REWRITE_FORMAT, &f_dboid, &f_relid,
               &f_hi, &f_lo, &f_mapped_xid, &f_create_xid) != 6)
    ...
    if (!TransactionIdInArray(f_mapped_xid, snapshot->subxip,
                              snapshot->subxcnt))
      continue;
  ...
  files_a = static_cast<RewriteMappingFile **>(
      palloc(list_length(files) * sizeof(RewriteMappingFile *)));
  off = 0;
  foreach (file, files) {
 ...
 * combocids.
 */
bool ResolveCminCmaxDuringDecoding(HTAB *tuplecid_data, Snapshot snapshot,
