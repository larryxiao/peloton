...
static LogicalDecodingContext *StartupDecodingContext(
    List *output_plugin_options, XLogRecPtr start_lsn,
    TransactionId xmin_horizon, XLogPageReadCB read_page,
  ...
  ctx = static_cast<LogicalDecodingContext *>(
 ...
 * startup function.
 */
LogicalDecodingContext *CreateInitDecodingContext(
    char *plugin, List *output_plugin_options, XLogPageReadCB read_page,
    LogicalOutputPluginWriterPrepareWrite prepare_write,
    LogicalOutputPluginWriterWrite do_write) {
  ...
  if (IsTransactionState() &&
    ...
    ereport(ERROR, (errcode(ERRCODE_ACTIVE_SQL_TRANSACTION),
                    errmsg(
                        "cannot create logical replication slot in transaction "
  ...
  while (true) {
    ...
    if (XLogGetLastRemovedSegno() < segno) break;
  }
  ...
  ctx = StartupDecodingContext(NIL, InvalidXLogRecPtr, xmin_horizon, read_page,
 ...
 * startup function.
 */
LogicalDecodingContext *CreateDecodingContext(
    XLogRecPtr start_lsn, List *output_plugin_options, XLogPageReadCB read_page,
    LogicalOutputPluginWriterPrepareWrite prepare_write,
    LogicalOutputPluginWriterWrite do_write) {
  LogicalDecodingContext *ctx;
  ReplicationSlot *slot;
  ...
  ctx = StartupDecodingContext(output_plugin_options, start_lsn,
                               InvalidTransactionId, read_page, prepare_write,
                               do_write);
  ...
  ereport(LOG, (errmsg("starting logical decoding for slot \"%s\"",
                       NameStr(slot->data.name)),
                errdetail(
                    "streaming transactions committing after %X/%X, reading "
                    "WAL from %X/%X",
                    (uint32)(slot->data.confirmed_flush >> 32),
                    (uint32)slot->data.confirmed_flush,
...
void DecodingContextFindStartpoint(LogicalDecodingContext *ctx) {
  ...
  for (;;) {
    ...
    if (err) elog(ERROR, "%s", err);
    if (!record) elog(ERROR, "no record found"); /* shouldn't happen */
    ...
    if (DecodingContextReady(ctx)) break;
...
void FreeDecodingContext(LogicalDecodingContext *ctx) {
  if (ctx->callbacks.shutdown_cb != NULL) shutdown_cb_wrapper(ctx);
...
static void LoadOutputPlugin(OutputPluginCallbacks *callbacks, char *plugin) {
  ...
  plugin_init = (LogicalOutputPluginInit)load_external_function(
      plugin, "_PG_output_plugin_init", 0, NULL);
...
static void output_plugin_error_callback(void *arg) {
  ...
  if (state->report_location != InvalidXLogRecPtr)
    errcontext(
        "slot \"%s\", output plugin \"%s\", in the %s callback, associated LSN "
        "%X/%X",
        NameStr(state->ctx->slot->data.name),
        NameStr(state->ctx->slot->data.plugin), state->callback_name,
        (uint32)(state->report_location >> 32), (uint32)state->report_location);
  else
...
static void begin_cb_wrapper(ReorderBuffer *cache, ReorderBufferTXN *txn) {
  LogicalDecodingContext *ctx =
      static_cast<LogicalDecodingContext *>(cache->private_data);
  LogicalErrorCallbackState state;
  ErrorContextCallback errcallback;
...
void LogicalIncreaseXminForSlot(XLogRecPtr current_lsn, TransactionId xmin) {
  ...
  if (updated_xmin) LogicalConfirmReceivedLocation(slot->data.confirmed_flush);
}
...
void LogicalIncreaseRestartDecodingForSlot(XLogRecPtr current_lsn,
  ...
  if (slot->candidate_restart_valid == InvalidXLogRecPtr) {
    ...
    elog(DEBUG1,
         "failed to increase restart lsn: proposed %X/%X, after %X/%X, current "
         "candidate %X/%X, current after %X/%X, flushed up to %X/%X",
         (uint32)(restart_lsn >> 32), (uint32)restart_lsn,
         (uint32)(current_lsn >> 32), (uint32)current_lsn,
  ...
  if (updated_lsn) LogicalConfirmReceivedLocation(slot->data.confirmed_flush);
}
