...
static void SnapBuildEndTxn(SnapBuild *builder, XLogRecPtr lsn,
                            TransactionId xid);
...
SnapBuild *AllocateSnapshotBuilder(ReorderBuffer *reorder,
  ...
  context =
      AllocSetContextCreate(CurrentMemoryContext, "snapshot builder context",
  ...
  builder->committed.xip = static_cast<TransactionId *>(
...
static void SnapBuildFreeSnapshot(Snapshot snap) {
  ...
  if (snap->copied) elog(ERROR, "cannot free a copied snapshot");

  if (snap->active_count) elog(ERROR, "cannot free an active snapshot");
 ...
 */
static void SnapBuildSnapIncRefcount(Snapshot snap) { snap->active_count++; }
...
void SnapBuildSnapDecRefcount(Snapshot snap) {
  ...
  snap->active_count--;
  if (snap->active_count == 0) SnapBuildFreeSnapshot(snap);
}
...
static Snapshot SnapBuildBuildSnapshot(SnapBuild *builder, TransactionId xid) {
  ...
  snapshot =
      static_cast<Snapshot>(MemoryContextAllocZero(builder->context, ssize));
...
const char *SnapBuildExportSnapshot(SnapBuild *builder) {
  ...
  newxip =
      (TransactionId *)palloc(sizeof(TransactionId) * GetMaxSnapshotXidCount());
  ...
  for (xid = snap->xmin; NormalTransactionIdPrecedes(xid, snap->xmax);) {
     ...
     * meaning of ->xip.
     */
    test = bsearch(&xid, snap->xip, snap->xcnt, sizeof(TransactionId),
                   xidComparator);
 ...
 * be decoded.
 */
bool SnapBuildProcessChange(SnapBuild *builder, TransactionId xid,
                            XLogRecPtr lsn) {
  bool is_old_tx;
   ...
   */
  if (builder->state < SNAPBUILD_FULL_SNAPSHOT) return false;
...
static bool SnapBuildTxnIsRunning(SnapBuild *builder, TransactionId xid) {
  ...
  if (builder->running.xcnt &&
    ...
    TransactionId *search = static_cast<TransactionId *>(
...
static void SnapBuildDistributeNewCatalogSnapshot(SnapBuild *builder,
  ...
  dlist_foreach(txn_i, &builder->reorder->toplevel_by_lsn) {
     ...
     * from the subtxn.
     */
    if (!ReorderBufferXidHasBaseSnapshot(builder->reorder, txn->xid)) continue;

    elog(DEBUG2, "adding a new___ snapshot to %u at %X/%X", txn->xid,
...
static void SnapBuildAddCommittedTxn(SnapBuild *builder, TransactionId xid) {
  ...
  if (builder->committed.xcnt == builder->committed.xcnt_space) {
    ...
    builder->committed.xip = static_cast<TransactionId *>(
        repalloc(builder->committed.xip,
...
static void SnapBuildPurgeCommittedTxn(SnapBuild *builder) {
  ...
  /* not ready yet */
  if (!TransactionIdIsNormal(builder->xmin)) return;

  /* TODO: Neater algorithm than just copying and iterating? */
  workspace = static_cast<TransactionId *>(MemoryContextAlloc(
      builder->context, builder->committed.xcnt * sizeof(TransactionId)));
  ...
  elog(DEBUG3,
       ...
       (uint32)builder->committed.xcnt, (uint32)surviving_xids, builder->xmin,
       builder->xmax);
...
static void SnapBuildEndTxn(SnapBuild *builder, XLogRecPtr lsn,
                            TransactionId xid) {
  if (builder->state == SNAPBUILD_CONSISTENT) return;
 ...
 */
void SnapBuildAbortTxn(SnapBuild *builder, XLogRecPtr lsn, TransactionId xid,
...
void SnapBuildCommitTxn(SnapBuild *builder, XLogRecPtr lsn, TransactionId xid,
  ...
  if (builder->state < SNAPBUILD_CONSISTENT) {
    ...
    if (builder->start_decoding_at <= lsn) builder->start_decoding_at = lsn + 1;
    ...
    elog(DEBUG1,
         "forced to assume catalog changes for xid %u because it was running "
         "to early",
         xid);
  }
  ...
  for (nxact = 0; nxact < nsubxacts; nxact++) {
    ...
    if (forced_timetravel) {
      ...
      if (NormalTransactionIdFollows(subxid, xmax)) xmax = subxid;
    }
     ...
     */
      ...
      elog(DEBUG1, "found subtransaction %u:%u with catalog changes.", xid,
           subxid);
  ...
  if (forced_timetravel || top_needs_timetravel || sub_needs_timetravel) {
     ...
     */
    if (builder->state < SNAPBUILD_FULL_SNAPSHOT) return;
     ...
     */
    if (builder->snapshot) SnapBuildSnapDecRefcount(builder->snapshot);
 ...
 * anymore.
 */
void SnapBuildProcessRunningXacts(SnapBuild *builder, XLogRecPtr lsn,
                                  xl_running_xacts *running) {
  ReorderBufferTXN *txn;
  ...
  if (builder->state < SNAPBUILD_CONSISTENT) {
    ...
    if (!SnapBuildFindSnapshot(builder, lsn, running)) return;
  } else
  ...
  elog(DEBUG3, "xmin: %u, xmax: %u, oldestrunning: %u", builder->xmin,
       builder->xmax, running->oldestRunningXid);
   ...
   */
           ...
           builder->reorder->current_restart_decoding_lsn !=
               InvalidXLogRecPtr &&
...
static bool SnapBuildFindSnapshot(SnapBuild *builder, XLogRecPtr lsn,
  ...
  if (TransactionIdIsNormal(builder->initial_xmin_horizon) &&
    ...
    ereport(DEBUG1,
            (errmsg_internal(
                 "skipping snapshot at %X/%X while building logical decoding "
                 "snapshot, xmin horizon too low",
                 (uint32)(lsn >> 32), (uint32)lsn),
             errdetail_internal(
   ...
   */
    ...
    builder->running.xip = static_cast<TransactionId *>(MemoryContextAlloc(
        builder->context, builder->running.xcnt * sizeof(TransactionId)));
    memcpy(builder->running.xip, running->xids,
           builder->running.xcnt * sizeof(TransactionId));

    /* sort so we can do a binary search */
    qsort(builder->running.xip, builder->running.xcnt, sizeof(TransactionId),
          xidComparator);
...
static void SnapBuildSerialize(SnapBuild *builder, XLogRecPtr lsn) {
   ...
   */
  sprintf(path, "pg_logical/snapshots/%X-%X.snap", (uint32)(lsn >> 32),
          (uint32)lsn);
  ...
  ondisk_c = static_cast<char *>(
      MemoryContextAllocZero(builder->context, needed_length));
...
static bool SnapBuildRestore(SnapBuild *builder, XLogRecPtr lsn) {
  ...
  if (builder->state == SNAPBUILD_CONSISTENT) return false;
  ...
  if (ondisk.version != SNAPBUILD_VERSION)
    ereport(ERROR, (errmsg(
                       "snapbuild state file \"%s\" has unsupported version %u "
                       "instead of %u",
                       path, ondisk.version, SNAPBUILD_VERSION)));

  INIT_CRC32C(checksum);
  COMP_CRC32C(checksum, ((char *)&ondisk) + SnapBuildOnDiskNotChecksummedSize,
              SnapBuildOnDiskConstantSize - SnapBuildOnDiskNotChecksummedSize);
  ...
  ondisk.builder.running.xip = static_cast<TransactionId *>(
  ...
  ondisk.builder.committed.xip = static_cast<TransactionId *>(
      MemoryContextAllocZero(builder->context, sz));
  ...
  if (builder->running.xip) pfree(builder->running.xip);
...
snapshot_not_interesting:
  if (ondisk.builder.running.xip != NULL) pfree(ondisk.builder.running.xip);
  if (ondisk.builder.committed.xip != NULL) pfree(ondisk.builder.committed.xip);
  return false;
}
