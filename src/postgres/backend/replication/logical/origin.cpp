...
/* external variables */
RepOriginId replorigin_sesssion_origin =
    InvalidRepOriginId; /* assumed identity */
...
RepOriginId replorigin_by_name(char *roname, bool missing_ok) {
  ...
  if (HeapTupleIsValid(tuple)) {
    ...
    elog(ERROR, "cache lookup failed for replication origin '%s'", roname);
...
RepOriginId replorigin_create(char *roname) {
  ...
  for (roident = InvalidOid + 1; roident < PG_UINT16_MAX; roident++) {
    ...
    ScanKeyInit(&key, Anum_pg_replication_origin_roident, BTEqualStrategyNumber,
                F_OIDEQ, ObjectIdGetDatum(roident));

    scan = systable_beginscan(rel, ReplicationOriginIdentIndex,
                              true /* indexOK */, &SnapshotDirty, 1, &key);
      ...
      if (state->acquired_by != 0) {
        ereport(
            ...
            (errcode(ERRCODE_OBJECT_IN_USE),
             errmsg(
                 "cannot drop replication origin with oid %d, in use by pid %d",
...
Size ReplicationOriginShmemSize(void) {
   ...
   */
  if (max_replication_slots == 0) return size;

  size = add_size(size, offsetof(ReplicationStateCtl, states));

  size =
      add_size(size, mul_size(max_replication_slots, sizeof(ReplicationState)));
  return size;
}
...
void ReplicationOriginShmemInit(void) {
  ...
  replication_states_ctl = (ReplicationStateCtl *)ShmemInitStruct(
      "ReplicationOriginState", ReplicationOriginShmemSize(), &found);
  ...
  if (!found) {
    ...
    replication_states_ctl->tranche.array_base = &replication_states[0].lock;
    replication_states_ctl->tranche.array_stride = sizeof(ReplicationState);
...
/* don't want to overwrite already existing state */
  ...
  while (true) {
    ...
    if (readBytes != sizeof(disk_state)) {
      ereport(PANIC, (errcode_for_file_access(),
                      errmsg("could not read file \"%s\": read %d of %zu", path,
                             readBytes, sizeof(disk_state))));
    }
    ...
    elog(LOG, "recovered replication state of node %u to %X/%X",
         disk_state.roident, (uint32)(disk_state.remote_lsn >> 32),
  ...
  if (file_crc != crc)
    ereport(
        ...
        (errcode(ERRCODE_CONFIGURATION_LIMIT_EXCEEDED),
         errmsg(
             "replication_slot_checkpoint has wrong checksum %u, expected %u",
             crc, file_crc)));
 ...
 * setting up replorigin_sesssion_origin_lsn and replorigin_sesssion_origin that
 *ensures we
...
void replorigin_advance(RepOriginId node, XLogRecPtr remote_commit,
                        XLogRecPtr local_commit, bool go_backward,
                        bool wal_log) {
  int i;
  ...
  if (node == DoNotReplicateId) return;
  ...
  if (replication_state == NULL && free_state == NULL)
    ereport(ERROR, (errcode(ERRCODE_CONFIGURATION_LIMIT_EXCEEDED),
                    errmsg(
                        "no free replication state slot could be found for "
...
Datum pg_replication_origin_oid(PG_FUNCTION_ARGS) {
  ...
  if (OidIsValid(roident)) PG_RETURN_OID(roident);
  PG_RETURN_NULL();
}
...
Datum pg_replication_origin_session_progress(PG_FUNCTION_ARGS) {
  ...
  if (remote_lsn == InvalidXLogRecPtr) PG_RETURN_NULL();
...
#define REPLICATION_ORIGIN_PROGRESS_COLS 4
  ...
  for (i = 0; i < max_replication_slots; i++) {
    ...
    if (state->roident == InvalidRepOriginId) continue;
