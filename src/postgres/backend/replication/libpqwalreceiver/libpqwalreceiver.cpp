...
void _PG_init(void) {
  ...
  if (walrcv_connect != NULL || walrcv_identify_system != NULL ||
      walrcv_readtimelinehistoryfile != NULL || walrcv_startstreaming != NULL ||
      walrcv_endstreaming != NULL || walrcv_receive != NULL ||
...
static void libpqrcv_connect(char *conninfo) {
  ...
  if (PQstatus(streamConn) != CONNECTION_OK)
    ereport(ERROR, (errmsg("could not connect to the primary server: %s",
...
static void libpqrcv_identify_system(TimeLineID *primary_tli) {
  ...
  if (PQnfields(res) < 3 || PQntuples(res) != 1) {
    ...
    ereport(ERROR, (errmsg("invalid response from primary server"),
                    errdetail(
                        "Could not identify system: got %d rows and %d fields, "
  ...
  *primary_tli = pg_atoi(PQgetvalue(res, 0, 1), 4, 0);
  ...
  if (strcmp(primary_sysid, standby_sysid) != 0) {
    ...
    ereport(
        ERROR,
        (errmsg(
             "database system identifier differs between the primary and "
             "standby"),
         errdetail(
...
static bool libpqrcv_startstreaming(TimeLineID tli, XLogRecPtr startpoint,
  ...
  if (PQresultStatus(res) == PGRES_COMMAND_OK) {
    ...
    PQclear(res);
    ereport(ERROR, (errmsg("could not start WAL streaming: %s",
                           PQerrorMessage(streamConn))));
  }
  PQclear(res);
  return true;
...
static bool libpq_select(int timeout_ms) {
  ...
  /* We use poll(2) if available, otherwise select(2) */
  {
    ...
    ret =
        select(PQsocket(streamConn) + 1, &input_mask, NULL, NULL, ptr_timeout);
#endif /* HAVE_POLL */
  }

  if (ret == 0 || (ret < 0 && errno == EINTR)) return false;
  if (ret < 0)
    ereport(ERROR,
            (errcode_for_socket_access(), errmsg("select() failed: %m")));
  return true;
}
...
static PGresult *libpqrcv_PQexec(const char *query) {
   ...
   */
  if (!PQsendQuery(streamConn, query)) return NULL;
  ...
  for (;;) {
    ...
    while (PQisBusy(streamConn)) {
       ...
       */
      if (!libpq_select(-1)) continue;                  /* interrupted */
      if (PQconsumeInput(streamConn) == 0) return NULL; /* trouble */
    }
    ...
    if (result == NULL) break; /* query is complete */
...
static int libpqrcv_receive(int timeout, char **buffer) {
  ...
  if (recvBuf != NULL) PQfreemem(recvBuf);
  recvBuf = NULL;
  ...
  if (rawlen == 0) {
     ...
     */
    if (timeout > 0) {
      if (!libpq_select(timeout)) return 0;
    }
 ...
 */
static void libpqrcv_send(const char *buffer, int nbytes) {
  if (PQputCopyData(streamConn, buffer, nbytes) <= 0 || PQflush(streamConn))
