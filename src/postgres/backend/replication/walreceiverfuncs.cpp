...
void RequestXLogStreaming(TimeLineID tli, XLogRecPtr recptr,
   ...
   */
  if (recptr % XLogSegSize != 0) recptr -= recptr % XLogSegSize;
...
XLogRecPtr GetWalRcvWriteRecPtr(XLogRecPtr *latestChunkStart,
  ...
  if (latestChunkStart) *latestChunkStart = walrcv->latestChunkStart;
  if (receiveTLI) *receiveTLI = walrcv->receivedTLI;
  SpinLockRelease(&walrcv->mutex);
...
int GetReplicationApplyDelay(void) {
  ...
  if (receivePtr == replayPtr) return 0;

  chunckReplayStartTime = GetCurrentChunkReplayStartTime();

  if (chunckReplayStartTime == 0) return -1;

  TimestampDifference(chunckReplayStartTime, GetCurrentTimestamp(), &secs,
                      &usecs);
