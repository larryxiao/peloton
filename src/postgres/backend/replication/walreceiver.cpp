...
void WalReceiverMain(void) {
  ...
  if (walrcv_connect == NULL || walrcv_startstreaming == NULL ||
      walrcv_endstreaming == NULL || walrcv_identify_system == NULL ||
      walrcv_readtimelinehistoryfile == NULL || walrcv_receive == NULL ||
  ...
  for (;;) {
    ...
    if (primaryTLI < startpointTLI)
      ereport(ERROR, (errmsg(
                         "highest timeline %u of the primary is behind "
                         "recovery timeline %u",
    ...
    if (walrcv_startstreaming(startpointTLI, startpoint,
      ...
      if (first_stream)
        ...
        ereport(LOG, (errmsg("restarted WAL streaming at %X/%X on timeline %u",
                             (uint32)(startpoint >> 32), (uint32)startpoint,
                             startpointTLI)));
      first_stream = false;
      ...
      while (!endofwal) {
        ...
        if (!RecoveryInProgress())
          ereport(FATAL, (errmsg(
                             "cannot continue WAL streaming, recovery has "
                             "already ended")));
...
static void WalRcvWaitForStartPosition(XLogRecPtr *startpoint,
  ...
  if (update_process_title) set_ps_display("idle", false);
...
static void WalRcvFetchTimeLineHistoryFiles(TimeLineID first, TimeLineID last) {
  ...
  for (tli = first; tli <= last; tli++) {
    ...
    if (tli != 1 && !existsTimeLineHistory(tli)) {
      ...
      ereport(LOG, (errmsg(
                       "fetching timeline history file for timeline %u from "
                       "primary server",
                       tli)));
      ...
      if (strcmp(fname, expectedfname) != 0)
        ereport(ERROR, (errcode(ERRCODE_PROTOCOL_VIOLATION),
                        errmsg_internal(
                            "primary reported unexpected file name for "
                            "timeline history file of timeline %u",
                            tli)));
...
static void WalRcvDie(int code, Datum arg) {
  ...
  if (walrcv_disconnect != NULL) walrcv_disconnect();
...
static void WalRcvSigHupHandler(SIGNAL_ARGS) { got_SIGHUP = true; }
...
static void XLogWalRcvProcessMsg(unsigned char type, char *buf, Size len) {
  ...
  switch (type) {
    case 'w': /* WAL records */
    {
      ...
      sendTime =
          IntegerTimestampToTimestampTz(pq_getmsgint64(&incoming_message));
...
static void XLogWalRcvWrite(char *buf, Size nbytes, XLogRecPtr recptr) {
  ...
  while (nbytes > 0) {
        ...
        ereport(PANIC,
                (errcode_for_file_access(),
                 errmsg("could not seek in log segment %s to offset %u: %m",
                        XLogFileNameP(recvFileTLI, recvSegNo), startoff)));
      recvOff = startoff;
    }
    ...
    if (byteswritten <= 0) {
      ...
      ereport(PANIC, (errcode_for_file_access(),
                      errmsg(
                          "could not write to log segment %s "
                          "at offset %u, length %lu: %m",
                          XLogFileNameP(recvFileTLI, recvSegNo), recvOff,
...
static void XLogWalRcvSendReply(bool force, bool requestReply) {
   ...
   * probably OK.
   */
  if (!force && writePtr == LogstreamResult.Write &&
...
static void XLogWalRcvSendHSFeedback(bool immed) {
  ...
  if (nextXid < xmin) nextEpoch--;

  elog(DEBUG2, "sending hot standby feedback xmin %u epoch %u", xmin,
       nextEpoch);
...
static void ProcessWalSndrMessage(XLogRecPtr walEnd, TimestampTz sendTime) {
  ...
  /* Update shared-memory status */
  SpinLockAcquire(&walrcv->mutex);
  if (walrcv->latestWalEnd < walEnd) walrcv->latestWalEndTime = sendTime;
  ...
  if (log_min_messages <= DEBUG2) {
    ...
    if (applyDelay == -1)
      elog(DEBUG2,
           "sendtime %s receipttime %s replication apply delay (N/A) transfer "
           "latency %d ms",
           sendtime, receipttime, GetReplicationTransferLatency());
    else
      elog(DEBUG2,
           "sendtime %s receipttime %s replication apply delay %d ms transfer "
           "latency %d ms",
