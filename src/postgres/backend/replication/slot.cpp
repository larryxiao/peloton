      ...
      ereport(elevel,
              (errcode(ERRCODE_INVALID_NAME),
               errmsg("replication slot name \"%s\" contains invalid character",
                      name),
               errhint(
                   "Replication slot names may only contain letters, numbers, "
...
void ReplicationSlotAcquire(const char *name) {
  ...
  for (i = 0; i < max_replication_slots; i++) {
    ...
    if (s->in_use && strcmp(name, NameStr(s->data.name)) == 0) {
      ...
      if (active_pid == 0) vslot->active_pid = MyProcPid;
...
static void ReplicationSlotDropAcquired(void) {
  ...
  if (rename(path, tmppath) == 0) {
    ...
    ereport(
        fail_softly ? WARNING : ERROR,
        (errcode_for_file_access(),
         errmsg("could not rename file \"%s\" to \"%s\": %m", path, tmppath)));
  }
...
void ReplicationSlotsComputeRequiredXmin(bool already_locked) {
  ...
  if (!already_locked) LWLockAcquire(ReplicationSlotControlLock, LW_SHARED);
  ...
  if (!already_locked) LWLockRelease(ReplicationSlotControlLock);
...
XLogRecPtr ReplicationSlotsComputeLogicalRestartLSN(void) {
  ...
  if (max_replication_slots <= 0) return InvalidXLogRecPtr;
  ...
  for (i = 0; i < max_replication_slots; i++) {
    ...
    if (s->data.database == InvalidOid) continue;
...
bool ReplicationSlotsCountDBSlots(Oid dboid, int *nslots, int *nactive) {
  ...
  *nslots = *nactive = 0;
  ...
  if (max_replication_slots <= 0) return false;
  ...
  for (i = 0; i < max_replication_slots; i++) {
    ...
    if (s->data.database != dboid) continue;

    /* count slots with spinlock held */
    SpinLockAcquire(&s->mutex);
    (*nslots)++;
    if (s->active_pid != 0) (*nactive)++;
    SpinLockRelease(&s->mutex);
  }
  LWLockRelease(ReplicationSlotControlLock);
...
void CheckSlotRequirements(void) {
  if (max_replication_slots == 0)
    ereport(ERROR, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                    (errmsg(
                        "replication slots can only be used if "
...
static void CreateSlotOnDisk(ReplicationSlot *slot) {
   ...
   */
  if (stat(tmppath, &st) == 0 && S_ISDIR(st.st_mode)) rmtree(tmppath, true);
...
static void SaveSlotToPath(ReplicationSlot *slot, const char *dir, int elevel) {
  ...
  COMP_CRC32C(cp.checksum, (char *)(&cp) + SnapBuildOnDiskNotChecksummedSize,
   ...
   */
  {
    ...
    SpinLockAcquire(&vslot->mutex);
    if (!vslot->just_dirtied) vslot->dirty = false;
    SpinLockRelease(&vslot->mutex);
  }
...
static void RestoreSlotFromDisk(const char *name) {
  ...
  if (readBytes != ReplicationSlotOnDiskConstantSize) {
    ...
    ereport(PANIC,
            (errcode_for_file_access(),
             errmsg("could not read file \"%s\", read %d of %u: %m", path,
                    readBytes, (uint32)ReplicationSlotOnDiskConstantSize)));
  }
  ...
  readBytes =
      read(fd, (char *)&cp + ReplicationSlotOnDiskConstantSize, cp.length);
  ...
  COMP_CRC32C(checksum, (char *)&cp + SnapBuildOnDiskNotChecksummedSize,
              SnapBuildOnDiskChecksummedSize);
  FIN_CRC32C(checksum);
