...
void InitWalSender(void) {
  ...
  CurrentResourceOwner =
      ResourceOwnerCreate(NULL, "walsender top-level resource owner");
...
void WalSndErrorCleanup() {
  ...
  if (walsender_ready_to_stop) proc_exit(0);
...
static void StartReplication(StartReplicationCmd *cmd) {
  ...
  if (cmd->slotname) {
    ...
    if (MyReplicationSlot->data.database != InvalidOid)
      ereport(ERROR, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                      (errmsg(
                          "cannot use a logical replication slot for physical "
                          "replication"))));
  }
  ...
  if (!sendTimeLineIsHistoric || cmd->startpoint < sendTimeLineValidUpto) {
    ...
    if (FlushPtr < cmd->startpoint) {
      ereport(ERROR,
              (errmsg(
                  "requested starting point %X/%X is ahead of the WAL flush "
                  "position of this server %X/%X",
                  (uint32)(cmd->startpoint >> 32), (uint32)(cmd->startpoint),
...
static int logical_read_xlog_page(XLogReaderState *state,
  ...
  if (targetPagePtr + XLOG_BLCKSZ <= flushptr) count = XLOG_BLCKSZ;
...
static void CreateReplicationSlot(CreateReplicationSlotCmd *cmd) {
  ...
  if (cmd->kind == REPLICATION_KIND_LOGICAL) {
    ...
    ctx = CreateInitDecodingContext(cmd->plugin, NIL, logical_read_xlog_page,
...
static void StartLogicalReplication(StartReplicationCmd *cmd) {
  ...
  logical_decoding_ctx = CreateDecodingContext(
      cmd->startpoint, cmd->options, logical_read_xlog_page, WalSndPrepareWrite,
      WalSndWriteData);
...
static void WalSndPrepareWrite(LogicalDecodingContext *ctx, XLogRecPtr lsn,
  ...
  if (!last_write) lsn = InvalidXLogRecPtr;
...
static void WalSndWriteData(LogicalDecodingContext *ctx, XLogRecPtr lsn,
  ...
  memcpy(&ctx->out->data[1 + sizeof(int64) + sizeof(int64)], tmpbuf.data,
         sizeof(int64));

  /* fast path */
  /* Try to flush pending output to the client */
  if (pq_flush_if_writable() != 0) WalSndShutdown();

  if (!pq_is_send_pending()) return;
  ...
  for (;;) {
    ...
    wakeEvents = WL_LATCH_SET | WL_POSTMASTER_DEATH | WL_SOCKET_WRITEABLE |
                 WL_SOCKET_READABLE | WL_TIMEOUT;

    /* Sleep until something happens or we time out */
    WaitLatchOrSocket(MyLatch, wakeEvents, MyProcPort->sock, sleeptime);
  }
...
static XLogRecPtr WalSndWaitForWal(XLogRecPtr loc) {
  ...
  for (;;) {
     ...
     * down.
     */
    if (walsender_ready_to_stop) break;
     ...
     */
    if (MyWalSnd->flush < sentPtr && MyWalSnd->write < sentPtr &&
...
void exec_replication_command(const char *cmd_string) {
  ...
  cmd_context =
      AllocSetContextCreate(CurrentMemoryContext, "Replication command context",
  ...
  for (;;) {
    ...
    if (streamingDoneReceiving && firstchar != 'X')
      ereport(FATAL, (errcode(ERRCODE_PROTOCOL_VIOLATION),
                      errmsg(
                          "unexpected standby message type \"%c\", after "
                          "receiving CopyDone",
                          firstchar)));
...
static void ProcessStandbyReplyMessage(void) {
  ...
  elog(DEBUG2, "write %X/%X flush %X/%X apply %X/%X%s",
       (uint32)(writePtr >> 32), (uint32)writePtr, (uint32)(flushPtr >> 32),
       (uint32)flushPtr, (uint32)(applyPtr >> 32), (uint32)applyPtr,
       replyRequested ? " (reply requested)" : "");

  /* Send a reply if the standby requested one. */
  if (replyRequested) WalSndKeepalive(false);
  ...
  if (!am_cascading_walsender) SyncRepReleaseWaiters();
...
static void ProcessStandbyHSFeedbackMessage(void) {
  ...
  elog(DEBUG2, "hot standby feedback xmin %u epoch %u", feedbackXmin,
       feedbackEpoch);

  /* Unset WalSender's xmin if the feedback message value is invalid */
  if (!TransactionIdIsNormal(feedbackXmin)) {
    MyPgXact->xmin = InvalidTransactionId;
    if (MyReplicationSlot != NULL) PhysicalReplicationSlotNewXmin(feedbackXmin);
    return;
  }
  ...
  if (feedbackXmin <= nextXid) {
    if (feedbackEpoch != nextEpoch) return;
  } else {
    if (feedbackEpoch + 1 != nextEpoch) return;
  }
...
static long WalSndComputeSleeptime(TimestampTz now) {
  ...
  if (wal_sender_timeout > 0 && last_reply_timestamp > 0) {
     ...
     */
    wakeup_time =
        TimestampTzPlusMilliseconds(last_reply_timestamp, wal_sender_timeout);
    ...
    TimestampDifference(now, wakeup_time, &sec_to_timeout,
                        &microsec_to_timeout);
...
static void WalSndCheckTimeOut(TimestampTz now) {
  ...
  if (last_reply_timestamp <= 0) return;
...
static void WalSndLoop(WalSndSendDataCallback send_data) {
  ...
  for (;;) {
    ...
    if (WalSndCaughtUp && !pq_is_send_pending()) {
       ...
       * is not sure which.
       */
      if (walsender_ready_to_stop) WalSndDone(send_data);
    }
  ...
  while (nbytes > 0) {
    ...
    if (sendFile < 0 || !XLByteInSeg(recptr, sendSegNo)) {
      ...
      if (sendTimeLineIsHistoric) {
        ...
        if (sendSegNo == endSegNo) curFileTimeLine = sendTimeLineNextTLI;
      }
        ...
        ereport(ERROR,
                (errcode_for_file_access(),
                 errmsg("could not seek in log segment %s to offset %u: %m",
                        XLogFileNameP(curFileTimeLine, sendSegNo), startoff)));
      sendOff = startoff;
    }
      ...
      ereport(
          ...
          (errcode_for_file_access(),
           errmsg(
               ...
               XLogFileNameP(curFileTimeLine, sendSegNo), sendOff,
...
static void XLogSendPhysical(void) {
  ...
  if (sendTimeLineIsHistoric) {
    ...
    if (!RecoveryInProgress()) {
       ...
       */
      if (sendTimeLine != ThisTimeLineID) becameHistoric = true;
    }
    ...
    if (becameHistoric) {
      ...
      sendTimeLineValidUpto =
          tliSwitchPoint(sendTimeLine, history, &sendTimeLineNextTLI);
  ...
  resetStringInfo(&tmpbuf);
  pq_sendint64(&tmpbuf, GetCurrentIntegerTimestamp());
  memcpy(&output_message.data[1 + sizeof(int64) + sizeof(int64)], tmpbuf.data,
         sizeof(int64));
...
static void XLogSendLogical(void) {
  ...
  record =
      XLogReadRecord(logical_decoding_ctx->reader, logical_startptr, &errm);
  logical_startptr = InvalidXLogRecPtr;

  /* xlog record was invalid */
  if (errm != NULL) elog(ERROR, "%s", errm);
...
static void WalSndDone(WalSndSendDataCallback send_data) {
   ...
   */
  replicatedPtr =
      XLogRecPtrIsInvalid(MyWalSnd->flush) ? MyWalSnd->write : MyWalSnd->flush;
  ...
  if (!waiting_for_ping_response) WalSndKeepalive(true);
}
...
static void WalSndLastCycleHandler(SIGNAL_ARGS) {
   ...
   */
  if (!replication_active) kill(MyProcPid, SIGTERM);
...
#define PG_STAT_GET_WAL_SENDERS_COLS 8
  ...
  for (i = 0; i < max_wal_senders; i++) {
    ...
    if (!superuser()) {
      ...
      if (write == 0) nulls[3] = true;
      values[3] = LSNGetDatum(write);

      if (flush == 0) nulls[4] = true;
      values[4] = LSNGetDatum(flush);

      if (apply == 0) nulls[5] = true;
...
static void WalSndKeepaliveIfNecessary(TimestampTz now) {
  ...
  if (waiting_for_ping_response) return;
   ...
   */
  ping_time =
      TimestampTzPlusMilliseconds(last_reply_timestamp, wal_sender_timeout / 2);
  if (now >= ping_time) {
...
XLogRecPtr GetOldestWALSendPointer(void) {
  ...
  for (i = 0; i < max_wal_senders; i++) {
    ...
    if (recptr.xlogid == 0 && recptr.xrecoff == 0) continue;

    if (!found || recptr < oldest) oldest = recptr;
    found = true;
  }
  return oldest;
}
