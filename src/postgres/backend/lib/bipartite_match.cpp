...
BipartiteMatchState *BipartiteMatch(int u_size, int v_size, short **adjacency) {
  BipartiteMatchState *state =
      static_cast<BipartiteMatchState *>(palloc(sizeof(BipartiteMatchState)));
  ...
  state->pair_uv =
      static_cast<short int *>(palloc0((u_size + 1) * sizeof(short)));
  state->pair_vu =
      static_cast<short int *>(palloc0((v_size + 1) * sizeof(short)));
  ...
  while (hk_breadth_search(state)) {
    ...
    for (u = 1; u <= u_size; ++u)
      if (state->pair_uv[u] == 0)
        if (hk_depth_search(state, u, 1)) state->matching++;
