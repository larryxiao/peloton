...
typedef ULONG (*__ldap_start_tls_sA)(IN PLDAP ExternalHandle,
...
static void auth_failed(Port *port, int status, char *logdetail) {
   ...
   * events.)
   */
  if (status == STATUS_EOF) proc_exit(0);
  ...
  switch (port->hba->auth_method) {
    ...
    case uaCert:
      errstr =
          gettext_noop("certificate authentication failed for user \"%s\"");
      break;
    case uaRADIUS:
    ...
    default:
      errstr = gettext_noop(
          "authentication failed for user \"%s\": invalid authentication "
          "method");
      break;
  }
    ...
    case uaReject:
       ...
       */
      {
        ...
        pg_getnameinfo_all(&port->raddr.addr, port->raddr.salen, hostinfo,
                           sizeof(hostinfo), NULL, 0, NI_NUMERICHOST);

        if (am_walsender) {
#ifdef USE_SSL
          ereport(FATAL, (errcode(ERRCODE_INVALID_AUTHORIZATION_SPECIFICATION),
                          errmsg(
                              "pg_hba.conf rejects replication connection for "
                              "host \"%s\", user \"%s\", %s",
          ...
          ereport(FATAL, (errcode(ERRCODE_INVALID_AUTHORIZATION_SPECIFICATION),
                          errmsg(
                              "pg_hba.conf rejects connection for host \"%s\", "
                              "user \"%s\", database \"%s\", %s",
...
#define HOSTNAME_LOOKUP_DETAIL(port)                                           \
  (port->remote_hostname                                                       \
       ? (port->remote_hostname_resolv == +1                                   \
              ? errdetail_log(                                                 \
                    "Client IP address resolved to \"%s\", forward lookup "    \
                    "matches.",                                                \
                    port->remote_hostname)                                     \
              : port->remote_hostname_resolv == 0                              \
                    ? errdetail_log(                                           \
                          "Client IP address resolved to \"%s\", forward "     \
                          "lookup not checked.",                               \
                          port->remote_hostname)                               \
                    : port->remote_hostname_resolv == -1                       \
                          ? errdetail_log(                                     \
                                "Client IP address resolved to \"%s\", "       \
                                "forward lookup does not match.",              \
                                port->remote_hostname)                         \
                          : port->remote_hostname_resolv == -2                 \
                                ? errdetail_log(                               \
                                      "Could not translate client host name "  \
                                      "\"%s\" to IP address: %s.",             \
                                      port->remote_hostname,                   \
                                      gai_strerror(                            \
       ...
       : (port->remote_hostname_resolv == -2                                   \
              ? errdetail_log(                                                 \
                    "Could not resolve client IP address to a host name: %s.", \
        ...
        if (am_walsender) {
          ...
          ereport(FATAL, (errcode(ERRCODE_INVALID_AUTHORIZATION_SPECIFICATION),
                          errmsg(
                              "no pg_hba.conf entry for host \"%s\", user "
                              "\"%s\", database \"%s\", %s",
                              hostinfo, port->user_name, port->database_name,
                              port->ssl_in_use ? _("SSL on") : _("SSL off")),
                          HOSTNAME_LOOKUP_DETAIL(port)));
...
static void sendAuthRequest(Port *port, AuthRequest areq) {
  ...
  if (areq == AUTH_REQ_MD5) pq_sendbytes(&buf, port->md5Salt, 4);
...
static int recv_and_check_password_packet(Port *port, char **logdetail) {
  ...
  if (passwd == NULL) return STATUS_EOF; /* client wouldn't send password */
...
static void pg_GSS_error(int severity, char *errmsg, OM_uint32 maj_stat,
                         OM_uint32 min_stat) {
  ...
  msg_ctx = 0;
  gss_display_status(&lmin_s, maj_stat, GSS_C_GSS_CODE, GSS_C_NO_OID, &msg_ctx,
                     &gmsg);
  ...
  msg_ctx = 0;
  gss_display_status(&lmin_s, min_stat, GSS_C_MECH_CODE, GSS_C_NO_OID, &msg_ctx,
                     &gmsg);
   ...
   */
  ereport(severity, (errmsg_internal("%s", errmsg),
...
static int pg_GSS_recvauth(Port *port) {
  ...
  do {
    ...
    maj_stat = gss_accept_sec_context(
        &min_stat, &port->gss->ctx, port->gss->cred, &gbuf,
        GSS_C_NO_CHANNEL_BINDINGS, &port->gss->name, NULL, &port->gss->outbuf,
        &gflags, NULL, NULL);
    ...
    if (maj_stat != GSS_S_COMPLETE && maj_stat != GSS_S_CONTINUE_NEEDED) {
      ...
      pg_GSS_error(ERROR, gettext_noop("accepting GSS security context failed"),
                   maj_stat, min_stat);
    }

    if (maj_stat == GSS_S_CONTINUE_NEEDED) elog(DEBUG4, "GSS continue needed");
  ...
  if (maj_stat != GSS_S_COMPLETE)
    pg_GSS_error(ERROR, gettext_noop("retrieving GSS user name failed"),
                 maj_stat, min_stat);
...
static int pg_SSPI_recvauth(Port *port) {
   ...
   */
  r = AcquireCredentialsHandle(NULL, "negotiate", SECPKG_CRED_INBOUND, NULL,
                               NULL, NULL, NULL, &sspicred, &expiry);
  if (r != SEC_E_OK)
  ...
  do {
    ...
    if (sspictx == NULL) {
      ...
      if (sspictx == NULL) ereport(ERROR, (errmsg("out of memory")));
    }

    memcpy(sspictx, &newctx, sizeof(CtxtHandle));

    if (r == SEC_I_CONTINUE_NEEDED) elog(DEBUG4, "SSPI continue needed");
  ...
  if (_QuerySecurityContextToken == NULL) {
    ...
    ereport(ERROR, (errmsg_internal(
                       "could not locate QuerySecurityContextToken in "
                       "secur32.dll: error code %lu",
                       GetLastError())));
  }
  ...
  if (r != SEC_E_OK) {
    FreeLibrary(secur32);
    pg_SSPI_error(ERROR, _("could not get token from SSPI security context"),
                  r);
  }
  ...
  if (tokenuser == NULL) ereport(ERROR, (errmsg("out of memory")));

  if (!GetTokenInformation(token, TokenUser, tokenuser, retlen, &retlen))
    ereport(ERROR, (errmsg_internal("could not get user token: error code %lu",
                                    GetLastError())));

  if (!LookupAccountSid(NULL, tokenuser->User.Sid, accountname,
                        &accountnamesize, domainname, &domainnamesize,
                        &accountnameuse))
    ereport(ERROR,
  ...
  if (port->hba->krb_realm && strlen(port->hba->krb_realm)) {
    if (pg_strcasecmp(port->hba->krb_realm, domainname) != 0) {
      elog(DEBUG2, "SSPI domain (%s) and configured domain (%s) don't match",
  ...
  if (port->hba->include_realm) {
    ...
    return check_usermap(port->hba->usermap, port->user_name, accountname,
                         true);
}
#endif /* ENABLE_SSPI */
...
static bool interpret_ident_response(const char *ident_response,
  ...
  if (strlen(ident_response) < 2)
    ...
    if (*cursor != ':')
      ...
      while (pg_isblank(*cursor)) cursor++; /* skip blanks */
      i = 0;
...
static int ident_inet(hbaPort *port) {
   ...
   */
  pg_getnameinfo_all(&remote_addr.addr, remote_addr.salen, remote_addr_s,
                     sizeof(remote_addr_s), remote_port, sizeof(remote_port),
                     NI_NUMERICHOST | NI_NUMERICSERV);
  pg_getnameinfo_all(&local_addr.addr, local_addr.salen, local_addr_s,
                     sizeof(local_addr_s), local_port, sizeof(local_port),
                     NI_NUMERICHOST | NI_NUMERICSERV);
  ...
  if (rc < 0) {
    ereport(LOG, (errcode_for_socket_access(),
                  errmsg(
                      "could not send query to Ident server at address \"%s\", "
                      "port %s: %m",
                      remote_addr_s, ident_port)));
    ident_return = false;
    goto ident_inet_done;
...
ident_inet_done:
  ...
  if (ident_serv) pg_freeaddrinfo_all(remote_addr.addr.ss_family, ident_serv);
  if (la) pg_freeaddrinfo_all(local_addr.addr.ss_family, la);

  if (ident_return) /* Success! Check the usermap */
...
static int auth_peer(hbaPort *port) {
  ...
  pw = getpwuid(uid);
  if (!pw) {
    ereport(LOG, (errmsg("could not look up local user ID %ld: %s", (long)uid,
    ...
    switch (msg[i]->msg_style) {
      case PAM_PROMPT_ECHO_OFF:
        if (strlen(passwd) == 0) {
          ...
          if (strlen(passwd) == 0) {
            ereport(LOG, (errmsg("empty password returned by client")));
            goto fail;
          }
        }
        if ((reply[i].resp = strdup(passwd)) == NULL) goto fail;
        reply[i].resp_retcode = PAM_SUCCESS;
        break;
      case PAM_ERROR_MSG:
        ereport(LOG,
                (errmsg("error from underlying PAM layer: %s", msg[i]->msg)));
      /* FALL THROUGH */
      case PAM_TEXT_INFO:
        /* we don't bother to log TEXT_INFO messages */
        if ((reply[i].resp = strdup("")) == NULL) goto fail;
        reply[i].resp_retcode = PAM_SUCCESS;
        break;
      default:
        elog(LOG, "unsupported PAM conversation %d/\"%s\"", msg[i]->msg_style,
...
static int CheckPAMAuth(Port *port, char *user, char *password) {
  ...
  if (retval != PAM_SUCCESS) {
    ereport(LOG,
            (errmsg("pam_acct_mgmt failed: %s", pam_strerror(pamh, retval))));
    pam_passwd = NULL; /* Unset pam_passwd */
    return STATUS_ERROR;
  }
  ...
  *ldap = ldap_init(port->hba->ldapserver, port->hba->ldapport);
  ...
  if (port->hba->ldaptls) {
    ...
    if (_ldap_start_tls_sA == NULL) {
      ...
      _ldap_start_tls_sA =
          (__ldap_start_tls_sA)GetProcAddress(ldaphandle, "ldap_start_tls_sA");
      if (_ldap_start_tls_sA == NULL) {
        ldap_unbind(*ldap);
        ereport(
            LOG,
...
static int CheckLDAPAuth(Port *port) {
  ...
  if (port->hba->ldapport == 0) port->hba->ldapport = LDAP_PORT;
  ...
  if (port->hba->ldapbasedn) {
    ...
    r = ldap_simple_bind_s(
        ldap, port->hba->ldapbinddn ? port->hba->ldapbinddn : "",
    ...
    r = ldap_search_s(ldap, port->hba->ldapbasedn, port->hba->ldapscope, filter,
                      attributes, 0, &search_message);
      ...
      if (count == 0)
        ereport(LOG,
                (errmsg("LDAP user \"%s\" does not exist", port->user_name),
                 errdetail(
                     "LDAP search for filter \"%s\" on server \"%s\" returned "
                     "no entries.",
                     filter, port->hba->ldapserver)));
      else
        ereport(LOG, (errmsg("LDAP user \"%s\" is not unique", port->user_name),
                      errdetail_plural(
                          "LDAP search for filter \"%s\" on server \"%s\" "
                          "returned %d entry.",
                          "LDAP search for filter \"%s\" on server \"%s\" "
                          "returned %d entries.",
                          count, filter, port->hba->ldapserver, count)));
    ...
    if (dn == NULL) {
      ...
      ereport(LOG, (errmsg(
                       "could not get dn for the first entry matching \"%s\" "
                       "on server \"%s\": %s",
    ...
    fulluser = psprintf(
        "%s%s%s", port->hba->ldapprefix ? port->hba->ldapprefix : "",
        port->user_name, port->hba->ldapsuffix ? port->hba->ldapsuffix : "");
...
static int CheckCertAuth(Port *port) {
  ...
  return check_usermap(port->hba->usermap, port->user_name, port->peer_cn,
                       false);
}
#endif
...
static void radius_add_attribute(radius_packet *packet, uint8 type,
  ...
  if (packet->length + len > RADIUS_BUFFER_SIZE) {
     ...
     */
    elog(WARNING,
         "Adding attribute code %d with length %d to radius packet would "
         "create oversize packet, ignoring",
         type, len);
    return;
  }
...
static int CheckRADIUSAuth(Port *port) {
  ...
  if (port->hba->radiusport == 0) port->hba->radiusport = 1812;
  ...
  if (strlen(passwd) > RADIUS_VECTOR_LENGTH) {
    ereport(LOG, (errmsg(
                     "RADIUS authentication does not support passwords longer "
                     "than 16 characters")));
    return STATUS_ERROR;
  }
  ...
  radius_add_attribute(packet, RADIUS_SERVICE_TYPE, (unsigned char *)&service,
                       sizeof(service));
  radius_add_attribute(packet, RADIUS_USER_NAME,
                       (unsigned char *)port->user_name,
                       strlen(port->user_name));
  radius_add_attribute(packet, RADIUS_NAS_IDENTIFIER,
                       (unsigned char *)identifier, strlen(identifier));

  /*
   * RADIUS password attributes are calculated as: e[0] = p[0] XOR
   * MD5(secret + vector)
   */
  cryptvector = static_cast<uint8 *>(
      palloc(RADIUS_VECTOR_LENGTH + strlen(port->hba->radiussecret)));
  ...
  if (!pg_md5_binary(cryptvector,
                     RADIUS_VECTOR_LENGTH + strlen(port->hba->radiussecret),
                     encryptedpassword)) {
  ...
  if (sendto(sock, radius_buffer, packetlength, 0, serveraddrs[0].ai_addr,
             serveraddrs[0].ai_addrlen) < 0) {
  ...
  while (true) {
    ...
    if (timeoutval <= 0) {
      ereport(LOG, (errmsg("timeout waiting for RADIUS response")));
      closesocket(sock);
      return STATUS_ERROR;
    }
    ...
    {
#ifdef HAVE_IPV6
      ereport(LOG, (errmsg("RADIUS response was sent from incorrect port: %d",
    ...
    if (packetlength < RADIUS_HEADER_LENGTH) {
      ereport(LOG, (errmsg("RADIUS response too short: %d", packetlength)));
      continue;
    }
    ...
    if (packet->id != receivepacket->id) {
      ereport(
          LOG,
          (errmsg(
              "RADIUS response is to a different request: %d (should be %d)",
     ...
     */
    cryptvector = static_cast<uint8 *>(
        palloc(packetlength + strlen(port->hba->radiussecret)));

    memcpy(cryptvector, receivepacket, 4); /* code+id+length */
    memcpy(cryptvector + 4, packet->vector,
           RADIUS_VECTOR_LENGTH);            /* request
                                              * authenticator, from
                                              * original packet */
    if (packetlength > RADIUS_HEADER_LENGTH) /* there may be no
                                              * attributes at all */
      memcpy(cryptvector + RADIUS_HEADER_LENGTH,
             receive_buffer + RADIUS_HEADER_LENGTH,
    ...
    if (memcmp(receivepacket->vector, encryptedpassword,
      ...
      ereport(LOG, (errmsg("RADIUS response has incorrect MD5 signature")));
      continue;
    }
