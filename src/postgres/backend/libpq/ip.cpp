...
static int getnameinfo_unix(const struct sockaddr_un *sa, int salen, char *node,
                            int nodelen, char *service, int servicelen,
                            int flags);
#endif
...
int pg_getnameinfo_all(const struct sockaddr_storage *addr, int salen,
                       char *node, int nodelen, char *service, int servicelen,
                       int flags) {
  int rc;

#ifdef HAVE_UNIX_SOCKETS
  if (addr && addr->ss_family == AF_UNIX)
    rc = getnameinfo_unix((const struct sockaddr_un *)addr, salen, node,
                          nodelen, service, servicelen, flags);
  else
#endif
    rc = getnameinfo((const struct sockaddr *)addr, salen, node, nodelen,
...
static int getnameinfo_unix(const struct sockaddr_un *sa, int salen, char *node,
  ...
  if (node) {
    ...
    if (ret == -1 || ret > nodelen) return EAI_MEMORY;
  }
 ...
 */
int pg_sockaddr_cidr_mask(struct sockaddr_storage *mask, char *numbits,
                          int family) {
  long bits;
  char *endptr;
  ...
  if (numbits == NULL) {
    ...
    if (*numbits == '\0' || *endptr != '\0') return -1;
  }
 ...
 */
static void run_ifaddr_callback(PgIfAddrCallback callback, void *cb_data,
...
int pg_foreach_ifaddr(PgIfAddrCallback callback, void *cb_data) {
  ...
  while (n_ii < 1024) {
    ...
    if (WSAIoctl(sock, SIO_GET_INTERFACE_LIST, 0, 0, ii,
                 n_ii * sizeof(INTERFACE_INFO), &length, 0,
                 0) == SOCKET_ERROR) {
  ...
  for (i = 0; i < length / sizeof(INTERFACE_INFO); ++i)
    run_ifaddr_callback(callback, cb_data, (struct sockaddr *)&ii[i].iiAddress,
...
int pg_foreach_ifaddr(PgIfAddrCallback callback, void *cb_data) {
  ...
  if (sock == PGINVALID_SOCKET) return -1;
  ...
  while (n_buffer < 1024 * 100) {
     ...
     */
    if (lifc.lifc_len < n_buffer - 1024) break;
  }
...
int pg_foreach_ifaddr(PgIfAddrCallback callback, void *cb_data) {
  ...
  while (n_buffer < 1024 * 100) {
     ...
     * it all unless there's lots of slop.
     */
    if (ifc.ifc_len < n_buffer - 1024) break;
  }
...
int pg_foreach_ifaddr(PgIfAddrCallback callback, void *cb_data) {
  ...
  run_ifaddr_callback(callback, cb_data, (struct sockaddr *)&addr,
  ...
  run_ifaddr_callback(callback, cb_data, (struct sockaddr *)&addr6,
                      (struct sockaddr *)&mask);
#endif
