 ...
 */
bool pg_isblank(const char c) { return c == ' ' || c == '\t' || c == '\r'; }
  ...
  *terminating_comma = false;
  ...
  while (c != '\0' && c != '\n' && (!pg_isblank(c) || in_quote)) {
    ...
    if (c == '"') {
      in_quote = !in_quote;
      saw_quote = true;
      if (buf == start_buf) *initial_quote = true;
    }
...
static List *tokenize_inc_file(List *tokens, const char *outer_filename,
  ...
  if (inc_file == NULL) {
    ereport(LOG, (errcode_for_file_access(),
                  errmsg(
                      "could not open secondary authentication file \"@%s\" as "
                      "\"%s\": %m",
  ...
  linecxt =
      tokenize_file(inc_fullname, inc_file, &inc_lines, &inc_line_nums, NULL);
...
static MemoryContext tokenize_file(const char *filename, FILE *file,
  ...
  linecxt = AllocSetContextCreate(
      TopMemoryContext, "tokenize file cxt", ALLOCSET_DEFAULT_MINSIZE,
  ...
  *lines = *line_nums = NIL;
  ...
  while (!feof(file) && !ferror(file)) {
    ...
    if (!fgets(rawline, sizeof(rawline), file)) break;
    if (strlen(rawline) == MAX_LINE - 1) /* Line too long! */
      ...
      if (list_length(current_field) > 0) {
        if (current_line == NIL) {
          ...
          *line_nums = lappend_int(*line_nums, line_number);
          if (raw_lines) *raw_lines = lappend(*raw_lines, pstrdup(rawline));
        } else {
...
static bool is_member(Oid userid, const char *role) {
  ...
  if (!OidIsValid(userid)) return false; /* if user not exist, say "no" */
...
static bool check_role(const char *role, Oid roleid, List *tokens) {
  ...
  foreach (cell, tokens) {
    ...
    if (!tok->quoted && tok->string[0] == '+') {
      if (is_member(roleid, tok->string + 1)) return true;
 ...
 */
static bool check_db(const char *dbname, const char *role, Oid roleid,
                     List *tokens) {
  ListCell *cell;
  HbaToken *tok;
  ...
  foreach (cell, tokens) {
    ...
    if (am_walsender) {
      ...
      if (token_is_keyword(tok, "replication")) return true;
    } else if (token_is_keyword(tok, "all"))
      return true;
    else if (token_is_keyword(tok, "sameuser")) {
      if (strcmp(dbname, role) == 0) return true;
    } else if (token_is_keyword(tok, "samegroup") ||
               token_is_keyword(tok, "samerole")) {
      if (is_member(roleid, dbname)) return true;
...
static bool ipv6eq(struct sockaddr_in6 *a, struct sockaddr_in6 *b) {
  ...
  for (i = 0; i < 16; i++)
    if (a->sin6_addr.s6_addr[i] != b->sin6_addr.s6_addr[i]) return false;
...
static bool check_hostname(hbaPort *port, const char *hostname) {
  ...
  if (port->remote_hostname_resolv < 0) return false;
  ...
  if (!port->remote_hostname) {
    ...
    ret = pg_getnameinfo_all(&port->raddr.addr, port->raddr.salen,
                             remote_hostname, sizeof(remote_hostname), NULL, 0,
                             NI_NAMEREQD);
    if (ret != 0) {
  ...
  if (!hostname_match(hostname, port->remote_hostname)) return false;

  /* If we already verified the forward lookup, we're done */
  if (port->remote_hostname_resolv == +1) return true;
  ...
  if (gai_result) freeaddrinfo(gai_result);

  if (!found)
    elog(DEBUG2,
         "pg_hba.conf host name \"%s\" rejected because address resolution did "
         "not return a match with IP address of client",
         hostname);
 ...
 */
static bool check_ip(SockAddr *raddr, struct sockaddr *addr,
                     struct sockaddr *mask) {
  if (raddr->addr.ss_family == addr->sa_family &&
      pg_range_sockaddr(&raddr->addr, (struct sockaddr_storage *)addr,
...
static void check_network_callback(struct sockaddr *addr,
                                   struct sockaddr *netmask, void *cb_data) {
 ...
 *it's
 *						 not supported.
 * REQUIRE_AUTH_OPTION = same as INVALID_AUTH_OPTION, except it also checks if
 *the
 *						 method is actually the one specified. Used as a shortcut
 *when
...
#define INVALID_AUTH_OPTION(optname, validmethods)                             \
  do {                                                                         \
    ereport(LOG, (errcode(ERRCODE_CONFIG_FILE_ERROR), /* translator: the       \
                                                         second %s is a list   \
                                                         of auth methods */    \
                  errmsg(                                                      \
                      "authentication option \"%s\" is only valid for "        \
                      "authentication methods %s",                             \
                      optname, _(validmethods)),                               \
                  errcontext("line %d of configuration file \"%s\"", line_num, \
                             HbaFileName)));                                   \
    return false;                                                              \
  } while (0);
  ...
  do {                                                                    \
    if (argvar == NULL) {                                                 \
      ereport(LOG, (errcode(ERRCODE_CONFIG_FILE_ERROR),                   \
                    errmsg(                                               \
                        "authentication method \"%s\" requires argument " \
                        "\"%s\" to be set",                               \
                        authname, argname),                               \
                    errcontext("line %d of configuration file \"%s\"",    \
...
/* SSL support must be actually active, else complain */
  ...
  foreach (tokencell, tokens) {
    parsedline->databases =
        lappend(parsedline->databases,
  ...
  if (parsedline->conntype != ctLocal) {
    ...
    if (token_is_keyword(token, "all")) {
      ...
      if (ret == 0 && gai_result)
        ...
        ereport(LOG, (errcode(ERRCODE_CONFIG_FILE_ERROR),
                      errmsg("invalid IP address \"%s\": %s", str,
                             gai_strerror(ret)),
                      errcontext("line %d of configuration file \"%s\"",
                                 line_num, HbaFileName)));
      ...
      if (cidr_slash) {
        ...
        if (!field) {
          ereport(LOG, (errcode(ERRCODE_CONFIG_FILE_ERROR),
                        ...
                        errhint(
                            "Specify an address range in CIDR notation, or "
                            "provide a separate netmask."),
                        errcontext("line %d of configuration file \"%s\"",
                                   line_num, HbaFileName)));
          return NULL;
        }
        ...
        if (ret || !gai_result) {
          ereport(LOG, (errcode(ERRCODE_CONFIG_FILE_ERROR),
                        errmsg("invalid IP mask \"%s\": %s", token->string,
                               gai_strerror(ret)),
                        errcontext("line %d of configuration file \"%s\"",
                                   line_num, HbaFileName)));
          if (gai_result) pg_freeaddrinfo_all(hints.ai_family, gai_result);
  ...
  if (unsupauth) {
    ereport(LOG, (errcode(ERRCODE_CONFIG_FILE_ERROR),
                  errmsg(
                      "invalid authentication method \"%s\": not supported by "
                      "this build",
                      token->string),
                  errcontext("line %d of configuration file \"%s\"", line_num,
                             HbaFileName)));
  ...
  while ((field = lnext(field)) != NULL) {
    ...
    foreach (tokencell, tokens) {
      ...
      if (val == NULL) {
        ...
        ereport(LOG,
                (errcode(ERRCODE_CONFIG_FILE_ERROR),
                 errmsg("authentication option not in name=value format: %s",
                        token->string),
                 errcontext("line %d of configuration file \"%s\"", line_num,
                            HbaFileName)));
        return NULL;
  ...
  if (parsedline->auth_method == uaLDAP) {
    ...
    if (parsedline->ldapprefix || parsedline->ldapsuffix) {
      if (parsedline->ldapbasedn || parsedline->ldapbinddn ||
          parsedline->ldapbindpasswd || parsedline->ldapsearchattribute) {
        ereport(LOG, (errcode(ERRCODE_CONFIG_FILE_ERROR),
                      errmsg(
                          "cannot use ldapbasedn, ldapbinddn, ldapbindpasswd, "
                          "ldapsearchattribute, or ldapurl together with "
                          "ldapprefix"),
                      errcontext("line %d of configuration file \"%s\"",
                                 line_num, HbaFileName)));
        return NULL;
      ...
      ereport(LOG, (errcode(ERRCODE_CONFIG_FILE_ERROR),
                    errmsg(
                        "authentication method \"ldap\" requires argument "
                        "\"ldapbasedn\", \"ldapprefix\", or \"ldapsuffix\" to "
                        "be set"),
                    errcontext("line %d of configuration file \"%s\"", line_num,
                               HbaFileName)));
      return NULL;
...
static bool parse_hba_auth_opt(char *name, char *val, HbaLine *hbaline,
  ...
  if (strcmp(name, "map") == 0) {
    if (hbaline->auth_method != uaIdent && hbaline->auth_method != uaPeer &&
        hbaline->auth_method != uaGSS && hbaline->auth_method != uaSSPI &&
        hbaline->auth_method != uaCert)
      INVALID_AUTH_OPTION("map",
                          gettext_noop("ident, peer, gssapi, sspi, and cert"));
    ...
    if (strcmp(val, "1") == 0) {
      if (!secure_loaded_verify_locations()) {
        ereport(LOG,
                (errcode(ERRCODE_CONFIG_FILE_ERROR),
                 errmsg(
                     "client certificates can only be checked if a root "
      ...
      if (hbaline->auth_method == uaCert) {
        ereport(LOG, (errcode(ERRCODE_CONFIG_FILE_ERROR),
                      errmsg(
                          "clientcert can not be set to 0 when using \"cert\" "
                          "authentication"),
                      errcontext("line %d of configuration file \"%s\"",
                                 line_num, HbaFileName)));
        return false;
    ...
    if (rc != LDAP_SUCCESS) {
      ereport(LOG, (errcode(ERRCODE_CONFIG_FILE_ERROR),
                    errmsg("could not parse LDAP URL \"%s\": %s", val,
                           ldap_err2string(rc))));
      return false;
    }
    ...
    if (urldata->lud_attrs)
      hbaline->ldapsearchattribute =
          pstrdup(urldata->lud_attrs[0]); /* only use first one */
...
static void check_hba(hbaPort *port) {
  ...
  foreach (line, parsed_hba_lines) {
    ...
    if (hba->conntype == ctLocal) {
      if (!IS_AF_UNIX(port->raddr.addr.ss_family)) continue;
    } else {
      if (IS_AF_UNIX(port->raddr.addr.ss_family)) continue;
      ...
      if (port->ssl_in_use) {
        ...
        if (hba->conntype == ctHostNoSSL) continue;
      } else {
        /* Connection is not SSL, match both "host" and "hostnossl" */
        if (hba->conntype == ctHostSSL) continue;
      }
      ...
      switch (hba->ip_cmp_method) {
        case ipCmpMask:
          if (hba->hostname) {
            if (!check_hostname(port, hba->hostname)) continue;
          } else {
...
bool load_hba(void) {
  ...
  ListCell *line, *line_num, *raw_line;
  ...
  hbacxt = AllocSetContextCreate(
      TopMemoryContext, "hba parser context", ALLOCSET_DEFAULT_MINSIZE,
  ...
  forthree(line, hba_lines, line_num, hba_line_nums, raw_line, hba_raw_lines) {
    ...
    if ((newline = parse_hba_line(
             static_cast<List *>(lfirst(line)), lfirst_int(line_num),
             static_cast<char *>(lfirst(raw_line)))) == NULL) {
      /*
  ...
  if (parsed_hba_context != NULL) MemoryContextDelete(parsed_hba_context);
...
static IdentLine *parse_ident_line(List *line, int line_number) {
  ...
  if (parsedline->ident_user[0] == '/') {
    ...
    wstr = static_cast<pg_wchar *>(
        palloc((strlen(parsedline->ident_user + 1) + 1) * sizeof(pg_wchar)));
    wlen = pg_mb2wchar_with_len(parsedline->ident_user + 1, wstr,
...
static void check_ident_usermap(IdentLine *identLine, const char *usermap_name,
                                ...
                                bool case_insensitive, bool *found_p,
                                bool *error_p) {
  *found_p = false;
  *error_p = false;
  ...
  if (identLine->ident_user[0] == '/') {
    ...
    wstr = static_cast<pg_wchar *>(
        palloc((strlen(ident_user) + 1) * sizeof(pg_wchar)));
    ...
    if ((ofs = strstr(identLine->pg_role, "\\1")) != NULL) {
      ...
      if (matches[1].rm_so < 0) {
        ereport(LOG, (errcode(ERRCODE_INVALID_REGULAR_EXPRESSION),
                      errmsg(
                          "regular expression \"%s\" has no subexpressions as "
                          "requested by backreference in \"%s\"",
       ...
       */
      regexp_pgrole = static_cast<char *>(
          palloc0(strlen(identLine->pg_role) - 2 +
                  (matches[1].rm_eo - matches[1].rm_so) + 1));
      offset = ofs - identLine->pg_role;
      memcpy(regexp_pgrole, identLine->pg_role, offset);
      memcpy(regexp_pgrole + offset, ident_user + matches[1].rm_so,
     ...
     */
    if (case_insensitive) {
      if (pg_strcasecmp(regexp_pgrole, pg_role) == 0) *found_p = true;
    } else {
      if (strcmp(regexp_pgrole, pg_role) == 0) *found_p = true;
    }
    pfree(regexp_pgrole);
 ...
 */
int check_usermap(const char *usermap_name, const char *pg_role,
                  const char *auth_user, bool case_insensitive) {
  bool found_entry = false, error = false;

  if (usermap_name == NULL || usermap_name[0] == '\0') {
    if (case_insensitive) {
      if (pg_strcasecmp(pg_role, auth_user) == 0) return STATUS_OK;
    } else {
      if (strcmp(pg_role, auth_user) == 0) return STATUS_OK;
    }
    ereport(LOG, (errmsg(
                     "provided user name (%s) and authenticated user name (%s) "
                     "do not match",
                     pg_role, auth_user)));
    return STATUS_ERROR;
  } else {
    ...
    foreach (line_cell, parsed_ident_lines) {
      check_ident_usermap(static_cast<IdentLine *>(lfirst(line_cell)),
                          usermap_name, pg_role, auth_user, case_insensitive,
                          &found_entry, &error);
      if (found_entry || error) break;
    }
  }
  if (!found_entry && !error) {
    ereport(LOG, (errmsg(
                     "no match in usermap \"%s\" for user \"%s\" authenticated "
                     "as \"%s\"",
...
bool load_ident(void) {
  ...
  ListCell *line_cell, *num_cell, *parsed_line_cell;
  List *new_parsed_lines = NIL;
  bool ok = true;
  MemoryContext linecxt;
  MemoryContext oldcxt;
  ...
  linecxt =
      tokenize_file(IdentFileName, file, &ident_lines, &ident_line_nums, NULL);
  FreeFile(file);

  /* Now parse all the lines */
  ident_context = AllocSetContextCreate(
      TopMemoryContext, "ident parser context", ALLOCSET_DEFAULT_MINSIZE,
      ALLOCSET_DEFAULT_MINSIZE, ALLOCSET_DEFAULT_MAXSIZE);
  oldcxt = MemoryContextSwitchTo(ident_context);
  forboth(line_cell, ident_lines, num_cell, ident_line_nums) {
    if ((newline = parse_ident_line(static_cast<List *>(lfirst(line_cell)),
                                    lfirst_int(num_cell))) == NULL) {
      /*
      ...
      foreach (parsed_line_cell, new_parsed_lines) {
        ...
        if (newline->ident_user[0] == '/') pg_regfree(&newline->re);
      }
  ...
  if (parsed_ident_context != NULL) MemoryContextDelete(parsed_ident_context);
 ...
 */
void hba_getauthmethod(hbaPort *port) { check_hba(port); }
