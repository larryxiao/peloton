...
static PQcommMethods PqCommMqMethods = {
    mq_comm_reset, mq_flush, mq_flush_if_writable, mq_is_send_pending,
    mq_putmessage, mq_putmessage_noblock, mq_startcopyout, mq_endcopyout};

/*
 * Arrange to redirect frontend/backend protocol messages to a shared-memory
 * message queue.
 */
void pq_redirect_to_shm_mq(shm_mq *mq, shm_mq_handle *mqh) {
...
static void mq_comm_reset(void) { /* Nothing to do. */ }
...
static int mq_putmessage(char msgtype, const char *s, size_t len) {
   ...
   */
  if (pq_mq_busy) {
    if (pq_mq != NULL) shm_mq_detach(pq_mq);
    pq_mq = NULL;
    return EOF;
  }
  ...
  for (;;) {
    ...
    if (pq_mq_parallel_master_pid != 0)
      SendProcSignal(pq_mq_parallel_master_pid, PROCSIG_PARALLEL_MESSAGE,
                     pq_mq_parallel_master_backend_id);

    if (result != SHM_MQ_WOULD_BLOCK) break;
  ...
  if (result != SHM_MQ_SUCCESS) return EOF;
  return 0;
}
...
static void mq_startcopyout(void) { /* Nothing to do. */ }

static void mq_endcopyout(bool errorAbort) { /* Nothing to do. */ }
  ...
  for (;;) {
    ...
    switch (code) {
      ...
      case PG_DIAG_SQLSTATE:
        if (strlen(value) != 5) elog(ERROR, "malformed sql state");
        edata->sqlerrcode =
