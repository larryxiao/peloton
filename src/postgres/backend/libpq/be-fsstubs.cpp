 ...
 *
 *	  These functions store LargeObjectDesc structs in a private___
 *MemoryContext,
  ...
  do {                                                              \
    if (fscxt == NULL)                                              \
      fscxt = AllocSetContextCreate(                                \
          TopMemoryContext, "Filesystem", ALLOCSET_DEFAULT_MINSIZE, \
...
int lo_read(int fd, char *buf, int len) {
  ...
  if ((lobj->flags & IFS_RD_PERM_OK) == 0) {
    if (!lo_compat_privileges &&
        pg_largeobject_aclcheck_snapshot(lobj->id, GetUserId(), ACL_SELECT,
...
int lo_write(int fd, const char *buf, int len) {
  ...
  if ((lobj->flags & IFS_WR_PERM_OK) == 0) {
    if (!lo_compat_privileges &&
        pg_largeobject_aclcheck_snapshot(lobj->id, GetUserId(), ACL_UPDATE,
                                         lobj->snapshot) != ACLCHECK_OK)
      ereport(ERROR,
              (errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
               errmsg("permission denied for large object %u", lobj->id)));
...
void AtEOXact_LargeObject(bool isCommit) {
  ...
  if (fscxt == NULL) return; /* no LO operations in this xact */
  ...
  for (i = 0; i < cookies_size; i++) {
    if (cookies[i] != NULL) {
      if (isCommit) inv_close(cookies[i]);
      deleteLOfd(i);
    }
  }
...
static int newLOfd(LargeObjectDesc *lobjCookie) {
  ...
  if (cookies_size <= 0) {
    ...
    i = 0;
    newsize = 64;
    cookies = (LargeObjectDesc **)MemoryContextAllocZero(
        fscxt, newsize * sizeof(LargeObjectDesc *));
    cookies_size = newsize;
  } else {
    /* Double size of array */
    i = cookies_size;
    newsize = cookies_size * 2;
    cookies = (LargeObjectDesc **)repalloc(cookies,
...
static void deleteLOfd(int fd) { cookies[fd] = NULL; }
...
static bytea *lo_get_fragment_internal(Oid loOid, int64 offset, int32 nbytes) {
  ...
  /* Permission check */
  if (!lo_compat_privileges &&
      pg_largeobject_aclcheck_snapshot(loDesc->id, GetUserId(), ACL_SELECT,
