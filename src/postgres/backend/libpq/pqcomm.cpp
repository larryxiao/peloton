...
static thread_local int
    PqSendPointer; /* Next index to store a byte in PqSendBuffer */
static thread_local int
    PqSendStart; /* Next index to send a byte in PqSendBuffer */

static thread_local char PqRecvBuffer[PQ_RECV_BUFFER_SIZE];
static thread_local int
    PqRecvPointer; /* Next index to read a byte from PqRecvBuffer */
static thread_local int
    PqRecvLength; /* End of data available in PqRecvBuffer */
...
void pq_init(void) {
  ...
  PqSendBuffer = static_cast<char *>(
      MemoryContextAlloc(TopMemoryContext, PqSendBufferSize));
...
int StreamServerPort(int family, char *hostName, unsigned short portNumber,
                     char *unixSocketDir, pgsocket ListenSocket[],
                     int MaxListen) {
  pgsocket fd;
  int err;
  int maxconn;
  ...
  if (ret || !addrs) {
    if (hostName)
      ereport(LOG, (errmsg(
                       "could not translate host name \"%s\", service \"%s\" "
                       "to address: %s",
  ...
  for (addr = addrs; addr; addr = addr->ai_next) {
    ...
    for (; listen_index < MaxListen; listen_index++) {
      if (ListenSocket[listen_index] == PGINVALID_SOCKET) break;
    }
    if (listen_index >= MaxListen) {
      ereport(LOG, (errmsg(
                       "could not bind to all requested addresses: MAXLISTEN "
                       "(%d) exceeded",
                       MaxListen)));
      break;
    }
    ...
    switch (addr->ai_family) {
      ...
      default:
        snprintf(familyDescBuf, sizeof(familyDescBuf),
                 _("unrecognized address family %d"), addr->ai_family);
    ...
    if (addr->ai_family == AF_INET6) {
      if (setsockopt(fd, IPPROTO_IPV6, IPV6_V6ONLY, (char *)&one,
                     sizeof(one)) == -1) {
        ereport(LOG, (errcode_for_socket_access(),
    ...
    if (maxconn > PG_SOMAXCONN) maxconn = PG_SOMAXCONN;
...
static int Lock_AF_UNIX(char *unixSocketDir, char *unixSocketPath) {
   ...
   */
  if (sock_paths == NIL) on_proc_exit(StreamDoUnlink, 0);
...
static int Setup_AF_UNIX(char *sock_path) {
  ...
  if (Unix_socket_group[0] != '\0') {
#ifdef WIN32
    elog(WARNING,
         "configuration item unix_socket_group is not supported on this "
         "platform");
#else
    char *endptr;
    unsigned long val;
    ...
    if (*endptr == '\0') { /* numeric group id */
      ...
      if (!gr) {
        ereport(LOG,
                (errmsg("group \"%s\" does not exist", Unix_socket_group)));
        return STATUS_ERROR;
      }
      gid = gr->gr_gid;
    }
    if (chown(sock_path, -1, gid) == -1) {
      ereport(LOG,
              (errcode_for_file_access(),
               errmsg("could not set group of file \"%s\": %m", sock_path)));
      return STATUS_ERROR;
    }
#endif
  }
...
int StreamConnection(pgsocket server_fd, Port *port) {
  ...
  if ((port->sock = accept(server_fd, (struct sockaddr *)&port->raddr.addr,
   ...
   */
  if (port->raddr.addr.ss_family == 0) port->raddr.addr.ss_family = AF_UNIX;
#endif
  ...
  if (!IS_AF_UNIX(port->laddr.addr.ss_family)) {
...
#endif
    on = 1;
    if (setsockopt(port->sock, SOL_SOCKET, SO_KEEPALIVE, (char *)&on,
                   sizeof(on)) < 0) {
    ...
    if (setsockopt(port->sock, SOL_SOCKET, SO_SNDBUF, (char *)&on, sizeof(on)) <
        0) {
 ...
 */
void StreamClose(pgsocket sock) { closesocket(sock); }
 ...
 * --------------------------------
 */
bool pq_is_reading_msg(void) { return PqCommReadingMsg; }
...
static int internal_flush(void) {
  ...
  while (bufptr < bufend) {
    ...
    if (r <= 0) {
      if (errno == EINTR) continue; /* Ok if we were interrupted */
...
static int socket_flush_if_writable(void) {
  ...
  if (PqSendPointer == PqSendStart) return 0;
...
static int socket_putmessage(char msgtype, const char *s, size_t len) {
  ...
  PqCommBusy = true;
  if (msgtype)
    if (internal_putbytes(&msgtype, 1)) goto fail;
  if (PG_PROTOCOL_MAJOR(FrontendProtocol) >= 3) {
    uint32 n32;

    n32 = htonl((uint32)(len + 4));
    if (internal_putbytes((char *)&n32, 4)) goto fail;
  }
  if (internal_putbytes(s, len)) goto fail;
  PqCommBusy = false;
  return 0;
 ...
 * --------------------------------
 */
static void socket_startcopyout(void) { DoingCopyOut = true; }
 ...
 */
static void socket_endcopyout(bool errorAbort) {
  if (!DoingCopyOut) return;
  if (errorAbort) pq_putbytes("\n\n\\.\n", 5);
...
static int pq_setkeepaliveswin32(Port *port, int idle, int interval) {
  ...
  if (idle <= 0) idle = 2 * 60 * 60; /* default = 2 hours */
  if (interval <= 0) interval = 1;   /* default = 1 second */

  ka.onoff = 1;
  ka.keepalivetime = idle * 1000;
  ka.keepaliveinterval = interval * 1000;

  if (WSAIoctl(port->sock, SIO_KEEPALIVE_VALS, (LPVOID)&ka, sizeof(ka), NULL, 0,
               &retsize, NULL, NULL) != 0) {
    elog(LOG, "WSAIoctl(SIO_KEEPALIVE_VALS) failed: %ui", WSAGetLastError());
    return STATUS_ERROR;
  }
  if (port->keepalives_idle != idle) port->keepalives_idle = idle;
...
int pq_getkeepalivesidle(Port *port) {
  ...
  if (port == NULL || IS_AF_UNIX(port->laddr.addr.ss_family)) return 0;

  if (port->keepalives_idle != 0) return port->keepalives_idle;
...
int pq_setkeepalivesidle(int idle, Port *port) {
  if (port == NULL || IS_AF_UNIX(port->laddr.addr.ss_family)) return STATUS_OK;

#if defined(TCP_KEEPIDLE) || defined(TCP_KEEPALIVE) || \
    defined(SIO_KEEPALIVE_VALS)
  if (idle == port->keepalives_idle) return STATUS_OK;
  ...
  if (idle == 0) idle = port->default_keepalives_idle;

#ifdef TCP_KEEPIDLE
  if (setsockopt(port->sock, IPPROTO_TCP, TCP_KEEPIDLE, (char *)&idle,
                 sizeof(idle)) < 0) {
    elog(LOG, "setsockopt(TCP_KEEPIDLE) failed: %m");
    return STATUS_ERROR;
  }
#else
  if (setsockopt(port->sock, IPPROTO_TCP, TCP_KEEPALIVE, (char *)&idle,
                 sizeof(idle)) < 0) {
...
int pq_getkeepalivesinterval(Port *port) {
  ...
  if (port->keepalives_interval != 0) return port->keepalives_interval;
...
int pq_setkeepalivesinterval(int interval, Port *port) {
...
#if defined(TCP_KEEPINTVL) || defined(SIO_KEEPALIVE_VALS)
  if (interval == port->keepalives_interval) return STATUS_OK;
  ...
  if (interval == 0) interval = port->default_keepalives_interval;

  if (setsockopt(port->sock, IPPROTO_TCP, TCP_KEEPINTVL, (char *)&interval,
                 sizeof(interval)) < 0) {
...
int pq_getkeepalivescount(Port *port) {
  ...
  if (port->keepalives_count != 0) return port->keepalives_count;
...
int pq_setkeepalivescount(int count, Port *port) {
...
#ifdef TCP_KEEPCNT
  if (count == port->keepalives_count) return STATUS_OK;
  ...
  if (count == 0) count = port->default_keepalives_count;

  if (setsockopt(port->sock, IPPROTO_TCP, TCP_KEEPCNT, (char *)&count,
                 sizeof(count)) < 0) {
