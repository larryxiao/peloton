...
static Query *rewriteRuleAction(Query *parsetree, Query *rule_action,
                                Node *rule_qual, int rt_index, CmdType event,
                                bool *returning_flag);
static List *adjustJoinTreeList(Query *parsetree, bool removert, int rt_index);
static List *rewriteTargetListIU(List *targetList, CmdType commandType,
                                 Relation target_relation, int result_rti,
                                 List **attrno_list);
...
static List *matchLocks(CmdType event, RuleLock *rulelocks, int varno,
...
void AcquireRewriteLocks(Query *parsetree, bool forExecute,
          ...
          if (aliasvar && IsA(aliasvar, Var)) {
            ...
            if (aliasvar->varno != curinputvarno) {
              ...
              if (curinputvarno >= rt_index)
                elog(ERROR, "unexpected varno %d in JOIN RTE %d", curinputvarno,
                     rt_index);
  ...
  if (parsetree->hasSubLinks)
    query_tree_walker(parsetree, reinterpret_cast<query_tree_walker_fptr>(
                                     acquireLocksOnSubLinks),
                      &context, QTW_IGNORE_RC_SUBQUERIES);
}
...
static bool acquireLocksOnSubLinks(Node *node,
  ...
  return expression_tree_walker(
      node,
      reinterpret_cast<expression_tree_walker_fptr>(acquireLocksOnSubLinks),
      context);
}
...
static Query *rewriteRuleAction(Query *parsetree, Query *rule_action,
   ...
   */
  sub_action->rtable =
      list_concat((List *)copyObject(parsetree->rtable), sub_action->rtable);
  ...
  if (parsetree->hasSubLinks && !sub_action->hasSubLinks) {
    ...
    foreach (lc, parsetree->rtable) {
      ...
      if (sub_action->hasSubLinks) break; /* no need to keep scanning rtable */
    }
  }
  ...
  if (sub_action->commandType != CMD_UTILITY) {
    ...
    keeporig =
        (!rangeTableEntry_used((Node *)sub_action->jointree, rt_index, 0)) &&
  ...
  if (parsetree->cteList != NIL && sub_action->commandType != CMD_UTILITY) {
    ...
    foreach (lc, parsetree->cteList) {
      ...
      foreach (lc2, sub_action->cteList) {
        ...
        if (strcmp(cte->ctename, cte2->ctename) == 0)
          ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                          errmsg(
                              "WITH query name \"%s\" appears in both a rule "
                              "action and the query being rewritten",
                              cte->ctename)));
      }
    }
    ...
    sub_action->cteList =
        list_concat(sub_action->cteList,
  ...
  if ((event == CMD_INSERT || event == CMD_UPDATE) &&
    ...
    sub_action = (Query *)ReplaceVarsFromTargetList(
        (Node *)sub_action, new_varno, 0,
        rt_fetch(new_varno, sub_action->rtable), parsetree->targetList,
        (event == CMD_UPDATE) ? REPLACEVARS_CHANGE_VARNO
  ...
  if (!parsetree->returningList)
    ...
    *returning_flag = true;
    rule_action->returningList = (List *)ReplaceVarsFromTargetList(
        (Node *)parsetree->returningList, parsetree->resultRelation, 0,
        rt_fetch(parsetree->resultRelation, parsetree->rtable),
        rule_action->returningList, REPLACEVARS_REPORT_ERROR, 0,
...
static List *adjustJoinTreeList(Query *parsetree, bool removert, int rt_index) {
  List *newjointree =
      static_cast<List *>(copyObject(parsetree->jointree->fromlist));
  ListCell *l;
...
static List *rewriteTargetListIU(List *targetList, CmdType commandType,
  ...
  foreach (temp, targetList) {
    ...
    if (!old_tle->resjunk) {
      ...
      if (attrno_list) *attrno_list = lappend_int(*attrno_list, attrno);

      /* We can (and must) ignore deleted attributes */
      if (att_tup->attisdropped) continue;
  ...
  for (attrno = 1; attrno <= numattrs; attrno++) {
    ...
    if ((new_tle == NULL && commandType == CMD_INSERT) ||
      ...
      if (!new_expr) {
        if (commandType == CMD_INSERT)
          ...
          new_expr =
              (Node *)makeConst(att_tup->atttypid, -1, att_tup->attcollation,
                                att_tup->attlen, (Datum)0, true, /* isnull */
                                att_tup->attbyval);
          /* this is to catch a NOT NULL domain constraint */
          new_expr =
              coerce_to_domain(new_expr, InvalidOid, -1, att_tup->atttypid,
                               COERCE_IMPLICIT_CAST, -1, false, false);
        }
      }
    ...
    if (new_tle == NULL && commandType == CMD_UPDATE &&
      ...
      new_expr = (Node *)makeVar(result_rti, attrno, att_tup->atttypid,
    ...
    if (new_tle) new_tlist = lappend(new_tlist, new_tle);
  }
...
static TargetEntry *process_matched_tle(TargetEntry *src_tle,
  ...
  if (src_input == NULL || prior_input == NULL ||
    ...
    ereport(ERROR,
            (errcode(ERRCODE_SYNTAX_ERROR),
             errmsg("multiple assignments to same column \"%s\"", attrName)));
  ...
  for (;;) {
    ...
    if (newbottom == NULL) break; /* found the original Var reference */
...
static Node *get_assignment_input(Node *node) {
  ...
  if (IsA(node, FieldStore)) {
    ...
    if (aref->refassgnexpr == NULL) return NULL;
...
Node *build_column_default(Relation rel, int attrno) {
  ...
  if (expr == NULL) return NULL; /* No default anywhere */
  ...
  expr = coerce_to_target_type(NULL, /* no UNKNOWN params here */
                               expr, exprtype, atttype, atttypmod,
    ...
    ereport(ERROR,
            (errcode(ERRCODE_DATATYPE_MISMATCH),
             errmsg(
                 ...
                 NameStr(att_tup->attname), format_type_be(atttype),
...
static bool searchForDefault(RangeTblEntry *rte) {
  ...
  foreach (lc, rte->values_lists) {
    ...
    foreach (lc2, sublist) {
      ...
      if (IsA(col, SetToDefault)) return true;
    }
  }
  return false;
}
...
static void rewriteValuesRTE(RangeTblEntry *rte, Relation target_relation,
   ...
   */
  if (!searchForDefault(rte)) return; /* nothing to do */

  /* Check list lengths (we can assume all the VALUES sublists are alike) */
  Assert(list_length(attrnos) ==
         list_length((List *)linitial(rte->values_lists)));
...
static void rewriteTargetListUD(Query *parsetree, RangeTblEntry *target_rte,
  ...
  if (target_relation->rd_rel->relkind == RELKIND_RELATION ||
     ...
     */
    var = makeVar(parsetree->resultRelation, SelfItemPointerAttributeNumber,
...
static List *matchLocks(CmdType event, RuleLock *rulelocks, int varno,
  ...
  if (rulelocks == NULL) return NIL;

  if (parsetree->commandType != CMD_SELECT) {
    if (parsetree->resultRelation != varno) return NIL;
  }
  ...
  for (i = 0; i < nlocks; i++) {
    ...
    if (oneLock->event == CMD_UPDATE) *hasUpdate = true;
 ...
 */
static Query *ApplyRetrieveRule(Query *parsetree, RewriteRule *rule,
                                int rt_index, Relation relation,
                                List *activeRIRs, bool forUpdatePushedDown) {
  Query *rule_action;
  ...
  if (rule->qual != NULL) elog(ERROR, "cannot handle qualified ON SELECT rule");
  ...
  if (rt_index == parsetree->resultRelation) {
    ...
    if (parsetree->commandType == CMD_INSERT)
      ...
      parsetree->returningList =
          static_cast<List *>(copyObject(parsetree->returningList));
...
static void markQueryForLocking(Query *qry, Node *jtnode,
  ...
  if (IsA(jtnode, RangeTblRef)) {
    ...
    foreach (l, f->fromlist)
      markQueryForLocking(qry, static_cast<Node *>(lfirst(l)), strength,
                          waitPolicy, pushedDown);
...
static bool fireRIRonSubLink(Node *node, List *activeRIRs) {
  ...
  if (IsA(node, SubLink)) {
    ...
    /* Do what we came for */
    sub->subselect =
        (Node *)fireRIRrules((Query *)sub->subselect, activeRIRs, false);
    /* Fall through to process lefthand args of SubLink */
  }
   ...
   */
  return expression_tree_walker(
      node, reinterpret_cast<expression_tree_walker_fptr>(fireRIRonSubLink),
      (void *)activeRIRs);
}
...
static Query *fireRIRrules(Query *parsetree, List *activeRIRs,
  ...
  while (rt_index < list_length(parsetree->rtable)) {
    ...
    if (rte->rtekind == RTE_SUBQUERY) {
      rte->subquery =
          fireRIRrules(rte->subquery, activeRIRs,
                       (forUpdatePushedDown ||
                        get_parse_rowmark(parsetree, rt_index) != NULL));
      continue;
    }
     ...
     */
    if (rte->relkind == RELKIND_MATVIEW) continue;
    ...
    if (rules != NULL) {
      ...
      for (i = 0; i < rules->numLocks; i++) {
        rule = rules->rules[i];
        if (rule->event != CMD_SELECT) continue;
      ...
      if (locks != NIL) {
        ...
        foreach (l, locks) {
          ...
          parsetree = ApplyRetrieveRule(parsetree, rule, rt_index, rel,
                                        activeRIRs, forUpdatePushedDown);
        }
  ...
  foreach (lc, parsetree->cteList) {
    ...
    cte->ctequery =
        (Node *)fireRIRrules((Query *)cte->ctequery, activeRIRs, false);
  }
  ...
  if (parsetree->hasSubLinks)
    query_tree_walker(
        parsetree, reinterpret_cast<query_tree_walker_fptr>(fireRIRonSubLink),
        (void *)activeRIRs, QTW_IGNORE_RC_SUBQUERIES);
  ...
  foreach (lc, parsetree->rtable) {
     ...
     */
    get_row_security_policies(parsetree, parsetree->commandType, rte, rt_index,
    ...
    if (securityQuals != NIL || withCheckOptions != NIL) {
      if (hasSubLinks) {
        ...
        expression_tree_walker(
            (Node *)securityQuals,
            reinterpret_cast<expression_tree_walker_fptr>(fireRIRonSubLink),
            (void *)activeRIRs);
    ...
    if (hasSubLinks) parsetree->hasSubLinks = true;
 ...
 */
static Query *CopyAndAddInvertedQual(Query *parsetree, Node *rule_qual,
                                     int rt_index, CmdType event) {
  ...
  if (event == CMD_INSERT || event == CMD_UPDATE)
    new_qual = ReplaceVarsFromTargetList(
        new_qual, PRS2_NEW_VARNO, 0, rt_fetch(rt_index, parsetree->rtable),
        parsetree->targetList,
        (event == CMD_UPDATE) ? REPLACEVARS_CHANGE_VARNO
                              : REPLACEVARS_SUBSTITUTE_NULL,
        rt_index, &parsetree->hasSubLinks);
...
static List *fireRules(Query *parsetree, int rt_index, CmdType event,
                       List *locks, bool *instead_flag, bool *returning_flag,
                       Query **qual_product) {
  List *results = NIL;
  ListCell *l;
  ...
  foreach (l, locks) {
    ...
    if (qsrc == QSRC_QUAL_INSTEAD_RULE) {
      ...
      if (!*instead_flag) {
        ...
        *qual_product =
            CopyAndAddInvertedQual(*qual_product, event_qual, rt_index, event);
      }
    }
    ...
    foreach (r, actions) {
      ...
      if (rule_action->commandType == CMD_NOTHING) continue;

      rule_action = rewriteRuleAction(parsetree, rule_action, event_qual,
                                      rt_index, event, returning_flag);
...
static bool view_has_instead_trigger(Relation view, CmdType event) {
  ...
  switch (event) {
    case CMD_INSERT:
      if (trigDesc && trigDesc->trig_insert_instead_row) return true;
      break;
    case CMD_UPDATE:
      if (trigDesc && trigDesc->trig_update_instead_row) return true;
      break;
    case CMD_DELETE:
      if (trigDesc && trigDesc->trig_delete_instead_row) return true;
      break;
    default:
...
static const char *view_col_is_auto_updatable(RangeTblRef *rtr,
   ...
   * query's targetlist.
   */
  if (tle->resjunk) return gettext_noop("Junk view columns are not updatable.");
...
const char *view_query_is_auto_updatable(Query *viewquery, bool check_cols) {
  ...
  if (viewquery->setOperations != NULL)
    return gettext_noop(
        "Views containing UNION, INTERSECT, or EXCEPT are not automatically "
        "updatable.");
  ...
  if (viewquery->hasAggs)
    return gettext_noop(
        "Views that return aggregate functions are not automatically "
        "updatable.");
  ...
  if (expression_returns_set((Node *)viewquery->targetList))
    return gettext_noop(
        "Views that return set-returning functions are not automatically "
        "updatable.");
  ...
  if (check_cols) {
    ...
    if (!found)
      return gettext_noop(
          "Views that have no updatable columns are not automatically "
          "updatable.");
  }
...
static const char *view_cols_are_auto_updatable(Query *viewquery,
  ...
  if (updatable_cols != NULL) *updatable_cols = NULL;
  if (non_updatable_col != NULL) *non_updatable_col = NULL;
  ...
  foreach (cell, viewquery->targetList) {
    ...
    if (col_update_detail == NULL) {
      ...
      if (non_updatable_col != NULL) *non_updatable_col = tle->resname;
 ...
 */
int relation_is_updatable(Oid reloid, bool include_triggers,
...
#define ALL_EVENTS ((1 << CMD_INSERT) | (1 << CMD_UPDATE) | (1 << CMD_DELETE))
  ...
  if (include_triggers) {
    ...
    if (trigDesc) {
      if (trigDesc->trig_insert_instead_row) events |= (1 << CMD_INSERT);
      if (trigDesc->trig_update_instead_row) events |= (1 << CMD_UPDATE);
      if (trigDesc->trig_delete_instead_row) events |= (1 << CMD_DELETE);
  ...
  if (rel->rd_rel->relkind == RELKIND_VIEW) {
    ...
    if (view_query_is_auto_updatable(viewquery, false) == NULL) {
      ...
      if (base_rte->relkind != RELKIND_RELATION) {
        ...
        include_cols =
            adjust_view_column_set(updatable_cols, viewquery->targetList);
        auto_events &=
            relation_is_updatable(baseoid, include_triggers, include_cols);
      }
      events |= auto_events;
    }
  }
    ...
    if (attno == InvalidAttrNumber) {
      ...
      foreach (lc, targetlist) {
        ...
        result = bms_add_member(
            result, var->varattno - FirstLowInvalidHeapAttributeNumber);
      }
    } else {
      /*
...
static Query *rewriteTargetView(Query *parsetree, Relation view) {
  ...
  auto_update_detail = view_query_is_auto_updatable(
      viewquery, parsetree->commandType != CMD_DELETE);
    ...
    switch (parsetree->commandType) {
      case CMD_INSERT:
        ereport(ERROR, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                        ...
                        errhint(
                            "To enable inserting into the view, provide an "
      ...
      case CMD_DELETE:
        ereport(ERROR, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                        ...
                        errdetail_internal("%s", _(auto_update_detail)),
                        errhint(
                            "To enable deleting from the view, provide an "
                            "INSTEAD OF DELETE trigger or an unconditional ON "
                            "DELETE DO INSTEAD rule.")));
        break;
      default:
    ...
    foreach (lc, parsetree->targetList) {
      ...
      if (!tle->resjunk)
        modified_cols = bms_add_member(
            modified_cols, tle->resno - FirstLowInvalidHeapAttributeNumber);
    }
          ...
          ereport(ERROR,
                  (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                   errmsg("cannot insert into column \"%s\" of view \"%s\"",
                          non_updatable_col, RelationGetRelationName(view)),
                   errdetail_internal("%s", _(auto_update_detail))));
          break;
        case CMD_UPDATE:
          ereport(ERROR,
   ...
   */
  if (parsetree->commandType == CMD_INSERT) new_rte->inh = false;
   ...
   */
  parsetree = (Query *)ReplaceVarsFromTargetList(
      (Node *)parsetree, parsetree->resultRelation, 0, view_rte,
      view_targetlist, REPLACEVARS_REPORT_ERROR, 0, &parsetree->hasSubLinks);
...
static List *RewriteQuery(Query *parsetree, List *rewrite_events) {
  ...
  foreach (lc1, parsetree->cteList) {
    ...
    if (ctequery->commandType == CMD_SELECT) continue;
    ...
    if (list_length(newstuff) == 1) {
      ...
      ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                      errmsg(
                          "DO INSTEAD NOTHING rules are not supported for "
      ...
      foreach (lc2, newstuff) {
        ...
        if (q->querySource == QSRC_QUAL_INSTEAD_RULE)
          ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                          errmsg(
                              "conditional DO INSTEAD rules are not supported "
                              "for data-modifying statements in WITH")));
        if (q->querySource == QSRC_NON_INSTEAD_RULE)
          ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                          errmsg(
                              "DO ALSO rules are not supported for "
                              "data-modifying statements in WITH")));
      }

      ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                      errmsg(
                          "multi-statement DO INSTEAD rules are not supported "
                          "for data-modifying statements in WITH")));
    }
  }
  ...
  if (event != CMD_SELECT && event != CMD_UTILITY) {
    ...
    if (event == CMD_INSERT) {
      ...
      if (list_length(parsetree->jointree->fromlist) == 1) {
        RangeTblRef *rtr =
            (RangeTblRef *)linitial(parsetree->jointree->fromlist);

        if (IsA(rtr, RangeTblRef)) {
          RangeTblEntry *rte = rt_fetch(rtr->rtindex, parsetree->rtable);

          if (rte->rtekind == RTE_VALUES) values_rte = rte;
        }
      }
      ...
      if (values_rte) {
        ...
        parsetree->targetList = rewriteTargetListIU(
            parsetree->targetList, parsetree->commandType, rt_entry_relation,
            parsetree->resultRelation, &attrnos);
      ...
      if (parsetree->onConflict &&
        ...
        parsetree->onConflict->onConflictSet = rewriteTargetListIU(
            parsetree->onConflict->onConflictSet, CMD_UPDATE, rt_entry_relation,
            parsetree->resultRelation, NULL);
      }
     ...
     */
    locks = matchLocks(event, rt_entry_relation->rd_rules, result_relation,
                       parsetree, &hasUpdate);
     ...
     */
    if ((instead || qual_product != NULL) && parsetree->returningList &&
        !returning) {
      switch (event) {
        case CMD_INSERT:
          ereport(ERROR,
    ...
    if (parsetree->onConflict && (product_queries != NIL || hasUpdate) &&
      ...
      ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                      errmsg(
                          "INSERT with ON CONFLICT clause cannot be used with "
                          "table that has INSERT or UPDATE rules")));
  ...
  if (parsetree->cteList != NIL) {
    ...
    foreach (lc1, rewritten) {
      ...
      if (q->commandType != CMD_UTILITY) qcount++;
    }
    if (qcount > 1)
      ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                      errmsg(
                          "WITH cannot be used in a query that is rewritten by "
                          "rules into multiple queries")));
  }
...
List *QueryRewrite(Query *parsetree) {
  ...
  if (!foundOriginalQuery && lastInstead != NULL) lastInstead->canSetTag = true;
