...
bool IsDefinedRewriteRule(Oid owningRel, const char *ruleName) {
  return SearchSysCacheExists2(RULERELNAME, ObjectIdGetDatum(owningRel),
...
Oid get_rewrite_oid(Oid relid, const char *rulename, bool missing_ok) {
  ...
  tuple = SearchSysCache2(RULERELNAME, ObjectIdGetDatum(relid),
                          PointerGetDatum(rulename));
  if (!HeapTupleIsValid(tuple)) {
 ...
 */
Oid get_rewrite_oid_without_relid(const char *rulename, Oid *reloid,
                                  bool missing_ok) {
  ...
  ScanKeyInit(&scanKeyData, Anum_pg_rewrite_rulename, BTEqualStrategyNumber,
              F_NAMEEQ, CStringGetDatum(rulename));
  ...
  if (!HeapTupleIsValid(htup)) {
    ...
    if (reloid != NULL) *reloid = ((Form_pg_rewrite)GETSTRUCT(htup))->ev_class;
