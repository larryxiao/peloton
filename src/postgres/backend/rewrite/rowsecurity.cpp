...
static void process_policies(Query *root, List *policies, int rt_index,
                             Expr **final_qual, Expr **final_with_check_qual,
                             bool *hassublinks, BoolExprType boolop);
 ...
 */
void get_row_security_policies(Query *root, CmdType commandType,
  ...
  *hasSubLinks = false;
   ...
   */
  if (rte->relid < FirstNormalObjectId || rte->relkind != RELKIND_RELATION ||
  ...
  rowsec_policies = pull_row_security_policies(commandType, rel, user_id);
   ...
   */
  if (((RowSecurityPolicy *)linitial(rowsec_policies))->policy_id == InvalidOid)
    defaultDeny = true;
  ...
  if (row_security_policy_hook_restrictive) {
    hook_policies_restrictive =
        (*row_security_policy_hook_restrictive)(commandType, rel);

    /* Build the expression from any policies returned. */
    if (hook_policies_restrictive != NIL)
      process_policies(
          root, hook_policies_restrictive, rt_index, &hook_expr_restrictive,
          &hook_with_check_expr_restrictive, hasSubLinks, AND_EXPR);
  }

  if (row_security_policy_hook_permissive) {
    hook_policies_permissive =
        (*row_security_policy_hook_permissive)(commandType, rel);

    /* Build the expression from any policies returned. */
    if (hook_policies_permissive != NIL)
      process_policies(root, hook_policies_permissive, rt_index,
                       &hook_expr_permissive, &hook_with_check_expr_permissive,
                       hasSubLinks, OR_EXPR);
  }
  ...
  if (commandType == CMD_INSERT || commandType == CMD_UPDATE) {
    ...
    /* Handle the case where there are both. */
      ...
      combined_quals =
          lcons(copyObject(rowsec_with_check_expr), combined_quals);

      combined_quals =
          lcons(copyObject(hook_with_check_expr_permissive), combined_quals);
    ...
    if (root->onConflict && root->onConflict->action == ONCONFLICT_UPDATE &&
      ...
      get_row_security_policies(root, CMD_UPDATE, rte, rt_index,
                                ...
                                &conflictHasRowSecurity, &conflictHasSublinks);

      if (conflictHasRowSecurity) *hasRowSecurity = true;
      if (conflictHasSublinks) *hasSubLinks = true;

      /*
       * Append WITH CHECK OPTIONs/RLS checks, which should not conflict
       * between this INSERT and the auxiliary UPDATE
       */
      *withCheckOptions = list_concat(*withCheckOptions, conflictWCOs);
...
static List *pull_row_security_policies(CmdType cmd, Relation relation,
  ...
  foreach (item, relation->rd_rsdesc->policies) {
    ...
    if (policy->polcmd == '*' && check_role_for_policy(policy->roles, user_id))
  ...
  if (policies == NIL) {
    ...
    policy =
        static_cast<RowSecurityPolicy *>(palloc0(sizeof(RowSecurityPolicy)));
...
static void process_policies(Query *root, List *policies, int rt_index,
  ...
  foreach (item, policies) {
    ...
    if (policy->qual != NULL) quals = lcons(copyObject(policy->qual), quals);

    if (policy->with_check_qual != NULL && rt_index == root->resultRelation)
      with_check_quals =
          lcons(copyObject(policy->with_check_qual), with_check_quals);
    ...
    if (policy->qual != NULL && policy->with_check_qual == NULL &&
      ...
      with_check_quals = lcons(copyObject(policy->qual), with_check_quals);

    if (policy->hassublinks) *hassublinks = true;
  }
...
static bool check_role_for_policy(ArrayType *policy_roles, Oid user_id) {
  ...
  if (roles[0] == ACL_ID_PUBLIC) return true;

  for (i = 0; i < ARR_DIMS(policy_roles)[0]; i++) {
    if (has_privs_of_role(user_id, roles[i])) return true;
  }
