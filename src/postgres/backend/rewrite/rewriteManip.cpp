...
typedef struct { int sublevels_up; } contain_aggs_of_level_context;
...
typedef struct { int win_location; } locate_windowfunc_context;

static bool contain_aggs_of_level_walker(
...
bool contain_aggs_of_level(Node *node, int levelsup) {
  ...
  return query_or_expression_tree_walker(
      node, reinterpret_cast<query_or_expression_tree_walker_fptr>(
...
static bool locate_agg_of_level_walker(Node *node,
  ...
  return expression_tree_walker(
      node,
      reinterpret_cast<expression_tree_walker_fptr>(locate_agg_of_level_walker),
      (void *)context);
}
...
bool contain_windowfuncs(Node *node) {
   ...
   * it's a Query, we don't want to increment sublevels_up.
   */
  return query_or_expression_tree_walker(node, contain_windowfuncs_walker, NULL,
                                         0);
}
...
static bool locate_windowfunc_walker(Node *node,
  ...
  return expression_tree_walker(
      node,
      reinterpret_cast<expression_tree_walker_fptr>(locate_windowfunc_walker),
      (void *)context);
}
...
bool checkExprHasSubLink(Node *node) {
   ...
   */
  return query_or_expression_tree_walker(node, checkExprHasSubLink_walker, NULL,
...
static bool OffsetVarNodes_walker(Node *node, OffsetVarNodes_context *context) {
  ...
  if (IsA(node, CurrentOfExpr)) {
    ...
    if (context->sublevels_up == 0) cexpr->cvarno += context->offset;
    return false;
  }
  if (IsA(node, RangeTblRef)) {
    RangeTblRef *rtr = (RangeTblRef *)node;

    if (context->sublevels_up == 0) rtr->rtindex += context->offset;
  ...
  if (IsA(node, JoinExpr)) {
    ...
    if (j->rtindex && context->sublevels_up == 0) j->rtindex += context->offset;
  ...
  if (IsA(node, Query)) {
    ...
    result = query_tree_walker(
        (Query *)node,
        reinterpret_cast<query_tree_walker_fptr>(OffsetVarNodes_walker),
        (void *)context, 0);
    context->sublevels_up--;
    return result;
  }
  return expression_tree_walker(
      node,
      reinterpret_cast<expression_tree_walker_fptr>(OffsetVarNodes_walker),
      (void *)context);
}
...
void OffsetVarNodes(Node *node, int offset, int sublevels_up) {
  ...
  if (node && IsA(node, Query)) {
    ...
    if (sublevels_up == 0) {
      ...
      if (qry->resultRelation) qry->resultRelation += offset;
    ...
    query_tree_walker(
        qry, reinterpret_cast<query_tree_walker_fptr>(OffsetVarNodes_walker),
        (void *)&context, 0);
  } else
...
static bool ChangeVarNodes_walker(Node *node, ChangeVarNodes_context *context) {
  ...
  if (IsA(node, PlanRowMark)) {
    ...
    if (context->sublevels_up == 0) {
      if (rowmark->rti == context->rt_index) rowmark->rti = context->new_index;
  ...
  if (IsA(node, Query)) {
    ...
    result = query_tree_walker(
        (Query *)node,
        reinterpret_cast<query_tree_walker_fptr>(ChangeVarNodes_walker),
        (void *)context, 0);
    context->sublevels_up--;
    return result;
  }
  return expression_tree_walker(
      node,
      reinterpret_cast<expression_tree_walker_fptr>(ChangeVarNodes_walker),
      (void *)context);
}
...
void ChangeVarNodes(Node *node, int rt_index, int new_index, int sublevels_up) {
  ...
  if (node && IsA(node, Query)) {
    ...
    if (sublevels_up == 0) {
      ...
      if (qry->resultRelation == rt_index) qry->resultRelation = new_index;
      ...
      foreach (l, qry->rowMarks) {
        ...
        if (rc->rti == rt_index) rc->rti = new_index;
      }
    }
    query_tree_walker(
        qry, reinterpret_cast<query_tree_walker_fptr>(ChangeVarNodes_walker),
        (void *)&context, 0);
  } else
...
static bool IncrementVarSublevelsUp_walker(
    Node *node, IncrementVarSublevelsUp_context *context) {
  if (node == NULL) return false;
  if (IsA(node, Var)) {
    Var *var = (Var *)node;
  ...
  if (IsA(node, Query)) {
    ...
    result = query_tree_walker((Query *)node,
                               reinterpret_cast<query_tree_walker_fptr>(
 ...
 */
void IncrementVarSublevelsUp_rtable(List *rtable, int delta_sublevels_up,
                                    int min_sublevels_up) {
  IncrementVarSublevelsUp_context context;
  ...
  range_table_walker(rtable, reinterpret_cast<range_table_walker_fptr>(
                                 IncrementVarSublevelsUp_walker),
                     (void *)&context, QTW_EXAMINE_RTES);
}
...
static bool rangeTableEntry_used_walker(Node *node,
  ...
  if (IsA(node, Query)) {
    ...
    result = query_tree_walker(
        (Query *)node,
        reinterpret_cast<query_tree_walker_fptr>(rangeTableEntry_used_walker),
        (void *)context, 0);
    context->sublevels_up--;
    return result;
  }
...
Query *getInsertSelectQuery(Query *parsetree, Query ***subquery_ptr) {
  ...
  if (subquery_ptr) *subquery_ptr = NULL;
  ...
  if (list_length(selectquery->rtable) >= 2 &&
      ...
      strcmp(rt_fetch(PRS2_NEW_VARNO, selectquery->rtable)->eref->aliasname,
             "new") == 0) {
    if (subquery_ptr) *subquery_ptr = &(selectrte->subquery);
    return selectquery;
  }
...
void AddQual(Query *parsetree, Node *qual) {
  ...
  if (parsetree->setOperations != NULL) {
    ...
    ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                    errmsg(
                        "conditional UNION/INTERSECT/EXCEPT statements are not "
                        "implemented")));
  }
 ...
 */
Node *replace_rte_variables(Node *node, int target_varno, int sublevels_up,
  ...
  result = query_or_expression_tree_mutator(
      node, reinterpret_cast<query_or_expression_tree_mutator_fptr>(
  ...
  if (context.inserted_sublink) {
    if (result && IsA(result, Query))
      ...
      *outer_hasSubLinks = true;
      ...
      elog(ERROR,
           "replace_rte_variables inserted a SubLink, but has noplace to "
           "record it");
  }
...
Node *replace_rte_variables_mutator(Node *node,
  ...
  if (IsA(node, Var)) {
    ...
    newnode = query_tree_mutator((Query *)node,
                                 reinterpret_cast<query_tree_mutator_fptr>(
                                     replace_rte_variables_mutator),
                                 (void *)context, 0);
  ...
  return expression_tree_mutator(node,
                                 reinterpret_cast<expression_tree_mutator_fptr>(
                                     replace_rte_variables_mutator),
                                 (void *)context);
}
...
Node *map_variable_attnos(Node *node, int target_varno, int sublevels_up,
...
static Node *ReplaceVarsFromTargetList_callback(
    Var *var, replace_rte_variables_context *context) {
  ...
  if (var->varattno == InvalidAttrNumber) {
     ...
     */
    expandRTE(rcon->target_rte, var->varno, var->varlevelsup, var->location,
  ...
  if (tle == NULL || tle->resjunk) {
    ...
    switch (rcon->nomatch_option) {
      ...
      case REPLACEVARS_SUBSTITUTE_NULL:
        ...
        return coerce_to_domain(
            (Node *)makeNullConst(var->vartype, var->vartypmod, var->varcollid),
            InvalidOid, -1, var->vartype, COERCE_IMPLICIT_CAST, -1, false,
            false);
    }
    ...
    if (contains_multiexpr_param(newnode, NULL))
      ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                      errmsg(
                          "NEW variables in ON UPDATE rules cannot reference "
                          "columns that are part of a multiple assignment in "
                          "the subject UPDATE command")));

    return newnode;
  }
}

Node *ReplaceVarsFromTargetList(Node *node, int target_varno, int sublevels_up,
                                RangeTblEntry *target_rte, List *targetlist,
                                ReplaceVarsNoMatchOption nomatch_option,
                                int nomatch_varno, bool *outer_hasSubLinks) {
