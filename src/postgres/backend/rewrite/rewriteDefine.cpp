 ...
 *	  relation "pg_rewrite"
 */
static Oid InsertRule(char *rulname, int evtype, Oid eventrel_oid,
                      bool evinstead, Node *event_qual, List *action,
                      bool replace) {
   ...
   */
  oldtup = SearchSysCache2(RULERELNAME, ObjectIdGetDatum(eventrel_oid),
  ...
  if (HeapTupleIsValid(oldtup)) {
    ...
    tup = heap_modify_tuple(oldtup, RelationGetDescr(pg_rewrite_desc), values,
                            nulls, replaces);
  ...
  recordDependencyOn(&myself, &referenced, (evtype == CMD_SELECT)
                                               ? DEPENDENCY_INTERNAL
                                               : DEPENDENCY_AUTO);
...
ObjectAddress DefineRule(RuleStmt *stmt, const char *queryString) {
  ...
  /* ... and execute */
  return DefineQueryRewrite(stmt->rulename, relId, whereClause, stmt->event,
                            stmt->instead, stmt->replace, actions);
}
 ...
 */
ObjectAddress DefineQueryRewrite(char *rulename, Oid event_relid,
                                 Node *event_qual, CmdType event_type,
                                 bool is_instead, bool replace, List *action) {
  ...
  foreach (l, action) {
    ...
    if (query->resultRelation == 0) continue;
    /* Don't be fooled by INSERT/SELECT */
    if (query != getInsertSelectQuery(query, NULL)) continue;
  ...
  if (event_type == CMD_SELECT) {
    ...
    if (query->hasModifyingCTE)
      ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                      errmsg(
                          "rules on SELECT must not contain data-modifying "
                          "statements in WITH")));
    ...
    if (event_qual != NULL)
      ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                      errmsg(
                          "event qualifications are not implemented for rules "
                          "on SELECT")));
    ...
    if (event_relation->rd_rel->relkind != RELKIND_VIEW &&
      ...
      if (heap_getnext(scanDesc, ForwardScanDirection) != NULL)
        ereport(ERROR, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                        errmsg(
                            "could not convert table \"%s\" to a view because "
                            "it is not empty",
                            RelationGetRelationName(event_relation))));
      ...
      if (event_relation->rd_rel->relhastriggers)
        ereport(ERROR, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                        errmsg(
                            ...
                            RelationGetRelationName(event_relation)),
                        errhint(
                            "In particular, the table cannot be involved in "
    ...
    foreach (l, action) {
      ...
      if (!query->returningList) continue;
      if (haveReturning)
        ereport(ERROR,
                (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
...
static void checkRuleResultList(List *targetList, TupleDesc resultDesc,
  ...
  foreach (tllist, targetList) {
    ...
    if (requireColumnNameMatch && strcmp(tle->resname, attname) != 0)
      ereport(ERROR, (errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
                      errmsg(
                          "SELECT rule's target entry %d has different column "
                          "name from column \"%s\"",
                          i, attname),
    ...
    if (attr->atttypid != tletypid)
      ereport(
          ...
          (errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
           isSelect ? errmsg(
                          "SELECT rule's target entry %d has different type "
                          "from column \"%s\"",
                          i, attname)
                    : errmsg(
                          "RETURNING list's entry %d has different type from "
                          "column \"%s\"",
                          i, attname),
           isSelect
               ? errdetail(
                     "SELECT target entry has type %s, but column has type %s.",
                     format_type_be(tletypid), format_type_be(attr->atttypid))
               : errdetail(
                     "RETURNING list entry has type %s, but column has type "
                     "%s.",
    ...
    if (attr->atttypmod != tletypmod && attr->atttypmod != -1 &&
        tletypmod != -1)
      ereport(
          ERROR,
          (errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
           isSelect ? errmsg(
                          "SELECT rule's target entry %d has different size "
                          "from column \"%s\"",
                          i, attname)
                    : errmsg(
                          "RETURNING list's entry %d has different size from "
                          "column \"%s\"",
                          i, attname),
           isSelect
               ? errdetail(
...
static bool setRuleCheckAsUser_walker(Node *node, Oid *context) {
  ...
  return expression_tree_walker(
      node,
      reinterpret_cast<expression_tree_walker_fptr>(setRuleCheckAsUser_walker),
      (void *)context);
}
...
static void setRuleCheckAsUser_Query(Query *qry, Oid userid) {
  ...
  if (qry->hasSubLinks)
    query_tree_walker(qry, reinterpret_cast<query_tree_walker_fptr>(
                               setRuleCheckAsUser_walker),
                      (void *)&userid, QTW_IGNORE_RC_SUBQUERIES);
}
...
void EnableDisableRule(Relation rel, const char *rulename, char fires_when) {
   ...
   */
  pg_rewrite_desc = heap_open(RewriteRelationId, RowExclusiveLock);
  ruletup = SearchSysCacheCopy2(RULERELNAME, ObjectIdGetDatum(owningRel),
  ...
  InvokeObjectPostAlterHook(RewriteRelationId, HeapTupleGetOid(ruletup), 0);
 ...
 */
static void RangeVarCallbackForRenameRule(const RangeVar *rv, Oid relid,
 ...
 */
ObjectAddress RenameRewriteRule(RangeVar *relation, const char *oldName,
                                const char *newName) {
  Oid relid;
  Relation targetrel;
  Relation pg_rewrite_desc;
   ...
   */
  relid = RangeVarGetRelidExtended(relation, AccessExclusiveLock, false, false,
  ...
  ruletup = SearchSysCacheCopy2(RULERELNAME, ObjectIdGetDatum(relid),
