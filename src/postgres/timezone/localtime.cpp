...
int tzload(const char *name, char *canonname, struct state *sp, int doextend) {
  ...
  for (stored = 4; stored <= 8; stored *= 2) {
    ...
    for (i = 0; i < sp->typecnt; ++i) {
      ...
      if (ttisp->tt_abbrind < 0 || ttisp->tt_abbrind > sp->charcnt) return -1;
    }
    ...
    for (i = 0; i < sp->typecnt; ++i) {
      ...
      if (ttisstdcnt == 0)
        ...
        ttisp->tt_ttisstd = *p++;
        if (ttisp->tt_ttisstd != TRUE && ttisp->tt_ttisstd != FALSE) return -1;
      }
    }
    for (i = 0; i < sp->typecnt; ++i) {
      struct ttinfo *ttisp;
      ...
      if (ttisgmtcnt == 0)
        ...
        ttisp->tt_ttisgmt = *p++;
        if (ttisp->tt_ttisgmt != TRUE && ttisp->tt_ttisgmt != FALSE) return -1;
      }
    }
     ...
     */
    if (stored >= (int)sizeof(pg_time_t) && TYPE_INTEGRAL(pg_time_t)) break;
  }
  if (doextend && nread > 2 && u.buf[0] == '\n' && u.buf[nread - 1] == '\n' &&
    ...
    if (result == 0 && ts.typecnt == 2 &&
      ...
      for (i = 0; i < 2; ++i) ts.ttis[i].tt_abbrind += sp->charcnt;
      for (i = 0; i < ts.charcnt; ++i) sp->chars[sp->charcnt++] = ts.chars[i];
      i = 0;
      while (i < ts.timecnt && ts.ats[i] <= sp->ats[sp->timecnt - 1]) ++i;
  ...
  switch (rulep->r_type) {
    ...
    case MONTH_NTH_DAY_OF_WEEK:
      ...
      for (i = 1; i < rulep->r_week; ++i) {
        if (d + DAYSPERWEEK >= mon_lengths[leapyear][rulep->r_mon - 1]) break;
        d += DAYSPERWEEK;
      }
...
static void gmtload(struct state *sp) {
  if (tzload(gmt, NULL, sp, TRUE) != 0) (void)tzparse(gmt, sp, TRUE);
}
...
int pg_next_dst_boundary(const pg_time_t *timep, long int *before_gmtoff,
                         int *before_isdst, pg_time_t *boundary,
  ...
  if ((sp->goback && t < sp->ats[0]) ||
    ...
    ++tcycles;
    icycles = tcycles;
    if (tcycles - icycles >= 1 || icycles - tcycles >= 1) return -1;
    seconds = icycles;
    seconds *= YEARSPERREPEAT;
    seconds *= AVGSECSPERYEAR;
    if (t < sp->ats[0])
    ...
    result = pg_next_dst_boundary(&newt, before_gmtoff, before_isdst, boundary,
                                  after_gmtoff, after_isdst, tz);
    if (t < sp->ats[0])
      *boundary -= seconds;
    else
      *boundary += seconds;
...
bool pg_interpret_timezone_abbrev(const char *abbrev, const pg_time_t *timep,
  ...
  while (abbrind < sp->charcnt) {
    if (strcmp(abbrev, abbrs + abbrind) == 0) break;
    while (abbrs[abbrind] != '\0') abbrind++;
    abbrind++;
  }
  if (abbrind >= sp->charcnt) return FALSE; /* not there! */
...
bool pg_get_timezone_offset(const pg_tz *tz, long int *gmtoff) {
  ...
  for (i = 1; i < sp->typecnt; i++) {
    if (sp->ttis[i].tt_gmtoff != sp->ttis[0].tt_gmtoff) return false;
  }
...
bool pg_tz_acceptable(pg_tz *tz) {
  ...
  if (!tt || tt->tm_sec != 0) return false;
