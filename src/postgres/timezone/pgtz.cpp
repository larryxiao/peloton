...
int pg_open_tzfile(const char *name, char *canonname) {
  ...
  for (;;) {
    ...
    if (fullnamelen + 1 + fnamelen >= MAXPGPATH) return -1; /* not gonna fit */
...
static bool init_timezone_hashtable(void) {
  ...
  if (!timezone_cache) return false;
...
pg_tz *pg_tzset(const char *name) {
  ...
  if (strlen(name) > TZ_STRLEN_MAX) return NULL; /* not going to fit */

  if (!timezone_cache)
    if (!init_timezone_hashtable()) return NULL;
   ...
   */
  p = uppername;
  while (*name) *p++ = pg_toupper((unsigned char)*name++);
  *p = '\0';
...
pg_tz *pg_tzset_offset(long gmtoffset) {
  ...
  snprintf(offsetstr, sizeof(offsetstr), "%02ld", absoffset / SECSPERHOUR);
...
pg_tz *pg_tzenumerate_next(pg_tzenum *dir) {
  while (dir->depth >= 0) {
    ...
    snprintf(fullname, MAXPGPATH, "%s/%s", dir->dirname[dir->depth],
             direntry->d_name);
     ...
     * the cache
     */
    if (tzload(fullname + dir->baselen, dir->tz.TZname, &dir->tz.state, TRUE) !=
        0) {
