...
static void doabbr(char *abbr, const char *format, const char *letters,
...
static struct lookup const line_codes[] = {{"Rule", LC_RULE},
                                           {"Zone", LC_ZONE},
                                           {"Link", LC_LINK},
                                           {"Leap", LC_LEAP},
                                           {NULL, 0}};
...
static void error(const char *string) {
  ...
  if (rfilename != NULL)
    (void)fprintf(stderr, _(" (rule from \"%s\", line %d)"), rfilename,
                  rlinenum);
  (void)fprintf(stderr, "\n");
  ++errors;
}
...
int main(int argc, char *argv[]) {
  ...
  if (directory == NULL) directory = "data";
  if (yitcommand == NULL) yitcommand = "yearistype";
  ...
  for (i = optind; i < argc; ++i) infile(argv[i]);
  ...
  for (i = 0; i < nzones; i = j) {
     ...
     */
    for (j = i + 1; j < nzones && zones[j].z_name == NULL; ++j) continue;
    outzone(&zones[i], j - i);
  }
...
static void dolink(const char *fromfield, const char *tofield) {
   ...
   * us.
   */
  if (!itsdir(toname)) (void)remove(toname);
  if (link(fromname, toname) != 0) {
    int result;

    if (mkdirs(toname) != 0) exit(EXIT_FAILURE);
    ...
    if (result != 0 && access(fromname, F_OK) == 0 && !itsdir(fromname)) {
      ...
      if (result == 0) warning(_("hard link failed, symbolic link used"));
...
static void setboundaries(void) {
  ...
  min_time = -1;
  for (i = 0; i < TIME_T_BITS_IN_FILE - 1; ++i) min_time *= 2;
  max_time = -(min_time + 1);
}
...
static void associate(void) {
  ...
  if (nrules != 0) {
    (void)qsort((void *)rules, (size_t)nrules, (size_t)sizeof *rules, rcomp);
    for (i = 0; i < nrules - 1; ++i) {
      if (strcmp(rules[i].r_name, rules[i + 1].r_name) != 0) continue;
      if (strcmp(rules[i].r_filename, rules[i + 1].r_filename) == 0) continue;
      eat(rules[i].r_filename, rules[i].r_linenum);
      warning(_("same rule name in multiple files"));
      eat(rules[i + 1].r_filename, rules[i + 1].r_linenum);
      warning(_("same rule name in multiple files"));
      for (j = i + 2; j < nrules; ++j) {
        if (strcmp(rules[i].r_name, rules[j].r_name) != 0) break;
        if (strcmp(rules[i].r_filename, rules[j].r_filename) == 0) continue;
        if (strcmp(rules[i + 1].r_filename, rules[j].r_filename) == 0) continue;
        break;
      }
      i = j - 1;
    }
  ...
  for (base = 0; base < nrules; base = out) {
    ...
    for (i = 0; i < nzones; ++i) {
      zp = &zones[i];
      if (strcmp(zp->z_rule, rp->r_name) != 0) continue;
      zp->z_rules = rp;
  ...
  for (i = 0; i < nzones; ++i) {
    ...
    if (zp->z_nrules == 0) {
       ...
       * a bad thing.
       */
      if (strchr(zp->z_format, '%') != NULL) error(_("%s in ruleless zone"));
    }
  }
  if (errors) exit(EXIT_FAILURE);
}
...
static void infile(const char *name) {
  ...
  for (num = 1;; ++num) {
    eat(name, num);
    if (fgets(buf, (int)sizeof buf, fp) != buf) break;
    cp = strchr(buf, '\n');
    if (cp == NULL) {
    ...
    *cp = '\0';
    ...
    while (fields[nfields] != NULL) {
      ...
      if (strcmp(fields[nfields], "-") == 0) fields[nfields] = &nada;
      ++nfields;
    }
    if (nfields == 0) {
      /* nothing to do */
...
static long gethms(const char *string, const char *errstring, int signable) {
  ...
  if (sscanf(string, scheck(string, "%ld"), &hh) == 1)
    ...
    ss = 0;
  else if (sscanf(string, scheck(string, "%ld:%d:%d"), &hh, &mm, &ss) != 3) {
    error(errstring);
    return 0;
  }
...
static int inzone(char **fields, int nfields) {
  ...
  for (i = 0; i < nzones; ++i)
    if (zones[i].z_name != NULL &&
      ...
      buf = erealloc(buf, (int)(132 + strlen(fields[ZF_NAME]) +
                                strlen(zones[i].z_filename)));
      (void)sprintf(buf, _("duplicate zone name %s (file \"%s\", line %d)"),
...
static int inzsub(char **fields, int nfields, int iscont) {
  ...
  if (max_format_len < strlen(z.z_format)) max_format_len = strlen(z.z_format);
  ...
  if (hasuntil) {
    ...
    if (iscont && nzones > 0 && z.z_untiltime > min_time &&
        z.z_untiltime < max_time && zones[nzones - 1].z_untiltime > min_time &&
        zones[nzones - 1].z_untiltime < max_time &&
        zones[nzones - 1].z_untiltime >= z.z_untiltime) {
      error(
          _("Zone continuation line end time is not after end time of previous "
            "line"));
      return FALSE;
    }
  }
...
static void inleap(char **fields, int nfields) {
  ...
  cp = fields[LP_DAY];
  if (sscanf(cp, scheck(cp, "%d"), &day) != 1 || day <= 0 ||
...
static void convert(long val, char *buf) {
  ...
  for (i = 0, shift = 24; i < 4; ++i, shift -= 8) buf[i] = val >> shift;
}
...
static void convert64(zic_t val, char *buf) {
  ...
  for (i = 0, shift = 56; i < 8; ++i, shift -= 8) buf[i] = val >> shift;
}
...
static int is32(zic_t x) { return x == ((zic_t)((int32)x)); }
...
static void writezone(const char *name, const char *string) {
   ...
   */
  if (timecnt > 1)
    (void)qsort((void *)attypes, (size_t)timecnt, (size_t)sizeof *attypes,
                atcomp);
   ...
   */
  {
    ...
    toi = 0;
    fromi = 0;
    while (fromi < timecnt && attypes[fromi].at < min_time) ++fromi;
    if (isdsts[0] == 0)
  ...
  leapcnt32 = leapcnt;
  leapi32 = 0;
  while (timecnt32 > 0 && !is32(ats[timecnt32 - 1])) --timecnt32;
  while (timecnt32 > 0 && !is32(ats[timei32])) {
    --timecnt32;
    ++timei32;
  }
  while (leapcnt32 > 0 && !is32(trans[leapcnt32 - 1])) --leapcnt32;
  while (leapcnt32 > 0 && !is32(trans[leapi32])) {
    --leapcnt32;
    ++leapi32;
  }
  fullname =
      erealloc(fullname, (int)(strlen(directory) + 1 + strlen(name) + 1));
  ...
  if ((fp = fopen(fullname, "wb")) == NULL) {
    if (mkdirs(fullname) != 0) (void)exit(EXIT_FAILURE);
    if ((fp = fopen(fullname, "wb")) == NULL) {
      const char *e = strerror(errno);
  ...
  for (pass = 1; pass <= 2; ++pass) {
    ...
    for (i = 0; i < typecnt; ++i) writetype[i] = thistimecnt == timecnt;
    if (thistimecnt == 0) {
      /*
       * * No transition times fall in the current * (32- or 64-bit)
       * window.
       */
      if (typecnt != 0) writetype[typecnt - 1] = TRUE;
    } else {
      for (i = thistimei - 1; i < thistimelim; ++i)
        if (i >= 0) writetype[types[i]] = TRUE;

      /*
       * * For America/Godthab and Antarctica/Palmer
       */
      if (thistimei == 0) writetype[0] = TRUE;
    }
    thistypecnt = 0;
    for (i = 0; i < typecnt; ++i)
    ...
    for (i = 0; i < typecnt; ++i) {
      ...
      if (!writetype[i]) continue;
      if (indmap[abbrinds[i]] >= 0) continue;
...
#undef DO
    ...
    if (thischarcnt != 0)
      (void)fwrite((void *)thischars, (size_t)sizeof thischars[0],
    ...
    for (i = 0; i < typecnt; ++i)
      if (writetype[i]) (void)putc(ttisstds[i], fp);
    for (i = 0; i < typecnt; ++i)
      if (writetype[i]) (void)putc(ttisgmts[i], fp);
  }
  (void)fprintf(fp, "\n%s\n", string);
...
static void doabbr(char *abbr, const char *format, const char *letters,
  ...
  if (slashp == NULL) {
    ...
    if (slashp > format)
      (void)strncpy(abbr, format, (unsigned)(slashp - format));
  ...
  abbr[len + 2] = '\0';
  abbr[len + 1] = '>';
  for (; len > 0; --len) abbr[len] = abbr[len - 1];
  abbr[0] = '<';
}

static void updateminmax(int x) {
  if (min_year > x) min_year = x;
  if (max_year < x) max_year = x;
}
...
static int stringoffset(char *result, long offset) {
  ...
  if (minutes != 0 || seconds != 0) {
    ...
    if (seconds != 0) (void)sprintf(end(result), ":%02d", seconds);
  }
  return 0;
}

static int stringrule(char *result, const struct rule *rp, long dstoff,
                      long gmtoff) {
  long tod;
  ...
  if (rp->r_dycode == DC_DOM) {
    ...
    if (rp->r_dayofmonth == 29 && rp->r_month == TM_FEBRUARY) return -1;
    total = 0;
    for (month = 0; month < rp->r_month; ++month) total += len_months[0][month];
    ...
    if (rp->r_dycode == DC_DOWGEQ) {
      ...
      if ((week - 1) * DAYSPERWEEK + 1 != rp->r_dayofmonth) return -1;
    } else if (rp->r_dycode == DC_DOWLEQ) {
      if (rp->r_dayofmonth == len_months[1][rp->r_month])
        week = 5;
      else {
        week = 1 + rp->r_dayofmonth / DAYSPERWEEK;
        if (week * DAYSPERWEEK - 1 != rp->r_dayofmonth) return -1;
      }
    } else
  ...
  tod = rp->r_tod;
  if (rp->r_todisgmt) tod += gmtoff;
  if (rp->r_todisstd && rp->r_stdoff == 0) tod += dstoff;
  if (tod < 0) {
    result[0] = '\0';
    return -1;
  }
  if (tod != 2 * SECSPERMIN * MINSPERHOUR) {
    (void)strcat(result, "/");
    if (stringoffset(end(result), tod) != 0) return -1;
  }
  return 0;
}

static void stringzone(char *result, const struct zone *zpfirst,
                       int zonecount) {
  ...
  for (i = 0; i < zp->z_nrules; ++i) {
    rp = &zp->z_rules[i];
    if (rp->r_hiwasnum || rp->r_hiyear != INT_MAX) continue;
    if (rp->r_yrtype != NULL) continue;
    if (rp->r_stdoff == 0) {
      if (stdrp == NULL)
        stdrp = rp;
      else
  ...
  if (stdrp == NULL && dstrp == NULL) {
     ...
     * zone.
     */
    if (stdrp != NULL && stdrp->r_hiyear == 2037) return;
  }
  if (stdrp == NULL && (zp->z_nrules != 0 || zp->z_stdoff != 0)) return;
...
static void outzone(const struct zone *zpfirst, int zonecount) {
  ...
  for (i = 0; i < zonecount; ++i) {
    zp = &zpfirst[i];
    if (i < zonecount - 1) updateminmax(zp->z_untilrule.r_loyear);
    for (j = 0; j < zp->z_nrules; ++j) {
      rp = &zp->z_rules[j];
      if (rp->r_lowasnum) updateminmax(rp->r_loyear);
      if (rp->r_hiwasnum) updateminmax(rp->r_hiyear);
    }
  }
   ...
   */
  if (min_year > 1900) min_year = 1900;
  if (max_year < 2037) max_year = 2037;
  for (i = 0; i < zonecount; ++i) {
    /*
    ...
    if (useuntil && zp->z_untiltime <= min_time) continue;
    gmtoff = zp->z_gmtoff;
    eat(zp->z_filename, zp->z_linenum);
    *startbuf = '\0';
    ...
    if (zp->z_nrules == 0) {
      ...
      for (year = min_year; year <= max_year; ++year) {
        ...
        for (j = 0; j < zp->z_nrules; ++j) {
          ...
          if (rp->r_todo) rp->r_temp = rpytime(rp, year);
        }
        for (;;) {
          int k;
          zic_t jtime, ktime = 0;
          ...
          for (j = 0; j < zp->z_nrules; ++j) {
            ...
            if (!rp->r_todisstd) offset = oadd(offset, stdoff);
            jtime = rp->r_temp;
          ...
          rp = &zp->z_rules[k];
          rp->r_todo = FALSE;
          if (useuntil && ktime >= untiltime) break;
          stdoff = rp->r_stdoff;
          if (usestart && ktime == starttime) usestart = FALSE;
          if (usestart) {
            if (ktime < starttime) {
              startoff = oadd(zp->z_gmtoff, stdoff);
              doabbr(startbuf, zp->z_format, rp->r_abbrvar, rp->r_stdoff != 0,
                     FALSE);
              continue;
            }
          ...
          type = addtype(offset, ab, rp->r_stdoff != 0, rp->r_todisstd,
                         rp->r_todisgmt);
          addtt(ktime, type);
        }
      }
    if (usestart) {
      ...
      if (*startbuf == '\0')
        error(
            _("cannot determine time zone abbreviation to use just after until "
              "time"));
      else
        addtt(starttime, addtype(startoff, startbuf, startoff != zp->z_gmtoff,
                                 startttisstd, startttisgmt));
    }
    ...
    if (useuntil) {
      ...
      if (!startttisstd) starttime = tadd(starttime, -stdoff);
      if (!startttisgmt) starttime = tadd(starttime, -gmtoff);
    }
  }
...
static void addtt(const zic_t starttime, int type) {
  if (starttime <= min_time || (timecnt == 1 && attypes[0].at < min_time)) {
    ...
    if (abbrinds[type] != 0) (void)strcpy(chars, &chars[abbrinds[type]]);
    abbrinds[0] = 0;
...
static int addtype(long gmtoff, const char *abbr, int isdst, int ttisstd,
  ...
  for (j = 0; j < charcnt; ++j)
    if (strcmp(&chars[j], abbr) == 0) break;
  if (j == charcnt) newabbr(abbr);
  abbrinds[i] = j;
  ++typecnt;
  return i;
}
...
static int itsabbr(const char *abbr, const char *word) {
  if (lowerit(*abbr) != lowerit(*word)) return FALSE;
  ++word;
  while (*++abbr != '\0') do {
...
static const struct lookup *byword(const char *word,
  ...
  for (lp = table; lp->l_word != NULL; ++lp)
    if (ciequal(word, lp->l_word)) return lp;
...
static char **getfields(char *cp) {
  ...
  nsubs = 0;
  for (;;) {
    while (isascii((unsigned char)*cp) && isspace((unsigned char)*cp)) ++cp;
    if (*cp == '\0' || *cp == '#') break;
    array[nsubs++] = dp = cp;
    do {
      if ((*dp = *cp++) != '"')
    ...
    if (isascii(*cp) && isspace((unsigned char)*cp)) ++cp;
    *dp = '\0';
  }
  array[nsubs] = NULL;
  return array;
...
static zic_t tadd(const zic_t t1, long t2) {
  ...
  if (t1 == max_time && t2 > 0) return max_time;
  if (t1 == min_time && t2 < 0) return min_time;
  t = t1 + t2;
  if ((t2 > 0 && t <= t1) || (t2 < 0 && t >= t1)) {
    error(_("time overflow"));
    exit(EXIT_FAILURE);
...
static zic_t rpytime(const struct rule *rp, int wantedy) {
  ...
  if (wantedy == INT_MIN) return min_time;
  if (wantedy == INT_MAX) return max_time;
  dayoff = 0;
  m = TM_JANUARY;
  y = EPOCH_YEAR;
  while (wantedy != y) {
...
#define LDAYSPERWEEK ((long)DAYSPERWEEK)
    ...
    if (dayoff >= 0)
      ...
      if (wday < 0) wday += LDAYSPERWEEK;
    }
    while (wday != eitol(rp->r_wday))
      if (rp->r_dycode == DC_DOWGEQ) {
        dayoff = oadd(dayoff, (long)1);
        if (++wday >= LDAYSPERWEEK) wday = 0;
        ++i;
      } else {
        dayoff = oadd(dayoff, (long)-1);
        if (--wday < 0) wday = LDAYSPERWEEK - 1;
        --i;
      }
    if (i < 0 || i >= len_months[isleap(y)][m]) {
      if (noise)
        warning(
            _("rule goes past start/end of month--\
...
static void newabbr(const char *string) {
  ...
  if (strcmp(string, GRANDPARENTED) != 0) {
    ...
    cp = string;
    wp = NULL;
    while (isascii((unsigned char)*cp) && isalpha((unsigned char)*cp)) ++cp;
    if (cp - string == 0)
...
static int mkdirs(char *argname) {
  ...
  if (argname == NULL || *argname == '\0') return 0;
...
int link(const char *oldpath, const char *newpath) {
  if (!CopyFile(oldpath, newpath, FALSE)) return -1;
  return 0;
}
#endif
 ...
 */
int pg_open_tzfile(const char *name, char *canonname) { return -1; }
